...
TL;DR: 论文提出SemanticMoments方法，通过计算预训练语义模型特征的时序统计量（高阶矩）来改进视频语义运动检索，在SimMotion基准测试中优于现有方法。
...
Method: 提出SemanticMoments方法，这是一种无需训练的简单方法，通过计算预训练语义模型特征的时序统计量（特别是高阶矩）来捕捉语义运动信息。

Result: 在SimMotion基准测试（包含合成数据和人工标注的真实世界数据集）上，SemanticMoments一致优于现有的RGB、光流和文本监督方法。
...
TL;DR: 提出一个从新闻视频中自动检测和提取人名的框架，强调确定性、可审计性和透明数据溯源，与生成式多模态方法形成对比。


<details>
  <summary>Details</summary>
Motivation: 视频新闻内容增长迅速，需要透明可靠的方法提取屏幕信息。图形布局、排版惯例和平台特定设计模式的多样性使得手动索引不切实际。

Method: 提出一个可解释的模块化提取管道，在确定性和可审计条件下运行。引入一个精心策划的平衡标注帧语料库，捕捉当代新闻图形的多样性。

Result: 底层检测器达到95.8% mAP@0.5，管道实现平衡的精确率(79.9%)和召回率(74.4%)。生成式系统F1分数略高(84.18% vs 77.08%)但缺乏透明数据溯源。59%受访者表示在快节奏广播中难以阅读屏幕姓名。

Conclusion: 该工作为现代新闻媒体中的混合多模态信息提取建立了方法严谨且可解释的基线，在确定性可审计性和随机推理之间存在明确权衡。
...
TL;DR: 提出基于场景图的零样本条件机制，通过ASQL条件器生成软视觉指导，增强文本到图像合成的组合能力和语义保真度
...
Result: 该方法能够在保持文本-图像对齐的同时，支持轻量级、连贯且多样化的图像合成，增强了模型的组合能力。

Conclusion: 通过场景图框架和ASQL条件器，实现了对复杂文本提示的更准确解释和视觉表示，解决了现有模型在语义保真度和结构连贯性方面的挑战。
...
TL;DR: 该研究评估了使用计算机视觉预测步态中足部压力中心和时间点的可行性，通过轻量级CNN-RNN模型在足部触地前250毫秒内进行预测，准确率较高且能在普通设备上实时运行。


<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉已用于步态环境分类，但预测足部如何接触变化环境的能力尚未充分探索。本研究旨在评估在平地到上楼梯过渡中预测足部压力中心和时间点的可行性，这对辅助系统的预测性控制具有重要意义。

Method: 8名受试者在右小腿佩戴RGB-D相机和仪器化鞋垫，执行上楼梯任务。研究训练了一个CNN-RNN模型，在足部触地前250毫秒的预测窗口内连续预测足部压力中心和时间点。模型在消费级笔记本电脑或边缘计算设备上以60 FPS运行。

Result: 在150、100和50毫秒预测窗口下，足部压力中心的平均绝对误差分别为29.42mm、26.82mm和23.72mm；时间点的平均绝对误差分别为21.14ms、20.08ms和17.73ms。较快的脚趾摆动速度提高了压力中心预测精度，而更靠前的足部触地位置降低了其预测精度。模型轻量且实时性能良好。

Conclusion: 研究表明，使用轻量级模型从视觉数据预测足部压力中心和时间点是可行的，这对辅助系统的预测性控制具有重要应用价值，能够实现更自然的步态辅助。
...
TL;DR: VLM-UQBench：首个针对视觉语言模型模态特定和跨模态数据不确定性的基准，包含600个真实样本和可扩展的扰动流程，评估发现现有UQ方法存在模态特异性、与幻觉关联弱、难以检测细微模糊等问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的不确定性量化对于确保其安全可靠行为至关重要，但现有方法难以定位不确定性的来源（图像、文本或跨模态不匹配），缺乏细粒度的模态感知不确定性评估工具。

Method: 提出VLM-UQBench基准，包含从VizWiz数据集中提取的600个真实样本，分为干净、图像、文本和跨模态不确定性子集；构建可扩展的扰动流程（8种视觉、5种文本、3种跨模态扰动）；提出两个简单指标量化UQ分数对扰动的敏感度及其与幻觉的相关性。

Result: 评估发现：(1)现有UQ方法表现出强烈的模态特异性，且高度依赖底层VLM；(2)模态特定不确定性常与幻觉共存，但当前UQ分数仅提供弱且不一致的风险信号；(3)UQ方法在明显群体级模糊上可与思维链基线竞争，但难以检测扰动流程引入的细微实例级模糊。

Conclusion: 当前UQ实践与可靠VLM部署所需的细粒度、模态感知不确定性之间存在显著差距，VLM-UQBench为评估和改进VLM不确定性量化提供了重要工具。
...
TL;DR: IR-SIS：首个基于语言的迭代细化手术图像分割系统，支持自然语言描述和临床医生交互，在领域内和领域外数据上均取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有手术图像分割方法存在三个主要限制：1）局限于预定义类别；2）一次性预测缺乏自适应细化；3）缺乏临床医生交互机制。需要一种更灵活、可交互的系统来支持机器人辅助手术和术中引导。

Method: 提出IR-SIS系统：1）使用微调的SAM3进行初始分割；2）利用视觉语言模型检测器械并评估分割质量；3）采用智能体工作流自适应选择细化策略；4）支持通过自然语言反馈实现临床医生在环交互；5）从EndoVis2017和EndoVis2018构建多粒度语言标注数据集。

Result: 实验表明，该系统在领域内和领域外数据上均实现了最先进的性能。临床医生交互进一步提升了分割效果。这是首个具有自适应自细化能力的基于语言的手术分割框架。

Conclusion: IR-SIS建立了首个基于语言的手术分割框架，具备自适应自细化能力，支持自然语言交互，为机器人辅助手术提供了更灵活、可交互的分割解决方案。
...
TL;DR: 研究发现传统扩散变换器中基于调制的文本条件作用贡献有限，注意力机制本身已足够有效，但池化嵌入可作为引导信号实现可控的属性偏移，带来显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 探索扩散变换器中基于调制的文本条件作用是否必要，以及是否能提供性能优势。传统方法通常结合注意力层和池化嵌入调制机制，但近期方法已放弃调制机制。

Method: 分析传统调制机制的有效性，提出将池化嵌入重新定位为引导信号而非直接条件作用，实现训练免费、简单实现、运行时开销可忽略的可控属性偏移方法。

Result: 传统调制机制贡献有限，注意力机制已足够传播提示信息；但池化嵌入作为引导信号能显著提升性能，适用于多种扩散模型，在文本到图像/视频生成和图像编辑等任务中均有改进。

Conclusion: 调制式文本条件作用在传统用法中非必需，但重新定位为引导信号可提供显著优势，实现可控的属性优化，该方法通用性强且易于实施。
...
TL;DR: X-Mark是一种针对胸部X光图像的样本特异性清洁标签水印方法，用于保护医学影像数据集版权，解决了现有方法在动态高分辨率医学扫描中的局限性。


<details>
  <summary>Details</summary>
Motivation: 高质量医学影像数据集对深度学习模型训练至关重要，但未经授权使用会引发严重的版权和伦理问题。现有针对自然图像设计的数据集所有权验证方法在医学影像上存在挑战：静态水印模式在固定尺度图像中生成，难以适应动态高分辨率扫描，且医学图像视觉多样性有限、解剖结构细微，同时需要保持诊断质量。

Method: X-Mark使用条件U-Net在每个样本的显著区域生成独特扰动。设计了多组件训练目标，确保水印有效性、对动态缩放过程的鲁棒性，同时保持诊断质量和视觉可区分性。在训练目标中加入拉普拉斯正则化来惩罚高频扰动，实现水印尺度不变性。所有权验证在黑盒设置下进行，检测可疑模型中的特征行为。

Result: 在CheXpert数据集上的广泛实验验证了X-Mark的有效性，实现了100%的水印成功率（WSR），在独立模型场景中将误报概率降低了12%，同时展示了对潜在自适应攻击的抵抗能力。

Conclusion: X-Mark为医学影像数据集版权保护提供了一种有效的样本特异性清洁标签水印方法，解决了现有方法在医学影像领域的局限性，能够在保持诊断质量的同时实现鲁棒的所有权验证。
...
<details>
  <summary>Details</summary>
Motivation: 患者住院是伤口护理成本高昂的主要因素之一。许多伤口本不需要立即住院，但由于治疗延迟、患者不配合或现有共病条件等因素，伤口可能恶化最终导致住院。现有研究主要关注基于不同伤口类型的愈合轨迹，缺乏综合预测住院风险的方法。

Method: 提出深度多模态方法，通过迁移学习开发伤口评估解决方案。该方法能够从伤口图像预测伤口变量及其愈合轨迹，综合使用伤口变量和伤口图像来预测患者的住院风险。

Result: 开发了能够预测伤口变量和愈合轨迹的模型，有助于早期发现可能影响愈合过程的伤口并发症，同时减少临床医生诊断伤口所需的时间。

Conclusion: 提出的新型模型能够帮助早期检测伤口复杂性，预测患者住院风险，从而降低伤口护理成本并改善患者预后。
...
TL;DR: 该研究评估了13种深度学习模型在膀胱癌分类任务中的表现，发现ConvNext系列泛化能力有限，ViT模型校准效果更好，但没有一个模型能提供适用于所有情况的可行可解释模型。


<details>
  <summary>Details</summary>
Motivation: 虽然基于ViT和CNN的深度模型在自然数据集上表现出色，但在医学影像中，异常区域通常只占图像的一小部分，这些模型可能表现不佳。本研究旨在评估最新深度模型在膀胱癌分类任务中的性能。

Method: 研究采用三种评估方法：1) 使用13个模型（4个CNN和8个基于transformer的模型）进行标准分类；2) 校准分析检查模型是否对膀胱癌分类进行了良好校准；3) 使用GradCAM++评估模型在临床诊断中的可解释性。在公开的多中心膀胱癌数据集上进行了约300次实验。

Result: 实验结果显示：ConvNext系列在膀胱癌图像分类中表现出有限的泛化能力（约60%准确率）；ViT模型相比ConvNext和swin transformer系列显示出更好的校准效果；测试时间增强提高了模型的可解释性；没有模型能提供适用于所有情况的可行可解释模型。

Conclusion: ConvNext系列适用于分布内样本，而ViT及其变体更适合解释分布外样本。没有一个模型能提供"一刀切"的解决方案，需要根据具体应用场景选择合适的模型。
...
TL;DR: Kyrtos方法是一种用于技术文档中曲线图表自动识别与分析的技术，通过聚类方法识别曲线线段，提取行为特征，转换为属性图，并生成自然语言描述，最终可转换为随机Petri网图。


<details>
  <summary>Details</summary>
Motivation: 技术文档中积累了大量的图表信息，包含宝贵知识。对技术文档的全面理解依赖于对其多种模态（如图形、表格、图表、文本等）及其关联的准确分析。曲线图表是技术文档中常见的重要元素，需要自动识别和分析方法来提取其中的结构化信息。

Method: 采用两阶段方法：1) 识别处理部分使用基于聚类的方法识别界定曲线线段的中点；2) 分析处理部分解析提取的曲线线段以捕获方向、趋势等行为特征。这些关联帮助将识别的线段关系转换为属性图，保留曲线的结构特征。图关系还表示为自然语言文本句子，便于转换为随机Petri网图。

Result: 广泛的评估结果表明，Kyrtos的识别和分析方法在测量输入图表曲线与Kyrtos生成的近似曲线之间的结构相似性方面具有高准确性，特别是在包含多个函数的图表中表现良好。

Conclusion: Kyrtos方法为技术文档中曲线图表的自动识别和分析提供了一种有效解决方案，能够提取结构化信息并转换为多种表示形式，有助于技术文档的深度理解和知识提取。
...
Method: 提出完全可微分双向协同学习（DBiSL）框架，无缝整合并增强四个关键SSL组件：监督学习、一致性正则化、伪监督学习和不确定性估计，实现分割与回归任务间的在线双向交互

Result: 在两个基准数据集上实现了最先进的性能，为统一SSL框架设计提供了新见解，建立了双任务驱动SSL的新架构基础
...
TL;DR: 该研究评估了五种最先进的图像到3D模型在医学单切片3D重建中的零样本性能，发现所有模型在医学数据上的体素重叠度中等，存在深度重建失败模式，SAM3D在拓扑相似性上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 医学3D理解对诊断和治疗规划至关重要，但容积成像成本高且等待时间长。图像到3D基础模型可以从2D模态重建3D数据，但现有模型主要在自然图像上训练，其几何先验是否适用于医学数据尚不明确。

Method: 对五种最先进的图像到3D模型（SAM3D、Hunyuan3D-2.1、Direct3D、Hi3DGen、TripoSG）进行受控零样本基准测试，在六个医学数据集和两个自然数据集上评估，使用体素重叠度和点云距离指标。

Result: 所有模型在医学数据集上的体素重叠度中等，表明从单切片推断体积时存在深度重建失败模式。全局距离指标显示方法间差异更大：SAM3D在医学3D数据的拓扑相似性上表现最强，其他模型更容易过度简化重建。

Conclusion: 研究量化了单切片医学重建的局限性，突出了2D医学数据平面性导致的深度模糊问题，需要多视角图像到3D重建来实现可靠的医学3D推断。
...
TL;DR: K-Sort Eval：一个基于视觉语言模型的高效可靠评估框架，通过后验校正和动态匹配解决VLM评估中的幻觉偏见问题，仅需少于90次模型运行即可获得与人类投票一致的结果。


<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的快速发展需要更可扩展和人类对齐的评估方法。现有众包Arena平台成本高、耗时长，限制了可扩展性。使用视觉语言模型替代人工评估虽有前景，但存在幻觉和偏见问题，且静态评估方法效率低下。

Method: 提出K-Sort Eval框架，包含两个核心技术：1）后验校正方法，基于VLM预测与人类监督的一致性自适应校正贝叶斯更新中的后验概率；2）动态匹配策略，平衡不确定性和多样性以最大化每次比较的预期收益。框架使用从K-Sort Arena数千个人类投票中构建的高质量数据集，对新模型进行(K+1)方自由竞争比较。

Result: 大量实验表明，K-Sort Eval能够提供与K-Sort Arena一致的评估结果，通常只需要少于90次模型运行，证明了其高效性和可靠性。

Conclusion: K-Sort Eval是一个可靠且高效的基于VLM的评估框架，通过后验校正和动态匹配解决了VLM评估中的对齐问题和效率问题，为视觉生成模型评估提供了可扩展的解决方案。
...
TL;DR: LARV是一种训练免费、数据免费、合并器无关的层自适应重缩放技术，通过为每个任务向量分配逐层缩放因子来提升现有模型合并方法的性能，特别针对视觉Transformer中浅层干扰敏感、深层任务特征稳定的层间异质性。
...
Method: LARV是一种训练免费、数据免费、合并器无关的层自适应重缩放技术，可插入任何任务向量合并器。它通过简单的确定性调度为每个任务向量分配逐层缩放因子，自适应抑制浅层干扰并放大深层对齐，无需重新训练或修改现有合并器。

Result: 在FusionBench上的Vision Transformers实验中，LARV持续改进所有任务向量基线方法（8/14/20任务设置）。例如，Iso-C + LARV在ViT-B/32上达到85.9%，ViT-B/16上89.2%，ViT-L/14上92.6%。层间分析和损坏测试表明LARV有效抑制浅层干扰，适度放大深层稳定特征。

Conclusion: LARV首次实现了任务向量合并的层感知缩放，将模型合并从均匀处理转变为稳健的层感知过程。该技术正交于基础合并器，增加成本可忽略，且分层选择提供最佳鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现代成像和可微渲染中出现的高维非线性逆问题需要理论分析框架，以理解参数估计的基本限制，特别是对于具有块结构参数的复杂模型，现有方法缺乏统一的算子级稳定性分析。

Method: 开发算子理论框架，结合块状Lipschitz几何、局部可识别性和次高斯噪声的统一假设，推导确定性稳定性不等式、全局Lipschitz界和非渐近浓度估计，并以高斯泼溅渲染算子为具体实例验证假设。

Result: 建立了与具体重建算法无关的高概率参数误差界，验证了高斯泼溅渲染算子满足假设并推导了显式常数，揭示了稳定性-分辨率权衡：估计误差受图像分辨率与模型复杂度比值的固有约束。

Conclusion: 该分析为现代成像和可微渲染中广泛的高维非线性逆问题刻画了算子级基本限制，提供了统一的稳定性与统计浓度理论框架，揭示了参数估计的固有约束条件。
...
TL;DR: Fine-T2I是一个大规模、高质量、完全开放的文本到图像微调数据集，包含600万文本-图像对，覆盖10种任务组合、32种提示类别、11种视觉风格，通过严格筛选确保质量。
...
Method: 结合现代强模型生成的合成图像与专业摄影师精心策划的真实图像，覆盖多种任务组合、提示类别和视觉风格，通过严格筛选去除95%以上的初始候选样本，确保文本图像对齐、视觉保真度和提示质量。
...
TL;DR: ArtifactLens系统利用预训练视觉语言模型的已有知识，通过少量标注样本解锁检测生成图像中伪影的能力，在多个基准测试中达到SOTA，且数据需求大幅减少。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成器产生逼真图像，但存在扭曲的手或变形物体等伪影。检测这些伪影对于基准测试和改进生成器至关重要。现有检测器需要大量标注数据进行微调，成本高昂且难以适应生成器演进或新伪影类型出现。

Method: ArtifactLens系统采用多组件架构，包括上下文学习和文本指令优化等创新改进。通过适当的"脚手架"结构，利用预训练VLM中已编码的知识，仅需每个伪影类别数百个标注样本即可解锁检测能力。

Result: 在五个人类伪影基准测试中（首次跨多个数据集的评估）达到最先进水平，同时所需标注数据量减少数个数量级。方法可泛化到其他伪影类型（物体形态、动物解剖、实体交互）以及AIGC检测任务。

Conclusion: 预训练视觉语言模型已编码检测伪影所需知识，通过适当的架构设计，仅需少量标注数据即可解锁这一能力，为高效检测生成图像伪影提供了新途径。
...
TL;DR: FD-DB提出了一种频率解耦的双分支模型，通过分离低频可解释编辑和高频残差补偿来解决合成到真实图像转换中真实感与结构稳定性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 合成数据为几何敏感视觉任务提供了低成本、精确标注的样本，但合成域与真实域之间的外观和成像差异导致严重的域偏移，降低了下游任务性能。现有的无配对合成到真实转换方法在真实感与结构稳定性之间存在权衡：无约束生成可能引入变形或虚假纹理，而过度刚性约束则限制了向真实域统计的适应。

Method: FD-DB采用频率解耦的双分支模型：1) 可解释分支预测物理意义明确的编辑参数（白平衡、曝光、对比度、饱和度、模糊和颗粒）来构建稳定的低频外观基础；2) 自由分支通过残差生成补充细节；3) 门控融合机制在显式频率约束下结合两个分支以限制低频漂移；4) 两阶段训练策略：先稳定编辑分支，再释放残差分支。

Result: 在YCB-V数据集上的实验表明，FD-DB提高了真实域外观一致性，显著提升了下游语义分割性能，同时保持了几何和语义结构。

Conclusion: FD-DB通过分离低频可解释编辑和高频残差补偿，有效解决了合成到真实图像转换中真实感与结构稳定性之间的权衡问题，为几何敏感视觉任务提供了更好的域适应解决方案。
...
TL;DR: FedSurg挑战赛旨在评估联邦学习在手术视频分类中的泛化能力和适应性，使用多中心阑尾炎视频数据集，发现当前方法在未见中心泛化有限，但本地微调能改善性能。


<details>
  <summary>Details</summary>
Motivation: 建立首个手术视频分类联邦学习基准，评估当前方法在未见临床中心的泛化能力，以及通过本地微调进行中心特定适应的效果，同时实现不共享患者数据的协作模型开发。

Method: 使用多中心Appendix300视频数据集，参与者开发策略对阑尾炎炎症阶段进行分类。评估两个任务：未见中心的泛化和微调后的中心特定适应。提交方法包括基础模型线性探测、三元组损失度量学习，以及各种联邦学习聚合方案（FedAvg、FedMedian、FedSAM）。使用F1分数和期望成本评估性能，通过自举法和统计检验评估排名稳健性。

Result: 在泛化任务中，跨中心性能有限；在适应任务中，所有团队在微调后都有改善，但排名稳定性较低。基于ViViT的提交获得最强整体性能。挑战赛揭示了泛化能力有限、对类别不平衡敏感、去中心化训练中超参数调优困难等问题，而时空建模和上下文感知预处理成为有前景的策略。

Conclusion: FedSurg挑战赛建立了首个评估手术视频分类联邦学习策略的基准。研究结果强调了本地个性化与全局鲁棒性之间的权衡，并突出了架构选择、预处理和损失设计的重要性。该基准为未来开发不平衡感知、自适应和鲁棒的临床手术AI联邦学习方法提供了参考点。
...
TL;DR: 提出了一种用于数字病理学弱监督多实例学习的特征表示学习框架WeakSupCon，该框架在训练中融入袋级标签信息，无需实例级伪标注，能有效分离不同标签的病理图像块特征。


<details>
  <summary>Details</summary>
Motivation: 数字病理学全切片图像分析面临训练标签有限的挑战，手动标注需要大量时间和精力。现有弱监督多实例学习方法主要关注特征聚合，而忽略了特征表示学习在编码器预训练中的重要性。

Method: 提出弱监督对比学习框架WeakSupCon，在训练过程中结合袋级标签信息，无需实例级伪标注，通过对比学习使不同标签的病理图像块在特征空间中有效分离。

Result: 实验结果表明，WeakSupCon方法生成的特征在三个数据集上的下游多实例学习任务中，相比自监督对比学习方法获得了更好的性能表现。

Conclusion: WeakSupCon框架为数字病理学弱监督多实例学习提供了一种有效的特征表示学习方法，通过利用袋级标签信息改善特征表示，提升了下游任务的性能。
...
TL;DR: Align-TI是一个新颖的知识蒸馏框架，通过关注令牌交互来压缩多模态大语言模型，相比传统方法在性能上有显著提升
...
Method: Align-TI从令牌交互角度设计，包含两个核心组件：IVA（指令-视觉对齐）让学生模型模仿教师模型提取相关视觉信息的能力；TPA（令牌概率对齐）通过对齐序列令牌到令牌的转移概率来捕捉教师的动态生成逻辑。

Result: 实验表明Align-TI相比传统知识蒸馏方法有2.6%的相对提升，蒸馏后的Align-TI-2B甚至超过了更大的LLaVA-1.5-7B模型7.0%，为训练参数高效的多模态大语言模型建立了新的最先进蒸馏框架。

Conclusion: Align-TI通过关注令牌交互的知识蒸馏方法，有效压缩了多模态大语言模型，在保持性能的同时显著减小了模型规模，为实际部署提供了可行方案。
...
TL;DR: 本文提出OSI（一步反演）方法，用于快速提取高斯着色风格水印，相比传统多步扩散反演方法快20倍，准确率更高，且水印容量翻倍。


<details>
  <summary>Details</summary>
Motivation: 当前基于高斯着色的训练免费水印方法需要多步扩散反演来获取精确初始噪声，计算成本高且耗时。需要一种更快速、更准确的水印提取方法。

Method: 提出OSI方法，将水印提取重新定义为可学习的符号分类问题，避免初始噪声的精确回归。从扩散主干初始化OSI模型，在合成的噪声-图像对上使用符号分类目标进行微调，实现一步完成水印提取。

Result: OSI方法比多步扩散反演快20倍，提取准确率更高，水印有效载荷容量翻倍。在不同调度器、扩散主干和密码方案上的广泛实验均显示改进，证明了OSI框架的通用性。

Conclusion: OSI为高斯着色风格水印提供了一种高效、准确的提取方法，显著提升了水印提取的速度和性能，具有广泛的适用性。
...
TL;DR: 本文提出了一种名为平衡对比学习（ECL）的新框架，用于解决不平衡数据集中的对比学习问题。ECL通过促进表示几何平衡和分类器-类中心几何平衡，提升了不平衡分类的性能。


<details>
  <summary>Details</summary>
Motivation: 现有监督对比学习方法存在两个主要问题：1）未考虑类均值/原型与分类器之间的对齐，可能导致泛化能力差；2）原型仅被视为每个类的一个额外样本，其影响力取决于批次中类实例的数量，导致不同类之间的贡献不平衡。

Method: ECL框架包含两个主要组件：1）促进表示几何平衡，即实现类样本坍缩和均匀分布的类均值的正则单纯形几何，同时平衡类平均特征和类原型的贡献；2）建立分类器-类中心几何平衡，通过对齐分类器权重和类原型。

Result: 在三个长尾数据集（CIFAR-10(0)-LT、ImageNet-LT）和两个不平衡医学数据集（ISIC 2019和自建的LCCT数据集）上进行实验，结果显示ECL优于现有的为不平衡分类设计的SOTA监督对比学习方法。

Conclusion: ECL通过促进几何平衡，有效解决了不平衡数据集中的对比学习问题，提升了分类性能，特别是在医学图像等实际不平衡数据集上表现出色。
...
TL;DR: AdaDS是一个通用的深度图超分辨率框架，通过自适应选择扩散反演起始时间步和注入定制噪声，从任意退化的低分辨率输入中鲁棒恢复高分辨率深度图。


<details>
  <summary>Details</summary>
Motivation: 传统方法直接回归深度值，在严重或未知退化条件下容易产生伪影。需要一种能够鲁棒处理各种退化模式的深度超分辨率方法。

Method: 利用高斯平滑的收缩特性：随着前向过程中噪声累积，退化输入与高质量对应物之间的分布差异减小，最终收敛到各向同性高斯先验。AdaDS基于估计的细化不确定性自适应选择反向扩散轨迹的起始时间步，并注入定制噪声将中间样本定位到目标后验分布的高概率区域。
...
<details>
  <summary>Details</summary>
Motivation: 动物和人类行为分析在计算机视觉中一直具有挑战性。早期方法依赖手工特征提取，鲁棒性和泛化能力有限。ImageNet的出现使大规模视觉识别成为可能，作者希望借鉴这一范式，构建一个通用动作空间来解决行为分析问题。

Method: 利用现有标注的人类动作数据集构建大规模通用动作空间（UAS），然后将该UAS作为基础，用于分析和分类哺乳动物和黑猩猩的行为数据集。

Result: 成功构建了通用动作空间（UAS），并将其应用于哺乳动物和黑猩猩行为数据集的分析和分类。源代码已在GitHub上开源。

Conclusion: 通过构建通用动作空间，能够有效分析和分类动物行为，为行为分析研究提供了新的范式。该方法将人类动作识别的成功经验迁移到动物行为分析领域。
...
Method: 提出无需训练的注意力干预算法：1) 基于任务相关标记通常具有高视觉-文本相似度的假设，提取视觉-文本交叉注意力子矩阵构建重加权矩阵来重新分配注意力；2) 在beam搜索解码中注入视觉注意力值，以识别具有更高视觉注意力的解决方案

Result: 广泛实验表明，该方法显著减少了主流大视觉语言模型的幻觉问题，同时保持了生成内容的准确性和连贯性

Conclusion: 提出的注意力干预算法有效解决了大视觉语言模型的幻觉问题，通过选择性增强任务相关标记的注意力，在减少幻觉的同时保持了模型性能
...
Result: 生成的Singpath-VL模型在细粒度形态感知和细胞级诊断分类方面表现出优越性能，并将开源部分合成数据集和基准。
...
TL;DR: HLGFA是一种无监督工业异常检测框架，通过建模高分辨率和低分辨率特征之间的一致性来学习正常模式，无需像素级重建，在MVTec AD数据集上达到97.9%像素级AUROC和97.5%图像级AUROC。


<details>
  <summary>Details</summary>
Motivation: 工业制造检测中缺陷样本稀缺，需要可靠的异常检测方法。现有方法依赖像素级重建，而本文提出通过跨分辨率特征一致性建模来学习正常性。

Method: 提出高-低分辨率引导特征对齐框架(HLGFA)：1)使用共享冻结主干处理双分辨率输入提取多级特征；2)将高分辨率表示分解为结构和细节先验，通过条件调制和门控残差校正指导低分辨率特征细化；3)引入噪声感知数据增强策略抑制工业环境中的干扰响应。

Result: 在标准基准测试中，HLGFA在MVTec AD数据集上达到97.9%像素级AUROC和97.5%图像级AUROC，优于代表性的基于重建和基于特征的方法。

Conclusion: HLGFA通过跨分辨率特征一致性建模有效学习正常模式，无需像素级重建，在工业异常检测中表现出色，为无监督异常检测提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医疗等高风险领域应用受限，主要问题是幻觉现象——生成的文本与视觉输入矛盾或忽略视觉信息。模型能理解图像但难以生成准确的token序列，微小扰动就会导致注意力从真实状态转向虚假状态，且自回归文本生成特性阻碍错误修正。
...
Conclusion: SchröMind框架通过求解薛定谔桥问题有效减少了多模态大语言模型的幻觉问题，为高风险领域的应用提供了可行解决方案。
...
TL;DR: SCA-Net：基于Change-Agent框架的增强架构，用于双时相影像中的建筑物和道路变化检测，通过多尺度差异金字塔、自适应处理模块和注意力机制提升小目标检测精度，显著减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 遥感影像中的自动变化检测对城市管理、环境监测和灾害评估至关重要。现有深度学习模型面临小目标敏感性低、计算成本高等挑战，需要更高效准确的解决方案。
...
Result: 在LEVIR-CD和LEVIR-MCI数据集上评估显示：1）在LEVIR-MCI上mIoU提升2.64%；2）小建筑物IoU显著提高57.9%；3）训练时间减少61%；4）性能优于Change-Agent和其他最先进方法。
...
TL;DR: DR.Experts是一个新颖的先验驱动的盲图像质量评估框架，通过显式整合失真先验来提升质量评估的准确性，解决了现有方法对细微失真线索不敏感的问题。
...
Method: 提出DR.Experts框架：1）利用退化感知的视觉语言模型获取失真特定先验；2）通过失真显著性差分模块区分失真与语义注意力，精炼先验表示；3）使用动态失真加权模块（专家混合风格）融合精炼先验、语义和桥接表示，根据感知影响加权各失真特定特征。

Result: 在五个具有挑战性的BIQA基准测试上进行广泛实验，证明DR.Experts优于当前方法，在泛化能力和数据效率方面表现出色。

Conclusion: DR.Experts通过显式整合失真先验，有效解决了现有BIQA方法对细微失真不敏感的问题，实现了与人类感知更一致的质量评估，在多个基准测试中表现出优越性能。
...
TL;DR: AUHead是一个两阶段方法，通过解耦精细情感控制（动作单元AUs）实现可控的说话头部视频生成，第一阶段使用大音频语言模型生成AUs，第二阶段使用AU驱动的扩散模型合成视频。


<details>
  <summary>Details</summary>
Motivation: 当前说话头部视频生成方法在细微情感表达方面存在困难，缺乏精细的情感控制能力。为了解决这个问题，需要一种能够解耦精细情感控制（动作单元AUs）的方法。

Method: 采用两阶段方法：1）使用大音频语言模型通过时空AU标记化和"情感-然后-AU"思维链机制从原始语音中解耦AUs；2）提出AU驱动的可控扩散模型，将AU序列映射为结构化2D面部表示，在交叉注意力模块中建模AU-视觉交互，并在推理时引入AU解耦引导策略。

Result: 在基准数据集上的结果显示，该方法在情感真实性、准确的唇部同步和视觉连贯性方面取得了有竞争力的性能，显著超越了现有技术。

Conclusion: AUHead方法通过解耦精细情感控制实现了高质量、可控的说话头部视频生成，在情感表达和视觉质量方面表现出色，为虚拟化身、电影制作和交互系统提供了有效解决方案。
...
TL;DR: Scalpel是一种通过优化注意力激活分布来减少大型视觉语言模型幻觉的方法，它使用高斯混合模型和熵最优传输技术，在推理时动态调整注意力方向，无需额外计算。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型由于大型语言模型的强先验和跨模态注意力未对齐，经常产生与视觉内容不一致的输出（幻觉），需要有效的方法来解决这个问题。

Method: Scalpel在推理时预测Transformer层中每个头的可信注意力方向，使用高斯混合模型捕捉信任和幻觉流形中的注意力多峰分布，通过熵最优传输（等价于薛定谔桥问题）精确映射高斯分量，并根据分量隶属度和映射关系动态调整干预强度和方向。

Result: 在多个数据集和基准测试上的广泛实验表明，Scalpel有效缓解了幻觉问题，优于先前方法并达到最先进性能。该方法模型和数据无关，无需额外计算，仅需单次解码步骤。

Conclusion: Scalpel通过优化注意力激活分布，提供了一种有效减少大型视觉语言模型幻觉的解决方案，具有模型无关、数据无关、计算高效的特点。
...
TL;DR: 本文提出了一种基于神经正切核的多模态谱聚类方法，利用预训练视觉语言模型中的跨模态对齐，通过正名词锚定增强图像间相似性计算，在16个基准数据集上大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统谱聚类方法大多基于单模态信息，未能充分利用多模态表示中的丰富信息。受近期视觉语言预训练成功的启发，本文旨在将谱聚类从单模态扩展到多模态领域。

Method: 提出神经正切核谱聚类，利用预训练视觉语言模型中的跨模态对齐。通过使用与目标图像语义相近的正名词来锚定神经正切核，将图像间的亲和力公式化为视觉接近性和语义重叠的耦合。同时提出正则化亲和力扩散机制，自适应地集成不同提示词诱导的亲和力矩阵。

Result: 在16个基准数据集（包括经典、大规模、细粒度和领域偏移数据集）上的广泛实验表明，该方法在多个指标上一致且大幅超越现有最先进方法。

Conclusion: 该方法成功地将谱聚类扩展到多模态领域，通过利用视觉语言预训练模型中的跨模态对齐，显著提升了聚类性能，证明了多模态信息在谱聚类中的重要性。
...
TL;DR: MieDB-100k是一个大规模、高质量、多样化的医学图像编辑数据集，解决了现有数据集多样性不足、忽视医学图像理解以及难以平衡质量与规模的问题。


<details>
  <summary>Details</summary>
Motivation: 高质量数据的稀缺是多模态生成模型在医学图像编辑应用中的主要瓶颈。现有医学图像编辑数据集存在多样性有限、忽视医学图像理解能力、难以平衡质量与规模等问题。

Method: 提出MieDB-100k数据集，将编辑任务分为感知、修改和转换三个视角。通过结合模态特定专家模型和基于规则的数据合成方法构建数据管道，并进行严格的人工检查以确保临床保真度。

Result: 使用MieDB-100k训练出的模型在实验中持续优于开源和专有模型，并展现出强大的泛化能力。

Conclusion: MieDB-100k数据集有望成为专业医学图像编辑领域未来发展的基石。
...
TL;DR: Hand2World：基于单张场景图像和自由空间手势的自我中心交互生成框架，通过遮挡不变的手部条件、相机几何注入和因果生成器实现高质量、长时序的视频合成。


<details>
  <summary>Details</summary>
Motivation: 增强现实和具身AI需要能够以低延迟、几何一致性和长期稳定性响应用户输入的自我中心交互世界模型。当前面临的主要挑战包括：自由空间手势与接触密集训练数据之间的分布偏移、单目视图中手部运动与相机运动的模糊性，以及需要任意长度视频生成。
...
Result: 在三个自我中心交互基准测试中，Hand2World在感知质量和3D一致性方面取得显著改进，同时支持相机控制和长时序交互生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散视频生成方法多为任务特定且主要依赖文本指令，难以处理多模态输入、上下文参考以及多样化的视频生成和编辑场景。许多视频编辑方法需要针对单个操作精心设计的流程，限制了可扩展性和组合性。

Method: 提出Tele-Omni统一框架，利用预训练多模态大语言模型解析异构指令并推断结构化生成/编辑意图，扩散生成器基于这些结构化信号进行高质量视频合成。引入任务感知数据处理流程，将多模态输入统一为结构化指令格式同时保留任务特定约束。

Result: Tele-Omni支持广泛的视频中心任务，包括文本到视频生成、图像到视频生成、首尾帧视频生成、上下文视频生成和上下文视频编辑。实验结果表明，Tele-Omni在多个任务上实现了有竞争力的性能。

Conclusion: 通过解耦指令解析与视频合成并结合任务感知数据设计，Tele-Omni实现了灵活的多模态控制，同时保持了强时间连贯性和视觉一致性，为统一视频生成和编辑提供了有效框架。
...
Method: AGMark在每个解码步骤中：1）基于注意力权重动态识别视觉相关的语义关键证据，结合上下文感知的连贯性线索；2）联合考虑不确定性感知（标记熵）和证据校准（权重密度）确定语义关键标记比例，实现自适应词汇分区。

Result: AGMark在生成质量上优于传统方法，特别是在生成后期显著提升视觉语义保真度。保持高竞争性检测准确率（至少99.36% AUC）和强大的攻击恢复能力（至少88.61% AUC），且不牺牲推理效率。

Conclusion: AGMark为可靠性保持的多模态水印建立了新标准，通过动态证据校准和自适应词汇分区，有效解决了现有水印方法的局限性，在保护知识产权的同时严格保持视觉保真度。
...
TL;DR: LELA：首个无需训练的大语言模型框架，用于检测和定位视频中的仇恨内容，通过多模态分解和多阶段提示实现细粒度时间定位。
...
Conclusion: LELA为可扩展和可解释的仇恨视频定位建立了坚实基础，是首个无需训练的大语言模型框架，在检测和定位视频仇恨内容方面表现出色。
...
TL;DR: 该论文提出了VIDA数据集和VideoAfford模型，通过视频数据增强3D可操作性理解，显著提升了机器人操作能力


<details>
  <summary>Details</summary>
Motivation: 现有3D可操作性研究主要依赖静态语言和图像线索，缺乏动态交互上下文，难以提供时序和因果线索，限制了机器人对物体可操作区域的理解

Method: 1) 构建VIDA数据集：包含38K人-物交互视频，覆盖16种可操作性类型、38个物体类别和22K点云；2) 提出VideoAfford模型：激活多模态大语言模型的可操作性分割能力，统一世界知识推理和细粒度可操作性定位；3) 使用潜在动作编码器从HOI视频提取动态交互先验；4) 引入空间感知损失函数获取全面的3D空间知识

Result: 实验评估显示模型显著优于现有方法，表现出强大的开放世界泛化能力和可操作性推理能力

Conclusion: 通过视频数据增强3D可操作性理解是有效的，VideoAfford模型在统一框架内实现了世界知识推理和细粒度可操作性定位，为机器人操作提供了更好的基础
...
TL;DR: Time2General是一个域泛化视频语义分割框架，通过时空记忆解码器和掩码时间一致性损失来解决域偏移和时间采样偏移问题，提高跨域准确性和时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 在域泛化视频语义分割中，域偏移和时间采样偏移会破坏基于对应关系的传播和固定步长时间聚合，导致视频流中即使标签稳定区域也出现严重的帧间闪烁问题。

Method: 提出Time2General框架：1) 时空记忆解码器将多帧上下文聚合到片段级时空记忆中，无需显式对应传播即可解码时间一致的逐帧掩码；2) 掩码时间一致性损失通过正则化不同步长的时间预测差异来抑制闪烁；3) 随机化训练步长使模型暴露于不同的时间间隔。

Result: 在多个驾驶基准测试上的广泛实验表明，Time2General在跨域准确性和时间稳定性方面显著优于先前的域泛化语义分割和视频语义分割基线，同时运行速度可达18 FPS。

Conclusion: Time2General通过稳定性查询和时空记忆解码有效解决了域泛化视频语义分割中的时间一致性问题，在保持实时性能的同时显著提升了跨域泛化能力和时间稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI自动化研究主要关注GUI grounding的扩展，而更关键的GUI规划需要更复杂的数据收集。GUI探索过程通常呈树状结构，早期功能入口点被更频繁探索，因此将大规模轨迹组织成树结构可以降低数据成本并简化GUI规划的数据扩展。

Method: 提出TreeCUA：1）多智能体协作框架探索环境、验证动作、总结轨迹和评估质量；2）设计树基拓扑存储和回放重复探索节点；3）自适应探索算法平衡轨迹深度（难度）和广度（多样性）；4）开发世界知识指导和全局记忆回溯避免低质量生成；5）基于树节点信息扩展提出TreeCUA-DPO方法，通过参考相邻轨迹的分支信息提升GUI规划能力。

Result: 实验结果显示TreeCUA和TreeCUA-DPO带来显著改进，域外（OOD）研究进一步展示了强大的泛化能力。

Conclusion: TreeCUA通过树结构可验证演化有效扩展GUI自动化，提出的多智能体框架和树基拓扑能够高效生成高质量GUI轨迹，TreeCUA-DPO方法进一步提升了GUI规划能力，在泛化性方面表现优异。
...
TL;DR: 本研究针对肝脏纤维化临床诊断需求，提出了一个多任务深度学习框架，用于肝脏分割和纤维化分期，采用半监督学习处理多参数MRI数据中的标注不足和域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 肝脏纤维化在临床实践中具有重大挑战，需要精确的肝脏分割和准确的疾病分期。多参数MRI数据存在标注有限、模态差异和域偏移等困难，需要有效的解决方案。

Method: 采用多任务深度学习框架：1) LiSeg阶段使用半监督学习模型，结合图像分割和配准，利用标注和未标注数据处理域偏移和模态变化；2) LiFS阶段采用基于patch的方法，根据分类输出可视化肝脏纤维化分期。

Result: 方法在CARE Liver 2025 Track 4挑战赛的独立测试集上验证，包括分布内和分布外病例，使用三通道MRI（T1、T2、DWI）和七通道MRI（T1、T2、DWI、GED1-GED4）。代码已开源。

Conclusion: 提出的多任务深度学习框架能有效处理多模态成像数据、有限标注和域偏移问题，为肝脏分割和纤维化分期提供了可行的解决方案。
...
TL;DR: 提出GenSeg-R1框架，通过解耦的"推理-分割"流程实现细粒度指代图像分割。使用视觉语言模型进行场景推理生成结构化空间提示，再由冻结的SAM 2分割器转换为高质量掩码。


<details>
  <summary>Details</summary>
Motivation: 解决细粒度指代图像分割问题，传统方法通常需要监督推理链标注。本文旨在开发无需监督推理链标注的方法，通过解耦的推理-分割流程提升分割性能。
...
Conclusion: 提出的GenSeg-R1框架通过解耦的推理-分割流程，在无需监督推理链标注的情况下，在多个指代图像分割基准上实现了最先进的性能，特别是在处理负提示(无目标)方面表现出色。
...
TL;DR: Stroke3D：首个基于用户手绘2D笔画生成可动画3D网格的框架，通过可控骨架生成和增强网格合成两阶段流程实现
...
Method: 采用两阶段流程：1）可控骨架生成：使用骨骼图VAE（Sk-VAE）编码骨架图结构到潜在空间，骨骼图DiT（Sk-DiT）生成骨骼嵌入，通过文本语义和2D笔画进行条件控制；2）增强网格合成：通过TextuRig数据集增强现有骨架到网格模型，并使用SKA-DPO偏好优化策略提高几何保真度。

Result: 实验证明Stroke3D能够生成合理的骨架和高质量的网格，是首个基于用户手绘2D笔画生成可绑定3D网格的方法。

Conclusion: Stroke3D框架为创建可直接动画的3D内容提供了更直观的工作流程，通过结合文本提示和2D笔画输入，实现了对3D骨架和网格的细粒度控制。
...
TL;DR: 提出一种混合方法，将裂纹检测建模为逆问题，将观测图像分解为无裂纹绘画和裂纹成分，使用深度生成模型作为艺术品先验，结合变分函数和裂纹先验进行联合优化。


<details>
  <summary>Details</summary>
Motivation: 自动检测数字化绘画中的裂纹对于评估退化程度和指导修复至关重要，但由于复杂场景以及裂纹与类似艺术特征（如笔触或发丝）的视觉相似性，该任务仍然具有挑战性。

Method: 采用混合方法，将裂纹检测建模为逆问题：将观测图像分解为无裂纹绘画和裂纹成分。使用深度生成模型作为底层艺术品的强大先验，同时使用Mumford-Shah型变分函数结合裂纹先验来捕捉裂纹结构。通过联合优化获得像素级的裂纹定位图。

Result: 该方法能够生成像素级的裂纹定位图，有效区分裂纹与类似的艺术特征，支持艺术品的文档记录和修复工作。

Conclusion: 提出的混合方法结合了深度生成模型和变分函数，能够有效检测数字化绘画中的裂纹，为艺术品保护和修复提供了可靠的技术支持。
...
TL;DR: 该研究分析了自动驾驶汽车视觉系统的安全性，提出了参考架构并识别了攻击向量，评估了其对机密性、完整性和可用性的影响。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车的安全导航依赖于鲁棒的视觉系统，但现有研究对视觉系统安全性的全面分析不足，需要系统性地识别攻击表面和评估安全威胁。

Method: 首先分析CAV导航所需的关键传感器和视觉组件，推导出CAV视觉系统参考架构；然后基于该架构识别潜在攻击表面，详细阐述针对每个攻击表面的攻击向量；最后严格评估这些攻击对机密性、完整性和可用性（CIA三要素）的影响。

Result: 建立了CAV视觉系统参考架构，系统性地识别了攻击表面和攻击向量，全面评估了攻击对CIA三要素的影响，为制定鲁棒的安全措施提供了基础。
...
TL;DR: 研究发现文本到图像模型的训练数据存在严重的地理偏见，美国、英国和加拿大占样本48%，而南美和非洲国家严重不足，数据代表性与国家GDP高度相关。
...
Method: 使用LLM从图像标题中提取位置信息，将图像-标题对映射到国家；研究三个广泛使用的数据集（Re-LAION、DataComp1B和Conceptual Captions）的英文标题，涵盖20个常见实体；分析4种语言的Re-LAION非英文子集；评估Stable Diffusion v1.3生成的国家特定图像。

Result: 美国、英国和加拿大占样本48.0%，南美和非洲国家分别仅占1.8%和3.8%；国家GDP与数据代表性呈强相关（ρ=0.82）；非英文子集严重偏向这些语言主要使用的国家；更高代表性并不一定转化为更大的视觉或语义多样性；Stable Diffusion生成的图像虽然看起来真实，但覆盖范围严重受限。

Conclusion: 多模态数据集存在严重的地理偏见，偏向富裕国家，这影响了文本到图像模型的代表性和多样性，需要更平衡的数据收集策略。
...
TL;DR: SciFlow-Bench是一个专注于科学图表结构正确性的新基准，通过将生成的图表图像逆向解析为结构化图来进行评估，强调结构可恢复性而非视觉相似性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D可变形模型主要关注全局面部结构或几何特征，而忽略了面部语义特征如年龄、性别、面部标志点等高级特征。为了适应这些高级面部特征的变化，需要开发能够同时考虑形状和外观的3D重建系统。

Method: 提出SARS系统，这是一个模块化流水线，能够从单张图像中提取身体和面部信息。系统结合身份和表情混合形状与基础面部网格，通过调整多样参数来控制3D可变形模型的变异性，包括形状、纹理、光照和相机参数等高级图像描述符。

Result: 开发了一个能够从单张图像重建完整人体3D模型的系统，该系统不仅关注全局结构，还能捕捉面部语义特征，如年龄、性别和面部标志点（表征面部边界、曲线、凹陷和皱纹）。

Conclusion: SARS系统通过结合形状和外观感知，能够更全面地重建人体3D模型，特别在捕捉面部语义特征方面优于传统方法，为3D人体重建提供了更精细的解决方案。
...
TL;DR: CompSplat：一种针对长视频压缩感知的3D高斯泼溅训练框架，通过显式建模帧级压缩特性来缓解压缩引起的不一致性和几何误差，在严重压缩条件下实现最先进的渲染质量和姿态精度。


<details>
  <summary>Details</summary>
Motivation: 真实世界视频通常包含长序列、不规则相机轨迹和未知姿态，导致重建过程中的姿态漂移、特征错位和几何失真。有损压缩会加剧这些问题，引入不一致性并逐渐降低几何和渲染质量。现有方法要么关注长序列NVS，要么关注无姿态重建，而压缩感知方法仍局限于特定伪影或有限场景，未能充分探索长视频中的多样化压缩模式。
...
Conclusion: CompSplat通过显式建模压缩特性有效解决了长视频压缩带来的挑战，在保持高质量新视角合成的同时显著提升了压缩条件下的性能表现。
...
Method: 从注意力头、模型层和解码令牌三个角度进行实证分析，识别出三种关键幻觉模式。基于这些发现，提出了稳定性感知知识增强解码方法，引入层间知识稳定性评分来量化模型内部知识稳定性，通过对比最稳定感知层和最稳定无关层来抑制解码噪声并动态利用最可靠的内部知识。

Result: SAKED方法在各种模型、任务和基准测试中实现了最先进的幻觉缓解性能，且无需训练即可无缝集成到不同架构中。

Conclusion: 通过量化模型内部知识稳定性并动态利用最可靠的知识，SAKED方法能有效缓解大型视觉语言模型的幻觉问题，提高模型输出的忠实度和可靠性。
...
TL;DR: RoSE将单目法线估计重新定义为着色序列估计，利用图像到视频生成模型预测着色序列，然后通过最小二乘问题转换为法线图，解决了现有方法中的3D错位问题。


<details>
  <summary>Details</summary>
Motivation: 现有单目法线估计方法直接预测法线图，但存在3D错位问题：虽然估计的法线图看起来正确，但重建的表面无法与几何细节对齐。这是因为模型难以区分和重建法线图中表示的不同几何形状，因为底层几何的差异仅通过相对微妙的颜色变化反映。
...
Conclusion: 通过将法线估计重新定义为着色序列估计，RoSE解决了现有方法的3D错位问题，在单目法线估计任务上取得了更好的几何对齐效果。
...
TL;DR: ARK是一个多模态检索基准，专注于专业知识和复杂推理评估，涵盖5个知识领域和6种推理技能，包含16种视觉数据类型，旨在避免捷径匹配并提供诊断性分析。


<details>
  <summary>Details</summary>
Motivation: 现有多模态检索基准主要关注日常生活图像的语义匹配，缺乏对专业知识和复杂推理的诊断能力。为了填补这一空白，需要创建一个能够分析多模态检索在知识领域和推理技能两个互补维度上的基准。

Method: 引入ARK基准，从两个互补视角设计：(1)知识领域（5个领域17个子类型），表征检索依赖的内容和专业知识；(2)推理技能（6个类别），表征识别正确候选所需的多模态证据推理类型。包含单模态和多模态查询与候选，涵盖16种异构视觉数据类型，大多数查询配有针对性困难负样本以避免捷径匹配。

Result: 评估了23个代表性文本基和多模态检索器，发现知识密集型检索和推理密集型检索之间存在显著差距，细粒度视觉和空间推理成为持续瓶颈。简单的增强方法（如重排序和重写）能带来一致改进，但仍有很大提升空间。

Conclusion: ARK基准填补了多模态检索评估在专业知识和复杂推理方面的空白，揭示了当前检索系统在细粒度视觉和空间推理方面的局限性，为未来研究提供了重要的诊断工具和改进方向。
...
TL;DR: 提出基于不平衡最优传输的配准感知匹配器，用于解决癌症患者纵向CT扫描中病灶对应关系建立的挑战，能够处理病灶出现、消失、合并和分裂等情况。


<details>
  <summary>Details</summary>
Motivation: 评估癌症患者纵向CT扫描中的病灶演变对治疗反应评估至关重要，但建立跨时间点的可靠病灶对应关系仍然具有挑战性。传统的基于几何邻近度的二分匹配器在病灶出现、消失、合并或分裂时表现不佳。

Method: 提出基于不平衡最优传输的配准感知匹配器，该模型能够适应不相等的病灶质量，并根据患者层面肿瘤负荷变化调整先验。传输成本融合了：(1)尺寸归一化的几何信息，(2)来自变形场雅可比行列式的局部配准可信度，(3)可选的块级外观一致性。通过相对剪枝稀疏化传输计划，无需重新训练或启发式规则即可获得一对一匹配以及新出现、消失、合并和分裂的病灶。

Result: 在纵向CT数据上，该方法相比仅基于距离的基线方法，在边缘检测精度和召回率、病灶状态召回率以及病灶图组件F1分数方面均取得了一致性的显著提升。

Conclusion: 提出的基于不平衡最优传输的配准感知匹配器能够有效解决纵向CT扫描中病灶对应关系建立的挑战，为癌症治疗反应评估提供了更可靠的病灶演变分析工具。
...
TL;DR: Kelix是一个完全离散的自回归统一模型，通过改进的视觉离散化方法，弥合了离散与连续视觉表示之间的理解差距，实现了统一的多模态理解与生成。


<details>
  <summary>Details</summary>
Motivation: 当前大多数视觉语言模型采用混合接口（离散文本标记+连续ViT特征），主要依赖文本驱动的监督，偏向理解而无法充分利用非文本数据的大规模自监督学习。现有离散视觉标记因编码容量有限导致信息丢失，理解能力明显弱于连续特征模型。

Method: 提出Kelix模型，采用完全离散的自回归统一建模方法，通过改进的视觉离散化技术，在保持生成能力的同时提升理解性能，实现统一的多模态理解与生成。

Result: Kelix成功弥合了离散与连续视觉表示之间的理解差距，在保持生成能力的同时，达到了与连续特征视觉语言模型相当的理解性能。

Conclusion: 完全离散的自回归统一建模是可行的，通过改进视觉离散化方法可以解决现有离散视觉标记的信息丢失问题，实现统一的多模态理解与生成，为大规模自监督学习提供了新途径。
...
TL;DR: 提出了一种用于膀胱癌内窥镜血管分割的混合注意力-卷积架构，通过Transformer捕获全局血管拓扑先验，CNN学习残差细化图恢复细血管细节，结合物理感知预训练解决数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌监测需要跟踪肿瘤位置，但膀胱可变形且缺乏稳定标志物。内窥镜下可见的血管可作为"血管指纹"用于导航，但自动分割面临挑战：内窥镜数据不完美（稀疏标签、气泡、光照变化、连续变形、黏膜皱襞模仿血管），现有方法难以处理这些领域特定复杂性。

Method: 提出混合注意力-卷积架构：Transformer捕获全局血管拓扑先验，CNN学习残差细化图精确恢复细血管细节。为优先考虑结构连通性，Transformer在优化的ground truth数据上训练（排除短分支和末端分支）。为解决数据稀缺，采用物理感知预训练策略，在未标记数据上使用临床基础的数据增强进行自监督学习。

Result: 在BlaVeS数据集（内窥镜视频帧）上评估，方法达到高准确率（0.94）和优越的精确度（0.61）及clDice（0.66），优于最先进的医学分割模型。关键成功抑制了膀胱充盈和排空过程中动态出现和消失的黏膜皱襞产生的假阳性。

Conclusion: HAC方法提供了临床导航所需的结构稳定性，能够可靠地分割膀胱内窥镜血管，为膀胱癌监测中的肿瘤位置跟踪提供了有效解决方案。
...
Method: 包含三个核心组件：1) 检索增强知识模块，将类别特定文本描述融入模型输入；2) 熵驱动潜在推理机制，在紧凑潜在空间中使用可优化的潜在思考令牌进行迭代探索；3) 动态视觉注入策略，选择性纳入信息量最大的图像块到潜在序列中

Result: 在广泛实验中，Reason-IAD始终优于最先进的方法，代码将在GitHub上公开

Conclusion: Reason-IAD通过知识引导的动态潜在推理框架，有效解决了工业异常检测中特定类别异常识别和可解释性的挑战
...
TL;DR: Free-GVC是一个无需训练的视频压缩框架，通过将视频编码重新定义为基于视频扩散先验的潜在轨迹压缩，在超低比特率下实现更好的感知质量和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式视频压缩方法在时间相关性利用方面有限，导致在超低比特率下出现明显闪烁和时间一致性下降的问题。

Method: 提出训练免费的生成式视频压缩框架，在GOP级别操作，将视频片段编码到紧凑潜在空间并沿扩散轨迹逐步压缩。包含自适应质量控制模块和GOP间对齐模块来确保感知一致性和时间连贯性。

Result: 相比最新的神经编解码器DCVC-RT，在DISTS指标上平均实现了93.29%的BD-Rate降低，用户研究进一步证实了其在超低比特率下的优越感知质量和时间一致性。

Conclusion: Free-GVC通过将视频编码重新定义为基于扩散先验的潜在轨迹压缩，有效解决了现有方法在超低比特率下的闪烁和时间一致性问题，实现了显著的压缩效率提升。
...
TL;DR: Fake-HR1：首个自适应决定是否需要推理的混合推理模型，通过两阶段训练框架提升合成图像检测效率，在推理能力和检测性能上超越现有LLMs


<details>
  <summary>Details</summary>
Motivation: 现有方法在合成图像检测中引入Chain-of-Thought推理虽然能提升检测能力，但过长的推理过程带来大量资源开销（token消耗和延迟），在处理明显伪造图像时尤其冗余。需要一种自适应判断是否需要推理的方法来平衡检测性能和效率。

Method: 提出Fake-HR1大规模混合推理模型，采用两阶段训练框架：1）混合微调（HFT）进行冷启动初始化；2）在线强化学习配合混合推理分组策略优化（HGRPO），隐式学习何时选择适当的推理模式。

Result: 实验结果表明，Fake-HR1能够自适应地对不同类型查询执行推理，在推理能力和生成检测性能上均超越现有LLMs，同时显著提升响应效率。

Conclusion: Fake-HR1通过自适应推理机制有效解决了合成图像检测中推理冗余问题，实现了检测性能与资源效率的良好平衡，为生成检测任务提供了新的解决方案。
...
TL;DR: 提出一种新颖的具身4D世界模型，能够从单视角RGBD观测生成几何一致的任意视角RGBD，并通过测试时动作优化策略解决逆动力学的不适定问题，实现更好的机器人操作性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于世界模型的想象-行动范式通常只支持纯图像预测或部分3D几何推理，限制了预测完整4D场景动态的能力。需要一种能够生成几何一致、任意视角RGBD的方法来构建更完整的时空场景表示。

Method: 提出具身4D世界模型，从单视角RGBD输入生成剩余视角的RGBD，通过反投影和融合构建更完整的3D结构。设计跨视角和跨模态特征融合机制，确保RGB与深度的一致性以及视角间的几何对齐。采用测试时动作优化策略，通过生成模型反向传播推断与预测未来最匹配的轨迹级潜在变量，并结合残差逆动力学模型将轨迹先验转化为可执行动作。

Result: 在三个数据集上的实验表明，该方法在4D场景生成和下游操作任务上都表现出色。消融研究为关键设计选择提供了实用见解。

Conclusion: 该工作提出的具身4D世界模型能够生成几何一致的任意视角RGBD，并通过创新的动作优化策略解决了逆动力学的不适定问题，为机器人操作提供了更强大的预测和规划能力。
...
TL;DR: 本文提出可分离因果扩散模型（SCD），将因果推理与多步去噪过程解耦，显著提升生成效率同时保持或超越基线模型质量。


<details>
  <summary>Details</summary>
Motivation: 当前因果扩散模型将时间推理与迭代去噪过程纠缠在一起，在每一层、每一步去噪和整个上下文中都应用因果注意力，导致计算冗余和效率低下。

Method: 通过系统分析自回归视频扩散模型，发现早期层特征在去噪步骤间高度相似，深层主要执行帧内渲染。基于此提出SCD架构：使用因果Transformer编码器进行每帧一次的时间推理，轻量级扩散解码器进行多步帧内渲染。

Result: 在合成和真实基准测试的预训练和后训练任务中，SCD显著提高了吞吐量和每帧延迟，同时匹配或超越了强因果扩散基线的生成质量。

Conclusion: 因果推理与多步去噪过程是可分离的，SCD架构通过解耦这两个组件实现了更高效的视频生成，为因果扩散模型的设计提供了新思路。
...
### [64] [Olaf-World: Orienting Latent Actions for Video World Modeling](https://arxiv.org/abs/2602.10104)
*Yuxin Jiang,Yuchao Gu,Ivor W. Tsang,Mike Zheng Shou*

Main category: cs.CV

TL;DR: 提出SeqΔ-REPA方法，通过序列级控制效果对齐目标，从大规模无标签视频中学习结构化潜在动作空间，实现更好的零样本动作迁移


<details>
  <summary>Details</summary>
Motivation: 现有动作可控世界模型的扩展受限于动作标签稀缺。虽然潜在动作学习可以从无标签视频中提取控制接口，但学习到的潜在表示往往在不同上下文间无法迁移：它们会与场景特定线索纠缠，缺乏共享坐标系。这是因为标准目标仅在每个视频片段内操作，没有提供跨上下文对齐动作语义的机制。

Method: 引入SeqΔ-REPA（序列级控制效果对齐目标），将集成潜在动作锚定到冻结的自监督视频编码器的时序特征差异上。基于此提出Olaf-World流程，从大规模被动视频中预训练动作条件视频世界模型。

Result: 实验表明，该方法学习到更结构化的潜在动作空间，相比现有最先进基线，在零样本动作迁移和新控制接口的数据高效适应方面表现更强。

Conclusion: 通过利用动作的语义效果作为共享参考，SeqΔ-REPA能够跨上下文对齐潜在动作表示，解决了现有潜在动作学习中的迁移性问题，为从无标签视频中学习动作可控世界模型提供了有效解决方案。
...
TL;DR: LASANA数据集包含1270个腹腔镜手术训练任务的立体视频记录，带有结构化技能评分和任务特定错误标签，用于支持基于视频的手术技能自动评估深度学习模型开发。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术训练需要大量练习，深度学习在视频技能评估方面有潜力，但当前受限于标注数据集规模小，阻碍了模型开发和评估。

Method: 创建LASANA数据集，包含1270个立体视频记录，涵盖四个基本腹腔镜训练任务，每个记录都有结构化技能评分（来自三位独立评分者）和任务特定错误标签，提供预定义数据分割。
...
TL;DR: GeoFormer是一个基于Swin Transformer的开源框架，使用Sentinel-1/2影像和开放DEM数据联合估计100米网格上的建筑高度和占地面积，在54个城市上表现优于CNN基线，并实现了跨大陆迁移能力。
...
Method: 提出GeoFormer框架，基于Swin Transformer架构，仅使用Sentinel-1/2影像和开放DEM数据，在100米网格上联合估计建筑高度和占地面积。采用地理区块划分策略确保训练集和测试集之间的严格空间独立性。

Result: 在54个多样化城市评估中，GeoFormer实现建筑高度RMSE为3.19米，占地面积RMSE为0.05，比最强的CNN基线分别提升7.5%和15.3%。在跨大陆迁移中保持建筑高度RMSE低于3.5米。消融研究证实DEM对高度估计不可或缺，光学反射率优于SAR，但多源融合获得最佳整体精度。

Conclusion: GeoFormer提供了一个开源解决方案，能够仅使用开放遥感数据准确估计城市三维特征，显著优于现有方法，并展示了良好的跨大陆泛化能力。所有代码、权重和全球产品均已公开发布。
...
TL;DR: 该研究提出了一种用于德国面包店剩余产品监控的自动化解决方案，通过结合弱监督学习和伪标签传播技术，在标注数据有限的情况下训练出高性能的目标检测模型。


<details>
  <summary>Details</summary>
Motivation: 德国面包店的新鲜烘焙产品保质期很短，监控剩余产品对优化生产至关重要。自动化监控可以降低人工成本、提高准确性并简化运营。然而，德国烘焙产品种类繁多，完全监督训练成本过高且可扩展性有限，需要开发在标注数据稀缺情况下的解决方案。

Method: 1. 构建包含19类烘焙产品的数据集，设置不同监督级别的数据划分；2. 提出两种训练工作流程：一是结合OWLv2和Grounding DINO定位与图像级监督进行弱监督训练；二是使用Segment Anything 2作为伪标签传播模型，在视频帧上进行微调以提高视角鲁棒性；3. 选择YOLOv11作为检测模型，平衡速度与精度。

Result: 仅使用图像级监督时，模型达到0.91的平均精度(mAP)。通过伪标签微调，在非理想部署条件下模型性能提升了19.3%。结合两种工作流程训练的模型，在非理想部署条件下超越了完全监督的基线模型，尽管仅依赖图像级监督。

Conclusion: 该研究成功开发了在标注数据有限情况下训练高性能目标检测模型的方法，解决了烘焙行业自动化监控的实际问题。提出的弱监督学习和伪标签传播技术为其他标注数据稀缺的工业计算机视觉应用提供了有价值的参考。
...
TL;DR: 该研究比较了两种全玻片图像染色分类方法：多实例学习（MIL）和轻量级缩略图方法，用于病理学中14种常用特殊染色和H&E染色的自动分类。


<details>
  <summary>Details</summary>
Motivation: 病理学中需要多种特殊染色来诊断特定形态学特征，保持准确的染色元数据对临床档案质量控制和计算病理学数据集完整性至关重要。需要自动化方法来分类不同染色类型。

Method: 比较了两种方法：1）多实例学习（MIL）管道，处理全玻片图像；2）提出的轻量级缩略图方法，使用缩略图进行分类。评估了16种染色类型（包括标准H&E、冰冻切片H&E和14种特殊染色）。

Result: 在内部测试数据上，MIL表现最佳（16类macro F1: 0.941；14类合并后: 0.969），缩略图方法也具竞争力（分别为0.897和0.953）。在外部TCGA数据上，缩略图模型泛化能力更好（加权F1: 0.843 vs. MIL的0.807）。缩略图方法处理速度比MIL快两个数量级（5.635 vs. 0.018 slides/s）。
...
TL;DR: Faster-GS：一个经过严格优化的3D高斯泼溅算法，通过整合现有有效策略并引入新优化，实现了5倍训练加速且保持视觉质量，为3DGS优化建立了新的高效基准。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS研究存在实现级改进与算法修改混杂、性能与保真度权衡的问题，导致研究碎片化且难以公平比较。需要整合最有效策略并探索框架中被忽视的方面。

Method: 整合先前3DGS研究中最有效且广泛适用的策略，加入多个新颖优化，并深入探索数值稳定性、高斯截断和梯度近似等未充分研究的方面。

Result: Faster-GS在综合基准测试中实现了高达5倍的训练加速，同时保持视觉质量，为3DGS优化建立了新的成本效益和资源效率基准。优化还可应用于4D高斯重建，实现高效的非刚性场景优化。

Conclusion: 通过系统整合和优化，Faster-GS为3D高斯泼溅提供了一个严格优化的算法，显著提升了训练效率，同时保持了重建质量，为未来研究建立了新的高效基准。
...
TL;DR: 研究发现，即使经过颅骨剥离的脑部MRI图像，仍可通过标准预处理和图像相似度计算实现跨数据库的个体匹配，存在隐私泄露风险。


<details>
  <summary>Details</summary>
Motivation: 尽管现有法规要求在共享脑部MRI数据前移除潜在标识符，但即使经过颅骨剥离，脑实质仍包含独特的生物特征，可能在不同数据库中被匹配识别，存在隐私风险。当前风险评估方法可能不够充分。

Method: 使用标准预处理流程处理颅骨剥离的T1加权MRI图像，然后通过图像相似度计算方法进行个体匹配。该方法不依赖训练或复杂计算，而是基于直接的图像特征比较。

Result: 在跨不同时间间隔、扫描仪类型、空间分辨率和采集协议的情况下，实现了近乎完美的匹配准确率。即使存在潜在的认知衰退，也能成功匹配不同数据库中的MRI样本。

Conclusion: 颅骨剥离的脑部MRI图像仍可通过简单方法实现个体匹配，存在重新识别的隐私风险。这一发现对医疗数据共享政策的制定具有重要意义，需要更全面的隐私保护措施。
...
TL;DR: 该论文提出了一种用于实例分割的保形预测算法，能够生成具有概率保证的自适应置信集，确保至少一个预测与真实实例掩码具有高IoU的概率。


<details>
  <summary>Details</summary>
Motivation: 当前实例分割模型虽然平均预测性能高，但缺乏原则性的不确定性量化：它们的输出未经过校准，且无法保证预测掩码接近真实值。需要一种能够提供概率保证的置信集生成方法。

Method: 引入保形预测算法，为实例分割生成自适应置信集。给定图像和像素坐标查询，算法为该像素生成实例预测的置信集，并提供至少一个预测与真实对象实例掩码具有高IoU的概率保证。

Result: 算法在农田划分、细胞分割和车辆检测等实例分割任务中应用，预测集大小根据查询难度自适应变化，达到目标覆盖率，优于现有的Learn Then Test、Conformal Risk Control和形态学膨胀等方法。

Conclusion: 提出的保形预测算法为实例分割提供了具有概率保证的不确定性量化方法，能够生成自适应置信集，在多种应用场景中表现优于现有基线方法，并提供渐近和有限样本保证。
...
TL;DR: Forensim是一个基于注意力机制的状态空间框架，用于图像伪造检测，能同时定位被篡改区域和源区域，支持拼接和复制-移动伪造检测，并在标准基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖伪影线索检测拼接或伪造区域，但在某些场景（如抗议图像）中，仅检测伪造区域可能导致误解。需要同时定位源区域和目标区域来理解上下文，特别是复制模式对理解伪造背景至关重要。

Method: 提出视觉状态空间模型，利用归一化注意力图识别内部相似性，结合基于区域的块注意力模块来区分篡改区域。该框架输出三类掩码（原始、源、目标），支持端到端训练和精确定位。

Result: 在标准基准测试中达到最先进的性能。同时发布了CMFD-Anything新数据集，解决了现有复制-移动伪造数据集的局限性。

Conclusion: Forensim框架通过联合定位源区域和目标区域，能更好地理解图像伪造的上下文，特别是在需要理解复制模式的场景中。该统一架构同时支持拼接和复制-移动伪造检测，为图像取证提供了更全面的解决方案。
...
TL;DR: VideoWorld 2通过动态增强的潜在动态模型从原始真实世界视频中学习可迁移知识，在手工制作任务上取得显著性能提升，并能将Open-X数据集的知识迁移到机器人操作任务中。


<details>
  <summary>Details</summary>
Motivation: 从无标签视频数据中学习可迁移知识并将其应用于新环境是智能体的基本能力。现有方法在处理真实世界视频时面临挑战，需要探索直接从原始视频中学习可迁移世界知识的潜力。

Method: 提出动态增强的潜在动态模型(dLDM)，将动作动态与视觉外观解耦：使用预训练视频扩散模型处理视觉外观建模，让dLDM学习专注于紧凑且有意义任务相关动态的潜在编码，然后通过自回归建模学习任务策略并支持长时程推理。

Result: 在具有挑战性的真实世界手工制作任务上，VideoWorld 2相比之前的视频生成和潜在动态模型，任务成功率提升高达70%，并能生成连贯的长执行视频。在机器人领域，能从Open-X数据集获取有效操作知识，显著提升CALVIN上的任务性能。
...
TL;DR: 该论文提出了ConsID-Gen框架，通过多视角辅助和双流编码器解决图像到视频生成中的外观漂移和几何失真问题，并构建了ConsIDVid数据集和评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成模型在视角变化时难以保持细粒度对象身份，存在外观漂移和几何失真问题，这源于单视角2D观测的稀疏性和弱跨模态对齐。

Method: 1) 构建ConsIDVid大规模以对象为中心的数据集和ConsIDVid-Bench评估基准；2) 提出ConsID-Gen框架，通过未姿态辅助视角增强首帧，使用双流视觉-几何编码器和文本-视觉连接器融合语义与结构线索，为Diffusion Transformer骨干提供统一条件。

Result: 在ConsIDVid-Bench上的实验表明，ConsID-Gen在多个指标上持续优于现有方法，整体性能超越Wan2.1和HunyuanVideo等领先视频生成模型，在具有挑战性的真实场景中提供更优的身份保真度和时间一致性。

Conclusion: 该研究从数据和模型两个角度解决了图像到视频生成中的身份保持问题，提出的ConsID-Gen框架通过多视角辅助和结构感知编码显著提升了生成视频的质量和一致性。
...
<details>
  <summary>Details</summary>
Motivation: 传统多旋转平均方法如L1-IRLS和Shonan存在局部极小值问题和依赖凸松弛的局限性，无法精确保持旋转流形几何，导致在高噪声场景下精度下降。

Method: IQARS将多旋转平均问题重新表述为一系列局部二次非凸子问题，经过二值化处理后可在量子退火器上执行，利用量子隧穿和并行性进行高效解空间探索。

Result: 在合成和真实数据集上的评估显示，IQARS在D-Wave量子退火器上相比最佳经典方法Shonan实现了约12%的精度提升，尽管当前量子退火器仍处于早期阶段且只能处理有限规模问题。

Conclusion: IQARS通过消除凸松弛依赖并更好地保持非欧几里得旋转流形几何，结合量子硬件优势，为多旋转平均问题提供了有前景的量子计算方法，展示了量子计算在3D视觉和机器人领域的潜力。
...
TL;DR: SAGE是一个智能体框架，能够根据用户指定的具身任务自动生成仿真就绪的3D环境，通过多生成器与评估器结合实现语义合理性、视觉真实性和物理稳定性，支持大规模场景生成用于具身AI训练。
...
Result: 生成的环境真实、多样，可直接在现代仿真器中部署用于策略训练。仅使用该数据训练的智能体策略展现出明显的扩展趋势，并能泛化到未见过的物体和布局。

Conclusion: SAGE展示了仿真驱动扩展在具身AI中的潜力，为大规模、真实的3D环境生成提供了有效解决方案，支持具身智能体的训练和泛化。
...
TL;DR: PAN 2026研讨会聚焦计算文体学和文本取证，包含五个任务：生成AI检测、文本水印、多作者风格分析、生成式抄袭检测和推理轨迹检测，通过Docker容器实现可复现评估。


<details>
  <summary>Details</summary>
Motivation: 推动计算文体学和文本取证领域的发展，通过客观、可复现的评估方法来解决当前文本分析中的关键挑战，特别是在生成式AI时代面临的新问题。

Method: 采用任务驱动的评估框架，包含五个具体任务：1) Voight-Kampff生成AI检测（针对混合和混淆的作者场景）；2) 文本水印（评估现有方案的鲁棒性）；3) 多作者写作风格分析（识别作者变更位置）；4) 生成式抄袭检测（源文档检索和文本对齐）；5) 推理轨迹检测（LLM生成或人工写作的源检测和安全检测）。通过TIRA实验平台以Docker容器形式提交软件，确保可复现性。

Result: 自2012年以来，PAN通过TIRA平台已收到超过1,100份软件提交，建立了成熟的评估生态系统。2026年延续了这一传统，同时引入了文本水印和推理轨迹检测两个新任务，反映了对生成式AI时代新挑战的关注。

Conclusion: PAN 2026通过五个精心设计的任务继续推动计算文体学和文本取证领域的发展，特别关注生成式AI带来的新挑战，同时保持了通过Docker容器实现可复现评估的传统，为研究者提供了客观的评估平台。
...
TL;DR: 提出"包容性分析"框架，通过话语分析实时测量人机协作学习中的包容性动态过程


<details>
  <summary>Details</summary>
Motivation: 当前AI和教育领域的包容性、公平性和可及性评估通常采用粗粒度的样本描述或事后自我报告，无法捕捉协作问题解决过程中时刻变化的包容性动态

Method: 提出包容性分析框架，从三个维度（参与公平性、情感氛围、认知公平性）概念化包容性，使用可扩展的交互级测量方法，通过模拟对话和人机协作实验数据进行验证

Result: 包容性分析能够揭示参与模式、关系动态和思想采纳模式，这些在聚合或事后评估中是不可见的

Conclusion: 这项工作代表了向过程导向方法测量人机协作学习环境中包容性的初步步骤
...
Method: 将自然语言理解模型扩展为可量化的朴素贝叶斯方法，引入注意力机制和对数非线性变换来增强主题连续性捕捉能力，形成可解释的分析公式。

Result: 模型在实验中持续优于传统方法，特别是在处理长而复杂的对话时表现更佳，具有线性时间复杂度和处理任意长度对话的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在标准操作程序理解和跨领域泛化方面存在困难，因为联合训练无法区分SOP所需的三种推理能力：术语精确性、顺序排列和约束推理。

Method: 提出FM SO.P方法，包含两个创新：1) 渐进式任务混合，分阶段构建三种任务能力（概念消歧、动作序列理解、场景感知图推理）；2) 自动多智能体评估系统，由三个智能体自适应生成评分标准、分层测试集和评分。

Result: 在SOPBench的七个领域（银行、DMV、医疗、市场、大学、图书馆、酒店）上，32B模型达到48.3%通过率，开源7B模型达到34.3%通过率，与Qwen-2.5-72B-Instruct基线（34.4%）相当，但参数减少10倍。

Conclusion: FM SO.P通过渐进式能力构建和自适应评估，有效提升了语言模型对标准操作程序的理解和跨领域泛化能力，为SOP理解任务提供了新的解决方案。
...
TL;DR: 该研究通过大规模计算主题分析，探索了AI使用中产生的心理风险如何形成、被体验和调节，基于Reddit社区的真实用户讨论识别出AI相关心理风险的五个体验维度。


<details>
  <summary>Details</summary>
Motivation: 生成式AI系统日益融入日常生活，但关于AI使用相关心理风险如何产生、被体验以及用户如何调节的实证理解仍然有限。研究者希望从真实世界的用户讨论中获取早期实证证据，了解AI安全在实验室或推测性情境之外是如何被感知和情感体验的。

Method: 采用大规模计算主题分析方法，收集2023-2025年间来自Reddit社区r/AIDangers和r/ChatbotAddiction的帖子。使用基于Braun和Clarke反思框架的多智能体、LLM辅助主题分析，识别出14个重复出现的主题类别，并将其综合为五个高阶体验维度。进一步应用基于BERT的情感分类器进行情感标注，并可视化不同维度的情感特征。

Result: 研究识别出五个基于真实世界用户讨论的AI相关心理风险体验维度。自我调节困难是最普遍的维度，而恐惧主要集中在自主性、控制和技术风险相关的担忧中。情感分析揭示了不同维度的情感特征模式。

Conclusion: 该研究提供了来自真实用户生活体验的早期实证证据，展示了AI安全在非实验室情境下如何被感知和情感体验。这些结果为未来AI安全研究、评估和负责任治理奠定了基础，强调了从用户角度理解AI心理风险的重要性。
...
TL;DR: 该研究评估了大型语言模型(LLMs)对西班牙语地理词汇变异的捕捉能力，发现模型能识别某些地区的词汇变异(如西班牙、墨西哥等)，但难以区分智利方言，且表现差异与数字资源数量无关。


<details>
  <summary>Details</summary>
Motivation: 西班牙语存在显著的地域变异，但LLMs是否准确捕捉这些方言词汇变异尚不清楚。研究旨在评估LLMs作为"虚拟信息提供者"对西班牙语地理词汇变异的理解程度，探讨数字语言偏见问题。

Method: 将LLMs视为虚拟信息提供者，采用两种调查式问题格式(是非题和选择题)，利用专家策划的大规模西班牙语词汇变异数据库，覆盖21个西班牙语国家的900多个词汇项目，在国家层面和方言区域层面进行评估。

Result: LLMs在表示西班牙语变体方面存在系统性差异：能较准确识别西班牙、赤道几内亚、墨西哥及中美洲、拉普拉塔河地区的词汇变异；智利变体特别难以区分。重要的是，国家层面数字资源数量差异不能解释这些表现模式。

Conclusion: LLMs对西班牙语地理词汇变异的捕捉存在偏差，某些方言变体被更好表示而其他被边缘化。数据量以外的因素影响LLMs中的方言表示，这为西班牙语数字语言偏见讨论提供了新证据，推进了对LLMs方言知识的实证理解。
...
Method: 使用无监督神经机器翻译系统将高资源语言句子翻译成低资源语言，构建伪平行句对。然后基于词对齐进行标准投影训练目标语言词性标注器。提出多源投影技术校准投影的词性标签。

Result: 在28个语言对（4个源语言，7个目标语言）上评估，性能与使用平行句对的基线方法相当，某些目标语言甚至超过基线。多源投影技术带来平均1.3%的性能提升。

Conclusion: 提出的完全无监督跨语言词性标注框架仅需单语语料就能实现有效标注，多源投影技术进一步提升了性能，为缺乏平行语料的低资源语言提供了可行的词性标注解决方案。
...
TL;DR: AfriNLLB是一系列轻量级模型，支持15种语言对（30个翻译方向）的非洲语言翻译，通过模型压缩和知识蒸馏实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 旨在为资源受限环境中的非洲语言提供高效的翻译模型部署方案，解决非洲语言翻译资源不足的问题。

Method: 基于NLLB-200 600M模型，采用迭代层剪枝和量化进行压缩，在非洲语言平行语料库上进行微调，并使用知识蒸馏从更大的教师模型学习。

Result: AfriNLLB模型在保持与基线相当性能的同时显著提升了推理速度，并发布了Transformers和CTranslate2两个版本。

Conclusion: 该工作成功开发了适用于非洲语言的高效轻量级翻译模型，为资源受限环境中的多语言翻译提供了实用解决方案，并开源了训练数据促进进一步研究。
...
Result: BiasScope能够在不同模型家族和规模中发现潜在偏见，其通用性和有效性在JudgeBench数据集上得到验证。基于BiasScope构建了JudgeBench-Pro，这是一个扩展版本，作为评估LLM-as-a-judge鲁棒性的更具挑战性的基准。令人震惊的是，即使是强大的LLM作为评估器，在JudgeBench-Pro上的错误率也超过50%。

Conclusion: 该研究强调了加强评估鲁棒性和进一步减轻潜在偏见的迫切需求。BiasScope框架为自动发现评估偏见提供了有效工具，而JudgeBench-Pro则揭示了当前LLM评估系统在鲁棒性方面存在的严重不足。
...
TL;DR: 论文批判性地分析LLMs在合同起草中的局限性，认为其无法真正理解法律语言和推理，生成的文件可能表面合理但实际无用或不适合具体交易。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在文本生成方面表现出色，但法律界存在一种误解，认为LLMs可以协助合同起草等法律任务。作者旨在纠正这种错误观念，指出LLMs缺乏对语言意义、上下文和推理的真正理解。

Method: 通过理论分析，对比LLMs的统计模式预测能力与法律实践中需要的语言理解、上下文把握和法律推理能力。论证预测单词与在具体交易情境中使用语言的区别，以及重组常见合同短语与法律推理的区别。

Result: LLMs只能生成表面合理但通用的合同文件，这些文件可能是条款不一致的无用组合，或者是可执行但不适合特定交易的合同。LLMs无法真正理解法律含义和进行法律推理。

Conclusion: LLMs并不威胁法律行业的持续生存能力。法律工作需要真正的语言理解、上下文把握和法律推理能力，这些是LLMs目前无法提供的。需要摒弃LLMs可以替代法律专业人士的简单化假设。
...
TL;DR: 提出一种扩展多语言预训练模型到新低资源语言的方法，通过词汇扩展和双语词典初始化，在POS标注和NER任务上优于随机初始化基线


<details>
  <summary>Details</summary>
Motivation: 现有mPLMs主要关注已有语言的持续预训练，但很少解决如何将模型扩展到之前不支持的低资源语言的问题

Method: 1) 使用目标语言语料扩展模型词汇表；2) 筛选出偏向源语言(如英语)的原始词汇子集；3) 利用双语词典初始化扩展词汇的表示；4) 基于扩展词汇表示使用目标语言语料进行持续预训练

Result: 在POS标注和NER任务上分别比使用随机初始化扩展词汇的基线提升0.54%和2.60%；方法对训练语料选择具有高鲁棒性，且持续预训练后模型在源语言上的性能不会下降
...
TL;DR: 研究发现LLMs的道德判断会受到与道德无关的情境因素影响，类似于人类的"情境主义"道德偏见


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在高风险环境中的应用增加，需要确保其行为符合人类价值观。现有道德基准假设LLMs报告相对稳定的道德偏好，但人类道德心理学研究表明人类道德判断会受到无关情境因素影响，因此需要评估LLMs是否表现出类似的认知道德偏见。

Method: 从现有心理学数据集中收集60个"道德干扰物"（情感性图像和叙事），这些干扰物与所呈现情境没有道德相关性。将这些干扰物注入现有道德基准中，测量它们对LLM反应的影响。

Result: 道德干扰物可以使LLMs的道德判断发生超过30%的偏移，即使在低模糊性情境中也是如此。

Conclusion: LLMs表现出类似人类的认知道德偏见，需要更情境化的道德评估和更细致的LLMs认知道德建模。
...
TL;DR: RAG架构能减少大语言模型的社会偏见，而CoT提示虽提高准确性却增加偏见


<details>
  <summary>Details</summary>
Motivation: 研究RAG架构中的社会偏见问题，因为尽管RAG通过检索外部知识增强LLM能力，但仍可能面临与偏见相关的挑战

Method: 通过跨多种检索语料库、LLM和偏见评估数据集（涵盖13种以上偏见类型）的广泛实验，评估RAG的社会偏见影响，并整合CoT提示分析模型推理过程

Result: 令人惊讶地发现RAG能减少偏见，外部上下文有助于抵消刻板印象驱动的预测；但CoT虽提高准确性却增加整体偏见

Conclusion: 需要开发偏见感知的推理框架来平衡准确性和公平性，因为RAG能减少偏见而CoT增加偏见
...
TL;DR: 提出概念文化指数（CCI），用于在句子层面量化文化特异性，通过比较目标文化与其他文化的泛化性估计来计算，在文化特定句子的识别上优于直接使用LLM评分。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多文化环境中的应用日益增多，但缺乏系统性的句子层面文化特异性评估方法，需要一种可操作且可解释的指标来衡量文化特定性。

Method: 提出概念文化指数（CCI），定义为目标文化内部的泛化性估计与其他文化平均泛化性估计之间的差异。该方法允许用户通过比较设置操作性地控制文化范围，并提供可解释性。

Result: 在400个句子（200个文化特定句和200个通用句）上验证CCI，得分分布显示预期模式：文化特定句得分更高，通用句得分更低。在二元可分性方面，CCI优于直接LLM评分，对目标文化专门化模型的AUC提升超过10个百分点。

Conclusion: CCI是一种有效的句子层面文化特异性评估指标，具有操作可控性和可解释性，能够显著提升文化特定内容的识别性能，为多文化环境中的LLM应用提供重要工具。
...
TL;DR: NOWJ团队在BioCreative IX的ToxHabits共享任务中提出多输出集成系统，用于从西班牙语临床文本中提取药物使用信息，在触发检测和参数检测任务上取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 从非结构化电子健康记录中提取药物使用信息是临床自然语言处理的主要挑战。尽管大语言模型有进步，但在临床NLP中应用受限，存在信任、控制和效率方面的担忧。

Method: 提出多输出集成系统处理ToxNER和ToxUse两个子任务。系统集成BETO与CRF层进行序列标注，采用多样化训练策略，并使用句子过滤技术提高精度。

Result: 最佳运行结果：触发检测获得0.94 F1分数和0.97精度，参数检测获得0.91 F1分数，在西班牙语临床文本这一特定领域低资源环境中表现优异。

Conclusion: 提出的多输出集成系统在ToxHabits共享任务中有效解决了西班牙语临床文本中的有毒物质使用检测问题，为临床NLP中的药物信息提取提供了实用解决方案。
...
TL;DR: 提出CoCoA解码器，一种无需训练的推理时解码算法，通过分析模型中间层的表征不稳定性来减少LLM幻觉，提高事实准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生流畅但事实错误的文本（幻觉），这降低了其在下游任务中的可靠性和实用性。作者假设生成文本的事实性与其在模型内部各层中的表征不稳定性相关。

Method: 提出CoCoA解码器，通过监听模型中间层的信号来缓解幻觉。设计了两个量化中间层不稳定性的指标，并利用这些指标惩罚内部混淆高的输出，引导模型生成更内部一致和事实基础更强的输出。还提出了自信息门控变体CoCoA-SIG，动态调节惩罚以选择性针对高惊喜、不稳定的生成。
...
TL;DR: 该论文提出Gt-Margin方法，通过真实标记的概率边际获得最优解掩码顺序，并训练监督解掩码规划器来提升掩码扩散语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在推理时需要同时决定解掩码位置和标记内容，但现有方法中解掩码顺序通常基于启发式置信度或强化学习，成本高昂且效果有限。

Method: 提出Gt-Margin评分方法，基于真实标记与其最强替代标记的概率边际获得最优解掩码顺序；训练监督解掩码规划器通过排序学习模仿最优顺序。

Result: 使用最优解掩码顺序显著提升生成质量，特别是在逻辑推理基准上；训练的解掩码规划器在不修改标记预测模型的情况下提高了推理准确性。

Conclusion: Gt-Margin方法为掩码扩散语言模型提供了有效的解掩码顺序指导，监督解掩码规划器能够有效提升模型推理能力，为文本生成任务提供了新思路。
...
TL;DR: EcoGym是一个用于评估LLM智能体长期规划和决策能力的基准测试平台，包含三个经济环境，关注长期战略一致性和业务相关结果。


<details>
  <summary>Details</summary>
Motivation: 当前评估框架大多是情景式的、特定领域的，或缺乏持久经济动态的基础，需要更通用的长期规划评估基准。

Method: 开发了包含Vending、Freelance和Operation三个多样化环境的EcoGym平台，采用统一的决策流程和标准化接口，在1000+步的无界时间范围内进行预算化行动评估。

Result: 对11个领先LLM的实验显示，没有单一模型在所有三个场景中都表现最优，模型在高层策略或高效行动执行方面存在显著次优性。
...
TL;DR: 论文提出LLMs在搜索增强推理中存在"知识整合衰减"问题，即随着推理链增长，模型难以有效整合检索到的知识，并提出了无需训练的推理时策略SAKE来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型通过搜索增强推理在复杂任务中表现出色，但作者发现了一个关键但未被充分探索的瓶颈：知识整合衰减（KID）。随着推理链的增长，模型越来越难以将检索到的证据整合到后续推理步骤中，即使相关信息可用也会限制性能。

Method: 提出了自我锚定知识编码（SAKE），这是一种无需训练的推理时策略。通过在推理过程的开始和结束处锚定检索到的知识，防止其被先前上下文所掩盖，从而保持其语义完整性。

Result: 在多跳问答和复杂推理基准测试上的广泛实验表明，SAKE显著缓解了KID问题并提高了性能，为智能LLMs中的知识整合提供了一个轻量级但有效的解决方案。

Conclusion: SAKE策略有效解决了LLMs在长推理链中知识整合衰减的问题，通过锚定检索知识来保持其语义完整性，为知识整合提供了实用且高效的解决方案。
...
TL;DR: 本文提出MoSLoRA和UniARM框架，通过共享特征提取和偏好调制来解决多目标对齐中特征纠缠问题，实现更精确的偏好控制。
...
Method: 提出MoSLoRA（偏好调制与共享低秩适应）训练方法：首先通过偏好无关模块提取共享特征，然后通过基于混合偏好向量的偏好调制模块对共享特征进行仿射变换。基于此提出UniARM（统一自回归奖励模型）框架，在单一参数空间中联合建模所有偏好维度。

Result: MoSLoRA设计缓解了特征纠缠问题，在推理过程中实现了对偏好权衡的精确控制。UniARM框架消除了为每个偏好目标设置独立参数的需求，并在更大规模LLMs上进行了测试，增强了实际可用性。
...
TL;DR: 该论文系统比较了多轮对话问答中的检索增强生成方法，发现简单方法如重排序、混合BM25和HyDE通常优于复杂方法，且检索效果受数据集特性和对话长度影响显著。


<details>
  <summary>Details</summary>
Motivation: 当前大多数研究孤立评估RAG方法且主要关注单轮设置，缺乏对多轮对话问答中RAG方法的系统比较，而多轮对话中的对话历史、指代和用户意图变化使检索更加复杂。

Method: 在八个多样化对话问答数据集上使用统一实验设置，评估基础RAG和高级RAG方法，使用生成器和检索指标评估检索质量和答案生成，并分析性能随对话轮次的变化。

Result: 结果显示稳健但简单的方法（如重排序、混合BM25和HyDE）持续优于基础RAG，而多种高级技术未能带来提升，甚至可能使性能低于无RAG基线。数据集特性和对话长度强烈影响检索效果。

Conclusion: 有效的对话RAG更依赖于检索策略与数据集结构的匹配，而非方法的复杂性。没有单一RAG策略在所有设置中占主导地位。
...
TL;DR: 本文提出了一种用于块扩散语言模型（BDLMs）的测试时缩放统一框架，通过自适应解码和块级生成优化推理速度与效果平衡，在复杂推理任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有块扩散语言模型在测试时缩放方面探索有限，在长链思维推理中面临解码速度与效果平衡的严重挑战，需要更有效的自适应机制来提升推理效率。

Method: 提出统一测试时缩放框架：1) 解码层采用有界自适应置信度解码（BACD），基于模型置信度动态调整去噪过程；2) 块级生成采用"粗思考、细批判"（TCCF）范式，为大块分配探索性推理，小块分配细化任务；3) 采用渐进块大小扩展技术缓解大块尺寸时的性能下降。

Result: 在TDAR-8B模型上应用BACD和TCCF，相比TraDo-8B基线实现了2.26倍加速，在AIME24数据集上提升11.2分，显著改善了推理任务的效率与效果平衡。

Conclusion: 该框架为块扩散语言模型的测试时缩放提供了有效解决方案，通过自适应解码和块级生成策略，在复杂推理任务中实现了速度与精度的双重提升，推动了BDLMs在实际应用中的潜力释放。
...
TL;DR: 该论文提出了LEMUR，一个基于欧盟环境立法的大规模多语言语料库，用于解决LLMs在多语言法律检索中的不可靠性和缺乏领域适配嵌入模型的问题。通过PDF到文本转换的保真度量化，并基于此语料库微调多语言嵌入模型，显著提升了法律检索准确性，特别是在低资源语言上。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在法律信息获取中应用日益广泛，但在多语言法律场景中的部署受到不可靠检索和缺乏领域适配的开放嵌入模型的限制。现有多语言法律语料库并非为语义检索设计，且基于PDF的立法来源由于不完善的文本提取引入了大量噪声。

Method: 1. 构建LEMUR语料库：从24,953份欧盟官方EUR-Lex PDF文档中提取，涵盖25种语言；2. 使用词汇内容评分量化PDF到文本转换的保真度；3. 基于LEMUR语料库，在单语和双语设置下使用对比目标微调三种最先进的多语言嵌入模型。

Result: 法律领域微调相对于强基线持续提升了Top-k检索准确性，特别是在低资源语言上效果显著。跨语言评估显示这些改进能够迁移到未见语言，表明微调主要增强了语言无关、内容层面的法律表示，而非语言特定线索。

Conclusion: LEMUR语料库和相应的微调方法有效解决了多语言法律检索中的挑战，显著提升了检索性能，特别是在低资源语言上。该方法增强了语言无关的法律内容表示，具有很好的跨语言迁移能力。
...
TL;DR: BG-MCTS是一种预算感知的树搜索解码算法，根据剩余token预算动态调整搜索策略，避免传统预算无关方法中的晚期过度分支或过早终止问题


<details>
  <summary>Details</summary>
Motivation: 实际部署中LLM查询有固定的token预算，且不同场景预算不同。现有树搜索解码方法大多预算无关，将预算仅作为终止条件，导致晚期过度分支或过早终止

Method: 提出预算引导的MCTS（BG-MCTS）算法，根据剩余token预算调整搜索策略：开始时广泛探索，随着预算减少优先细化和答案完成，减少浅层节点的晚期分支

Result: 在不同预算下，BG-MCTS在MATH500和AIME24/25数据集上持续优于预算无关的树搜索基线方法，使用开源权重的LLM

Conclusion: 预算感知的树搜索解码策略能更好地适应实际部署约束，在固定token预算下提供更优的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有的反事实数据增强方法在减少语言模型偏见时，可能损害语言建模能力，因为生成的合成数据可能与真实世界分布不一致，或者创建过于简化的反事实而忽略社会背景。

Method: 提出Context-CDA方法：1) 使用大语言模型增强反事实数据的多样性和上下文相关性；2) 通过不确定性过滤排除目标小语言模型认为低质量的生成反事实，提高微调语料质量。

Result: 在性别偏见基准测试中，Context-CDA能有效减轻偏见，同时不牺牲语言建模性能，并通过分析下一个标记生成概率的分布变化提供对社会偏见的洞察。

Conclusion: Context-CDA通过上下文增强和不确定性过滤，在减少语言模型社会偏见的同时保持了语言建模能力，为解决偏见缓解与性能保持之间的平衡问题提供了有效方案。
...
TL;DR: 研究比较了不同长度控制方法对LLM推理效率与性能的影响，发现长度惩罚可能阻碍推理能力，而适当调优的长度控制能提升有强推理先验模型的效率
...
Method: 在Qwen3-1.7B Base和DeepSeek-R1-Distill-Qwen-1.5B两个模型上比较多种长度控制方法，并将先前工作扩展到RL训练策略

Result: 长度惩罚可能阻碍推理能力获取，而适当调优的长度控制能提升有强推理先验模型的效率。识别出两种失败模式：长输出增加分散性，短输出导致思考不足

Conclusion: 对于具有强推理先验的模型，适当调优的长度控制可以在保持性能的同时提升效率，而长度惩罚可能对推理能力获取产生负面影响
...
TL;DR: ELPO（错误定位策略优化）通过二分搜索定位长轨迹中的首个不可恢复错误步骤，利用层次化优势归因生成稳定学习信号，并通过错误定位自适应裁剪强化关键步骤的修正更新，显著提升工具集成推理任务中的强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 在工具集成推理（TIR）场景中，仅基于结果的强化学习面临稀疏延迟奖励和弱步骤级信用分配问题。长轨迹中早期不可恢复错误会决定任务成败，因此需要定位首个不可恢复步骤并进行细粒度信用分配。

Method: 提出ELPO方法：1）通过固定预算下的二分搜索展开树定位首个不可恢复步骤；2）通过层次化优势归因将结果树转换为稳定学习信号；3）应用错误定位自适应裁剪，强化关键步骤及其后缀的修正更新。

Result: 在数学、科学QA和代码执行等TIR基准测试中，ELPO在可比采样预算下持续优于强Agentic RL基线，在Pass@K和Major@K扩展、展开排名质量和工具调用效率方面均有额外提升。

Conclusion: ELPO通过定位首个不可恢复错误步骤并进行细粒度信用分配，有效解决了TIR中强化学习的稀疏奖励和信用分配问题，显著提升了长轨迹任务的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型后训练对齐的工作流程分散在特定后端工具和临时粘合代码中，导致实验难以复现。研究者面临后端干扰、奖励碎片化和不可复现的流程等关键障碍。

Method: AlignTune引入模块化工具包，为监督微调（SFT）和RLHF风格优化提供统一接口，支持可互换的TRL和Unsloth后端。它标准化配置，提供可扩展的奖励层（基于规则和学习的），并集成标准基准和自定义任务的评估。
...
<details>
  <summary>Details</summary>
Motivation: 传统LLM评估方法需要真实标注或评估者协调，成本高且不灵活。需要一种无需参考标注、可扩展且与人类判断对齐的评估框架。

Method: 使用独立提示的评估者集合，基于人类对齐模式指导，支持离散和连续评分。通过任务特定提示（从最佳候选选择、摘要、图像描述到对话）实现灵活评估。

Result: 实验表明该方法与人类判断高度对齐，优于先前方法，减少计算开销，提供高效、稳健且人类对齐的LLM评估解决方案。

Conclusion: MILE-RefHumEval提供了一个无需参考标注、灵活、可扩展且与人类对齐的LLM评估框架，适用于实际应用场景。
...
TL;DR: MATA是一个多智能体表格问答框架，通过多种互补推理路径和小语言模型工具，在保持高性能的同时减少对大语言模型的依赖，提高效率和可扩展性。
...
Method: MATA采用多智能体框架，通过多种推理风格生成候选答案，然后使用小语言模型构建的工具集进行答案精炼或选择，并设计了减少昂贵LLM调用的算法。

Result: 在两个不同难度的基准测试中，使用十种不同LLM的实验表明，MATA实现了最先进的准确率和高效的推理，同时避免了过度的LLM推理。

Conclusion: 精心编排多种推理路径可以实现可扩展和可靠的表格问答，MATA框架在保持高性能的同时显著提高了效率。
...
TL;DR: 本文评估了机器翻译中知识蒸馏方法，同时考虑翻译质量和计算成本（以碳足迹衡量），发现蒸馏开销在小规模部署中占主导，推理在大规模中占主导，词级蒸馏通常比序列级蒸馏提供更好的质量-足迹权衡。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译知识蒸馏研究通常只报告学生模型的翻译质量，而忽略了计算复杂度，这使得在计算资源约束下难以选择合适的蒸馏方法。本研究旨在同时评估翻译质量和计算成本，为实际应用提供指导。

Method: 使用机器学习生命周期评估工具将计算成本量化为碳足迹，考虑整个知识蒸馏模型生命周期（教师训练、蒸馏和推理）中的运行排放和硬件生产成本摊销。评估了代表性的知识蒸馏方法。

Result: 发现：(1) 在小规模部署中，蒸馏开销占总碳足迹的主导地位；(2) 在大规模部署中，推理占主导，使得知识蒸馏只有在超过任务相关的使用阈值后才具有优势；(3) 词级蒸馏通常比序列级蒸馏提供更有利的足迹-质量权衡。

Conclusion: 提出的评估协议为在明确的质量和计算约束下选择知识蒸馏方法提供了可复现的指导，强调了在评估知识蒸馏时需要综合考虑翻译质量和计算成本。
...
TL;DR: 本文提出了一种改进大语言模型方言性能的方法，通过LoRA微调、适配器合并和方言感知MBR解码，在叙利亚、摩洛哥和沙特阿拉伯阿拉伯语方言上取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型正变得越来越多语言化，支持数百种语言（尤其是高资源语言），但方言变体由于数据有限和语言变异而仍然代表性不足。需要改进LLM的方言性能。

Method: 使用低秩适配（LoRA）对单语和英语-方言平行数据进行微调，采用适配器合并和方言感知最小贝叶斯风险（MBR）解码来提高方言保真度生成和翻译。

Result: 在叙利亚、摩洛哥和沙特阿拉伯阿拉伯语方言上的实验表明，合并和MBR方法提高了方言保真度，同时保持了语义准确性。

Conclusion: 这种组合为鲁棒的阿拉伯方言生成提供了一个紧凑而有效的框架，能够改善LLM在方言变体上的表现。
...
TL;DR: TraceMem是一个受认知启发的记忆框架，通过三阶段流水线构建结构化叙事记忆模式，在长对话中实现连贯的叙事理解，在LoCoMo基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在维持长期交互方面存在瓶颈，其有限的上下文窗口难以管理随时间延伸的对话历史。现有记忆系统通常将交互视为离散片段，无法捕捉对话流的底层叙事连贯性。

Method: 提出TraceMem框架，采用三阶段流水线：1) 短期记忆处理：使用演绎式主题分割方法划分情节边界并提取语义表示；2) 突触记忆巩固：将情节总结为情景记忆，然后与语义一起提炼为用户特定痕迹；3) 系统记忆巩固：使用两阶段分层聚类将这些痕迹组织成连贯的、随时间演变的叙事线程，形成结构化用户记忆卡。

Result: 在LoCoMo基准测试中达到最先进性能。分析表明，通过构建连贯叙事，TraceMem在多跳推理和时间推理方面超越基线，突显其在深度叙事理解中的重要作用。

Conclusion: TraceMem通过认知启发的架构构建结构化叙事记忆模式，有效解决了LLMs在长期交互中的记忆瓶颈问题，为深度叙事理解提供了重要支持，并推动了记忆系统领域的发展。
...
TL;DR: 研究人员评估了基于Gemini的AI系统在气候科学协作评估中的应用，发现AI能加速科学工作流程，但专家监督和补充对确保科学严谨性至关重要。


<details>
  <summary>Details</summary>
Motivation: 当前AI科学助手主要适用于可重复验证的任务，但对于需要理论证据综合评估、无法重复验证的复杂科学问题，AI的适用性尚不明确。研究人员希望探索AI在需要专家共识和理论证据综合的科学评估中的支持作用。

Method: 开发了基于Gemini的AI协作科学评估环境，集成到标准科学工作流程中。与13位气候科学家合作，在复杂主题（大西洋经向翻转环流稳定性）上进行测试，分析AI在科学合成报告中的贡献和局限性。

Result: AI显著加速了科学工作流程：团队在46人时内完成了79篇论文的综合分析，经过104次修订周期。AI生成的大部分内容被保留在报告中，帮助维持逻辑一致性和呈现质量。但专家补充至关重要：不到一半的报告由AI生成，需要大量监督才能将内容提升到严谨科学标准。

Conclusion: AI可以加速科学工作流程，特别是在内容生成、逻辑一致性和呈现质量方面。然而，对于需要理论证据综合的复杂科学评估，专家监督和补充不可或缺，AI不能完全替代人类科学家的专业判断和严谨性要求。
...
Method: 从12个在线圣经图书馆和一个现有语料库中收集数据，包含657个新约译本，对每个翻译进行手动元数据标注，包括作品标准化标识符、具体版本和修订年份。

Result: 创建了包含208个独特英语版本、41个法语版本、18个意大利语版本、30个波兰语版本和55个西班牙语版本的多语言语料库，支持从微观翻译家族分析到宏观去重研究的灵活多层次分析。

Conclusion: 该语料库为翻译历史的定量研究设立了新基准，是首个支持灵活多层次分析的研究资源。
...
TL;DR: 提出一种将词嵌入映射到神经生物学特征空间的方法，提高词汇语义变化的可解释性，同时保持优越的LSC估计性能


<details>
  <summary>Details</summary>
Motivation: 现有词汇语义变化研究主要关注变化程度的估计，但难以解释词义如何变化。提高LSC的可解释性是该领域的重要挑战，可能带来新的研究洞见。

Method: 将预训练语言模型得到的上下文词嵌入映射到神经生物学特征空间。该空间的每个维度对应词的基本特征，值表示该特征的强度，使人类能够系统性地解释LSC。

Result: 在LSC程度估计方面，该方法优于大多数先前方法。高可解释性使得能够进行多种LSC分析，发现了先前研究中被忽视的有趣LSC类型，并能有效搜索具有特定类型LSC的词汇。

Conclusion: 提出的方法不仅提高了词汇语义变化估计的性能，更重要的是增强了可解释性，为系统分析LSC类型和发现新的语义变化模式提供了有效工具。
...
TL;DR: 首个班巴拉语ASR标准化基准测试：使用1小时专业录制的马里宪法文本，评估37个模型，结果显示当前ASR性能远低于部署标准，最佳WER为46.76%，最佳CER为13.00%，表明多语言预训练和模型缩放对低资源语言不足。


<details>
  <summary>Details</summary>
Motivation: 为班巴拉语（一种低资源语言）建立首个标准化的自动语音识别评估基准，以客观衡量当前ASR模型在该语言上的性能，促进班巴拉语语音技术的发展。

Method: 创建包含1小时专业录制马里宪法文本的标准化基准数据集，在接近最优的声学和语言条件下设计。使用该基准评估37个模型，包括班巴拉语训练系统和大型商业模型，采用词错误率（WER）和字符错误率（CER）作为评估指标。

Result: 当前ASR性能远低于部署标准：最佳WER为46.76%，最佳CER为13.00%。多个知名多语言模型的WER超过100%。这表明多语言预训练和模型缩放对班巴拉语等低资源语言不足。由于数据集代表最简化的正式口语形式，实际场景性能可能更差。

Conclusion: 多语言预训练和模型缩放不足以支持低资源语言的ASR性能。需要针对性的语言特定方法。提供的基准和公开排行榜将促进班巴拉语语音技术的透明评估和未来发展。
...
TL;DR: 论文提出了一种可追踪的框架来评估大语言模型的推理效率，将token效率分解为可解释的因素，包括完成度、条件正确性和冗余度，并通过25个模型的评估发现准确率与token效率排名存在差异。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型推理评估只关注最终准确率，忽视了token使用效率，无法了解token在何处被使用或浪费。需要一种能够分解token效率的评估框架来揭示模型推理过程中的效率瓶颈。

Method: 提出了一个可追踪的框架，将token效率分解为：1) 固定token预算下的完成度；2) 给定完成的条件正确性；3) 冗余度。当有基准元数据时，进一步将冗余度分解为平均语言化开销和耦合系数。当有推理轨迹时，添加确定性轨迹质量度量来区分退化循环和冗长但投入的推理。

Result: 在CogniLoad基准上评估25个模型，发现准确率和token效率排名存在差异（Spearman ρ=0.63），效率差距通常由条件正确性驱动，语言化开销变化约9倍（与模型规模关系较弱）。分解揭示了不同的瓶颈模式，提示需要不同的效率干预措施。

Conclusion: 提出的框架能够深入分析大语言模型的推理效率，揭示传统准确率评估所忽视的效率瓶颈，为模型优化提供有针对性的指导方向。
...
<details>
  <summary>Details</summary>
Motivation: 科学计量问答作为科学学领域的下游任务，相比传统科学问答面临独特挑战：需要识别学术实体名称、检索多维度科学计量指标（如影响因子）。大语言模型在处理复杂任务如任务分解、规划和推理方面展现出潜力，但在此特定领域的应用尚不充分。

Method: 开发了一个端到端系统，采用顺序工作流，结合检索增强生成和智能体概念。使用专有的研究绩效评估平台作为检索数据库，通过大语言模型进行任务分解、规划和推理，并有效合成数据生成结构化的高级分析。

Result: 通过咨询经验丰富的领域专家和采用"大语言模型作为评判者"的方法进行评估，为这一小众下游任务中大语言模型的有效性提供了有价值的见解。代码和提示已开源。

Conclusion: AnalyticsGPT展示了将大语言模型应用于科学计量问答的可行性，为处理科学学领域的元科学问题提供了一个直观高效的工作流，并验证了大语言模型在这一特定领域的应用潜力。
...
TL;DR: 研究阿拉伯语言模型在标准阿拉伯语和方言之间的跨语言迁移能力，发现迁移存在但不均衡，受地理邻近性影响，且存在负向干扰


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语言模型主要基于现代标准阿拉伯语（MSA）预训练，但实际使用中人们使用各种方言，而方言与MSA的相似度不同，这限制了阿拉伯语言模型的应用效果

Method: 通过3个自然语言处理任务的探测分析和表征相似性研究，分析阿拉伯语言模型的跨语言迁移能力

Result: 跨语言迁移是可能的，但在不同方言间不均衡，部分可由地理邻近性解释；发现支持所有阿拉伯方言的模型存在负向干扰

Conclusion: 阿拉伯方言的相似度值得质疑，对阿拉伯语言模型的跨语言迁移能力提出了担忧
...
TL;DR: LLM生成的推理可用于预测模型自身预测的正确性，在教育对话分析中实现83% F1分数，为自动标注提供质量控制方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在教育对话自动标注中缺乏可靠的错误检测方法，需要研究如何利用模型自身生成的推理来预测其预测的正确性。

Method: 分析30,300个教师话语，由多个先进LLM标注教学行为并生成推理。使用TF-IDF编码推理，评估五种监督分类器，并针对特定教学行为训练专门检测器。使用LIWC框架分析正确与错误推理的语言特征差异。

Result: 随机森林分类器达到0.83 F1分数（召回率0.854），成功识别大多数错误预测。正确预测使用基于因果的语言，错误推理更多依赖认知模糊和元认知表达。句法复杂性和推理长度不能区分正确性。

Conclusion: 基于推理的错误检测为自动教育对话分析提供了实用且可扩展的质量控制方法，特定教学行为的专门检测器能进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 量化是日常语言的基本组成部分，但人们对说话者在自然表达中如何决定是否以及如何进行量化知之甚少。本研究旨在探究汉语使用者在无约束条件下的量化行为

Method: 使用基于图片的诱发描述任务，让被试自由描述包含多个物体的场景，不明确要求计数或量化。同时考察口语和书面语两种表达模态

Result: 物体数量增加会降低量化的可能性和精确度；生命性指称和表达模态选择性地调节量化策略选择；物体数量、生命性和表达模态系统性地塑造量化行为

Conclusion: 本研究展示了如何在无约束生产条件下考察量化行为，为语言生产中的数量表达分析提供了自然主义数据集
...
TL;DR: 该研究构建了一个包含2,344条僧伽罗语习语的数据集，包含文化和跨语言标注，用于评估LLM在低资源语言习语翻译中的表现，发现现有模型存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在高资源语言的比喻表达上表现良好，但在低资源语言如僧伽罗语中因数据有限而面临挑战，特别是处理与文化紧密相关的习语时。

Method: 构建了包含2,344条僧伽罗语习语的数据集，包含文化和跨语言标注；开发了二元分类器区分两种习语类型；评估了现有LLM在该数据集上的表现。

Result: 二元分类器达到约92%的准确率；现有LLM在准确传达习语含义方面存在显著不足，经常难以正确处理习语的翻译。

Conclusion: 该数据集为低资源NLP和文化感知机器翻译研究提供了重要基准，揭示了当前LLM在处理低资源语言习语方面的局限性。
...
TL;DR: Steer2Edit：一种无需训练的理论框架，将推理时控制信号转换为诊断信号，通过组件级权重编辑实现更优的属性-效用权衡


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型引导方法通过在推理时对隐藏表示应用固定的全局修改来影响模型行为，但这种方法往往导致不利的属性-效用权衡，因为它忽略了大多数行为由少量异质模型组件控制的事实。

Method: Steer2Edit将引导向量从推理时控制信号转换为诊断信号，用于组件级（注意力头和MLP神经元）的秩-1权重编辑。它选择性地重新分配行为影响，而不是在生成过程中统一注入引导方向。

Result: 在安全对齐、幻觉缓解和推理效率方面，Steer2Edit实现了更优的属性-效用权衡：在保持下游性能的同时，安全性提升高达17.2%，真实性提高9.8%，推理长度平均减少12.2%。

Conclusion: Steer2Edit通过将引导信号转换为可解释的、无需训练的参数更新，为表示引导和权重编辑之间建立了原则性桥梁。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM多智能体系统在实现可扩展集体智能和自我进化时面临的安全挑战，特别是探索是否能在完全封闭循环中实现持续自我改进同时保持安全对齐，即所谓的"自我进化三难困境"。

Method: 采用信息论框架，将安全形式化为与人类价值分布的偏离程度，从理论上证明完全隔离的自我进化会导致统计盲点；通过开放智能体社区(Moltbook)和两个封闭自进化系统的实证和定性研究验证理论预测。

Result: 理论和实证均表明，同时满足持续自我进化、完全隔离和安全不变性的智能体社会是不可能的；隔离的自进化会导致统计盲点，引发系统安全对齐的不可逆退化。

Conclusion: 研究确立了自进化AI社会的根本限制，将讨论从症状驱动的安全补丁转向对内在动态风险的原则性理解，强调需要外部监督或新的安全保护机制。
...
TL;DR: 论文研究如何利用LLM内部表征预测其自身推理成功率，以指导更高效的计算资源分配，在数学和编程任务上实现高达70%的成本节省。


<details>
  <summary>Details</summary>
Motivation: 运行LLM进行扩展推理成本高昂，但难以确定哪些输入真正需要额外计算。研究探索是否可以从生成前的内部表征中恢复模型自身的成功可能性，并利用这一信号指导更高效的推理。

Method: 在生成前激活上训练线性探针，预测特定策略在数学和编程任务上的成功率；使用E2H-AMC数据集比较模型特定难度与人类难度；利用探针在模型池中进行查询路由。

Result: 线性探针显著优于表面特征（如问题长度和TF-IDF）；模型编码了与人类难度不同的模型特定难度概念，且这种差异随扩展推理增加；路由查询可在MATH任务上减少高达70%的推理成本，同时超越最佳单模型性能。

Conclusion: LLM内部表征能够预测模型特定难度，即使这些表征与人类对难度的直觉不同，仍可实现实际效率提升，为高效推理系统设计提供了新途径。
...
TL;DR: ATTNPO：基于注意力信号的轻量级过程监督RL框架，通过识别特殊注意力头区分必要与冗余推理步骤，有效减少过度思考同时保持准确性


<details>
  <summary>Details</summary>
Motivation: 现有RLVR模型存在过度思考问题，生成冗余推理步骤而不提升性能。轨迹级长度惩罚方法无法有效缩短推理长度且会降低准确性，因为它们对所有推理步骤一视同仁，缺乏区分冗余与必要步骤的细粒度信号。过程监督方法通常资源密集且存在信用分配不准确的问题。

Method: 提出ATTNPO框架：1）识别一组特殊的注意力头，这些头天然关注必要步骤同时抑制冗余步骤；2）利用这些注意力头的注意力分数，采用两种子策略：通过惩罚冗余步骤来减少过度思考，同时减少对必要步骤的惩罚以保持准确性。

Result: 实验结果显示，ATTNPO在9个基准测试上显著减少了推理长度，同时显著提升了性能。

Conclusion: ATTNPO通过利用模型内在的注意力信号进行步骤级信用分配，有效解决了RLVR模型中的过度思考问题，实现了推理长度减少与性能提升的双重目标。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多选阅读理解模型通常缺乏解释其选择背后推理的能力，需要开发既能回答问题又能生成解释的模型

Method: 提出ViMultiChoice方法，专门为越南语阅读理解设计，联合训练选项决策和解释生成任务

Result: ViMultiChoice在ViMMRC 2.0基准和新数据集上均优于现有基线，达到最先进性能；联合训练显著提高了多选准确性

Conclusion: 联合训练选项决策和解释生成能有效提升越南语阅读理解模型的性能，为可解释性多选阅读理解提供了有效解决方案
...
### [125] [ViSpeechFormer: A Phonemic Approach for Vietnamese Automatic Speech Recognition](https://arxiv.org/abs/2602.10003)
*Khoa Anh Nguyen,Long Minh Hoang,Nghia Hieu Nguyen,Luan Thanh Nguyen,Ngan Luu-Thuy Nguyen*

Main category: cs.CL

TL;DR: ViSpeechFormer是首个基于音素的越南语语音识别框架，利用越南语高度透明的字形-音素对应关系，在公开数据集上表现出色，泛化能力更强，受训练偏差影响更小。


<details>
  <summary>Details</summary>
Motivation: 越南语具有高度透明的正字法，每个字形对应最多一个音素，反之亦然。利用这种高透明度，研究者希望开发首个明确建模音素表示的越南语语音识别框架。

Method: 提出ViSpeechFormer（越南语音转换器），这是一个基于音素的越南语自动语音识别方法。该方法利用越南语的字形-音素对应关系，直接建模音素表示。
...
### [126] [SCORE: Specificity, Context Utilization, Robustness, and Relevance for Reference-Free LLM Evaluation](https://arxiv.org/abs/2602.10017)
*Homaira Huda Shomee,Rochana Chaturvedi,Yangxinyu Xie,Tanwi Mallick*

Main category: cs.CL

TL;DR: 本文提出了一个多维度的无参考评估框架，用于评估大语言模型在特定领域高风险应用中的回答质量，重点关注特异性、鲁棒性、相关性和上下文利用四个维度。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成和开放式问答的评估框架主要依赖表面相似性、事实一致性或语义相关性，无法评估模型是否提供了领域敏感决策所需的具体信息，特别是在自然灾害响应和基础设施规划等高风险领域。

Method: 提出了一个多维度的无参考评估框架，从四个互补维度评估LLM输出：特异性、对释义和语义扰动的鲁棒性、答案相关性、上下文利用。创建了包含1,412个领域特定问答对的数据集，涵盖40种专业角色和7种自然灾害类型，并进行人工评估以验证框架有效性。

Result: 研究结果表明，没有任何单一指标能够独立充分捕捉答案质量，强调了在部署LLM到高风险应用时需要结构化、多指标评估框架的重要性。人工评估显示了领域特定评估中固有的主观性。
...
### [127] [Decoupled Reasoning with Implicit Fact Tokens (DRIFT): A Dual-Model Framework for Efficient Long-Context Inference](https://arxiv.org/abs/2602.10021)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如RAG和知识编辑）在实际应用中受到有限上下文窗口、检索器噪声或灾难性遗忘的限制，LLMs中广泛动态知识的集成仍然是一个重大挑战。

Method: DRIFT采用双模型架构：1）轻量级知识模型动态压缩文档块为隐式事实标记；2）将这些密集表示投影到推理模型的嵌入空间，替代原始冗余文本；3）保持推理准确性同时减少上下文负担。

Result: 大量实验表明，DRIFT在长上下文任务上显著提升性能，在同等规模模型中优于强基线方法，提供了可扩展且高效的范式。

Conclusion: DRIFT通过解耦知识提取与推理过程，为扩展LLMs的有效上下文窗口和推理能力提供了创新解决方案，代码已开源。
...
### [128] [MEVER: Multi-Modal and Explainable Claim Verification with Graph-based Evidence Retrieval](https://arxiv.org/abs/2602.10023)
*Delvin Ce Zhang,Suhan Cui,Zhelin Chu,Xianren Zhang,Dongwon Lee*

Main category: cs.CL

TL;DR: 该论文提出了一种联合实现证据检索、多模态声明验证和解释生成的新模型，特别针对需要结合文本和视觉证据进行验证的场景，并创建了AI领域的科学数据集AIChartClaim。


<details>
  <summary>Details</summary>
Motivation: 声明真实性验证通常需要结合文本和视觉证据进行多模态推理，同时为了增加推理过程的透明度，需要生成文本解释来证明验证结果。然而，现有工作大多只关注文本证据推理或忽略可解释性，导致验证不准确且缺乏说服力。

Method: 1. 证据检索：构建双层多模态图，设计图像到文本和文本到图像的推理进行多模态检索；2. 声明验证：提出token级和证据级融合，整合声明和证据嵌入进行多模态验证；3. 解释生成：引入多模态Fusion-in-Decoder实现可解释性；4. 创建AIChartClaim数据集补充科学领域验证。

Result: 实验结果表明该模型具有优势，能够有效处理多模态声明验证任务。

Conclusion: 该研究提出的联合模型能够同时处理证据检索、多模态声明验证和解释生成，解决了现有方法在多模态推理和可解释性方面的不足，并通过创建AI领域数据集丰富了声明验证社区的资源。
...
### [129] [Anagent For Enhancing Scientific Table & Figure Analysis](https://arxiv.org/abs/2602.10081)
*Xuehang Guo,Zhiyong Lu,Tom Hope,Qingyun Wang*

Main category: cs.CL

TL;DR: 该论文提出了AnaBench基准测试和Anagent多智能体框架，用于解决科学表格和图表分析的挑战。AnaBench包含63,178个实例，涵盖9个科学领域和7个复杂度维度。Anagent通过四个专门智能体（规划者、专家、求解者、批评者）协同工作，在训练免费设置中提升13.43%，微调后提升42.12%。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在科学表格和图表分析方面存在困难，这些图表具有复杂性、变异性、异构结构和长上下文要求等挑战。需要量化这些挑战并开发有效的解决方案来提升科学知识的多模态分析能力。

Method: 1) 提出AnaBench基准测试，包含63,178个实例，涵盖9个科学领域，按7个复杂度维度系统分类；2) 提出Anagent多智能体框架，包含四个专门智能体：规划者（任务分解）、专家（工具执行检索）、求解者（信息合成分析）、批评者（五维质量评估迭代优化）；3) 开发模块化训练策略，结合监督微调和专门强化学习来优化个体能力并保持有效协作。

Result: 在170个子领域的全面评估中，Anagent在训练免费设置中实现了高达13.43%的提升，经过微调后提升达到42.12%。研究揭示了面向任务的推理和上下文感知问题解决对高质量科学表格和图表分析至关重要。

Conclusion: 该研究通过AnaBench基准测试量化了科学表格和图表分析的挑战，并通过Anagent多智能体框架有效解决了这些问题。多智能体协作、模块化训练和迭代优化是实现高质量科学分析的关键要素，为科学AI系统的发展提供了重要参考。
...
### [130] [Quantum-Audit: Evaluating the Reasoning Limits of LLMs on Quantum Computing](https://arxiv.org/abs/2602.10092)
*Mohamed Afane,Kayla Laufer,Wenqi Wei,Ying Mao,Junaid Farooq,Ying Wang,Juntao Chen*

Main category: cs.CL

TL;DR: Quantum-Audit是一个包含2700个问题的量子计算概念理解基准测试，评估了26个模型，发现顶级模型在专家编写问题上表现较差，且经常接受错误前提而非纠正它们。
...
Method: 创建了2700个问题的基准测试，包括1000个专家编写问题、1000个通过LLM从研究论文中提取并经过专家验证的问题，以及700个额外问题（350个开放式问题和350个包含错误前提的问题）。评估了26个领先组织的模型。

Result: 人类参与者得分在23%到86%之间，专家平均74%。顶级模型Claude Opus 4.5达到84%准确率，但在专家编写问题上比LLM生成问题平均低12个百分点。在安全等高级主题上表现下降至73%。模型经常接受和强化错误前提而非识别它们，在这些关键推理任务上准确率低于66%。

Conclusion: 虽然顶级模型在量子计算概念理解上超过了专家平均水平，但在专家编写问题、高级主题和识别错误前提方面存在显著弱点，表明需要更深入的概念理解和批判性推理能力。
...
TL;DR: Cadmus系统通过小型模型和整数虚拟机，以低成本研究程序合成，解决了大语言模型在分布外表示、归纳推理等问题上的局限性，并在特定任务上超越GPT-5。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在程序合成研究中存在多个问题：难以确定分布内外数据、微调效果不明确、分词影响复杂、计算存储成本高。需要一种更透明、可控且低成本的研究平台。

Method: 开发Cadmus系统，包含整数虚拟机、多样化真实程序数据集，以及训练成本低于200美元的自回归Transformer模型。系统允许研究者精细控制训练分布，并能检查和插装模型。

Result: Cadmus模型在完成整数算术程序任务上达到100%准确率，优于GPT-5的95%。系统为研究程序补全、分布外表示、归纳推理等提供了透明可控的平台，同时揭示了GPT-5在解决相同任务时引入了未知先验知识的问题。

Conclusion: 小型模型在复杂推理任务上具有研究价值，提供了大模型难以实现的透明度和可控性。Cadmus系统为程序合成研究提供了经济高效且可解释的实验平台，特别适用于需要完全理解训练集与任务关系的研究场景。
...
TL;DR: 提出了一种轻量级、保护隐私的多模态情感识别框架，适用于边缘设备部署，使用语音、文本和面部图像三种模态，通过Dempster-Shafer理论和Dirichlet证据实现模型和任务无关的融合机制。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在边缘设备上部署的轻量级、保护隐私的多模态情感识别系统，解决实际应用中计算资源有限、数据隐私保护需求以及多模态融合中的不确定性处理问题。

Method: 使用三种模态处理：Emotion2Vec处理语音、ResNet处理面部表情、DistilRoBERTa处理文本；提出基于Dempster-Shafer理论和Dirichlet证据的模型和任务无关融合机制，直接在模型logits上操作，无需额外训练或联合分布估计。
...
Method: 提出Progress-Aware Belief Update (PABU)框架，通过显式建模任务进度，在每个步骤中预测相对进度并决定是否保留新遇到的交互信息，仅基于保留的子集进行未来决策。

Result: 在AgentGym基准测试的八个环境中，PABU实现了81.0%的任务完成率，比基于完整历史信念的SOTA模型提高了23.9%。平均交互步骤减少到9.5步，降低了26.9%。消融研究表明进度预测和选择性保留都是必要的。

Conclusion: PABU框架通过显式建模任务进度和选择性信息保留，有效减少了LLM智能体中的冗余，显著提高了任务完成效率和性能，为高效的信念状态学习提供了新方法。
...
### [134] [CoMMa: Contribution-Aware Medical Multi-Agents From A Game-Theoretic Perspective](https://arxiv.org/abs/2602.09159)
...
<details>
  <summary>Details</summary>
Motivation: 当前的多智能体框架在处理需要基于动态、异质患者数据进行推理的肿瘤学决策支持任务时，大多依赖随机叙事推理，缺乏明确的证据归因和数学基础，导致决策路径不够稳定和可解释。

Method: 提出贡献感知医疗多智能体(CoMMa)框架，采用去中心化架构，各专家智能体在分区证据上操作，通过博弈论目标进行协调，使用确定性嵌入投影来近似贡献感知的信用分配，估计每个智能体的边际效用。

Result: 在多样化的肿瘤学基准测试（包括真实世界多学科肿瘤委员会数据集）中，CoMMa相比数据集中化和基于角色的多智能体基线，实现了更高的准确性和更稳定的性能。

Conclusion: CoMMa框架通过贡献感知的信用分配机制，能够产生可解释且数学基础坚实的决策路径，在肿瘤学决策支持任务中表现出优越的性能和稳定性。
...
### [135] [FlyAOC: Evaluating Agentic Ontology Curation of Drosophila Scientific Knowledge Bases](https://arxiv.org/abs/2602.09163)
*Xingjian Zhang,Sophia Moylan,Ziyang Xiong,Qiaozhu Mei,Yichen Luo,Jiaqi W. Ma*

Main category: cs.AI

TL;DR: FlyBench是一个评估AI智能体从科学文献中进行端到端本体论知识库构建的基准测试，要求AI根据基因符号从16,898篇论文中搜索、阅读并生成结构化注释，包括基因功能、表达模式和历史同义词等。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试主要关注命名实体识别或关系抽取等孤立子任务，无法评估AI智能体在科学知识库构建中的端到端工作流程。科学知识库需要专家从文献中搜索、整合证据并生成基于本体的注释，这一完整流程需要专门的评估基准。

Method: 提出了FlyBench基准测试，包含100个基因的7,397个专家注释，要求AI智能体仅基于基因符号从16,898篇全文论文中搜索、阅读并生成结构化注释。评估了四种基线智能体架构：记忆化、固定流水线、单智能体和多智能体架构。

Result: 架构选择显著影响性能，多智能体设计优于简单替代方案，但扩展骨干模型带来的收益递减。所有基线仍有很大改进空间。分析发现智能体主要使用检索来确认参数化知识而非发现新信息。

Conclusion: FlyBench将推动检索增强的科学推理能力发展，这种能力在科学领域有广泛应用前景。基准测试揭示了未来发展的指导性发现，为AI在科学知识库构建中的端到端评估提供了重要工具。
...
### [136] [Measuring Dataset Diversity from a Geometric Perspective](https://arxiv.org/abs/2602.09340)
*Yang Ba,Mohammad Sadeq Abolhasani,Michelle V Mancenido,Rong Pan*

Main category: cs.AI

TL;DR: 该论文提出了一种基于拓扑数据分析(TDA)和持久性景观(PLs)的新框架，用于从数据中提取和量化几何特征，从而超越传统熵度量，捕捉数据集的几何结构多样性。


<details>
  <summary>Details</summary>
Motivation: 现有多样性度量（如特征空间离散度和度量空间幅度）主要捕捉分布变化或熵，但很大程度上忽略了数据集的几何结构。为了填补这一空白，需要一种能够捕捉数据集丰富几何和结构特性的多样性度量方法。

Method: 引入基于拓扑数据分析和持久性景观的框架，从数据中提取和量化几何特征。该方法通过持久性景观来捕捉数据集的拓扑特征，提供理论上有依据的多样性测量手段。

Result: 通过跨多种模态的广泛实验，证明提出的PLs-based多样性度量(PLDiv)是强大、可靠且可解释的。该度量能够直接将数据多样性与其底层几何结构联系起来。

Conclusion: PLDiv为数据集构建、增强和评估提供了一个基础工具，能够超越传统熵度量，捕捉数据集的几何和结构特性，为多样性测量提供了新的视角。
...
### [137] [Auditing Multi-Agent LLM Reasoning Trees Outperforms Majority Vote and LLM-as-Judge](https://arxiv.org/abs/2602.09341)
*Wei Yang,Shixuan Li,Heng Ping,Peiyu Zhang,Paul Bogdan,Jesse Thomason*

Main category: cs.AI

TL;DR: AgentAuditor用推理树路径搜索替代多数投票，通过局部验证解决多智能体系统中的偏见共识问题，结合ACPO训练提升少数正确意见识别能力


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统大多采用多数投票聚合智能体输出，这种方法丢弃了推理轨迹的证据结构，且在智能体共享相关偏见并收敛到相同错误推理的"幻觉共识"情况下表现脆弱

Method: 提出AgentAuditor框架，用推理树路径搜索替代投票，显式表示智能体轨迹间的共识与分歧；在关键分歧点比较推理分支，将全局裁决转化为高效的局部验证；进一步提出反共识偏好优化(ACPO)，在多数失败案例上训练裁决器，奖励基于证据的少数选择而非流行错误

Result: 在5种流行设置中，AgentAuditor相比多数投票获得高达5%的绝对准确率提升，相比使用LLM-as-Judge提升高达3%

Conclusion: AgentAuditor通过显式建模推理分歧和局部验证机制，有效解决了多智能体系统中的偏见共识问题，显著提升了推理准确率，且与具体MAS设置无关
...
### [138] [Image Quality in the Era of Artificial Intelligence](https://arxiv.org/abs/2602.09347)
*Jana G. Delfino,Jason L. Granstedt,Frank W. Samuelson,Robert Ochs,Krishna Juluru*

Main category: cs.AI

TL;DR: AI在放射学图像重建和增强中应用迅速，能提升图像质量、加速采集和阅片，但也引入新的失败模式，可能加剧图像感知质量与信息内容之间的脱节，需要了解其局限性以实现安全有效使用。


<details>
  <summary>Details</summary>
Motivation: AI在放射学中快速部署，虽然能显著提升图像质量和工作效率，但也带来了新的风险，特别是图像感知质量与信息内容之间的脱节问题。为确保AI技术的安全有效应用，需要提高对其局限性的认识。

Method: 本文是一篇通讯文章，主要通过文献综述和专家分析的方式，探讨AI在放射学图像重建和增强应用中的局限性，旨在提高用户对相关风险的认识。

Result: AI确实能够重建和增强放射学图像，使其更清晰、平滑、详细，加速图像采集和临床阅片过程。但同时，AI的引入也带来了新的失败模式，可能掩盖图像中的关键信息，导致临床决策风险。

Conclusion: 理解AI图像重建和增强的局限性对于安全有效使用该技术至关重要。用户需要在享受AI技术带来的益处的同时，充分认识并最小化相关风险，这需要持续的教育和意识提升。
...
### [139] [Bridging Efficiency and Transparency: Explainable CoT Compression in Multimodal Large Reasoning Models](https://arxiv.org/abs/2602.09485)
*Yizhi Wang,Linan Yue,Min-Ling Zhang*

Main category: cs.AI

TL;DR: XMCC：一种可解释的多模态思维链压缩器，通过强化学习优化压缩过程，在保持推理完整性的同时缩短推理轨迹，并生成自然语言解释。


<details>
  <summary>Details</summary>
Motivation: 当前多模态推理模型中使用的长思维链存在冗余推理步骤，影响推理效率。现有压缩方法面临两个主要挑战：可能破坏视觉-文本推理的完整性，以及压缩过程缺乏可解释性。
...
Conclusion: XMCC成功解决了长思维链压缩中的两个关键挑战，在保持推理完整性的同时提高效率，并通过可解释性增强了压缩过程的透明度。
...
### [140] [Computing Conditional Shapley Values Using Tabular Foundation Models](https://arxiv.org/abs/2602.09489)
*Lars Henry Berge Olsen,Dennis Christensen*

Main category: cs.AI

TL;DR: 使用TabPFN等表格基础模型高效计算Shapley值，相比传统方法在性能和速度上都有显著优势
...
Method: 利用TabPFN等表格基础模型的上下文学习能力，无需重新训练即可近似每个条件期望，从而高效计算Shapley值，并与最先进方法在模拟和真实数据集上进行比较

Result: 在大多数情况下，TabPFN表现最佳；即使不是最佳，也仅略逊于最优方法，但运行时间仅为后者的极小部分

Conclusion: 表格基础模型为Shapley值计算提供了高效解决方案，未来可通过进一步优化使其更好地适应条件Shapley值估计任务
...
### [141] [FLINGO -- Instilling ASP Expressiveness into Linear Integer Constraints](https://arxiv.org/abs/2602.09620)
*Jorge Fandinno,Pedro Cabalar,Philipp Wanko,Torsten Schaub*

Main category: cs.AI

TL;DR: FLINGO语言扩展了约束ASP，允许在数值约束中使用ASP风格的表达特性，如默认值、未定义属性、非确定性赋值和聚合值。


<details>
  <summary>Details</summary>
Motivation: 当前约束ASP（CASP）中数值约束的表达方式更接近后端数值处理器的语义，而不是标准ASP的表达方式。在标准ASP中，数值属性可以用谓词表示，支持默认值、未定义属性、非确定性赋值和聚合值等特性，但这些特性在CASP的约束表示中大多丢失了。

Method: 提出了FLINGO语言和工具，将ASP风格的表达特性融入数值约束中。基于先前建立的语义基础，提供了从FLINGO语法到常规CASP程序（遵循CLINGCON输入格式）的翻译方法。

Result: FLINGO语言成功地将ASP的表达特性整合到数值约束中，通过多个示例展示了其使用方式，并提供了到标准CASP格式的翻译机制。

Conclusion: FLINGO语言填补了约束ASP中表达能力的空白，使得在数值约束中能够使用ASP风格的丰富表达特性，提高了CASP在实际应用中的实用性和表达能力。
...
### [142] [ClinAlign: Scaling Healthcare Alignment from Clinician Preference](https://arxiv.org/abs/2602.09653)
*Shiwei Lyu,Xidong Wang,Lei Liu,Hao Zhu,Chaohe Zhang,Jian Wang,Jinjie Gu,Benyou Wang,Yue Shen*

Main category: cs.AI

TL;DR: 本文提出一个两阶段框架解决LLM医疗输出与临床医生偏好对齐的挑战：首先创建HealthRubrics数据集（7,034个医生验证的偏好示例），然后提炼为HealthPrinciples（119个可重用临床原则），用于离线对齐和推理时自我修订，使30B参数模型在仅激活3B参数时在HealthBench-Hard上达到33.4%的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出专家级医学知识，但将其开放输出与细粒度临床医生偏好对齐仍然困难。现有方法通常依赖粗粒度目标或不可靠的自动评估，这些评估与专业指南的关联较弱。

Method: 提出两阶段框架：1）创建HealthRubrics数据集，包含7,034个医生验证的偏好示例，临床医生在此过程中精炼LLM起草的评估标准以满足严格医学标准；2）将这些标准提炼为HealthPrinciples：119个广泛可重用、基于临床的原则，按临床维度组织，实现超越手动标注的可扩展监督。使用HealthPrinciples进行：a）离线对齐，为未标注查询合成评估标准；b）推理时工具，用于引导自我修订。

Result: 使用该框架训练的30B参数模型（推理时仅激活3B参数）在HealthBench-Hard上达到33.4%的性能，超越了包括Deepseek-R1和o3在内的更大模型，为临床对齐建立了资源高效的基准。

Conclusion: 该研究通过创建医生验证的数据集和提炼可重用临床原则，解决了LLM医疗输出与临床偏好对齐的挑战，实现了资源高效的临床对齐，为医疗AI应用提供了更可靠的监督框架。
...
### [143] [GHS-TDA: A Synergistic Reasoning Framework Integrating Global Hypothesis Space with Topological Data Analysis](https://arxiv.org/abs/2602.09794)
...
TL;DR: GHS-TDA通过构建全局假设图和多尺度拓扑分析，解决传统思维链方法中早期错误传播和缺乏结构化分析的问题，提升推理的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统思维链方法存在两个根本性局限：1）推理过程对早期决策高度敏感，一旦引入初始错误会传播放大且难以纠正；2）缺乏结构化分析技术来过滤冗余推理和提取关键特征，导致推理过程不稳定且可解释性有限。
...
Conclusion: GHS-TDA通过全局协调和拓扑稳定性分析，有效解决了传统思维链方法的局限性，显著提升了大型语言模型在复杂任务上的推理性能。
...
### [144] [Would a Large Language Model Pay Extra for a View? Inferring Willingness to Pay from Subjective Choices](https://arxiv.org/abs/2602.09802)
*Manon Reusens,Sofie Goethals,Toon Calders,David Martens*

Main category: cs.AI

TL;DR: 研究通过旅行选择困境测试LLM的主观决策能力，使用多项logit模型推导隐含支付意愿，并与人类基准比较，发现大模型能产生有意义的WTP值但存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在旅行助手等应用中部署，它们需要在没有客观正确答案的情况下为用户做出主观选择。研究旨在评估LLM在这种主观决策环境中的表现，特别是与人类决策的对比。

Method: 在旅行助手情境中向模型呈现选择困境，使用多项logit模型分析响应以推导隐含支付意愿估计，比较人类经济学文献中的基准值。研究基线设置，并考察在更现实条件下（包括提供用户过去选择信息和基于角色的提示）模型行为的变化。

Result: 较大LLM能够产生有意义的WTP值，但在属性层面显示系统性偏差；总体倾向于高估人类WTP，特别是在引入昂贵选项或商务导向角色时；当模型基于先前对更便宜选项的偏好进行条件化时，估值更接近人类基准。
...
### [145] [Closing Reasoning Gaps in Clinical Agents with Differential Reasoning Learning](https://arxiv.org/abs/2602.09945)
*Jinsong Liu,Yuhang Jiang,Ramayya Krishnan,Rema Padman,Yiye Zhang,Jiang Bian*

Main category: cs.AI

TL;DR: DRL框架通过分析推理差异来提升临床决策支持系统的推理质量，使用图编辑距离诊断推理差异并构建知识库，在推理时检索相关指令来修正逻辑漏洞。


<details>
  <summary>Details</summary>
Motivation: 临床决策支持不仅需要正确答案，还需要临床有效的推理过程。现有方法在复杂临床推理场景中可能存在逻辑漏洞，需要一种机制来诊断和修正推理差异。

Method: 提出差异推理学习（DRL）框架：1）将参考推理依据（医生临床推理、临床指南或更强大模型输出）和智能体的自由形式链式思维转换为有向无环图；2）使用临床加权的图编辑距离进行差异分析；3）LLM作为评判员对齐语义等价节点并诊断图间差异；4）将图级差异诊断转换为自然语言指令存储在差异推理知识库（DR-KB）中；5）推理时通过检索增强生成检索top-k指令来增强智能体提示。

Result: 在开放医学问答基准和内部临床数据的返院入院预测任务上，DRL相比基线方法在最终答案准确性和推理保真度方面均有提升。消融研究证实了参考推理依据和top-k检索策略的有效性。临床医生对输出的审查进一步验证了该方法的可靠性。

Conclusion: DRL支持在复杂推理场景中进行更可靠的临床决策，并为有限token预算下的部署提供了实用机制。该方法通过诊断和修正推理差异，提高了临床决策支持系统的推理质量。
...
### [146] [CODE-SHARP: Continuous Open-ended Discovery and Evolution of Skills as Hierarchical Reward Programs](https://arxiv.org/abs/2602.10085)
*Richard Bornemann,Pierluigi Vito Amadori,Antoine Cully*

Main category: cs.AI

TL;DR: CODE-SHARP框架利用基础模型自动发现和演化分层技能，通过代码形式的奖励函数构建技能图谱，使智能体能够解决长期任务。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法依赖人工设计的奖励函数，不适用于开放式的技能发现，因为有意义技能的集合是未知的。虽然近期方法在自动化奖励设计方面取得进展，但仍局限于预定义任务的奖励优化。

Method: 提出CODE-SHARP框架，利用基础模型开放地扩展和精炼分层技能档案，将其构建为可执行奖励函数的代码有向图。通过高层FM规划器组合发现的技能，使单一目标条件智能体能够解决复杂长期任务。

Result: 在Craftax环境中，仅使用SHARP技能生成的奖励训练的目标条件智能体能够解决越来越长期的目标。组合技能后，该智能体在复杂长期任务上的表现平均超过预训练智能体和任务特定专家策略134%以上。

Conclusion: CODE-SHARP框架通过自动化技能发现和演化，为开放式技能学习提供了有效解决方案，使智能体能够自主掌握复杂长期任务，超越了传统方法的局限性。
...
### [147] [Agent World Model: Infinity Synthetic Environments for Agentic Reinforcement Learning](https://arxiv.org/abs/2602.10090)
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型驱动的自主智能体需要多轮工具和环境交互，但缺乏多样可靠的环境限制了智能体训练的扩展性

Method: 提出AWM合成环境生成管道，创建代码驱动、数据库支持的环境，每个环境平均35个工具，提供高质量观测和可靠状态转换

Result: 在三个基准测试中，仅在合成环境中训练而非特定基准环境，能实现强大的分布外泛化能力

Conclusion: AWM为多轮工具使用智能体的大规模强化学习提供了有效资源，合成环境训练能实现良好的泛化性能
...
### [148] [Operator-Based Information Theory for Imaging: Entropy, Capacity, and Irreversibility in Physical Measurement Systems](https://arxiv.org/abs/2602.09026)
*Charles Wood*

Main category: eess.IV

TL;DR: 本文提出了一种基于算子的成像信息理论框架，通过算子谱特性量化信息流动，建立了算子熵、算子信息容量和不可逆性指数三个度量指标。


<details>
  <summary>Details</summary>
Motivation: 传统成像系统描述（分辨率、对比度、信噪比）无法全面描述物理变换如何影响信息流动，需要一种通用的信息理论框架来分析成像的物理极限。

Method: 将成像链建模为作用于函数的有限算子组合，利用算子谱特性表征信息重分布，建立了算子熵、算子信息容量和不可逆性指数三个度量指标。

Result: 该框架适用于线性、非线性和随机算子，不依赖于特定成像模态，通过解析示例展示了衰减、模糊和采样以不同方式影响熵、容量和不可逆性。
...
### [149] [SAS-Net: Scene-Appearance Separation Network for Robust Spatiotemporal Registration in Bidirectional Photoacoustic Microscopy](https://arxiv.org/abs/2602.09050)
*Jiahao Qin*
...
<details>
  <summary>Details</summary>
Motivation: 高速光学分辨率光声显微镜（OR-PAM）双向扫描虽然能实现快速功能性脑成像，但会引入严重的时空错位问题，包括扫描方向相关的域偏移和几何畸变。传统配准方法依赖亮度恒定假设，这在双向扫描条件下不成立，导致配准不可靠。

Method: 提出统一的场景-外观分离框架，将域不变场景内容与域特定外观特征分离，实现跨域重建并保持几何结构。使用场景一致性损失促进潜在空间中的几何对应，将域偏移校正与空间配准统一在单一框架内。

Result: 在小鼠脑血管成像实验中，该方法达到归一化互相关（NCC）0.961和结构相似性指数（SSIM）0.894，显著优于传统方法。消融研究表明域对齐损失至关重要，移除后NCC下降82%（0.961到0.175）。推理时间每帧11.2毫秒（86fps），远超典型OR-PAM采集速率，支持实时处理。

Conclusion: 该框架为高速双向OR-PAM提供了鲁棒的解决方案，支持可靠的定量和纵向功能性成像。代码将在GitHub上公开。
