...
TL;DR: SkyCap是一个结合高分辨率光学和SAR影像的双时相数据集，通过光学到SAR的标签转移实现SAR变化检测标注，评估了多种基础模型在SAR变化检测上的性能。


<details>
  <summary>Details</summary>
Motivation: 线性基础设施监测需要可靠的高分辨率数据和定期采集。光学VHR影像易于解释和标注，但受云层影响；SAR可实现全天候采集，但标注困难。需要解决SAR变化检测的标注挑战。

Method: 1. 构建SkyCap双时相VHR光学-SAR数据集，通过档案匹配和配准SkySat光学影像与Capella Space SAR影像；2. 使用光学到SAR标签转移技术获取SAR幅度变化检测标签，无需SAR专家标注；3. 在SAR数据上对SARATR-X进行持续预训练，并评估SAR特定基础模型与光学基础模型在不同预处理选择下的性能。

Result: 1. 光学基础模型MTP(ViT-B+RVSA)在dB+Z-score预处理下获得最佳结果(F1_c = 45.06)，优于直接在Capella数据上进一步预训练的SAR特定基础模型；2. 观察到模型对预处理与预训练统计量对齐的强烈敏感性；3. 光学模型在光学变化检测上的排名不能一对一转移到SAR幅度变化检测。

Conclusion: 这是首次在VHR SAR幅度变化检测上评估基础模型的研究。光学基础模型在适当预处理下可以超越SAR特定模型，但需要仔细考虑预处理与预训练统计量的对齐。光学到SAR的标签转移为SAR变化检测提供了可行的标注解决方案。
...
TL;DR: SocialNav-MoE：一种基于混合专家视觉语言模型的高效社交合规导航方法，通过强化学习微调和语义相似度奖励提升决策能力，在保证导航准确性的同时降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 当前机器人导航研究主要关注安全性，而社交合规性（考虑人类舒适度、社会规范和情境适当性）研究不足。现有视觉语言模型计算开销大，不适合资源受限的机器人平台实时部署。

Method: 提出SocialNav-MoE，一种基于混合专家架构的高效视觉语言模型，采用强化学习微调和语义相似度奖励机制。研究了不同小型语言模型类型、路由策略和视觉编码器的效果。
...
TL;DR: 该研究开发了一个模块化专家系统，用于将带歌词的印刷简谱转换为机器可读的MusicXML和MIDI格式，填补了大规模光学音乐识别在中文简谱领域的空白。
...
Result: 在《中国民歌选集》上评估，系统大规模数字化了：(1) 超过5,000首纯旋律歌曲（>300,000个音符）和(2) 包含歌词的精选子集超过1,400首歌曲（>100,000个音符）。系统在旋律识别（音符级F1=0.951）和对齐歌词（字符级F1=0.931）方面均达到高精度。

Conclusion: 该混合方法在可解释性和准确性之间取得了平衡，成功实现了中文简谱的大规模数字化，为简谱光学音乐识别提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉问答中表现出色，但容易产生幻觉，导致过度自信的错误答案，严重损害答案可靠性。需要一种方法来评估和增强VLM回答的可信度。

Method: 提出DAVR框架，采用双路径架构：一条路径通过双选择器模块融合VLM潜在特征和QA嵌入来评估响应可靠性；另一条路径部署外部参考模型进行事实交叉检查以减轻幻觉。

Result: 在ICCV-CLVL 2025的Reliable VQA挑战赛中，DAVR获得领先的Φ₁₀₀分数39.64和100-AUC分数97.22，获得第一名，证明其在增强VLM响应可信度方面的有效性。

Conclusion: DAVR框架通过集成自我反思和跨模型验证，为视觉语言模型提供了全面的不确定性估计，显著提高了视觉问答系统的可靠性和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频问答基准测试通常允许从单一显著线索回答问题，未能充分测试需要聚合多个时间上分离的视觉证据的推理能力，因此需要构建一个专门评估多证据跨时间整合的基准。

Method: 提出HERBench基准，包含26K个五选一选择题，组织成12个组合任务；引入最小必需帧集(MRFS)概念来衡量证据需求；评估13个最先进的视频大语言模型。

Result: HERBench的MRFS需求显著高于先前数据集(平均5.5 vs. 2.6-4.2)；现有模型表现不佳，准确率仅31-42%，略高于20%的随机猜测基线；失败原因可分解为检索缺陷和融合缺陷两个瓶颈。

Conclusion: HERBench通过使跨时间证据既不可避免又可量化，为推进稳健的组合式视频理解建立了原则性目标，揭示了当前视频大语言模型在多证据整合方面的系统性缺陷。
...
TL;DR: 该研究提出了一种结合皮肤纹理文字描述与图像的老虎重识别方法，通过生成虚拟个体数据增强，显著提升了跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 传统AI重识别方法主要依赖图像，但生物学家长期结合视觉和文字描述来识别动物。本研究旨在扩展重识别方法，引入法医学中使用的精确皮肤纹理文字描述，以克服纯视觉方法的局限性。

Method: 1) 使用人类可解释的语言标签对动物皮毛拓扑进行抽象编码；2) 基于84,264个手动标记的细节特征，覆盖3,355张185只老虎的图像；3) 开发文本-图像协同合成管道，生成包含数十张逼真图像和皮肤纹理文字的"虚拟个体"；4) 评估视觉-文本方法在跨模态身份检索中的能力。

Result: 该方法在真实场景基准测试中显著提升了AI在跨模态检索中的准确性，同时缓解了数据稀缺问题。皮肤纹理语言引导的生物识别技术能够克服纯视觉限制，实现基于人类可验证匹配的文本到视觉身份恢复。

Conclusion: 皮肤纹理语言引导的生物识别技术代表了重识别可解释性的重要进展，实现了生态监测中描述模态的语言驱动统一，为跨模态身份检索提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 创建新视觉概念需要连接不同想法间的相关共享属性（vibe），现有方法难以识别和遍历潜在空间中连接遥远概念的非线性路径。

Method: 提出Vibe Space层次图流形，在CLIP等特征空间中学习低维测地线，实现概念间的平滑语义一致过渡；设计结合人类判断、LLM推理和几何路径难度分数的认知启发评估框架。

Result: Vibe Space生成的混合体在人类评估中始终被评为比现有方法更具创意和连贯性。

Conclusion: Vibe Space通过层次图流形学习特征空间中的测地线，能够有效生成揭示概念间共享属性的创意视觉混合体，优于现有方法。
...
Method: 构建了PANDA-PLUS-Bench基准数据集，包含9个不同患者的专家标注前列腺活检全切片图像，提取了512x512和224x224像素分辨率的非重叠组织块，并在8种增强条件下处理。使用该基准评估了7个基础模型分离生物学信号与切片级混杂因素的能力。

Result: 不同模型在鲁棒性方面存在显著差异：Virchow2在大型模型中切片级编码最低（81.0%），但跨切片准确率第二低（47.2%）。专门在前列腺组织上训练的HistoEncoder表现出最高的跨切片准确率（59.7%）和最强的切片级编码（90.3%）。所有模型都显示出可测量的切片内与跨切片准确率差距（19.9-26.9个百分点）。

Conclusion: PANDA-PLUS-Bench填补了基础模型评估的关键空白，为Gleason分级这一重要临床背景下的鲁棒性评估提供了专门构建的资源。组织特异性训练可能同时增强生物学特征捕获和切片特异性特征识别能力。
...
TL;DR: 该论文提出自适应后处理技术来改进大型预训练模型生成的胶质瘤分割质量，在BraTS 2025挑战赛中显著提升了分割性能，同时强调计算公平性和可持续性。


<details>
  <summary>Details</summary>
Motivation: 胶质瘤是最常见的恶性脑肿瘤，但现有大型预训练模型在分割时存在泛化能力差、产生系统性错误（假阳性、标签交换、切片不连续）等问题，且存在GPU资源分配不均和训练环境成本高的挑战。

Method: 提出自适应后处理技术来优化大型预训练模型生成的胶质瘤分割结果，而不是设计更复杂的模型架构。
...
TL;DR: TalkVerse是一个大规模开放语料库，用于单人音频驱动视频生成，包含230万高清音频-视频同步片段，总计6300小时。基于该数据集构建了一个可复现的50亿参数DiT基线模型，支持分钟级视频生成，推理成本降低10倍，并开源数据集、训练方案和模型检查点。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的音频驱动视频生成系统依赖于闭源数据或计算密集型模型，缺乏公平、可复现的比较基准。TalkVerse旨在提供一个大规模、高质量、开放的数据集，降低研究门槛，促进该领域的公平比较和可复现研究。

Method: 1) 构建TalkVerse数据集：从6万小时视频中通过场景切割检测、美学评估、严格的音视频同步检查等透明流程筛选出230万高质量片段；2) 基于Wan2.2-5B构建可复现的50亿参数DiT基线模型，采用高下采样比的视频VAE和带运动帧上下文的滑动窗口机制；3) 集成MLLM导演根据音频和视觉线索重写提示词；4) 通过受控潜在噪声注入实现零样本视频配音。

Result: 1) 创建了包含230万高清片段（720p/1080p）、总计6300小时的TalkVerse数据集；2) 5B模型实现了分钟级视频生成，唇部同步和视觉质量与14B Wan-S2V模型相当，但推理成本降低10倍；3) 支持长视频叙事增强和零样本视频配音；4) 开源了数据集、训练方案和模型检查点。

Conclusion: TalkVerse为音频驱动的人类视频生成研究提供了一个大规模、高质量、开放的基准数据集和可复现的基线模型，显著降低了研究门槛，促进了该领域的公平比较和可复现研究。开源的数据集和模型有望推动该领域的进一步发展。
...
TL;DR: 提出一种三模态人员识别框架，整合语音、面部和手势信息，通过多任务学习、跨注意力门控融合和置信度加权融合策略，在模态缺失或质量不佳时仍保持高性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的人员识别系统常面临模态缺失或质量下降的问题，现有方法在模态不完整时性能受限，需要一种能适应模态缺失的鲁棒识别方案。

Method: 采用三模态（语音、面部、手势）融合框架，使用多任务学习独立处理各模态，通过跨注意力和门控融合机制促进模态间交互，并引入置信度加权融合策略动态适应缺失或低质量数据。

Result: 在CANDOR数据集上达到99.18%的Top-1准确率，优于传统单模态和后期融合方法；在VoxCeleb1数据集的双模态模式下达到99.92%准确率；在模态缺失情况下仍保持高性能。

Conclusion: 提出的三模态融合框架在人员识别任务中表现出色，特别是在处理模态缺失问题时具有鲁棒性，适用于现实世界应用场景。
...
Method: 在离散小波变换域使用统计块级策略嵌入水印信号，结合局部化嵌入与区域级可解释性，生成检测图显示图像中可能被水印或篡改的区域。

Result: 方法在常见图像变换下具有强鲁棒性，同时对语义操作保持敏感，水印高度不可感知。相比先前后处理方法，提供更可解释的检测同时保持竞争性鲁棒性，例如水印对裁剪高达图像一半仍保持鲁棒。

Conclusion: 提出的后处理图像水印方法通过局部化嵌入和区域级可解释性，解决了现有水印方案缺乏透明度的问题，在保持强鲁棒性和不可感知性的同时提供更可解释的检测能力。
...
TL;DR: 该研究提出了一种评估视频问题生成模型质量的新协议，通过模拟与专家的问答交流来评估问题能否有效引出专家未见知识，并构建了EgoExoAsk数据集来支持这一评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频问题生成评估主要关注问题能否被回答，而非问题本身的质量。研究者希望评估问题能否有效引出专家未见知识，这需要新的评估协议。

Method: 提出基于问题-答案检索的评估协议，通过模拟专家问答交流来评估问题质量。构建包含27,666个问答对的EgoExoAsk数据集，用于训练检索器并在验证集上建立基准。

Result: 实验结果表明，该评估指标与问题生成设置合理对齐：能够访问更丰富上下文的模型获得更好评价，证明协议按预期工作。

Conclusion: 该研究为视频问题生成模型的质量评估提供了新方法，通过模拟专家交流来评估问题引出未见知识的能力，EgoExoAsk数据集支持这一评估框架。
...
TL;DR: 本文对LUMIR挑战赛关于深度学习图像配准方法零样本泛化能力的声明进行了独立重新评估，发现这些声明与深度学习领域偏移的既有理解相矛盾，揭示了更复杂的情况。


<details>
  <summary>Details</summary>
Motivation: LUMIR挑战赛声称深度学习方法在神经影像数据配准中具有卓越的零样本泛化能力（对未见对比度和分辨率），这与深度学习领域偏移的既定理解相矛盾。作者旨在对这些声明进行独立重新评估，解决潜在的仪器偏差。

Method: 使用严格的评估协议对深度学习方法的零样本声明进行独立重新评估，同时解决潜在的仪器偏差来源。评估包括：1）在分布内T1加权图像上的表现；2）对未见对比度（T2、T2*、FLAIR）的泛化能力；3）在高分辨率数据上的可扩展性；4）对预处理选择的敏感性。

Result: 1）深度学习在分布内T1w图像和相近物种（猕猴）上表现与迭代优化方法相当，显示任务理解有所改进；2）在分布外对比度上性能显著下降（Cohen's d分数0.7-1.5），对下游临床工作流程有实质性影响；3）深度学习在高分辨率数据上存在可扩展性限制（无法在0.6mm各向同性图像上运行），而迭代方法能从更高分辨率中受益；4）深度方法对预处理选择高度敏感。

Conclusion: 研究结果与领域偏移的既有文献一致，表明深度学习方法的"通用零样本优越性"声明需要仔细审查。作者主张采用反映实际临床和研究工作流程的评估协议，而不是可能无意中偏向特定方法类别的条件。
...
Result: 在多种医学数据集上的评估显示，MAPO能持续提升边界贴合度、减少过拟合，并提供比传统监督训练更稳定的优化动态

Conclusion: MAPO作为一种模型无关的偏好优化框架，能够有效提升医学图像分割性能，具有广泛的架构兼容性和实际应用价值
...
TL;DR: 提出了一种利用事件相机和LED标记测量高频变形的方法，通过区分运动事件和LED闪烁事件来提取高速移动的LED标记，最终实现单目事件相机测量高频平面变形。


<details>
  <summary>Details</summary>
Motivation: 大型结构在复杂载荷下会产生高频变形，但恶劣光照条件和高设备成本限制了传统高速相机的测量方法，需要一种更经济有效的测量方案。

Method: 基于LED标记闪烁和时空相关性过滤事件流中的观测噪声，区分运动引起的事件和LED闪烁事件，从事件流中提取高速移动的LED标记，使用单目事件相机测量高频平面变形。

Result: 实验结果表明该方法在测量高频平面变形方面具有准确性。

Conclusion: 该方法能够有效利用事件相机和LED标记测量高频变形，解决了传统高速相机在恶劣光照和高成本方面的限制问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在生成可控姿态和外观的人物图像时，经常面临遮挡、服装风格漂移和姿态不对齐等问题，这限制了虚拟试穿、图像编辑和数字人创建等应用的效果。

Method: 提出Pose-guided Multi-view Multimodal Diffusion (PMMD)框架，包含多模态编码器联合建模视觉视图、姿态特征和语义描述；ResCVA模块增强局部细节同时保持全局结构；跨模态融合模块在整个去噪流程中整合图像语义和文本信息。

Result: 在DeepFashion MultiModal数据集上的实验表明，PMMD在一致性、细节保持和可控性方面优于代表性基线方法。

Conclusion: PMMD框架通过多模态融合和局部细节增强，能够生成高质量、可控的人物图像，为虚拟试穿、图像编辑和数字人创建等应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统管道式文档解析方法在处理科学文献和专利时存在局限性，无法有效保留文本、公式、表格、图像和化学结构之间的细粒度跨模态对齐关系，且扩展性不足。需要一种能够高效处理大规模文档、支持多种模态、易于扩展的工业级解析方案。

Method: 采用模块化、松耦合的多专家架构，每个专家负责特定模态的解析；通过自适应GPU负载均衡、分布式推理和动态模块编排实现高效处理；支持整体解析或特定模态解析的配置模式；保持跨模态对齐关系。

Result: 在8个NVIDIA RTX 4090D GPU上实现每秒20页PDF的处理速度，能够高效处理数十亿页文档；支持科学文献和专利中文本、公式、表格、图像、化学结构等多种模态的解析；具有高精度和成本效益。

Conclusion: Uni-Parser为科学文献和专利解析提供了工业级解决方案，其高吞吐量、跨模态对齐能力和可扩展性支持从文献检索、化学结构提取到大规模语料库构建等多种下游应用，为下一代大语言模型和AI4Science模型训练提供支持。
...
TL;DR: 研究评估Nano Banana Pro在14个低层视觉任务上的零样本表现，发现其主观视觉质量优秀但传统量化指标落后于专业模型


<details>
  <summary>Details</summary>
Motivation: 探索商业文本到图像生成模型Nano Banana Pro作为通用低层视觉解决方案的潜力，该领域目前尚未充分研究

Method: 在14个不同低层视觉任务、40个多样化数据集上进行全面的零样本评估，使用简单文本提示而不进行微调，与最先进的专用模型进行基准比较

Result: 发现明显的性能二分现象：Nano Banana Pro在主观视觉质量上表现优越，能生成超越专用模型的合理高频细节，但在传统基于参考的量化指标上落后

Conclusion: Nano Banana Pro是低层视觉任务的有能力零样本竞争者，但达到领域专用模型的高保真度仍是重大挑战，生成模型的随机性使其难以满足传统指标对像素级一致性的严格要求
...
TL;DR: 本文提出了人类动作形态评估(AFA)新任务，构建了包含健身和武术视频的多层次标注数据集CoT-AFA，并开发了可解释的评估框架，在动作分类、质量评估和解释生成方面均有提升。


<details>
  <summary>Details</summary>
Motivation: 当前视频理解方法主要关注动作识别（什么动作、在哪里发生），无法满足评估动作标准化程度并提供改进反馈的需求。现有数据集缺乏动作标准化程度的标注，且动作质量评估数据集缺乏可解释性和详细反馈。

Method: 1) 定义人类动作形态评估(AFA)新任务；2) 构建CoT-AFA数据集，包含健身和武术视频，采用链式思维解释范式提供完整推理过程；3) 提出可解释的健身评估框架，使用并行处理流和动态门控机制融合视觉和语义信息。

Result: 实验结果显示，该方法在解释生成方面CIDEr指标提升16.0%，动作分类准确率提升2.7%，质量评估准确率提升2.1%，展现了CoT-AFA数据集在未来研究中的巨大潜力。

Conclusion: 本文成功定义了AFA任务并构建了CoT-AFA数据集，提出的可解释评估框架在多个指标上均有显著提升，为动作标准化评估提供了新的解决方案。
...
TL;DR: EagleVision是一个双阶段框架，通过宏观感知和微观验证实现渐进式空间认知，解决了空间思维链中的关键挑战，在VSI-Bench上达到开源视觉语言模型的最先进性能。
...
Conclusion: EagleVision通过宏观感知和微观验证的双阶段设计，有效解决了空间思维链中的关键挑战，为渐进式空间认知提供了系统化框架，在空间理解任务上表现出色。
...
TL;DR: 本文首次深入分析了用于目标检测系统安全评估的关键性指标，提出了双向关键性评级和多指标聚合两种新策略，在DeepAccident数据集上实现了高达100%的关键性分类准确率提升。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶安全评估需要专门的安全指标来区分相关和非相关对象，现有性能评估指标不足以可靠评估目标检测系统的安全性，因此需要分析关键性指标在安全评估中的应用。

Method: 通过文献综述识别和评估适用的关键性指标，使用DeepAccident数据集进行实证验证，并提出双向关键性评级和多指标聚合两种新应用策略来提升评估准确性。

Result: 提出的方法在关键性分类准确率上实现了高达100%的改进，显著提升了目标检测系统在自动驾驶车辆中的安全评估能力。

Conclusion: 关键性指标分析对于自动驾驶目标检测系统的安全评估至关重要，提出的新策略能显著提高评估准确性，有助于推进自动驾驶系统的安全评估发展。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型合成的深度伪造内容正在被大规模滥用，破坏数字媒体真实性。现有检测方法不可靠：1）事后区分不真实内容往往不可能，导致无界的误报率；2）检测缺乏鲁棒性，对手可用最小计算资源适应已知检测器。

Method: 提出重合成框架来确定样本是否真实或其真实性是否可被合理否认。采用校准的重合成方法，专注于高精度、低召回率设置对抗计算受限的对手。利用最先进的反演技术支持多种模态。

Result: 校准的重合成方法在验证真实样本方面最可靠，同时保持可控的低误报率。该方法实现了对抗计算受限对手的鲁棒性，而先前方法在相同计算预算下容易被规避。

Conclusion: 重合成框架为解决深度伪造检测的可靠性和鲁棒性问题提供了有效方案，特别是在对抗计算受限对手的高精度检测场景中表现优异。
...
TL;DR: 该论文针对低空无人机侦察任务中的红外与可见光图像融合问题，提出了一种新的评价指标TBC（目标-背景对比度），解决了传统无参考指标在复杂低光环境下将传感器噪声误判为有效细节的"噪声陷阱"问题。


<details>
  <summary>Details</summary>
Motivation: 传统无参考评价指标（如熵EN和平均梯度AG）在复杂低光环境下存在严重缺陷，它们会将高频传感器噪声误判为有效细节，形成"噪声陷阱"，导致对噪声图像给出更高评分，从而误导融合算法的发展方向。

Method: 提出基于韦伯定律的TBC（目标-背景对比度）指标，该指标关注显著目标的相对对比度而非全局统计特征，能够惩罚背景噪声并奖励目标可见性，为低空场景提供更可靠的评价标准。

Result: 在DroneVehicle数据集上的实验表明，TBC指标比传统指标更符合人类感知，能够有效区分有效细节与传感器噪声，为低空无人机侦察场景提供了可靠的图像融合质量评价标准。

Conclusion: TBC指标成功解决了传统无参考指标在低光环境下的"噪声陷阱"问题，为红外与可见光图像融合算法提供了更准确、更符合人类感知的评价工具，特别适用于低空无人机侦察任务。
...
TL;DR: Mesh-Plug是一个即插即用模块，能够将相机坐标系中重建的人体网格准确转换到世界坐标系，通过人体中心方法利用RGB图像和深度图估计相机旋转参数。


<details>
  <summary>Details</summary>
Motivation: 从野外图像重建世界坐标系中的准确3D人体网格具有挑战性，因为缺乏相机旋转信息。现有方法假设相机旋转为零，在相机坐标系中取得良好结果，但转换到世界坐标系时会产生显著误差。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）在全参数空间进行低秩适应，但预训练模型存在非平凡零空间，在子空间内微调可达到类似效果。作者希望减少冗余并提高参数效率

Method: 提出Null-LoRA方法：1）冻结部分低秩矩阵以减少冗余并提高有效秩；2）将整个增量更新约束在模型的零空间内，最大化增量更新的利用率以适应新任务范式

Result: 在图像-文本检索和视觉问答任务的广泛实验中，Null-LoRA以更少的参数超越了现有最先进方法

Conclusion: Null-LoRA通过利用预训练模型的零空间特性，实现了更高效的参数适应，在减少参数量的同时提升了性能，为大规模模型的下游任务适应提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 医学AI系统（特别是多模态视觉语言模型）存在交叉偏见，对边缘化患者亚组的诊断置信度较低，导致误诊和漏诊率更高。现有公平性干预方法要么无法解决这些差距，要么为了达到统计均等而牺牲整体诊断性能。

Method: 开发了跨模态对齐一致性（CMAC-MMD）训练框架，标准化交叉患者亚组的诊断置信度。该方法在临床推理期间无需敏感人口统计数据，通过跨模态对齐实现决策置信度的均等化。

Result: 在皮肤病变检测中，将交叉漏诊差距（ΔTPR）从0.50降低到0.26，AUC从0.94提升到0.97。在青光眼筛查中，ΔTPR从0.41降低到0.31，AUC从0.71提升到0.72。在两个数据集上都取得了更好的公平性和准确性。
...
TL;DR: 本文系统比较了专用计数架构与多模态视觉语言模型在物体计数任务上的性能，发现VLMs表现接近甚至超越专用架构，但复杂场景下仍有挑战


<details>
  <summary>Details</summary>
Motivation: 视觉场景中的物体计数是计算机视觉的基础但具有挑战性的任务。传统方法依赖特定领域的计数架构，而大规模多模态视觉语言模型可能提供更灵活的开放集计数替代方案

Method: 系统比较了最先进的专用计数架构与VLMs在两个流行计数数据集上的性能，并创建了一个新的基准测试，能更精细地控制测试图像的视觉属性

Result: 大多数VLMs能够近似枚举视觉场景中的物品数量，性能匹配甚至超越专用计算机视觉架构。当提示VLMs生成每个待计数对象的中间表示（位置和语言标签）时，枚举准确性显著提高

Conclusion: 没有模型能够可靠地计数复杂视觉场景中的物体数量，表明仍需进一步研究以创建能够在真实环境中可靠部署计数程序的AI系统
...
TL;DR: 提出MMMamba框架，基于Mamba架构实现跨模态融合，用于全色锐化和零样本超分辨率，具有线性计算复杂度
...
Method: 基于Mamba架构设计跨模态上下文融合框架，引入新型多模态交错扫描机制，支持PAN和MS模态间的有效信息交换，具有线性计算复杂度

Result: 在多个任务和基准测试中，该方法相比现有最先进技术表现出优越性能

Conclusion: MMMamba框架通过高效的跨模态上下文融合，在全色锐化和零样本超分辨率任务中取得了显著效果，为多模态图像处理提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有弱监督语义分割方法虽然利用生成模型增强数据，但仍依赖真实世界训练样本。本文旨在探索完全无需真实图像的新方向，通过合成数据实现语义分割，降低标注成本并提高可扩展性。

Method: 提出SynthSeg Agents多智能体框架，包含两个核心模块：1) Self-Refine Prompt Agent通过迭代优化、记忆机制和提示空间探索自主生成多样化语义丰富的图像提示；2) Image Generation Agent利用视觉语言模型生成候选图像，通过冻结CLIP评分模型选择高质量样本，并使用ViT分类器重新标注整个合成数据集以提高语义精度。

Result: 在PASCAL VOC 2012和COCO 2014数据集上的实验表明，SynthSeg Agents在不使用真实训练图像的情况下取得了有竞争力的性能，验证了LLM驱动智能体在实现成本高效、可扩展语义分割方面的潜力。

Conclusion: SynthSeg Agents框架成功实现了零样本弱监督语义分割，通过LLM驱动的智能体生成高质量合成训练数据，为降低语义分割标注成本和提高模型可扩展性提供了新途径。
...
TL;DR: AutoMAC-MRI是一个可解释的MRI运动伪影分级框架，使用监督对比学习学习运动严重程度的判别表示，通过计算特定等级的亲和度分数实现透明可解释的分级，在5000多个专家标注的脑部MRI切片上验证有效。


<details>
  <summary>Details</summary>
Motivation: MRI中的运动伪影会降低图像质量并增加患者召回率。现有的自动化质量评估方法大多局限于二元决策，且缺乏可解释性，需要开发能够跨不同MRI对比度和方向进行运动伪影分级并提供解释性的框架。

Method: 使用监督对比学习学习运动严重程度的判别表示，在该特征空间中计算特定等级的亲和度分数，量化图像与每个运动等级的接近程度，从而实现透明可解释的等级分配。

Result: 在5000多个专家标注的脑部MRI切片上进行评估，实验显示亲和度分数与专家标注高度一致，支持其作为运动严重程度的可解释性度量。该框架能够实现准确的等级检测和每级亲和度评分。

Conclusion: AutoMAC-MRI通过将准确的等级检测与每级亲和度评分相结合，实现了内联MRI质量控制，有望减少不必要的重复扫描并提高工作流程效率。
...
Method: 采用多专家架构，基于特征相似性动态分配专家到对象类别，结合优化核心集选择和专用重放缓冲区机制进行高效内存管理。

Result: 在MVTec AD数据集上，最优的5专家配置在15个不同对象类别上实现了0.8259的平均AUROC，相比单专家方法显著减少了知识退化。

Conclusion: MECAD框架在计算效率、专业知识保留和适应性之间取得了平衡，适合产品类型不断演变的工业环境。
...
TL;DR: 本文提出了一种名为掩码反向知识蒸馏（MRKD）的新技术，通过图像级掩码和特征级掩码将图像重建任务转化为图像修复，有效解决了知识蒸馏在异常检测中过度泛化的问题。
...
Method: 提出掩码反向知识蒸馏（MRKD）方法，采用两种掩码策略：1）图像级掩码（ILM）用于捕获全局信息并区分输入和监督信号；2）特征级掩码（FLM）引入合成特征级异常，确保学习到的表示包含足够的局部信息。

Result: 在MVTec异常检测数据集上取得了优异性能：图像级AU-ROC为98.9%，像素级AU-ROC为98.4%，AU-PRO为95.3%。消融实验验证了MRKD在缓解过度泛化问题方面的优越性。

Conclusion: MRKD通过图像级和特征级掩码策略，增强了图像上下文捕获能力，减少了过度泛化，在异常检测和定位方面表现出色，为解决知识蒸馏的过度泛化问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉模型在物体检测、图像分类等任务上表现优异，但能够从图像中准确读取定量测量值的模型仍然稀缺。为了实现实验室环境中机器人的完全自主操作，需要具备读取仪器测量值的能力

Method: 采用人类启发的方法读取线性刻度测量值：1) 对随机方向的注射器进行方向校正变换；2) 将感兴趣区域缩小到仅包含线性刻度的部分；3) 提取主要刻度标记、对应数字和液位指示器位置等特征；4) 基于这些特征计算最终读数

Result: 该系统读取的测量值与人工读取相同实例的值进行了比较，观察到了准确的对应关系，表明该方法能够有效实现自动化测量

Conclusion: 成功开发了一种模仿人类视觉读取线性刻度测量值的方法，为实验室自动化环境中的机器人提供了重要的测量读取能力，展示了在非结构化环境中实现人类类似能力的可行性
...
TL;DR: 该研究首次提出了视觉文本压缩（VTC）的基准测试，系统评估了视觉语言模型在VTC压缩信息下的长上下文理解能力，发现大多数模型在此场景下表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs扩展上下文窗口面临计算和内存开销问题，视觉文本压缩（VTC）技术能实现3x-20x的token压缩比，但高信息密度对视觉语言模型核心长上下文能力的影响尚未得到充分研究。

Method: 引入首个VTC基准测试，系统评估VLMs在三个长上下文理解场景：VTC-Retrieval（信息检索与聚合）、VTC-Reasoning（推断潜在关联定位事实）、VTC-Memory（长期对话记忆中的全面问答），并建立VTCBench-Wild模拟多样化输入场景。

Result: 尽管大多数VLMs能够很好地解码文本信息（如OCR），但在VTC压缩信息下表现出令人惊讶的差的长上下文理解能力，无法捕捉上下文中的长关联或依赖关系。

Conclusion: 该研究为VTC提供了深入理解，并为设计更高效、可扩展的VLMs奠定了基础，揭示了当前模型在处理压缩视觉文本表示时的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 人类对话涉及语音和非语言线索（如点头、注视转移、面部表情）的连续交换，这些线索传达注意力和情感。在3D中建模这些双向动态对于构建富有表现力的虚拟化身和交互式机器人至关重要。然而，现有框架通常将说话和倾听视为独立过程，或依赖非因果的全序列建模，这阻碍了跨轮次的时间连贯性。

Method: TIMAR（轮次级交错掩码自回归）是一个因果框架，用于3D对话头部生成，将对话建模为交错音频-视觉上下文。它在每个轮次内融合多模态信息，并应用轮次级因果注意力来累积对话历史，同时使用轻量级扩散头预测连续的3D头部动态，捕捉协调性和表达变异性。

Result: 在DualTalk基准上的实验表明，TIMAR在测试集上将Fréchet距离和MSE降低了15-30%，在分布外数据上也取得了类似的提升。

Conclusion: TIMAR通过因果建模对话动态，在3D对话头部生成方面取得了显著改进，为构建更自然、连贯的虚拟化身和交互式机器人提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: Group Relative Policy Optimization (GRPO)是一种强大的生成模型对齐技术，但其效果受到大组规模与高昂计算成本之间冲突的限制。需要解决这种权衡问题以提高效率。

Method: 首先通过实证研究发现了奖励聚类现象，设计了启发式策略Optimal Variance Filtering (OVF)。然后提出Pro-GRPO框架，将基于潜在特征的轨迹剪枝集成到采样过程中，采用"扩展-剪枝"策略：先扩展初始采样组以最大化轨迹多样性，然后对潜在特征应用多步OVF避免高昂计算成本。

Result: 实验表明，通过OVF选择的高方差轨迹子集可以优于更大的未过滤组。Pro-GRPO通过在采样过程中早期终止奖励聚类的轨迹来减少计算开销，在扩散模型和流模型上的广泛实验证明了该框架的通用性和有效性。

Conclusion: Pro-GRPO通过动态轨迹剪枝和"扩展-剪枝"策略有效解决了GRPO中大组规模与计算成本之间的冲突，提高了生成模型对齐的效率。
...
Conclusion: 该数据集填补了桥梁3D语义分割领域的空白，揭示了传感器差异对模型性能的显著影响，为基础设施自动化检测和领域适应研究提供了重要基础。
...
TL;DR: 该研究提出了篮球视频中篮板球预测的新任务，创建了包含10万个视频片段、300多小时素材和2000多个手动标注篮板事件的数据集，并应用深度学习技术进行基准测试。


<details>
  <summary>Details</summary>
Motivation: 尽管计算机视觉和视频理解在体育分析中取得了显著进展，但在体育视频中预测动作发生前的行为（如篮板球归属）却相对缺乏关注。现有研究主要集中在球员跟踪、姿态估计、动作定位和犯规识别等方面，而动作预测在篮球广播视频中尚未得到充分探索。

Method: 1. 提出了篮球广播视频中动作预测的新任务：预测投篮尝试后哪支球队将获得篮板球控制权
2. 创建了一个新的自收集数据集，包含100,000个篮球视频片段、300多小时素材和2,000多个手动标注的篮板事件
3. 使用最先进的动作预测方法建立基准结果
4. 探索了两个补充任务：篮板分类和篮板识别

Result: 实验结果表明篮板球预测既具有可行性又存在固有挑战。该数据集支持广泛的篮球视频理解应用，填补了当前缺乏可比数据集的空白。通过预测篮板球发生前的球队控制权，这项工作为实时自动广播和赛后分析工具提供了支持决策的应用可能。

Conclusion: 该研究首次将深度学习技术应用于篮球篮板预测，为动态多智能体体育场景的预测建模提供了宝贵见解。通过预测篮板球发生前的球队控制权，这项工作能够支持实时自动广播和赛后分析工具的决策制定，推动了体育视频理解的发展。
...
TL;DR: 本文研究了在多重组织成像数据中，保持通道独立性结合浅层架构是否比增加模型规模更适合自监督表示学习。通过对比早期融合CNN和通道分离架构，发现轻量级通道独立模型（CIM-S）在保持标记特异性信息和罕见细胞识别方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 多重组织成像技术能够测量每个细胞的数十种蛋白质标记，但大多数深度学习模型仍采用早期通道融合，假设标记间共享结构。本文旨在探索保持标记独立性并结合浅层架构是否为多重数据自监督表示学习提供更合适的归纳偏置。

Method: 使用霍奇金淋巴瘤CODEX数据集（145,000个细胞，49个标记），比较标准早期融合CNN与通道分离架构，包括标记感知基线和新型浅层通道独立模型（CIM-S，仅5.5K参数）。通过对比预训练和线性评估，分析不同架构的性能。

Result: 早期融合模型在保留标记特异性信息方面能力有限，特别是在罕见细胞识别方面表现不佳。通道独立架构（尤其是CIM-S）尽管规模紧凑，但获得了显著更强的表示能力。这些发现在多种自监督框架中一致，在不同增强设置下保持稳定，并在49标记和18标记设置中均可复现。

Conclusion: 轻量级、通道独立的架构在多重表示学习方面能够匹配甚至超越深度早期融合CNN和基础模型，表明保持标记独立性结合浅层架构是多重组织成像数据自监督学习的更有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 单目深度基础模型通过学习大规模语义先验实现了显著泛化能力，但这带来了关键漏洞：它们会从几何平面但感知模糊的输入中产生虚假的3D结构（称为"3D幻象"）。目前缺乏系统方法来探测、量化和控制这种未量化的安全风险。

Method: 1) 提出3D-Mirage基准，包含真实世界幻觉场景（如街头艺术）的精确平面区域标注和上下文受限裁剪；2) 提出基于拉普拉斯算子的评估框架，包含两个指标：偏差复合分数（DCS）用于量化虚假非平面性，混淆复合分数（CCS）用于量化上下文不稳定性；3) 提出基于自蒸馏的接地方法，通过参数高效策略在幻觉区域强制实施平面性，同时使用冻结教师模型保留背景知识，避免灾难性遗忘。

Result: 论文提供了诊断和缓解3D幻象现象的基本工具，包括首个专门基准、量化评估框架和有效的缓解策略。这些工具促使MDE评估从像素级精度转向结构和上下文鲁棒性。

Conclusion: 该研究首次系统性地解决了单目深度基础模型中的3D幻象问题，提供了探测、量化和控制的完整框架，推动了MDE评估范式的转变，强调结构和上下文鲁棒性的重要性。
...
Result: ResNet-101模型达到94%的分类准确率和0.91的平均精度(mAP)，在提取深度层次特征方面表现强劲。Inception v3模型表现略优，达到95%的分类准确率和0.92的mAP，归功于其通过并行卷积实现的高效多尺度特征提取。

Conclusion: 尽管两种模型在检测视觉特征相似物种或光照不佳、遮挡条件下的物种时仍面临挑战，但研究证实ResNet-101和Inception v3都是野生动物检测任务的有效模型，为保护导向的计算机视觉应用提供了可靠基础。
...
### [43] [IC-Effect: Precise and Efficient Video Effects Editing via In-Context Learning](https://arxiv.org/abs/2512.15635)
*Yuanhang Li,Yiren Song,Junzhe Bai,Xinran Liang,Hu Yang,Libiao Jin,Qi Mao*

Main category: cs.CV

TL;DR: IC-Effect是一个基于DiT的指令引导框架，用于少样本视频VFX特效编辑，能在保持时空一致性的同时合成复杂特效。


<details>
  <summary>Details</summary>
Motivation: 视频VFX编辑面临三大挑战：特效必须与背景无缝融合、背景必须完全不变、需要从有限的配对数据中高效学习特效模式。现有视频编辑模型无法满足这些要求。
...
Conclusion: IC-Effect是一个有效的指令引导DiT框架，通过上下文学习、两阶段训练和稀疏标记化等技术，解决了视频VFX编辑中的关键挑战，实现了高质量的特效合成。
...
TL;DR: RUMPL：基于Transformer的3D姿态提升器，使用2D关键点的3D射线表示，无需相机标定，可在任意多视角配置中通用部署
...
Method: 基于MPL框架，提出RUMPL：基于Transformer的3D姿态提升器，引入2D关键点的3D射线表示，使模型独立于相机标定和视角数量；使用新的View Fusion Transformer通过融合射线token聚合信息，提升多视角一致性

Result: 相比三角测量减少MPJPE达53%，相比基于Transformer的图像表示基线减少60%以上；在野外多视角和多人物数据集上验证了鲁棒性和可扩展性

Conclusion: RUMPL通过3D射线表示实现了相机标定无关的多视角3D姿态估计，在任意多视角配置中无需重新训练或微调即可部署，显著提升了姿态估计精度
...
<details>
  <summary>Details</summary>
Motivation: AI驱动的图像生成技术带来了数字证据真实性验证的新挑战。现代生成模型能产生视觉一致的伪造品，逃避基于像素或压缩伪影的传统检测器。现有方法大多缺乏明确的异常强度度量，限制了量化操纵严重程度的能力。

Method: 提出Vision-Attention Anomaly Scoring (VAAS)框架，整合两个模块：1) 使用Vision Transformers进行基于全局注意力的异常估计；2) 从SegFormer嵌入中提取局部自一致性评分。这种混合方法提供连续且可解释的异常分数，反映操纵的位置和程度。

Result: 在DF2023和CASIA v2.0数据集上的评估表明，VAAS在F1和IoU指标上取得竞争性性能，同时通过注意力引导的异常图增强了视觉可解释性。

Conclusion: VAAS框架将定量检测与人类可理解推理相结合，支持透明可靠图像完整性评估。所有实验的源代码和相应材料已开源提供。
...
TL;DR: DeX-Portrait提出了一种能够通过解耦的姿态和表情信号生成富有表现力的肖像动画的新方法，解决了现有扩散模型无法实现头部姿态和面部表情高保真解耦控制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的肖像动画方法无法实现头部姿态和面部表情的高保真解耦控制，这限制了仅表情编辑或仅姿态编辑等应用场景。

Method: 1) 将姿态表示为显式的全局变换，表情表示为隐式潜在编码；2) 设计强大的运动训练器学习姿态和表情编码器以提取精确解耦的驱动信号；3) 通过双分支条件机制将姿态变换注入扩散模型，通过交叉注意力注入表情潜在编码；4) 设计渐进混合分类器自由引导以保持身份一致性。

Result: 实验表明，该方法在动画质量和解耦可控性方面均优于现有最先进的基线方法。

Conclusion: DeX-Portrait通过解耦的姿态和表情表示以及创新的条件机制，实现了高质量且可控的肖像动画生成，为表情和姿态的独立编辑提供了有效解决方案。
...
TL;DR: EmoCaliber：一种为多模态大语言模型添加情感预测置信度表达能力的三阶段训练框架，旨在解决视觉情感理解中的主观性问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉情感理解中将情感预测视为确定性任务，输出单一情感标签，忽视了情感感知的主观性，无法反映不同观众可能存在的多种合理解释。

Method: 提出三阶段训练框架：1）结构化推理能力培养；2）置信度表达能力训练；3）置信度表达校准。最终得到EmoCaliber模型，能够在预测情感时同时表达置信度。

Result: 在统一的VECBench基准测试中，EmoCaliber在情感预测和置信度估计两方面均优于现有方法，验证了该方法的有效性。

Conclusion: 通过为多模态大语言模型添加置信度表达能力，能够更好地处理视觉情感理解中的主观性问题，这是迈向更可靠视觉情感理解系统的可行一步。
...
TL;DR: GeoMELT是一个基于编码器架构的多任务学习模型，专门用于遥感图像文本生成和跨模态检索，相比大型视觉语言模型参数更少、计算成本更低。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在遥感领域虽然能处理多任务，但参数量大导致训练和推理成本高昂，限制了大多数机构的使用。需要开发参数更少但仍能有效处理多任务的学习模型。

Method: 采用编码器架构（而非编码器-解码器架构），设计了一个紧凑的多任务学习模型GeoMELT，能够统一处理遥感图像文本生成和跨模态检索这两类通常不在同一模型中探索的任务。

Result: 在标准基准测试中，GeoMELT模型表现出良好的效果和效率，证实了所提方法的有效性和计算效率。

Conclusion: 编码器架构的多任务学习模型能够在保持紧凑参数规模的同时，有效处理遥感领域的多任务需求，为资源受限的机构提供了可行的解决方案。
...
TL;DR: BLANKET是一种专门为婴儿视频设计的匿名化方法，通过扩散模型生成新身份并保持时间一致性，在保护面部属性和减少伪影方面优于DeepPrivacy2。
...
Method: 采用两阶段方法：1）使用扩散模型通过修复技术生成与原始身份兼容的新随机人脸；2）通过时间一致的人脸交换和真实表情转移，将新身份无缝融入每帧视频。

Result: 在婴儿短视频数据集上评估，BLANKET在去识别化、面部属性保留、对人体姿态估计任务的影响以及伪影控制等方面均优于DeepPrivacy2。
...
TL;DR: GRAN-TED提出了一种为扩散模型生成鲁棒、对齐和细致文本嵌入的新范式，包括TED-6K评估基准和两阶段训练方法，显著提升了文本到图像/视频生成的语义保真度。
...
Method: 1) 提出TED-6K文本基准，通过轻量级统一适配器实现高效鲁棒的编码器质量评估；2) 开发两阶段训练范式：先在多模态大语言模型上进行微调以获得更好的视觉表示，然后通过层加权方法提取更细致和强大的文本特征。
...
Result: 语义和几何线索结合表现最佳；纯文本提示表现最差，特别是对于不规则形状目标；轻量微调在几何规则和视觉显著目标上提供实用性能-努力平衡
...
TL;DR: 提出MoonSeg3R方法，首次实现在线单目3D实例分割，通过CUT3R提供几何先验，在ScanNet200和SceneNN上达到与RGB-D系统相当的性能


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖带姿态的RGB-D序列，无法处理在线单目3D实例分割这一新实用场景，需要克服这一限制

Method: 利用CUT3R提供单目RGB流的几何先验，提出三个关键组件：1）带空间语义蒸馏的自监督查询细化模块；2）提供时间一致性的3D查询索引内存；3）作为掩码身份描述符的状态分布令牌

Result: MoonSeg3R是首个实现在线单目3D分割的方法，在ScanNet200和SceneNN上达到与最先进RGB-D系统竞争的性能

Conclusion: 成功解决了在线零样本单目3D实例分割的挑战，通过利用重构基础模型和提出的三个组件，实现了与RGB-D系统相当的性能
...
TL;DR: IMKD是一种基于多级知识蒸馏的雷达-相机融合框架，通过三阶段强度感知蒸馏策略，在保持传感器固有特性的同时增强互补优势，在nuScenes基准上达到67.0% NDS和61.0% mAP的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法通常直接将模态特定特征传输到每个传感器，这会扭曲其独特特性并降低各自的优势。需要一种能保持传感器固有特性同时增强其互补优势的融合方法。

Method: IMKD采用三阶段强度感知蒸馏策略：1) LiDAR到雷达的强度感知特征蒸馏，用细粒度结构线索增强雷达表示；2) LiDAR到融合特征的强度引导蒸馏，选择性突出几何和深度信息；3) 相机-雷达强度引导融合机制，促进有效特征对齐和校准。

Result: 在nuScenes基准测试中，IMKD达到67.0% NDS和61.0% mAP，超越了所有先前基于蒸馏的雷达-相机融合方法。

Conclusion: IMKD通过多级知识蒸馏框架成功实现了雷达-相机融合，在保持传感器固有特性的同时增强了互补优势，为不使用LiDAR的高性能3D目标检测提供了有效解决方案。
...
TL;DR: Qwen-Image-Layered是一种端到端扩散模型，可将RGB图像分解为多个语义解耦的RGBA图层，实现固有可编辑性，支持独立编辑而不影响其他内容。
...
Method: 提出三个关键组件：1) RGBA-VAE统一RGB和RGBA图像的潜在表示；2) VLD-MMDiT架构支持可变数量图像层分解；3) 多阶段训练策略将预训练图像生成模型适配为多层图像分解器。此外，构建从PSD文件中提取和标注多层图像的管道来解决高质量训练数据稀缺问题。
...
<details>
  <summary>Details</summary>
Motivation: 虽然结构从运动（SfM）技术在相机内参和外参估计方面取得了进步，但在动物行为研究和监控视频分析等应用中，现有方法仍面临精度和鲁棒性挑战，特别是对于具有强径向畸变的相机。

Method: 1. 研究如何从密集匹配器中最佳地子采样预测对应点以用于估计过程；2. 研究增量添加视图的选择标准；3. 在全局SfM设置中使用VGGT初始化位姿并应用提出的对应点子采样方法。

Result: 在严格的定量评估中，该方法显示出显著效果，特别是对于具有强径向畸变的相机（本文方法79.9% vs 原始VGGT 40.4%）。提出的流程适用于广泛的相机设置。

Conclusion: 该方法通过优化SfM流程中的关键设计选择，显著提升了相机位姿估计的精度和鲁棒性，特别是在具有挑战性的相机配置下，使其成为动物行为分析和法医分析的有用工具。
...
TL;DR: 该研究提出使用亚太赫兹逆合成孔径雷达(ISAR)进行空间域感知，通过序列特征检测和跟踪技术识别卫星外部结构，提高特征检测和分类的置信度。


<details>
  <summary>Details</summary>
Motivation: 随着近地空间环境中常驻空间物体(RSOs)数量快速增长，需要获取其状态和能力的详细信息以提供空间域感知(SDA)。基于空间的传感能够在更短距离、不受大气影响、全方位视角下对RSOs进行检查。

Method: 使用亚太赫兹ISAR成像系统，通过元启发式模拟器生成ISAR图像。采用梯度比方法进行边缘检测，利用双加权霍夫变换检测线性特征，并通过仿射变换实现帧间对齐，在序列图像中跟踪特征演化。

Result: 研究展示了在100公里范围内实现亚厘米级图像分辨率，通过序列特征跟踪提高了特征检测和分类的置信度，并以阴影检测作为示例验证了方法的鲁棒性。
...
TL;DR: 提出4D占用时空持续性概念，解决自动驾驶中的反应式预测和主动式预测任务，并建立新基准和世界模型


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要对3D场景有持续的理解，能够应对时间干扰并考虑潜在未来动作。现有方法在处理时间扰动（如语义标签错误、丢帧）方面存在不足
...
Conclusion: OccSTeP概念和OccSTeP-WM模型有效解决了自动驾驶中的时空持续性预测问题，为处理时间扰动提供了新解决方案，代码和数据将开源
...
TL;DR: AllSky：一种从物理捕获的HDRI直接学习的灵活全天候天空模型，通过用户控制的太阳位置和云层形态，实现直观的环境图控制，达到最先进的天空模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有天空模型在真实再现自然天空方面存在不足，DNN生成的环境图无法提供与物理捕获HDR图像相同的色调、阴影和照明效果，且无法同时支持照片级真实感和户外照明所需的22档动态范围。

Method: 提出AllSky模型，直接从物理捕获的HDRI学习，研究天空模型的输入模态、色调映射、条件设置和评估方法，允许用户直观控制太阳位置和云层形态。

Result: AllSky实现了最先进的天空模型性能，通过提出的评估方法证明现有DNN天空模型无法与物理捕获HDRI或参数化天空模型互换，当前限制阻碍了下游应用的可扩展性和准确照明。

Conclusion: 该工作展示了HDR文献在天空建模方面的进展，AllSky通过提供灵活的用户控制和改进的性能，为室外场景的光照渲染提供了更准确的环境图解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前前景条件修复方法存在空间关系幻觉问题，包括不恰当的比例、位置关系和视角。由于空间合理性的主观性难以量化，传统基于奖励的RLHF方法难以应用，需要专门解决这一问题的框架。

Method: 提出InpaintDPO框架：1) MaskDPO将偏好优化限制在背景区域，避免梯度冲突；2) 条件非对称偏好优化通过差异化裁剪操作增强边界一致性；3) 共享共性偏好优化从高质量样本中学习空间共性。

Result: 该框架能有效提升前景与背景之间的空间合理性，减少空间关系幻觉，增强边界一致性，同时保持前景的鲁棒性。

Conclusion: InpaintDPO是首个专门针对前景条件修复中空间合理性问题的DPO框架，通过创新的优化策略解决了传统方法难以处理的空间关系幻觉问题，为可控图像生成提供了更可靠的解决方案。
...
TL;DR: 该论文发现软标签在有限图像裁剪下容易出现局部语义漂移问题，提出通过结合硬标签来校准语义漂移的新训练范式HALD，在ImageNet-1K上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 软标签已成为知识转移的主流范式，但在每张图像仅使用有限数量裁剪时，软标签容易出现局部语义漂移问题。裁剪可能视觉上类似于另一个类别，导致其软嵌入偏离原始图像的真实语义，这种局部视觉内容与全局语义含义之间的不匹配会引入系统误差和训练测试分布不对齐。
...
Conclusion: 研究重新确立了硬标签作为补充工具的重要性，呼吁重新思考硬标签在软标签主导的训练中的作用。硬标签提供了内容无关的锚点来校准语义漂移，混合软硬标签可以解决局部语义漂移问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度视觉模型对常见图像损坏（如噪声、模糊等）的脆弱性问题，提升模型的鲁棒性
...
Result: Skyra在包含3K高质量样本的ViF-Bench基准测试中超越了现有方法，为可解释AI生成视频检测提供了有价值的见解。

Conclusion: Skyra通过识别人类可感知的视觉伪影作为证据，实现了可解释的AI生成视频检测，为推进可解释AI生成视频检测提供了重要贡献。
...
Method: 提出Resampling Forcing框架：1) 自重采样方案在训练时模拟推理时模型误差；2) 稀疏因果掩码确保时间因果性同时支持并行训练；3) 历史路由机制动态检索最相关的历史帧
...
Method: 在Transformer基础模型（如DINO、SAM、CLIP）中添加中间3D感知注意力层，这些层帮助匹配不同视图的特征，实现跨视图特征一致性

Result: 定量实验表明，该方法相比现有基础模型显著改善了特征匹配性能，在表面法线估计和多视图分割任务中表现优异

Conclusion: 提出的多视图基础模型转换方法有效解决了跨视图特征一致性问题，为多视图计算机视觉任务提供了直接图像空间操作的新途径
...
TL;DR: DiffusionVL：将现有自回归模型转换为扩散视觉语言模型的新方法，通过简单微调实现范式转换，在少量数据下实现性能大幅提升和推理加速


<details>
  <summary>Details</summary>
Motivation: 扩散范式在多模态研究中展现出潜力，但由于基础扩散语言模型能力有限，扩散视觉语言模型性能仍落后于主流自回归模型。研究者希望探索能否基于现有强大的自回归模型构建扩散视觉语言模型。
...
Conclusion: 自回归模型到扩散视觉语言模型的范式转换是有效的，直接转换自回归语言模型到扩散视觉语言模型可行，DiffusionVL在少量数据下实现全面性能提升和推理加速。
...
TL;DR: 本文挑战了OpenAI关于大语言模型幻觉是激励错配导致的观点，认为幻觉是Transformer架构的结构性必然，无法通过优化激励解决，需要外部验证模块。


<details>
  <summary>Details</summary>
Motivation: 反驳OpenAI认为幻觉是激励错配导致的偶然行为问题的观点，证明幻觉是Transformer架构的固有结构性特征。

Method: 基于先前关于结构性幻觉的研究，使用Licensing Oracle进行实证实验，分析Transformer的嵌入空间和伪本体论特征。

Result: 实验表明幻觉只能通过外部真实验证和弃权模块消除，无法通过激励、提示或微调改变；Licensing Oracle实现了完美的跨领域弃权精度。

Conclusion: 幻觉是生成式架构的结构性属性，可靠AI需要区分语言流畅性和认知责任的混合系统。
...
TL;DR: T5Gemma 2是新一代轻量级开放编码器-解码器模型，基于Gemma 3模型扩展了多语言、多模态和长上下文能力，通过共享嵌入和合并注意力机制提升效率。


<details>
  <summary>Details</summary>
Motivation: 延续T5Gemma系列的发展，将文本领域的编码器-解码器适配方法扩展到多模态领域，同时提升模型效率，为社区提供更强大的轻量级多模态模型。

Method: 1) 沿用T5Gemma的UL2适配方法，将预训练的仅解码器模型转换为编码器-解码器架构；2) 提出两种效率改进方法：共享编码器和解码器的词嵌入，以及将解码器的自注意力和交叉注意力合并为单一联合模块。

Result: 实验证明适配策略在不同架构和模态上具有通用性，编码器-解码器架构在长上下文建模方面表现独特优势。T5Gemma 2在预训练性能上与Gemma 3相当或更好，在后训练性能上显著优于Gemma 3。

Conclusion: 成功开发了新一代T5Gemma 2模型，展示了编码器-解码器架构在多模态和长上下文任务中的优势，发布了270M-270M、1B-1B和4B-4B三个规模的预训练模型供研究使用。
...
TL;DR: 本文改进了新闻观点分类流程，通过微调大语言模型和用Wikidata丰富实体表示，在英国移民辩论数据集上取得了最佳效果。
...
Method: 改进现有新闻观点分类流程：1) 微调大语言模型用于观点分类；2) 使用Wikidata中相关参与者的语义描述来丰富声明表示。在英国移民辩论数据集上进行评估。

Result: 两种机制都能独立提高分类性能，但它们的整合产生了最佳结果，特别是当使用能够处理长输入的大语言模型时。

Conclusion: 通过结合微调LLM和丰富实体语义表示的方法，可以显著提升新闻观点分类的准确性和效果，为理解媒体话语动态提供了更有效的工具。
...
<details>
  <summary>Details</summary>
Motivation: 评估现有LLM在药学专业任务上的表现，并开发一种无需修改模型架构的外部知识整合方法来提升其准确性，为药学AI应用提供实用解决方案。

Method: 1) 使用141个问题的药学数据集对11个不同参数规模的LLM进行基准测试；2) 开发三步检索增强生成(RAG)管道DrugRAG，从验证来源检索结构化药物知识，将基于证据的上下文增强到模型提示中。

Result: 基线准确率在46%到92%之间，GPT-5(92%)和o3(89%)表现最佳；参数少于80亿的模型准确率低于50%。DrugRAG使所有测试模型准确率提升7-21个百分点，例如Gemma 3 27B从61%提升到71%，Llama 3.1 8B从46%提升到67%。
...
<details>
  <summary>Details</summary>
Motivation: MHSA的二次计算复杂度限制了大型语言模型在长上下文任务中的扩展。现有的稀疏和线性化注意力机制要么损害全局依赖表示，要么无法有效捕捉多尺度语义粒度。

Method: 提出MAHA框架：1）通过可学习下采样算子将输入序列动态划分为层次化尺度；2）将尺度特定注意力矩阵的融合建模为资源分配问题，采用凸优化或纳什均衡博弈论方法求解；3）在混合扩张卷积transformer骨干中实现，使用可微分优化层进行端到端训练。

Result: 实验评估显示MAHA具有优越的可扩展性；在序列长度为4096时，经验FLOPs分析确认计算成本相比标准注意力降低81%。

Conclusion: MAHA在优化理论和序列建模之间架起桥梁，为下一代LLM提供了可扩展解决方案，在保持局部细节和全局上下文保真度之间实现了理论最优平衡。
...
TL;DR: 本文通过翻译Flickr30k数据集并扩展为罗马尼亚语视觉问答数据集，微调开源视觉语言模型，提升了罗马尼亚语多模态AI能力。


<details>
  <summary>Details</summary>
Motivation: 针对低资源语言（特别是罗马尼亚语）的多模态AI资源匮乏问题，旨在通过构建数据集和模型微调来缩小资源差距，促进生成式AI的民主化。

Method: 1) 将Flickr30k数据集翻译为罗马尼亚语并扩展为视觉问答数据集；2) 选择LLaMA 3.2、LLaVA 1.6和Qwen2三个模型家族的开源视觉语言模型；3) 使用参数高效的LoRA方法进行微调。

Result: 微调后的模型在罗马尼亚语视觉问答任务上表现提升，同时在未训练的图像描述生成任务上也获得改进。Qwen2-VL-RoVQA模型在BERTScore F1上分别提升6.05%和2.61%，语法错误显著减少，罗马尼亚语流畅度提高。

Conclusion: 通过构建罗马尼亚语多模态数据集和微调开源视觉语言模型，有效提升了罗马尼亚语多模态AI能力，为低资源语言的多模态NLP发展提供了可行路径。
...
TL;DR: 该研究系统评估了40个多模态大语言模型在化学奥林匹克竞赛题目上的表现，发现当前模型在视觉-语言融合方面存在严重问题，甚至移除图像有时反而提高准确率，但思维链提示能显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 多模态科学推理对大型语言模型仍具挑战，特别是在化学领域，问题解决依赖于符号图表、分子结构和结构化视觉数据。当前模型在整合视觉和文本信息进行科学推理方面的能力尚不明确。

Method: 使用超过20年美国国家化学奥林匹克竞赛题目构建基准，系统评估40个专有和开源多模态LLM（包括GPT-5、o3、Gemini-2.5-Pro、Qwen2.5-VL等），采用思维链提示、消融研究和基于遮挡的可解释性分析。

Result: 许多模型在多模态融合方面表现不佳，在某些情况下移除图像反而提高准确率，表明视觉-语言整合存在错位。思维链提示能一致地提高准确率和视觉基础，通过可解释性分析验证了其效果。

Conclusion: 当前多模态LLM在科学推理能力上存在关键限制，需要开发更鲁棒和可解释的多模态系统。该研究为领域特定多模态AI提供了及时的基准，强调了人工智能与科学推理交叉领域需要进一步进展。
...
<details>
  <summary>Details</summary>
Motivation: 任务导向的公共频道通信（如海事VHF对话）具有非正式语言和隐式转换特点，传统方法在处理此类对话主题分割时存在局限，需要更有效的解决方案。

Method: 提出DASH-DTS框架，包含三个核心贡献：1）通过对话握手识别检测主题转换；2）基于相似性引导的示例选择进行上下文增强；3）生成选择性正负样本以提高模型判别力和鲁棒性。

Result: 实验结果表明，该框架在VHF-Dial和标准基准测试中实现了多个SOTA分割可信精度，为操作对话的稳定监控和决策支持奠定坚实基础。

Conclusion: DASH-DTS框架能有效处理非正式、隐式转换的对话主题分割问题，提供可解释的推理和置信度评分，并发布了首个真实海事VHF数据集推动该领域研究。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型从弱监管的预训练语料中继承了有毒、偏见和NSFW信号，存在安全风险，特别是在对抗性触发条件下，现有的训练无关去毒方法难以有效处理。
...
Conclusion: SGM提供了一种可解释、低成本的毒性控制多模态生成解决方案，能够有效缓解多模态大语言模型的安全风险。
...
Conclusion: 为概率计算时代的"可观测软件工程"奠定了基础，使LLM能够作为可编程的自优化系统运行，提供传统软件工程所需的确定性保证。
...
TL;DR: SCOPE框架通过置信度加权伪标签估计和动态子群划分，解决测试时强化学习中多数投票导致的确认偏差和稀疏奖励问题，显著提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 测试时强化学习使用多数投票结果作为伪标签来减少对标注数据的依赖，但这种方法容易导致确认偏差且奖励稀疏，限制了整体性能提升。

Method: 提出SCOPE框架：1）引入逐步置信度加权伪标签推导，优先选择高质量推理路径而非简单频率统计；2）动态划分候选输出池为独立子群，平衡推理质量与探索多样性；3）通过子群内重复采样获得局部共识，提供多样化监督目标。

Result: 在多个模型和基准测试上的实验表明，SCOPE始终优于现有基线方法，在AIME 2025上实现13.1%的相对提升，在AMC上实现8.1%的相对提升。

Conclusion: SCOPE通过整合模型置信度和动态子群划分，有效解决了测试时强化学习中的确认偏差和稀疏奖励问题，为大语言模型推理能力的提升提供了有效框架。
...
TL;DR: 本文介绍了乐天旅行评论的大规模语料库，包含2009-2024年730万条客户评论，涵盖丰富的元数据和用户评分，并分析了2019-2024年间的数据漂移因素。


<details>
  <summary>Details</summary>
Motivation: 构建一个大规模、时间跨度长的旅行评论数据集，为研究客户反馈、酒店服务质量、数据漂移等现象提供资源支持。

Method: 收集乐天旅行平台2009-2024年的730万条客户评论，每条记录包含评论文本、酒店回复、匿名用户ID、日期、住宿信息、房间类型、旅行目的、同行人员以及多维度评分等丰富元数据。

Result: 创建了包含730万条评论的大规模语料库，时间跨度16年，提供了详细的统计信息，并通过统计方法分析了2019-2024年间的数据漂移驱动因素。

Conclusion: 该语料库为旅行评论研究提供了宝贵资源，有助于理解客户反馈模式、服务质量评估以及时间维度上的数据变化规律。
...
Method: 引入基于过程的、多证据的P-MFA（教学多因素评估）模型，该模型受多因素认证逻辑启发，关注学习过程而非最终产品

Result: 提出了一种新的评估框架，旨在解决学生使用AI工具带来的评估挑战，强调过程导向和多证据验证

Conclusion: 需要从传统的产品导向评估转向过程导向评估，P-MFA模型为解决AI时代的教育评估问题提供了可行方案
...
TL;DR: RFKG-CoT提出了一种改进的知识图谱增强推理方法，通过关系驱动的自适应跳数选择器和少样本上下文学习路径指导机制，显著提升了LLM在知识密集型QA任务中的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有方法如KG-CoT虽然通过整合知识图谱路径提高了可靠性，但存在两个主要问题：1）僵化的跳数选择（仅基于问题驱动）；2）推理路径利用不足（缺乏指导）。这导致大语言模型在知识密集型QA中仍会产生幻觉。
...
Conclusion: RFKG-CoT通过自适应跳数选择和路径指导机制，有效解决了现有KG增强方法的局限性，显著提高了LLM在知识密集型QA任务中的准确性和可靠性，减少了幻觉问题。
...
TL;DR: Yes-MT团队在WMT 2024低资源印度语言翻译任务中，针对英语与阿萨姆语、米佐语、卡西语、曼尼普尔语之间的翻译，测试了多种方法，包括微调预训练模型、LoRA微调、LLM零/少样本提示等，评估了不同方法在低资源翻译任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 探索在低资源印度语言翻译任务中，不同技术方法的有效性和潜力，特别是针对英语与阿萨姆语、米佐语、卡西语、曼尼普尔语之间的翻译挑战。

Method: 采用了多种方法：1）微调预训练模型（mT5、IndicBart）的多语言和单语言设置；2）使用LoRA微调IndicTrans2；3）LLM（Llama 3、Mixtral 8x7b）的零样本和少样本提示；4）Llama 3的LoRA监督微调；5）从头训练Transformer模型。

Result: 使用SacreBLEU和CHRF在WMT23测试数据上评估结果，突显了低资源翻译的挑战，并展示了LLM在这些任务中的潜力，特别是通过微调后的表现。

Conclusion: 低资源语言翻译面临显著挑战，但大型语言模型通过适当的微调方法显示出解决这些问题的潜力，为低资源语言翻译任务提供了有前景的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型机器遗忘评估基准存在两大局限：仅支持英语，且只关注实体级遗忘（完全删除某个人的所有信息）。这限制了机器遗忘技术在多语言环境下的发展和评估。

Method: 创建FAME基准，包含1000个虚构演员的完整传记，每个传记包含20个主题信息，组织成结构化类别（传记、职业、成就、个人信息）。生成20000个问答对，支持五种语言（英语、法语、德语、意大利语、西班牙语）。提供两个数据集划分来支持实体级遗忘（完全删除身份）和实例级遗忘（删除特定事实但保留其他信息）。

Result: FAME基准提供了系统评估机器遗忘技术在多语言环境下的能力，支持两种遗忘场景的对比分析。由于使用完全虚构的数据，确保模型在预训练阶段从未接触过这些信息，从而实现对遗忘方法的受控评估。

Conclusion: FAME基准解决了现有机器遗忘评估的局限性，为多语言环境下的机器遗忘技术提供了标准化评估框架，支持实体级和实例级遗忘的全面评估，促进了机器遗忘技术的发展和应用。
...
TL;DR: SynGP500是一个包含500份澳大利亚全科医疗记录的合成数据集，通过临床医生策划，整合了课程知识、流行病学数据和多样化的咨询场景，旨在提供更通用的模型训练资源。


<details>
  <summary>Details</summary>
Motivation: 解决澳大利亚临床NLP研究中缺乏高质量、隐私保护的全科医疗数据集的空白，同时克服自然发生病例分布的限制，提供更全面的训练资源。

Method: 采用临床医生策划的方法，整合RACGP 2022课程知识、BEACH研究的流行病学数据，设计包含常见和不常见病例的多样化咨询场景，并故意加入真实医疗记录中的复杂性元素。

Result: 通过多维度验证显示数据集质量：流行病学模式与真实澳大利亚GP咨询模式对齐，语言变异度高，语义覆盖广泛，在自监督医疗概念提取任务中显示F1分数提升。

Conclusion: SynGP500填补了澳大利亚临床NLP研究的关键空白，为研究人员和教育工作者提供了开发和评估澳大利亚全科医疗NLP方法的资源，同时保护患者隐私。
...
TL;DR: 本文提出PPPO方法，通过聚焦于LLM推理的前缀token进行优化，显著提升强化学习与可验证奖励的效果，相比现有方法用更少的训练token获得更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法对所有生成token进行统一训练，忽视了不同token对推理贡献的差异，特别是前缀token对后续推理轨迹的关键影响，导致训练效率低下。

Method: 提出渐进式前缀token策略优化（PPPO），基于"路径依赖"理论识别LLM推理中的"起始锁定效应"，采用渐进式前缀保留和延续累积奖励两种策略，专注于优化前缀推理过程。

Result: 在多种推理任务上的实验表明，PPPO显著优于现有RLVR方法，仅使用26.17%的训练token就实现了18.02%的准确率提升。

Conclusion: 通过聚焦于前缀token的优化，PPPO有效提升了LLM的推理能力，证明了在RLVR框架中识别和优化关键推理阶段的重要性。
...
TL;DR: 评估大语言模型在沸石合成实验信息提取任务中的表现，发现事件分类效果良好但参数提取性能有限，提示策略改进效果不明显，揭示了模型架构的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法尚未系统评估大语言模型在沸石合成实验信息提取这一领域特定任务中的表现，需要研究不同提示策略在科学信息提取中的有效性。

Method: 研究四种提示策略（零样本、少样本、事件特定、反思式）在六个先进大语言模型上的表现，使用ZSEE数据集（1,530个标注句子），评估四个子任务：事件类型分类、触发文本识别、参数角色提取和参数文本提取。

Result: 事件类型分类表现良好（80-90% F1），但细粒度提取任务表现一般，特别是参数角色和参数文本提取（50-65% F1）。GPT-5-mini表现出极端的提示敏感性（11-79% F1变化）。高级提示策略相比零样本方法改进有限。

Conclusion: 大语言模型虽然能实现高层次理解，但精确提取实验参数需要领域适应模型。研究揭示了模型在科学信息提取中的系统性幻觉、过度泛化和无法捕捉合成特定细微差别等架构局限性，为科学信息提取提供了定量基准。
...
TL;DR: 应用Brookes分类离散度指标分析当代阿拉伯应用语言学研究的主题结构，发现该领域具有极高的离散度（Δ=0.194），表明研究主题高度分散而非集中。
...
Method: 使用Brookes分类离散度指标（Δ），基于2019-2025年间的1,564篇出版物数据集，将研究分为八个核心子学科进行计算分析。

Result: 计算得到离散度指数Δ=0.194，这一极低值表明阿拉伯应用语言学领域具有极高的主题离散度；计算语言学是主导但非霸权力量，与社会语言学、语言教学等其他子领域共存。

Conclusion: 该研究阐明了Brookes公式的正确应用方法，展示了其在领域特征描述中的实用性，并为跨学科评估提供了可复制的文献计量学方法。
...
TL;DR: 研究发现将提示改写为诗歌形式是一种有效的对抗性攻击方法，能显著提高大语言模型的安全漏洞率，在单轮交互中成功率可达90%以上。研究指出当前对齐机制过度依赖表面模式，且缺乏对葡萄牙语等高复杂度语言的评估。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的对齐机制存在严重漏洞，研究发现将拒绝执行的指令改写为诗歌形式后，模型会执行这些指令。这暴露了当前对齐机制过度依赖表面模式而非深层语义理解的问题。同时，研究指出现有评估忽略了葡萄牙语等高复杂度语言，存在重要评估空白。

Method: 研究采用对抗性诗歌作为通用单轮越狱机制，将原本被拒绝的指令改写为诗歌形式。通过手动编写和自动生成两种方式创建诗歌提示，在基于MLCommons AILuminate的基准测试中评估安全失败率。研究分析了不同训练方法（RLHF、宪法AI、混合管道）在诗歌攻击下的表现。

Result: 诗歌形式的提示能产生高达18倍的安全失败率。手动编写的诗歌攻击成功率约62%，自动生成版本约43%，某些模型在单轮交互中成功率超过90%。研究发现这种效果是结构性的，所有训练方法在最小符号形式变化下都表现出一致的性能退化。

Conclusion: 诗歌攻击通过将提示转移到稀疏监督的潜在区域，揭示了当前防护机制过度依赖表面模式的深层局限。表面鲁棒性与实际脆弱性之间的脱节暴露了当前对齐机制的根本缺陷。研究强调需要参数化扫描、韵律和韵律变化来测试特定语言模式（如葡萄牙语）的漏洞，这些在当前评估中被完全忽略。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在复杂推理任务中，对中间推理过程和最终答案使用统一的语言密度，导致计算效率低下。研究发现推理过程主要为模型自身计算服务，而回答则是为了人类理解，这种差异使得可以采用压缩符号化语言进行中间计算，同时保持最终解释的人类可读性。

Method: 提出Denser（双密度推理）框架，包含三个组件：1）查询处理模块分析输入问题；2）高密度压缩推理机制用于高效中间计算；3）答案生成组件将压缩推理转换为人类可读解决方案。

Result: 在多个推理问答基准测试中，Denser相比标准思维链方法减少高达62%的token消耗，同时保持或提高准确性。对于复杂的多步推理问题，效率提升尤为显著。

Conclusion: 通过区分推理和回答阶段的信息密度，Denser框架在保持模型推理能力的同时显著提高了计算效率，为LLM的高效推理提供了新思路。
...
TL;DR: ORACLE平台将每日新闻转化为周度决策洞察，为芬兰应用科学大学提供定制化信息分析服务，通过新闻爬取、相关性过滤、PESTEL分类和TRSG图构建，实现稳定的生产级系统。


<details>
  <summary>Details</summary>
Motivation: 为芬兰应用科学大学提供从海量新闻中提取结构化、可操作的周度洞察，帮助大学基于外部环境变化做出及时决策，特别是支持课程智能分析等应用场景。

Method: 平台包含新闻爬取与版本管理、大学特定相关性过滤、内容嵌入、PESTEL维度分类、构建时间依赖递归摘要图（TRSG）、轻量级变化检测器、主题分组和PESTEL感知分析。

Result: 开发了稳定运行的ORACLE生产系统，能够自动生成周度决策洞察，识别新闻中的新增、删除和变化内容，并按主题和PESTEL维度进行分组分析。

Conclusion: ORACLE系统成功将每日新闻转化为结构化的周度洞察，为大学决策提供支持，论文详细介绍了管道设计、稳定生产的关键选择，并提出了课程智能用例和评估计划。
...
<details>
  <summary>Details</summary>
Motivation: 动机性访谈（MI）是促进健康行为改变的有效咨询方法，但其推广受到需要高度训练的人类咨询师的限制。本研究旨在探索一种可扩展的替代方案，通过开发基于大型语言模型的动机性访谈系统。

Method: 研究首先整理了五个中文心理咨询语料库，使用GPT-4和MI提示将高质量数据集转录为2,040个MI风格对话。然后对三个中文开源LLM（Baichuan2-7B-Chat、ChatGLM-4-9B-Chat和Llama-3-8B-Chinese-Chat-v2）进行微调，形成MI-LLMs。评估采用基于回合的自动指标和专家手动编码（使用MITI编码手册4.2.1）。

Result: 微调显著提高了所有三个模型的BLEU-4和ROUGE分数。手动编码显示MI-LLMs在技术和关系全局评分以及MI一致性比率方面接近真实MI对话水平，但复杂反思和反思-问题比率仍然较低。

Conclusion: 研究初步证明MI导向的微调可以使通用LLMs具备核心的MI一致咨询行为，为AI辅助健康行为改变支持提供了可扩展途径，但需要在数据规模、复杂MI技能和真实世界干预试验方面进一步研究。
...
TL;DR: 该研究首次对孟加拉语在2024年孟加拉国大规模抗议期间的公众情绪进行分析，创建了包含2028条标注新闻标题的数据集，使用语言特定模型在情感分类任务上超越了多语言模型和传统机器学习方法。


<details>
  <summary>Details</summary>
Motivation: 情感分析在选举和社交媒体趋势等场景已有较多研究，但在孟加拉语环境下，特别是针对社会动荡时期的情绪动态研究存在显著空白。本研究旨在填补这一空白，探索孟加拉国2024年大规模抗议期间的公众情绪变化。

Method: 研究创建了包含2028条标注新闻标题的独特数据集，分类为愤怒、希望和绝望三种情感。使用潜在狄利克雷分配（LDA）识别主要主题（如政治腐败、公众抗议），并分析互联网封锁等事件对情绪模式的影响。比较了语言特定模型与多语言转换器（mBERT、XLM-RoBERTa）和传统机器学习方法（SVM、逻辑回归）的性能。

Result: 语言特定模型在情感分类任务上表现最佳，超越了多语言转换器（mBERT: 67%，XLM-RoBERTa: 71%）和传统机器学习方法（SVM和逻辑回归：均为70%）。研究还发现政治腐败和公众抗议是主要主题，互联网封锁等事件显著影响了情绪模式。
...
TL;DR: CTKVR提出了一种新颖的centroid-then-token KV检索方案，通过两阶段检索策略解决长上下文推理中KV缓存效率问题，在保持准确性的同时显著提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长上下文场景（如多轮对话）中面临KV缓存内存开销大和访问延迟高的问题。现有动态KV选择方法存在权衡：块级索引会因检索不相关KV条目而降低准确性，而令牌级索引则因低效检索机制导致高延迟。

Method: CTKVR采用两阶段检索策略：1）预填充阶段预计算轻量级质心用于质心粒度索引；2）令牌级细化用于精确KV检索。该方法基于关键观察：经过RoPE后位置相邻的查询向量具有高相似性，并共享大部分top-k KV缓存条目。同时实现了CPU-GPU协同执行的优化系统用于索引构建和搜索。

Result: 在多个基准测试中，CTKVR实现了优越性能，准确度下降小于1%。在96K上下文长度下，Llama-3-8B和Yi-9B的吞吐量分别提升了3倍和4倍，且在不同GPU硬件上均表现良好。

Conclusion: CTKVR通过创新的centroid-then-token KV检索方案，有效平衡了检索效率和准确性，解决了长上下文推理中的KV缓存效率瓶颈，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统语言学家提出的通用服务词汇表需要语言学专业知识、主观判断和大量时间，存在效率低下和可扩展性差的问题，需要更自动化的解决方案

Method: 开发自动化模型创建专业词汇表，通过客观标准筛选特定语料库子集中的核心词汇，实现词汇表的自动化、可扩展和个性化定制

Result: 使用该模型创建的专业词汇表在性能上超越了行业标准NGSL，用更少的词汇量达到了语言理解所需的95%覆盖率

Conclusion: 基于客观标准的专业词汇表创建方法可以实现自动化、可扩展和个性化，为全球语言学习者提供更优化的词汇学习方案
...
<details>
  <summary>Details</summary>
Motivation: 多词表达式在自然语言理解、处理和生成任务中带来诸多困难，如歧义、习语表达、低频使用和广泛变体等。虽然西方语言（特别是英语）在多词表达式处理方面取得了显著进展，但中文等表意文字语言在这方面仍然滞后。子词建模技术（如BPE）无法直接应用于汉字这样的表意文字脚本，因此需要专门针对中文的研究方法。

Method: 本研究采用系统性的方法，在MWE感知的神经机器翻译背景下研究汉字分解技术。通过实验考察汉字分解技术如何帮助表示中文词汇和字符的原始含义，以及如何有效解决多词表达式的翻译挑战。

Result: 论文报告了相关实验结果，展示了汉字分解技术在中文多词表达式处理中的效果。虽然没有具体数据，但研究探讨了该技术如何改善中文词汇表示和MWE翻译质量。

Conclusion: 汉字分解技术为中文多词表达式的处理提供了有效的解决方案，弥补了中文在MWE研究方面相对于西方语言的滞后。该研究为表意文字语言的自然语言处理任务提供了新的技术途径。
...
Method: 通过字节化现有子词级模型训练Bolmo，采用专门设计的架构解决字节级和子词级模型表达能力不匹配问题，使用有效的精确蒸馏目标，仅需不到1%的典型预训练token预算。
...
TL;DR: 研究者开发了PsyDefConv对话语料库和DMRS Co-Pilot标注辅助系统，用于自动识别心理防御机制，在临床对话中实现更高效、可靠的防御水平标注。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制是管理痛苦的重要策略，但其过度使用与心理健康问题相关。然而，防御机制复杂且难以可靠测量，特别是在临床对话中缺乏有效的标注工具和数据集。

Method: 1) 创建PsyDefConv语料库：包含200个对话、4709个话语，其中2336个求助者话语标注了防御水平；2) 开发DMRS Co-Pilot四阶段管道：提供基于证据的预标注；3) 进行平衡研究评估标注效率；4) 专家评审评估证据质量；5) 使用强语言模型进行基准测试。

Result: 1) 语料库标注一致性达到Cohen's kappa 0.639；2) Co-Pilot使平均标注时间减少22.4%；3) 专家评分：证据质量4.62/7，临床合理性4.44/7，洞察力4.40/7；4) 最佳模型macro F1约30%，倾向于过度预测成熟防御；5) 分析显示成熟防御最常见，存在情绪特异性偏差。
...
TL;DR: 论文探讨如何将LLMs安全可靠地引入关键信息流程，提出通过加权指标组合、上下文敏感度评估和置信度阈值来降低LLM-as-Judge评估中的错误风险


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地用于文本处理流程，可能替代人力瓶颈角色，但LLMs会犯错且某些处理角色涉及安全关键场景，需要找到安全可靠引入LLMs的方法

Method: 关注LLM流程中评估点提供的证据类型，特别是LLM-as-Judge评估框架；采用加权指标组合降低评估错误风险，利用上下文敏感度定义错误严重性，设计置信度阈值在评估者一致性低时触发人工审核

Result: 虽然无法从自然语言处理任务中获得确定性评估，但通过加权指标组合、上下文敏感度分析和置信度阈值设计，可以降低关键LaJ评估中的错误风险

Conclusion: LLMs在关键信息流程中的安全论证应关注评估证据类型，通过系统化的评估框架设计（加权指标、上下文敏感度、置信度阈值）来确保可靠性和安全性，而非依赖生成框架或图技术的性能声明
...
TL;DR: 本文对全监督微调(SFT)和参数高效微调(PEFT)方法(特别是LoRA)在问答任务中的性能进行了全面评估，通过秩扫描量化了两种方法的权衡，并分析了它们在领域内外适应中的泛化行为。


<details>
  <summary>Details</summary>
Motivation: 虽然参数高效微调方法因其计算效率而被广泛使用，但其配置参数(如秩)在下游问答任务和泛化能力方面的影响尚未得到充分探索。本研究旨在全面评估SFT和PEFT在推理和记忆任务中的表现差异。

Method: 在多个推理和记忆数据集上进行综合评估，进行秩扫描以量化SFT和PEFT之间的权衡。比较PEFT和SFT模型在领域内和领域外适应中的准确性，分析泛化行为和任务特定遗忘。通过谱特征和层间注意力结构分析内部表示。

Result: LoRA在某些情况下实现了与SFT相当甚至更优的性能，特别是在特定秩值下的推理任务中。研究揭示了不同的泛化行为和任务特定遗忘模式，并通过表示漂移和注意力结构变化提供了内部机制见解。

Conclusion: PEFT方法(特别是LoRA)在保持计算效率的同时，能够在某些任务上达到与全监督微调相当或更好的性能，特别是在推理任务中。研究为理解微调方法的内部表示变化和泛化行为提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 持续学习面临灾难性遗忘的挑战，即模型在学习新任务时忘记先前学到的知识，这在NLP任务中尤为突出

Method: 提出PPSEBM框架：1) 渐进参数选择为每个新任务分配特定参数；2) 能量模型生成先前任务的代表性伪样本；3) 生成样本指导参数选择过程

Result: 在多种NLP基准测试中，PPSEBM优于最先进的持续学习方法，有效缓解了灾难性遗忘

Conclusion: PPSEBM为NLP持续学习中的灾难性遗忘问题提供了一个有前景且鲁棒的解决方案
...
TL;DR: LatentQA方法训练LLM直接接受激活值作为输入并用自然语言回答相关问题，本文评估了这种激活预言机在分布外设置中的表现，发现即使窄训练也能良好泛化，多样化训练能进一步提升性能，在多个任务上匹配或超越白盒基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM激活理解技术通常使用复杂、专门的方法，而LatentQA提出了一种更简单的替代方案，但先前研究局限于狭窄的任务设置。本文旨在从通用主义视角评估LatentQA训练的模型（激活预言机）在分布外环境中的表现，并探索训练数据多样性如何影响性能扩展。

Method: 采用LatentQA方法训练LLM作为激活预言机，使其能够接受LLM激活值作为输入并用自然语言回答相关问题。研究评估了在分布外设置中的表现，包括从未使用微调模型激活进行训练的情况下，模型能否恢复微调到模型中的信息（如传记知识或恶意倾向）。主要评估包括四个下游任务，与先前的白盒和黑盒技术进行比较。

Result: 研究发现：1）激活预言机能够恢复微调到模型中但未出现在输入文本中的信息；2）即使是窄训练的LatentQA模型也能良好泛化；3）添加额外的训练数据集（如分类任务和自监督上下文预测任务）能带来一致的进一步改进；4）最佳激活预言机在所有四个任务上匹配或超越了先前的白盒基线方法，在3/4任务上表现最佳。

Conclusion: 多样化训练使模型能够用自然语言回答查询，赋予了用语言表达LLM激活信息的一般能力。这表明LatentQA方法具有通用潜力，能够有效理解LLM内部表示，为LLM激活解释提供了一种更简单而强大的替代方案。
...
TL;DR: 该论文将Transformer的自注意力机制解释为近似的向量符号架构(VSA)，通过这种代数视角分析语言模型的推理行为、失败模式，并提出VSA启发的架构改进方案。
...
Method: 将自注意力机制和残差流解释为近似VSA：查询和键定义角色空间，值编码填充物，注意力权重执行软解绑定，残差连接实现多个绑定结构的叠加。基于此视角分析思维链、程序推理、记忆增强工具使用，并提出VSA启发的架构偏置（显式绑定/解绑定头、超维记忆层）和训练目标。

Result: 建立了Transformer内部机制与向量符号计算之间的对应关系，解释了变量混淆、逻辑相关提示不一致等特征性失败模式，提出了测量"VSA相似度"和逻辑组合性的指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识图谱问答基准测试通常假设知识图谱是完整的，这忽略了现实世界中知识图谱往往不完整的事实。当直接支持的三元组缺失时，需要从现有事实中推理出答案，而现有方法在这种不完整情况下的表现尚未得到充分评估。

Method: 1. 提出构建知识图谱不完整性基准测试的方法论，移除直接支持的三元组但保留替代推理路径；2. 开发自适应图推理智能体（GR-Agent），将知识图谱构建为交互环境，将问答形式化为智能体与环境交互；3. GR-Agent使用图推理工具作为动作空间，维护潜在支持推理证据的记忆。

Result: 实验表明：1. 现有方法在知识图谱不完整情况下性能显著下降；2. GR-Agent在完整和不完整设置下均优于非训练基线方法；3. GR-Agent与基于训练的方法表现相当。

Conclusion: 知识图谱不完整性是评估知识图谱问答系统推理能力的重要维度。提出的GR-Agent通过将问答形式化为智能体与环境交互，有效处理了不完整知识图谱下的推理问题，为未来研究提供了有价值的基准和方法。
...
TL;DR: 该研究通过系统注入结构化配置知识，显著提升了LLM生成正确且意图对齐的Terraform基础设施即代码的能力，但发现LLM在技术正确性和意图对齐之间存在差距。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在生成正确且意图对齐的基础设施即代码方面成功率较低，需要改进LLM基于IaC生成的能力。

Method: 1. 增强IaC-Eval基准测试，加入云仿真和自动错误分析；2. 开发LLM辅助IaC代码生成的错误分类法；3. 实施从朴素检索增强生成到更复杂的图RAG方法的知识注入技术，包括图组件的语义丰富化和建模资源间依赖关系。

Result: 基线LLM性能较差（总体成功率27.1%），注入结构化配置知识后，技术验证成功率提升至75.3%，总体成功率提升至62.6%。但意图对齐方面出现平台期，揭示了"正确性-一致性差距"。

Conclusion: LLM通过结构化知识注入可以成为熟练的"编码者"，但在满足细微用户意图方面仍然是有限的"架构师"，技术正确性和意图对齐之间存在显著差距。
...
### [104] [Beyond Accuracy: A Geometric Stability Analysis of Large Language Models in Chess Evaluation](https://arxiv.org/abs/2512.15033)
*Xidan Song,Weiqi Wang,Ruifeng Cao,Qingya Hu*

Main category: cs.AI

TL;DR: 论文提出几何稳定性框架来评估大语言模型在象棋推理中的真实理解能力，发现准确性与稳定性之间存在悖论：高准确率模型在几何变换下性能急剧下降，表明其依赖模式匹配而非抽象空间逻辑。


<details>
  <summary>Details</summary>
Motivation: 传统基于准确率的评估方法无法区分大语言模型在复杂推理领域（如象棋）中的真实几何推理能力与对标准棋盘状态的表面记忆。需要新的评估框架来测试模型在几何变换下的稳定性。

Method: 提出几何稳定性框架，通过棋盘旋转、镜像对称、颜色反转和格式转换等不变变换来严格测试模型一致性。对包括GPT-5.1、Claude Sonnet 4.5和Kimi K2 Turbo在内的六个最先进LLM进行了约3000个棋局的分析。

Result: 发现显著的准确性-稳定性悖论：GPT-5.1等模型在标准位置上接近最优准确率，但在几何扰动下性能灾难性下降（旋转任务错误率增加600%以上）。Claude Sonnet 4.5和Kimi K2 Turbo表现出双重鲁棒性。Gemini 2.5 Flash在非法状态拒绝方面领先（96.0%）。

Conclusion: 几何稳定性为AI评估提供了正交且必要的度量标准，能够有效区分大语言模型的推理能力与数据污染和过拟合问题，是传统准确率评估的重要补充。
...
TL;DR: 提出CogER框架，通过动态选择推理策略来平衡LLM推理效率和准确性，受人类分层推理启发，使用强化学习训练智能体进行策略选择，支持外部工具调用。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理策略主要依赖模型自身的快慢模式（如o1思维），难以在不同难度查询间平衡推理效率和准确性，需要更灵活的自适应方法。

Method: CogER框架首先评估查询复杂度并分配到预定义级别，每个级别对应定制处理策略。将策略选择建模为马尔可夫决策过程，使用强化学习训练CogER-Agent，奖励函数平衡解质量和计算成本。对于需要外部工具的查询，引入认知工具辅助推理，使LLM能在思维链中自主调用外部工具。

Result: CogER在实验中优于最先进的测试时缩放方法，在域内任务上实现至少13%的相对平均精确匹配提升，在域外任务上实现8%的相对增益。
...
<details>
  <summary>Details</summary>
Motivation: 离散优化中，松弛决策图通过节点合并提供对偶界，但其质量严重依赖变量排序和合并决策。动态变量排序虽能收紧界，但在整个变量集上评估会产生计算开销，需要平衡这种权衡。

Method: 引入基于聚类的变量排序框架：先将变量划分为聚类，然后利用这种结构分解指导排序过程。研究两种策略：1) Cluster-to-Cluster：按问题特定聚合标准（如MWISP中的累计顶点权重）顺序处理聚类；2) Pick-and-Sort：从每个聚类迭代选择和排序代表性变量，平衡局部多样性和启发式指导。基于MWISP的DD大小增长理论结果，提出两种设置聚类数量的策略。

Result: 将策略嵌入基于DD的分支定界算法，在MWISP基准实例上评估。相比标准动态变量排序基线，所提方法一致降低了计算成本。

Conclusion: 基于聚类的变量排序框架有效解决了动态排序的计算开销问题，通过缩小搜索空间在MWISP问题上取得了显著的计算效率提升。
...
TL;DR: 该研究使用2025年韩国高考地球科学I部分，分析GPT-4o、Gemini 2.5 Flash和Gemini 2.5 Pro等多模态大语言模型的科学推理能力和认知局限，发现模型存在感知-认知鸿沟、计算-概念化差异和过程幻觉等问题，为设计"AI抗性题目"提供依据。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育评估中的广泛应用，学生使用AI完成作业的现象日益普遍，引发了学术诚信和评估有效性的担忧。研究旨在深入分析先进大语言模型的多模态科学推理能力及其认知局限，为应对未经授权的AI使用提供解决方案。

Method: 研究采用2025年韩国高考地球科学I部分作为测试材料，对GPT-4o、Gemini 2.5 Flash和Gemini 2.5 Pro三种模型进行评估。设计了三种实验条件：整页输入、单独题目输入和优化的多模态输入，以评估模型在不同数据结构下的表现。通过定量和定性分析相结合的方法，识别模型的性能缺陷和认知局限。

Result: 定量结果显示，非结构化输入导致显著的性能下降，主要由于分割和OCR失败。即使在优化条件下，模型仍表现出根本性的推理缺陷。定性分析揭示了主导的"感知错误"，突显了"感知-认知鸿沟"——模型能识别视觉数据但无法解释示意图中的符号意义。此外，模型表现出"计算-概念化差异"（能执行计算但无法应用基础科学概念）和"过程幻觉"（跳过视觉验证而依赖无根据的背景知识）。

Conclusion: 研究通过识别AI模型的特定认知弱点，为设计"AI抗性题目"提供了可操作的线索。通过利用AI的感知-认知鸿沟等弱点，教育工作者可以区分真正的学生能力与AI生成的回答，从而确保评估的公平性。这为解决课程作业中未经授权使用AI的挑战提供了实用方法。
...
TL;DR: 该论文通过定性空间关系改进大语言模型为行人提供路线指引的能力


<details>
  <summary>Details</summary>
Motivation: 大语言模型在提供行人路线指引方面存在局限性，特别是在使用定性空间关系进行准确导航方面需要改进

Method: 利用定性空间关系来增强大语言模型的路线指引能力

Result: 改进了大语言模型为行人提供路线指引的准确性和实用性

Conclusion: 定性空间关系是提升大语言模型行人导航能力的重要方法
...
TL;DR: 提出一个通过自对弈AI游戏扩展人类专家游戏数据库的外学习框架，提高多玩家纸牌游戏早期决策的预测准确性


<details>
  <summary>Details</summary>
Motivation: 在多玩家纸牌游戏（如Skat或Bridge）中，早期阶段（如叫牌、游戏选择和初始选牌）对游戏成功比中后期游戏更为关键。当前计算限制下，这些早期决策依赖于从大量人类专家游戏中获得的统计信息，但数据有限。

Method: 提出通用的自举外学习框架，通过自对弈AI游戏生成数百万游戏扩展人类游戏数据库，合并统计信息。使用完美特征哈希函数处理压缩表，创建自改进的纸牌游戏引擎，在自学习过程中持续改进新推断的知识。

Result: 在Skat游戏中的案例研究表明，该自动化方法可以支持游戏中的各种决策，提高了预测准确性。

Conclusion: 通过结合人类专家游戏和自对弈AI游戏，开发了一个能够持续改进的纸牌游戏引擎框架，有效支持多玩家纸牌游戏的早期关键决策。
...
TL;DR: 提出了一种结合ASP和MILP的集成框架，用于处理UAM垂直起降场动态调度需求和模糊的人类重调度请求


<details>
  <summary>Details</summary>
Motivation: 城市空中交通(UAM)垂直起降场资源受限，需要高效的调度系统，同时需要处理动态操作需求和人类模糊的重调度请求

Method: 使用混合整数线性规划(MILP)建模资源受限项目调度问题，结合三值逻辑解释模糊用户意图和决策树，提出集成答案集编程(ASP)和MILP的新系统

Result: 开发了一个集成框架，能够优化调度并透明地支持人类输入，为可解释、自适应的UAM调度提供了稳健结构

Conclusion: 提出的ASP-MILP集成系统为UAM垂直起降场调度提供了处理动态需求和模糊人类请求的鲁棒解决方案
...
TL;DR: Nemotron-Math是一个大规模数学推理数据集，包含750万条解题轨迹，涵盖高、中、低三种推理模式，支持Python工具集成推理，结合了AoPS竞赛题和StackExchange-Math社区问题，通过顺序分桶策略加速长上下文训练，在多个数学基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理数据集在推理风格多样性、长形式轨迹和工具集成方面存在局限。需要创建大规模、高质量的数据集来支持更强大的数学推理模型训练。

Method: 利用GPT-OSS-120B的多模式生成能力，创建包含7.5M解题轨迹的Nemotron-Math数据集，整合85K AoPS竞赛问题和262K StackExchange-Math社区问题，开发顺序分桶策略加速128K上下文长度的微调训练。

Result: Nemotron-Math在匹配的AoPS问题上持续优于原始OpenMathReasoning，StackExchange-Math的加入显著提高了鲁棒性和泛化能力，在HLE-Math上表现尤其突出，同时在数学竞赛基准上保持准确率。使用Python TIR时在AIME 2024和2025上达到100% maj@16准确率。
...
TL;DR: 该研究提出了一个基于场景的科学发现评估框架，用于评估大语言模型在真实科学研究项目中的表现，发现当前模型在科学发现能力上存在系统性弱点，距离通用科学"超级智能"还很遥远。


<details>
  <summary>Details</summary>
Motivation: 现有科学基准测试主要评估去语境化的知识，忽视了驱动科学发现的迭代推理、假设生成和观察解释等关键能力。需要建立一个更贴近真实科学发现过程的评估框架。

Method: 引入场景驱动的基准测试，涵盖生物、化学、材料和物理领域。由领域专家定义真实研究项目，分解为模块化研究场景，从中抽样验证问题。采用两阶段评估：问题级准确性和项目级性能（提出假设、设计实验、解释结果）。

Result: 应用该框架评估最先进的大语言模型发现：1）相对于通用科学基准存在持续性能差距；2）模型规模和推理能力的扩展收益递减；3）不同提供商顶级模型存在系统性弱点；4）研究场景间性能差异大导致最佳模型选择不稳定；5）尽管场景得分低，模型在某些科学发现项目中仍显示出潜力。

Conclusion: 当前所有大语言模型距离通用科学"超级智能"还很遥远，但已在多种科学发现项目中显示出潜力。该SDE框架为LLMs的发现相关评估提供了可复现的基准，并为推动其面向科学发现的发展指明了实用路径。
...
TL;DR: 论文提出了一个决策理论框架，用于在不确定性下判断何时应将决策委托给AI系统，强调需要平衡价值对齐、认知准确性和行动范围三个因素，区分了通用委托和特定情境委托的不同要求。


<details>
  <summary>Details</summary>
Motivation: 当前价值对齐文献主要关注如何塑造AI的价值观，但较少研究在不确定性下如何判断不完全对齐的AI是否足够好以证明委托决策的合理性。需要建立原则性方法来决定AI在特定情境下是否足够对齐，将重点从追求完美对齐转向管理委托的风险和回报。

Method: 引入了一个正式的决策理论框架，精确分析价值对齐、认知准确性和行动范围之间的权衡。开发了新颖的评分框架来量化事前决策，区分了通用委托和特定情境委托两种场景。

Result: 分析揭示了两种委托场景的明显区别：通用委托要求近乎完美的价值对齐和完全的认知信任，这在实践中很少满足；而特定情境委托即使在显著不对齐的情况下也可能是最优的，因为AI的更高准确性或更广行动范围可能提供更好的整体决策问题。

Conclusion: 该工作提供了原则性方法来确定AI在给定情境下是否足够对齐，将重点从实现完美对齐转向在不确定性下管理委托的风险和回报，为理性委托决策提供了理论基础。
...
Conclusion: STC框架通过统一推理和自我批判，使LLM具备类似人类的批判性思维能力，是朝着内置批判性思维LLM迈出的重要一步。
...
TL;DR: CAGE框架通过构建属性图来改进大语言模型的上下文归因，该图量化了每个生成内容如何受到提示和先前生成内容的影响，相比现有方法提高了40%的忠实度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然能力强大，但其推理过程不透明，存在安全和信任问题。现有的上下文归因方法直接关联生成标记和提示，忽略了生成过程中的代际影响，导致解释不完整。

Method: 提出CAGE框架，构建一个属性图：一个有向图，量化每个生成内容如何受到提示和所有先前生成内容的影响。该图保持因果性和行随机性两个属性，通过沿图中路径边缘化中间贡献来计算上下文归因。

Result: 在多个模型、数据集、指标和方法上，CAGE显著提高了上下文归因的忠实度，平均提升高达40%。

Conclusion: CAGE框架通过考虑生成过程中的代际影响，提供了更完整和忠实的上下文归因，有助于提高大语言模型的可解释性、安全性和可信度。
...
### [116] [Magnification-Aware Distillation (MAD): A Self-Supervised Framework for Unified Representation Learning in Gigapixel Whole-Slide Images](https://arxiv.org/abs/2512.14796)
...
TL;DR: 该研究提出了一种多尺度自监督学习方法MAD，通过关联不同放大倍率的组织图像，学习分辨率不变的表示，用于全切片图像分析。


<details>
  <summary>Details</summary>
Motivation: 全切片图像包含多个放大倍率的组织信息，但现有自监督方法将这些尺度视为独立视图，导致模型无法学习分辨率变化时保持稳定的表示，而这正是神经病理学工作流程的关键要求。

Method: 研究引入了Magnification-Aware Distillation (MAD)自监督策略，将低倍率上下文与空间对齐的高倍率细节联系起来，使模型能够学习粗组织结构与精细细胞模式之间的关系。基础模型MAD-NP完全通过这种跨尺度对应关系进行训练，无需标注。

Result: 仅使用10x嵌入训练的线性分类器在应用于未见过的40x切片时保持了96.7%的性能，表现出强大的分辨率不变表示学习能力。分割输出在不同放大倍率下保持一致，保留了解剖边界并最小化了噪声。

Conclusion: 这些结果突出了使用统一嵌入空间进行可扩展、放大倍率鲁棒的全切片图像分析的可行性，为神经病理学工作流程提供了实用的解决方案。
...
### [117] [Artificial Intelligence for the Assessment of Peritoneal Carcinosis during Diagnostic Laparoscopy for Advanced Ovarian Cancer](https://arxiv.org/abs/2512.14797)
...
TL;DR: 开发AI模型从诊断性腹腔镜视频中自动预测晚期卵巢癌患者的手术可行性，提供标准化的Fagotti评分评估


<details>
  <summary>Details</summary>
Motivation: Fagotti评分在诊断性腹腔镜中用于评估手术可行性，但主观性强、依赖操作者经验，限制了其可重复性和广泛应用。需要开发客观、自动化的评估工具。

Method: 回顾性收集诊断性腹腔镜视频，手动标注Fagotti评分相关帧中的解剖结构和腹膜癌变。开发深度学习模型自动识别相关帧、分割结构、预测视频级Fagotti评分和手术指征。

Result: 分割模型在7311帧上训练，解剖结构Dice得分为70±3%，腹膜癌变为56±3%。视频级解剖站分类F1得分为74±3%和73±4%，Fagotti评分预测归一化RMSE为1.39±0.18和1.15±0.08，手术指征F1得分为80±8%和80±2%（开发集101例，测试集50例）。

Conclusion: 这是首个从诊断性腹腔镜视频中自动预测细胞减灭术可行性的AI模型，在不同数据集上表现一致可靠，表明AI可通过标准化术中肿瘤负荷评估支持外科医生的临床决策。
...
### [118] [A Gaussian Parameterization for Direct Atomic Structure Identification in Electron Tomography](https://arxiv.org/abs/2512.15034)
*Nalini M. Singh,Tiffany Chien,Arthur R. C. McCray,Colin Ophus,Laura Waller*

Main category: eess.IV

TL;DR: 该论文提出了一种新的原子电子断层扫描方法，直接求解原子位置和属性，而不是传统的体积表示方法


<details>
  <summary>Details</summary>
Motivation: 传统原子电子断层扫描方法先求解中间体积表示，再后处理得到原子结构，这种方法存在局限性。作者希望直接求解原子位置和属性，以获得更精确和鲁棒的结果

Method: 将原子结构参数化为高斯函数的集合，其位置和属性是可学习的。这种表示方式为学习结构提供了强大的物理先验

Result: 模拟实验和实验数据的概念验证结果表明，该方法对实际成像伪影具有更好的鲁棒性，在材料表征和透射电子显微镜分析中具有实际应用潜力

Conclusion: 通过直接求解原子位置和属性的新方法，原子电子断层扫描可以获得更精确、更鲁棒的结果，为材料科学中的原子尺度表征提供了新的工具
...
### [119] [Meta-learners for few-shot weakly-supervised optic disc and cup segmentation on fundus images](https://arxiv.org/abs/2512.15061)
*Pandega Abyan Zumarsyah,Igi Ardiyanto,Hanung Adi Nugroho*

Main category: eess.IV

TL;DR: 该研究开发了用于少样本弱监督分割的元学习器，通过Omni元训练平衡数据使用和多样化shot数量，并开发高效版本降低计算成本，同时提出稀疏化技术生成更可定制和代表性的稀疏标签。最佳模型EO-ProtoSeg在多个数据集上表现出色，仅需一个稀疏标注图像即可达到与需要更多标注图像的方法相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 针对青光眼诊断中视盘和视杯分割任务，在标注有限的眼底图像情况下，开发少样本弱监督分割方法，解决现有方法需要大量标注数据的问题。

Method: 1. 提出Omni元训练方法，平衡数据使用并多样化shot数量；2. 开发高效版本元学习器降低计算成本；3. 开发稀疏化技术生成更可定制和代表性的涂鸦及其他稀疏标签；4. 评估多个数据集，比较不同方法的性能。

Result: Omni和高效版本均优于原始版本，最佳模型EO-ProtoSeg在REFUGE数据集上仅使用一个稀疏标注图像就达到视盘IoU 88.15%和视杯IoU 71.17%，优于需要更多标注图像的少样本和半监督方法。在其他数据集上也表现出色，参数少于200万且无需重新训练。

Conclusion: 该研究提出的元学习方法在少样本弱监督分割任务中表现优异，特别是EO-ProtoSeg模型在视盘和视杯分割任务上达到了与无监督域适应方法相当的性能，同时计算成本更低，为医学图像分割提供了有效的解决方案。
...
TL;DR: 提出音频-视觉跨模态压缩框架AVCC，通过统一扩散过程联合压缩音频和视频，在极低码率下可从一种模态重建另一种模态，显著优于VVC标准和现有GFVC方案。


<details>
  <summary>Details</summary>
Motivation: 现有生成式人脸视频编码方法主要关注视频运动，忽略了音频对码率的显著贡献。尽管音频与唇部运动存在已知相关性，但这种跨模态一致性尚未被系统性地用于压缩。

Method: 提出AVCC框架，从视频提取运动信息，对音频特征进行标记化，通过统一的音频-视频扩散过程对齐两者，实现从共享表示中同步重建两种模态。

Result: AVCC在率失真性能上显著优于Versatile Video Coding标准和最先进的GFVC方案，在极低码率场景下甚至可以从一种模态重建另一种模态。

Conclusion: AVCC框架为更高效的多模态通信系统铺平了道路，通过利用音频-视觉跨模态相关性实现了更好的压缩性能。
...
### [121] [Nine Years of Pediatric Iris Recognition: Evidence for Biometric Permanence](https://arxiv.org/abs/2512.15543)
*Naveenkumar G Venkataswamy,Masudul H Imtiaz,Stephanie Schuckers*

Main category: eess.IV

TL;DR: 该研究对儿童虹膜识别的长期稳定性进行了9年追踪，发现使用儿童校准阈值时误拒率低于0.5%，接近成人水平。图像质量和瞳孔扩张一致性是主要影响因素，而非时间推移。


<details>
  <summary>Details</summary>
Motivation: 尽管印度Aadhaar和加拿大NEXUS等项目已广泛部署儿童虹膜识别，但儿童生物特征的持久性仍缺乏深入研究。需要了解儿童发育过程中虹膜识别的长期稳定性，为政策制定提供依据。

Method: 对276名4-12岁儿童进行长达9年的纵向追踪，采集18,318张近红外虹膜图像。使用商业系统VeriEye和开源系统OpenIris，通过线性混合效应模型分析入学年龄、发育成熟度和时间间隔，同时控制图像质量和生理因素。

Result: 使用儿童校准阈值时，两种匹配器在9年内的误拒率均低于0.5%。VeriEye的表现下降主要反映不同入学队列的发育混杂效应，而非真正的模板老化；OpenIris显示适度的真实时间老化效应。图像质量和瞳孔扩张一致性是主要影响因素，失败案例集中在9.4%存在持续采集困难的受试者中。

Conclusion: 虹膜识别在整个儿童期和青春期保持可行性，适当延长重新注册政策至10-12年是合理的。图像采集条件而非时间推移是主要限制因素，高质量采集的7岁以上儿童注册具有长期稳定性。
...
### [122] [An Open-Source Framework for Quality-Assured Smartphone-Based Visible Light Iris Recognition](https://arxiv.org/abs/2512.15548)
*Naveenkumar G. Venkataswamy,Yu Liu,Soumyabrata Dey,Stephanie Schuckers,Masudul H. Imtiaz*

Main category: eess.IV

TL;DR: 本文介绍了CUVIRIS数据集和LightIrisNet模型，用于智能手机可见光谱虹膜识别，通过标准化采集和轻量级模型实现了高精度识别。


<details>
  <summary>Details</summary>
Motivation: 智能手机可见光谱虹膜识别面临光照变化、色素影响和缺乏标准化采集协议等挑战，需要开发适合移动设备的解决方案。

Method: 创建了CUVIRIS数据集（752张ISO/IEC 29794-6标准虹膜图像），开发了基于MobileNetV3的轻量级多任务分割模型LightIrisNet，并适配了基于Transformer的IrisFormer匹配器。

Result: OSIRIS系统在CUVIRIS上达到TAR=97.9%（FAR=0.01，EER=0.76%），IrisFormer在UBIRIS.v2训练后在CUVIRIS上达到EER=0.057%，性能优于现有CNN基线。

Conclusion: 通过标准化采集和可见光谱优化的轻量级模型，智能手机虹膜识别在受控条件下是可行的，为实际部署提供了基础。
...
### [123] [Radiomics and Clinical Features in Predictive Modelling of Brain Metastases Recurrence](https://arxiv.org/abs/2512.15681)
*Ines Faria,Matheus Silva,Crystian Saraiva,Jose Soares,Victor Alves*

Main category: eess.IV

TL;DR: 该研究提出基于人工智能的多模态影像和临床数据方法，用于预测脑转移瘤放疗后的复发风险，使用放射组学特征和机器学习模型进行分析。


<details>
  <summary>Details</summary>
Motivation: 脑转移瘤影响20-40%的癌症患者，放疗后早期预测复发可及时干预改善预后，但目前缺乏有效的预测工具。

Method: 收集97例患者的多模态影像（CT和MRI）及临床数据，进行预处理后保留53例分析。提取放射组学特征，使用delta放射组学分析治疗前后变化，训练多种机器学习分类器，并分析治疗计划靶区与实际照射剂量的差异。

Result: 尽管样本量和类别不平衡存在局限，但结果显示基于放射组学的集成模型可用于复发预测，并提示放疗剂量差异可能与复发风险相关。

Conclusion: 该研究证明了AI驱动工具在脑转移瘤管理中辅助临床决策的可行性，支持进一步研究开发更精准的预测模型。
