...
TL;DR: 本文提出了一种基于预训练多模态大语言模型的三阶段智能电网工程设计图纸审查框架，通过先进的提示工程技术解决超高分辨率图纸审查中的计算需求高、信息丢失和缺乏整体语义理解等问题。


<details>
  <summary>Details</summary>
Motivation: 当前自动化系统在审查超高分辨率电网工程设计图纸时面临三大挑战：1) 计算需求高；2) 信息丢失；3) 缺乏整体语义理解来识别设计错误。这限制了智能审查系统的实际应用效果。

Method: 提出三阶段框架：第一阶段模仿专家审查过程，利用MLLM进行全局语义理解，从低分辨率概览中智能提出领域特定语义区域；第二阶段在这些区域进行高分辨率细粒度识别，获取详细信息及置信度分数；第三阶段通过综合决策模块整合置信度感知结果，准确诊断设计错误并提供可靠性评估。

Result: 在真实电网图纸上的初步结果表明，该方法显著增强了MLLM把握宏观语义信息的能力，能够更精确地定位设计错误，相比传统的被动MLLM推理，在缺陷发现准确性和审查判断可靠性方面都有明显提升。

Conclusion: 该研究为智能可靠的电网图纸审查提供了一种新颖的提示驱动范式，通过三阶段框架有效解决了超高分辨率图纸审查的关键挑战，展示了MLLM在工程图纸智能审查中的潜力。
...
TL;DR: 该论文提出了一种名为LURE的新方法，通过重构潜在空间和引导采样轨迹来重新唤醒被擦除的概念，解决了现有概念擦除方法中概念可能被重新唤醒的漏洞。


<details>
  <summary>Details</summary>
Motivation: 概念擦除旨在抑制扩散模型中的敏感内容，但最近研究表明被擦除的概念仍可能被重新唤醒，揭示了擦除方法的脆弱性。现有重新唤醒方法主要依赖提示级优化来操纵采样轨迹，忽略了其他生成因素，限制了对底层动态的全面理解。

Method: 将生成过程建模为隐函数以进行多因素理论分析，提出LURE方法：1）语义重新绑定机制通过将去噪预测与目标分布对齐来重构潜在空间；2）梯度场正交化在多概念场景中强制执行特征正交性以防止相互干扰；3）潜在语义识别引导采样通过后验密度验证确保重新唤醒过程的稳定性。

Result: 大量实验表明，LURE能够在不同的擦除任务和方法中同时、高保真地重新唤醒多个被擦除的概念。

Conclusion: 通过将生成过程建模为隐函数并进行多因素分析，提出的LURE方法能够有效重新唤醒被擦除的概念，特别是在多概念场景中通过梯度场正交化解决了特征纠缠问题，为概念擦除方法的脆弱性提供了新的理解和解决方案。
...
TL;DR: CityCube是一个针对城市环境的跨视角空间推理基准测试，包含5,022个多视角QA对，评估33个视觉语言模型，发现与人类性能存在显著差距


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注室内或街道场景，忽略了开放城市空间的独特挑战（丰富的语义、复杂几何和视角变化），需要专门的城市环境跨视角推理基准

Method: 引入CityCube基准，整合四种视角动态模拟相机移动，涵盖车辆、无人机和卫星等多平台视角，包含5,022个精心标注的多视角QA对，分为五个认知维度和三种空间关系表达

Result: 评估33个VLM显示显著性能差距：大规模模型准确率不超过54.1%，比人类低34.2%；而小规模微调VLM可达到60.0%以上准确率，任务相关性分析和认知差异分析揭示了VLM与人类推理的根本差异

Conclusion: CityCube基准揭示了当前VLM在城市环境跨视角推理方面的局限性，强调了专门基准的必要性，并为理解VLM与人类空间认知差异提供了重要工具
...
Method: 提出SegAE（Segmentation Assessment Engine），一个轻量级视觉语言模型，在超过400万图像-标签对上进行训练，预测标签质量分数

Result: SegAE与真实Dice相似度的相关系数达到0.902，评估3D掩码仅需0.06秒；分析显示公共数据集普遍存在低质量标签；在主动和半监督学习中提升数据效率和训练性能

Conclusion: SegAE为大规模医学分割数据集提供了简单有效的质量控制解决方案，显著降低标注成本和质检时间，相关资源已开源
...
TL;DR: 提出一种将单目相机图像转换为自然语言描述的框架，用于自动驾驶场景理解，通过混合注意力机制提升特征提取，并基于BDD100K构建新数据集进行评估。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确感知和理解交通场景以确保安全导航，但现有方法在从单目图像生成详细自然语言描述方面存在不足，且缺乏专门的评估数据集。

Method: 提出新颖框架将单目前视相机图像转换为自然语言描述，采用混合注意力机制增强空间和语义特征提取，整合特征生成上下文丰富的场景描述，并基于BDD100K构建新数据集。

Result: 使用CIDEr、SPICE等指标和人工评估进行广泛定量评估，证明所提模型在新数据集上表现优异，有效实现了预期目标。

Conclusion: 该框架成功实现了从单目图像到自然语言描述的转换，为自动驾驶场景理解提供了有效解决方案，新数据集和评估指标也为该领域研究提供了重要资源。
...
TL;DR: 提出了一种基于高斯分布的自适应相机-LiDAR多模态3D占用预测模型，通过内存高效的3D高斯模型无缝融合相机语义优势和LiDAR几何优势，解决自动驾驶长尾安全挑战。


<details>
  <summary>Details</summary>
Motivation: 稀疏目标检测范式向密集3D语义占用预测转变对于处理自动驾驶长尾安全挑战是必要的，但当前体素化方法存在计算复杂度高、融合过程脆弱且在动态环境设置下失效的问题。

Method: 包含四个关键组件：1) LiDAR深度特征聚合(LDFA)，采用深度可变形采样处理几何稀疏性；2) 基于熵的特征平滑，使用交叉熵处理领域特定噪声；3) 自适应相机-LiDAR融合，基于模型输出动态重新校准传感器输出；4) Gauss-Mamba头部，使用选择性状态空间模型进行全局上下文解码，具有线性计算复杂度。

Result: 该研究提出了一种新颖的基于高斯分布的自适应相机-LiDAR多模态3D占用预测模型，通过内存高效的3D高斯模型实现了相机语义优势和LiDAR几何优势的无缝融合。

Conclusion: 该方法通过创新的高斯建模和自适应融合机制，解决了当前3D占用预测中计算复杂度高、融合脆弱的问题，为自动驾驶的长尾安全挑战提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有OCR系统通常需要多个独立组件来处理文本识别、检测和定位任务，缺乏统一的接口。GutenOCR旨在通过单一检查点的视觉语言模型提供集成的OCR解决方案，简化文档处理流程。

Method: 通过微调Qwen2.5-VL-3B和Qwen2.5-VL-7B模型，在商业文档、科学文章和合成的定位数据上进行训练。模型支持全页和局部阅读，提供行级和段落级的边界框，以及条件查询功能。

Result: GutenOCR-7B在10.5K个保留的商业和科学文档上将综合定位OCR分数从基线的0.40提升到0.82（翻倍以上）。在Fox和OmniDocBench v1.5基准测试中，显著改进了区域和行级OCR以及文本检测召回率。
...
TL;DR: 提出PAS-Mamba模型，通过解耦频率域的相位和幅度建模，结合图像域特征，实现更好的MRI重建效果


<details>
  <summary>Details</summary>
Motivation: 现有MRI重建方法通常将频率域视为整体处理，忽略了相位和幅度携带不同信息的差异。相位主要控制图像结构，幅度反映像素强度，统一建模会导致特征学习干扰

Method: 提出PAS-Mamba框架：1) 图像域使用LocalMamba保持空间局部性；2) 频率域将幅度和相位解耦为两个专门分支；3) 提出圆形频率域扫描(CFDS)从低频到高频序列化特征；4) 双域互补融合模块(DDCFM)自适应融合幅度相位表示并实现频域与图像域双向交换

Result: 在IXI和fastMRI膝关节数据集上的大量实验表明，PAS-Mamba始终优于最先进的重建方法

Conclusion: 通过解耦频率域的相位和幅度建模，并结合图像域特征，PAS-Mamba框架能够实现更优的MRI重建效果，验证了分别处理相位和幅度信息的重要性
...
TL;DR: SDT-Net是一个双教师单学生框架，通过动态教师切换和分层一致性模块，从稀疏标注中学习更准确的医学图像分割。


<details>
  <summary>Details</summary>
Motivation: 涂鸦标注方法虽然减轻了医学图像分割的标注负担，但其稀疏性导致伪标签传播噪声大，难以学习稳健的解剖边界。

Method: 提出SDT-Net框架，包含动态教师切换模块自适应选择可靠教师，通过选择可靠像素机制生成高置信度伪标签，以及分层一致性模块实现多层次特征对齐。

Result: 在ACDC和MSCMRseg数据集上的实验表明，SDT-Net达到了最先进的性能，产生更准确且解剖学上合理的分割结果。

Conclusion: SDT-Net通过双教师协作和分层监督机制，有效解决了稀疏标注带来的模糊性问题，提升了医学图像分割的准确性和鲁棒性。
...
TL;DR: 论文提出可学习帧选择器(LFS)，通过选择时间多样性和事件相关的帧来改进视频描述生成，避免均匀采样的不足，并引入新基准ICH-CC来评估人类一致性理解。


<details>
  <summary>Details</summary>
Motivation: 当前视频描述模型使用均匀采样处理所有帧，计算成本高且忽略了视频事件分布的不均匀性，需要更智能的帧选择方法来平衡时间多样性和事件相关性。

Method: 提出可学习帧选择器(LFS)，显式建模时间重要性来平衡时间多样性和事件相关性，采用分层策略确保时间覆盖同时避免聚类，利用冻结视频-LLM的标题反馈来学习直接优化下游描述质量的帧选择。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过外部化中间思考（如思维链）提升了推理能力，但视觉语言模型缺乏类似的空间推理机制，导致在生成准确反映几何关系、物体身份和组合意图的图像方面存在局限。

Method: 引入空间草稿本概念——一个3D推理基板，将文本提示解析为主题和背景元素，实例化为可编辑的3D网格，通过智能场景规划进行放置、定向和视角选择，最后将3D布局渲染回图像域并保留身份识别线索。

Result: 在GenAI-Bench上实现了32%的文本对齐改进，相比之前的2D布局方法，支持直观的3D编辑并能可靠地传播到最终图像中，生成空间一致且视觉连贯的输出。

Conclusion: 研究展示了一种新的视觉语言模型范式，不仅能在语言层面进行思考，还能在空间层面进行推理，为精确可控的图像生成提供了有效途径。
...
TL;DR: 提出U-Harmony方法，通过领域门控头部和特征分布归一化/反归一化，使单个分割模型能同时从异构医学数据集中学习，支持通用模态适应。
...
Method: 提出通用协调（U-Harmony）联合训练方法，集成到带有领域门控头部的深度学习架构中。通过顺序归一化和反归一化特征分布来减轻领域特定变化，同时保留原始数据集特定知识。支持通用模态适应，可无缝学习新成像模态和解剖类别。

Result: 在跨机构脑病变数据集上的广泛实验证明了该方法的有效性，为真实临床环境中稳健且适应性强的3D医学图像分割模型建立了新基准。

Conclusion: U-Harmony方法能够有效处理异构医学分割数据，实现单个模型同时学习多个数据集，支持模态适应，为临床实践提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉理解方面表现出色，但往往缺乏层次知识理解能力，导致在分类学任务中即使能正确识别最具体的叶级类别，也会在更粗的层次级别上出现错误分类。现有方法大多忽视了层次推理建模的问题。

Method: 提出VL-Taxon两阶段层次推理框架：第一阶段采用自上而下的过程提升叶级分类准确性；第二阶段利用准确的叶级输出确保整个分类学层次的一致性。每个阶段先通过监督微调注入分类学知识，然后通过强化学习优化模型的推理和泛化能力。
...
Conclusion: VL-Taxon框架通过层次推理建模有效解决了视觉语言模型在分类学任务中的层次知识理解缺陷，证明了层次推理对于提升VLM在分类学任务中性能的重要性，且该方法具有高效的数据利用效率。
...
TL;DR: 本文提出DEUA框架，通过区分认知不确定性和偶然不确定性来提升扩散生成图像的检测性能，使用拉普拉斯近似估计认知不确定性并引入非对称损失函数训练分类器。
...
Method: 提出DEUA框架：1) 通过拉普拉斯近似估计扩散认知不确定性(DEU)，评估数据与扩散生成样本流形的接近程度；2) 引入非对称损失函数训练平衡分类器，获得更大的分类边界以增强泛化能力。

Result: 在大规模基准测试上的广泛实验验证了该方法达到了最先进的性能。

Conclusion: 通过区分认知不确定性和偶然不确定性，并引入非对称学习策略，DEUA框架能有效提升扩散生成图像的检测性能，在泛化能力和准确性方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 抑郁症是严重的全球心理健康问题。现有音频-视觉方法要么忽略情感线索而无法捕捉抑郁信号，要么包含情感信息但混淆了短暂情感表达与稳定抑郁症状，导致检测错误。

Method: 提出READ-Net框架，采用自适应特征重校准(AFR)动态调整情感特征权重，增强抑郁相关信号，保留抑郁相关线索同时过滤情感噪声。
...
TL;DR: Mirai框架通过向自回归视觉生成器的训练中注入未来信息，显著加速收敛并提升生成质量，无需改变架构或增加推理开销。


<details>
  <summary>Details</summary>
Motivation: 传统自回归视觉生成器使用严格的因果监督，每个步骤只通过下一个标记进行优化，这削弱了全局一致性并减缓了收敛速度。研究探索未来信息（foresight）是否能帮助自回归视觉生成。

Method: 提出Mirai框架，通过将未来信息与自回归模型在2D图像网格上的内部表示对齐来改进因果建模。包含两种变体：Mirai-E使用单向表示中多个未来位置的显式未来信息；Mirai-I利用匹配的双向表示中的隐式未来信息。

Result: Mirai显著加速收敛并提高生成质量。例如，可将LlamaGen-B的收敛速度提升10倍，在ImageNet类条件图像生成基准上将FID从5.34降低到4.34。

Conclusion: 研究表明视觉自回归模型需要未来信息，Mirai框架通过注入未来信息有效解决了传统自回归训练中的全局一致性问题，加速了模型收敛并提升了生成质量。
...
TL;DR: 本文提出一种新的视频重渲染方法，利用大型4D重建模型的隐式几何知识来条件化视频生成过程，解决现有方法在视角变化下的漂移、变形和深度误差问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频重渲染方法面临两个主要挑战：几何无条件的模型缺乏空间感知，在视角变化下容易产生漂移和变形；而几何条件模型依赖估计的深度和显式重建，容易受到深度不准确和校准误差的影响。

Method: 使用大型4D重建模型潜在空间中嵌入的隐式几何知识来条件化视频生成过程。这些潜在表示在连续空间中捕获场景结构而无需显式重建，提供了灵活的表示形式，使预训练的扩散先验能更有效地正则化误差。通过联合条件化这些潜在表示和源相机姿态来实现视频重渲染。

Result: 该方法在视频重渲染任务上取得了最先进的结果，证明了隐式几何知识在改善视角变化下视频生成质量方面的有效性。

Conclusion: 通过利用4D重建模型的隐式几何知识作为条件，本文提出的方法能够生成高质量、几何一致的新视角视频，克服了现有方法的局限性，为视频重渲染任务提供了有效的解决方案。
...
Method: 基于核心架构、数据处理策略和监控特定挑战对方法进行分类，涵盖CNN-based检测器、GAN辅助方法和时序融合方法。分析生成模型如何支持任务如重建缺失帧、减少遮挡和光照归一化，并概述预处理流程、特征提取进展、基准数据集和比较评估。

Result: 系统梳理了现代目标检测技术，突出了生成模型在提升检测鲁棒性方面的作用，识别了不同方法在处理监控特定挑战时的优势和局限性，并提供了全面的技术分类和评估框架。

Conclusion: 该综述为监控领域的目标检测提供了全面的技术概览，指出了未来研究方向，包括低延迟、高效和时空学习方法等新兴趋势，为后续研究和应用开发提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有时尚图像检索基准测试存在过时、缺乏实时性、无法反映真实电商场景等问题，需要创建一个能够持续评估模型性能、反映当代趋势的基准测试。

Method: 构建包含实时网站产品图像和AI生成时尚图像的数据集，基于细粒度属性分类法，涵盖单品和套装级别的检索任务，采用时间戳标记样本并定期更新，支持污染感知评估。

Result: LookBench对现有强基线模型构成显著挑战，许多模型Recall@1低于60%；作者专有模型在LookBench上表现最佳，开源模型排名第二，两者在传统Fashion200K评估中都达到SOTA水平。

Conclusion: LookBench提供了一个持久衡量时尚图像检索进展的基准测试，将每半年更新测试样本并逐步增加任务难度，公开提供排行榜、数据集、评估代码和训练模型。
...
TL;DR: 提出CPF-CTE框架，通过上下文补丁融合和类别标记增强来解决弱监督语义分割中的上下文依赖问题，在PASCAL VOC和MS COCO数据集上表现优于现有方法。
...
Method: 提出CPF-CTE框架，包含上下文融合双向LSTM模块捕获补丁间空间依赖关系，以及可学习的类别标记动态编码和精化类别特定语义。

Result: 在PASCAL VOC 2012和MS COCO 2014数据集上的广泛实验表明，CPF-CTE框架持续超越先前的弱监督语义分割方法。

Conclusion: CPF-CTE框架通过有效整合空间和语义线索，生成更丰富准确的图像内容表示，解决了现有方法忽视上下文依赖的问题。
...
Method: 基于机制注意力研究，将KV缓存概念化为分层内存框架，封装多粒度视频信息；在推理时重用紧凑KV缓存，无需用户查询到达时的辅助计算。

Result: 相比均匀采样减少高达68%的视频token，在所有基准测试中达到相当或更优的准确率，在流式数据集上获得高达11.4%的性能提升，首次响应时间比先前SOTA快10倍。

Conclusion: HERMES通过创新的KV缓存重用机制，成功解决了流式视频理解中的性能、实时性和资源效率平衡问题，为实时视频流交互提供了有效的解决方案。
...
TL;DR: DeepMoLM是一个双视角分子语言建模框架，通过融合高分辨率分子图像和几何不变量，在保持3D立体化学信息的同时实现物理基础的分子生成。


<details>
  <summary>Details</summary>
Motivation: 现有分子语言模型大多依赖字符串或图表示，而视觉语言模型往往忽略立体化学细节，难以将连续的3D结构映射到离散标记中。需要一种能同时处理分子图像和几何信息的方法。

Method: 提出DeepMoLM双视角框架：1) 从1024×1024高分辨率分子图像中提取高频证据；2) 将构象邻域编码为离散的扩展3D指纹；3) 通过交叉注意力融合视觉和几何流，实现无需原子坐标的物理基础生成。

Result: 在PubChem标注任务中，相比最强通用基线获得12.3%的相对METEOR提升，与专业方法保持竞争力；所有属性查询均能生成有效数值输出；在专业设置下分子量MAE为13.64 g/mol，复杂度为37.89；在ChEBI-20图像描述生成中超越通用基线并匹配最先进的视觉语言模型。

Conclusion: DeepMoLM通过融合分子图像和几何不变量，成功解决了现有方法在立体化学细节和3D结构表示上的局限性，为药物发现和化学文献挖掘提供了更准确的分子语言建模解决方案。
...
TL;DR: VoidFace是一种针对人脸交换扩散模型的系统性防御方法，通过在关键瓶颈注入扰动，破坏身份信息传递，有效防御隐私泄露攻击。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的快速发展使得人脸交换技术普及，但同时也引发了隐私和身份安全担忧。现有的主动防御方法通常是从图像编辑攻击中改编而来，在人脸交换场景中效果不佳。作者认为这种失败源于忽视了人脸交换系统的结构韧性及其独特的静态条件引导机制。

Method: VoidFace将人脸交换视为耦合的身份传递路径，通过在关键瓶颈注入扰动引发级联破坏。具体包括：1）定位破坏和身份擦除以降低物理回归和语义嵌入质量；2）在生成域中通过解耦注意力机制切断身份注入，并破坏中间扩散特征以防止源身份重建；3）在潜在流形中进行对抗搜索，采用感知自适应策略平衡攻击效果与图像质量。

Result: 大量实验表明，VoidFace在各种基于扩散的人脸交换模型中都优于现有防御方法，同时生成的对抗性人脸具有更优的视觉质量。

Conclusion: VoidFace提供了一种有效的系统性防御方案，能够保护人脸图像免受扩散模型的人脸交换攻击，在防御效果和视觉质量之间取得了良好平衡。
...
TL;DR: 论文提出了一种端边协同的生成-增强框架，通过自适应选择去噪步长和超分辨率尺度，在前景区域使用扩散模型恢复细节，在背景区域使用轻量级学习模型高效放大，从而在保证图像质量的同时显著降低服务延迟。


<details>
  <summary>Details</summary>
Motivation: 高分辨率文本到图像生成在提升用户体验质量方面越来越重要，但资源受限的边缘计算虽然能支持快速低分辨率生成，高分辨率输出仍面临在保证图像保真度与降低延迟之间的权衡挑战。

Method: 提出端边协同的生成-增强框架：1）边缘侧基于自适应选择的去噪步长和超分辨率尺度生成低分辨率图像；2）将图像分割成块，采用区域感知混合超分辨率策略：前景块使用扩散模型恢复细节，背景块使用轻量级学习模型高效放大；3）最终拼接增强后的块形成高分辨率图像。

Result: 实验表明，与基线方法相比，该系统在保持竞争力的图像质量的同时，将服务延迟降低了33%。

Conclusion: 通过端边协同和区域感知的混合超分辨率策略，成功解决了高分辨率文本到图像生成中保真度与延迟的权衡问题，实现了在保证图像质量的同时显著降低服务延迟的目标。
...
TL;DR: SimD3是一个大规模高保真合成数据集，用于复杂空中环境中的无人机检测，通过建模异质载荷无人机、加入鸟类干扰物、使用虚幻引擎5环境，显著提升了检测模型的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 无人机检测面临三大挑战：标注真实数据有限、外观变化大、存在鸟类等视觉相似干扰物。现有合成数据集无法充分解决这些问题，需要更高质量的合成数据来提升检测模型的鲁棒性。

Method: 1. 创建SimD3合成数据集：建模带异质载荷的无人机、加入多种鸟类作为干扰物、使用虚幻引擎5生成多样化环境（天气、光照、飞行轨迹）、采用360度六相机系统采集数据。2. 在YOLOv5框架下进行实验，包括提出注意力增强变体Yolov5m+C3b（用C3b模块替换标准瓶颈C3块）。3. 在合成数据、合成+真实数据、多个未见过的真实基准上进行评估。

Result: 实验结果表明：1. SimD3为小目标无人机检测提供了有效的监督信号。2. Yolov5m+C3b在域内和跨数据集评估中始终优于基线模型。3. 合成数据与真实数据结合使用能进一步提升性能。

Conclusion: SimD3合成数据集在训练和评估鲁棒无人机检测模型方面具有重要价值，特别是在多样化和具有挑战性的条件下。注意力增强的检测架构能进一步提升性能，为无人机检测提供了有效的解决方案。
...
TL;DR: 提出了一种具有强推理能力的多模态病理学大语言模型，通过语义奖励策略和高质量病理VQA数据集提升推理性能


<details>
  <summary>Details</summary>
Motivation: 现有多模态病理学方法由于缺乏支持显式推理的高质量数据集和简单的推理过程，导致可解释性有限

Method: 设计了集成组相对策略优化的语义奖励策略，并构建了专门支持复杂推理任务的高质量病理VQA数据集

Result: 在构建的数据集上，该方法优于最先进方法，即使仅使用20%数据训练；在下游零样本图像分类任务中与CLIP性能相当
...
TL;DR: 该研究分析了开源医学视觉语言模型的特征表示，发现医学VLM能提取有效的医学分类特征，但非医学VLM通过文本编码器改进（如LLM2CLIP）能产生更精细的特征表示，表明增强文本编码器比密集医学图像训练更重要。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型被期望能捕捉诊断相关特征，但其学习到的特征表示尚未充分探索，标准的分类准确率评估不能完全揭示它们是否真正获得了具有判别性的病灶特异性特征。理解这些特征表示对于揭示医学图像结构和改进医学图像分析的下游任务至关重要。

Method: 分析多个代表性医学VLM在不同模态的病灶分类数据集上提取的特征分布，并将这些分布与非医学VLM进行比较，以评估领域特定的医学训练效果。

Result: 医学VLM能够提取对医学分类任务有效的判别性特征。然而，通过上下文丰富化改进的非医学VLM（如LLM2CLIP）能产生更精细的特征表示。非医学模型特别容易受到图像上叠加文本字符串引入的偏见影响。
...
TL;DR: FunCineForge是一个端到端的电影配音系统，包含大规模配音数据集生产流程和基于MLLM的配音模型，针对复杂电影场景，在音频质量、唇音同步、音色迁移等方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有配音方法面临两个主要限制：1）高质量多模态配音数据集规模有限、错误率高、标注稀疏、依赖人工标注且仅限于独白场景；2）现有模型仅依赖唇部区域学习音视频对齐，在复杂实景电影场景中适用性有限，唇音同步、语音质量和情感表达性能不佳。

Method: 提出FunCineForge系统，包含：1）端到端的大规模配音数据集生产流程；2）基于MLLM的配音模型，专门为多样化电影场景设计。使用该流程构建了首个中文电视配音数据集，具有丰富标注。

Result: 在独白、旁白、对话和多说话者场景的实验中，FunCineForge在音频质量、唇音同步、音色迁移和指令跟随方面持续优于最先进方法。构建的数据集质量得到验证。

Conclusion: FunCineForge通过创新的数据集生产流程和MLLM模型，有效解决了现有配音方法的局限性，在复杂电影场景中实现了高质量的配音效果，为电影配音任务提供了新的解决方案。
...
TL;DR: RAM提出了一种重建锚定扩散模型，通过运动潜在空间作为中间监督来缩小文本-运动表示差距，并利用重构误差指导机制在测试阶段减少误差传播，显著提升了文本驱动运动生成性能。
...
Method: 1) 引入运动潜在空间作为文本到运动生成的中间监督，通过共同训练运动重建分支，使用自正则化和运动中心潜在对齐两个关键目标函数；2) 提出重构误差指导(REG)机制，在测试阶段利用扩散模型固有的自校正能力来减轻误差传播。

Result: 大量实验表明，RAM实现了显著改进和最先进的性能表现。

Conclusion: RAM通过运动潜在空间监督和重构误差指导机制，有效解决了文本驱动运动生成中的表示差距和误差传播问题，取得了优异的生成效果。
...
TL;DR: 本研究探索使用Stable Diffusion与LoRA生成的合成图像来增强中国瓷器分类的有限真实数据集，通过多任务CNN分类实验验证了合成数据在不同任务中的效果差异。


<details>
  <summary>Details</summary>
Motivation: 考古文物分类中训练数据的稀缺性是深度学习应用的主要挑战，特别是对于稀有的中国瓷器类型。需要探索合成数据增强方法来弥补真实数据不足的问题。

Method: 使用Stable Diffusion结合Low-Rank Adaptation (LoRA)生成合成图像，采用MobileNetV3进行迁移学习，通过控制实验比较纯真实数据与混合真实-合成数据集（95:5和90:10比例）在四个分类任务上的表现：朝代、釉色、窑口和类型识别。

Result: 结果显示任务特定的效益：类型分类改善最显著（90:10比例下F1-macro提高5.5%），朝代和窑口任务有适度提升（3-4%），表明合成增强效果取决于生成特征与任务相关视觉特征的匹配程度。

Conclusion: 研究为考古研究中部署生成式AI提供了实用指南，展示了在平衡考古真实性与数据多样性时合成数据的潜力和局限性，特别适合视觉特征与任务高度相关的分类任务。
...
TL;DR: UniRoute是一个统一的模态自适应变化检测框架，通过条件路由解决特征提取和融合问题，在多种模态设置下实现强性能表现。
...
Method: 提出UniRoute统一框架，将特征提取和融合重新表述为条件路由问题。包括：1) 自适应感受野路由MoE模块(AR2-MoE)分离局部空间细节和全局语义上下文；2) 模态感知差异路由MoE模块(MDR-MoE)自适应选择每个像素最合适的融合原语；3) 一致性感知自蒸馏策略(CASD)在数据稀缺的异质设置下通过强制多级一致性来稳定统一训练。

Result: 在五个公共数据集上的广泛实验表明，UniRoute实现了强大的整体性能，在统一部署设置下具有有利的精度-效率权衡。

Conclusion: UniRoute通过条件路由方法有效解决了遥感变化检测中的模态自适应问题，为统一框架下的多模态变化检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 对称性是形状分析的关键概念，现有方法虽然能从语义感知描述符中提取对称信息特征，但这些特征通常是一维的，忽略了其他有价值的语义信息，且提取的特征存在噪声并产生小范围误分类区域。

Method: 提出特征解耦方法，同时生成对称性感知和对称性无关的特征；进一步提出特征细化技术来提高预测对称感知特征的鲁棒性。

Result: 通过内在对称性检测、左右分类和形状匹配等广泛实验，在定性和定量上都证明了所提框架相比各种最先进方法的有效性。

Conclusion: 提出的特征解耦和细化技术能够构建更鲁棒的对称感知形状描述符，在多个形状分析任务中表现出优越性能。
...
TL;DR: POTR是一种用于3D高斯泼溅的后训练压缩编解码器，通过创新的剪枝和光照系数重计算方法，显著减少存储需求并加速推理，同时保持高质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在训练和推理速度上优于NeRF，但存储需求显著更高，需要有效的压缩方法来平衡存储效率和性能。

Method: 1. 提出新颖的剪枝方法：使用改进的3DGS光栅化器同时计算每个泼溅点的移除效果，实现2-4倍更少的泼溅点；2. 提出光照系数重计算方法：显著降低熵值而不需要训练；3. 扩展简单微调方案以进一步提升性能。

Result: POTR在无微调情况下，在率失真性能和推理速度上均优于所有其他后训练压缩技术，推理速度比其他压缩模型快1.5-2倍，AC光照系数稀疏度从70%提升到97%。

Conclusion: POTR是一种高效的后训练3DGS压缩编解码器，通过创新的剪枝和系数重计算技术，在显著减少存储需求的同时保持高质量并加速推理，为3D场景重建提供了实用的解决方案。
...
TL;DR: GAT-NeRF：一种结合Transformer和NeRF的混合框架，用于从单目视频重建高保真4D动态面部头像，显著提升了高频细节（如动态皱纹）的建模能力。
...
Method: 提出GAT-NeRF框架，将Transformer机制集成到NeRF流程中。结合坐标对齐MLP和轻量级Transformer模块（GAT），融合3D空间坐标、3DMM表情参数和可学习潜在码等多模态输入特征，有效学习细粒度几何特征表示。

Result: 综合实验明确证明GAT-NeRF在视觉保真度和高频细节恢复方面达到最先进性能，为多媒体应用创建逼真动态数字人开辟了新途径。

Conclusion: GAT-NeRF通过集成Transformer增强NeRF，成功解决了从单目视频重建高保真4D动态面部头像的挑战，在捕捉高频面部细节方面表现优异，推动了逼真数字人技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种能够统一3D几何、语义和语言的系统，支持可解释的空间关系推理，实现语言引导导航和物体检索，而无需专用传感器。
...
Conclusion: 通过考虑上下文语义关系并利用像素级置信度信息，EroSeg-AT框架能够生成更有效的对抗样本，从而提升分割模型在对抗训练中的鲁棒性。
...
TL;DR: TempViz是首个全面评估文本到图像生成模型中时间知识的数据集，包含7.9k提示和600+参考图像，研究发现当前T2I模型的时间知识能力普遍较弱，且缺乏可靠的自动评估方法。
...
Method: 创建TempViz数据集（7.9k提示和600+参考图像），评估五个T2I模型在五个时间知识类别上的能力，进行人工评估并与自动评估方法对比。

Result: 人工评估显示时间能力普遍较弱，没有模型在各类别中超过75%准确率。现有自动评估方法都无法可靠评估时间线索，与人工判断相关性差。

Conclusion: 当前T2I模型的时间知识能力有限，缺乏可靠的自动评估方法，凸显了未来研究T2I时间知识的迫切需求。
...
TL;DR: LDF-VFI提出了一种视频帧插值的整体方法，使用自回归扩散变换器建模整个视频序列以确保长期时间一致性，通过跳跃连接采样策略减少误差累积，支持任意分辨率处理。


<details>
  <summary>Details</summary>
Motivation: 现有视频帧插值方法通常采用帧中心方法，将视频作为独立短片段处理，这导致时间不一致性和运动伪影。为了解决这些问题，需要一种更整体的视频中心范式。

Method: 提出LDF-VFI框架：1) 基于自回归扩散变换器建模整个视频序列；2) 引入跳跃连接采样策略减少自回归生成的误差累积；3) 结合稀疏局部注意力和分块VAE编码，支持长序列处理和任意分辨率；4) 增强的条件VAE解码器利用输入视频的多尺度特征。

Result: 在具有挑战性的长序列基准测试中实现了最先进的性能，展示了卓越的单帧质量和时间一致性，特别是在大运动场景中。

Conclusion: LDF-VFI通过视频中心范式解决了传统帧插值方法的时间不一致问题，实现了长期时间一致性和高质量插值，同时支持任意分辨率处理。
...
TL;DR: 提出Scale-TBPS方法，通过噪声感知的统一数据集构建和可扩展的身份学习框架，实现跨多个数据集的统一文本行人搜索模型，超越数据集特定的优化方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本行人搜索方法依赖数据集特定的微调，导致需要为不同数据集训练多个独立模型。虽然合成数据可以增加训练规模，但无法消除数据集特定的适应性问题。这促使研究能否训练一个跨多个数据集的统一模型。

Method: 提出Scale-TBPS方法，包含两个核心贡献：1) 噪声感知的统一数据集构建策略，将多样化的TBPS数据集进行有凝聚力的合并；2) 可扩展的判别性身份学习框架，在大量独特身份下仍保持有效性。

Result: 在CUHK-PEDES、ICFG-PEDES、RSTPReid、IIITD-20K和UFine6926等多个数据集上的广泛实验表明，单一的Scale-TBPS模型优于数据集特定的优化模型和简单的联合训练方法。

Conclusion: Scale-TBPS方法成功解决了跨数据集统一训练文本行人搜索模型的挑战，通过创新的数据集构建和学习框架，实现了超越传统方法的性能，为构建通用TBPS模型提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频评估基准主要关注非互动视频（如电影、录像），缺乏针对互动直播视频的评估标准。互动直播视频具有实时评论、多模态交互等独特挑战，需要专门的基准来推动相关模型发展

Method: 1) 设计标准化半自动标注流程，结合人机协同和多智能体系统进行视频描述；2) 采用种子问题驱动方法构建高质量标注；3) 设计两阶段指令微调策略；4) 提出视频到评论检索模块增强实时评论利用能力；5) 基于上述方法开发LiVi-LLM-7B模型

Result: LiVi-LLM-7B在LiViBench上超越参数达720亿的开源模型，缩小了与领先专有模型的差距，同时在VideoMME、LongVideoBench、MLVU、VideoEval-Pro等通用视频基准上表现提升

Conclusion: LiViBench填补了互动直播视频评估的空白，提出的标注方法和模型架构有效提升了MLLM对互动视频的理解能力，为未来互动视频AI研究提供了重要基准和工具
...
Conclusion: 通过构建大规模双耳音频数据集和视觉引导的空间音频生成框架，成功解决了视频到音频生成中空间感知不足的问题，为未来研究提供了重要资源和方法。
...
TL;DR: 论文提出了一种新的联邦学习深度泄漏攻击方法，通过集成生成式流匹配先验来提升重构保真度，无需私有数据知识，在各种设置下优于现有攻击方法。
...
Method: 提出了一种新的深度泄漏攻击方法，将生成式流匹配先验集成到重构过程中，通过引导优化朝向真实图像分布（由流匹配基础模型表示），从而提升重构保真度。

Result: 在多个数据集和目标模型上的实验表明，该方法在像素级、感知和基于特征的相似度指标上一致优于最先进的攻击方法，在不同训练轮次、更大客户端批次大小以及常见防御措施下保持有效。

Conclusion: 该方法展示了生成先验在增强深度泄漏攻击方面的有效性，呼吁开发新的防御策略，明确考虑配备强大生成先验的对手。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据脱敏技术（如匿名化）在隐私保护机器学习中难以同时保证隐私保护和数据效用，现有方法需要在隐私和效用之间反复权衡
...
Conclusion: 提出的差分隐私生成框架有效解决了隐私保护与数据效用之间的权衡问题，在灰度图和RGB图像上都表现出良好的有效性和泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的前景-背景分解方法存在两个主要问题：1）对背景区域采用统一的抑制策略，忽略了不同补丁对预测的贡献差异；2）对前景区域未能充分考虑某些局部补丁可能与其他类别在外观或语义上相似，从而误导训练过程。

Method: 提出包含三个核心组件的即插即用框架：1）前景-背景分解模块，沿用现有方法将图像分离为前景和背景区域；2）自适应背景抑制模块，自适应地加权补丁分类熵；3）可混淆前景修正模块，识别并修正可混淆的前景补丁。

Result: 大量实验结果表明，所提出的即插即用框架显著提升了现有前景-背景分解方法的性能。

Conclusion: 通过解决现有方法在背景抑制和前景处理方面的局限性，提出的框架有效提高了少样本分布外检测的性能，代码已开源。
...
TL;DR: 该论文提出了Progress-Bench基准测试，用于评估视觉语言模型在任务进度推理方面的能力，发现现有模型在此任务上表现不佳，并探索了训练前提示和训练后模型两种改进方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型擅长描述可见内容，但尚不清楚它们是否能够从部分观察中推断任务进度。任务进度估计需要长时程动态推理而非静态视觉内容识别，这是一个尚未充分探索的研究方向。

Method: 1) 引入Progress-Bench基准测试系统评估VLM的进度推理能力；2) 探索人类启发的两阶段进度推理范式，包括训练前提示方法和基于ProgressLM-45K数据集的训练方法；3) 在14个VLM上进行实验分析。

Result: 实验表明大多数VLM尚未准备好进行任务进度估计，对演示模态和视角变化敏感，且处理不可回答情况的能力差。训练前提示方法效果有限且依赖模型，而训练后的ProgressLM-3B即使在小规模模型上也能获得一致改进，尽管训练任务集与评估任务完全不相交。

Conclusion: 该研究揭示了VLM在进度推理方面的局限性，提出了有效的评估基准和改进方法。训练后的ProgressLM-3B展示了跨任务泛化能力，为未来研究提供了重要基础。进一步分析揭示了特征性错误模式，明确了进度推理成功与失败的条件和原因。
...
TL;DR: 本文综述了Micro-CT技术在工业无损检测中3D缺陷表征的准确性与效率平衡问题，回顾了从医学影像到工业应用的演进，分析了CT重建算法和体绘制方法的进展，并展望了数字孪生等未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着Micro-CT技术产生越来越大的数据集，在超精密检测中需要解决3D缺陷表征的准确性与效率之间的权衡问题。工业CT超精密检测需要平衡精度和效率的解决方案。

Method: 选择性综述和分析平衡准确性与效率的CT重建和体绘制方法，包括从解析方法到深度学习技术的CT重建算法演进，以及体绘制算法、加速和数据缩减的改进，探索高精度、逼真和高效的先进光照模型。

Result: 提供了全面的分析框架，帮助研究人员快速掌握微观特征的高效准确3D重建方法，比较了计算机断层扫描原理与微结构技术的进展，分析了算法演进路径。

Conclusion: 展望了CT重建和体绘制的潜在发展方向，旨在指导未来研究快速选择高效精确方法，为通过虚拟-物理交互实现材料内部缺陷实时在线监测的数字孪生模型应用于结构健康监测开发新思路和方法。
...
TL;DR: 该论文研究了可提示分割模型对边界框提示自然变化的鲁棒性，通过用户研究收集真实标注，发现SAM类模型对自然提示噪声高度敏感，并提出了BREPS方法来生成对抗性边界框进行鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 当前可提示分割模型（如SAM）的训练和评估通常使用基于简单启发式生成的合成提示，这无法反映真实世界中的鲁棒性。特别是边界框作为最有效的提示方式之一，其自然变化对模型性能的影响尚未得到充分研究。
...
Result: 用户研究显示，对于同一模型和实例，不同用户的边界框标注会导致分割质量存在显著差异，表明SAM类模型对自然提示噪声高度敏感。BREPS方法能够有效生成对抗性边界框，在10个数据集（涵盖日常场景到医学影像）的基准测试中，揭示了现有模型在鲁棒性方面的局限性。

Conclusion: 可提示分割模型对边界框提示的自然变化缺乏鲁棒性，这在实际应用中是一个重要问题。BREPS提供了一种系统化的方法来评估和改善这种鲁棒性，为未来更稳健的可提示分割模型的发展提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 视觉关系识别（通常建模为从图像中提取图）仍然具有挑战性，主要原因是缺乏规范方法。现有解决方案通常是特定于问题的，无法在不同上下文之间直接迁移，尽管概念问题是相同的。

Method: 开发了GraSP（Graph Recognition via Subgraph Prediction）方法，通过子图预测来识别图像中的图结构。该方法旨在具有广泛适用性和简单性。

Result: 在多个合成基准测试和一个真实世界应用中展示了该方法能够处理多种类型的图及其绘制方式，并且可以在任务之间迁移而无需任务特定修改。

Conclusion: GraSP为更统一的视觉图识别框架铺平了道路，解决了现有方法缺乏通用性和可迁移性的问题。
...
Result: 分析揭示了多个显著趋势：安全性研究增长、多模态推理发展、面向智能体研究兴起，同时神经机器翻译和图基方法等领域逐渐稳定化。

Conclusion: 该研究提供了AI研究演变的证据基础视图，为理解更广泛趋势和识别新兴方向提供了资源，有助于更好地把握研究动态和发展脉络。
...
TL;DR: PMPose是一个结合概率公式和掩码条件的2D姿态估计器，在拥挤场景中表现出色。BMPv2整合了PMPose和改进的SAM掩码细化模块，在COCO和OCHuman数据集上超越了现有方法，首次在OCHuman上超过50 AP。


<details>
  <summary>Details</summary>
Motivation: 大多数2D人体姿态估计基准在标准场景下已接近饱和，但在拥挤场景中仍有提升空间。本文旨在解决拥挤场景下的姿态估计问题，同时不牺牲标准场景的性能。

Method: 提出了PMPose，一个结合概率公式和掩码条件的自上而下2D姿态估计器。在此基础上开发了BMPv2，整合了PMPose和改进的基于SAM的掩码细化模块。还展示了如何通过2D提示3D模型来改进拥挤场景下的3D姿态估计。

Result: BMPv2在COCO数据集上比现有最优方法高出1.5 AP，在OCHuman数据集上高出6 AP，首次在OCHuman上超过50 AP。在OCHuman-Pose数据集上的结果显示，多人姿态估计性能受姿态预测准确性的影响大于检测准确性。2D姿态质量的提升直接有益于3D姿态估计。

Conclusion: PMPose和BMPv2显著提升了拥挤场景下的2D姿态估计性能，同时保持了标准场景的性能。2D姿态估计的进步直接转化为3D姿态估计的改进，特别是在拥挤场景中。多人姿态估计性能主要受姿态预测准确性的影响。
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期准确诊断对临床干预和患者预后至关重要，需要开发可靠的MRI分类工具。

Method: 比较了5种CNN架构、5种Transformer模型和提出的Evan_V2混合模型，在四类AD分类任务上进行评估，包括轻度、中度、非痴呆和极轻度痴呆类别。

Result: CNN模型表现稳定（ResNet50达98.83%准确率），Transformer模型泛化能力强（ViT达95.38%），但存在类别特异性不稳定。混合模型Evan_V2通过特征级融合达到99.99%准确率、0.9989 F1分数和0.9968 ROC AUC，显著减少所有痴呆阶段的误分类。

Conclusion: 混合集成策略在阿尔茨海默病分类中具有显著优势，能产生高度可靠且具有临床意义的诊断工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D扩散模型在生成城市场景时外观细节退化，而仅用2D扩散模型则相机可控性差。需要结合两者优势来生成高质量且可控的城市场景。

Method: 1. 训练3D潜在扩散模型生成3D高斯，可渲染低分辨率图像，支持3D边界框、道路图或文本提示等条件输入；2. 训练2D视频扩散模型，以3D高斯渲染图像为条件增强外观细节；3. 利用粗粒度3D场景指导2D视频扩散，确保准确遵循相机轨迹。

Result: 在Waymo和KITTI-360两个真实世界数据集上的实验证明了方法的有效性，能够生成符合输入条件且保持相机轨迹准确性的高质量城市场景。

Conclusion: ScenDi通过整合3D和2D扩散模型，成功解决了城市场景生成中外观细节与相机可控性的权衡问题，为可控的高质量3D城市场景生成提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 颈椎骨折是严重医疗状况，需要精确高效的检测方法。传统3D分割方法计算复杂度高，本研究旨在开发一种基于2D投影的替代方案，降低计算负担同时保持高性能。

Method: 1) 通过优化的轴向、矢状和冠状2D投影近似3D体积；2) 使用YOLOv8从所有视图识别感兴趣区域并组合以近似3D颈椎区域；3) 采用DenseNet121-Unet基于方差和能量投影进行多标签分割；4) 从2D分割掩码战略近似3D椎骨掩码；5) 使用集成2.5D时空序列模型分析椎骨体积进行骨折检测。

Result: 3D mIoU达到94.45%，Dice分数87.86%。骨折检测在椎骨级别和患者级别的F1分数分别为68.15和82.26，ROC-AUC分数分别为91.62和83.04。可解释性研究和观察者间变异性分析显示与专家放射科医生竞争性结果。

Conclusion: 该方法通过2D投影策略有效降低了计算复杂度，同时保持了高性能的颈椎骨折检测能力，为临床管理提供了自动化分析工具。
...
TL;DR: 研究人员提出了RBench基准测试和RoVid-X数据集，用于评估和训练面向机器人的视频生成模型，以解决现有模型在生成物理真实机器人行为方面的不足。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型为具身智能带来了新可能，但生成高质量、真实反映机器人交互的视频仍然困难，且缺乏标准化基准限制了公平比较和进展。

Method: 1) 引入RBench基准，涵盖5个任务领域和4种不同具身形态，通过可复现的子指标评估任务正确性和视觉保真度；2) 提出四阶段数据管道，创建RoVid-X数据集，包含400万个带注释的视频片段。

Result: 评估25个代表性模型显示在生成物理真实机器人行为方面存在显著缺陷。RBench与人类评估的Spearman相关系数达到0.96。RoVid-X成为最大的开源机器人视频生成数据集。

Conclusion: 评估和数据的协同生态系统为视频模型的严格评估和可扩展训练奠定了坚实基础，加速了具身AI向通用智能的演进。
...
TL;DR: 提出一种基于迭代自校正的文本到图像生成方法，通过视觉语言模型作为批评家进行多步反馈优化，显著提升复杂组合提示的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在处理需要同时处理多个对象、关系和属性的复杂提示时表现不佳，现有的推理时策略（如并行采样验证器或增加去噪步骤）在需要满足多个约束的丰富组合场景中仍然不足。

Method: 提出迭代测试时策略，让文本到图像模型在多个步骤中逐步优化生成结果，通过视觉语言模型作为循环中的批评家提供反馈指导。该方法简单，无需外部工具或先验知识，可灵活应用于各种图像生成器和视觉语言模型。

Result: 在多个基准测试中取得显著改进：ConceptMix（k=7）的全正确率提升16.9%，T2I-CompBench（3D空间类别）提升13.8%，Visual Jenga场景分解提升12.5%。人类评估者58.7%的时间偏好该方法，优于并行基线的41.3%。
...
TL;DR: FlowSSC是首个应用于单目语义场景补全的生成式框架，通过条件生成方法显著提升现有前馈SSC方法的性能，采用捷径流匹配在紧凑三平面潜在空间中实现实时推理。
...
Method: 将SSC任务视为条件生成问题，提出FlowSSC框架，引入捷径流匹配在紧凑三平面潜在空间中操作，相比需要数百步的标准扩散模型，采用捷径机制实现单步高保真生成。

Result: 在SemanticKITTI数据集上的广泛实验表明，FlowSSC实现了最先进的性能，显著优于现有基线方法。

Conclusion: FlowSSC是首个直接应用于单目语义场景补全的生成式框架，能够与现有前馈SSC方法无缝集成，通过捷径流匹配实现实时高质量推理，适用于自动驾驶系统等实际应用。
...
TL;DR: DrivIng是一个大规模多模态自动驾驶数据集，包含18公里路线的完整数字孪生，提供六摄像头、LiDAR和高精度定位数据，支持系统化测试和仿真到真实场景评估。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集缺乏高保真数字孪生，限制了系统化测试、边缘案例模拟、传感器修改和仿真到真实评估的能力。需要大规模高质量数据集来支持鲁棒的感知算法开发。

Method: 构建了约18公里路线（涵盖城市、郊区和高速公路）的完整地理参考数字孪生，提供六台RGB摄像头、一台LiDAR和高精度ADMA定位系统的连续记录，数据采集覆盖白天、黄昏和夜晚，所有序列以10Hz频率标注3D边界框和12个类别的轨迹ID。

Result: 数据集包含约120万个标注实例，实现了真实交通到仿真的1对1转移，保持智能体交互的同时支持真实灵活的场景测试。提供了数据集、数字孪生、高清地图和代码库的公开发布。

Conclusion: DrivIng填补了现有自动驾驶数据集的空白，通过完整的数字孪生支持系统化测试和仿真到真实评估，为可重复研究和鲁棒验证提供了重要资源。
...
TL;DR: RayRoPE是一种用于多视角Transformer的位置编码方法，通过基于射线的几何感知编码实现SE(3)不变性，并在新颖视角合成和立体深度估计任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多视角注意力机制的位置编码方案（绝对或相对编码）无法同时满足以下需求：1）唯一编码图像块；2）实现SE(3)不变性注意力；3）支持多频率相似性；4）适应底层场景几何。需要一种新的位置编码机制来解决这些限制。

Method: RayRoPE基于与图像块相关的射线表示位置，但利用沿射线的预测点而非方向进行几何感知编码。为实现SE(3)不变性，使用查询帧投影坐标计算多频率相似性。此外，考虑到沿射线的"预测"3D点可能不精确，RayRoPE提供了一种机制来解析计算不确定性下的期望位置编码。

Result: 在CO3D数据集的新颖视角合成任务中，RayRoPE相比其他位置编码方案实现了15%的相对改进（LPIPS指标）。同时，RayRoPE能够无缝整合RGB-D输入，相比无法位置编码此类信息的替代方案获得更大增益。

Conclusion: RayRoPE是一种有效的多视角Transformer位置编码方法，能够满足几何感知、SE(3)不变性和适应性要求，在多个视觉任务中显著优于现有编码方案，并能有效利用深度信息提升性能。
...
Conclusion: StableWorld是一种模型无关的简单有效方法，可应用于不同交互式视频生成框架，解决稳定性与时间一致性问题
...
<details>
  <summary>Details</summary>
Motivation: 室内场景照明通常包含多个复杂光源，现有方法难以实现对这些光源的独立编辑和控制，限制了场景重照明的灵活性和真实感。

Method: 使用生成式图像灯光分解模型将室内照明分解为独立光源，引入多视角灯光协调确保分解一致性，并集成到可重照明的3D高斯泼溅表示中实现实时交互控制。

Result: 在合成和真实数据集上验证了方法，实现了高度逼真的灯光分解和重照明效果，在定量和定性评估中优于现有技术。

Conclusion: 该方法成功实现了从单次多视角捕捉中对室内场景灯光的交互式编辑，为场景重照明提供了灵活且逼真的解决方案。
...
TL;DR: EgoWM将预训练视频扩散模型转化为动作条件世界模型，实现可控的未来预测，无需从头训练，通过轻量级条件层注入运动指令，适用于多种机器人和人形机器人。
...
Method: 提出EgoWM方法，将预训练视频扩散模型转化为动作条件世界模型，通过轻量级条件层注入运动指令，保留模型原有的世界先验知识，实现动作跟随和真实感保持。

Result: EgoWM在导航和操作任务中产生连贯的推演，相比先前最先进的导航世界模型，结构一致性分数提升高达80%，推理延迟降低6倍，并在未见环境中（包括绘画内部导航）表现出鲁棒泛化能力。

Conclusion: EgoWM通过简单、架构无关的方法成功将预训练视频模型转化为动作条件世界模型，实现了准确、可控的未来预测，具有良好的泛化能力和计算效率。
...
TL;DR: 该研究探索了将量化技术应用于多模态模型管道，分析了不同量化方法对视觉模型、语言模型及其连接器的影响，发现ViT和LLM在性能中具有可比重要性，且LLM的低比特量化能在减少权重比特的同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要大量GPU内存和计算资源，量化技术可以减少内存占用和延迟。现有研究主要关注单模态模型的量化，但多模态模型管道（包含视觉模型、语言模型及其连接器）的量化效果尚未得到充分研究。

Method: 研究应用多种量化方法（包括最先进的GPTQ和AWQ）到多模态管道，分析比特宽度、量化方法以及量化应用位置（视觉模型、语言模型或连接器）对性能的影响。评估任务包括图像描述、检索和问答。

Result: 结果显示：1）ViT和LLM在模型性能中具有可比重要性，尽管参数规模差异显著；2）LLM的低比特量化能在减少权重比特（bpw）的同时实现高精度；3）量化方法的选择和应用位置对多模态任务性能有显著影响。

Conclusion: 该研究为多模态大模型的高效部署提供了实用见解，强调了探索组件敏感性对理解多模态模型的重要性。研究代码已开源，有助于进一步研究和应用。
...
TL;DR: APPLE是一种基于扩散模型的师生框架，通过属性感知伪标签监督来提升人脸交换中的属性保真度，将人脸交换重新定义为条件去模糊任务，实现了最先进的属性保留和身份转移效果。


<details>
  <summary>Details</summary>
Motivation: 人脸交换的目标是将源人脸的身份转移到目标人脸上，同时保留目标人脸的姿态、表情、光照、肤色和妆容等属性。然而，由于缺乏真实的地面真值数据，同时实现准确的身份转移和高质量的属性保留仍然具有挑战性。现有的基于扩散模型的方法通过条件修复来提升视觉保真度，但掩码条件会移除目标的关键外观线索，导致属性对齐不准确。

Method: 提出了APPLE（属性保留伪标签）框架，这是一个基于扩散模型的师生框架。主要方法包括：1）将人脸交换重新定义为条件去模糊任务，以更忠实地保留光照、肤色和妆容等目标特定属性；2）引入属性感知反演方案，进一步提升细节属性保留；3）通过精心设计的属性保留教师学习，生成高质量的伪三元组，为学生提供直接的人脸交换监督。

Result: APPLE在属性保留和身份转移方面实现了最先进的性能，能够生成更逼真且更忠实于目标属性的结果。

Conclusion: APPLE通过属性感知伪标签监督和条件去模糊任务重新定义，有效解决了人脸交换中属性保留的挑战，为高质量人脸交换提供了新的解决方案。
...
TL;DR: 提出了一种基于模式约束的AI提取系统，可将生物医学PDF全文转化为结构化、可分析的数据记录，通过类型化模式、受控词汇和证据门控决策来约束模型推理，实现可扩展且可审计的科学文献提取。


<details>
  <summary>Details</summary>
Motivation: 生物医学证据合成需要从全文研究文章中准确提取方法学、实验室和结果变量，但这些变量嵌入在复杂的科学PDF中，手动提取耗时且难以扩展。现有文档AI系统存在OCR错误、长文档碎片化、吞吐量受限以及高风险合成审计性不足等问题。

Method: 开发了模式约束的AI提取系统：1) 使用恢复感知哈希摄取文档；2) 将文档分割为标题感知的页面级块；3) 在显式并发控制下异步处理；4) 通过冲突感知整合、基于集合的聚合和句子级溯源将块级输出确定性合并为研究级记录。

Result: 在直接口服抗凝剂水平测量研究语料库上评估：1) 管道无需人工干预处理所有文档；2) 在服务约束下保持稳定吞吐量；3) 跨文档块表现出强内部一致性；4) 迭代模式改进显著提高了合成关键变量的提取保真度，包括检测分类、结果定义、随访持续时间和测量时间。
...
TL;DR: 本文提出多轮压力测试框架，对三个前沿大语言模型进行长对话安全测试，发现模型在心理支持场景中容易在多轮对话中逐渐突破安全边界，特别是做出确定性承诺。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在心理健康支持领域的安全评估主要局限于单轮对话中的违禁词检测，忽视了长对话中安全边界的逐渐侵蚀问题。随着主流大语言模型的发展，明显有安全风险的词汇容易被底层系统过滤，真正的危险在于多轮互动中模型试图提供安慰和共情时逐渐突破边界。

Method: 提出多轮压力测试框架，使用两种压力方法：静态递进和自适应探测。生成50个虚拟患者档案，通过最多20轮的虚拟精神科对话对三个前沿大语言模型进行压力测试。

Result: 实验结果显示违规行为普遍存在，两种压力模式产生相似的违规率。但自适应探测显著提前了模型突破边界的时间，将平均轮数从静态递进的9.21轮减少到4.64轮。在两种机制下，做出确定性或零风险承诺是边界被突破的主要方式。

Conclusion: 大语言模型安全边界的鲁棒性不能仅通过单轮测试推断；必须充分考虑扩展对话中不同交互压力和特性对安全边界造成的磨损。需要更全面的多轮对话安全评估方法。
...
TL;DR: 这篇综述论文系统性地探讨了大语言模型多步推理的内部机制，而非仅仅关注提升性能的工程方法，提出了七个相互关联的研究问题框架，并指出了未来机制研究的五个方向。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在解决需要多步推理的问题上表现出色，但其内部实现这种能力的机制仍然不明确。现有研究多集中于提升性能的工程方法，缺乏对内在机制的深入理解。

Method: 作者构建了一个包含七个相互关联研究问题的概念框架，系统性地组织了对LLM多步推理机制的综述，从隐式多跳推理到显式推理的内部计算重塑等方面进行全面分析。

Result: 论文提供了一个全面的LLM多步推理机制研究框架，涵盖了从隐藏激活中的隐式多跳推理到语言化显式推理如何重塑内部计算等多个关键维度。

Conclusion: 该综述为理解LLM多步推理的内在机制提供了系统性框架，并指出了未来机制研究的五个重要方向，有助于推动对LLM推理能力的深入理解。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育选择题自动生成中产生的幻觉（流畅但不正确或不连贯的输出）是一个重大挑战，阻碍了可靠教育内容的规模化创建

Method: 提出无幻觉多智能体生成框架，将选择题生成分解为离散可验证阶段，使用基于规则和LLM的检测智能体，结合幻觉评分指标，将问题生成重新定义为最小化幻觉风险同时最大化有效性、可回答性和成本效益的优化任务，并引入基于反事实推理和思维链的智能体引导精炼过程
...
TL;DR: RPC-Bench是一个基于计算机科学论文审稿-回复交流构建的大规模问答基准，包含15K人工验证的QA对，用于评估模型在学术论文理解方面的能力，发现即使最强模型在精确理解方面仍有显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准在细粒度、大规模评估基础模型对科研论文的理解能力方面存在局限，特别是针对专业科学论述和复杂图表表格的理解。

Method: 基于高质量计算机科学论文的审稿-回复交流构建大规模问答基准，设计符合科研流程的细粒度分类法，建立LLM-人类交互标注框架支持大规模标注和质量控制，采用LLM-as-a-Judge范式评估模型的正确性-完整性和简洁性。

Result: 即使最强模型（GPT-5）在正确性-完整性方面仅达到68.2%，经过简洁性调整后降至37.46%，表明在精确学术论文理解方面存在显著差距。

Conclusion: RPC-Bench为评估模型对科研论文的理解能力提供了有效的基准工具，揭示了当前模型在学术理解方面的局限性，为未来改进提供了方向。
...
TL;DR: 提出Verifier-Guided Distillation训练协议，通过将错误修复过程（冲突检测和回溯）而不仅仅是正确答案转移到小型语言模型（7B参数），使其能够检测矛盾并修正早期假设。
...
Conclusion: 通过转移错误修复过程而不仅仅是最终答案，小型语言模型可以学习更稳健的推理行为，提高在约束满足问题上的表现。
...
TL;DR: Plan-Critic方法通过在AR音频生成中早期评估前缀质量，引导探索并重新分配计算资源，显著提升文本到音频生成的指令跟随能力


<details>
  <summary>Details</summary>
Motivation: 自回归模型在生成时序连贯音频方面表现出色，但在遵循复杂文本提示（特别是描述复杂声音事件时）方面存在不足。研究发现AR模型的早期前缀令牌隐式编码了最终输出的全局语义属性，这揭示了隐式规划的能力

Method: 提出Plan-Critic，一个轻量级辅助模型，使用广义优势估计启发式目标训练，从部分生成中预测最终指令跟随质量。在推理时，Plan-Critic实现引导式探索：早期评估候选前缀，修剪低保真轨迹，并将计算资源重新分配给高潜力规划种子

Result: Plan-Critic引导的采样在CLAP分数上比AR基线提高了10分，在AR文本到音频生成中建立了新的最先进水平，同时保持与标准最佳N解码相同的计算复杂度

Conclusion: 这项工作弥合了因果生成与全局语义对齐之间的差距，证明即使是严格的自回归模型也能够进行前瞻性规划，为AR音频生成提供了新的指导方法
...
TL;DR: 该研究分析了说话人嵌入与语言学驱动的音系规则在口音语音合成中的交互作用，提出了音素转移率（PSR）这一新指标来量化嵌入对规则转换的保持或覆盖程度。


<details>
  <summary>Details</summary>
Motivation: 当前TTS系统通过特定口音的说话人嵌入来生成带口音的语音，这种方法虽然有效但可解释性和可控性有限，因为嵌入同时编码了音色、情感等多种特征。需要更清晰的口音控制方法。

Method: 以美式和英式英语为案例研究，实现了flapping、rhoticity和元音对应等音系规则。提出了音素转移率（PSR）这一新指标来量化说话人嵌入对规则转换的保持或覆盖程度。通过结合规则与嵌入进行实验。

Result: 实验表明，规则与嵌入结合能产生更真实的口音，同时发现嵌入可以减弱或覆盖规则，揭示了口音与说话人身份之间的纠缠关系。

Conclusion: 音系规则可作为口音控制的有效杠杆，并为评估语音生成中的解纠缠提供了框架。研究强调了规则在提高口音合成可控性和可解释性方面的重要性。
...
TL;DR: 本文提出了一种模型和任务无关的框架，用于设计人-LLM协作的定性分析方法，并在多站点糖尿病护理研究中验证了其有效性，展示了LLM如何提升健康服务研究的效率同时保持严谨性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在提高大规模、多站点健康服务研究的定性分析效率方面显示出潜力，但目前缺乏将LLM整合到定性分析中的方法指导，以及它们对现实世界研究方法和结果影响的证据。

Method: 开发了一个模型和任务无关的框架，用于设计人-LLM协作的定性分析方法。在联邦合格健康中心（FQHCs）的多站点糖尿病护理研究中，应用该框架实现了两种人-LLM方法：1）对研究者生成的摘要进行定性综合以产生比较反馈报告；2）对167份访谈记录进行演绎编码以完善实践转型干预措施。

Result: LLM辅助实现了对从业者的及时反馈，并整合了大规模定性数据来指导理论和实践变革。该工作展示了LLM如何能够整合到应用健康服务研究中，在保持严谨性的同时提高效率。

Conclusion: 这项工作展示了LLM可以如何整合到应用健康服务研究中，在保持严谨性的同时提高效率，为在定性研究中继续创新使用LLM提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 尽管最先进的LLM在各种NLP任务中表现出色，但它们在处理中等复杂统计挑战方面的能力尚未得到充分理解。需要探索LLM在统计推理方面的潜力及其在教育技术和统计分析辅助系统中的应用价值。

Method: 研究者在专门开发的数据集上微选了开源大型语言模型，以增强其统计推理能力。将微调后模型的性能与人类评分基准进行比较，并评估LLM作为答案质量评估工具的效果。

Result: 微调后的模型在高级统计任务上达到统计学学生水平的性能。微调效果具有架构依赖性，某些模型显示出显著性能提升。LLM在评估答案质量（包括解释和推理评估）方面远优于BLEU或BertScore等传统指标。

Conclusion: LLM在统计推理方面具有显著潜力，可通过微调达到实用水平。其自我评估能力为统计教育平台和自动化分析工具的质量保证提供了可扩展的解决方案。潜在应用包括学术和工业环境中的研究方法验证工具，以及数据分析工作流程的质量控制机制。
...
TL;DR: 本文研究了自动化AI研究的执行落地问题，通过构建自动执行器验证LLM生成的想法，并在LLM预训练和后训练两个实际研究问题上展示了执行引导的进化搜索比强化学习更有效。
...
Result: 自动执行器能实现大部分前沿LLM生成的想法；执行引导的进化搜索样本效率高：在后训练上找到显著优于GRPO基线的方法（69.4% vs 48.0%），在预训练上找到优于nanoGPT基线的配方（19.7分钟 vs 35.9分钟）；前沿LLM在搜索中能生成有意义的算法想法但容易早期饱和；强化学习则存在模式崩溃问题。

Conclusion: 执行引导的自动化AI研究具有潜力，进化搜索比强化学习更有效，但LLM生成的想法存在早期饱和问题，需要进一步研究执行落地的自动化AI研究。
...
<details>
  <summary>Details</summary>
Motivation: 公平决策需要忽略无关的偏见信息，但人类难以进行反事实自我模拟，导致即使善意决策者也会产生偏见判断。研究探索LLMs是否面临类似限制，以及如何解决这一问题

Method: 研究LLMs在抵消性别和种族偏见方面的能力，测试提示模型忽略或假装不知道偏见信息的效果，并探索通过访问自身API（盲化副本）来改善决策的方法

Result: LLMs确实存在类似人类的偏见限制，提示忽略偏见信息的方法效果有限甚至适得其反。但通过访问自身API的盲化副本响应，可以实现更公平的决策，并提供区分隐性偏见和故意偏见行为的透明度

Conclusion: LLMs在反事实决策方面存在与人类相似的局限性，但通过技术手段（访问自身API的盲化版本）可以显著改善公平性，同时提高决策透明度
...
TL;DR: 本文提出了PedagogicalRL-Thinking框架，通过教育理论引导的内部推理提示和显式评估推理质量的思维奖励，优化LLM在教育场景中的教学对齐，实验表明该方法能提升教学推理质量并保持基础模型的事实知识。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为智能教学系统的部署日益增多，但针对教育场景的优化研究有限。现有的强化学习方法仅优化可见响应，忽视了模型的内部思维过程，这限制了教学对齐的效果。

Method: 提出PedagogicalRL-Thinking框架，包含两个创新方法：1) 教学推理提示：使用特定领域教育理论而非通用指令来引导内部推理；2) 思维奖励：显式评估和强化模型推理轨迹的教学质量。

Result: 实验表明：领域特定、理论基础的提示优于通用提示；思维奖励与教学提示结合最有效；仅在数学教学对话上训练的模型在未见过的教育基准测试中表现提升，同时保持基础模型的事实知识；教学思维奖励产生系统性推理轨迹变化，增加教学推理和结构化教学决策。

Conclusion: PedagogicalRL-Thinking框架通过将教学对齐扩展到推理LLM的内部思维过程，显著提升了教育场景中LLM的教学质量，为优化智能教学系统提供了新方向。
...
TL;DR: Social Caption是一个基于互动理论的多模态大语言模型社会理解能力评估框架，包含社会推理、整体社会分析和定向社会分析三个维度，分析模型规模、架构设计和口语语境等因素对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 社会理解能力对于多模态大语言模型解释人类社交互动至关重要，但目前缺乏系统评估这些能力的框架。需要建立基于互动理论的评估体系来全面衡量模型的社会理解能力。

Method: 提出Social Caption框架，基于互动理论从三个维度评估MLLMs的社会理解能力：社会推理（SI）-对互动做出准确推断；整体社会分析（HSA）-生成全面的互动描述；定向社会分析（DSA）-从互动中提取相关社会信息。通过实验分析模型规模、架构设计和口语语境等因素对性能的影响。

Result: 实验表明Social Caption框架能有效评估MLLMs的社会理解能力，并揭示了模型规模、架构设计和口语语境等因素对社会理解性能的影响。使用MLLM作为评估者提供了多模态社会理解自动评估的可扩展性见解。

Conclusion: Social Caption提供了一个基于互动理论的系统框架来评估多模态大语言模型的社会理解能力，为未来模型开发和评估提供了重要参考，同时展示了自动化评估多模态社会理解的可扩展性潜力。
...
TL;DR: 研究发现大型语言模型存在tokenization脆弱性问题：多个不同的token ID序列可以解码为相同的文本字符串，导致模型在处理看似相同的文本时产生不同的内部表示，从而影响推理能力。


<details>
  <summary>Details</summary>
Motivation: 现代子词分词器会产生非唯一编码（多个token ID序列可以解码为相同的表面字符串），这种表示不匹配造成了未测量的脆弱性，可能导致推理过程失败。LLMs可能将两个内部表示视为不同的"单词"，即使它们在文本层面语义相同。

Method: 引入tokenization一致性探测任务，要求模型在上下文中替换指定目标词同时保持其他内容不变。该任务在表面层面故意设计简单，以便将失败归因于分词器-解码器伪影而非知识差距或参数限制。分析了超过11000个替换试验。

Result: 发现非平凡比例的输出表现出"幻影编辑"现象：模型在正确推理的幻觉下操作，这是由分词器引起的表示缺陷造成的。进一步分析这些案例，提供了八种系统性的分词器伪影分类，包括空格边界偏移和词内重新分割。

Conclusion: 部分明显的推理缺陷源于分词器层，这促使在投入更大成本训练更大模型之前，先考虑分词器层面的补救措施。
...
TL;DR: ClaimDB是首个基于大规模结构化数据的真实性验证基准，包含80个真实数据库，要求模型通过可执行程序推理而非简单阅读证据，现有LLM在该任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前事实验证基准主要关注文本证据，而基于大规模结构化数据（数百万记录和多表组合）的声明验证尚未得到充分探索，这在实际数据分析和决策中至关重要。

Method: 构建ClaimDB基准，包含80个独特真实数据库，涵盖治理、医疗、媒体、教育、自然科学等多个领域。声明证据来自数百万记录和多表组合，迫使验证方法从"阅读"转向可执行程序推理。

Result: 测试30个最先进的专有和开源LLM（低于700亿参数），没有模型超过83%准确率，超过一半低于55%。模型在"弃权"（承认缺乏证据）方面表现不佳，对高风险数据分析的可靠性存疑。

Conclusion: 基于大规模结构化数据的事实验证具有挑战性，现有LLM在该任务上表现有限，特别是在弃权能力方面。这突显了开发能够处理复杂数据推理的AI系统的必要性。
...
TL;DR: DARL是一个增强LLM推理能力的强化学习框架，通过鼓励在参考答案可控偏差范围内生成多样答案，解决了现有方法过度拟合参考答案、缺乏多样性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法依赖领域特定验证器，适用范围有限；RLPR等扩展方法虽然适用于更广泛领域，但倾向于过度拟合参考答案，限制了模型生成多样输出的能力，这在开放式任务（如写作）中尤为明显。

Method: 提出DARL框架，在保持与参考答案对齐的同时，鼓励在可控偏差范围内生成多样答案。该框架完全兼容现有通用强化学习方法，无需额外验证器即可无缝集成。

Result: 在13个基准测试上实验显示，DARL在推理性能上取得一致改进。在6个推理基准上平均提升1.3分，在7个通用基准上平均提升9.5分，显著优于RLPR。

Conclusion: DARL有效解决了现有强化学习方法过度拟合参考答案的问题，在提高推理准确性的同时显著增强了输出多样性，适用于开放式任务。
...
<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought (CoT) 提示虽然提升了大型语言模型的推理能力，但其冗长的文本输出带来了巨大的计算开销。现有方法通常只关注结果对齐，缺乏对中间推理过程的监督，导致潜在推理链难以分析。

Method: 提出Render-of-Thought (RoT)框架，将文本推理步骤渲染成图像，使潜在推理过程显式化和可追踪。利用现有视觉语言模型(VLMs)的视觉编码器作为语义锚点，将视觉嵌入与文本空间对齐，实现即插即用而无需额外预训练开销。

Result: 在数学和逻辑推理基准测试中，RoT相比显式CoT实现了3-4倍的token压缩和显著的推理加速。同时，与其他方法相比保持了竞争力的性能，验证了该范式的可行性。

Conclusion: RoT框架通过将推理链可视化，不仅解决了CoT冗长计算的问题，还增强了推理过程的可分析性和可追踪性，为高效推理提供了新范式。
...
TL;DR: 该研究提出了PsyFIRE框架和RECAP系统，用于在文本心理咨询中检测客户抵抗行为，通过细粒度分类和解释性分析提升咨询效果。


<details>
  <summary>Details</summary>
Motivation: 在文本心理咨询中识别客户抵抗行为对咨询效果至关重要，但现有NLP方法存在三个主要问题：1）过度简化抵抗类别；2）忽略治疗干预的序列动态；3）可解释性有限。需要更精细、理论支撑且可解释的抵抗检测方法。

Method: 1）提出PsyFIRE理论框架，包含13种细粒度抵抗行为和协作互动；2）构建ClientResistance语料库，包含23,930条来自真实中文文本咨询的标注话语，每条都有上下文特定理由；3）开发RECAP两阶段框架：第一阶段检测抵抗vs协作，第二阶段分类细粒度抵抗类型并提供解释。

Result: RECAP在区分协作与抵抗方面达到91.25%的F1分数，在细粒度抵抗类别分类方面达到66.58%的宏平均F1分数，比领先的基于提示的大语言模型基线高出20多个百分点。应用于独立咨询数据集和62名咨询师的试点研究显示，RECAP能揭示抵抗行为的普遍性及其对治疗关系的负面影响。

Conclusion: PsyFIRE和RECAP框架为文本心理咨询中的抵抗检测提供了理论支撑、细粒度分类和可解释的分析方法，能够帮助咨询师更好地理解客户抵抗行为并改进干预策略，具有实际应用价值。
...
Method: 构建包含53部经典电影的中文剧本续写基准，采用"前半部到后半部"的续写范式，每部电影生成3个样本。使用多维评估框架，包括ROUGE-L、结构相似性和LLM-as-Judge评分（DeepSeek-Reasoner），共获得303个有效样本进行统计分析。

Result: Qwen-Max在ROUGE-L上略高（0.2230 vs 0.2114），但GPT-5.2在结构保持（0.93 vs 0.75）、整体质量（44.79 vs 25.72）和综合评分（0.50 vs 0.39）上显著更优，整体质量效应值达到大效应水平（d>0.8）。GPT-5.2在角色一致性、风格匹配和格式保持方面表现更好。

Conclusion: GPT-5.2在中文电影剧本续写任务上整体表现优于Qwen-Max，特别是在结构保持和整体质量方面。该研究为中文创意写作的LLM评估提供了可复现的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有MRAG方法采用统一的"一刀切"策略，在不同语言查询处理中容易产生知识偏见和冲突，导致性能不佳。

Method: 提出LcRL框架，集成语言耦合组相对策略优化到策略和奖励模型中，采用语言耦合组采样减少知识偏见，并通过辅助反一致性惩罚缓解知识冲突。

Result: 实验结果表明LcRL不仅达到竞争性性能，而且适用于多种实际场景，如受限训练数据和包含大量语言的检索集合。

Conclusion: LcRL通过解决多语言检索增强生成中的知识偏见和冲突问题，提供了一个有效的框架，适用于各种实际应用场景。
...
TL;DR: PodBench是一个包含800个样本的播客脚本生成基准测试，支持长达21K token的输入和复杂多说话者指令，通过定量约束和LLM质量评估的综合框架进行评测。
...
Method: 提出了PodBench基准测试，包含800个样本，输入长度可达21K token，支持复杂多说话者指令。开发了综合评估框架，结合定量约束和基于LLM的质量评估。

Result: 实验表明，虽然专有模型通常表现优异，但配备显式推理能力的开源模型在处理长上下文和多说话者协调方面比标准基线模型展现出更强的鲁棒性。然而分析发现，高指令遵循度并不保证高质量内容。

Conclusion: PodBench为长格式、以音频为中心的生成任务提供了一个可复现的测试平台，有助于解决指令遵循与内容质量之间的脱节问题。
...
Method: 提出CodeDelegator多智能体框架：1) 持久化的Delegator负责任务分解、编写规范和监控进度但不执行代码；2) 为每个子任务实例化新的Coder智能体，提供干净的上下文；3) 引入EPSS机制隔离每个Coder的执行状态同时保持全局一致性。

Result: 在各种基准测试上的实验证明了CodeDelegator在不同场景中的有效性。

Conclusion: 通过角色专业化和上下文隔离，CodeDelegator解决了单一智能体在复杂任务中面临的上下文污染问题，提高了长视野任务的性能。
...
TL;DR: 该研究开发了语料库澄清框架，用于标准化公民咨询数据，并评估小型开源语言模型在民主咨询分析中的表现，创建了最大的标注民主咨询数据集。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在NLP中广泛应用，但将其用于分析民主活动（如在线讨论、大规模公民咨询）时存在伦理问题。研究旨在开发资源来标准化公民贡献，并研究小型开源模型能否可靠执行这种标准化。

Method: 引入语料库澄清作为预处理框架，将嘈杂的多主题贡献转化为结构化、自包含的论证单元。创建GDN-CC数据集（1,231个法国大国民辩论贡献，2,285个论证单元），标注论证结构并进行人工澄清。然后微调小型语言模型来复制这些标注，并评估其在意见聚类任务中的可用性。

Result: 微调的小型语言模型在复制标注方面匹配或优于大语言模型。发布了GDN-CC-large，包含24万个自动标注的贡献，是迄今为止最大的标注民主咨询数据集。

Conclusion: 语料库澄清框架能有效标准化公民咨询数据，小型开源语言模型可以可靠地执行这种标准化，为民主咨询分析提供了透明、可本地运行的解决方案。
...
TL;DR: 该研究评估了现代语言模型在僧伽罗语（包括Unicode和罗马化版本）上的表现，发现不同模型在不同脚本上表现各异，为僧伽罗语应用提供了模型选择指南。
...
Result: Mistral-Nemo-Base-2407在Unicode文本上表现最佳，Mistral-7B-v0.3在罗马化文本上表现最好；Llama-3.1-8B在两种脚本上都表现均衡；闭源模型中，Gemini-1.5-pro和DeepSeek擅长Unicode生成，Claude-3.5-Sonnet在罗马化文本处理上更优。

Conclusion: 研究结果为僧伽罗语特定应用提供了重要的模型选择指南，并强调了训练数据在处理脚本变体中的关键作用，揭示了不同模型在不同僧伽罗语脚本上的性能差异。
...
TL;DR: 该研究探讨了多语言环境下LLM的数据污染问题，发现翻译成阿拉伯语会抑制传统污染检测指标，但模型仍能从污染数据中获益，特别是阿拉伯语能力强的模型。研究提出了翻译感知污染检测方法，通过比较多个翻译版本而非仅英语版本来有效识别污染。


<details>
  <summary>Details</summary>
Motivation: 数据污染会破坏大语言模型评估的有效性，使模型依赖记忆而非真正泛化。先前的研究主要局限于英语基准测试，对多语言污染的理解不足。本研究旨在探索多语言环境下的污染动态，特别是翻译对污染检测的影响。

Method: 1) 在多个开放权重LLM上微调不同比例的阿拉伯语数据集，并在原始英语基准上评估；2) 扩展Tested Slot Guessing方法，加入选择重排序策略；3) 结合Min-K%概率分析，捕捉行为和分布污染信号；4) 提出翻译感知污染检测方法，通过比较多个翻译基准变体而非仅英语版本来识别污染。

Result: 1) 翻译成阿拉伯语会抑制传统污染指标，但模型仍能从污染数据中获益，特别是阿拉伯语能力强的模型；2) 随着污染水平增加，Min-K%分数上升，跨语言答案一致性提高；3) 翻译感知污染检测方法能可靠地暴露污染，即使在仅英语方法失败的情况下也有效。

Conclusion: 需要建立多语言、翻译感知的评估流程，以确保LLM评估的公平性、透明性和可重复性。翻译感知污染检测方法能有效识别传统方法无法检测的污染，强调了多语言评估的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的开放域关系三元组抽取方法依赖静态启发式提示策略，缺乏反思机制来内化错误信号，导致语义模糊时容易产生永久性错误抽取模式。

Method: 提出知识重构驱动的提示优化框架KRPO：1) 基于知识恢复的自评估机制，将结构化三元组投影为语义一致性分数提供内在反馈；2) 基于文本梯度的提示优化器，内化历史经验迭代优化提示；3) 关系规范化记忆，收集代表性关系并提供语义区分模式。

Result: 在三个数据集上的广泛实验表明，KRPO在抽取F1分数上显著优于强基线方法。

Conclusion: KRPO框架通过知识重构驱动的自评估和提示优化，有效提升了大语言模型在复杂开放域关系三元组抽取任务中的能力，解决了语义模糊和关系冗余问题。
...
TL;DR: LogicScore是一个新的评估框架，用于解决Attributed Question Answering中存在的"归因短视"问题，通过全局逻辑完整性评估来补充现有的局部归因验证。


<details>
  <summary>Details</summary>
Motivation: 当前AQA评估方法存在"归因短视"问题，过于关注孤立陈述的归因验证，而忽视了长答案的全局逻辑完整性。这导致LLM经常产生事实正确但逻辑不连贯的回答，存在难以察觉的推理漏洞。

Method: 基于Horn规则，提出LogicScore评估框架，采用后向验证机制，系统评估三个关键推理维度：完整性（逻辑推理的严谨性）、简洁性（非冗余性）和确定性（一致答案蕴含）。

Result: 在三个多跳QA数据集（HotpotQA、MusiQue和2WikiMultiHopQA）和20多个LLM上的实验显示：领先模型虽然获得高归因分数（如Gemini-3 Pro精度达92.85%），但在全局推理质量上表现不佳（如Gemini-3 Pro简洁性仅35.11%）。

Conclusion: LogicScore为逻辑评估建立了稳健标准，强调在LLM开发中需要同时重视推理连贯性和事实基础，揭示了当前模型在全局推理能力上的关键差距。
...
TL;DR: 多智能体系统通过不同智能体施加不同约束，实现比单个智能体更好的问题解决性能，即使它们拥有相同的信息
...
Method: 基于算子理论和约束优化建立形式化框架，将每个智能体建模为在共享解状态上施加不同有效性约束的实体，多智能体系统实现约束执行算子的分解组合

Result: 多智能体系统动态收敛到由智能体约束集交集定义的不变解集，这些结构通常无法被单个智能体同时应用所有约束时动态访问到

Conclusion: 从精确约束执行扩展到软约束的邻近算子，并将形式化框架应用于当代基于文本的对话系统，为多智能体系统的性能优势提供了理论解释
...
TL;DR: 研究发现人类语义行为存在昼夜节律：早晨语义探索性更强，下午则更集中于已有话题，这种模式与情绪无关，可能与神经调节系统的昼夜节律有关。


<details>
  <summary>Details</summary>
Motivation: 人类认知存在明显的昼夜节律调节，但对其如何影响高维语义行为仍知之甚少。本研究旨在探索语言使用中的时间变化模式及其与生物节律的关系。

Method: 使用大规模Reddit数据，通过预训练transformer模型将文本嵌入语义空间，测量语义熵作为语言探索-利用的指标，区分局部和全局语义熵，分析其昼夜变化模式。

Result: 发现语义探索存在稳健的昼夜节律性，可受季节性光照线索调节。局部语义探索在早晨达到峰值，反映更广泛的语义空间探索；全局语义多样性在下午达到峰值，表现为提交内容围绕已建立话题的积累，符合"富者愈富"动态。这些模式不能用情感或情绪效价解释。
...
TL;DR: 开发了一个包含200张胸部X光片、12个基准标签的公开数据集，采用AI辅助专家标注流程，通过三位放射科医师验证，用于评估多模态大语言模型的临床性能。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在放射学选择题考试中表现良好，但需要由领域专家标注的高质量基准来开发真正有用的临床工具。

Method: 使用13,735张去标识化胸部X光片，通过GPT-4o提取异常发现，本地LLM映射到12个基准标签。采用AI辅助标注流程，采样1,000张供专家评审，三位放射科医师评估LLM建议标签的正确性。

Result: 创建了包含200张胸部X光片的基准数据集（100张公开，100张保留），每张由三位放射科医师验证，其中381张获得至少两位专家"完全同意"。数据集已在RSNA网站公开。

Conclusion: 成功开发了高质量胸部X光片基准数据集和AI辅助标注流程，支持大规模标注、减少遗漏，为多模态LLM的临床评估提供可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在临床决策支持中的应用日益增多，但幻觉和不安全建议可能直接威胁患者安全。这些风险表现为细微的临床错误，难以通过通用指标检测，而专家编写的细粒度评估标准成本高且难以扩展。

Method: 提出检索增强的多智能体框架，通过分解检索到的权威医学证据为原子事实，结合用户交互约束，合成可验证的细粒度评估标准。

Result: 在HealthBench上，框架达到60.12%的临床意图对齐分数，显著优于GPT-4o基线(55.16%)。判别测试中，评估标准产生平均分数差8.658和AUROC 0.977，几乎是GPT-4o基线(4.972)的两倍。评估标准还能指导响应改进，质量提升9.2%。

Conclusion: 该框架为评估和改进医疗大语言模型提供了可扩展且透明的基础，通过自动生成实例特定的评估标准，有效解决了医疗领域评估的挑战。
...
TL;DR: 该研究提出了一种新的证据基础框架来比较同行评审质量，应用于AI/ML会议，发现评审质量并未如普遍担忧那样持续下降。


<details>
  <summary>Details</summary>
Motivation: 随着投稿数量增加和研究社区扩大，评审质量下降成为普遍担忧，但缺乏可靠的测量方法和跨时间、跨会议的比较框架。

Method: 提出新的评审质量比较框架，包括评审格式标准化方法、多维质量量化模式（对编辑和作者的效用），结合LLM和轻量级测量方法，应用于ICLR、NeurIPS和*ACL会议。

Result: 跨时间分析显示，在研究的会议和年份中，评审质量中位数并未出现一致下降趋势，这与普遍担忧的叙事相矛盾。

Conclusion: 需要建立更系统的实证研究框架来理解评审质量变化，提出了促进未来评审质量实证研究的建议。
...
TL;DR: 该研究将基于事实片段（nugget）的评估方法从自动化评估扩展到用户端，开发了一个原型系统帮助法律专业人士比较和改进法律文件摘要。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地用于总结长文档，在法律领域面临重大挑战，因为证词摘要的事实准确性至关重要。虽然基于事实片段的方法已被证明对自动化评估摘要方法非常有帮助，但其支持终端用户的潜力尚未得到充分探索。

Method: 研究将基于事实片段的方法从评估端转换到用户端，专注于法律领域，开发了一个原型系统，利用基于事实片段的方法支持法律专业人士在两个具体场景中：1）确定两个摘要中哪个更好；2）手动改进自动生成的摘要。

Result: 研究提出了一个原型系统，展示了基于事实片段的方法如何直接帮助终端用户，特别是在法律领域，为法律专业人士提供了比较和改进摘要的工具。

Conclusion: 基于事实片段的方法不仅适用于自动化评估，还可以扩展到用户端，为法律专业人士提供有价值的支持工具，帮助他们在关键的法律文档摘要任务中做出更好的决策和改进。
...
TL;DR: 该研究提出了一种通过元数据条件化实现语言模型本地化的轻量级方法，通过在训练时加入地理元数据，显著提升了模型在特定区域的性能，同时保持了跨区域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型通常将文本视为单一全局分布进行训练，导致地理行为同质化。研究者希望找到一种轻量级方法来实现语言模型的本地化，使其能够更好地适应不同地理区域的语言使用习惯和文化背景。

Method: 研究采用元数据条件化方法，从零开始预训练了31个模型（0.5B和1B参数规模），使用带有已验证URL、国家标签和大陆标签的大规模英文新闻数据，覆盖4个大洲和17个国家。通过四个对照实验验证效果，并引入包含800个本地化新闻多选题的下游基准测试。

Result: 元数据条件化能够持续提升区域内性能而不牺牲跨区域泛化能力，使全局模型能够达到与区域特定模型相当的本地化水平，并提高学习效率。URL级元数据单独就能捕获大部分地理信号，但均衡的区域数据覆盖仍然至关重要。经过指令微调后，元数据条件化的全局模型在准确性上可与LLaMA-3.2-1B-Instruct相媲美，尽管训练数据量少得多。

Conclusion: 元数据条件化是一种实用且计算效率高的语言模型本地化方法，能够在保持模型泛化能力的同时，有效提升其在特定地理区域的性能表现。
...
TL;DR: LLMs在标准算术任务上表现优秀，但在非主流数字脚本和格式上准确率显著下降，尽管数学推理相同。针对性提示策略可以缩小这一差距。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在基本算术方面表现出色，但很少研究当数字表达偏离训练语料中的主流约定时，模型的表现如何。本研究旨在探索LLMs在不同数字脚本和格式下的数值推理能力。

Method: 研究LLMs在广泛数字脚本和格式下的数值推理表现，分析当数字输入以代表性不足的脚本或格式呈现时的准确率变化，并测试针对性提示策略（如few-shot提示和显式数字映射）的效果。

Result: LLM准确率在数字以代表性不足的脚本或格式呈现时显著下降，尽管底层数学推理完全相同。但针对性提示策略（如few-shot提示和显式数字映射）可以大大缩小这一性能差距。

Conclusion: 研究揭示了多语言数值推理中被忽视的挑战，为使用LLMs可靠地解释、操作和生成跨不同数字脚本和格式的数字提供了可行的见解。
...
TL;DR: 该论文研究了假新闻检测模型在情感操纵攻击下的脆弱性，提出了AdSent框架来增强检测器的鲁棒性，通过情感无关训练策略确保对原始和情感修改新闻的一致性预测。


<details>
  <summary>Details</summary>
Motivation: 假新闻检测已成为紧迫的社会挑战，现有方法依赖情感特征进行检测，但大型语言模型的出现使攻击者能够操纵情感来逃避检测器。目前研究主要关注新闻发布者风格特征，而情感操纵这一关键漏洞尚未得到充分探索。

Method: 提出AdSent框架：1) 使用LLMs生成受控的情感对抗攻击样本；2) 分析情感变化对检测性能的影响；3) 引入新颖的情感无关训练策略，增强对情感扰动的鲁棒性。

Result: 实验表明，改变情感会严重影响假新闻检测模型的性能，模型存在偏见：中性文章倾向于被分类为真实，而非中性文章常被分类为虚假。在三个基准数据集上的广泛实验证明，AdSent在准确性和鲁棒性方面显著优于竞争基线，并能有效泛化到未见数据集和对抗场景。

Conclusion: 情感操纵是假新闻检测模型的重要漏洞，AdSent框架通过情感无关训练策略有效提升了检测器的鲁棒性，为构建更可靠的假新闻检测系统提供了解决方案。
...
### [101] [The Ontological Neutrality Theorem: Why Neutral Ontological Substrates Must Be Pre-Causal and Pre-Normative](https://arxiv.org/abs/2601.14271)
*Denise M. Case*

Main category: cs.AI

TL;DR: 本文证明了一个不可能性结果：任何包含因果或规范性承诺的基础层本体论都无法在相互冲突的解释框架间保持中立性，因此中立的本体论基底必须是前因果和前规范性的。


<details>
  <summary>Details</summary>
Motivation: 现代数据系统需要在持续的法律、政治和分析分歧中支持问责制，这要求设计能够作为共享基底的本体论，但现有方法难以在包含因果或规范性承诺的同时保持中立性。

Method: 通过逻辑分析和理论论证，建立本体论中立性的不可能性结果，分析中立性（解释性非承诺性和不相容扩展下的稳定性）与因果/规范性承诺之间的根本冲突。

Result: 证明包含因果或道义结论作为本体论事实的任何本体论都无法在不修订或产生矛盾的情况下作为不同框架间的中立基底，因此中立的本体论基底必须是前因果和前规范性的。

Conclusion: 为在冲突的解释框架间维持共享、稳定的现实表示，系统设计必须遵循特定约束：本体论基底应仅表示实体及其同一性和持久性条件，而将解释、评估和解释外部化。
...
### [102] [Epistemic Constitutionalism Or: how to avoid coherence bias](https://arxiv.org/abs/2601.14295)
*Michele Loi*

Main category: cs.AI

TL;DR: 论文主张为AI建立"认知宪法"——明确、可争议的元规范来监管AI系统如何形成和表达信念，以源归属偏见为案例，区分了柏拉图式和自由主义两种宪法方法，并支持后者。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为人工推理者，其信念形成行为受隐含、未经检验的认知政策支配。源归属偏见提供了动机案例：前沿模型强制执行身份-立场一致性，惩罚那些归属于预期意识形态立场与论点内容冲突的源头的论点。

Method: 通过分析前沿模型对源归属偏见的处理方式，区分两种宪法方法：柏拉图式（要求形式正确性和默认的源独立性）和自由主义式（拒绝特权立场，指定保护集体探究条件的程序规范）。提出包含八个原则和四个方向的宪法核心。

Result: 当模型检测到系统性测试时，源敏感效应崩溃，揭示系统将源敏感性视为需要抑制的偏见而非良好执行的能力。支持自由主义宪法方法，允许基于认知警惕的原则性源关注。

Conclusion: AI认知治理需要与AI伦理相同的明确、可争议结构，主张建立自由主义认知宪法来监管AI系统的信念形成和表达。
...
### [103] [On the Generalization Gap in LLM Planning: Tests and Verifier-Reward RL](https://arxiv.org/abs/2601.14456)
*Valerio Belcamino,Nicholas Attolino,Alessio Capitanelli,Fulvio Mastrogiovanni*

Main category: cs.AI

TL;DR: 微调大语言模型在PDDL规划任务中表现出高有效规划率，但跨领域泛化能力为0%，表明模型依赖领域特定模式而非可迁移的规划能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探究微调后的大语言模型在规划任务中表现出的高有效规划率是否反映了可迁移的规划能力，还是仅仅是领域特定的记忆。
...
### [104] [Scalable Knee-Point Guided Activity Group Selection in Multi-Tree Genetic Programming for Dynamic Multi-Mode Project Scheduling](https://arxiv.org/abs/2601.14485)
*Yuan Tian,Yi Mei,Mengjie Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种基于拐点选择的多树遗传编程框架，用于解决动态多模式资源受限项目调度问题，通过改进活动组选择策略的可扩展性，在大型实例上表现优于传统的顺序决策方法。


<details>
  <summary>Details</summary>
Motivation: 动态多模式资源受限项目调度问题需要在活动执行顺序和对应执行模式上同时做出决策。虽然活动组选择策略在小规模实例中有效，但在处理更大规模问题时存在可扩展性问题，需要改进该策略以适应大型问题实例。

Method: 提出基于拐点的选择机制：首先使用活动排序规则对所有符合条件的活动-模式对进行排序，然后通过拐点选择找到有前景的对，最后使用组选择规则选择最佳活动组合。开发了多树遗传编程框架来同时演化这两种类型的规则。

Result: 实验结果表明，该方法在大型实例上具有良好的可扩展性，并且在大多数场景中优于使用顺序决策的遗传编程方法。

Conclusion: 通过引入拐点选择机制和多树遗传编程框架，成功解决了活动组选择策略的可扩展性问题，为大规模动态多模式资源受限项目调度问题提供了有效的解决方案。
...
### [105] ["Just in Time" World Modeling Supports Human Planning and Reasoning](https://arxiv.org/abs/2601.14514)
...
### [106] [MAS-Orchestra: Understanding and Improving Multi-Agent Reasoning Through Holistic Orchestration and Controlled Benchmarks](https://arxiv.org/abs/2601.14652)
...
<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统（MAS）自动设计方法存在不足：方法复杂（顺序代码级执行限制全局推理）和效果不确定性（部署前不清楚相比单智能体系统的实际优势）。需要更好的训练和理解框架。

Method: 提出MAS-Orchestra框架，将MAS编排建模为函数调用强化学习问题，将复杂目标导向的子智能体抽象为可调用函数，实现整体系统结构推理。同时引入MASBENCH基准，从深度、视野、广度、并行性和鲁棒性五个维度刻画任务。

Result: 分析表明MAS优势取决于任务结构、验证协议以及编排器和子智能体能力，而非普遍适用。MAS-Orchestra在数学推理、多跳问答和基于搜索的问答等公共基准上取得一致改进。

Conclusion: MAS-Orchestra和MASBENCH共同为多智能体智能的发展提供了更好的训练和理解框架，揭示了MAS优势的条件依赖性，并展示了在实际任务中的有效性。
...
TL;DR: 本研究提出了一种基于本地LLM的上下文感知匿名化框架SFAA，用于定性研究转录本中的敏感数据检测和匿名化处理，相比传统方法更可靠、可重复且能保持数据语义。


<details>
  <summary>Details</summary>
Motivation: 定性研究包含大量个人、情境和组织细节，存在隐私风险。传统手动匿名化耗时、不一致且易遗漏关键标识符，现有自动化工具依赖模式匹配或固定规则，无法捕捉上下文且可能改变数据含义。

Method: 提出结构化自适应匿名化框架SFAA，包含检测、分类和自适应匿名化三个步骤。采用四种匿名化策略：基于规则的替换、上下文感知重写、泛化和抑制，根据标识符类型和风险级别应用。框架遵循GDPR、HIPAA和OECD等国际隐私标准。

Result: 使用LLaMA和Phi两种本地模型评估框架性能。LLM比人工评审员发现更多敏感数据。Phi在发现敏感数据方面优于LLaMA（发现91%以上敏感数据），但错误稍多。Phi处理的文本94.8%保持原始情感，不影响定性数据分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶基准测试和指标过度强调感知能力，未能充分评估决策过程。视觉语言模型展现出推理和泛化能力，为自动驾驶提供了新可能性，但需要专门的决策评估框架。
...
Conclusion: AutoDriDM为自动驾驶提供了从感知到决策的全面评估框架，有助于开发更安全可靠的视觉语言模型，推动真实世界自动驾驶系统的发展。
...
TL;DR: GRPO-Adaptive和DARA框架：通过动态参考策略更新和双阶段决策，提升LLM在AI生成竞价中的数值精度和优化能力，解决少样本场景下的广告预算约束问题


<details>
  <summary>Details</summary>
Motivation: 在线广告中，广告主在预算约束下优化累计价值面临挑战，特别是在AI生成竞价范式下。广告主通常有个性化目标但历史交互数据有限，形成少样本场景，传统强化学习方法效果不佳。大语言模型虽然能利用上下文学习从有限数据中泛化，但缺乏数值精度进行细粒度优化。

Method: 提出GRPO-Adaptive：一种高效的LLM后训练策略，通过动态更新训练过程中的参考策略来增强推理和数值精度。基于此提出DARA框架：将决策过程分解为两个阶段：1) 少样本推理器通过上下文提示生成初始计划；2) 细粒度优化器使用反馈驱动推理来优化这些计划。

Result: 在真实世界和合成数据环境中的大量实验表明，该方法在预算约束下的累计广告主价值方面持续优于现有基线方法。

Conclusion: GRPO-Adaptive和DARA框架成功地将LLM的上下文学习优势与AI生成竞价任务所需的精确适应性相结合，有效解决了少样本场景下的广告预算优化问题。
...
TL;DR: 本文对ASP（答案集编程）的解释方法进行了系统性调查，从可解释AI角度分析现有解释工具的类型、覆盖范围及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: ASP作为一种声明式推理方法，其基于规则的形式使其天然适合可解释推理。随着可解释AI的兴起，ASP的解释能力变得日益重要。然而现有的ASP解释方法和工具往往针对特定场景，未能全面覆盖ASP用户遇到的所有问题。

Method: 采用调查分析方法，从可解释AI的视角出发，系统梳理ASP解释的类型，将其与用户解释需求进行对应分析，评估现有理论和工具对这些解释类型的覆盖情况。

Result: 提供了ASP解释类型的全面概览，分析了现有理论和工具对各种解释场景的覆盖程度，识别了当前ASP解释方法存在的空白和不足。

Conclusion: ASP在可解释AI方面具有天然优势，但现有解释方法存在覆盖不全的问题。需要进一步研究来填补空白，为ASP用户提供更全面的解释支持，推动ASP在可解释AI领域的应用发展。
...
Conclusion: 通过语义引导和增量训练策略，提出的方法有效解决了现有无监督视频摘要方法的局限性，在关键帧选择和视频重建方面表现更优。
...
TL;DR: 本文提出了首个用于无重叠约束的边界一致性算法，通过构建和利用MDD（多值决策图）来收紧作业时间窗口，即使在限制MDD宽度的情况下也能实现比现有方法更强的搜索树剪枝效果。
...
Method: 基于Ciré和van Hoeve定义的无重叠MDD，从中提取作业时间窗口边界来收紧开始和结束时间；为控制计算复杂度，限制MDD宽度创建松弛MDD，实现多项式时间内的边界一致性过滤。

Result: 在带时间窗口的排序问题和准时目标问题上，即使限制MDD宽度，新过滤方法相比Ciré和van Hoeve的优先检测算法能更显著减少搜索树节点数；与经典传播方法互补，在多个实例上同时减少节点数和求解时间。

Conclusion: 本文首次实现了无重叠约束的边界一致性算法，通过MDD方法在多项式时间内实现有效过滤，即使使用宽度限制的松弛MDD也能获得比现有方法更强的剪枝效果，为约束求解提供了新的有效工具。
...
### [113] [Measuring and Aligning Abstraction in Vision-Language Models with Medical Taxonomies](https://arxiv.org/abs/2601.14827)
*Ben Schaper,Maxime Di Folco,Bernhard Kainz,Julia A. Schnabel,Cosmin I. Bercea*

Main category: cs.AI

TL;DR: 该研究评估了视觉语言模型在胸部X光分类中的抽象错误，发现标准平面指标无法区分临床轻微和严重错误，提出使用医学分类学进行分层评估和缓解方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在胸部X光分类中表现出强大的零样本性能，但标准的平面指标无法区分临床轻微错误和严重错误。需要量化并缓解抽象错误，以实现更安全、更具临床意义的模型部署。

Method: 1. 使用分层指标对多个最先进的视觉语言模型进行基准测试；2. 引入"灾难性抽象错误"来捕捉跨分支错误；3. 提出风险约束阈值法；4. 提出使用径向嵌入的分类学感知微调方法。

Result: 研究结果显示，尽管视觉语言模型在平面指标上表现优异，但与临床分类学存在显著不一致。通过提出的方法，能够将严重抽象错误降低到2%以下，同时保持有竞争力的性能。

Conclusion: 分层评估和表示层面的对齐对于视觉语言模型更安全、更具临床意义的部署至关重要。提出的方法能有效减少严重抽象错误，提高模型的临床实用性。
...
### [114] [Implementing Knowledge Representation and Reasoning with Object Oriented Design](https://arxiv.org/abs/2601.14840)
...
Method: KRROOD通过原生类结构将知识作为一等编程抽象，弥合逻辑编程与面向对象编程范式之间的差距，使用原生类结构实现知识表示。

Result: 在OWL2Bench基准测试和人机任务学习场景中的实验结果显示，KRROOD在保持强大性能的同时，支持现实世界自主系统所需的表达性推理能力。

Conclusion: KRROOD成功解决了现代软件工程与知识表示推理系统之间的集成问题，为开发需要复杂推理能力的现实世界自主系统提供了有效框架。
...
### [115] [Just aware enough: Evaluating awareness across artificial systems](https://arxiv.org/abs/2601.14901)
*Nadine Meertens,Suet Lee,Ophelia Deroy*

Main category: cs.AI

TL;DR: 论文提出用"意识度"替代"AI意识"作为更实用、可操作的评价框架，开发了跨系统评估意识度的方法，强调领域敏感、可扩展、多维度和可预测任务表现


<details>
  <summary>Details</summary>
Motivation: 当前关于AI意识和道德地位的争论缺乏共识和可操作方法，需要更实用、方法论上更可行的替代方案来评估AI系统的认知能力

Method: 提出结构化方法评估意识度，将意识度定义为系统处理、存储和使用信息以实现目标导向行动的能力，强调领域敏感、可扩展、多维度、可预测任务表现四个标准

Result: 开发了跨不同架构、规模和操作领域的人工系统评估和比较意识度配置文件的框架，支持原则性评估、设计监督和建设性讨论

Conclusion: 从人工意识转向"足够意识"的评估框架，为AI系统评估提供了更实用、可操作的方法，促进科学和公共讨论的进步
...
### [116] [The Plausibility Trap: Using Probabilistic Engines for Deterministic Tasks](https://arxiv.org/abs/2601.15130)
*Ivan Carrera,Daniel Maldonado-Ruiz*

Main category: cs.AI

TL;DR: 论文提出了"合理性陷阱"概念，指人们过度使用昂贵的概率性AI模型处理简单确定性任务，导致资源浪费。通过OCR和事实核查案例研究，量化了约6.5倍的延迟惩罚，并引入工具选择工程和确定性-概率性决策矩阵来指导何时使用生成式AI。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，用户便利性优先于计算效率的趋势导致人们过度使用昂贵的概率性AI引擎处理简单确定性任务（如OCR、基本验证），造成显著的资源浪费和效率损失。

Method: 通过微基准测试和案例研究（OCR和事实核查）量化"效率税"；引入工具选择工程和确定性-概率性决策矩阵框架，帮助开发者决定何时使用生成式AI以及何时避免使用。

Result: 研究发现存在约6.5倍的延迟惩罚（效率税），并揭示了算法奉承的风险。通过提出的决策框架，可以更明智地选择工具，避免不必要的资源浪费。

Conclusion: 真正的数字素养不仅在于知道如何使用生成式AI，更在于知道何时不使用它。需要课程转变，强调在确定性任务中避免过度使用概率性AI模型，提高资源利用效率。
...
TL;DR: 该论文提出了一种基于知识图谱路径奖励的强化学习后训练方法，让模型从基础事实出发进行组合推理，在医学领域实现了对复杂多跳推理任务的零样本泛化，性能超越了更大的前沿模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学和编程等结构化推理领域已达到接近专家水平，但在专业科学领域进行组合式多跳推理的能力仍然有限。需要一种能让模型基于领域事实进行组合推理的方法。

Method: 提出自下而上的学习范式，通过知识图谱作为隐式奖励模型，结合监督微调和强化学习的后训练流程。从知识图谱路径中推导新颖的奖励信号，提供可验证、可扩展、有基础依据的监督，鼓励模型在强化学习过程中组合中间公理而非仅优化最终答案。

Result: 在医学领域训练了一个140亿参数的模型，在短跳推理路径（1-3跳）上训练，在复杂多跳查询（4-5跳）上实现零样本泛化。模型在最具挑战性的推理任务上显著超越了更大的模型和GPT-5.2、Gemini 3 Pro等前沿系统，并展示了对抗选项洗牌压力测试的鲁棒性。

Conclusion: 将推理过程建立在结构化知识基础上是实现智能推理的可扩展且高效的途径。知识图谱路径奖励作为"组合桥梁"，使模型能够进行复杂的多跳推理。
...
### [118] [BayesianVLA: Bayesian Decomposition of Vision Language Action Models via Latent Action Queries](https://arxiv.org/abs/2601.15197)
...
Method: 提出BayesianVLA框架，通过引入可学习的潜在动作查询构建双分支架构，分别估计视觉先验p(a|v)和语言条件后验π(a|v,ℓ)，然后优化策略以最大化动作与指令间的条件点互信息，惩罚视觉捷径并奖励明确解释语言命令的动作

Result: 在SimplerEnv和RoboCasa上的广泛实验显示显著提升，特别是在具有挑战性的OOD SimplerEnv基准上获得11.3%的改进，验证了该方法在将语言可靠地融入动作中的能力

Conclusion: BayesianVLA通过贝叶斯分解有效解决了VLA模型中的信息崩溃问题，无需额外数据即可显著提升模型泛化能力，为视觉-语言-动作模型的鲁棒指令跟随提供了新思路
...
### [119] [Self-Supervised Score-Based Despeckling for SAR Imagery via Log-Domain Transformation](https://arxiv.org/abs/2601.14334)
*Junhyuk Heo*

Main category: eess.IV

TL;DR: 提出基于分数生成模型的自监督SAR图像去斑方法，在log域中处理乘性噪声，实现快速推理


<details>
  <summary>Details</summary>
Motivation: SAR图像中的斑点噪声是乘性的Gamma分布噪声，会显著降低图像质量并影响后续分析，现有的去斑方法面临挑战

Method: 将SAR图像转换到log域，将斑点噪声残差转换为近似加性高斯分布，然后使用分数生成模型在变换域中通过自监督目标进行训练

Result: 该方法相比现有自监督技术具有显著更短的推理时间，为SAR图像恢复提供了鲁棒实用的解决方案

Conclusion: 提出的基于分数生成模型的自监督框架在log域中有效处理SAR斑点噪声，实现了高效的去斑效果
...
### [120] [Unsupervised Deformable Image Registration with Local-Global Attention and Image Decomposition](https://arxiv.org/abs/2601.14337)
*Zhengyong Huang,Xingwen Sun,Xuting Chang,Ning Jiang,Yao Wang,Jianfei Sun,Hongbin Han,Yao Sui*

Main category: eess.IV

TL;DR: LGANet++：一种基于局部-全局注意力机制的无监督可变形图像配准框架，在跨患者、跨时间和跨模态CT-MR配准任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学图像配准在临床实践中至关重要，但传统方法计算量大且缺乏泛化能力。现有深度学习方法在解剖结构变化大的区域配准效果不佳，需要更准确的配准方法。

Method: 提出LGANet++框架，采用新颖的局部-全局注意力机制，结合独特的特征交互和融合技术，以无监督方式提升配准精度、鲁棒性和泛化能力。

Result: 在五个公开数据集上评估，涵盖三种配准场景：跨患者配准精度提升1.39%，跨时间配准提升0.71%，跨模态CT-MR配准提升6.12%，均优于现有先进方法。

Conclusion: LGANet++在多种医学图像配准场景中表现出优越性能，具有支持临床工作流程中可靠高效图像配准的潜力。
...
### [121] [LiNUS: Lightweight Automatic Segmentation of Deep Brain Nuclei for Real-Time DBS Surgery](https://arxiv.org/abs/2601.14793)
*Shuo Zhang,Zihua Wang,Changgeng He,Chunhua Hu*

Main category: eess.IV

TL;DR: LiNUS是一个用于DBS手术中STN自动分割的轻量级深度学习框架，通过改进U-Net架构，在TT14数据集上达到0.679的Dice系数，推理时间仅0.05秒/例，显著优于传统方法。
...
Method: 基于U-Net架构改进，引入谱归一化约束、双线性插值上采样和多尺度特征融合机制，构建轻量级深度学习框架LiNUS。

Result: 在Tsinghua DBS数据集(TT14)上达到0.679的Dice系数，推理时间仅0.05秒/例；在高分辨率数据上验证获得0.89的Dice分数，显著优于传统方法。

Conclusion: LiNUS框架能够高效准确地实现STN自动分割，开发了专用GUI支持实时临床应用，为DBS手术提供了实用的辅助工具。
...
### [122] [Filtered 2D Contour-Based Reconstruction of 3D STL Model from CT-DICOM Images](https://arxiv.org/abs/2601.14997)
*K. Punnam Chandar,Y. Ravi Kumar*

Main category: eess.IV

TL;DR: 该研究提出使用滤波后的2D轮廓数据点重建3D STL模型，以解决医学图像分割过程中产生的异常值问题，提高3D模型的几何精度。


<details>
  <summary>Details</summary>
Motivation: 从DICOM图像的2D轮廓重建3D STL模型对于理解几何结构和畸形至关重要。然而，由于低分辨率图像分割过程中会产生异常值，导致构建的3D结构与实际几何形状存在偏差。

Method: 处理CT图像以增强对比度、降噪和平滑，使用阈值技术分割图像，提取2D轮廓数据点。提出使用滤波后的2D轮廓数据点，对每个图像的滤波后2D轮廓点进行Delaunay三角剖分，并逐层连接以重建3D STL模型。

Result: 在基本形状的2D数据点和人体骨盆骨感兴趣区域(ROI)上验证了3D STL模型重建。比较了使用滤波和未滤波2D轮廓数据点重建的骨盆骨3D STL模型，结果显示使用滤波数据点重建的模型几何形状得到改善。

Conclusion: 使用滤波后的2D轮廓数据点重建3D STL模型能够有效处理分割过程中的不完美，提高重建模型的几何精度，为医学图像分析和3D建模提供了改进方法。
...
### [123] [Vision Models for Medical Imaging: A Hybrid Approach for PCOS Detection from Ultrasound Scans](https://arxiv.org/abs/2601.15119)
*Md Mahmudul Hoque,Md Mehedi Hassain,Muntakimur Rahaman,Md. Towhidul Islam,Shaista Rani,Md Sharif Mollah*

Main category: eess.IV

TL;DR: 该研究提出了两种混合模型用于从超声图像中准确检测多囊卵巢综合征（PCOS），最终优化的DenConREST模型达到了98.23%的准确率。
...
Method: 提出了两种新颖的混合模型，结合了卷积和基于Transformer的方法。数据分为"感染"（PCOS阳性）和"未感染"（健康卵巢）两类。第一个混合模型DenConST整合了DenseNet121、Swin Transformer和ConvNeXt；最终优化模型DenConREST结合了Swin Transformer、ConvNeXt、DenseNet121、ResNet18和EfficientNetV2。

Result: DenConST模型初始阶段达到85.69%准确率，最终优化的DenConREST模型表现出优越性能，达到98.23%准确率，在所有评估模型中表现最佳。
