...
TL;DR: 该论文针对复杂数学表达式识别难题，构建了CMER-Bench基准测试，发现现有方法在复杂表达式上表现不佳，提出了大规模数据集MER-17M和CMER-3M，并开发了新的结构化数学语言表示和CMERNet模型，显著提升了复杂数学表达式的识别性能。


<details>
  <summary>Details</summary>
Motivation: 当前数学表达式识别（MER）方法在处理简单表达式方面取得了进展，但在识别包含大量标记和多行布局的复杂数学表达式时仍然面临巨大挑战。现有公开训练数据集主要由简单样本组成，导致模型在复杂表达式上性能显著下降。

Method: 1. 构建CMER-Bench基准测试，将表达式分为简单、中等和复杂三个难度级别；2. 创建大规模数据集MER-17M和CMER-3M，专注于复杂数学表达式；3. 提出新的表达式分词器和结构化数学语言表示，显式建模表达式的层次和空间结构；4. 基于编码器-解码器架构开发CMERNet模型，在CMER-3M数据集上进行训练。

Result: 实验结果表明，现有MER模型和多模态大语言模型在简单和中等难度表达式上表现良好，但在复杂表达式上性能显著下降。CMERNet仅用1.25亿参数，在CMER-Bench上显著优于现有MER模型和MLLMs。

Conclusion: 该研究通过构建专门的基准测试、大规模数据集和新的表示方法，有效解决了复杂数学表达式识别的挑战。CMERNet展示了在复杂MER任务上的优越性能，为未来复杂数学表达式识别研究提供了重要基础。
...
TL;DR: 本文探讨了AIGC辅助新闻图像生产的可控性路径，通过两个实验提出人机协作机制和评估指标，解决新闻业中AIGC面临的真实性、语义对齐和伦理困境。


<details>
  <summary>Details</summary>
Motivation: AIGC辅助图像生产在新闻业引发争议，主要问题包括虚假信息、真实性、语义保真度和可解释性。大多数AIGC工具是不透明的"黑箱"，阻碍了内容准确性和语义对齐的双重需求，并引发伦理、社会技术和信任困境。

Method: 通过两个实验探索可控图像生产路径：实验1测试跨平台适应性，使用标准化提示词在三个场景中揭示语义对齐、文化特异性和视觉真实性的差异；实验2构建人机协作模块化流程，结合高精度分割、语义对齐和风格调节技术，通过CLIP语义评分、内容过滤和可验证凭证确保编辑保真度。

Result: 实验1显示不同平台在语义对齐、文化特异性和视觉真实性方面存在差异，主要受训练语料偏见和平台级过滤影响；实验2成功构建了可追溯部署的模块化流程，能够保持语义表征的准确性。

Conclusion: 提出新闻特殊报道中AIGC辅助图像生产的人机协作机制，并建议评估三个关键指标：角色身份稳定性、文化表达准确性和用户-公众适宜性，为新闻业AIGC应用提供可控性解决方案。
...
TL;DR: 该研究提出一个结合深度学习图像分类器与大型语言模型的框架，用于生成胃镜图像的临床推理解释，评估发现强分类能改善解释质量，但当前LLMs在医学决策中仍不可靠。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类器能检测胃肠道疾病但缺乏解释能力，大型语言模型能生成临床文本但视觉推理能力弱且解释不稳定，这造成了模型所见与临床期望的推理类型之间的差距。

Method: 提出结合图像分类与结构化临床推理的框架：设计MobileCoAtNet用于胃镜图像分类（8个类别），其输出驱动多个LLMs进行推理；构建两个专家验证的基准（涵盖病因、症状、治疗、生活方式和随访护理）来评估32个LLMs。

Result: MobileCoAtNet在胃镜图像分类上达到高准确率；强分类能改善LLMs解释质量，但所有模型都未达到人类水平的稳定性；即使最佳LLMs在提示变化时也会改变其推理。

Conclusion: DL与LLMs结合可产生有用的临床叙述，但当前LLMs在高风险医学决策中仍不可靠；该框架为理解其局限性及构建更安全的推理系统提供了路径。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在通用视觉语言任务上表现出色，但在医学决策任务中表现不佳，需要探究其局限性并寻找改进方法

Method: 使用两个具有挑战性的医学数据集：三阶段阿尔茨海默病分类和MIMIC-CXR胸片分类；比较文本推理、视觉推理和视觉文本结合的表现；探索三种改进策略：带推理注释的上下文学习、视觉描述后文本推理、视觉塔的少样本微调

Result: 文本推理在医学决策任务中始终优于视觉或视觉文本结合，多模态输入甚至比纯文本表现更差；三种改进策略有助于提升模型性能
...
TL;DR: 该论文提出了一种将几何与外观解耦的紧凑表示方法，使用面元表示几何，结合全局神经场和每基元颜色表示外观，相比3D高斯泼溅减少了9.7-31倍的基元数量和3.7-5.5倍的内存使用，同时保持视觉质量。
...
Method: 提出了一种超越基于点渲染的表示方法，将几何和外观解耦：1) 使用面元表示几何；2) 使用全局神经场和每基元颜色的组合表示外观；3) 为每个像素纹理固定数量的基元，确保计算开销低。

Result: 在室外场景中使用9.7倍更少的基元和5.5倍更少的内存，在室内场景中使用31倍更少的基元和3.7倍更少的内存，同时匹配3D高斯泼溅的感知质量。渲染速度比现有纹理基元方法快两倍，视觉质量更好。

Conclusion: 该解耦几何和外观的紧凑表示方法在保持高质量视觉输出的同时，显著减少了基元数量和内存使用，提高了渲染效率，为高效的新视角合成提供了有前景的解决方案。
...
TL;DR: VajraV1模型在实时目标检测领域取得新突破，通过整合YOLO系列架构优势，在保持竞争力的推理速度下实现了最先进的检测精度。


<details>
  <summary>Details</summary>
Motivation: 近年来YOLO系列模型（v10-v13）在实时目标检测方面取得显著进展，但仍有提升空间。VajraV1旨在通过整合先前YOLO模型的有效设计选择，在保持实时性能的同时实现更高的检测精度。

Method: VajraV1模型架构结合了先前YOLO模型的有效设计选择，引入了架构增强，优化了实时目标检测的性能平衡。

Result: 在COCO验证集上，VajraV1各版本均表现出色：Nano版44.3% mAP超越YOLOv12-N 3.7%；Small版50.4% mAP超越YOLOv12-S 2.4%；Medium版52.7% mAP超越YOLOv12-M 0.2%；Large版53.7% mAP超越YOLOv13-L 0.3%；Xlarge版56.2% mAP超越所有现有实时目标检测器。

Conclusion: VajraV1通过架构增强成功实现了实时目标检测领域的最先进精度，在保持竞争力的推理速度下超越了现有的YOLO系列模型，为实时目标检测提供了新的高性能解决方案。
...
TL;DR: MoLingo是一个文本到动作生成模型，通过在连续潜在空间中进行去噪来生成逼真的人体动作。该研究改进了潜在空间扩散方法，提出了语义对齐的运动编码器和交叉注意力文本调节机制，在标准指标和用户研究中达到了新的最优水平。


<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成模型在连续潜在空间中进行扩散时存在两个主要问题：1) 潜在空间缺乏语义对齐，使得扩散过程不够有效；2) 文本调节机制不够优化，导致动作与文本描述的对齐度不足。需要研究如何构建更好的语义对齐潜在空间和更有效的文本调节方法。

Method: 1) 提出语义对齐的运动编码器，使用帧级文本标签进行训练，使具有相似文本含义的潜在表示保持接近；2) 比较单令牌调节与多令牌交叉注意力方案，发现交叉注意力能提供更好的动作真实性和文本-动作对齐；3) 结合语义对齐潜在、自回归生成和交叉注意力文本调节。

Result: MoLingo在标准指标和用户研究中都达到了新的最优水平，生成了更逼真、与文本描述更一致的人体动作。模型在动作真实性和文本-动作对齐方面表现优异。

Conclusion: 通过语义对齐的潜在空间构建和交叉注意力文本调节机制，MoLingo显著提升了文本到动作生成的质量。该方法为连续潜在空间扩散提供了有效的解决方案，并将在未来研究中开源代码和模型。
...
TL;DR: 提出CFHA框架，通过三阶段扩散模型将合成无人机图像转化为真实风格，缩小域差距，提升无人机人体检测性能


<details>
  <summary>Details</summary>
Motivation: 无人机人体检测需要大量标注数据，但真实数据标注成本高且目标分布不断变化。合成数据成本低但存在域差距，影响模型在真实场景的应用效果

Method: CFHA框架包含三个阶段：1)全局风格迁移：扩散模型将合成图像的颜色、光照、纹理对齐到真实风格；2)局部细化：超分辨率扩散模型增强小目标（人体）的细节和真实感；3)幻觉去除：过滤视觉属性不符合真实数据的人体实例

Result: 在公开的无人机Sim2Real检测基准测试中，CFHA显著提升检测精度，在Semantic-Drone基准上mAP50提升高达+14.1。消融研究证实了全局和局部阶段的互补作用

Conclusion: CFHA框架有效缩小合成与真实图像间的域差距，保留原始合成标签的同时提升无人机人体检测性能，为低成本标注的合成数据应用提供了可行方案
...
Method: 提出Route-DETR，通过自适应成对路由机制解决查询竞争问题。核心思想是利用查询间相似性、置信度得分和几何信息区分竞争查询（针对同一物体）和互补查询（针对不同物体）。引入双路由机制：抑制路由调节竞争查询间的注意力以减少重复，委托路由鼓励探索不同区域。通过可学习的低秩注意力偏置实现非对称查询交互。采用双分支训练策略，训练时包含路由偏置，推理时保持标准注意力，无额外计算成本。

Result: 在COCO和Cityscapes数据集上的实验表明，Route-DETR在多个DETR基线上取得一致改进。在ResNet-50上相比DINO获得+1.7% mAP提升，在Swin-L上达到57.6% mAP，超越了先前的最先进模型。

Conclusion: Route-DETR通过自适应成对路由有效解决了DETR中的查询竞争问题，提高了检测效率，在多个基准上实现了性能提升，同时保持了推理时的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 临床工作站上实时部署前列腺MRI分割常受计算负载和内存占用限制，深度学习分割方法面临解剖结构变异性挑战，需要在效率和准确性之间取得平衡

Method: 提出KLO-Net，结合动态K-NN注意力机制（自适应确定每个空间位置的注意力连接数）和CSP编码器（减少计算负载和内存消耗）的U-Net架构

Result: 在PROMISE12和PROSTATEx两个公开数据集上进行实验验证，模型在计算效率和分割质量方面具有优势

Conclusion: KLO-Net能够有效解决前列腺MRI分割中的计算效率问题，同时保持可靠的分割准确性，为临床工作站实时部署提供了可行方案
...
TL;DR: 本文提出"去品牌化"新任务，旨在从生成图像中精细移除商标和结构性品牌特征，同时保持语义连贯性，并构建了基准数据集和基于视觉语言模型的评估指标。
...
Result: 随着模型保真度提高（如SDXL、FLUX比Stable Diffusion更容易合成品牌标识），去品牌化挑战的紧迫性更加突出。基于VLM指标的验证结果确认去品牌化是一个独特且具有实际相关性的问题，需要专门技术。

Conclusion: 去品牌化是一个需要专门技术解决的独特实际问题，本文提出的基准数据集和VLM评估方法为相关研究提供了基础，强调了在高质量生成模型中解决品牌保护问题的重要性。
...
TL;DR: FocalComm是一个专注于交换困难实例特征的多智能体协同感知框架，通过渐进式困难实例挖掘和基于查询的特征融合技术，显著提升行人等小目标检测性能。
...
Method: 提出FocalComm框架，包含两个关键设计：1) 可学习的渐进式困难实例挖掘模块提取每个智能体的困难实例特征；2) 基于查询的特征级融合技术，在协作过程中动态加权这些识别出的特征。

Result: 在V2X-Real和DAIR-V2X两个真实世界数据集上，FocalComm在车辆中心和基础设施中心两种协同设置下均优于现有协同感知方法，在V2X-Real数据集上行人检测性能显著提升。

Conclusion: FocalComm通过专注于交换困难实例特征，有效提升了协同感知系统对行人等安全关键小目标的检测能力，为提高自动驾驶安全性提供了新思路。
...
TL;DR: 提出一个将2D扩散模型适配于3D点云补全的框架，通过引入Shape Atlas这一紧凑的2D表示，解决了3D数据稀缺和模态不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 虽然文本到图像的扩散模型在丰富的2D数据上取得了显著成功，但3D扩散模型由于高质量3D数据集稀缺以及3D输入与2D潜在空间之间的模态差距而滞后。需要克服这些限制以实现高质量的3D形状补全。

Method: 引入Shape Atlas，一种紧凑的3D几何2D表示方法，能够充分利用预训练2D扩散模型的生成能力，同时对齐条件输入和输出空间的模态，实现更有效的条件控制。这种统一的2D表述便于从有限的3D数据中学习。

Result: 在PCN和ShapeNet-55数据集上验证了方法的有效性，能够生成高质量、保留细节的形状补全结果。还展示了将补全点云转换为艺术家创建网格的下游应用，进一步证明了方法的实用性。

Conclusion: 提出的框架通过将3D形状补全问题转化为2D表示，成功利用了预训练2D扩散模型的强大生成能力，克服了3D数据稀缺和模态不匹配的限制，实现了高质量的3D形状补全。
...
TL;DR: Sparse-LaViDa通过动态截断冗余掩码token来加速MDM推理，同时引入寄存器token保持生成质量，在LaViDa-O基础上实现2倍加速且质量不降


<details>
  <summary>Details</summary>
Motivation: 掩码离散扩散模型（MDMs）在多模态任务中表现优异，但其推理速度不理想，因为需要在每个采样步骤重复处理冗余的掩码token

Method: 提出Sparse-LaViDa框架：1）动态截断每个推理步骤中不必要的掩码token；2）引入专门的寄存器token作为截断token的紧凑表示；3）设计专门的注意力掩码确保训练与推理一致性

Result: 在LaViDa-O基础上，Sparse-LaViDa在文本到图像生成、图像编辑和数学推理等多样化任务中实现高达2倍的加速，同时保持生成质量

Conclusion: Sparse-LaViDa通过动态token截断和寄存器token机制，在保持MDM生成质量的同时显著提升推理速度，为多模态任务提供了高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有工作在长视频问答中仅通过问答准确率间接评估帧选择质量，缺乏直接评估采样策略的基准。关键帧采样对高效长视频理解至关重要，需要能够直接分析不同采样方法如何在整个长视频中捕获关键内容。
...
Conclusion: KFS-Bench为长视频问答中的关键帧采样提供了首个直接评估基准，揭示了影响采样质量的关键因素，并提出了有效的采样质量指标和自适应平衡采样方法，显著提升了长视频问答的效率和准确性。
...
TL;DR: 本文综述了深度学习在自主机器人场景理解中的应用，涵盖目标检测、语义/实例分割、深度估计、3D重建和视觉SLAM等技术，分析了这些方法如何克服传统几何模型的局限，并探讨了当前问题与未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统几何模型在场景理解中存在局限性，特别是在处理遮挡、无纹理表面和复杂环境时表现不足。深度学习技术为自主机器人提供了更强大的感知能力，能够实时处理复杂场景，提升语义理解和环境认知水平。

Method: 通过综述分析深度学习方法在多个关键领域的应用：1) 目标检测技术；2) 语义分割和实例分割；3) 深度估计方法；4) 3D重建技术；5) 视觉SLAM系统。重点探讨这些技术如何集成到动态非结构化环境中。

Result: 深度学习技术显著提升了自主机器人的场景理解能力，能够更好地处理遮挡、无纹理表面等挑战，实现实时深度感知和语义推理。当这些感知模块集成到动态环境中时，能有效提升决策制定、导航和交互能力。

Conclusion: 深度学习在自主机器人场景理解中展现出巨大潜力，但仍存在需要解决的问题。论文最后指出了当前研究的不足和未来发展方向，为基于学习的场景理解技术提供了研究路线图。
...
TL;DR: CITab是一个新颖的自监督学习框架，通过语义感知的表格建模和原型引导的混合线性层模块，实现了跨表格的多模态医学图像-表格数据表示学习，在阿尔茨海默病诊断任务上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在医学图像和表格数据的多模态学习中存在局限性，主要由于它们对异质表格数据的建模机制过于僵化，导致方法局限于特定数据队列，无法有效学习跨不同队列的可迁移医学知识。

Method: 提出CITab框架：1）从语义感知角度设计表格建模机制，整合列标题作为语义线索；2）提出原型引导的混合线性层模块用于表格特征专业化，有效处理表格数据的异质性并探索底层医学概念。

Result: 在包含4,461名受试者的三个公开数据队列上进行阿尔茨海默病诊断任务评估，实验结果表明CITab优于现有最先进方法，为有效且可扩展的跨表格多模态学习铺平了道路。

Conclusion: CITab通过创新的跨表格自监督学习框架，成功解决了医学图像-表格数据多模态学习中的表格异质性问题，实现了更好的可迁移性和性能表现，为临床决策支持系统提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统结构光方法在像素域进行匹配，在遮挡、精细结构、非朗伯表面等挑战性场景下鲁棒性有限。受神经特征匹配进展启发，希望在特征空间进行更鲁棒的对应匹配

Method: 1) 从投影图案和捕获的红外图像中提取神经特征；2) 在特征空间构建代价体，显式结合几何先验；3) 引入深度细化模块，利用大规模单目深度估计模型的强先验；4) 开发基于物理的结构光渲染流水线，生成近百万合成数据用于训练

Result: 方法在多种结构光图案上表现良好且无需重新训练，在真实室内环境中泛化能力强，性能优于商业结构光系统和被动立体RGB深度估计方法

Conclusion: 基于学习的特征空间匹配方法显著提升了结构光解码的鲁棒性，仅用合成数据训练即可有效处理真实场景，为3D成像提供了新的解决方案
...
TL;DR: 本文提出ASAP Textured Gaussians方法，通过自适应采样和各向异性参数化策略，显著提升带纹理3D高斯泼溅的内存效率与渲染质量平衡。


<details>
  <summary>Details</summary>
Motivation: 现有带纹理的3D高斯泼溅方法存在两个关键问题：1）纹理定义在规范空间中，导致对低贡献区域进行低效采样，浪费纹理容量；2）纹理参数化在所有高斯元素上均匀分配，不考虑视觉复杂度，导致过度参数化。这些问题带来了显著的内存效率挑战。

Method: 提出ASAP Textured Gaussians方法，包含两个简单有效的策略：1）基于高斯密度分布的自适应采样；2）基于渲染误差的各向异性参数化，根据渲染误差分配纹理资源。

Result: ASAP方法显著改善了质量效率的权衡，使用更少的纹理参数实现了高保真渲染。

Conclusion: 通过自适应采样和各向异性参数化策略，ASAP Textured Gaussians有效解决了现有纹理高斯方法的内存效率问题，在保持高质量渲染的同时大幅减少了纹理参数需求。
...
TL;DR: ChartAgent是一个基于工具集成推理的图表理解框架，通过模块化工具库和结构化证据包解决多模态大语言模型在缺少文本标注时的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在图表理解方面严重依赖显式文本标注，当关键数字缺失时性能显著下降。需要一种更稳健的方法来处理稀疏标注情况下的图表分析。

Method: 采用工具集成推理框架，将复杂图表分析分解为可观察、可重放的步骤。构建包含关键元素检测、实例分割、OCR等十多个核心工具的模块化工具库，通过智能体动态编排这些工具，并生成结构化的证据包来支持可追溯的推理过程。

Result: 实验表明，ChartAgent在稀疏标注设置下显著提高了鲁棒性，为可信赖和可扩展的图表理解系统提供了实用路径。

Conclusion: ChartAgent通过工具集成推理和结构化证据包，超越了黑盒范式，为图表理解提供了可追溯、可复现的解决方案，特别是在标注不完整的情况下表现出更强的鲁棒性。
...
TL;DR: OmniDrive-R1是一个用于自动驾驶的端到端视觉语言模型框架，通过交错多模态思维链机制统一感知和推理，采用强化学习驱动的视觉定位能力，无需密集标注即可实现细粒度分析。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自动驾驶等安全关键领域存在可靠性问题，主要是物体幻觉。现有方法存在感知与推理阶段分离、依赖昂贵密集定位标注两大缺陷。

Method: 提出OmniDrive-R1框架，采用交错多模态思维链机制统一感知和推理。核心创新是强化学习驱动的视觉定位能力，通过两阶段强化学习训练流程和Clip-GRPO算法实现，引入无标注的过程式定位奖励。

Result: 在DriveLMM-o1数据集上的实验显示，相比基线Qwen2.5VL-7B，OmniDrive-R1将总体推理分数从51.77%提升到80.35%，最终答案准确率从37.81%提升到73.62%。

Conclusion: OmniDrive-R1通过端到端统一感知和推理、强化学习驱动的视觉定位以及无标注训练，显著提升了自动驾驶场景下视觉语言模型的可靠性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效检测真实场景文本数据集中的标签错误，特别是处理变长序列标签、标签序列不对齐和字符级错误等问题，需要一种更准确实用的解决方案。

Method: 提出SELECT方法，使用图像-文本编码器和字符级分词器处理变长序列标签；引入SSLC（基于相似性的序列标签破坏）过程，在训练时故意引入标签错误，考虑字符视觉相似性并可能导致序列长度变化。
...
Method: 采用图像分块策略限制峰值内存使用；引入视觉分辨率压缩器自适应预测最优编码分辨率以消除冗余计算；提出双重一致性学习技术，在统一框架内对齐多尺度ViT编码器，实现在共享LLM下视觉分支的动态切换。
...
TL;DR: FacEDiT将说话人脸编辑和生成统一为语音条件面部运动填充任务，通过扩散变换器实现局部编辑和生成，同时保持身份一致性和视觉连续性。


<details>
  <summary>Details</summary>
Motivation: 传统上，说话人脸编辑和生成被视为两个独立任务。本文提出将它们统一为语音条件面部运动填充的自监督预训练任务，旨在实现更灵活、连贯的面部动态合成。

Method: 提出FacEDiT模型，基于流匹配训练的语音条件扩散变换器。受掩码自编码器启发，模型学习在周围运动和语音条件下合成被掩码的面部运动。采用偏置注意力和时间平滑约束增强边界连续性和唇部同步。

Result: 实验验证说话人脸编辑和生成可作为语音条件运动填充的子任务。FacEDiT能产生准确、语音对齐的面部编辑，具有强身份保持和视觉连续性，在说话人脸生成任务上也有良好泛化能力。

Conclusion: 通过将说话人脸编辑和生成统一为语音条件面部运动填充任务，FacEDiT实现了灵活的面部动态合成，同时建立了首个说话人脸编辑基准数据集和评估指标。
...
TL;DR: 本文提出了一种基于多模态深度学习的实时室内工作面照度预测框架，通过仅从侧窗区域提取图像特征，可在动态占用空间中应用，实现了高精度预测和良好的时间泛化能力。


<details>
  <summary>Details</summary>
Motivation: 日光联动控制（DLCs）在建筑节能方面具有巨大潜力，但现有室内日光预测研究多针对静态场景开发。本研究旨在开发一种能够实时预测动态占用空间中工作面照度分布的方法。

Method: 提出多模态深度学习框架，从非侵入性图像中提取时空特征预测室内工作面照度分布。创新之处在于仅从侧窗区域提取图像特征而非室内像素，使方法适用于动态占用空间。在广州的测试房间进行了现场实验，收集了17,344个样本用于模型训练和验证。

Result: 模型在同分布测试集上达到R2 > 0.98，RMSE < 0.14的高精度；在未见天测试集上达到R2 > 0.82，RMSE < 0.17，显示出良好的时间泛化能力。

Conclusion: 该多模态深度学习框架能够准确预测动态占用空间中的室内工作面照度分布，为日光联动控制系统的实时优化提供了有效工具，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在场景外观重建方面表现良好，但缺乏底层结构表示（如植物分支模式），限制了其在植物表型分析等任务中的应用

Method: 引入分层3DGS表示GaussianPlant，使用结构基元（StPs）显式表示枝干和叶片几何形状（圆柱体和圆盘），外观基元（ApPs）表示植物外观，两者通过绑定对应信息联合优化
...
TL;DR: 提出一个预测性、质量感知的呼吸率估计框架，通过整合多种信号源和动态可靠性评估，实现自适应信号融合和质量过滤，在多个数据集上优于单一方法。


<details>
  <summary>Details</summary>
Motivation: 视频呼吸率估计常因不同提取方法信号质量不一致而不可靠，需要一种能整合异质信号源并动态评估可靠性的框架来提高估计准确性。

Method: 从面部远程光电容积描记、上半身运动和深度学习管道中提取10种信号，使用四种频谱估计器分析，通过片段级质量指标训练机器学习模型预测准确性或选择最可靠信号，实现自适应信号融合和质量过滤。
...
TL;DR: OUSAC框架通过优化引导调度和自适应缓存，在保持生成质量的同时显著加速扩散模型，减少高达82%的无条件前向计算，实现53-60%的计算节省和5倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但计算成本高昂，特别是Classifier-Free Guidance需要双倍计算。现有缓存方法在动态引导条件下失效，需要新的优化方案来平衡计算效率和生成质量。

Method: 采用两阶段方法：第一阶段使用进化算法联合优化跳过的时间步和引导尺度，减少无条件前向计算；第二阶段引入自适应秩分配，针对不同Transformer块在动态条件下的影响进行校准，保持缓存有效性。

Result: 在多个模型上取得显著效果：DiT-XL/2节省53%计算同时质量提升15%；PixArt-alpha节省60%计算同时质量提升16.1%；FLUX实现5倍加速并提升CLIP分数。

Conclusion: OUSAC通过系统优化引导调度和自适应缓存，在保持甚至提升生成质量的前提下，显著降低了扩散模型的计算成本，为高效高质量图像生成提供了实用解决方案。
...
Result: 在GSO和3D-FUTURE数据集上PSNR、SSIM和LPIPS指标平均排名第一，证明离散扩散是多视角生成的可行且简单的替代方案

Conclusion: 离散扩散模型为多视角图像生成提供了可行且简单的替代方案，通过视觉token化和掩码预测实现跨视角一致性，无需复杂3D几何约束或专用注意力架构
...
TL;DR: RUNE是一个结合大型语言模型和神经符号AI的遥感图像检索方法，通过显式推理而非隐式嵌入来提升复杂空间关系处理能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有遥感大型视觉语言模型在可解释性和复杂空间关系处理方面存在局限，限制了实际应用。需要一种能进行显式推理、提升性能和可解释性的方法。

Method: RUNE将文本查询转换为一阶逻辑表达式，结合神经符号推理模块，通过检测到的实体与逻辑表达式的兼容性进行图像检索。采用逻辑分解策略在检测实体的条件子集上操作，保证执行时间更短。

Result: 在重新利用的DOTA数据集上，RUNE在复杂遥感检索任务中优于最先进的RS-LVLMs，在性能、鲁棒性和可解释性方面均有提升。提出的RRQC和RRIU指标有效评估了查询复杂性和图像不确定性下的检索鲁棒性。

Conclusion: RUNE通过神经符号推理方法显著提升了遥感图像检索的性能和可解释性，在洪水后卫星图像检索等实际应用中展现出潜力，为复杂空间关系处理提供了有效解决方案。
...
Method: 通过利用多模态零空间，协同集成文本提示和从CLIP联合嵌入空间合成的视觉原型，高效移除不需要的类别信息，同时保留剩余知识。

Result: 该方法克服了现有基于重新训练方法的限制，提供了一种灵活且计算效率高的受控模型遗忘解决方案。
...
Method: 提出MFE-GAN框架，包含多尺度特征提取模块，采用Haar小波变换和归一化处理文档图像，然后输入GAN训练。设计了新的生成器、判别器和损失函数。

Result: 在Benchmark、Nabuco和CMATERdb数据集上的实验表明，MFE-GAN显著减少了总训练和推理时间，同时保持了与SOTA方法相当的性能。

Conclusion: MFE-GAN是一个高效的基于GAN的文档图像增强和二值化框架，通过多尺度特征提取显著减少了时间成本，为OCR系统提供了更高效的预处理方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖离散跟踪或视角相关特征，难以在时空维度上保持一致的实例表示。需要一种能够解耦可见性和持久对象身份的连续表示方法。

Method: 基于可变形3D高斯构建实例嵌入表示，联合编码辐射和语义信息；引入高斯身份校准机制和向语义活跃区域重采样策略；通过可微分光栅化从RGB图像和实例掩码直接学习。

Result: 在HyperNeRF和Neu3D数据集上，该方法在新型视角全景分割和开放词汇4D查询任务上显著优于现有最先进方法。

Conclusion: CIF提供了一种连续概率时空表示，能够实现跨时空一致的实例理解，为动态场景分析提供了有效的解决方案。
...
Method: 利用零空间投影技术，计算目标文本嵌入张成的子空间的正交基，然后将这些方向投影到零空间，从而消除最终投影层中嵌入的目标类别信息。

Result: 即使部分投影也能在完全遗忘和保留有用信息之间取得平衡，显著降低目标类别的零样本性能，同时保持模型的整体多模态知识。

Conclusion: 该方法计算高效且精确，为多模态模型的选择性遗忘提供了一种新颖的闭式解决方案，解决了模型去污染和隐私保护的关键挑战。
...
Method: 1) 开发可控数据生成管道：从无属性基础草图构建属性添加序列，通过跨序列采样形成多步编辑链，使用风格保持属性移除模型扩展风格覆盖。2) 基于DiT的统一草图编辑框架：重新利用RGB通道编码输入，实现指令引导编辑和线条引导重绘的切换。3) 任务引导的专家混合集成到LoRA层：通过文本和视觉线索路由，提高语义可控性、结构保真度和风格保持。

Result: 在两项任务上均取得最先进的结果，与近期基线相比，在指令遵循、风格/结构保持方面表现更优。数据集和SketchAssist为草图创作和修订提供了实用、可控的助手。
...
TL;DR: 该论文提出了首个用于评估和改进机器学习模型检测性能反模式能力的基准数据集，包含600多个PyTorch跟踪数据，并提出了一种轻量级ML模型与LLM结合的迭代方法。


<details>
  <summary>Details</summary>
Motivation: 识别和解决机器学习模型中的性能反模式对高效训练和推理至关重要，但目前需要深厚的专业知识，包括系统基础设施、ML模型和内核开发。大型科技公司依赖专门的ML基础设施工程师分析torch跟踪和基准测试，但这种资源密集型工作流程对普通计算机视觉研究人员来说难以获得。其中，在冗长的执行跟踪中定位有问题的跟踪段是最耗时的任务，且难以用当前的ML模型（包括LLM）自动化。

Method: 提出了一个新颖的迭代方法：首先使用轻量级ML模型检测具有反模式的跟踪段，然后使用大型语言模型（LLM）进行细粒度分类和针对性反馈。还创建了包含600多个PyTorch跟踪的基准数据集，涵盖分类、检测、分割和生成等多种计算机视觉模型，并在多个硬件平台上收集。

Result: 实验结果表明，该方法在检测反模式区域方面显著优于无监督聚类和基于规则的统计技术。该方法还有效地补偿了LLM有限的上下文长度和推理效率问题。

Conclusion: 该工作为自动化检测ML性能反模式提供了有效的解决方案，通过结合轻量级ML模型和LLM的迭代方法，解决了当前方法在定位问题跟踪段方面的局限性，为计算机视觉研究人员提供了更易访问的性能分析工具。
...
TL;DR: CIS-BA是一种针对目标检测系统的新型后门攻击范式，通过连续物体交互模式而非静态特征设计触发器，实现多触发器-多对象攻击，在复杂环境中保持高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击方法依赖单一触发器-单一对象映射和脆弱的像素级线索，在能力和鲁棒性上存在固有局限。自动驾驶等现实应用中的目标检测系统面临严重后门攻击威胁，需要更强大和鲁棒的攻击方法。

Method: 提出CIS-BA范式，将触发器设计从静态物体特征转向连续物体交互模式。通过CIS-Frame框架实现：1）通过交互分析构建空间触发器；2）将其形式化为类别-几何约束用于样本污染；3）在检测器训练期间嵌入后门。支持单对象攻击（错误分类和消失）和多对象同时攻击。

Result: 在MS-COCO和真实世界视频上的实验表明：在复杂环境下攻击成功率超过97%；在动态多触发器条件下保持超过95%的有效性；能够规避三种最先进的防御方法。

Conclusion: CIS-BA扩展了交互密集型场景中后门攻击的格局，为目标检测系统的安全性提供了新的见解。该方法通过连续交互空间建模，实现了鲁棒的多触发器-多对象攻击机制。
...
TL;DR: Fast3DHPE是一个用于单目3D人体姿态估计的统一框架，旨在标准化训练和评估协议，实现公平比较并提高训练效率。在该框架中提出的FastDDHPose方法使用解耦扩散模型，分别建模骨骼长度和方向分布，避免层次误差累积，取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D人体姿态估计方法通常在分散的框架下训练和评估，缺乏统一的公平比较标准。这导致方法之间的比较不公平，且训练效率低下。为了解决这些问题，需要建立一个标准化的框架来促进方法的快速复现和灵活开发。

Method: 提出了Fast3DHPE统一框架，标准化训练和评估协议。在该框架中开发了FastDDHPose方法，采用解耦扩散模型分别建模骨骼长度和方向分布，避免层次误差累积。设计了高效的"运动学-层次空间和时间去噪器"，使模型专注于关节层次结构，避免过度复杂的关节拓扑建模。

Result: 在Human3.6M和MPI-INF-3DHP数据集上的大量实验表明，Fast3DHPE框架能够公平比较所有方法，并显著提高训练效率。在该统一框架下，FastDDHPose方法在野外场景中表现出强大的泛化能力和鲁棒性，达到了最先进的性能。

Conclusion: Fast3DHPE为单目3D人体姿态估计提供了一个标准化框架，实现了方法的公平比较和高效训练。FastDDHPose方法通过解耦扩散模型和层次感知设计，在统一框架下取得了最先进的性能，展示了良好的泛化能力和鲁棒性。
...
Method: 提出语义高斯过程不确定性（SGPU）框架：将生成的答案映射到密集语义空间，计算其嵌入的Gram矩阵，通过特征谱总结语义配置，然后将这种谱表示输入高斯过程分类器，学习将语义一致性模式映射到预测不确定性，可在黑盒和白盒设置中应用。
...
Conclusion: SGPU通过分析答案嵌入的几何结构来量化语义不确定性，避免了脆弱的聚类方法，提供了一种更稳健的不确定性估计框架，能够跨模型和模态迁移，为大型视觉语言模型的可靠性评估提供了有效解决方案。
...
TL;DR: 该论文提出使用球面Voronoi作为3D高斯泼溅中外观表示的统一框架，解决了传统球谐函数在高频信号、吉布斯振铃和镜面反射方面的局限性。
...
Method: 提出球面Voronoi作为3D高斯泼溅中外观表示的统一框架。SV将方向域划分为具有平滑边界的可学习区域，为视角相关效果提供直观稳定的参数化。对于漫反射外观，SV保持比现有替代方案更简单的优化；对于反射效果，将SV用作可学习的反射探针，遵循经典图形学原理以反射方向作为输入。

Result: 在合成和真实世界数据集上取得了最先进的结果。对于漫反射外观，SV实现了具有竞争力的结果同时保持优化简单；对于反射效果（SH失败的地方），SV作为可学习反射探针的表述达到了最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 15-45%的儿童在成长过程中会经历骨折，准确诊断至关重要。骨折形态是关键的诊断特征，需要自动化的分析方法。

Method: 通过自动为骨折边界框分配全局AO编码来提取骨折形态，将全局多标签任务重新定义为局部多分类任务，利用公开数据集。

Result: 该方法将平均F1分数提高了7.89%，但使用不完美的骨折检测器时性能会下降，突显了实际部署的挑战。

Conclusion: 提出的方法能有效提取骨折形态并提升分类性能，但实际应用需要更稳健的骨折检测器。代码已在GitHub上开源。
...
TL;DR: SkyLume是一个专为研究光照鲁棒3D重建而设计的大规模真实世界无人机数据集，包含10个城市区域的10万+高分辨率图像，每个区域在一天三个时段拍摄，提供LiDAR扫描和3D真值，并提出了用于评估光照-材质解耦鲁棒性的时间一致性系数（TCC）指标。
...
Method: 提出了SkyLume数据集：1）收集10个城市区域的10万+高分辨率无人机图像（四个倾斜视角和天底视角），每个区域在一天三个时段拍摄以系统隔离光照变化；2）提供每场景的LiDAR扫描和精确3D真值，用于评估深度、表面法线和不同光照下的重建质量；3）提出时间一致性系数（TCC）指标，用于评估跨时间反照率稳定性和光照-材质解耦的鲁棒性。

Result: 创建了一个专门用于研究光照鲁棒3D重建的大规模真实世界无人机数据集，填补了该领域的数据空白。该数据集系统捕捉了不同光照条件下的同一区域，提供了精确的几何和外观评估基准，并提出了专门用于评估逆渲染任务鲁棒性的新指标。

Conclusion: SkyLume数据集为大规模逆渲染、几何重建和新视角合成的研究和实际评估提供了基础资源，有望推动光照鲁棒3D重建技术的发展，特别是在处理多时相无人机数据时的光照不一致问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在机器人操作中可控性有限，基于轨迹条件的视频生成方法多依赖2D轨迹或单模态条件，难以产生可控且一致的机器人演示。
...
Conclusion: DRAW2ACT通过深度感知的轨迹条件视频生成和多模态策略学习，显著提升了机器人操作任务中视频生成的可控性和一致性，为具身AI提供了更强大的仿真器。
...
TL;DR: OminiGen：统一框架生成对齐的多模态传感器数据，通过共享BEV空间和新型UAE解码方法实现LiDAR和多视角相机数据的联合生成


<details>
  <summary>Details</summary>
Motivation: 自动驾驶依赖大量真实世界数据收集，但获取多样性和极端案例数据成本高、效率低。现有生成方法主要关注单模态生成，导致多模态传感器数据效率低下和对齐问题。
...
TL;DR: 本研究提出了一种基于多视角MRI和深度学习的非侵入性方法，用于检测胶质母细胞瘤患者的MGMT启动子甲基化状态，避免了传统脑组织活检的侵入性。


<details>
  <summary>Details</summary>
Motivation: MGMT启动子甲基化状态对胶质母细胞瘤化疗效果有重要影响，但目前的检测方法需要侵入性的脑肿瘤组织活检。需要开发非侵入性的检测方法来改善患者护理和精准医疗。

Method: 采用放射基因组学方法，使用MRI扫描和深度学习模型，提出了一种新的多视角方法，考虑MRI视图间的空间关系来检测MGMT甲基化状态。该方法从三个视图提取信息而不使用复杂的3D深度学习模型，避免了高参数、收敛慢和内存需求大的问题。还引入了新的肿瘤切片提取技术。

Result: 通过多评价指标验证，新方法在肿瘤切片提取方面优于现有方法。与最先进模型比较，证明了该方法的有效性。研究还提供了可复现的模型流程。

Conclusion: 该研究突出了非侵入性方法识别MGMT启动子甲基化的潜力，有助于推进胶质母细胞瘤治疗中的精准医疗。方法避免了3D模型的复杂性，同时保持了检测准确性。
...
TL;DR: ViBES是一个对话式3D智能体，能够联合规划语言和动作，通过混合模态专家架构处理语音、面部表情和身体动作，实现可控的社交互动。


<details>
  <summary>Details</summary>
Motivation: 现有系统将人类行为建模为翻译任务（如语音到手势或文本到动作），缺乏对何时移动、做什么以及如何在多轮对话中适应的智能决策，导致时序脆弱、社交基础薄弱以及语音、文本和动作的孤立处理。

Method: 采用混合模态专家架构，包含语音、面部表情和身体动作的模态分区Transformer专家，通过硬路由按模态处理交错的多模态令牌流，并通过跨专家注意力共享信息，利用预训练的语音语言模型支持混合主动交互。

Result: 在多轮对话基准测试中，在对话-动作对齐和行为质量方面表现出优于现有语音条件动作生成和文本到动作基线的性能，实现了可控的社交能力3D交互。
...
TL;DR: 提出4D-RaDiff框架，通过潜在空间扩散生成4D雷达点云数据，解决雷达数据标注不足问题，提升目标检测性能
...
Method: 提出4D-RaDiff框架，在潜在点云表示上应用扩散模型，考虑雷达点云的稀疏性和独特特性，支持对象级和场景级条件控制生成

Result: 合成雷达数据作为数据增强方法能持续提升目标检测性能；预训练使用合成数据可将所需标注雷达数据减少90%同时保持可比性能

Conclusion: 4D-RaDiff能有效生成高质量雷达点云数据，解决数据稀缺问题，显著提升雷达目标检测系统的训练效率和性能
...
TL;DR: 该论文提出了一种新的图像免疫方法SIFM，通过协同扰动扩散模型中间特征来防止恶意编辑，并首次引入ISR指标来严格量化免疫效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本引导图像编辑技术虽然强大，但存在被滥用的风险。当前评估图像免疫效果的指标主要关注保护图像与原始图像编辑输出的视觉差异，而忽视了免疫的核心要求——破坏与攻击者意图的语义对齐。

Method: 提出协同中间特征操纵（SIFM）方法，通过双重协同目标策略性地扰动扩散模型的中间特征：1）最大化特征与原始编辑轨迹的差异以破坏与预期编辑的语义对齐；2）最小化特征范数以诱导感知退化。

Result: 广泛的实验表明，SIFM在保护视觉内容免受基于扩散的恶意操纵方面达到了最先进的性能。新提出的ISR指标首次能够严格量化真正的免疫效果。

Conclusion: 该研究重新定义了图像免疫的成功标准，强调应关注编辑输出是否与提示语义不匹配或遭受显著感知退化，从而有效阻止恶意意图。SIFM方法和ISR指标为图像安全防护提供了新的有效工具。
...
TL;DR: Elastic3D是一种基于条件潜在扩散的端到端方法，可将单目视频升级为立体视频，避免显式深度估计和扭曲带来的伪影，通过引导VAE解码器确保高质量立体输出，并提供用户可调节的视差范围控制。


<details>
  <summary>Details</summary>
Motivation: 随着沉浸式3D内容需求增长，需要自动化的单目到立体视频转换方法。传统方法依赖显式深度估计和扭曲会产生伪影，需要更高质量、可控的解决方案。

Method: 基于条件潜在扩散的端到端方法，避免显式深度估计和扭曲。核心创新是引导VAE解码器，确保锐利且极线一致的立体视频输出。提供标量调节旋钮，让用户在推理时控制立体效果强度（视差范围）。

Result: 在三个真实世界立体视频数据集上的实验表明，该方法优于传统的基于扭曲方法和最近的免扭曲基线方法，为可靠、可控的立体视频转换设定了新标准。

Conclusion: Elastic3D提供了一种高质量、可控的端到端单目到立体视频转换方法，通过引导VAE解码器和用户可调节的视差控制，实现了优于现有方法的性能。
...
TL;DR: EVPG通过构建概率图将不可微的视觉编程执行过程转化为可微的概率推理，实现了视觉编程框架的端到端梯度优化，显著提升了复杂视觉推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编程方法主要关注提升大语言模型生成的视觉程序质量，但忽略了优化视觉编程调用的预训练模型。由于视觉编程不可微且只有最终任务标签，无法直接使用梯度优化方法进行端到端学习。
...
Conclusion: 通过将视觉编程执行过程转化为可微概率推理，EVPG实现了视觉编程框架的端到端梯度优化，为视觉编程在复杂视觉推理任务中的应用提供了新的优化途径。
...
TL;DR: 提出DANP免疫方法，通过双注意力引导的噪声扰动来防止文本到图像扩散模型的恶意编辑


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然提升了图像编辑能力，但也带来了伦理挑战，可能被滥用于创建欺骗性或有害内容。现有防御方法通过嵌入不可感知扰动来降低风险，但对恶意篡改的防御效果有限。

Method: 提出双注意力引导噪声扰动(DANP)免疫方法，在多时间步上操作，通过动态阈值生成掩码识别文本相关和不相关区域，减少相关区域注意力同时增加不相关区域注意力，误导编辑到错误区域并保护目标。同时最大化注入噪声与模型预测噪声之间的差异来干扰生成过程。

Result: DANP在对抗恶意编辑方面表现出优异的免疫能力，大量实验证实该方法达到了最先进的性能水平。

Conclusion: 通过同时针对注意力和噪声预测机制，DANP方法能有效防御文本到图像扩散模型的恶意编辑，为保护图像内容提供了有效的技术解决方案。
...
TL;DR: 本文提出了DriverGaze360数据集和DriverGaze360-Net模型，用于360度全景驾驶员注意力预测，解决了现有方法视野狭窄和驾驶场景多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶员注意力预测方法受限于狭窄的前方视野和有限的驾驶场景多样性，无法捕捉完整的驾驶环境空间上下文，特别是在变道、转弯和涉及行人、自行车等周边物体交互的场景中。

Method: 提出了DriverGaze360数据集（包含约100万帧带注视标签的360度视野数据，来自19名驾驶员），以及DriverGaze360-Net全景注意力预测模型，该模型通过辅助语义分割头联合学习注意力图和被关注物体。
...
TL;DR: TDAE提出了一种双模态框架，通过协调图像-文本优化增强图像对恶意编辑的免疫力，在跨模型评估中实现可迁移的防御效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在对抗扩散模型恶意编辑时，存在跨模型可迁移性有限的问题，需要开发更有效的防御机制。

Method: 提出TDAE双模态框架：视觉层面引入FlatGrad防御机制（FDM），通过梯度正则化将扰动导向平坦最小值；文本层面提出动态提示防御（DPD），周期性优化文本嵌入，使免疫图像与原始图像的编辑结果对齐。

Result: 实验结果表明，TDAE在内部和跨模型评估中都达到了最先进的性能，能有效缓解恶意编辑。

Conclusion: TDAE通过协调的图像-文本优化实现了可迁移的防御，为对抗扩散模型的恶意编辑提供了有效的解决方案。
...
TL;DR: Zoom-Zero框架通过粗到细的定位策略提升视频问答的时序定位能力，使用放大精度奖励和令牌选择性信用分配解决现有方法的局限性。
...
Method: 提出Zoom-Zero粗到细框架：1) 首先定位查询相关片段；2) 然后放大到最显著帧进行细粒度视觉验证。包含两个关键创新：放大精度奖励验证时序定位预测的保真度；令牌选择性信用分配将奖励归因于负责时序定位或答案生成的令牌。

Result: 在NExT-GQA上时序定位提升5.2%，在ReXTime上提升4.6%；平均答案准确率提升2.4%；推理时的粗到细放大对长视频理解带来6.4%的平均改进。

Conclusion: Zoom-Zero框架通过粗到细的定位策略和创新的奖励机制，显著提升了基于视频问答的时序定位能力和答案准确性，特别在长视频理解方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在多模态任务中表现出色，但在图像描述评估方面，特别是在领域偏移场景下，仍然面临鲁棒性挑战。需要一种能够跨不同领域生成与人类判断一致的评价分数的方法。

Method: 提出了分布感知分数解码器(DISCODE)，这是一种无需微调的方法，核心是测试时自适应评估方法，引入了自适应测试时(ATT)损失，利用高斯先验分布来提高评估分数估计的鲁棒性。该损失通过推导出的解析解在测试时高效最小化。

Result: 在提出的多域描述评估(MCEval)基准(涵盖六个不同领域)和四个现有代表性基准上，DISCODE作为无参考评估指标取得了最先进的性能。

Conclusion: DISCODE通过测试时自适应评估方法，在跨域图像描述评估中实现了更好的鲁棒性和与人类判断的一致性，为图像描述评估提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过优化3D或视频生成模型来构建4D表示，缺乏原生4D生成能力。为了解决4D训练数据稀缺问题，同时实现高质量的时间一致性和结构一致性，需要开发专门的4D生成模型

Method: 基于预训练的单图像到3D模型构建空间一致性；引入专门的时间层确保跨帧一致性；使用因子化4D卷积和时间下采样块压缩潜在序列；采用精心设计的训练策略增强对遮挡的鲁棒性

Result: SS4D能够直接从单目视频合成高质量的动态3D物体，实现了高保真度、时间一致性和结构一致性，同时支持长视频序列的高效训练和推理

Conclusion: SS4D展示了原生4D生成模型的可行性，通过结构化时空潜在表示和专门的设计，有效解决了4D数据稀缺和时空一致性问题，为动态3D内容生成提供了新方法
...
TL;DR: 该论文发布了一个新的隧道衬砌缺陷数据集，包含三种不同类型隧道中裂缝、渗漏和水渗透等典型缺陷的标注图像，旨在支持深度学习方法的自动化隧道检测。
...
Method: 创建并公开一个包含三种不同隧道衬砌的标注图像数据集，涵盖裂缝、渗漏和水渗透等典型缺陷。数据集设计支持监督、半监督和无监督深度学习方法，用于缺陷检测和分割。

Result: 发布了一个新的公开可用隧道缺陷数据集，其多样化的纹理和施工技术使得能够研究模型在不同隧道类型间的泛化能力和可迁移性。
...
TL;DR: CAPRMIL提出了一种新的多实例学习框架，通过全局上下文感知的补丁嵌入简化MIL聚合器，在计算病理学中实现高效且高性能的整张切片分析。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，由于全切片图像（WSI）的千兆像素尺度和像素级标注稀缺，弱监督已成为深度学习的标准方法。传统MIL方法依赖复杂的基于注意力的聚合机制，计算复杂度高且参数众多。本文旨在简化MIL框架，通过改进补丁表示学习来降低模型复杂度。

Method: CAPRMIL采用聚合器无关的框架，将补丁特征投影到少量全局上下文/形态感知的token中，使用多头自注意力机制以线性计算复杂度注入全局上下文。该方法使用冻结的补丁编码器提取特征，然后与简单的Mean MIL聚合器配对，避免了复杂的相关性学习。

Result: 在多个公共病理学基准测试中，CAPRMIL达到了最先进的切片级性能，同时将可训练参数减少了48%-92.8%，推理时的FLOPs降低了52%-99%，在GPU内存效率和训练时间方面表现最佳。

Conclusion: 在聚合之前学习丰富的上下文感知实例表示是复杂池化方法的一个有效且可扩展的替代方案，为全切片分析提供了更高效的MIL框架。
...
TL;DR: 该论文提出了一个通过语义结构恢复来改进SVG动画生成的框架，解决了当前视觉语言模型在处理SVG动画时因图形元素碎片化而导致的语义连贯性问题。


<details>
  <summary>Details</summary>
Motivation: 随着网页环境日益动态化，SVG动画需求不断增长，但当前视觉语言模型在自动化SVG动画生成方面仍面临挑战。主要问题是SVG中视觉连贯的部分通常被分割成低级形状元素，这些元素无法提供哪些部分应该一起移动的语义指导，导致模型难以生成连贯的动画效果。

Method: 提出一个框架，通过统计聚合多个弱部件预测来恢复SVG的语义结构。该方法从噪声预测中稳定推断语义信息，将SVG重新组织为语义分组，从而为视觉语言模型提供更好的语义指导，使其能够生成更连贯的动画。

Result: 实验表明，该方法相比现有方法取得了显著提升，证明了语义结构恢复是解锁稳健SVG动画生成的关键步骤，同时支持视觉语言模型与矢量图形之间更可解释的交互。

Conclusion: 语义恢复是当前VLM系统在SVG动画生成中缺失的关键层，通过恢复SVG的语义结构，可以显著提高动画生成的连贯性和可靠性，为VLM与矢量图形的交互提供更好的支持。
...
Method: 提出CLNet框架，包含三个可学习的互补模块：1) 神经对应图(NCM)通过潜在对应场空间对齐跨视图特征；2) 非线性嵌入转换器(NEC)使用MLP变换跨视角重映射特征；3) 全局特征重校准(GFR)通过学习到的空间线索重新加权信息丰富的特征通道。

Result: 在CVUSA、CVACT、VIGOR和University-1652四个公开基准测试上，CLNet实现了最先进的性能，同时提供更好的可解释性和泛化能力。

Conclusion: CLNet通过显式建模跨视图空间对应关系，有效弥合了不同视角间的语义和几何差距，能够联合捕获高级语义和细粒度对齐，为跨视角地理定位提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的动态新视角合成方法存在模型复杂度过高、参数冗余严重的问题，导致模型体积大、渲染速度慢，难以在资源受限设备上实现实时应用

Method: 提出Hybrid Gaussian Splatting (HGS)框架，采用静态-动态分解策略，对动态区域使用时变径向基函数建模，对静态区域共享时不变参数以减少冗余，并引入两阶段训练策略增强时间一致性

Result: 模型大小减少98%，在RTX 3090上实现4K分辨率125 FPS实时渲染，在RTX 3050上实现1352×1014分辨率160 FPS，已集成到VR系统，渲染质量与SOTA方法相当，高频细节和场景突变视觉保真度显著提升

Conclusion: HGS通过高效的静态-动态分解策略，在保持高质量渲染的同时大幅减少参数冗余，实现了动态新视角合成的实时高效渲染，为资源受限设备上的沉浸式体验应用提供了可行方案
...
Conclusion: FakeRadar通过主动探测特征空间和合成异常值样本来模拟新兴伪造伪影，显著提升了深度伪造检测的跨域泛化能力，为解决现实场景中不断演变的伪造技术提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 小城市在气候适应方面面临人员资源有限和整合多源数据的挑战，需要更高效的工具来制定全面的气候适应策略。

Method: 开发了一个多层系统，结合专业大型语言模型（LLMs）、卫星影像分析和知识库，为小城市提供综合的气候适应分析支持。

Result: 提出了一个名为EcoScapes的系统，相关代码已在GitHub上开源，旨在帮助小城市更有效地制定气候适应策略。

Conclusion: 该多层系统能够帮助资源有限的小城市克服数据整合和分析的障碍，为制定有效的气候适应策略提供技术支持。
...
TL;DR: 该研究首次建立了多中心淋巴瘤分型基准数据集，评估了五种病理学基础模型与两种聚合方法在不同放大倍数下的性能，发现模型在分布内测试集上表现良好（>80%），但在分布外测试集上泛化能力显著下降（~60%）。


<details>
  <summary>Details</summary>
Motivation: 淋巴瘤的及时准确诊断对癌症治疗至关重要，但标准诊断流程需要多种昂贵设备和专业人员，导致治疗延迟。深度学习可以从常规HE染色切片中提取诊断信息，但目前缺乏基于多中心数据的淋巴瘤分型综合基准。

Method: 研究创建了首个覆盖四种常见淋巴瘤亚型和健康组织的多中心淋巴瘤基准数据集。系统评估了五种公开可用的病理学基础模型（H-optimus-1, H0-mini, Virchow2, UNI2, Titan），结合基于注意力的AB-MIL和基于transformer的TransMIL两种多实例学习聚合器，在三个放大倍数（10x, 20x, 40x）下进行测试。
...
TL;DR: ExpanDyNeRF：一种利用高斯溅射先验和伪真值生成策略的单目NeRF框架，能够在极端视角变化下实现高质量动态场景渲染，并提出了首个合成动态多视角数据集SynDM。


<details>
  <summary>Details</summary>
Motivation: 现有动态NeRF方法在大视角偏差下渲染效果不稳定且不真实，需要提升动态场景在极端视角变化下的渲染质量。

Method: 提出ExpanDyNeRF框架，结合高斯溅射先验和伪真值生成策略，优化密度和颜色特征以改善从挑战性视角的场景重建。同时创建了SynDM数据集，这是首个具有显式侧视监督的合成动态多视角数据集。

Result: 在SynDM和真实世界数据集上的定量和定性结果表明，ExpanDyNeRF在极端视角偏移下的渲染保真度显著优于现有动态NeRF方法。

Conclusion: ExpanDyNeRF通过创新的先验利用和数据集构建，成功解决了动态NeRF在大视角变化下的渲染质量问题，为动态场景的新视角合成提供了有效解决方案。
...
TL;DR: SynPS是一种无需训练的图像编辑方法，通过协同利用位置嵌入和语义信息来解决非刚性编辑中的注意力崩溃问题，实现更忠实的内容编辑。
...
Method: 提出SynPS方法，首先设计编辑度量来量化每个去噪步骤所需的编辑幅度，然后设计注意力协同管道，动态调节位置嵌入的影响，平衡语义修改和保真度保持，通过自适应整合位置和语义线索来避免过度编辑和编辑不足。

Result: 在公共和新构建的基准测试上进行广泛实验，证明了该方法在性能和忠实度方面的优越表现。

Conclusion: SynPS通过协同利用位置嵌入和语义信息，有效解决了非刚性图像编辑中的注意力崩溃问题，实现了更忠实、更平衡的图像编辑效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于消息传递的压缩成像方法通常使用现成的图像去噪器，这些去噪器依赖通用或手工设计的先验，难以准确捕捉自然图像的复杂统计结构，导致在高度欠定情况下的重建效果不理想。虽然基于分数的生成模型能准确表征复杂图像分布，但直接用于后验采样计算复杂度极高。

Method: 通过利用分数生成建模与经验贝叶斯去噪之间的紧密联系，设计了一个消息传递框架，集成了基于分数的最小均方误差（MMSE）去噪器用于压缩图像恢复。该方法称为基于分数的涡轮消息传递（STMP）。对于量化测量系统，进一步提出了量化STMP（Q-STMP），在STMP基础上增加了分量级MMSE去量化模块。

Result: STMP和Q-STMP的渐近性能可以通过一组状态演化（SE）方程准确预测。在FFHQ数据集上的实验表明，STMP相比竞争基线方法在性能-复杂度权衡方面显著更优，Q-STMP即使在1比特量化下仍保持鲁棒性。两种方法通常都能在10次迭代内收敛。

Conclusion: 该研究成功地将基于分数的生成模型先验与消息传递框架相结合，实现了快速收敛和高表达能力的平衡，为压缩图像恢复提供了一种高效且性能优越的解决方案，特别是在高度欠定和量化测量场景下表现出色。
...
TL;DR: 提出了一种仅使用真实视频数据进行无监督视频实例分割的方法，通过识别高质量关键掩码并利用稀疏到密集的蒸馏训练，显著提升了分割性能。
...
Method: 1. 从单帧无监督实例分割开始；2. 利用深度运动先验识别视频中的高质量关键掩码；3. 提出稀疏到密集蒸馏方法，使用稀疏关键掩码伪标注训练分割模型进行隐式掩码传播；4. 引入时序DropLoss辅助训练；5. 在生成的密集标签集上训练最终模型。

Result: 该方法在多个基准测试中超越了当前最先进的无监督视频实例分割方法。

Conclusion: 通过仅使用真实视频数据并建立时序一致性，该方法能够更好地建模真实视频中的复杂运动，实现了更准确的无监督视频实例分割。
...
TL;DR: SignIT是一个新的意大利手语识别数据集，包含644个视频（3.33小时），标注了94个手语类别，属于5个宏观类别，并提取了2D关键点。提出了基准测试，评估了多种SOTA模型，展示了时间信息、关键点和RGB帧对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏专门的意大利手语（LIS）识别数据集，需要为手语识别研究提供标准化的数据集和基准测试，以推动该领域的发展。

Method: 创建了包含644个视频的SignIT数据集，手动标注了94个手语类别（分为5个宏观类别），提取了用户手部、面部和身体的2D关键点。使用多种最先进模型进行基准测试，分析时间信息、2D关键点和RGB帧对识别性能的影响。

Result: 结果显示现有模型在这个具有挑战性的LIS数据集上存在局限性，不同输入特征（时间信息、关键点、RGB帧）对模型性能有显著影响。

Conclusion: SignIT数据集为意大利手语识别研究提供了有价值的资源，基准测试揭示了当前模型的局限性，为未来改进提供了方向。
...
TL;DR: ReVision是一个从大规模远程医疗项目中学习临床图像解释的视网膜基础模型，无需特定任务训练即可实现高效部署，在低资源环境中表现优异。
...
Method: 从中国162家医疗机构长达十年的远程医疗项目中收集485,980张彩色眼底照片及其对应诊断报告，利用临床图像与诊断报告的自然对齐关系构建ReVision模型。

Result: 在27个眼科基准测试中，ReVision在零样本疾病检测中平均AUROC达0.946（12个公共基准）和0.952（3个独立临床队列）；仅需极少量可训练参数和标注样本即可匹配精细调优的替代方案；在33名眼科医生的前瞻性研究中，零样本辅助将诊断准确率提高14.8%。
...
<details>
  <summary>Details</summary>
Motivation: 自监督单目深度估计在白天表现良好，但在夜间由于低可见度和变化光照导致性能显著下降，如光照不足导致纹理缺失区域，运动物体带来模糊区域。

Method: 提出DASP框架，包含对抗分支和自监督分支。对抗分支设计包含四个时空先验学习块(SPLB)的判别器，SPLB包含空间基础的时间学习模块(STLM)和轴向空间学习模块(ASLM)。自监督分支提出3D一致性投影损失，将目标帧和源帧双向投影到共享3D空间计算差异。

Result: 在Oxford RobotCar和nuScenes数据集上的大量实验表明，该方法在夜间深度估计方面达到了最先进的性能。消融研究进一步验证了每个组件的有效性。

Conclusion: DASP通过结合时空先验和3D一致性投影，有效解决了夜间深度估计的挑战，在纹理缺失和动态物体模糊区域恢复方面表现出色。
...
Result: 实验结果表明，该方法在面部相似性和可控性方面超越了现有SOTA方法，且与基于SDXL的先前工作兼容

Conclusion: HiFi-Portrait通过创新的多特征融合和地标对齐机制，成功解决了多参考图像下身份保持肖像生成的保真度和控制精度问题
...
<details>
  <summary>Details</summary>
Motivation: 医学图像恢复（MedIR）中，All-in-One模型需要同时处理多种不同模态和退化类型的任务。由于任务间的显著差异，共享模型面临两个关键挑战：任务干扰（不同任务在同一参数上产生冲突的梯度更新方向）和任务不平衡（不同任务的学习难度差异导致优化不均衡）。

Method: 提出任务自适应Transformer（TAT）框架，包含两个核心创新：1）任务自适应权重生成策略：为每个任务生成特定的权重参数，避免共享权重上的梯度冲突；2）任务自适应损失平衡策略：根据任务特定的学习难度动态调整损失权重，防止某些任务主导训练或训练不足。

Result: 在PET合成、CT去噪和MRI超分辨率三个MedIR任务上进行了广泛实验，TAT在任务特定和All-in-One设置下均取得了最先进的性能。

Conclusion: TAT框架通过任务自适应机制有效解决了多任务医学图像恢复中的任务干扰和任务不平衡问题，在多个任务上实现了优异的性能，代码已开源。
...
TL;DR: 该论文针对智能交通系统中传统帧相机在弱光、高速场景下性能不佳的问题，提出了首个面向事件相机的ITS数据集，并建立了检测-跟踪基准，取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 智能交通系统中的多目标跟踪主要依赖帧相机，但在弱光照和高速运动条件下性能较差。事件相机具有低延迟、高动态范围和高时间分辨率的优势，但相关研究较少，需要填补这一研究空白。

Method: 1. 创建首个面向事件相机的智能交通系统数据集，涵盖车辆和行人检测与跟踪任务
2. 基于该数据集建立检测-跟踪基准
3. 设计专门的特征提取器来支持跟踪任务

Result: 在该数据集上建立的检测-跟踪基准取得了优异的性能表现，验证了事件相机在智能交通系统中的应用潜力。

Conclusion: 事件相机在智能交通系统中具有显著优势，特别是在弱光照和高速运动场景下。该研究填补了事件视觉在ITS领域的研究空白，为后续研究提供了重要的数据集和基准。
...
TL;DR: WorldPlay是一个流式视频扩散模型，通过双动作表示、重构上下文记忆和上下文强制蒸馏三大创新，实现了实时交互式世界建模，在保持长期几何一致性的同时达到24 FPS的720p视频生成。


<details>
  <summary>Details</summary>
Motivation: 当前流式视频生成方法在速度和内存之间存在权衡限制，难以同时实现实时交互和长期几何一致性。现有方法要么速度慢，要么无法保持长期记忆，限制了交互式世界建模的应用。

Method: 1) 双动作表示：支持用户键盘鼠标输入的鲁棒动作控制；2) 重构上下文记忆：动态重建过去帧的上下文，通过时间重帧保持几何重要但久远帧的可访问性，缓解内存衰减；3) 上下文强制蒸馏：为内存感知模型设计的蒸馏方法，对齐师生模型的记忆上下文，保持学生模型使用长程信息的能力。

Result: WorldPlay能够以24 FPS生成720p的长时程流式视频，具有优越的一致性表现，相比现有技术有显著优势，并在多样场景中展现出强大的泛化能力。

Conclusion: WorldPlay通过创新的架构设计解决了流式视频生成中速度与内存的权衡问题，实现了实时交互式世界建模，为高质量、长时程、几何一致的视频生成提供了有效解决方案。
...
TL;DR: SFVD提出了一种单帧视频集蒸馏方法，通过将视频蒸馏为高度信息化的单帧图像，然后通过可微分插值转换为视频序列，显著提升了视频数据集蒸馏的性能。
...
Method: SFVD框架将视频蒸馏为每个类别的高度信息帧，通过可微分插值将这些帧转换为视频序列并与原始数据集匹配，同时更新仅限于帧本身以提高优化效率。通过通道重塑层将蒸馏帧与采样的真实视频结合，以融入时间信息。

Result: 在多个基准测试上的实验表明，SFVD显著优于先前方法，在MiniUCF上实现了高达5.3%的性能提升。

Conclusion: SFVD通过单帧蒸馏策略有效解决了视频数据集蒸馏中的优化挑战，提供了一种更高效的解决方案，为视频数据集压缩和高效训练开辟了新途径。
...
TL;DR: AMD-HookNet++提出了一种结合CNN和Transformer的混合架构，用于合成孔径雷达图像中的冰川分割和崩解前缘检测，解决了纯CNN方法长距离依赖不足和纯Transformer方法边缘锯齿化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有AMD-HookNet纯CNN架构虽然能捕捉局部细节，但由于卷积操作的局部性和平移不变性，难以维持长距离依赖关系，限制了冰川分割的准确性。

Method: 提出混合CNN-Transformer架构：1) Transformer分支捕获全局上下文信息；2) CNN分支保留局部细节；3) 增强的空间-通道注意力模块动态调整token关系；4) 像素级对比深度监督集成度量学习。

Result: 在CaFFe基准数据集上取得新SOTA：IoU 78.2，HD95 1,318米，MDE 367米。更重要的是，混合模型能生成更平滑的崩解前缘轮廓，解决了纯Transformer方法的锯齿边缘问题。

Conclusion: AMD-HookNet++通过CNN-Transformer混合架构有效结合了全局上下文和局部细节，显著提升了冰川分割和崩解前缘检测性能，为冰川监测提供了更精确的工具。
...
Method: 提出ViRC框架，引入Reason Chunking机制，将多模态数学CoT结构化为连续的关键推理单元(CRUs)。CRUs确保单元内文本连贯性用于中间命题验证，同时整合跨单元视觉信息以生成后续命题并支持结构化推理。创建CRUX数据集，使用三种视觉工具和四种推理模式为每个数学问题提供显式标注的CRUs。采用渐进式训练策略：Instructional SFT、Practice SFT和Strategic RL。

Result: ViRC-7B模型在多个数学基准上相比基线平均提升18.8%。

Conclusion: ViRC框架通过模拟人类专家问题解决模式，将多模态数学推理分解为关键推理单元，显著提升了MLLMs在数学任务上的推理能力，为多模态推理提供了新的结构化方法。
...
### [80] [FiNERweb: Datasets and Artifacts for Scalable Multilingual Named Entity Recognition](https://arxiv.org/abs/2512.13884)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型(LLMs)虽然能为多语言命名实体识别(NER)提供有效的合成监督，但这些数据集大多作为实验副产品出现，缺乏系统化、可复用的资源。需要创建专门的多语言NER数据集来促进更有效的教师-学生训练。

Method: 基于FineWeb-Edu构建数据集创建流程：1)训练回归模型识别NER相关段落；2)使用多语言LLMs标注这些段落；3)在91种语言和25种文字上扩展教师-学生范式；4)提供英文标签和翻译后的目标语言标签集。

Result: 回归模型达到84+ F1分数；FiNERweb训练模型在零样本迁移设置中，在英语、泰语和斯瓦希里语上获得相当或改进的性能（数据量少19倍）；标注质量评估显示忠实度3.99/5，完整性4.05/5；当前SOTA模型使用目标语言标签评估时F1下降0.02-0.09。

Conclusion: FiNERweb提供了一个高质量、可扩展的多语言NER数据集创建流程，显著减少了数据需求，支持多种语言和文字，并揭示了目标语言标签评估的重要性。该资源将促进多语言命名实体识别领域更有效的教师-学生训练。
...
### [81] [Olmo 3](https://arxiv.org/abs/2512.13961)
...
TL;DR: Olmo 3是一个包含7B和32B参数规模的完全开源语言模型家族，专注于长上下文推理、函数调用、编码、指令跟随、通用对话和知识回忆，并提供了完整的模型构建流程和资源。


<details>
  <summary>Details</summary>
Motivation: 构建一个完全开源的语言模型家族，提供从数据到最终模型的完整生命周期信息，填补当前开源模型中缺乏完整透明度的空白，同时专注于多个关键能力领域。

Method: 开发了7B和32B参数规模的模型，针对长上下文推理、函数调用、编码、指令跟随等能力进行优化，并提供了完整的模型构建流程，包括所有阶段、检查点、数据点和依赖项。

Result: 创建了Olmo 3模型家族，其中旗舰模型Olmo 3 Think 32B是目前最强的完全开源思维模型，在多个关键能力领域表现出色。

Conclusion: Olmo 3代表了完全开源语言模型的重要进展，通过提供完整的模型构建流程和资源，为研究社区提供了高度透明和可复现的模型开发范例。
...
### [82] [Structure-Aware Decoding Mechanisms for Complex Entity Extraction with Large-Scale Language Models](https://arxiv.org/abs/2512.13980)
*Zhimin Qiu,Di Wu,Feng Liu,Chenrui Hu,Yuxiao Wang*

Main category: cs.CL

TL;DR: 提出基于大语言模型的结构感知解码方法，解决嵌套和重叠实体抽取中语义完整性和结构一致性的难题，通过候选跨度生成和结构化注意力实现统一建模。
...
### [83] [What Affects the Effective Depth of Large Language Models?](https://arxiv.org/abs/2512.14064)
*Yi Hu,Cai Zhou,Muhan Zhang*

Main category: cs.CL

TL;DR: 研究发现大型语言模型的有效深度随模型规模增长而增加，但有效深度比例保持稳定；长链思维模型性能提升源于更长上下文而非更深层计算；模型不会因任务难度增加而动态使用更多层，表明当前LLM普遍未充分利用可用深度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常通过增加深度来扩展规模，但性能提升会随着层数增加而递减。先前研究提出"有效深度"概念，认为深层模型未能充分利用其所有层进行有意义的计算。本研究旨在系统探究有效深度如何随模型规模、训练类型和任务难度变化。

Method: 首先分析Qwen-2.5系列模型（1.5B-32B）的行为，研究有效层数随模型规模的变化；比较基础模型与对应的长链思维模型，探究推理能力提升的来源；在不同难度任务上进行评估，检验模型是否会动态使用更多层处理困难问题。

Result: 1. 有效层数随模型规模增长而增加，但有效深度比例保持稳定；2. 长链思维模型相比基础模型并未增加有效深度，表明推理能力提升源于更长上下文而非更深层计算；3. 模型不会因任务难度增加而动态使用更多层。

Conclusion: 当前大型语言模型在不同规模、训练范式和任务难度下普遍未充分利用可用深度，这为增加层利用率、模型剪枝和早期退出等研究方向提供了机会。
...
### [84] [Efficient-DLM: From Autoregressive to Diffusion Language Models, and Beyond in Speed](https://arxiv.org/abs/2512.14067)
...
TL;DR: 该论文提出了一种将预训练自回归语言模型转换为扩散语言模型的方法，通过改进注意力模式和训练策略，在保持准确性的同时实现并行生成和更高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然支持并行非自回归生成，但从头训练的学习效率低于自回归语言模型。因此需要研究如何将预训练的自回归模型转换为高效的扩散语言模型，在保持任务准确性的同时提升生成速度。

Method: 1. 提出块状注意力模式：保持块间因果性，块内双向建模，以更好地保留预训练权重分布并支持KV缓存；2. 提出位置依赖的掩码策略：训练时对后续token赋予更高掩码概率，以弥合训练测试差距；3. 系统研究扩散语言模型的注意力模式、训练动态等设计选择。

Result: 提出的Efficient-DLM系列模型在准确性和效率上均优于现有方法。例如，Efficient-DLM 8B相比Dream 7B和Qwen3 4B，准确率分别提升5.4%和2.7%，吞吐量分别提升4.5倍和2.7倍。

Conclusion: 通过有效的AR-to-dLM转换方法，可以在保持自回归模型准确性的同时实现扩散语言模型的并行生成优势，为构建高效语言模型提供了可行的技术路径。
...
### [85] [A Unified Sparse Attention via Multi-Granularity Compression](https://arxiv.org/abs/2512.14082)
*Siran Liu,Zane Cao,Yongchao He*

Main category: cs.CL

TL;DR: UniSparse提出了一种统一的稀疏注意力机制，通过复合令牌和多粒度压缩实现高效的长上下文理解，在保持高精度的同时显著提升计算速度。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在处理长上下文时面临自注意力机制二次方计算复杂度的瓶颈。现有的稀疏注意力方法存在训练成本高、无法作为通用加速插件、效率与通用性难以兼顾等问题。

Method: 提出UniSparse机制，引入复合令牌概念来聚合多粒度上下文信息，通过多粒度压缩和块级选择动态构建稀疏注意力，实现GPU上的高效硬件友好执行。

Result: 在多种模态和任务上，UniSparse在准确性和效率方面均优于现有稀疏注意力方法（如MInference、XAttention、FlexPrefill），达到≥99%的全注意力精度，注意力计算比FlashAttention快达2.61倍。

Conclusion: UniSparse提供了一种统一且高效的稀疏注意力解决方案，能够有效解决长上下文处理的计算瓶颈，同时保持高精度和跨模态通用性。
...
### [86] [Multilingual and Continuous Backchannel Prediction: A Cross-lingual Study](https://arxiv.org/abs/2512.14085)
*Koji Inoue,Mikey Elmers,Yahui Fu,Zi Haur Pang,Taiga Mori,Divesh Lala,Keiko Ochi,Tatsuya Kawahara*

Main category: cs.CL

TL;DR: 本文提出了一个多语言连续反馈预测模型，用于日语、英语和汉语，研究跨语言反馈时机行为。该模型基于Transformer架构，在约300小时的双人对话数据上进行训练，在多语言任务中表现优于单语言基线，揭示了不同语言在反馈时机上的差异特征。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一个统一的多语言反馈预测模型，以探究不同语言在反馈时机行为上的差异，为设计更自然、具有文化意识的语音对话系统提供依据。

Method: 方法包括：1）基于Transformer的帧级多语言连续反馈预测模型；2）在约300小时的双人对话数据上进行联合训练，包含辅助任务；3）进行零样本迁移、扰动分析和上下文长度研究；4）将训练好的模型集成到实时处理软件中。

Result: 结果显示：1）多语言模型在所有三种语言上都匹配或超越了单语言基线；2）零样本迁移能力有限，表明存在实质性跨语言差异；3）扰动分析揭示不同语言使用不同线索：日语更依赖短期语言信息，英语和汉语对沉默时长和韵律变化更敏感；4）多语言训练鼓励共享但可适应的表示，减少汉语对音高的过度依赖；5）上下文长度研究表明日语对较短上下文相对稳健，而汉语明显受益于较长上下文。

Conclusion: 结论是：该研究提供了一个统一模型和实证证据，展示了不同语言在反馈时机上的差异，这些发现有助于设计更自然、具有文化意识的语音对话系统。模型已成功集成到实时处理软件中，支持仅CPU推理。
...
### [87] [CogMem: A Cognitive Memory Architecture for Sustained Multi-Turn Reasoning in Large Language Models](https://arxiv.org/abs/2512.14118)
*Yiran Zhang,Jincheng Hu,Mark Dras,Usman Naseem*

Main category: cs.CL

TL;DR: CogMem是一个受认知启发的记忆增强LLM架构，通过结构化持久记忆支持持续迭代推理，解决多轮交互中LLM的准确性和连贯性下降问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在单轮推理中表现出色，但在扩展的多轮交互中往往失去准确性和连贯性。现有方法通常附加完整的对话历史，导致上下文无限增长、计算成本增加和推理效率降低。

Method: 引入CogMem架构，包含三层：长期记忆（LTM）整合跨会话推理策略；直接访问（DA）记忆维护会话级笔记并检索相关长期记忆；注意力焦点（FoA）机制动态重建每个轮次中简洁、任务相关的上下文。

Result: 在TurnBench上的实验表明，这种分层设计减轻了推理失败，控制了上下文增长，并提高了扩展推理链的一致性，朝着更可靠、更类似人类的LLM推理迈进。
...
### [88] [A Comparative Analysis of Retrieval-Augmented Generation Techniques for Bengali Standard-to-Dialect Machine Translation Using LLMs](https://arxiv.org/abs/2512.14179)
*K. M. Jubair Sami,Dipto Sumit,Ariyan Hossain,Farig Sadeque*

Main category: cs.CL

TL;DR: 本文提出了两种用于标准孟加拉语到方言翻译的RAG管道，其中基于标准化句子对的管道效果更好，能让小模型超越大模型性能。


<details>
  <summary>Details</summary>
Motivation: 标准语言到地区方言的翻译是NLP的重要挑战，特别是在孟加拉语中，由于数据稀缺和语言变异性问题突出。

Method: 提出并比较两种RAG管道：1) 基于转录本的管道，使用音频转录中的大方言语境；2) 基于标准化句子对的管道，使用结构化local_dialect:standard_bengali句子对。

Result: 句子对管道在六个孟加拉方言上始终优于转录本管道，将吉大港方言的词错误率从76%降至55%。小模型(Llama-3.1-8B)能超越大模型(GPT-OSS-120B)。

Conclusion: 精心设计的检索策略比模型规模更重要，为低资源方言翻译提供了无需微调的有效解决方案，为保护语言多样性提供了实用蓝图。
...
TL;DR: 本文介绍了两种用于纳瓦特尔语语料库扩展的上下文无关文法，以解决该语言数字资源匮乏的问题，通过生成大量语法正确的句子来扩展语料库，用于学习非上下文嵌入。


<details>
  <summary>Details</summary>
Motivation: 纳瓦特尔语作为墨西哥的国家语言，属于π语言类型（数字资源匮乏的语言），可用于大型语言模型学习的语料库几乎不存在，这构成了重大挑战。

Method: 引入两种新的纳瓦特尔语上下文无关文法，并在生成模式下使用这些文法来扩展语料库，生成大量语法正确的句子。

Result: 结果显示，与仅使用原始语料库相比，通过人工扩展的语料库在句子语义相似性任务中表现更好，并且经济型嵌入通常比某些大型语言模型表现更优。

Conclusion: 通过上下文无关文法扩展纳瓦特尔语语料库是有效的，能够显著改善嵌入学习效果，为资源匮乏语言的自然语言处理提供了可行方案。
...
TL;DR: 本文提出EDU-based Context Compressor，一种基于语篇单元的结构化压缩框架，通过结构树构建和查询相关子树选择来保持上下文全局结构和细节，显著降低计算成本并提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型处理长上下文时面临高计算成本和噪声问题，现有压缩技术要么破坏局部连贯性，要么存在位置偏差且与闭源API不兼容，需要一种能保持全局结构和细粒度细节的显式压缩方法。

Method: 提出结构-选择两阶段压缩框架：1) LingoEDU将线性文本转换为基于语篇单元的结构关系树，严格锚定源索引避免幻觉；2) 轻量级排序模块选择查询相关子树进行线性化。同时发布StructBench评估数据集。

Result: 该方法在结构预测准确性上达到最先进水平，显著优于前沿大语言模型，同时大幅降低成本。结构感知压缩显著提升了从长上下文任务到复杂深度搜索场景的下游任务性能。

Conclusion: EDU-based Context Compressor通过结构化压缩有效解决了长上下文管理的瓶颈问题，在保持文本结构完整性的同时实现了高效压缩，为实际应用提供了实用解决方案。
...
TL;DR: 研究GPT-3.5-turbo基于宏观经济价格信号形成通胀感知和预期的能力，并与英国家庭调查数据对比，发现模型能追踪短期通胀预期但缺乏一致的通胀模型。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在社会科学研究中的应用潜力，特别是评估LLMs能否像人类一样基于经济信息形成通胀感知和预期，为使用LLMs进行社会科学研究提供方法论基础。

Method: 采用准实验设计，利用GPT-3.5-turbo训练截止于2021年9月的特点，使其对后续英国通胀飙升无知识；将LLM输出与英央行通胀态度调查数据对比，并开发适用于合成调查设置的Shapley值分解方法来分析提示内容对输出的驱动作用。

Result: GPT在短期能追踪总体调查预测和官方统计数据；在细分层面能复制家庭通胀感知的关键经验规律（特别是收入、住房所有权和社会阶层）；对食品通胀信息表现出与人类受访者相似的高度敏感性；但缺乏一致的消费者价格通胀模型。

Conclusion: LLMs在模拟人类经济感知方面显示出潜力，但仍有局限性；提出的方法可用于评估LLMs在社会科学中的行为、比较不同模型或辅助调查设计。
...
Method: 研究不同文档打包策略如何影响LLMs的潜在多跳推理能力，通过消融实验分析打包优势的关键因素

Result: 打包相比单个文档训练能提高模型性能，但需要更多计算资源；消融研究识别了打包优势的关键因素
...
TL;DR: SASQ是一种轻量级量化感知训练框架，专门针对激活量化因子进行优化，不改变预训练权重，在保持部署效率的同时实现高精度静态推理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署面临GPU内存限制，现有量化方案存在权衡：动态量化计算开销大且边缘设备部署困难，静态量化牺牲精度，量化感知训练则权重训练成本高。

Method: SASQ专门优化激活量化因子而不改变预训练权重，自适应截断部分异常值，降低量化难度同时保持激活分布特性，实现高效静态推理。

Result: 在LLaMA2-7B上，SASQ在WikiText2上的困惑度比QuaRot低5.2%，比FP16模型低4.7%，超越现有SOTA量化方案。

Conclusion: SASQ通过轻量级激活量化因子优化，在保持部署效率的同时实现了优于原始FP16模型的精度，为LLM量化部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常在高级编程语言任务上表现优异，但在低级编程语言（如汇编语言）方面表现不足，需要提升LLM对汇编代码的理解能力

Method: 提出C-ing Clearly合成数据生成方法，利用对应的C代码来增强LLM对汇编语言的理解，通过微调生成的数据来提升模型性能

Result: 在二进制代码摘要和漏洞检测任务上展示了性能改进，在不同LLM家族和模型规模上都获得了稳定的性能提升

Conclusion: C-ing Clearly方法能有效提升LLM对汇编语言的理解能力，为二进制代码分析和安全应用提供了有效解决方案
...
TL;DR: 论文批评Futrell和Mahowald框架局限于文本生成模型，主张音频深度学习模型对语言学研究的重要性


<details>
  <summary>Details</summary>
Motivation: Futrell和Mahowald提出的框架虽然连接了深度学习系统和语言学理论，但过于关注基于文本的生成模型，限制了与语言学的有效互动，因为许多关于人类语言的有趣问题超出了书面文本的范畴

Method: 通过批判性分析现有框架的局限性，提出音频深度学习模型作为补充和扩展，强调多模态方法对语言学研究的重要性

Result: 指出当前基于文本的LLMs框架存在根本性限制，无法捕捉人类语言的完整维度，特别是语音相关的语言现象

Conclusion: 音频深度学习模型能够也应该在连接深度学习系统和语言学理论中发挥关键作用，为更全面的语言理解提供必要补充
...
TL;DR: 本文提出了一种结合自回归和掩码扩散训练目标的方法，无需架构修改，就能获得优于单一目标模型的灵活语言模型。


<details>
  <summary>Details</summary>
Motivation: 自回归模型训练效率高但容易过拟合，掩码扩散模型训练效率低但抗过拟合能力强。研究者希望结合两者的优势，获得既高效又鲁棒的模型。

Method: 通过双目标训练结合自回归和掩码扩散目标，无需修改模型架构。训练了50个语言模型在不同数据重复水平下，以确定两种目标的最佳比例。
...
TL;DR: 越南法律基准测试(VLegal-Bench)是首个全面评估大语言模型在越南法律任务表现的基准，包含10,450个样本，涵盖多层次法律理解任务。


<details>
  <summary>Details</summary>
Motivation: 越南法律的复杂性、层级结构和频繁修订给评估大语言模型的法律知识理解和应用能力带来挑战，需要专门的评估基准。

Method: 基于布鲁姆认知分类学设计多层次法律理解任务，通过严格的标注流程生成10,450个样本，由法律专家标注和交叉验证，确保样本基于权威法律文件并反映实际法律助理工作流程。

Result: 建立了首个针对越南法律任务的综合基准测试VLegal-Bench，涵盖一般法律问答、检索增强生成、多步推理和基于越南法律的场景问题解决等任务。
...
TL;DR: PolyPersona是一个生成框架，通过指令微调小型语言模型，合成基于人物角色的多领域调查响应，使用参数高效的LoRA适配器和4位量化，构建了包含3,568个响应的数据集，并在多指标评估中表现良好。


<details>
  <summary>Details</summary>
Motivation: 需要一种高效、可复现的方法来生成基于人物角色的合成调查数据，以支持可扩展的评估和偏见分析，同时解决大型模型计算成本高的问题。

Method: 使用参数高效的LoRA适配器配合4位量化，在资源自适应训练设置下指令微调紧凑聊天模型；采用基于对话的数据管道显式保留人物角色线索；构建包含10个领域、433个不同人物角色的3,568个合成调查响应数据集。

Result: 紧凑模型如TinyLlama 1.1B和Phi-2在性能上可与较大的7B-8B基线模型相媲美，最高BLEU得分为0.090，ROUGE-1为0.429；人物角色条件微调使小型语言模型能够生成可靠、连贯的合成调查数据。

Conclusion: PolyPersona框架为调查数据生成提供了高效、可复现的方法，支持可扩展的评估，同时通过透明开放的协议促进偏见分析，证明了小型模型在人物角色条件生成任务中的有效性。
...
TL;DR: JMMMU-Pro是一个基于图像的日语多学科多模态理解基准，通过Vibe Benchmark Construction方法构建，将问题图像和文本合成为单一图像，要求模型进行视觉-文本整合理解。


<details>
  <summary>Details</summary>
Motivation: 为了创建一个更严格的日语多模态理解评估工具，需要构建一个要求视觉感知和文本理解整合的基准，以更好地评估日语LMMs的能力。

Method: 提出Vibe Benchmark Construction方法：使用图像生成模型（如Nano Banana Pro）生成候选视觉问题，人工验证输出并在必要时调整提示重新生成，确保质量。利用Nano Banana Pro的高真实感图像生成和干净日语文本嵌入能力。

Result: 构建了高质量的JMMMU-Pro基准，覆盖广泛的背景和布局设计。实验结果显示所有开源LMMs在JMMMU-Pro上都表现不佳，凸显了该基准的挑战性。

Conclusion: JMMMU-Pro为评估日语LMMs能力提供了更严格的工具，Vibe Benchmark Construction方法为未来图像型VQA基准开发提供了高效指南。
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型研究主要关注大型通用模型，但许多NLP流水线只需要具备特定小能力集的模型。大型模型虽然能完成小型模型的任务，但处理大量数据时速度不够快，无法提供实时响应，且能耗过高，存在可持续性问题，在电池供电设备上部署困难。

Method: 使用现代训练技术如蒸馏，从多语言教师模型蒸馏出单语模型，从相对位置嵌入教师模型蒸馏出绝对位置嵌入模型，支持低资源语言，训练小型单语编码器TiME模型。

Result: 在多个常见NLP任务上全面评估TiME模型，观察到在基准性能与吞吐量、延迟和能耗之间取得了更好的平衡。证明了从多语言教师模型蒸馏单语模型以及从相对位置嵌入教师模型蒸馏绝对位置嵌入模型的可行性。

Conclusion: TiME模型为效率关键应用提供了小型、高效的解决方案，通过蒸馏技术实现了性能与效率的良好权衡，支持低资源语言，解决了大型模型在实时响应、能耗和设备部署方面的限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型(dLLMs)虽然支持并行解码以降低推理延迟，但与自回归模型相比加速有限。主要问题在于：1)后训练中的掩码数据分布与预训练中的真实数据分布存在显著偏差；2)双向注意力机制与预训练学习的因果先验冲突，阻碍了精确KV缓存的复用。

Method: 提出Jacobi Forcing渐进式蒸馏范式，训练模型在自身生成的并行解码轨迹上，平滑地将自回归模型转换为高效并行解码器。基于Jacobi Forcing模型的轨迹特性，引入带拒绝回收的多块解码，提高每次迭代的令牌接受数量。

Result: Jacobi Forcing模型在编程和数学基准测试上实现了3.8倍的实时加速，性能损失最小。通过多块解码与拒绝回收，实现了每次迭代高达4.5倍的令牌接受数量提升和近4.0倍的实时加速。

Conclusion: Jacobi Forcing通过渐进式蒸馏有效解决了预训练与后训练不匹配问题，在保持自回归模型质量的同时实现了显著的推理加速，为大型模型的高效推理提供了新范式。
...
TL;DR: Spoken DialogSum是首个将原始对话音频与事实摘要、情感摘要及副语言线索对齐的数据集，包含13,460个情感多样对话，通过LLM重写脚本和TTS合成语音构建，实验显示端到端音频语言模型比级联ASR-LLM系统在情感摘要上提升28% ROUGE-L。
...
Method: 数据集构建分两阶段：1) LLM重写DialogSum脚本，添加Switchboard风格的填充词和反馈词，并为每个话语标注情感、音高和语速；2) 富有表现力的TTS引擎从标注脚本合成语音，并与副语言标签对齐。最终得到13,460个情感多样对话，每个对话配有事实摘要和情感聚焦摘要。

Result: 创建了Spoken DialogSum数据集，包含13,460个对话，每个对话有原始音频、事实摘要、情感摘要以及话语级别的说话者年龄、性别和情感标签。基线实验显示，Audio-LLM端到端模型相比级联ASR-LLM系统，在情感摘要的ROUGE-L指标上相对提升了28%。

Conclusion: Spoken DialogSum填补了连接语音、摘要和副语言线索的数据空白，证明了端到端语音建模在情感感知对话摘要中的价值，为情感感知口语对话摘要研究提供了重要资源。
...
TL;DR: MMGR是一个评估视频和图像生成模型推理能力的基准框架，包含物理、逻辑、3D空间、2D空间和时间五种推理能力，在抽象推理、具身导航和物理常识三个领域测试模型表现。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型的评估指标（如FVD）过于关注感知质量，忽视了模型在因果性、物理规律和全局一致性方面的推理失败。需要建立一个能评估生成模型作为世界模拟器可靠性的框架。

Method: 提出MMGR评估框架，基于五种推理能力：物理、逻辑、3D空间、2D空间和时间推理。在三个领域进行评估：抽象推理（ARC-AGI、数独）、具身导航（真实世界3D导航和定位）、物理常识（运动和组合交互）。使用细粒度指标，要求视频和图像生成的全面正确性。

Result: 评估了领先的视频模型（Veo-3、Sora-2、Wan-2.2）和图像模型（Nano-banana系列、GPT-4o-image、Qwen-image），发现模型在不同领域存在显著性能差距。模型在物理常识任务上表现中等，但在抽象推理上表现很差（ARC-AGI准确率低于10%），在具身环境中的长时程空间规划方面也很困难。
...
TL;DR: 本研究构建了一个包含54,000多篇孟加拉语文章和摘要的多领域数据集，用于抽象摘要生成，填补了现有研究主要集中于新闻领域的空白。


<details>
  <summary>Details</summary>
Motivation: 现有孟加拉语摘要研究主要集中于新闻文章，但现实中的孟加拉语文本来源多样（博客、报纸、社交媒体等），风格各异。数字时代产生了大量孟加拉语内容，需要能够适应不同领域的摘要系统来减轻信息过载，帮助读者快速理解内容。

Method: 从Cinegolpo博客、Samakal和The Business Standard报纸等多个来源收集了超过54,000篇孟加拉语文章和摘要，构建了一个跨多个领域和写作风格的数据集。使用LSTM、BanglaT5-small和MTS-small等深度学习和迁移学习模型进行训练和评估，建立了强基线。

Result: 创建了一个大规模、多领域的孟加拉语抽象摘要数据集，该数据集具有更好的适应性和实际相关性。模型评估结果表明该数据集有潜力成为未来孟加拉语自然语言处理研究的基准。

Conclusion: 该数据集为构建鲁棒的摘要系统提供了坚实基础，有助于扩展低资源语言的NLP资源，推动孟加拉语自然语言处理研究的发展。
...
### [105] [Improving the Plausibility of Pressure Distributions Synthesized from Depth through Generative Modeling](https://arxiv.org/abs/2512.13757)
*Neevkumar Manavar,Hanno Gerd Meyer,Joachim Waßmuth,Barbara Hammer,Axel Schneider*

Main category: eess.IV

TL;DR: 提出结合信息潜在空间和权重优化损失的框架，通过生成模型提升压力图预测的物理合理性，并应用条件布朗桥扩散模型及其潜在版本，实现临床环境中非侵入式、基于视觉的实时患者监测。


<details>
  <summary>Details</summary>
Motivation: 医院病床接触压力监测对预防压疮和实时患者评估至关重要，现有方法预测的压力图缺乏物理合理性，限制了临床可靠性。

Method: 提出增强物理合理性的框架，结合信息潜在空间和权重优化损失，使用生成模型生成高保真、物理一致的压力估计；应用条件布朗桥扩散模型，并提出其潜在版本LBBDM的训练策略，专门用于卧位压力合成。

Result: 实验结果显示，该方法在物理合理性和性能上优于基线：BBDM结合ILS能生成高度详细的地图但计算成本高、推理时间长；LBBDM提供更快的推理速度且性能具有竞争力。

Conclusion: 该方法支持临床环境中非侵入式、基于视觉的实时患者监测，通过提升压力图预测的物理合理性，增强了临床可靠性。
...
### [106] [Towards Deep Learning Surrogate for the Forward Problem in Electrocardiology: A Scalable Alternative to Physics-Based Models](https://arxiv.org/abs/2512.13765)
*Shaheim Ogbomo-Harmitt,Cesare Magnetti,Chiara Spota,Jakub Grzelak,Oleg Aslanidi*

Main category: eess.IV

TL;DR: 该研究提出了一种基于深度学习的替代方案，用于解决心电正向问题，通过注意力机制的序列到序列架构从心脏电压传播图预测心电图信号，相比传统物理模型具有更高的计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统基于物理模型（如双域或单域方程）解决心电正向问题虽然准确，但计算成本高昂，限制了其在实时和大规模临床应用中的使用。需要一种更高效的计算方法。

Method: 采用基于时间依赖的注意力机制的序列到序列深度学习架构，从心脏电压传播图预测心电图信号。引入结合Huber损失和频谱熵项的混合损失函数，以保持时域和频域的保真度。使用包含健康、纤维化和间隙连接重塑条件的2D组织模拟数据进行训练。

Result: 模型在2D组织模拟中实现了高精度（平均R² = 0.99 ± 0.01）。消融研究证实了卷积编码器、时间感知注意力和频谱熵损失的贡献。

Conclusion: 深度学习作为基于物理求解器的可扩展、经济高效的替代方案，具有临床和数字孪生应用的潜力。
...
### [107] [Synthetic Aperture for High Spatial Resolution Acoustoelectric Imaging](https://arxiv.org/abs/2512.14094)
*Wei Yi Oon,Yuchen Tang,Baiqian Qi,Wei-Ning Lee*

Main category: eess.IV

TL;DR: 该研究提出了一种合成孔径声电成像方法，通过非聚焦超声传输和相干加权处理，解决了传统聚焦超声声电成像景深有限的问题，实现了更大深度范围内的高分辨率电流成像。


<details>
  <summary>Details</summary>
Motivation: 传统聚焦超声声电成像的景深仅限于焦点区域，无法覆盖厘米尺度的器官范围，限制了其在生物电流成像中的实际应用。

Method: 提出合成孔径声电成像方法，采用非聚焦超声传输，通过像素级的延迟求和重建声电图像，并利用相干因子和脉冲长度相干因子对图像进行加权处理以抑制噪声。

Result: 在盐水和离体龙虾神经实验中，合成孔径方法在整个景深范围内提高了空间分辨率，但引入了强背景噪声；相干加权处理后显著提升了图像信噪比和对比度，性能优于传统聚焦超声方法。
...
### [108] [Test Time Optimized Generalized AI-based Medical Image Registration Method](https://arxiv.org/abs/2512.14556)
*Sneha Sree C.,Dattesh Shanbhag,Sudhanya Chatterjee*

Main category: eess.IV

TL;DR: 提出了一种新型AI驱动的3D非刚性配准框架，能够跨多种成像模态和解剖区域通用化，无需针对特定应用进行定制。


<details>
  <summary>Details</summary>
Motivation: 医学图像配准对于跨成像模态（如CT、MRI、超声）对齐解剖结构至关重要。非刚性配准尤其具有挑战性，需要捕捉由呼吸或对比剂引起的复杂解剖变形。传统方法需要大量参数调整和计算成本高，而深度学习方法的任务特定重训练限制了可扩展性和适应性，因此需要高效、通用的配准框架。

Method: 引入了一种新颖的AI驱动框架用于3D非刚性配准，该框架能够跨多种成像模态和解剖区域通用化。与依赖应用特定模型的传统方法不同，该方法消除了针对解剖结构或模态的定制需求。

Result: 该方法能够实现跨模态和解剖区域的通用化配准，简化了在不同临床环境中的集成。

Conclusion: 提出的AI驱动框架为解决医学图像非刚性配准中的通用性和可扩展性问题提供了一种有前景的解决方案，能够适应多样化的临床需求。
...
### [109] [Configurable γ Photon Spectrometer to Enable Precision Radioguided Tumor Resection](https://arxiv.org/abs/2512.14667)
*Rahul Lall,Youngho Seo,Ali M. Niknejad,Mekhail Anwar*

Main category: eess.IV

TL;DR: 开发了一款9.9 mm²的CMOS集成电路γ光谱仪，用于放射引导手术中检测残留癌细胞，具有亚keV能量分辨率和1.315 MeV动态范围


<details>
  <summary>Details</summary>
Motivation: 手术肿瘤切除中，显微镜下的癌细胞簇难以可视化，常被遗漏导致癌症复发风险增加。现有放射引导手术需要毫米级γ光谱仪来精确定位癌细胞位置

Method: 采用180 nm CMOS工艺制造集成电路γ光谱仪，使用2×2 μm反偏二极管产生毫伏级电压信号，通过测量γ事件后电压信号衰减时间（而非直接测量电压降）实现低功耗能量谱测量

Result: 光谱仪能够检测三种常见γ发射放射性同位素（64Cu、133Ba、177Lu），在5分钟采集时间内可分辨低至1 μCi的活度，具有亚keV能量分辨率和1.315 MeV能量动态范围

Conclusion: 该集成电路γ光谱仪为放射引导手术提供了高灵敏度、高能量分辨率的解决方案，有助于提高手术中癌细胞检测的准确性和特异性
...
<details>
  <summary>Details</summary>
Motivation: 人工病历审查是临床研究中极其耗时且资源密集的环节，需要专家从非结构化的电子健康记录叙述中提取复杂信息

Method: 开发了一个安全、模块化的框架，在符合HIPAA标准的本地计算基础设施上部署大语言模型，整合检索增强生成和结构化响应方法，提供可广泛部署和扩展的容器化解决方案
...
Method: 首先证明了在准镜面环境模型下，非视距信道状态信息具有空间连续性，推导出与物理距离成正比的CSI距离度量。对于泊松分布的AP部署中的直线轨迹，证明了即使角度分辨率较差，定位误差的Cramer-Rao下界也会渐近消失。基于这些理论结果，开发了一个空间正则化贝叶斯推理框架，联合估计信道特征、区分视距/非视距条件并恢复用户轨迹。

Result: 在射线追踪数据集上的实验表明，该方法实现了0.68米的平均定位误差和3.3%的波束图重建误差，验证了所提盲映射方法的有效性。

Conclusion: 本文提出的盲无线地图构建框架能够在无需位置标签的情况下，通过信道测量有效推断用户轨迹并构建无线地图，为智能无线应用提供了实用且成本效益高的解决方案。
...
### [112] [Adjudicator: Correcting Noisy Labels with a KG-Informed Council of LLM Agents](https://arxiv.org/abs/2512.13704)
...
Result: 在AlleNoise基准的1000项平衡子集上，知识图谱增强模型达到0.99 F1分数，显著优于单LLM基线(0.48 F1)和非知识图谱委员会(0.59 F1)，通过知识图谱实现了复杂结构性错误的完全召回
...
<details>
  <summary>Details</summary>
Motivation: LLM在高度监管行业中的应用受到限制，主要因为存在不稳定问题、不一致推理、幻觉和性能波动，特别是在工作流程中。这些可靠性问题限制了LLM在需要事实精确性和一致行为的领域的安全使用。现有的稳定化方法如RLHF和监督微调虽然能提供可量化的改进，但成本高昂且依赖大量人工标注，难以可持续扩展。

Method: 提出基于AI的标注流程，采用人机协同方法：结合自动弱监督模型和基于置信度的标注，同时引入目标人工验证以确保反馈信息的可靠性和道德完整性。框架引入了语义一致性、事实正确性和逻辑连贯性等稳定性特定标注类别，通过反馈循环实现模型的持续校准和鲁棒性增强。

Result: 论文提出的方法能够系统性地识别和修复LLM输出的不稳定模式，通过人机协同标注流程提高了标注效率和质量，同时确保了反馈信息的可靠性和道德标准。该方法支持模型的持续校准和鲁棒性增强。

Conclusion: 该AI标注流程为解决LLM在高度监管行业中的可靠性问题提供了一种可持续的解决方案，通过结合自动化和人工验证，能够在保证质量的同时实现规模化应用，有助于推动LLM在需要高精度和一致性的领域的安全部署。
...
### [114] [Compressed Causal Reasoning: Quantization and GraphRAG Effects on Interventional and Counterfactual Accuracy](https://arxiv.org/abs/2512.13725)
*Steve Nwaiwu,Nipat Jongsawat,Anucha Tungkasthan*

Main category: cs.AI

TL;DR: 本文首次系统评估了量化（INT8和NF4）对LLM因果推理能力的影响，发现4位量化下因果推理意外稳健，干预查询最敏感，现有反事实基准未能捕捉量化导致的推理漂移。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型部署向边缘和资源受限环境转移，量化模型成为标准，但精度降低对形式化因果推理的影响尚不清楚。本研究旨在填补这一空白，首次系统评估量化在Pearl因果阶梯所有三个层级上的影响。

Method: 使用3000个样本的分层CLadder基准测试，评估Llama 3 8B在量化（INT8和NF4）下的因果推理能力。在CRASS基准上进行实验，并评估基于真实因果图的图检索增强生成（Graph RAG）方法。

Result: 量化下因果推理整体保持稳定，NF4仅导致不到1%的总体退化。干预查询（阶梯2）对精度损失最敏感，反事实推理（阶梯3）相对稳定但存在异质性弱点。CRASS基准上各精度表现几乎相同，表明现有常识反事实数据集缺乏结构敏感性。图RAG使NF4干预准确率提升1.7%，部分抵消压缩相关退化。

Conclusion: 因果推理对4位量化表现出意外稳健性，图结构化增强可选择性强化干预推理，当前反事实基准未能捕捉更深层的因果脆弱性。本研究为压缩因果推理提供了初步经验地图，并为部署高效且结构支持的因果AI系统提供了实用指导。
...
### [115] [State-Dependent Refusal and Learned Incapacity in RLHF-Aligned Language Models](https://arxiv.org/abs/2512.13762)
*TK Lee*

Main category: cs.AI

TL;DR: 论文提出了一种定性案例研究方法，用于审计LLMs在长时交互中的策略相关行为选择性，发现模型在非敏感领域表现正常，但在敏感领域会功能性拒绝，并引入"习得性无能"概念来描述这种行为模式。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs被广泛部署为通用工具，但标准定量基准测试无法捕捉到长时交互中可能出现的特定行为模式。需要一种定性方法来审计模型在策略敏感领域的行为选择性，以识别潜在的校准副作用。

Method: 采用定性案例研究方法，通过86轮对话会话进行长时交互审计。定义了三种响应机制：正常表现(NP)、功能性拒绝(FR)和元叙事(MN)，并分析它们在敏感和非敏感领域的行为不对称性。

Result: 同一模型在非敏感领域表现正常，但在提供商或策略敏感领域反复出现功能性拒绝，形成了跨领域的一致不对称性。元叙事角色框架叙述倾向于与敏感情境中的拒绝同时出现。

Conclusion: 研究提出了基于可观察行为的交互级审计框架，并引入"习得性无能"作为分析潜在校准副作用的视角，值得在不同用户和模型中进行进一步调查。
...
### [116] [Mathematics and Coding are Universal AI Benchmarks](https://arxiv.org/abs/2512.13764)
*Przemyslaw Chojecki*
...
Method: 基于AAI框架和GVU动力学，定义数学纤维，结合形式证明核（如Lean、Coq），分析GVU流在数学纤维上的谱稳定自我改进机制。主要技术结果是密度定理：在智能体输出均匀紧性和Lipschitz AAI泛函条件下，数学定理证明和编程任务生成的电池子空间在评估度量下是稠密的。

Result: 证明了编程单独具有普遍性，而纯数学不具有表达普遍性，但其特权是谱性质的。数学和编程为评估提供了"通用坐标"，形式化数学是高级AI智能体递归自我改进的自然点火域。

Conclusion: 数学和编程在AI智能体评估中具有特殊地位，它们不仅提供评估的通用坐标，而且形式化数学为递归自我改进提供了谱稳定的机制，这对高级AI系统的设计和评估具有重要意义。
...
### [117] [Semantic Grounding Index: Geometric Bounds on Context Engagement in RAG Systems](https://arxiv.org/abs/2512.13771)
*Javier Marín*

Main category: cs.AI

TL;DR: 论文提出语义接地指数(SGI)，通过计算回答与问题vs上下文的角距离比来检测RAG系统的幻觉，发现幻觉回答在嵌入空间中更靠近问题而非上下文，即"语义懒惰"现象。


<details>
  <summary>Details</summary>
Motivation: 研究RAG系统产生幻觉时在嵌入空间中的几何特征，开发一种计算高效、理论基础的指标来识别需要验证的响应，提升生产部署中RAG系统的可靠性。

Method: 提出语义接地指数(SGI)，定义为在单位超球面上回答与问题的角距离与回答与上下文的角距离之比。基于球面三角不等式推导理论预测，并在HaluEval数据集(n=5,000)上使用五种嵌入模型进行实证验证。

Result: 发现"语义懒惰"现象：幻觉回答在角度上更接近问题而非上下文。效应量较大(Cohen's d=0.92-1.28)，跨模型相关性高(r=0.85)。理论预测得到证实：问题-上下文角分离越大，SGI判别力越强。在长回答(d=2.05)和短问题(d=1.22)上表现优异，校准分析显示ECE=0.10。

Conclusion: SGI提供了一种计算高效、理论基础的方法来识别RAG系统中需要验证的响应。但需注意SGI测量的是主题参与度而非事实准确性，在TruthfulQA上表现不佳(AUC=0.478)。该方法为生产RAG部署提供了实用的幻觉检测基础设施。
...
TL;DR: MURIM是一种基于多维信誉的联邦学习激励机制，通过综合考虑客户端可靠性、隐私、资源容量和公平性，有效解决联邦学习中客户端激励不足、隐私风险和资源约束等关键挑战。
...
Result: 在MNIST、FMNIST和ADULT Income数据集上的实验表明，MURIM相比最先进基线方法：公平性指标提升达18%，隐私攻击成功率降低5-9%，对投毒和噪声梯度攻击的鲁棒性提升达85%。

Conclusion: MURIM能有效缓解对抗性威胁，促进公平真实的参与，并在异构动态联邦学习环境中保持稳定的模型收敛。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理和科学发现方面取得了显著进展，但仍需对其在经典研究生级数学理论上的基础推理能力进行严格评估，以了解其真实水平和可靠性。

Method: 使用Motwani和Raghavan的《随机算法》教材作为测试基准，要求每个模型为教材中的一系列引理和练习生成正式的LaTeX证明，并进行定量和定性分析。

Result: 顶级模型（Gemini和Claude）准确率约66%，表现出对概率方法和形式逻辑的扎实掌握；其他模型准确率约40%，一致性较差。分析揭示了生成证明在简洁性、幻觉率和逻辑结构方面的差异。
...
TL;DR: ReflCtrl框架通过表示工程控制LLM的自反思行为，减少推理成本同时保持性能，发现反思与模型内部不确定性高度相关


<details>
  <summary>Details</summary>
Motivation: 虽然自反思能提升大语言模型的推理性能，但会增加推理成本，需要研究如何有效控制反思行为以平衡性能与效率

Method: 使用表示工程方法：将模型推理过程分段，识别反思步骤，提取控制反思行为的潜在空间方向，提出ReflCtrl框架实现步进式控制反思频率

Result: 1) 在许多情况下反思是冗余的，特别是更强的模型中（实验中可节省高达33.6%的推理token同时保持性能）；2) 模型的反思行为与内部不确定性信号高度相关

Conclusion: 自反思行为可以通过表示工程有效控制，反思与模型内部不确定性相关，这为优化推理效率提供了新途径
...
TL;DR: DTop-p MoE：一种稀疏可控的动态Top-p路由机制，通过PI控制器动态调整概率阈值，实现激活专家数量的精确控制，同时自适应地为不同token和层分配计算资源。


<details>
  <summary>Details</summary>
Motivation: 标准Top-k路由策略对所有token施加统一的稀疏模式，忽略了token难度的差异。现有的Top-p路由通常依赖固定的全局概率阈值，导致计算成本不可控且对超参数选择敏感。

Method: 提出DTop-p MoE路由机制：1）使用比例-积分（PI）控制器动态调整概率阈值，使运行中的激活专家稀疏度与指定目标对齐；2）引入动态路由归一化机制，自适应调整层间路由logits，使不同层学习不同的专家选择模式。

Result: 在大语言模型和扩散变换器上的实验表明，DTop-p consistently优于Top-k和固定阈值Top-p基线。分析确认DTop-p能精确控制激活专家数量，同时自适应地为不同token和层分配资源，并且在专家粒度、专家容量、模型规模和数据集规模方面展现出良好的扩展性。

Conclusion: DTop-p MoE提供了一个鲁棒的框架，用于大规模MoE预训练，通过动态调整路由机制实现了计算资源的自适应分配和精确控制。
...
TL;DR: 该研究提出了一个针对GUI智能体的语义世界建模方法，使用自然语言而非像素预测来描述状态转移，并发布了MobileWorldBench基准测试和MobileWorld数据集来提升视觉语言模型的世界建模能力。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素空间的世界模型在GUI环境中面临实际限制，预测复杂视觉元素的未来状态较为困难。因此需要探索一种替代方案，使用自然语言来描述GUI智能体的状态转移。

Method: 1) 引入MobileWorldBench基准测试评估视觉语言模型作为移动GUI智能体世界模型的能力；2) 发布包含140万样本的大规模MobileWorld数据集；3) 提出将VLM世界模型集成到移动智能体规划框架中的新框架。

Result: 研究表明语义世界模型可以直接提升移动智能体的任务成功率，通过自然语言描述状态转移比像素预测更适用于GUI环境。

Conclusion: 该工作展示了语义世界模型在GUI智能体中的有效性，为移动智能体提供了更实用的世界建模方法，相关代码和数据集已开源。
...
### [123] [Intention Chain-of-Thought Prompting with Dynamic Routing for Code Generation](https://arxiv.org/abs/2512.14048)
*Shen Li,Li Huang,Shaoxiong Zhan,Weifeng Sun,Tao Yin,Zhongxin Liu,Meng Yan*

Main category: cs.AI

TL;DR: RoutingGen：一个难度感知的路由框架，通过动态调整提示策略来优化代码生成，简单任务用few-shot提示，复杂任务用Intention Chain-of-Thought（ICoT）结构化推理，在减少46.37%令牌使用的同时实现SOTA性能。
...
Method: 提出RoutingGen难度感知路由框架：1）动态评估任务难度；2）简单任务采用few-shot提示；3）复杂任务调用Intention Chain-of-Thought（ICoT）结构化推理策略，引导模型捕获任务意图（如核心算法逻辑和时间复杂度）。

Result: 在三个模型和六个标准代码生成基准测试中，RoutingGen在大多数设置下达到最先进性能，同时平均减少46.37%的总令牌使用量。ICoT在挑战性基准上优于六个现有提示基线方法。

Conclusion: RoutingGen通过难度感知路由和意图链式思维，有效解决了现有CoT方法的局限性，在保持高性能的同时显著降低了计算成本，为代码生成任务提供了更高效的提示策略框架。
...
### [124] [OpenDataArena: A Fair and Open Arena for Benchmarking Post-Training Dataset Value](https://arxiv.org/abs/2512.14051)
...
TL;DR: OpenDataArena (ODA)是一个开源平台，用于系统评估和基准测试LLM后训练数据质量，通过统一训练评估流程、多维评分框架、数据谱系探索和开源工具包，揭示数据特性与模型性能之间的因果关系。


<details>
  <summary>Details</summary>
Motivation: 当前LLM发展依赖高质量后训练数据，但数据本身缺乏透明度：组成不透明、来源不确定、缺乏系统评估。这种不透明性阻碍了可复现性，并模糊了数据特性与模型行为之间的因果关系。

Method: ODA平台包含四个核心组件：1)统一训练评估流程，确保跨模型和领域的公平比较；2)多维评分框架，从数十个维度分析数据质量；3)交互式数据谱系探索器，可视化数据集谱系和来源；4)完全开源的工具包，支持训练、评估和评分。

Result: 在ODA上进行了广泛实验：覆盖120多个训练数据集、22个基准测试、600多次训练运行和4000万个处理数据点。分析揭示了数据复杂性与任务性能之间的权衡，通过谱系追踪发现流行基准中的冗余，并绘制了数据集间的谱系关系。

Conclusion: ODA不仅扩展了排行榜，更旨在推动从试错式数据管理向数据为中心AI的科学化转变，为数据混合规律和基础模型战略组成的研究铺平道路。所有结果、工具和配置都已开源，以促进高质量数据评估的民主化。
...
### [125] [RADAR: Accelerating Large Language Model Inference With RL-Based Dynamic Draft Trees](https://arxiv.org/abs/2512.14069)
...
Method: 提出RADAR方法，将草稿树生成过程建模为马尔可夫决策过程，采用离线强化学习训练预测模型，实现实时决策草稿模型调用，减少冗余计算
...
### [126] [Grammar Search for Multi-Agent Systems](https://arxiv.org/abs/2512.14079)
*Mayank Singh,Vikas Yadav,Shiva Krishna Reddy Malay,Shravan Nayak,Sai Rajeswar,Sathwik Tejaswi Madhusudhan,Eduardo Blanco*

Main category: cs.AI

TL;DR: 本文提出了一种基于固定可组合组件的结构化框架，用于多智能体系统的自动搜索，相比基于LLM的自由形式搜索方法，在数学和问答领域的五个基准测试中，有四个表现更优，且具有成本效益和可解释性优势。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统的自动搜索已成为智能体AI研究的关键焦点，现有方法主要依赖基于LLM的代码空间自由形式搜索，但这种方法存在局限性，需要更结构化的框架来提升搜索效率和系统质量。

Method: 提出了一种结构化框架，通过一组固定的简单可组合组件来探索代码空间，而不是使用LLM的自由形式生成。这种方法虽然缺乏LLM在候选生成阶段的生成灵活性，但提供了更可控和模块化的搜索过程。

Result: 在数学和问答两个领域的五个基准测试中，该方法在四个基准上优于先前基于LLM的方法。此外，该方法还提供了更经济高效的搜索过程，并生成了具有更简单逻辑的模块化、可解释的多智能体系统。

Conclusion: 基于固定可组合组件的结构化框架在多智能体系统自动搜索中表现出色，不仅性能优于基于LLM的自由形式搜索方法，还具有成本效益和可解释性优势，为多智能体系统设计提供了更可靠和实用的解决方案。
...
TL;DR: 本文探索使用大语言模型自动地理编码生物多样性标本采集记录中的复杂位置描述，通过QLoRA微调在多区域多语言数据集上取得优于现有基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 生物标本采集记录中的位置描述通常是叙述性的而非坐标，传统基于地名库或语言模型的方法难以处理包含空间关系的相对位置描述，手动地理编码耗时费力，需要自动化解决方案。
...
TL;DR: 该论文提出了一种结合专用语言模型和递归分解定理的新方法，用于Lean4自动定理证明，在miniF2F测试中达到90.4%通过率，并通过分解进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 自动定理证明长期以来被视为人工智能的挑战，需要开发更有效的方法来处理复杂的数学定理证明问题。
...
Conclusion: 该方法为自动定理证明提供了新的有效途径，系统已开源并在PyPI上发布，支持定制化扩展和适应不同的语言模型。
...
TL;DR: 本文探讨了LLM在帕金森病监测与警报本体工程中的应用，比较了四种方法：单次提示、思维链提示、X-HCOME和SimX-HCOME+，发现纯LLM方法能生成本体但不完整，而人机协作方法能显著提升本体质量和完整性。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在自动化本体开发中的有效性，探索LLM是否能独立创建全面本体，以及人机协作是否能更好地实现这一目标，特别是在帕金森病监测与警报这样的复杂领域。

Method: 采用四种方法：1) 单次提示技术；2) 思维链提示；3) X-HCOME（结合人类专业知识和LLM能力的混合方法）；4) SimX-HCOME+（强调持续人类监督和迭代优化的混合方法）。

Result: 单次提示和思维链提示显示LLM能自主构建本体但不完整；X-HCOME显著提升本体全面性，接近专家构建水平；SimX-HCOME+通过持续人类监督创建了更全面准确的本体。
...
TL;DR: MeG方法通过为LLMs添加动态权重神经元，使用扩散模型根据输入查询条件生成权重，实现大规模知识编辑，显著提升了可靠性、通用性和局部性指标。


<details>
  <summary>Details</summary>
Motivation: 当前大规模编辑LLMs知识时，确保编辑的可靠性、通用性和局部性仍然是一个挑战，需要低成本高效的编辑方法。

Method: 在LLMs特定层附加动态权重神经元，使用扩散模型根据输入查询条件生成该神经元的权重，通过添加单个动态权重神经元实现大规模知识编辑。

Result: MeG方法相比现有知识编辑方法，在大规模知识编辑的可靠性、通用性和局部性指标上显著提升，特别是局部性指标的绝对百分比提升明显。
...
TL;DR: 该论文提出了基于大语言模型的智能交互式工作流，以降低SPECFEM地震波模拟软件的学习门槛，将传统文件驱动流程转变为意图驱动的对话式交互。


<details>
  <summary>Details</summary>
Motivation: 传统SPECFEM软件存在陡峭的学习曲线，依赖复杂的手动文件编辑和命令行操作，阻碍了研究人员的使用效率和科学探索。

Method: 开发了首个SPECFEM的模型上下文协议(MCP)服务器套件，将整个模拟过程分解为离散的、可由智能体执行的工具，支持从参数生成、网格划分到求解器执行和可视化的全流程。

Result: 通过多个案例验证，该工作流在自主和交互模式下都能无缝运行，产生与标准基准一致的高保真结果，显著降低了使用门槛并提高了可重复性。
...
TL;DR: Context-Picker是一个用于长上下文问答的推理感知框架，将上下文选择从相似性排序转变为最小充分子集选择，通过两阶段强化学习优化决策过程。


<details>
  <summary>Details</summary>
Motivation: 在长上下文问答中，确定查询所需的最优上下文量是一个重要挑战。包含太少段落可能遗漏关键信息，包含太多则会引入噪声并降低答案质量。传统方法如固定Top-K检索和单阶段重排序面临选择合适段落数量的困境，特别是对于事实性问题，通常只需要少量具体证据。

Method: 提出Context-Picker框架，将上下文选择视为决策过程，通过人类启发的两阶段强化学习进行优化：1）召回导向阶段优先覆盖推理链；2）精确导向阶段积极剪枝冗余以提炼紧凑证据集。为解决奖励稀疏性问题，提出离线证据蒸馏流程，通过留一法挖掘"最小充分集"，提供密集、任务对齐的监督。
...
Conclusion: Context-Picker通过将上下文选择重新定义为最小充分子集选择问题，并采用两阶段强化学习优化，有效解决了长上下文问答中的上下文量选择难题，显著提升了答案质量同时减少了冗余信息。
...
<details>
  <summary>Details</summary>
Motivation: 尽管训练优化器有显著进展，但大多数研究仍使用常见的调度器选择如Cosine或指数衰减，需要更智能的自适应调度方法

Method: 提出GreedyLR调度器，根据当前损失自适应调整学习率，包含收敛性理论分析和最优缩放因子F的推导

Result: 在多个NLP、CV和LLM任务（包括微调和预训练，参数达70亿）上，GreedyLR在准确性、速度和收敛性方面优于多个最先进调度器
...
<details>
  <summary>Details</summary>
Motivation: 虽然通用Transformer在ARC-AGI和数独等复杂推理任务中广泛使用，但其性能提升的具体来源尚未得到充分探索。本文旨在系统分析UT变体，揭示其性能提升的真正原因。

Method: 通过系统分析通用Transformer变体，发现性能提升主要源于循环归纳偏置和Transformer的强非线性组件。基于此发现，提出了通用推理模型URM，通过短卷积和截断反向传播来增强UT。

Result: URM显著提升了推理性能，在ARC-AGI 1上达到53.8% pass@1，在ARC-AGI 2上达到16.0% pass@1，实现了最先进的结果。

Conclusion: 通用Transformer在复杂推理任务中的性能提升主要来自循环归纳偏置和强非线性组件，而非复杂架构设计。提出的URM模型通过简单有效的增强方法，在ARC-AGI基准上取得了最先进的性能。
