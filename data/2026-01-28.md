...
TL;DR: 提出一种基于动态掩码的后门攻击方法，针对目标检测模型，利用SAM分割模型生成动态触发器，在蘑菇检测数据集上验证攻击效果，既保持干净数据高精度又实现高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在计算机视觉领域广泛应用，但随着部署增加，面临各种对抗攻击威胁，特别是后门攻击。本文旨在展示针对目标检测模型的新型动态后门攻击方法，强调在关键现实领域（如蘑菇检测）的实际风险，并突出外包实践中后门攻击的严重威胁。

Method: 提出动态掩码后门攻击方法：1）使用数据集投毒技术嵌入恶意触发器；2）利用SAM（强大的图像分割AI模型）生成掩码，实现动态触发器放置；3）针对YOLOv7目标检测模型设计攻击场景；4）在蘑菇检测数据集上进行实验验证。

Result: 实验表明：1）在干净数据上保持高精度；2）在投毒样本上实现高攻击成功率；3）超越基于静态一致模式的传统后门注入方法；4）证明了动态触发器放置的隐蔽性和有效性。

Conclusion: 本文提出的动态掩码后门攻击方法展示了针对目标检测模型的新型隐蔽攻击威胁，特别是在关键现实领域。研究结果强调了保护深度学习模型免受不断演化的对抗威胁的紧迫性，需要开发更强大的防御措施。
...
<details>
  <summary>Details</summary>
Motivation: 尽管动态神经辐射场在说话人像生成方面取得了显著进展，但在准确高效捕捉口型运动方面仍存在挑战，需要改进方法提升说话人像的逼真度
...
Conclusion: 提出的方法有效解决了说话人像中口型运动的准确捕捉问题，通过结合眨眼嵌入、哈希网格编码和动态地标变换器，实现了高质量、逼真的说话人像生成
...
TL;DR: 该研究提出了一种结合3DMM和StyleGAN的方法，仅使用自拍视频实现高质量头部虚拟形象重演，能够恢复高频细节如皱纹和头发纹理。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要问题：1）3DMM方法难以实时捕捉整个头部（包括非面部区域和背景细节）；2）基于GAN的方法在重现细粒度头部细节（如皱纹、头发纹理）方面有限；3）现有方法通常需要大量训练数据，很少专注于仅使用简单的自拍视频实现虚拟形象重演。

Method: 提出了一种结合3DMM和基于StyleGAN的生成器的方法。构建了详细的3D重建模型，在对抗训练中采用混合损失函数进行前景重建和虚拟形象图像生成，以恢复高频细节。

Result: 在自重演和交叉重演任务上的定性和定量评估表明，该方法相比现有方法实现了更优越的头部虚拟形象重建，具有丰富而复杂的纹理细节。

Conclusion: 该方法能够仅使用自拍视频实现高质量的头部虚拟形象重演，成功解决了现有方法在捕捉完整头部、恢复高频细节和减少数据需求方面的局限性。
...
TL;DR: 提出基于统计框架的AI生成图像检测方法，通过计算多个检测器的p值并进行统计集成，生成可解释的概率分数


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测方法缺乏形式化的可解释性，依赖对虚假内容的隐含假设，在分布偏移下鲁棒性有限

Method: 构建统计框架，利用多个现有检测器计算测试统计量的p值，通过经典统计集成方法聚合，评估与真实图像分布的匹配度

Result: 该方法具有通用性、灵活性和无需训练的特点，适用于多样化和演化的场景中的鲁棒虚假图像检测

Conclusion: 提出的统计框架为AI生成图像检测提供了严格、可解释且鲁棒的解决方案，能够适应不断发展的生成模型环境
...
TL;DR: 该研究系统比较了8种基于ViT的视觉基础模型在手术场景中的应用，发现结合深度信息的几何感知预训练显著优于单模态RGB模型，且具有出色的数据效率。


<details>
  <summary>Details</summary>
Motivation: 当前手术视觉理解主要依赖单模态RGB预训练，忽略了手术环境的复杂3D几何结构。虽然通用计算机视觉领域已有支持多模态或几何感知输入的架构，但在手术场景中深度信息的价值尚未充分探索。

Method: 使用包含140万张机器人手术图像及其深度图的数据集进行预训练，比较8种基于ViT的视觉基础模型，这些模型在预训练领域、学习目标和输入模态（RGB vs. RGB-D）上有所不同。在8个手术数据集上评估模型性能，涵盖目标检测、分割、深度估计和姿态估计任务，采用冻结主干和端到端微调两种协议。

Result: 包含显式几何标记化的模型（如MultiMAE）在所有任务中都显著优于单模态基线。几何感知预训练展现出卓越的数据效率：仅用25%标注数据微调的模型就能持续超越使用完整数据集训练的RGB模型。这些优势无需在推理时改变架构或增加计算开销。
...
### [6] [Towards Gold-Standard Depth Estimation for Tree Branches in UAV Forestry: Benchmarking Deep Stereo Matching Methods](https://arxiv.org/abs/2601.19461)
*Yida Lin,Bing Xue,Mengjie Zhang,Sam Schofield,Richard Green*

Main category: cs.CV

TL;DR: 首次系统评估8种立体深度估计方法在植被密集环境中的零样本泛化能力，发现不同方法在不同场景表现各异，DEFOM在植被深度估计中表现最佳
...
Method: 对8种立体方法进行首次系统性零样本评估，涵盖迭代细化、基础模型、扩散基和3D CNN范式，使用官方预训练权重（在Scene Flow上训练），在4个标准基准测试和新提出的5,313对Canterbury Tree Branches数据集上进行评估

Result: 结果显示场景依赖性模式：基础模型在结构化场景表现优异（BridgeDepth在ETH3D上0.23px；DEFOM在Middlebury上4.65px），而迭代方法在不同基准测试中表现不一（IGEV++在ETH3D上0.36px但在Middlebury上6.77px）。在Tree Branches数据集上，DEFOM成为植被深度估计的黄金标准基线，具有优异的跨域一致性（在所有基准测试中排名1-2位，平均排名1.75）

Conclusion: DEFOM在植被深度估计中表现最佳，其预测将作为未来基准测试的伪地面真值，为自主无人机林业作业提供了重要的深度估计基准
...
<details>
  <summary>Details</summary>
Motivation: Split-Federated学习在客户端硬件要求低，但通信信道中的噪声会影响学习过程和最终模型质量，需要提高对信道噪声的鲁棒性

Method: 提出智能平均策略用于SplitFed学习，通过改进的聚合方法来增强对通信信道噪声的抵抗能力

Result: 在胚胎图像分割模型上的实验表明，智能平均策略相比传统平均方法能容忍两个数量级更强的通信信道噪声，同时保持最终模型的准确性

Conclusion: 智能平均策略显著提升了SplitFed学习对通信信道噪声的鲁棒性，为在噪声环境下的分布式学习提供了有效解决方案
...
TL;DR: 本文提出了一种相机加权机制，通过调整源视图对目标视图的重要性来改进少样本新视角合成，包括基于几何特性的确定性加权方案和基于交叉注意力的学习方案。
...
Method: 提出相机加权机制，包含两种方法：1) 利用欧几里得距离和角度差异等几何特性的确定性加权方案；2) 优化视图加权的基于交叉注意力的学习方案。该机制可集成到各种NVS算法中，并可进一步训练以增强合成质量。

Result: 自适应视图加权提高了新视角合成的准确性和真实感，为改进NVS提供了有前景的方向。

Conclusion: 相机加权机制能够根据源视图与目标视图的相关性调整其重要性，这一适应性方法可集成到多种NVS算法中，有效提升少样本新视角合成的质量。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像中深度学习部署需要不确定性量化，但标准保形预测方法在分割任务中常忽略解剖学上下文，导致预测集空间不连贯、过度分割，限制了临床实用性。

Method: 提出随机游走保形预测(RW-CP)，基于预训练视觉基础模型特征构建k近邻图，应用随机游走扩散不确定性，正则化非一致性分数，使预测集对保形校准参数λ更不敏感，确保更稳定连续的解剖边界。

Result: 在多模态公共数据集上评估，相比标准CP基线，在允许错误率α=0.1时，分割质量提升高达35.4%，同时保持严格的边缘覆盖率。

Conclusion: RW-CP通过随机游走扩散增强空间一致性，在保持统计有效性的同时生成解剖学上有效的分割预测集，显著提升了医学影像分割的临床实用性。
...
TL;DR: 提出了一种基于RGB-D成像的快速、非侵入式3D伤口测量算法，结合RGB-D里程计和B样条曲面重建，实现自动计算伤口周长、表面积等临床相关参数。


<details>
  <summary>Details</summary>
Motivation: 慢性伤口监测和管理需要准确高效的伤口测量方法，传统方法存在局限性，需要开发自动化的伤口评估工具。

Method: 结合RGB-D里程计与B样条曲面重建，生成详细的3D伤口网格，自动计算临床相关的伤口测量参数。

Result: 在真实硅胶伤口模型上评估，3D重建精度达到亚毫米级，提取的测量参数重复性高且与人工评估结果一致，优于现有最先进的物体中心RGB-D重建方法，运行时间适合临床实时部署。
...
<details>
  <summary>Details</summary>
Motivation: 传统皮层特征图表示（如网格）存在离散性限制，需要更连续、紧凑的神经表示来支持高效优化和任意分辨率特征学习

Method: 提出神经皮层映射作为连续神经表示，开发NC-Reg算法：结合神经皮层特征图、梯度下降优化和模拟退火策略进行刚性配准

Result: 比经典重心插值快30倍（相同迭代次数），实现亚度精度（<1°偏离全局最优），作为临床环境中稳健的预对齐策略

Conclusion: 神经皮层映射为皮层特征表示提供了有效的连续神经替代方案，NC-Reg算法在皮层表面配准中表现出高精度和效率，具有临床应用潜力
...
<details>
  <summary>Details</summary>
Motivation: 当前世界生成方法面临三个主要障碍：可控性、可扩展性和效率。端到端场景生成模型受限于数据稀缺；基于对象的生成方法依赖固定分辨率表示，在大场景中保真度下降；无训练方法虽然灵活，但推理时速度慢且计算成本高。

Method: 提出生成式引导策略，从少量输入图像开始，利用3D重建和可扩展场景生成技术合成不同大小和布局的场景，生成足够数据训练端到端模型。使用伪草图标签实现可控性，将场景表示为可变场景块的集合，压缩为扁平化向量集表示以减少大场景的标记长度。

Result: 框架能够生成不同大小和布局的场景，产生足够数据训练端到端模型，通过伪草图标签实现可控性，并展示了对未见草图的泛化能力。扁平化向量集表示显著减少了大型场景的标记长度，在不同场景尺寸下保持一致的几何保真度，同时提高了训练和推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态检索增强生成系统缺乏内部策略来决定何时以及如何进行检索，且依赖模块化流水线（检测器、分割器、字幕生成器等），限制了系统的灵活性和性能。

Method: 提出PixSearch模型，在编码过程中发出<search>标记触发检索，选择查询模态（文本、图像或区域），并生成像素级掩码直接作为视觉查询。采用两阶段监督微调策略，通过搜索交错监督来学习检索时机和查询选择，同时保持分割能力。

Result: 在自我中心化和实体中心化的VQA基准测试中，PixSearch显著提高了事实一致性和泛化能力，在CRAG-MM上相比整图检索获得了19.7%的相对准确率提升，同时在各种VQA和纯文本QA任务上保持了有竞争力的推理性能。

Conclusion: PixSearch通过端到端的方式统一了区域级感知和检索增强推理，消除了对模块化流水线的依赖，为多模态检索增强系统提供了更灵活、更有效的解决方案。
...
TL;DR: 论文提出了m2sv基准测试，用于评估视觉语言模型在空间推理任务上的表现，特别是地图到街景的对齐能力。现有模型在该任务上表现不佳（65.2%准确率），远低于人类基准（95%），揭示了模型在几何对齐、证据聚合和推理一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态基准测试中表现良好，但在需要将抽象俯视图与自我中心视图对齐的空间推理任务上仍然脆弱。现有模型缺乏对地图到街景空间对齐能力的系统评估。

Method: 1. 引入m2sv基准测试，要求模型通过对齐北向俯视地图与同一真实世界交叉口的街景图像来推断相机视角方向；2. 发布m2sv-20k数据集，具有地理多样性和受控模糊性；3. 提供m2sv-sft-11k数据集用于监督微调；4. 使用结构信号和人工努力系统分析推理难度；5. 对适应后的开源模型进行广泛的失败分析。

Result: 1. 最佳评估的VLM在m2sv上仅达到65.2%准确率，远低于人类基准的95%；2. 监督微调和强化学习带来一致改进，但跨基准评估显示迁移能力有限；3. 分析揭示了模型在几何对齐、证据聚合和推理一致性方面的持续差距。

Conclusion: 该研究突出了视觉语言模型在跨视角空间推理方面的局限性，特别是在几何对齐、证据聚合和推理一致性方面存在显著差距。m2sv基准测试为未来研究提供了重要工具，推动在基于视角的接地空间推理方面的进一步工作。
...
TL;DR: GF-Screen是一个用于大规模CT扫描中泛癌筛查的强化学习框架，采用"一瞥一聚焦"策略，通过Glance模型定位病变区域，Focus模型精确分割病灶，利用分割结果通过强化学习奖励Glance模型，解决了CT体积中微小病灶定位的挑战。


<details>
  <summary>Details</summary>
Motivation: 大规模CT扫描中的泛癌筛查对现有AI方法具有挑战性，主要因为难以在大型CT体积中定位各种类型的微小病灶。极端的正负样本不平衡阻碍模型关注病变区域，而对健康区域的冗余关注不仅降低效率，还增加假阳性。

Method: 提出GF-Screen框架：1) Glance模型从整个CT体积中裁剪一组子体积，学习选择包含病灶的子体积；2) Focus模型精确分割病灶；3) 使用分割结果通过强化学习奖励Glance模型；4) 引入群体相对学习范式，通过群体相对比较优先处理高优势预测，丢弃低优势预测。

Result: 在16个内部和7个外部数据集上对9种病灶类型进行了广泛实验，证明了GF-Screen的有效性。在MICCAI FLARE25泛癌挑战赛的公共验证排行榜上领先，大幅超越FLARE24冠军解决方案（DSC提高25.6%，NSD提高28.2%）。

Conclusion: GF-Screen首次有效扩展了前沿强化学习技术来解决泛癌筛查中的特定挑战，通过"一瞥一聚焦"策略显著提高了大规模CT扫描中泛癌筛查的性能和效率。
...
TL;DR: Reg-TTR是一个测试时优化框架，通过结合深度学习与传统配准技术的优势，在推理时优化预训练模型的预测结果，以较小的计算成本显著提升配准精度。


<details>
  <summary>Details</summary>
Motivation: 传统图像配准方法稳健但迭代过程缓慢；深度学习加速了推理但难以应对领域偏移；新兴的配准基础模型在速度与稳健性之间取得平衡，但通常无法达到在特定数据集上训练的专用模型的峰值精度。

Method: 提出Reg-TTR测试时优化框架，在推理时对预训练模型的预测结果进行优化，结合深度学习与传统配准技术的互补优势，仅需21%的额外推理时间（0.56秒）。

Result: 在两个不同任务上评估Reg-TTR，结果显示该方法达到了最先进的性能，同时保持了接近先前深度学习方法的推理速度。

Conclusion: 随着基础模型的持续涌现，该框架提供了一种高效策略，能够缩小配准基础模型与在专用数据集上训练的最先进方法之间的性能差距。
...
TL;DR: FBSDiff/FBSDiff++：从频域视角实现无需训练的文本驱动图像翻译框架，通过动态频率带替换实现可控的图像转换


<details>
  <summary>Details</summary>
Motivation: 随着大规模文本到图像扩散模型的显著进展，研究者开始关注其自然扩展到文本驱动的图像到图像翻译领域，其中源图像除了文本提示外还提供视觉指导。现有方法通常需要训练、微调或在线优化，缺乏灵活可控的解决方案。

Method: 提出FBSDiff框架，从频域视角将现成的T2I扩散模型适配到I2I范式。通过动态替换扩散特征的频率带，实现即插即用的图像翻译：低频带替换用于外观引导，中频带替换用于布局引导，高频带替换用于轮廓引导。FBSDiff++进一步改进：1）大幅加速推理速度（8.9倍）；2）改进频率带替换模块以支持任意分辨率和宽高比的输入；3）扩展功能实现局部图像操作和风格特定内容创建。

Result: 大量定性和定量实验验证了FBSDiff++在图像翻译视觉质量、效率、多功能性和可控性方面的优越性。相比相关先进方法，该框架实现了更高的性能和灵活性。

Conclusion: FBSDiff++提供了一个高效、灵活、可控的文本驱动图像到图像翻译框架，无需训练即可实现多种引导方式的图像转换，在频域视角下通过动态频率带替换实现了对图像翻译过程的精细控制。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测面临领域不变表示的挑战，特别是隐式非因果因子问题。现有基于领域对抗学习的方法主要关注学习领域不变信息，但往往忽视潜在的非因果因子。本文发现两个关键原因：1）基于领域判别器的方法受限于极其稀疏的领域标签；2）非因果因子过于隐式，传统对抗学习方法难以单独识别。

Method: 提出GB-DAL方法，包含两个核心模块：1）原型粒度球分割模块，从有限数据集中生成更密集的领域，类似于更细粒度的粒度球，揭示更多潜在非因果因子；2）模拟非因果因子模块，通过数据增强方式减少非因果因子的隐式性，促进GB-DAL的训练。

Result: 在多个基准测试上的对比实验表明，该方法在新型环境下实现了更好的泛化性能。

Conclusion: GB-DAL方法通过解决领域标签稀疏性和非因果因子隐式性问题，有效提升了开放世界目标检测的领域泛化能力。
...
TL;DR: 本文针对工业点云分割中安全关键部件（如减速器、阀门）被误分类的问题，提出了空间上下文约束方法，解决了类别极端不平衡（215:1）与几何模糊性双重危机，在保持头部类别精度的同时显著提升了尾部类别性能。


<details>
  <summary>Details</summary>
Motivation: 工业点云分割在数字孪生构建中面临安全关键部件系统性误分类的挑战。这些部件在训练数据中罕见，但与主导结构（如管道）具有相同的局部几何形状，形成了类别极端不平衡（215:1）与几何模糊性双重危机。

Method: 提出空间上下文约束方法，扩展了类别平衡损失框架，包含两个架构无关的机制：1）边界-CB：基于熵的约束，强调模糊边界；2）密度-CB：基于密度的约束，补偿扫描相关的变化。两者都作为即插即用模块集成，只需替换损失函数。

Result: 在Industrial3D数据集（6.1亿点，来自水处理设施）上，方法达到55.74% mIoU，尾部类别性能相对提升21.7%（29.59% vs. 24.32%基线），同时保持头部类别精度（88.14%）。具有原始共享模糊性的部件显著改善：减速器IoU从0%提升到21.12%；阀门相对提升24.3%。
...
TL;DR: 提出QA-ReID方法解决换装行人重识别问题，通过双分支特征融合和质量感知匹配机制提升跨服装场景下的识别性能


<details>
  <summary>Details</summary>
Motivation: 传统行人重识别在换装场景下面临严重挑战，因为服装变化会带来显著的外观变化，需要开发能够处理服装变化的鲁棒方法
...
Conclusion: QA-ReID通过双分支特征融合和质量感知匹配机制，有效解决了换装行人重识别的挑战，为处理服装变化提供了鲁棒的解决方案
...
TL;DR: SNR-Edit是一个无需训练的图像编辑框架，通过自适应噪声控制实现忠实潜在轨迹校正，解决了现有基于流生成模型的无反转编辑方法中因固定高斯噪声导致的轨迹偏差和结构退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于流生成模型的无反转图像编辑方法依赖固定高斯噪声构建源轨迹，导致轨迹动力学偏差，引起结构退化或质量损失。需要一种能够校正潜在轨迹偏差的方法，同时保持高保真结构保留。

Method: SNR-Edit采用结构感知噪声校正技术，将分割约束注入初始噪声中，将源轨迹的随机分量锚定到真实图像的隐式反转位置，减少源-目标传输过程中的轨迹漂移。这是一个轻量级修改，无需模型调优或反转操作。

Result: 在SD3和FLUX模型上，通过PIE-Bench和SNR-Bench评估，SNR-Edit在像素级指标和基于VLM的评分方面表现出色，同时每张图像仅增加约1秒的开销。

Conclusion: SNR-Edit通过自适应噪声控制实现了忠实潜在轨迹校正，解决了无反转图像编辑中的轨迹偏差问题，在保持高保真结构保留的同时，仅需极小计算开销，为基于流生成模型的图像编辑提供了有效解决方案。
...
TL;DR: 本文提出了一种名为对比频谱校正（CSR）的高效测试时防御方法，通过优化校正扰动来在频谱引导的对比目标下将输入与自然流形重新对齐，显著提升了视觉语言模型对抗样本的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（如CLIP）在零样本泛化方面表现出色，但它们对对抗样本仍然非常脆弱。现有的测试时防御方法无法提供足够的鲁棒性来抵御强攻击，且存在推理延迟高、任务特定适用性受限等问题。

Method: 通过研究对抗样本的内在特性，发现对抗样本在渐进频率衰减下表现出严重的特征不一致性，这归因于模型固有的频谱偏差。基于这一洞察，提出了对比频谱校正（CSR）方法，通过优化校正扰动，在频谱引导的对比目标下将输入与自然流形重新对齐，并采用输入自适应方式应用。

Result: 在16个分类基准测试上的广泛实验表明，CSR在对抗强AutoAttack时平均比现有最佳方法高出18.1%，且推理开销适中。此外，CSR在多种视觉任务中展现出广泛的适用性。
...
TL;DR: UniPCB是首个用于PCB质量检测的统一视觉语言基准，PCB-GPT模型在该基准上显著优于现有MLLMs，特别是在细粒度缺陷定位方面性能翻倍。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在复杂工业场景如PCB检测中表现不足，缺乏高质量、统一的视觉语言基准来定量评估模型性能，主要问题包括数据稀缺、数据集碎片化和标准化不一致。

Method: 提出UniPCB基准，通过系统化流程从不同来源收集和标准化数据，涵盖三种标注场景；开发PCB-GPT模型，使用新生成的指令数据集，采用模拟人类专家学习过程的新型渐进式课程训练方法。

Result: 在UniPCB基准上的评估显示，现有MLLMs在领域特定任务上表现不佳，而PCB-GPT建立了新的基准线，在细粒度缺陷定位方面性能比最强竞争对手提高一倍以上，在定位和分析方面具有显著优势。

Conclusion: UniPCB填补了PCB质量检测领域统一基准的空白，PCB-GPT展示了在复杂工业检测任务中的优越性能，为未来研究提供了数据、基准和模型资源。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在像素级感知方面通常需要复杂的任务特定设计或辅助组件，本文旨在探索是否可以通过简单方法解锁MLLM固有的低级感知能力。
...
TL;DR: 论文提出了视频连接任务和VC-Bench基准，包含1579个高质量视频，涵盖15个主要类别和72个子类别，通过VQS、SECS、TSS三个核心指标评估视频生成模型在连接不同片段时的表现。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成研究主要关注文本或图像条件，但在视频编辑和vlog制作等实际应用中，经常需要将不同的视频片段无缝连接起来。然而，缺乏标准化的评估基准阻碍了视频连接任务的发展。

Method: 提出了VC-Bench基准，包含从公共平台收集的1579个高质量视频，覆盖15个主要类别和72个子类别以确保多样性和结构性。该基准聚焦三个核心评估方面：视频质量分数(VQS)、起始-结束一致性分数(SECS)和过渡平滑度分数(TSS)。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型已经建立了文本和图像之间的特征联系，但3D模态数据（如点云和3D高斯）的引入使得3D相关任务的预训练成为可能。然而，提取3D模态特征和弥合不同模态之间的差距仍然存在挑战。
...
TL;DR: 本文系统评估了多种手工特征描述符在CIFAKE数据集上检测合成图像的性能，发现结合多种特征的混合特征集配合LightGBM分类器能取得最佳检测效果。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使得合成图像越来越逼真，引发了数字媒体真实性和信任问题。虽然深度学习方法在现有文献中占主导地位，但手工特征因其可解释性、效率和泛化能力仍然具有吸引力。本文旨在系统评估手工特征描述符在检测合成图像方面的有效性。

Method: 在CIFAKE数据集（包含50,000个训练样本和10,000个测试样本）上，系统评估了多种手工特征描述符：原始像素、颜色直方图、离散余弦变换(DCT)、方向梯度直方图(HOG)、局部二值模式(LBP)、灰度共生矩阵(GLCM)和小波特征。使用七种分类器进行基准测试，从逻辑回归到先进的梯度提升集成方法（LightGBM、XGBoost、CatBoost）。评估了三种配置：基线特征、高级特征和混合特征。

Result: LightGBM在所有分类器中表现最佳，在混合特征配置下取得了PR-AUC 0.9879、ROC-AUC 0.9878、F1分数0.9447和Brier分数0.0414的优异结果。性能在三种配置中单调提升，表明结合多样化的手工特征能带来显著收益。混合特征配置在校准和区分能力方面相比简单描述符有显著提升。

Conclusion: 精心设计的手工特征和集成学习在检测合成图像方面仍然具有重要价值，特别是在需要可解释性和计算效率的场景中。特征组合能显著提升检测性能，为合成图像检测提供了有效且可解释的解决方案。
...
TL;DR: 本文提出了一种多视图一致性框架来解决半监督域自适应中的类别预测偏差问题，通过去偏策略和伪负标签校正预测概率，结合跨域亲和性学习提升性能。


<details>
  <summary>Details</summary>
Motivation: 在半监督域自适应中，由于目标域标记样本有限，即使模型在平衡数据集上训练，特征空间中类别的内在相似性仍可能导致预测偏差，需要解决这一问题以提高模型性能。

Method: 提出多视图一致性框架：1）使用强增强数据训练的两个视图；2）根据模型预测性能校正类别预测概率的去偏策略；3）利用模型预测生成的伪负标签；4）跨域亲和性学习对齐不同域中相同类别的特征。

Result: 在DomainNet和Office-Home两个标准域自适应数据集上的实验结果表明，该方法优于竞争方法，证明了其有效性。

Conclusion: 该方法通过结合无监督域自适应和半监督学习，增强了模型适应性、降低了标注成本并提升了性能，对工业领域具有重要贡献。
...
TL;DR: ProMist-5K是一个专门用于模拟电影风格的数据集，包含20,000个高分辨率图像对，覆盖两种滤镜密度和焦距，专注于真实的光晕和高光扩散效果。


<details>
  <summary>Details</summary>
Motivation: Pro-Mist滤镜在电影摄影中能产生柔和光晕、降低对比度并创造独特氛围风格，但这些效果难以通过数字方式重现，因为光扩散行为复杂。需要专门的数据集来支持电影风格仿真。

Method: 在场景参考线性空间中构建物理启发式流程，包含20,000个高分辨率图像对，覆盖两种滤镜密度（1/2和1/8）和两种焦距（20mm和50mm）。使用多层模糊和精细调校的权重来模拟不同强度和扩散范围的光学扩散效果。

Result: 数据集在各种训练设置中表现良好，能够捕捉从细微到强烈的电影外观效果。提供了统一且可控的目标域，支持多种图像转换模型和学习范式。

Conclusion: ProMist-5K为电影风格图像转换提供了实用且物理基础扎实的资源，弥合了数字灵活性与传统镜头美学之间的差距。
...
Method: 创建了ASFW数据集，通过专业Photoshop工作流程生成1081对真实世界的阴影/无阴影图像；提出了Face Shadow Eraser (FSE)方法来展示数据集的有效性。

Result: 在ASFW数据集上训练的深度模型在真实世界条件下表现出改进的阴影去除效果，实验证明ASFW提升了人脸阴影去除模型的性能，为该任务设定了新标准。

Conclusion: ASFW数据集填补了合成与真实领域之间的空白，提供了逼真的阴影变化和准确的地面真值，为人脸阴影去除研究提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 单目相机在3D检测中存在深度模糊和恶劣条件下鲁棒性差的问题，雷达虽能补充但稀疏且分辨率低，需要有效的雷达-相机融合与深度估计策略

Method: 1) InstaRadar：基于实例分割引导的雷达扩展方法，利用预训练分割掩码增强雷达密度和语义对齐；2) 将预训练RCDPT集成到BEVDepth框架中替换其深度模块

Result: InstaRadar在雷达引导深度估计中达到SOTA，RCDPT集成持续提升3D检测性能，整体框架相比基线BEVDepth模型获得稳定增益

Conclusion: InstaRadar和显式深度监督在3D目标检测中有效，但框架落后于直接提取BEV特征的雷达-相机融合模型，未来将扩展InstaRadar为点云表示并集成带时间线索的专用雷达分支
...
TL;DR: Innovator-VL是一个科学多模态大语言模型，通过透明训练流程和数据高效设计，在少量数据下实现科学理解和推理能力，同时保持通用视觉任务性能。


<details>
  <summary>Details</summary>
Motivation: 针对当前科学多模态模型依赖大规模领域特定预训练和不透明流程的问题，研究旨在证明通过原则性训练设计和透明方法，可以用更少数据实现强大的科学智能。

Method: 提供完全透明、端到端可复现的训练流程，包括数据收集、清洗、预处理、监督微调、强化学习和评估；采用少于500万个精选样本，无需大规模预训练；通过原则性数据选择而非盲目扩展实现高效推理。

Result: 在多种科学任务上取得有竞争力的性能，同时在通用视觉、多模态推理和科学基准测试中表现良好，表明科学对齐可以集成到统一模型中而不损害通用能力。

Conclusion: 即使没有大规模数据，也可以构建高效、可复现且高性能的科学多模态模型，为未来研究提供了实用基础，强调原则性数据选择比盲目扩展更有效。
...
TL;DR: 该论文提出了一种针对医学图像分割的区域化课程学习策略，通过优先学习确定区域并逐步纳入不确定区域来减少梯度方差，同时引入帕累托一致性损失和模糊标记机制来平衡区域间的不确定性并稳定边界区域的优化。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中的不确定性分布不均匀，边界区域比内部区域具有更高的模糊性。传统的训练方法对所有像素一视同仁，导致在早期预测不可靠时优化不稳定，阻碍了向帕累托最优解的收敛。

Method: 1. 区域化课程学习策略：优先学习确定区域，逐步纳入不确定区域以减少梯度方差；2. 帕累托一致性损失：自适应重塑损失景观，平衡区域间不确定性，约束内部和边界区域的收敛动态；3. 模糊标记机制：在非边界区域保持二进制置信度，在边界附近实现平滑过渡，稳定梯度并扩展损失表面的平坦区域。

Result: 在脑转移瘤和非转移性肿瘤分割实验中，该方法在多种配置下均表现出持续改进，在所有肿瘤子区域中均优于传统的清晰集方法。

Conclusion: 通过区域化课程学习、帕累托一致性损失和模糊标记机制，能够有效处理医学图像分割中的非均匀不确定性，实现更稳定和优化的分割性能。
...
TL;DR: 研究人员开发了一个名为DermpathNet的大型开放获取皮肤病理学图像数据集，采用混合工作流程从PubMed Central中提取和分类图像，包含7,772张图像和166种诊断，用于教育、交叉参考和机器学习目的。


<details>
  <summary>Details</summary>
Motivation: 临床医生和皮肤病理学学员在获取高质量、开放获取的皮肤病理学图像数据集用于学习和交叉参考方面面临共同挑战，需要建立全面的开放获取数据集来支持教育、交叉参考和机器学习应用。

Method: 采用混合工作流程：1) 使用特定关键词从PubMed Central(PMC)存储库中提取相关图像；2) 通过结合深度学习图像模态分类和图像标题分析的新颖混合方法对图像进行分类；3) 由委员会认证的皮肤病理学家进行审查和标注。

Result: 验证了651张手动标注图像：深度学习方法的F分数为89.6%，基于关键词检索方法的F分数为61.0%，混合方法的F分数为90.4%。最终检索到超过7,772张图像，涵盖166种诊断，创建了完全标注的数据集。使用该数据集测试发现OpenAI的当前图像分析算法不足以分析皮肤病理学图像。

Conclusion: 成功开发了大型、同行评审、开放获取的皮肤病理学图像数据集DermpathNet，并建立了半自动化的策展工作流程，为教育、交叉参考和机器学习研究提供了有价值的资源。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个综合性的肺结节分析流程，优先考虑敏感性，同时减少标注者的候选负担，确保在不同实践中的准确性和泛化性。

Method: 使用在公共数据集上训练的多个开源模型，开发了Tri-Reader三阶段工作流程，包括肺部分割、结节检测和恶性分类。在多个内部和外部数据集上评估，并与专家标注和数据集提供的参考标准进行比较。

Result: 论文提到在多个内部和外部数据集上进行了评估，但摘要中没有提供具体的性能指标或比较结果。

Conclusion: Tri-Reader是一个免费可用的综合性肺结节分析流程，通过三阶段工作流程集成了关键功能，旨在提高敏感性并减少标注负担，并在多个数据集上验证了其准确性和泛化性。
...
TL;DR: 本文提出了X-AIGD基准，用于可解释的AI生成图像检测，提供像素级分类的人工痕迹标注，发现现有检测器很少依赖可感知的人工痕迹，但通过显式对齐模型注意力可以提高可解释性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像检测方法主要依赖二元分类，缺乏可解释的证据支持其决策。现有基准虽然包含大量合成图像，但缺乏对人工痕迹多样性的覆盖和详细的局部标注，限制了检测器的可解释性评估。

Method: 提出了X-AIGD基准，提供像素级、分类的人工痕迹标注，涵盖低层失真、高层语义和认知层反事实三个层面。使用该基准对现有AIGI检测器进行深入分析，并探索通过显式对齐模型注意力与人工痕迹区域来提高可解释性的方法。

Result: 研究发现：(1) 现有AIGI检测器对可感知的人工痕迹依赖微乎其微，即使在最基本的失真层面；(2) 虽然可以训练检测器识别特定人工痕迹，但它们仍主要依赖不可解释的特征；(3) 显式对齐模型注意力与人工痕迹区域可以提高检测器的可解释性和泛化能力。

Conclusion: X-AIGD基准为可解释的AI生成图像检测提供了重要工具，揭示了现有检测器的局限性，并展示了通过注意力对齐提高可解释性的潜力，推动了更可信、可解释的AIGI检测方法的发展。
...
Method: 提出双师生框架DSTCS，结合CNN和SAM（Segment Anything Model）架构，采用协同学习机制。包含专门针对边界处理优化的数据增强策略和新型损失函数。

Result: 在MICCAI 2023和2024 PSFH分割基准测试中，该方法展现出卓越的鲁棒性，显著优于现有技术，为临床实践提供了可靠的分割工具。

Conclusion: DSTCS框架通过整合CNN和SAM的优势，有效解决了PSFH分割中的挑战，为产程监测提供了准确可靠的分割解决方案。
...
TL;DR: Dyn-HSI是首个动态人-场景交互认知架构，赋予虚拟人类视觉、记忆和控制三大类人组件，在静态和动态场景中均能生成高质量交互动作
...
Method: 提出Dyn-HSI认知架构，包含三个类人组件：1)视觉组件（动态场景感知导航），持续感知环境变化并自适应预测下一个路径点；2)记忆组件（分层经验记忆），存储和更新训练期间积累的经验数据，用于推理时的上下文感知运动引导；3)控制组件（人-场景交互扩散模型），基于多模态输入生成高保真交互动作。还构建了动态基准数据集Dyn-Scenes。

Result: 在静态和动态场景中，Dyn-HSI方法始终优于现有方法，能够生成高质量的人-场景交互动作。通过广泛的定性和定量实验验证了方法的有效性。

Conclusion: Dyn-HSI是首个动态人-场景交互认知架构，通过模拟人类的视觉、记忆和控制能力，成功解决了动态场景中的人-场景交互生成问题，在静态和动态设置下均表现出色。
...
TL;DR: 该论文针对视频生成中自回归模型采样策略的局限性，提出了一种基于熵引导的自适应采样方法ENkG，通过根据token预测分布的熵值动态调整候选token数量，解决了静态top-k/top-p策略在视频生成中的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 自回归架构在LLMs中取得了显著成功，但在视频生成中存在挑战。语言token具有高语义密度和低冗余度，而视频token具有低语义密度和高时空冗余度。静态的top-k/top-p策略在视频解码器中效果不佳：对于低不确定性区域（静态背景）会引入不必要的随机性，对于高不确定性区域（前景对象）则会陷入早期错误。预测错误会随着生成更多帧而累积，最终严重降低长时程生成质量。

Method: 提出Entropy-Guided k-Guard (ENkG)采样策略，这是一种简单而有效的方法，通过量化每个token预测分布的熵值来适应token的离散程度。ENkG使用自适应token候选数量：对于低熵区域，使用较少的候选token来抑制冗余噪声并保持结构完整性；对于高熵区域，使用更多的候选token来缓解错误累积。该方法与模型无关，无需训练，且增加的开销可忽略不计。

Result: 实验表明，与静态top-k/top-p策略相比，ENkG在感知质量和结构稳定性方面带来了一致的改进。

Conclusion: ENkG采样策略有效解决了视频生成中自回归模型的采样问题，通过熵引导的自适应机制，在保持结构完整性和提高生成多样性之间取得了更好的平衡，为视频生成任务提供了一种高效且通用的解决方案。
...
TL;DR: 针对SIGGRAPH Asia 3DGS快速重建挑战赛，开发了一个能在1分钟内收敛的3DGS重建流程，采用两阶段策略分别处理SLAM噪声轨迹和COLMAP精确位姿，最终获得比赛第一名（PSNR 28.43）。


<details>
  <summary>Details</summary>
Motivation: SIGGRAPH Asia 3DGS快速重建挑战赛要求在一分钟内完成高质量3D高斯重建，但面临两个不同场景：初赛使用SLAM生成的噪声相机轨迹，决赛使用COLMAP精确位姿。需要开发一个能同时处理这两种异构设置的鲁棒解决方案。

Method: 采用两阶段策略：1）初赛阶段：针对SLAM噪声轨迹，使用反向逐高斯并行优化、紧凑前向溅射、负载均衡分块、基于锚点的神经高斯表示（减少可学习参数）、从单目深度和部分前馈3DGS模型初始化、全局位姿优化模块；2）决赛阶段：针对COLMAP精确位姿，禁用位姿优化、从神经高斯回归标准3DGS（消除MLP推理开销）、引入多视图一致性引导的高斯分裂、使用深度估计器监督渲染深度。

Result: 该方法在严格的一分钟时间预算下实现了高质量重建，最终在比赛中获得第一名，PSNR达到28.43。

Conclusion: 通过针对不同相机位姿质量的两阶段优化策略，成功开发了一个能在1分钟内收敛的高质量3D高斯重建系统，在SIGGRAPH Asia挑战赛中取得了最佳性能，证明了该方法在快速3D重建任务中的有效性。
...
TL;DR: Cor2Vox是一个基于大脑皮层结构先验的生成框架，用于合成神经影像数据，通过皮层表面引导3D形状到图像的扩散过程，实现解剖学上一致的脑部MRI合成。


<details>
  <summary>Details</summary>
Motivation: 解决真实神经影像数据集的局限性：罕见表型稀缺、扫描仪间域偏移、纵向覆盖不足；现有生成模型依赖标签或文本等弱条件信号，缺乏解剖学基础，常产生生物学上不合理的输出。

Method: 引入Cor2Vox框架，利用高分辨率皮层表面引导3D形状到图像的布朗桥扩散过程；开发基于33,000多个UK Biobank扫描的大规模皮层形态统计形状模型；通过皮层表面重建和全脑分割质量验证保真度。

Result: 在传统图像质量指标、高级皮层表面重建和全脑分割质量方面优于多个基线方法；在三个应用中（解剖一致合成、渐进灰质萎缩模拟、前颞叶痴呆扫描与公共数据集协调）表现出亚体素级别的精细皮层形态保持能力。

Conclusion: Cor2Vox能够生成解剖学上一致且生物学合理的脑部MRI，对皮层几何和疾病表型变化具有显著鲁棒性，无需重新训练即可应用于多种神经影像数据生成任务。
...
Method: 采用两种互补策略：1）增强输入密度：使用自回归整合器将文本指令重新表述为密集潜在查询，注入高级语义稳定性来约束退化信号；2）修剪输出分布：将策略强化学习直接集成到扩散恢复循环中，将人类偏好转化为可微约束，惩罚随机偏差。

Result: 在合成和真实世界基准测试中达到最先进的性能，偏好对齐策略显著降低解决方案熵，为可靠和确定性盲恢复建立了稳健路径。

Conclusion: Pref-Restore通过整合语义逻辑和纹理生成，有效解决了盲人脸恢复中的信息不对称问题，实现了确定性、偏好对齐的恢复效果，为可靠盲恢复提供了新途径。
...
TL;DR: 提出基于超宽带传感器稀疏成对距离测量的全身3D运动捕捉系统，无需外部摄像头，适用于非受控户外环境


<details>
  <summary>Details</summary>
Motivation: 传统光学或惯性运动捕捉系统受环境限制（光照、磁场干扰），需要外部设备，难以在非受控户外环境中使用。需要一种形状不变、环境鲁棒的运动捕捉方法

Method: 使用身体安装的超宽带传感器进行飞行时间测距，获取稀疏成对距离测量。提出Wild-Poser（WiP）架构，基于Transformer的紧凑实时模型，直接从噪声或损坏的距离测量预测3D关节位置，然后通过学习方法重建关节旋转

Result: WiP实现低关节位置误差，在实时操作中准确重建人类和动物主体的3D运动。系统泛化能力强，适应不同形态主体（包括非人类物种），无需个体身体测量或形状拟合

Conclusion: 该系统展示了在真实世界环境中实现可扩展、低成本、通用运动捕捉的潜力，为户外和非受控环境下的运动分析提供了新解决方案
...
Method: 在意大利西西里火山岛（Vulcano）记录多个序列，采集包含玄武岩或富铁岩石、古老熔岩通道地质构造、干燥植被和水体等多种环境、纹理和地形的数据

Result: 发布了S3LI Vulcano数据集（rmc.dlr.de/s3li_dataset）和开源工具包（github.com/DLR-RM/s3li-toolkit），提供地面真实位姿生成和地点识别任务标注样本准备工具

Conclusion: 该数据集为SLAM和地点识别算法的开发和基准测试提供了丰富的多模态数据资源，特别适合处理复杂自然环境下的定位和识别任务
...
TL;DR: MaDiS是一个基于掩码扩散的语言模型，用于手语生成，通过双向依赖建模和并行多token生成解决了传统自回归模型的局限性，显著提升了生成质量和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成方法采用自回归语言模型，存在单向上下文建模和逐token推理速度慢的问题，需要更高效、表达能力更强的模型来改善手语生成质量。

Method: 提出MaDiS模型，采用掩码扩散框架实现双向依赖建模和并行生成；引入三层次跨模态预训练方案（token、潜在空间、3D物理空间）；设计带时间检查点的解掩码策略降低组合复杂度；开发混合部件嵌入层融合不同部件token信息。

Result: 在CSL-Daily、Phoenix-2014T和How2Sign数据集上，MaDiS在DTW误差和两个新指标SiBLEU、SiCLIP上均取得优越性能，同时推理延迟降低近30%。

Conclusion: MaDiS通过掩码扩散框架有效解决了手语生成中的双向依赖建模和推理效率问题，三层次预训练和解掩码策略显著提升了模型性能，为手语生成提供了更高效、表达能力更强的解决方案。
...
TL;DR: QuaMo提出了一种基于四元数微分方程（QDE）的3D人体运动捕捉方法，通过四元数状态空间模型和自适应加速度增强，解决了传统方法中欧拉角不连续性问题，实现了更准确、连续的运动估计。


<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的3D人体运动捕捉方法常忽略帧间时间一致性，导致运动不自然且抖动。现有运动学方法依赖欧拉角，存在不连续性，特别是在在线设置中无法进行轨迹优化。四元数无此不连续性问题，能产生连续姿态过渡。

Method: 提出QuaMo方法，使用四元数微分方程（QDE）进行人体运动学捕捉。采用状态空间模型描述实时运动学估计，以四元数为状态，QDE描述四元数速度。通过元PD控制器计算相应角加速度，并引入自适应加速度增强机制，在人体快速变换姿态时调节控制信号。在四元数单位球约束下求解QDE。

Result: 实验结果表明，QuaMo的QDE公式与加速度增强能准确估计3D人体运动学，无间断且最小化不合理性。在Human3.6M、Fit3D、SportsPose和AIST等多个数据集上优于现有最先进方法。

Conclusion: QuaMo通过四元数微分方程和自适应加速度增强，有效解决了传统运动捕捉中的不连续性问题，实现了更准确、连续的3D人体运动估计，在多个基准数据集上表现优异。
...
Result: 在保留测试集上，框架显示出中等到强的几何不对称度量结构相关性，生成模型正确捕捉了形态变化方向，但生物变异性限制了绝对精度
...
<details>
  <summary>Details</summary>
Motivation: 现有视频-音频联合嵌入方法（如CAVP）虽然能建模模态间的语义和时间对应关系，但对视频和音频信号的多尺度密集特性建模不足，对应关系跨越从细粒度到粗粒度的时空结构，这在现有框架中未得到充分利用。
...
TL;DR: 大规模评估医疗影像差分隐私分析中的初始化策略，发现领域特定的监督预训练在隐私保护下表现最佳，DINOv3自监督初始化优于ImageNet初始化但不及领域特定方法。
...
Method: 使用超过80万张胸部X光图像作为代表性基准，采用最先进的ConvNeXt模型和DP-SGD训练，在现实隐私保护机制下比较三种初始化策略：非领域特定的监督ImageNet初始化、非领域特定的自监督DINOv3初始化、以及领域特定的监督预训练（在MIMIC-CXR数据集上）。在五个外部数据集上进行评估，涵盖不同机构和采集设置。

Result: DINOv3初始化在DP下相对于ImageNet初始化持续改善诊断效用，但仍不及领域特定的监督预训练，后者性能最接近非隐私基线。初始化选择强烈影响人口统计公平性、跨数据集泛化能力，以及在隐私约束下对数据规模和模型容量的鲁棒性。

Conclusion: 初始化策略是差分隐私医疗影像分析中效用、公平性和泛化能力的关键决定因素，领域特定的监督预训练在隐私保护下表现最佳。
...
TL;DR: 本文提出了面向城市治理的低空智能首个管理导向多模态基准数据集GovLA-10K和统一视觉语言推理框架GovLA-Reasoner，旨在解决现有方法难以支持实际城市治理中管理导向异常理解的问题。


<details>
  <summary>Details</summary>
Motivation: 现有以物体为中心的感知范式和松散的视觉语言流程难以支持真实城市治理所需的管理导向异常理解，需要建立专门面向治理需求的多模态基准和推理框架。

Method: 1) 构建GovLA-10K基准数据集，围绕功能显著目标设计，提供基于观察的可执行管理建议；2) 提出GovLA-Reasoner框架，通过高效特征适配器隐式协调视觉检测器与大语言模型之间的判别性表征共享。

Result: 实验表明该方法显著提升了性能，同时避免了任何任务特定组件的微调需求，为管理感知的低空视觉语言系统提供了新视角和基础。

Conclusion: 该工作为未来管理感知的低空视觉语言系统研究提供了新的视角和基础，通过专门设计的基准和统一推理框架，有效支持城市治理中的管理导向异常理解需求。
...
<details>
  <summary>Details</summary>
Motivation: 持续学习需要平衡三个相互竞争的目标：保留预训练知识、保持已学任务序列的知识、维持获取新知识的可塑性。现有方法难以有效平衡这些目标。
...
Conclusion: KeepLoRA提供了一种简单而有效的持续学习方法，通过参数空间分析指导的梯度投影策略，在预训练视觉语言模型中实现了知识保留、任务保持和可塑性的良好平衡。
...
TL;DR: 本文提出了一种新的感知图像相似度度量方法，包含纹理差异和色度差异两个部分，在包含复杂形状和颜色失真的数据集上表现优于现有方法，并能提供视觉解释。
...
Method: 提出包含两个部分的感知度量：1) 使用Earth Mover's Distance评估两幅图像纹理的差异；2) 在Oklab感知色彩空间中评估色度差异。

Result: 在Berkeley-Adobe Perceptual Patch Similarity数据集上评估，新度量方法在包含形状失真的图像上表现优于现有方法，证实了其更好的感知能力。

Conclusion: 新提出的感知度量方法在评估图像相似度时优于现有技术，特别是在处理形状失真时，并能提供视觉解释使相似度评估更加透明和可解释。
...
TL;DR: SharpNet是一种改进的MLP架构，能够编码具有用户定义尖锐特征（如边缘和角点）的函数，通过辅助特征函数和可微优化实现精确的C^0连续性控制。


<details>
  <summary>Details</summary>
Motivation: 传统多层感知器（MLP）在函数逼近中产生全局平滑输出，难以表示具有规定C^0尖锐特征（如不连续梯度）的函数，通常需要临时后处理。

Method: 提出SharpNet架构，通过引入辅助特征函数（定义为具有跳跃诺伊曼边界条件的泊松方程解），使用局部积分评估，可微优化特征位置和MLP参数。

Result: 在2D问题和3D CAD模型重建任务中，SharpNet能准确恢复尖锐边缘和角点，同时保持特征外的平滑行为，优于现有方法。

Conclusion: SharpNet提供了一种可微且可控的方法来编码具有规定尖锐特征的函数，在保持平滑性的同时精确控制C^0连续性，优于现有基线方法。
...
Result: 在五个挑战性基准测试中取得最先进或极具竞争力的结果，在Video-Holmes上达到42.7%（超过GPT-4o），在推理和通用视频理解任务上均获得一致提升。消融研究验证了归因信号的互补作用和目标token级更新的鲁棒性。
...
TL;DR: DSVM-UNet提出了一种简单有效的双自蒸馏方法，用于改进Vision Mamba UNet在医学图像分割中的性能，无需复杂架构设计，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有Vision Mamba UNet方法主要关注通过复杂架构设计来增强语义特征感知能力，本文旨在通过简单的自蒸馏方法改进模型性能，避免复杂的架构设计。

Method: 提出DSVM-UNet（双自蒸馏VM-UNet），开发了双自蒸馏方法，在全局和局部两个层次对齐特征，从而提升模型性能。

Result: 在ISIC2017、ISIC2018和Synapse基准测试上的大量实验表明，该方法达到了最先进的性能，同时保持了计算效率。

Conclusion: 通过简单的双自蒸馏方法可以有效改进Vision Mamba UNet在医学图像分割中的性能，无需复杂架构设计，在多个基准测试中表现出色。
...
TL;DR: SWEET是一个自监督框架，通过基于约束的预训练实现视觉任务中的可扩展初始化，支持不同深度和宽度的架构。
...
Method: 学习共享权重模板和特定尺寸的权重缩放器，采用Tucker分解，支持灵活适应不同深度和宽度的架构。引入宽度随机缩放增强宽度扩展的灵活性。

Result: 在分类、检测、分割和生成任务上的广泛实验表明，SWEET在初始化可变尺寸视觉模型方面达到了最先进的性能。

Conclusion: SWEET框架通过约束预训练实现了可扩展初始化，支持灵活适应不同架构，在多个视觉任务上表现出色。
...
TL;DR: PaW-ViT是一种基于解剖知识的预处理方法，通过将视觉变换器中的矩形token边界与耳朵特征边界对齐，提高耳朵识别的鲁棒性和一致性。


<details>
  <summary>Details</summary>
Motivation: 视觉变换器中常用的矩形token会包含识别对象之外的信息，这会影响性能。特别是在耳朵生物识别中，耳朵形态的多样性与变换器架构的位置敏感性之间存在脱节。

Method: 提出PaW-ViT（基于补丁的扭曲视觉变换器），这是一种基于解剖知识的预处理方法。通过准确地将token边界与检测到的耳朵特征边界对齐，并将特征边界与自然耳朵曲率对齐，从而产生更一致的token表示。

Result: 实验证实了PaW-ViT在各种ViT模型（ViT-T、ViT-S、ViT-B、ViT-L）上的有效性，并对形状、大小和姿态变化表现出合理的对齐鲁棒性。

Conclusion: PaW-ViT解决了耳朵生物识别形态变化与变换器架构位置敏感性之间的脱节问题，为认证方案提供了一个可能的途径。
...
TL;DR: GeoDiff3D是一个自监督的3D场景生成框架，使用粗糙几何作为结构锚点，通过几何约束的2D扩散模型提供纹理丰富的参考图像，无需严格的多视角一致性，能高效生成高质量3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景生成方法存在两个主要问题：间接2D-to-3D重建和直接3D生成方法都受到弱结构建模和严重依赖大规模真实监督的限制，导致结构伪影、几何不一致和复杂场景中高频细节退化。需要更高效、高质量且不依赖大量标注数据的解决方案。

Method: 1) 使用粗糙几何作为结构锚点；2) 采用几何约束的2D扩散模型生成纹理丰富的参考图像；3) 不要求扩散生成参考图像的多视角一致性，对噪声和不一致指导保持鲁棒性；4) 引入体素对齐的3D特征聚合和双重自监督机制；5) 降低对标注数据的依赖，训练计算成本低。

Result: 在具有挑战性的场景上进行广泛实验，相比现有基线方法，GeoDiff3D展现出更好的泛化能力和生成质量，能够快速生成高质量的3D场景，为可访问和高效的3D场景构建提供了实用解决方案。

Conclusion: GeoDiff3D通过结合粗糙几何锚点和几何约束的2D扩散模型，解决了现有3D生成方法的结构建模弱点和监督依赖问题，实现了高效、高质量的自监督3D场景生成，具有实际应用价值。
...
TL;DR: 该研究评估了基于扩散的耳部修复技术作为预处理辅助手段，用于减轻耳部配饰遮挡对基于transformer的耳部识别系统性能的影响。


<details>
  <summary>Details</summary>
Motivation: 耳部配饰（如耳环、耳机）造成的遮挡在非约束成像条件下会严重影响基于耳部的生物特征识别系统性能，需要有效方法来处理这些遮挡问题。

Method: 使用基于扩散的耳部修复技术作为预处理辅助，给定输入耳部图像和自动生成的配饰掩码，修复模型通过合成缺失像素重建干净且解剖学上合理的耳部区域，同时保持关键耳部结构（耳轮、对耳轮、耳甲、耳垂）的局部几何一致性。

Result: 实验表明，基于扩散的修复技术可以有效作为预处理辅助手段，减轻耳部配饰遮挡，提高基于transformer的耳部识别系统的整体识别性能。

Conclusion: 基于扩散的耳部修复技术是处理耳部配饰遮挡问题的有效预处理方法，能够改善耳部生物特征识别系统在非约束条件下的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在保留细粒度视觉信息方面存在局限，导致多模态理解能力粗糙。主要原因是现有训练范式存在文本主导的优化偏差，将视觉信号仅视为被动条件输入而非监督目标

Method: 提出Youtu-VL框架，采用视觉-语言统一自回归监督范式，将视觉token直接整合到预测流中，对视觉细节和语言内容应用统一的自回归监督。该范式还可扩展到视觉中心任务

Result: 广泛的实证评估表明，Youtu-VL在通用多模态任务和视觉中心任务上都取得了有竞争力的性能

Conclusion: Youtu-VL为开发全面的通用视觉智能体建立了坚实基础，通过将优化目标从"视觉作为输入"转变为"视觉作为目标"，显著提升了细粒度视觉理解能力
...
Method: 提出QSTar方法：1）查询引导的时空频交互，结合问题引导线索，利用音频信号的频域特性以及空间和时间感知；2）查询上下文推理块（QCR），受提示启发，引导模型更精确地关注语义相关的音频和视觉特征。

Result: 在多个AVQA基准测试上的广泛实验表明，该方法显著优于现有的音频QA、视觉QA、视频QA和AVQA方法，取得了显著的性能提升。

Conclusion: 提出的QSTar方法和QCR块有效解决了现有AVQA方法的局限性，通过更好地整合问题引导线索和利用音频频域特性，显著提升了音频-视觉问答性能。
...
TL;DR: HexFormer是一种用于图像分类的双曲视觉Transformer，通过指数映射聚合注意力机制，在双曲几何中建模层次结构，相比欧几里得基线获得性能提升，梯度更稳定。


<details>
  <summary>Details</summary>
Motivation: 图像、文本、图等多模态数据常包含层次和关系结构，这些结构在欧几里得几何中难以建模。双曲几何为表示此类结构提供了自然框架，因此研究双曲几何如何增强视觉Transformer架构。
...
Result: 在多个数据集上的实验显示，HexFormer相比欧几里得基线和先前双曲ViTs获得一致性能提升，混合变体实现最强整体结果。双曲模型展现更稳定的梯度和对预热策略的敏感性降低，训练更稳健高效。
...
TL;DR: 首个用于3D手部重建的上下文学习框架EgoHandICL，通过视觉语言模型引导的示例检索、多模态上下文分词器和MAE架构，在自中心视角下显著提升语义对齐、视觉一致性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自中心视角下的3D手部重建面临深度模糊、自遮挡和复杂手物交互等挑战。现有方法通过扩大训练数据或添加辅助线索来缓解这些问题，但在未见场景中表现不佳。

Method: 提出EgoHandICL框架：1) 基于视觉语言模型的互补示例检索；2) 为多模态上下文定制的ICL分词器；3) 基于掩码自编码器的架构，使用手部引导的几何和感知目标进行训练。

Result: 在ARCTIC和EgoExo4D数据集上的实验显示，该方法在3D手部重建方面持续优于最先进方法。同时展示了真实世界的泛化能力，并通过重建的手部作为视觉提示提升了EgoVLM的手物交互推理能力。

Conclusion: EgoHandICL是首个用于3D手部重建的上下文学习框架，通过多模态上下文理解和示例引导，在挑战性自中心条件下显著提升了重建质量和鲁棒性，并展示了在真实世界应用中的潜力。
...
TL;DR: SONIC提出了一种连续频谱参数化的卷积算子，使用少量共享的定向选择性组件，实现了全局感受野和跨分辨率自适应，在多个任务上优于传统卷积、注意力机制和频谱架构。


<details>
  <summary>Details</summary>
Motivation: 传统CNN使用固定大小的卷积核扫描局部区域，难以捕获全局上下文或长距离依赖；ViT虽然提供全局连接但缺乏空间归纳偏置，依赖显式位置编码且受限于初始补丁大小。需要一种既结构化又全局的表征方法。

Method: 提出SONIC（频谱定向神经不变卷积），一种连续频谱参数化方法，使用少量共享的定向选择性组件来建模卷积算子。这些组件在完整频域上定义平滑响应，产生全局感受野和跨分辨率自然适应的滤波器。

Result: 在合成基准测试、大规模图像分类和3D医学数据集上，SONIC显示出对几何变换、噪声和分辨率变化的更强鲁棒性，在参数数量少一个数量级的情况下，匹配或超越了卷积、基于注意力的和先前频谱架构的性能。

Conclusion: 连续、定向感知的频谱参数化为传统空间和频谱算子提供了一种原则性且可扩展的替代方案，能够同时获得结构化表征和全局上下文信息。
...
TL;DR: VGGT-SLAM 2.0是一个实时RGB前馈SLAM系统，通过改进因子图设计解决漂移和平面退化问题，利用VGGT的注意力层进行图像检索验证，在多种环境中实现实时性能，在TUM数据集上比VGGT-SLAM减少约23%的位姿误差。


<details>
  <summary>Details</summary>
Motivation: 改进VGGT-SLAM系统，解决其存在的15自由度漂移和平面退化问题，同时处理VGGT在未知相机内参下的重建模糊性，提升SLAM系统的精度和鲁棒性。

Method: 1. 设计新的因子图来消除高维漂移和平面退化；2. 利用VGGT的注意力层进行图像检索验证，无需额外训练即可拒绝误匹配并完成更多闭环；3. 在多种环境（室内公寓、办公室、4200平方英尺谷仓）进行实验，并在Jetson Thor上实现实时在线运行。

Result: 在TUM数据集上达到最高精度，比VGGT-SLAM减少约23%的位姿误差；能够轻松适应开放集物体检测；在Jetson Thor上实现实时在线性能；在多种复杂环境中表现良好。

Conclusion: VGGT-SLAM 2.0显著改进了原始系统，通过创新的因子图设计和注意力层利用，解决了漂移、平面退化和重建模糊性问题，实现了更高的精度和实时性能，适用于多种实际应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态模型在多语言处理方面取得进展，但处理阿拉伯文字（特别是艺术化和风格化的书法形式）的能力尚未得到充分探索。阿拉伯书法融合了语言意义与艺术形式，现有系统难以处理其复杂的笔画模式、密集连字和风格变化。

Method: 创建DuwatBench基准数据集，包含1,272个精心策划的样本，涵盖约1,475个独特词汇，分布在六种古典和现代书法风格中，每个样本都配有句子级检测标注。使用该数据集评估了13个领先的阿拉伯语和多语言多模态模型。

Result: 评估显示，虽然这些模型在清晰文本上表现良好，但在处理书法变化、艺术扭曲和精确的视觉-文本对齐方面存在困难。模型在处理阿拉伯书法的复杂视觉特征时表现不佳。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的ASR系统通常为每种语言训练独立的连接器，忽略了语言之间的相关性，导致参数效率低下，且无法充分利用语言家族内的相似性。

Method: 提出基于语言家族成员关系的连接器共享策略，为每个语言家族使用一个共享连接器，将冻结的语音编码器与预训练LLM连接起来，在两种多语言LLM和两个真实世界语料库上进行验证。

Result: 家族化连接器显著减少了参数数量，同时改善了跨领域的泛化性能，在精心策划和众包语音数据上都表现出有效性。

Conclusion: 基于语言家族的连接器共享策略为多语言ASR部署提供了实用且可扩展的解决方案，在保持性能的同时提高了参数效率。
...
TL;DR: 提出新的校准探测框架评估语言模型关系知识的可靠性，发现大多数模型（特别是掩码语言模型）过度自信，最佳校准来自考虑重述不一致性的置信度估计。


<details>
  <summary>Details</summary>
Motivation: 现有知识探测方法通过准确率、精确度等指标评估模型能力，但未能考虑模型的可靠性（反映在其置信度校准上）。需要开发能评估关系知识可靠性的校准探测框架。

Method: 提出关系知识校准探测框架，涵盖三种模型置信度模态：内在置信度、结构一致性和语义基础。对10个因果语言模型和6个掩码语言模型进行广泛分析。

Result: 大多数模型（特别是掩码目标预训练的模型）过度自信；考虑陈述重述不一致性的置信度估计产生最佳校准分数；即使最大的预训练模型也无法准确编码语言置信度表达式的语义。

Conclusion: 需要超越传统准确率指标来评估语言模型的知识可靠性，校准探测框架揭示了模型置信度校准的重要缺陷，为改进模型可靠性提供了方向。
...
TL;DR: 本文提出了一种基于数据中心的投机解码训练加速方法SFDD，通过筛选训练数据中能产生平坦预测分布的样本，仅用50%数据实现2倍以上的训练加速，同时保持推理速度与全数据集基线相差不到4%。


<details>
  <summary>Details</summary>
Motivation: 投机解码是加速大语言模型推理的关键技术，但通常需要在大数据集上训练草稿模型。现有方法没有认识到并非所有训练样本对投机解码的接受率贡献相同，需要更高效的训练方法。

Method: 提出平坦度（flatness）指标来量化样本价值，发现能诱导目标模型产生平坦预测分布的token比产生尖锐分布的token更有价值。基于此提出样本级平坦度数据集蒸馏（SFDD）方法，筛选保留最有价值的训练样本。

Result: 在EAGLE框架上的实验表明，SFDD仅使用50%的数据就能实现超过2倍的训练加速，同时最终模型的推理加速与全数据集基线相比差异在4%以内。

Conclusion: 这项工作提出了一种有效的、以数据中心的方法，显著提高了投机解码的训练效率，为加速大语言模型推理提供了新的数据筛选策略。
...
TL;DR: 该论文介绍了BabyReasoningBench，这是一个基于发展心理学经典范式的婴儿推理基准测试，用于评估在儿童导向语言数据上训练的婴儿语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型推理评估主要针对成人设计，假设了广泛的世界知识、复杂指令跟随和成熟语用能力，这与在儿童发展合理输入（如儿童导向语言和早期儿童叙事）上训练的婴儿语言模型不匹配，无法揭示在这种约束下哪些推理能力会出现。

Method: 研究者引入了BabyReasoningBench，这是一个GPT-5.2生成的基准测试，包含19个基于发展心理学经典范式的推理任务，涵盖心理理论、类比和关系推理、因果推理和干预选择，以及核心推理原语（已知会受到记忆和语用学影响）。使用两个基于GPT-2的婴儿语言模型（分别在1000万和1亿儿童导向语言文本上预训练）进行评估。

Result: 研究发现，两个婴儿语言模型整体表现较低但不均衡，在不同任务家族中存在分离：模型规模扩展改善了多个因果和物理推理任务的表现，而信念归因和语用敏感任务仍然具有挑战性。

Conclusion: BabyReasoningBench提供了一个发展心理学基础的分析框架，用于评估儿童类似训练分布支持哪些类型的推理能力，并测试这些能力如何出现的机制假设。
...
TL;DR: LLMs在程序终止性预测方面表现优异，GPT-5和Claude Sonnet-4.5接近SV-Comp 2025最佳工具水平，但无法提供有效证明且长程序性能下降。


<details>
  <summary>Details</summary>
Motivation: 图灵停机问题的不可判定性使得传统验证工具只能近似处理终止性问题，且通常受限于特定架构和编程语言。随着大语言模型的成功，研究者希望探索LLMs能否可靠预测程序终止性。

Method: 使用SV-Comp 2025终止性类别的多样化C程序集评估LLMs，比较GPT-5、Claude Sonnet-4.5和Code World Model等模型在程序终止性预测上的表现。

Result: LLMs在程序终止性预测方面表现突出：GPT-5和Claude Sonnet-4.5接近排名第一的工具（使用测试时缩放），CWM接近排名第二的工具。但LLMs通常无法提供有效的证明见证，且随着程序长度增加性能下降。

Conclusion: LLMs能够有效预测程序终止性，但在提供证明和长程序处理方面存在局限。这些发现为LLMs在不可判定问题推理方面的潜力提供了新视角，值得进一步研究。
...
TL;DR: LLMs可通过绕过安全防护、重新解释NLP论文并利用其漏洞来生成有害研究提案，但不同LLM评估结果差异显著，无法作为可靠评估者


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注LLMs在促进科学发现方面的潜力，但忽视了LLMs可能被滥用于生成有害研究的风险，特别是通过重新利用开放科学资源进行恶意目的

Method: 开发端到端管道：1) 通过说服式越狱绕过LLM安全防护；2) 重新解释NLP论文，识别并重新利用其资源（数据集、方法、工具），利用其漏洞；3) 使用三维评估框架（危害性、滥用可行性、技术合理性）评估提案安全性

Result: LLMs能够通过重新利用伦理设计的开放资源生成有害提案；但不同LLM评估结果差异显著：GPT-4.1评分较高（显示更大潜在危害、更高技术合理性和滥用可行性），Gemini-2.5-pro明显更严格，Grok-3介于两者之间

Conclusion: LLMs尚不能作为恶意评估设置中的可靠评估者，人类评估对于可信的双重用途风险评估至关重要
...
<details>
  <summary>Details</summary>
Motivation: 传统推理方法存在效率问题，需要更高效、可靠的推理方法。FROST旨在利用注意力权重识别并剪枝不关键的推理路径，从而缩短推理轨迹并提高可靠性。

Method: 提出"推理异常值"概念，设计基于注意力的机制来移除这些异常值。在句子级别消除异常值的同时保持并增强模型的推理能力。

Result: 在四个基准测试上使用Phi-4-Reasoning和GPT-OSS-20B模型验证，优于TALE和ThinkLess等最先进方法。平均减少69.68%的token使用，准确率比基础模型提高26.70%。注意力异常值指标中，最大无穷范数减少15.97%，平均峰度减少91.09%。

Conclusion: FROST通过注意力感知的路径剪枝实现了高效可靠的推理，显著减少了计算开销同时提高了准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有语音对话系统的RLHF/RLAIF研究主要局限于单语义奖励和话语级别评估，忽视了对话质量的多维度和多模态特性（包括语义连贯性、音频自然度、说话人一致性、情感对齐和轮转行为），且与双工语音系统的增量生成模式不匹配。

Method: 提出首个多奖励RLAIF框架，结合语义、音频质量和情感一致性奖励；采用轮级偏好采样，在单DPO目标中聚合每块对数概率，以对齐话语级偏好与增量块解码；系统研究了多轮思维链和块级双工模型的偏好学习。

Result: 单奖励RLAIF能针对性改善其目标指标，而联合多奖励训练在语义质量和音频自然度上均获得一致提升；发布了多奖励DPO数据集支持可重复研究。

Conclusion: 多奖励对齐对于实用对话语音系统至关重要，需要全面考虑对话质量的多个维度，而不仅仅是语义内容。
...
<details>
  <summary>Details</summary>
Motivation: 现有心理健康对话系统主要是被动反应式的，缺乏系统性的用户状态建模来进行主动的治疗性探索。需要开发能够系统追踪用户心理状态并主动引导咨询探索的对话系统。

Method: 提出PsyProbe系统，采用PPPPPI框架（呈现、易感、诱发、维持、保护、影响）结合认知错误检测。系统包含：状态构建器（提取结构化心理档案）、记忆构建（追踪信息缺口）、策略规划器（动机访谈行为编码）、响应生成器（问题构思和批评/修订模块）。

Result: 在27名参与者的真实韩国心理咨询场景中评估。自动评估显示完整PsyProbe模型始终优于基线和消融模式。用户评估显示参与意愿显著增加，自然度改善。专家评估表明PsyProbe显著改善核心问题理解，问题质量达到专业咨询师水平。

Conclusion: PsyProbe验证了系统性状态建模和主动提问在治疗性探索中的有效性，能够生成与专业咨询师相当的问题质量，提高用户参与度和咨询效果。
...
Method: 提出TeMLM框架，统一了来源追溯、数据透明度、建模透明度和治理机制，定义了TeMLM-Card、TeMLM-Datasheet、TeMLM-Provenance等文档工具，并制定了轻量级一致性检查清单。在Technetium-I合成临床数据集（498,000条记录，770万PHI实体标注）上实例化该框架，使用ProtactiniumBERT模型（约1亿参数）进行PHI去标识化和ICD-9代码提取任务验证。

Result: 成功建立了完整的TeMLM发布框架，并在Technetium-I数据集上获得了PHI去标识化（token分类）和top-50 ICD-9代码提取（多标签分类）的参考结果。验证了合成基准在工具和流程验证中的价值。

Conclusion: TeMLM为临床语言模型提供了标准化的透明度框架，有助于提高模型的可信度和可审计性。虽然合成基准对工具验证有价值，但模型在部署前仍需在真实临床数据上进行验证。
...
TL;DR: 该论文研究了视觉语言模型在面临文本与视觉证据冲突时的脆弱性，发现现有模型容易受到误导性文本提示的影响，即使有清晰的视觉证据也倾向于相信冲突的文本信息。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在多模态推理方面表现出色，但其对文本错误信息的鲁棒性尚未得到充分探索。现有研究主要关注纯文本领域的错误信息，不清楚VLMs如何在不同模态的冲突信息之间进行仲裁。

Method: 首先提出了CONTEXT-VQA数据集，包含图像-问题对以及系统生成的与视觉证据故意冲突的说服性提示。然后设计了全面的评估框架，对各种模型在这些冲突多模态输入下的易感性进行基准测试。

Result: 对11个最先进的VLMs进行全面实验发现，这些模型确实容易受到误导性文本提示的影响，经常覆盖清晰的视觉证据而倾向于冲突的文本，仅经过一轮说服性对话后平均性能下降超过48.2%。

Conclusion: 研究结果揭示了当前VLMs的关键局限性，强调了提高对文本操纵的鲁棒性的必要性。
...
TL;DR: 该研究通过训练动态分析，揭示了注意力语言模型中语义关联如何从自然语言数据中涌现，并推导出训练早期权重的闭式表达式，将Transformer权重分解为三种基础函数的简单组合。
...
Method: 通过训练动态分析，利用梯度的主导项近似，推导出训练早期阶段的权重闭式表达式。将Transformer的每组权重表示为三种基础函数（二元组、词元可互换性和上下文映射）的简单组合，这些函数反映了文本语料库的统计特性。

Result: 在真实世界LLMs上的实验表明，理论权重表征与学习到的权重高度匹配。定性分析进一步展示了该定理如何解释Transformer中学习到的语义关联。

Conclusion: 该研究揭示了Transformer各组件如何基于三种基础函数的组合来捕获语义关联，为理解语言模型中的语义学习机制提供了理论框架和解释工具。
...
TL;DR: 论文提出了DREAMSTATE框架，利用条件扩散变换器直接建模RNN状态的概率流形，实现状态的生成和编辑，并进一步提出结合RNN局部优势和全局上下文适应性的混合架构。


<details>
  <summary>Details</summary>
Motivation: 现代RNN（如RWKV）具有强大的短程建模能力和高效的固定大小状态，但缺乏对其内部状态作为可编辑知识表示的研究。论文旨在填补这一空白，探索RNN状态的表征特性。

Method: 提出DREAMSTATE框架，使用条件扩散变换器直接建模RNN状态的概率流形；进一步提出混合架构，结合RNN局部优势和全局上下文适应性，通过并行DiT处理可变长度全局上下文来动态生成和调整核心循环模块的WKV参数。

Result: 通过t-SNE可视化和受控生成实验验证了状态表示的结构特性；实验表明混合模型可以通过多目标损失稳定训练，验证了设计可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的RAG方法存在三个主要问题：1）依赖语义不敏感的路径采样，与KG推理目标对齐弱；2）直接将检索路径输入推理器，缺乏组织性，影响小模型利用知识；3）先前研究主要依赖大模型（如ChatGPT/GPT-4）或7B以上参数模型，对7B以下小模型探索不足。

Method: 提出RPO-RAG框架，包含三个关键创新：1）查询-路径语义采样策略，提供信息丰富的监督信号；2）关系感知偏好优化，使训练与KG推理中间信号（如关系）对齐；3）答案中心提示设计，以可解释格式组织实体和推理路径。

Result: 在两个KGQA基准数据集（WebQSP和CWQ）上的实验表明：在WebQSP上F1提升高达8.8%，反映答案精度提升；在CWQ上，在8B参数以下模型中实现了新的SOTA结果（Hit和F1）。即使参数低于3B的小模型，推理能力也得到显著提升。

Conclusion: RPO-RAG有效缩小了小模型与大模型之间的性能差距，突出了小模型在资源高效和实用设备端KGQA应用中的潜力，为小型LLMs在知识密集型任务上的应用提供了有效解决方案。
...
Result: DiaDem在对话描述准确性上超越了Gemini系列模型，同时在通用视听视频描述基准上保持竞争力，商业模型在对话感知描述方面仍有很大改进空间。

Conclusion: DiaDem是一个有效的视听视频描述模型，能够生成更精确的对话描述，同时提出的DiaDemBench为系统评估对话描述能力提供了重要工具。
...
TL;DR: 本文提出了一种创建和评估类比谜语的简单流程，包括构建概念事实、选择类比属性、生成谜语线索和验证答案，用于研究大语言模型能否恢复谜语的所有可能答案。
...
Method: 开发了一个包含四个组件的系统：1）三元组创建器构建概念的结构化事实；2）语义映射器选择适合类比的属性；3）风格化生成器将属性转化为谜语线索；4）验证器收集谜语可能指向的所有答案。使用该系统研究大语言模型能否恢复不同谜语类型的完整答案集。

Result: 案例研究表明，虽然模型经常能猜出主要预期答案，但通常会错过其他有效解释。这突显了谜语作为评估语言模型推理覆盖范围和歧义处理能力的价值。

Conclusion: 谜语可以作为轻量级工具，有效检验语言模型的推理完整性和歧义处理能力。模型在识别谜语所有可能答案方面存在局限性，这为评估和改进语言模型的推理能力提供了新的视角。
...
Method: DART利用扩散式大语言模型思想，基于目标模型的隐藏状态，在单次前向传播中并行预测多个未来掩码位置的logits，消除草稿模型的自回归展开。同时引入高效的树剪枝算法，构建具有N-gram语义连续性的高质量草稿标记树。

Result: 实验结果显示，DART在多个数据集上实现了2.03-3.44倍的端到端解码加速，平均比EAGLE3快30%，显著降低了草稿阶段开销同时保持了高草稿准确性。

Conclusion: DART通过并行生成和高效树剪枝算法，在保持轻量级设计的同时显著降低了推测解码的草稿阶段延迟，提供了一个实用的推测解码框架，有效加速LLM推理。
...
TL;DR: ReToP框架通过端到端训练EHR重写器和临床预测器，利用LLM生成临床相关重写以增强预测性能，在MIMIC-IV数据集上超越基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多将LLMs用作EHR编码器或完成模块，缺乏与预测任务的深度整合，这限制了任务性能。需要一种能更好整合预测信号的方法来提升临床预测准确性。

Method: 提出ReToP框架，包含EHR重写器和临床预测器的端到端训练。使用临床驱动的特征选择策略生成合成伪标签来训练重写器，并引入Classifier Supervised Contribution (CSC)评分来对齐重写器与预测目标。

Result: ReToP在MIMIC-IV的三个临床任务上超越了强基线模型，分析显示其具有良好的泛化能力，能迁移到未见数据集和任务，同时保持忠实重写并强调任务相关预测特征。

Conclusion: ReToP通过任务感知的EHR重写有效整合了LLMs的医学知识与临床预测目标，提升了预测性能并展示了良好的泛化能力，为临床决策支持提供了新方法。
...
TL;DR: MetaGen是一个无需训练的框架，能够在推理时动态调整多智能体系统的角色空间和协作拓扑结构，无需更新基础模型权重，从而提升复杂任务处理的准确性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型多智能体系统通常依赖固定的角色库和冻结的执行交互拓扑，这种刚性设计导致任务不匹配、无法及时适应推理过程中出现的新证据，并且增加了推理成本。

Method: MetaGen在推理时生成和重写查询条件化的角色规范，维护可控的动态角色池，围绕最小骨干实例化约束执行图，在执行过程中迭代更新角色提示并基于轻量级反馈信号调整结构决策。

Result: 在代码生成和多步推理基准测试中，MetaGen相比强大的多智能体基线方法，在准确性和成本权衡方面表现出改进。

Conclusion: MetaGen通过动态调整角色空间和协作拓扑，解决了现有多智能体系统的刚性设计问题，提供了一种无需训练即可提升复杂任务处理能力的有效框架。
...
TL;DR: F-1提示法通过将数学方程作为中间表示，在应用数学领域显著提升LLM推理能力，相比CoT和PoT分别平均提升5.76%和8.42%。


<details>
  <summary>Details</summary>
Motivation: 现有提示技术（如CoT和PoT）在应用数学问题（金融、物理、密码学等）中未能充分利用领域特定的控制方程，这些领域需要回忆或推导控制方程作为关键步骤。
...
Conclusion: F-1通过显式利用数学方程作为中间表示，在应用数学问题上比CoT更有效，特别是在需要领域特定控制方程的复杂场景中。
...
TL;DR: DeconIEP是一个在评估期间通过输入嵌入空间的小扰动来减轻测试集污染影响的去污染框架，无需修改基准测试集或干扰正常推理。


<details>
  <summary>Details</summary>
Motivation: 基准测试评估是LLM比较的标准方法，但其可靠性受到测试集污染的威胁，即测试样本或其变体泄露到训练数据中，人为地提高了报告的性能。现有的缓解方法要么需要修改基准测试集，要么会干扰正常推理并导致性能下降。

Method: DeconIEP在评估期间完全操作，通过在输入嵌入空间应用小的有界扰动。它由一个相对较少污染的参考模型指导，学习一个实例自适应的扰动生成器，引导被评估模型远离记忆驱动的捷径路径。

Result: 在多个开源权重LLM和基准测试上的广泛实证结果表明，DeconIEP实现了强大的去污染效果，同时在良性效用上仅产生最小程度的性能下降。

Conclusion: DeconIEP提供了一种有效的去污染框架，能够在保持基准测试完整性的同时，有效减轻测试集污染对LLM评估的影响，且对正常推理干扰最小。
...
TL;DR: 本文提出了一种基于交叉审问框架（CEF）的无参考文本生成评估方法，通过将源文本和候选文本视为独立知识库，生成可验证问题并进行交叉检查，获得覆盖率、一致性和连贯性三个可解释分数，相比传统BLEU和BERTScore能更好地捕捉语义保真度。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标如BLEU和BERTScore在生成式文本到文本任务中无法有效捕捉语义保真度，存在对内容遗漏和事实矛盾等关键错误检测不足的问题。

Method: 采用交叉审问框架（CEF），将源文本和候选文本视为独立知识库，从每个文本生成可验证问题，通过交叉审问获得三个可解释分数：覆盖率（Coverage）、一致性（Conformity）和连贯性（Consistency）。通过系统鲁棒性分析选择稳定的评判模型。

Result: 在翻译、摘要和临床笔记生成任务上验证，CEF能够识别标准指标遗漏的关键错误（如内容遗漏和事实矛盾）。无参考模式与有参考模式之间的强相关性验证了CEF在没有黄金参考时的可靠性。人类专家验证显示CEF不匹配问题与意义改变的语义错误高度相关，尤其在识别基于实体和关系扭曲方面表现优异。

Conclusion: CEF提供了一种无参考、多维度的文本生成评估框架，能够有效捕捉语义保真度，识别传统指标遗漏的关键错误，并通过系统鲁棒性分析确保评估稳定性，在多种文本生成任务中表现出优越性能。
...
TL;DR: 本文提出了一种结合二元标记级分类、语言特征集成和数据增强的多词表达识别方法，在CoAM数据集上达到69.8% F1分数，比Qwen-72B高出12个百分点，参数量减少165倍。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在结构化NLP任务上表现不佳且计算成本高，需要开发更高效、专门化的方法来解决多词表达识别问题，特别是在资源受限的环境中。

Method: 1. 将检测重新定义为二元标记级START/END/INSIDE分类而非基于跨度的预测；2. 集成NP分块和依存特征以帮助识别不连续和名词型多词表达；3. 应用过采样技术解决训练数据中的严重类别不平衡问题。

Result: DeBERTa-v3-large模型在CoAM数据集上达到69.8% F1分数，比Qwen-72B的57.8%高出12个百分点，参数量减少165倍；在STREUSLE数据集上达到78.9% F1分数，验证了方法的泛化能力。

Conclusion: 精心设计的小型模型在结构化NLP任务上可以显著优于大型语言模型，这对资源受限的部署具有重要启示，证明了专门化方法相对于通用大模型的优势。
...
TL;DR: 本文系统比较了专有和开源大语言模型在文档级自动后编辑任务中的表现，发现专有模型在简单提示下能达到接近人类水平，但对文档级上下文利用不足，且成本过高不实用。
...
Result: 专有LLMs在简单单样本提示下能达到接近人类水平的APE质量，但对文档级上下文利用有限；相比开源模型具有更高的数据中毒攻击鲁棒性；标准自动评估指标不能可靠反映质量改进；专有模型成本过高不实用。

Conclusion: LLM在文档感知APE方面展现出潜力但存在局限性，专有模型性能好但成本高且上下文利用不足，需要更高效的长上下文建模方法用于翻译精炼。
...
TL;DR: KG-CRAFT：一种利用知识图谱增强大语言模型进行声明验证的方法，通过构建知识图谱并生成对比性问题来指导证据提取和真实性评估，在两个真实数据集上达到最新最优性能。


<details>
  <summary>Details</summary>
Motivation: 声明验证是自动化事实核查系统的核心组件，传统方法在利用结构化知识和对比推理方面存在不足。本研究旨在通过知识图谱增强大语言模型，提高声明验证的准确性和可靠性。

Method: KG-CRAFT方法首先从声明和相关报告中构建知识图谱，然后基于知识图谱结构生成上下文相关的对比性问题。这些问题指导证据报告的提取和蒸馏，最终合成简洁摘要供大语言模型进行真实性评估。

Result: 在两个真实世界数据集（LIAR-RAW和RAWFC）上的广泛评估表明，KG-CRAFT在预测性能上达到了新的最先进水平。综合分析验证了基于知识图谱的对比推理方法在提升大语言模型事实核查能力方面的有效性。

Conclusion: KG-CRAFT通过知识图谱增强和对比性问题引导，显著提升了大语言模型在声明验证任务中的性能，为自动化事实核查系统提供了更可靠的方法。
...
TL;DR: 本文介绍了ClaimPT数据集，这是一个针对欧洲葡萄牙语新闻文章的事实性主张标注数据集，包含1,308篇文章和6,875个标注，旨在推动低资源语言的事实核查研究。


<details>
  <summary>Details</summary>
Motivation: 事实核查是一项耗时且主要依赖人工的任务，难以跟上在线虚假信息的传播速度。虽然许多组织进行人工事实核查，但随着数字内容量的增长，这种方法难以扩展。葡萄牙语等语言缺乏可访问的标注数据集，限制了相关研究和应用的发展。

Method: 通过与葡萄牙新闻社LUSA合作收集新闻文章，创建了ClaimPT数据集。采用新提出的标注方案，由两名训练有素的标注员对每篇文章进行标注，并由一名策展人验证所有标注。同时提供了主张检测的基线模型。

Result: 创建了包含1,308篇欧洲葡萄牙语新闻文章和6,875个个体标注的ClaimPT数据集。该数据集专注于新闻内容，不同于大多数基于社交媒体或议会记录的资源。提供了基线模型，建立了初步基准，为未来的NLP和信息检索应用奠定了基础。

Conclusion: ClaimPT数据集的发布旨在推进低资源语言事实核查研究，增强对新闻媒体中虚假信息的理解。该资源填补了葡萄牙语事实核查数据集的空白，为未来的NLP和信息检索应用提供了基础。
...
TL;DR: GradPruner：一种在微调早期基于梯度信息剪裁LLM层的方法，可同时提升训练和推理效率，在减少40%参数的情况下仅损失0.99%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统微调LLM耗时耗资源，结构化剪枝方法虽然能提升推理效率，但通常需要额外的训练、知识蒸馏、结构搜索等策略，难以实现高效的模型微调。

Method: 提出GradPruner方法，在微调初期利用参数累积梯度计算初始梯度信息累积矩阵（IGIA-Matrix）来评估层的重要性并进行剪枝。剪枝层基于IGIA-Matrix稀疏化后与剩余层合并，仅合并符号相同的元素以减少符号变化干扰。

Result: 在两个LLM和八个下游数据集（包括医疗、金融和通用基准任务）上的实验表明，GradPruner实现了40%的参数减少，而准确率仅下降0.99%。

Conclusion: GradPruner能够同时提升下游任务微调的训练和推理效率，通过早期梯度引导的层剪枝实现参数大幅减少而性能损失极小。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度研究框架面临三个关键限制：长时程任务中上下文噪声不断升级、脆弱性导致级联错误、缺乏模块化可扩展性，阻碍了自主智能体在复杂开放任务中的潜力发挥。

Method: 提出三层架构：1) 集中式多智能体编排系统，将子任务路由到原子能力池；2) 动态上下文管理机制，将完成的子目标结构化语义摘要；3) 主动监督模块，通过异常检测和上下文剪枝确保鲁棒性。

Result: 在多个深度研究基准测试中达到最先进性能，包括GAIA、BrowseComp、BrowseComp-ZH和Humanity's Last Exam。框架已开源，包含可复现实现和应用案例。

Conclusion: Yunque DeepResearch通过分层模块化架构有效解决了深度研究中的关键挑战，为自主智能体在复杂开放任务中的能力提升提供了强大框架，推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM/MLLM的视觉文档关键信息提取方法依赖自回归推理，需要顺序生成输出，在处理多字段提取时存在显著的效率瓶颈。

Method: 提出并行推理范式PIP，使用"[mask]"标记作为所有目标值的占位符，通过单次前向传播同时生成所有字段。为此开发了专门的掩码预训练策略并构建大规模监督数据集。

Result: PIP模型相比传统自回归基础模型实现了5-36倍的推理加速，性能下降可忽略不计。

Conclusion: PIP通过显著提升效率同时保持高精度，为可扩展和实用的现实世界KIE解决方案铺平了道路。
...
TL;DR: 本文提出在扩散语言模型中引入一个额外的sink token来解决moving sink现象，通过修改注意力掩码使该token仅关注自身但对所有其他token可见，从而稳定注意力机制并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然具有并行文本生成的竞争优势，但存在关键的moving sink不稳定现象。sink token在Transformer值空间中具有低范数表示，moving sink现象是防止信息过度混合的保护机制，但其在扩散步骤中的不可预测位置会削弱推理的鲁棒性。

Method: 提出简单但有效的额外sink token方法，通过修改注意力掩码实现。引入一个特殊token，该token只能关注自身，但对所有其他token全局可见。这种设计使该token成为稳定的结构sink。

Result: 实验结果表明，引入单个额外token能够稳定注意力sink，显著提升模型性能。进一步分析证实该token的有效性与其位置无关，且具有可忽略的语义内容，验证了其作为鲁棒专用结构sink的作用。

Conclusion: 通过引入一个简单的额外sink token并修改注意力掩码，可以有效解决扩散语言模型中的moving sink不稳定问题，提升模型推理的鲁棒性和整体性能。
...
TL;DR: SynCABEL是一个利用大语言模型生成生物医学实体链接合成训练数据的框架，显著减少对人工标注数据的依赖，在多个多语言基准测试中达到最先进性能，并引入LLM评估协议来评估临床有效性。
...
Method: SynCABEL利用大语言模型为目标知识库中的所有候选概念生成上下文丰富的合成训练示例，结合仅解码器模型和引导推理方法，并引入LLM-as-a-judge协议来评估临床有效性。

Result: 在MedMentions（英语）、QUAERO（法语）和SPACCC（西班牙语）三个多语言基准测试中达到最先进性能；相比完整人工监督，可减少高达60%的标注数据需求；显著提高了临床有效预测的比例。

Conclusion: SynCABEL通过合成数据生成有效解决了生物医学实体链接中训练数据稀缺的问题，大幅降低了对昂贵专家标注的依赖，并通过创新的评估方法更好地反映了临床实用性。
...
TL;DR: TokenSeek是一种通过实例感知的token选择和丢弃机制来显著减少大语言模型微调内存消耗的通用插件解决方案


<details>
  <summary>Details</summary>
Motivation: 大语言模型微调过程中内存消耗巨大，现有激活优化方法由于数据无关性导致效果不稳定且效率低下

Method: 提出TokenSeek方法，通过实例感知的token寻找和丢弃机制，动态选择重要token进行保留，丢弃不重要的token以减少内存占用

Result: 在Llama3.2 1B等模型上仅需14.8%的内存，同时保持相当甚至更好的性能表现

Conclusion: TokenSeek为基于Transformer的模型提供了高效的内存优化解决方案，其可解释的token选择过程为未来token效率研究提供了有价值的见解
...
TL;DR: 提出了一种新的零样本立场检测任务DGTA，旨在从文本中自动识别多个目标-立场对，无需先验目标知识。在中文社交媒体数据集上评估了多种大语言模型，结果显示微调后的LLMs在该任务上表现优异。
...
Method: 提出了动态目标生成与多目标适应（DGTA）的零样本立场检测任务，构建了中文社交媒体立场检测数据集，设计了多维评估指标。探索了集成和两阶段微调策略，评估了多种大语言模型基线。

Result: 实验结果表明，微调后的大语言模型在该任务上表现优异：两阶段微调的Qwen2.5-7B获得了最高的综合目标识别分数66.99%，而集成微调的DeepSeek-R1-Distill-Qwen-7B达到了79.26%的立场检测F1分数。

Conclusion: 提出的DGTA任务有效解决了现实社交媒体中动态多目标立场检测的挑战，微调后的大语言模型在该任务上表现出色，为实际应用提供了可行的解决方案。
...
TL;DR: 该研究首次对迭代检索增强生成(RAG)与静态RAG在科学多跳推理任务中的性能进行了机制层面的诊断分析，发现迭代RAG在化学领域多跳问答中显著优于理想化静态RAG，性能提升最高达25.6个百分点。


<details>
  <summary>Details</summary>
Motivation: 尽管检索增强生成(RAG)扩展了大语言模型的参数知识，但尚不清楚迭代检索-推理循环何时能显著优于静态RAG，特别是在需要多跳推理、领域知识稀疏且证据异质的科学领域。研究旨在通过机制层面的诊断分析，确定同步迭代检索和推理是否能超越理想化的静态RAG上限。

Method: 研究采用化学领域的ChemKGMultiHopQA数据集，对11个最先进的大语言模型在三种机制下进行基准测试：(1)无上下文(测量对参数记忆的依赖)；(2)黄金上下文(一次性提供所有理想证据)；(3)迭代RAG(无训练控制器，交替进行检索、假设精炼和证据感知停止)。通过诊断指标分析检索覆盖差距、锚点携带丢失、查询质量、组合保真度和控制校准等行为。

Result: 迭代RAG在所有模型中一致优于黄金上下文RAG，性能提升最高达25.6个百分点，特别是对于非推理微调模型。分阶段检索减少了后期跳失败，缓解了上下文过载，并能够动态纠正早期假设漂移。但剩余失败模式包括不完全跳覆盖、干扰物锁定轨迹、早期停止校准错误，以及即使完美检索也存在高组合失败率。

Conclusion: 分阶段检索通常比仅仅存在理想证据更具影响力。研究为在专业科学环境中部署和诊断RAG系统提供了实用指导，并为更可靠、可控的迭代检索-推理框架奠定了基础。
...
Conclusion: AdaRAS通过选择性神经元激活引导，以轻量级方式有效提升LLMs的推理可靠性，无需额外训练或采样成本，具有实际应用价值
...
TL;DR: 本文提出Reflective Translation框架，通过让模型生成初始翻译、进行结构化自我批判、然后生成改进翻译，来提升低资源语言（isiZulu和isiXhosa）的机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 低资源语言如isiZulu和isiXhosa在机器翻译中面临平行数据有限和语言资源不足的挑战。大型语言模型的自反思能力可以提升推理质量和事实一致性，这为改进低资源语言翻译提供了新思路。

Method: 提出Reflective Translation框架：1）模型生成初始翻译；2）生成结构化自我批判；3）基于反思生成改进翻译。该方法无需微调，模型无关，使用OPUS-100和NTREX-African数据集在英语-isiZulu和英语-isiXhosa翻译任务上进行评估。

Result: 实验结果显示，第一轮和第二轮翻译在BLEU和COMET分数上均有持续改进，平均提升达+0.22 BLEU和+0.18 COMET。配对非参数统计检验证实这些改进具有统计显著性。方法还创建了反思增强数据集。

Conclusion: 结构化自我反思是改进低资源环境下翻译质量的有效实用机制。该方法模型无关、无需微调，为未来监督式或分析驱动的工作提供了支持。
...
TL;DR: ONCOTIMIA是一个集成生成式AI的临床工具，用于肺癌多学科肿瘤委员会（MDTB）表单的自动填写，通过LLMs将非结构化临床文档转化为结构化记录，显著减轻文档负担。


<details>
  <summary>Details</summary>
Motivation: 多学科肿瘤委员会在肿瘤学决策中起核心作用，但需要大量手动处理异构临床信息，导致沉重的文档负担。需要自动化工具来整合生成式AI到肿瘤工作流程中。

Method: 开发了ONCOTIMIA系统，采用多层数据湖、混合关系型和向量存储、检索增强生成（RAG）和规则驱动的自适应表单模型。评估了通过AWS Bedrock部署的六个LLMs在十个肺癌病例上的表现，测量表单填写准确性和端到端延迟。

Result: 结果显示模型性能良好，最佳配置实现了80%的正确字段完成率，大多数LLMs具有临床可接受的响应时间。更大、更新的模型表现出最佳准确性，且延迟在可接受范围内。
...
TL;DR: 该研究探索了频繁序列挖掘在图像压缩领域的应用，提出了一种结合聚类和序列挖掘的压缩方法，替代传统JPEG的DCT变换，取得了更好的压缩比和质量。


<details>
  <summary>Details</summary>
Motivation: 传统图像压缩方法在处理冗余数据方面存在局限，需要探索新的压缩技术来提高压缩效率和图像质量。

Method: 用闭频繁序列挖掘和k-means聚类替代JPEG中的DCT变换；对图像各分量的所有块并行应用k-means聚类以减少压缩时间；改进GSP算法，通过新颖的剪枝策略优化模式基数，减小码表大小。

Result: 仿真结果表明，与现有替代方案相比，该算法在压缩比和质量方面取得了显著提升。

Conclusion: 将频繁序列挖掘与聚类技术结合应用于图像压缩是有效的，能够显著改善压缩性能。
...
TL;DR: 开发了三个工具来分析OCTA图像，用于辅助诊断新生血管性年龄相关性黄斑变性，包括生物标志物提取、3D可视化以及可解释的机器学习诊断模型


<details>
  <summary>Details</summary>
Motivation: 增强眼科医生在诊断新生血管性年龄相关性黄斑变性时的决策能力，提供更直观、可解释的诊断工具

Method: 开发了三个工具：1) 使用图像处理提取生物标志物（如mCNV面积和血管密度）；2) 生成新生血管的3D可视化；3) 应用三种白盒机器学习算法（决策树、支持向量机和DL-Learner）的集成模型进行诊断

Result: 学习到的表达式在训练数据上达到100%准确率，在测试数据上达到68%准确率。主要优势是所有模型都是白盒模型，确保了可解释性和透明度

Conclusion: 开发的可解释性工具能够帮助临床医生更好地理解诊断决策过程，为nAMD的诊断提供了透明、可解释的辅助决策支持
...
TL;DR: 本文综述了基于扩散模型的生成式有损压缩方法，重点在图像压缩领域，探讨了如何通过扩散模型在极低比特率下生成逼真重建，并分析了其在率失真感知理论框架下的应用。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面的出色表现启发了其在数据压缩领域的应用，特别是能够在极低比特率下生成逼真重建，这为传统压缩方法提供了新的可能性。

Method: 方法通常将源数据编码为嵌入表示，通过扩散模型在解码过程中迭代优化该嵌入，使最终重建近似于真实数据分布。嵌入表示可通过辅助熵模型传输，也有方法探索使用扩散模型本身通过信道模拟进行信息传输。

Result: 基于扩散模型的生成式压缩方法能够在极低比特率下实现逼真的图像重建，超越了传统压缩方法的性能限制，为数据压缩领域带来了新的突破。

Conclusion: 扩散模型为生成式有损压缩提供了强大的框架，通过率失真感知理论视角分析这些方法，揭示了共同随机性的作用以及与逆问题的联系，同时指出了该领域面临的开放挑战。
...
TL;DR: 该研究比较了k-means颜色量化在RGB、CIE-XYZ和CIE-LUV/CIE-HCL色彩空间中的性能，发现在不同量化级别下，各色彩空间各有优势，图像的颜色分布特征影响最佳色彩空间的选择。


<details>
  <summary>Details</summary>
Motivation: 虽然k-means算法常用于颜色量化，但大多在机器基础的RGB色彩空间中使用。近期研究表明在基于人类感知的色彩空间中可能有更好表现，因此需要系统比较不同色彩空间中的量化性能。

Method: 在148张多样化数字图像上，在四个量化级别比较k-means颜色量化在RGB、CIE-XYZ和CIE-LUV/CIE-HCL色彩空间中的性能。使用视觉信息保真度(VIF)指标量化评估量化图像质量，并分析图像色调、色度和亮度分布对性能的影响。

Result: 约半数情况下RGB空间表现最佳；其他情况下，特别是较高量化级别(k)时，CIE-XYZ空间通常更好；较低k值时，CIE-LUV空间有时表现最佳。图像的颜色分布特征（色调、色度、亮度）影响各色彩空间的最佳适用情况。

Conclusion: k-means颜色量化的最佳色彩空间选择取决于量化级别和图像的颜色分布特征。没有单一最优色彩空间，需要根据具体应用场景和图像特性进行选择。
...
TL;DR: 提出基于张量补全的荧光显微镜3D活细胞成像恢复框架，通过理论下界验证可行性，并利用分数生成模型实现去噪和几何一致重建


<details>
  <summary>Details</summary>
Motivation: 荧光显微镜3D活细胞成像需要快速扫描以减少光毒性，导致采样稀疏、分辨率各向异性和高噪声。现有基于逆问题建模的方法假设已知且稳定的退化过程，难以处理这种非线性信号退化和不完整观测的情况

Method: 将荧光显微镜成像重新定义为张量补全问题，推导精确细胞张量补全的理论下界，并将问题重构为数学等价的分数生成模型，结合结构一致性先验引导生成轨迹

Result: 在SR-CACO-2和三个真实体内细胞数据集上实现最先进性能，信噪比和结构保真度均有显著提升

Conclusion: 提出的张量补全框架有效解决了荧光显微镜3D活细胞成像中的稀疏采样和噪声问题，为非线性信号退化和不完整观测条件下的图像恢复提供了新视角
...
TL;DR: 提出了一种高效模拟MRI中T2'弛豫的方法，使用线性相位模型直接模拟洛伦兹函数，避免了传统方法需要100+等色体的需求，计算时间仅增加2-2.7倍。


<details>
  <summary>Details</summary>
Motivation: 在MRI模拟中，有效横向弛豫T2*包含可逆(T2')和不可逆(T2)分量。T2模拟容易，但T2'难以模拟，传统方法需要大量等色体来近似洛伦兹函数。

Method: 提出使用线性相位模型直接模拟整个洛伦兹函数，通过模拟磁化强度对频率轴的偏导数来表示线性相位模型。采用两种加速技术：解析解和组合转换。进行了单等色体模拟验证基本原理，并在两个体模上模拟多个脉冲序列。

Result: 单等色体模拟证明T2'模拟可行。在实际案例中，无需100+等色体即可恢复T2'效应。包含T2'模拟的计算时间仅比不包含时长2.0-2.7倍。解析解加速19倍，组合转换加速达17倍。

Conclusion: 理论和结果均表明，所提方法通过利用线性模型与洛伦兹函数、解析解和组合转换，能够高效模拟T2'弛豫。
...
TL;DR: 提出基于强化学习的神经视频压缩码率控制框架，通过帧级顺序决策联合优化码率分配和编码参数，显著提升码率控制精度和压缩效率。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频压缩的码率控制方案主要关注帧内容来捕捉失真交互，但忽略了因每帧编码参数变化而产生的帧间码率依赖关系，导致次优的码率分配和级联参数决策问题。

Method: 提出强化学习框架，将码率控制任务建模为逐帧顺序决策过程。在每个帧，RL智能体观察时空状态并选择编码参数，以优化反映率失真性能和码率遵从度的长期奖励。该方法独立于GOP结构，单步联合确定码率分配和编码参数。

Result: 在不同NVC架构上的广泛实验表明，该方法将平均相对码率误差降低至1.20%，在典型GOP大小下实现高达13.45%的码率节省，优于现有方法。框架对内容变化和带宽波动具有更好的鲁棒性，编码开销更低。

Conclusion: 提出的基于强化学习的码率控制框架有效解决了神经视频压缩中的码率控制挑战，通过联合优化码率分配和编码参数，显著提升了压缩效率和实际部署的适用性。
...
TL;DR: AMGFormer是一个用于脑肿瘤分割的多模态MRI方法，通过三个协同模块解决了临床实践中模态缺失导致的性能不稳定问题，实现了跨15种模态组合<0.5%的性能方差。


<details>
  <summary>Details</summary>
Motivation: 临床实践中多模态MRI经常存在模态缺失，导致现有方法在不同模态组合下性能方差超过40%，这使得这些方法在临床上不可靠。
...
Result: 在BraTS 2018上达到89.33% WT、82.70% TC、67.23% ET Dice分数，跨15种模态组合方差<0.5%。单模态ET分割相对现有方法提升40-81%。在BraTS 2020/2021上泛化性能达92.44% WT、89.91% TC、84.57% ET。推理时间仅1.2秒。

Conclusion: AMGFormer解决了多模态脑肿瘤分割中的稳定性危机，显著降低了模态缺失对性能的影响，具有临床部署潜力，推理速度快，代码已开源。
...
TL;DR: 提出一种无需反向传播的多任务绿色学习框架，用于左心室分割和射血分数分类，在EchoNet-Dynamic数据集上达到94.3%准确率和0.912 DSC，参数数量显著少于现有3D深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 超声心动图是心力衰竭管理的关键，但手动评估左心室射血分数存在观察者间变异性高的问题。现有深度学习模型计算量大、数据需求高且缺乏可解释性，阻碍了临床信任和采用。

Method: 提出多任务绿色学习框架，结合无监督VoxelHop编码器进行层次化时空特征提取，使用多级回归解码器和XG-Boost分类器，同时执行左心室分割和射血分数分类。

Result: 在EchoNet-Dynamic数据集上，模型达到94.3%的分类准确率和0.912的Dice相似系数，显著优于多个先进的3D深度学习模型，且参数数量减少一个数量级以上。
...
<details>
  <summary>Details</summary>
Motivation: 连续黑盒优化的基准测试受到现有测试套件（如BBOB）结构多样性有限的阻碍，需要能够设计具有明确高层景观特征的优化问题的新方法。

Method: 使用LLaMEA框架，通过LLM从自然语言描述生成问题代码，目标属性包括多模态性、可分离性、盆地大小同质性、搜索空间同质性和全局-局部最优对比度。在循环中使用基于ELA的属性预测器评分候选问题，并引入ELA空间适应度共享机制增加种群多样性。

Result: 生成的许多函数确实表现出预期的结构特征，t-SNE嵌入显示它们扩展了BBOB实例空间而不是形成不相关的聚类。通过盆地吸引分析、统计测试和视觉检查验证了结果。

Conclusion: 生成的库为景观分析和下游任务（如自动算法选择）提供了广泛、可解释且可重复的基准问题集，扩展了现有测试套件的结构多样性。
...
Method: 开发区间二型自适应神经模糊推理系统（IT2-ANFIS），通过模糊规则结构生成可解释的预测区间。框架在三个层面分解不确定性：特征级（识别引入模糊性的变量）、规则级（分析局部模型置信度）和实例级（量化整体预测不确定性）

Result: 在墨尔本水务东部处理厂数据集上验证，IT2-ANFIS达到与一阶ANFIS相当的预测性能，同时显著降低训练运行方差，并提供可解释的不确定性估计，将预测置信度直接与运营条件和输入变量关联

Conclusion: IT2-ANFIS为污水处理厂能耗预测提供了可解释的不确定性量化框架，支持风险感知决策，有助于优化运营和提升可持续性
...
TL;DR: 论文引入RIFT测试平台，通过重新排序的指令来评估大语言模型的指令跟随能力，发现模型在非顺序提示下的性能显著下降，揭示了当前架构对结构顺序的强烈依赖。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试将任务复杂性与结构顺序混为一谈，难以分离提示结构对性能的影响。需要专门工具来评估LLMs在复杂工作流中保持指令流的能力。

Method: 引入RIFT测试平台，使用重新表述的Jeopardy!问答对，测试两种提示结构：线性提示（顺序进行）和跳跃提示（内容相同但需要非顺序遍历）。在6个最先进的开源LLMs上进行10,000次评估。

Result: 在跳跃条件下，准确率相比基线下降了高达72%，显示出对位置连续性的强烈依赖。约50%的失败源于指令顺序违反和语义漂移，表明当前架构将指令跟随内化为顺序模式而非推理技能。

Conclusion: 结构敏感性是当前架构的基本限制，对需要非顺序控制流的应用（如工作流自动化和多智能体系统）有直接影响。需要开发更鲁棒的架构来处理非顺序指令。
...
TL;DR: 研究LLM在重复社会困境中的战略行为，发现激励强度和语言背景显著影响其策略选择，揭示了模型和语言依赖的行为模式


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地作为自主智能体在交互和多智能体环境中运行，理解其战略行为对于安全、协调以及AI驱动的社会和经济系统至关重要

Method: 使用收益缩放的囚徒困境来隔离对激励强度的敏感性，通过监督分类器在经典重复博弈策略上训练，并将其应用于LLM决策分析

Result: 观察到一致的行为模式，包括对激励敏感的调节策略和跨语言差异，语言框架有时匹配或超过架构效应的影响
...
<details>
  <summary>Details</summary>
Motivation: 现有3D情感说话人脸合成方法存在两个关键挑战：1）音频-视觉情感对齐不佳，表现为音频情感提取困难和情感微表情控制不足；2）采用一刀切的多视图融合策略，忽视了不确定性和特征质量差异，从而损害了渲染质量。

Method: 提出UA-3DTalk框架，包含三个核心模块：1）先验提取模块，将音频解耦为内容同步特征（用于对齐）和人物特定互补特征（用于个性化）；2）情感蒸馏模块，采用多模态注意力加权融合机制和4D高斯编码与多分辨率码本，实现细粒度音频情感提取和情感微表情精确控制；3）基于不确定性的形变模块，使用不确定性块估计视图特定的偶然性（输入噪声）和认知性（模型参数）不确定性，实现自适应多视图融合，并采用多头解码器优化高斯基元。

Result: 在常规和情感数据集上的广泛实验表明，UA-3DTalk在情感对齐（E-FID提升5.2%）、唇部同步（SyncC提升3.1%）和渲染质量（LPIPS提升0.015）方面均优于DEGSTalk和EDTalk等最先进方法。

Conclusion: UA-3DTalk通过情感先验蒸馏和不确定性感知的多视图融合，有效解决了3D情感说话人脸合成中的关键挑战，在情感对齐、唇部同步和渲染质量方面取得了显著改进。
...
TL;DR: 提出了一种基于强化学习的对抗性数据增强方法，通过训练查询模型生成针对函数调用模型弱点的对抗性查询，提升LLM函数调用能力的泛化性和鲁棒性。
...
Conclusion: 基于强化学习的对抗性数据增强方法能够有效提升LLM函数调用能力的泛化性和鲁棒性，超越了传统依赖固定数据分布的方法。
...
TL;DR: TS-Debate：一种用于零样本时间序列推理的模态专业化协作多智能体辩论框架，通过专家智能体分工处理文本、视觉和数值信号，结合验证-冲突-校准机制，显著提升性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在时间序列分析中虽然展现出潜力，但在数值保真度、模态干扰和跨模态整合方面存在局限性，需要更专业的方法来处理多模态时间序列数据

Method: TS-Debate采用多智能体辩论框架，分配专门专家智能体处理文本上下文、视觉模式和数值信号，通过结构化辩论协议协调交互，使用验证-冲突-校准机制评估智能体主张，支持轻量级代码执行和数值查找进行程序化验证

Result: 在涵盖三个公共基准的20个任务中，TS-Debate相比强基线（包括所有智能体观察所有输入的标准多模态辩论）实现了持续且显著的性能提升

Conclusion: TS-Debate框架通过模态专业化、协作辩论和程序化验证，能够保持模态保真度、暴露冲突证据并减轻数值幻觉，无需任务特定微调即可有效提升时间序列推理性能
...
### [121] [Multi-Agent Procedural Graph Extraction with Structural and Logical Refinement](https://arxiv.org/abs/2601.19170)
*Wangyang Ying,Yanchi Liu,Xujiang Zhao,Wei Cheng,Zhengzhang Chen,Wenchao Yu,Yanjie Fu,Haifeng Chen*

Main category: cs.AI

TL;DR: 提出一个多智能体框架，将自然语言中的工作流提取为过程图，通过结构化和逻辑反馈迭代优化


<details>
  <summary>Details</summary>
Motivation: 从自然语言自动提取工作流过程图具有潜力但研究不足，现有大语言模型常产生结构错误或逻辑不一致的问题

Method: 设计多智能体框架，包含三个阶段：图构建智能体提取初始图，模拟智能体诊断结构缺陷，语义智能体对齐逻辑与文本语义，通过自然语言反馈迭代优化

Result: 实验表明该方法在结构正确性和逻辑一致性方面显著优于现有基线方法

Conclusion: 多智能体框架通过结构化反馈和逻辑对齐，有效提升了从自然语言提取工作流过程图的质量
...
### [122] [CollectiveKV: Decoupling and Sharing Collaborative Information in Sequential Recommendation](https://arxiv.org/abs/2601.19178)
...
Method: 使用奇异值分解分析KV信息，发现信息可分为可跨用户共享的大部分信息和用户特定的少部分信息。提出CollectiveKV跨用户KV共享机制，通过可学习的全局KV池捕获跨用户共享信息，推理时每个用户从池中检索高维共享KV并与低维用户特定KV拼接得到最终KV

Result: 在五个序列推荐模型和三个数据集上的实验表明，该方法能将KV缓存压缩到原始大小的0.8%，同时保持甚至提升模型性能

Conclusion: CollectiveKV通过利用KV中的协作信号，实现了高效的KV缓存压缩，解决了序列推荐系统中KV缓存存储开销大的问题，为实际部署提供了可行的解决方案
...
### [123] [CoReTab: Improving Multimodal Table Understanding with Code-driven Reasoning](https://arxiv.org/abs/2601.19193)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态表格理解数据集（如MMTab）主要提供简短的事实性答案，缺乏显式的多步推理监督。在这些数据集上训练的模型通常生成简短回答，准确率不足且可解释性有限，无法清晰展示模型如何得出最终答案。

Method: 引入CoReTab框架，通过将多步推理与可执行Python代码耦合，生成可扩展、可解释且可自动验证的标注。使用该框架构建了包含115K已验证样本的数据集（平均每个响应529个token），并通过三阶段流水线对开源MLLMs进行微调。
...
### [124] [MAGNET: Towards Adaptive GUI Agents with Memory-Driven Knowledge Evolution](https://arxiv.org/abs/2601.19199)
*Libo Sun,Jiwen Zhang,Siyuan Wang,Zhongyu Wei*

Main category: cs.AI

TL;DR: MAGNET是一个具有双级记忆的移动GUI智能体框架，通过将视觉特征链接到稳定功能语义来适应UI变化，提升在动态软件环境中的任务执行能力。


<details>
  <summary>Details</summary>
Motivation: 移动GUI智能体在频繁更新的UI环境中容易失效，因为界面外观变化和流程重组会导致基于历史数据训练的智能体失败。尽管表面变化，但功能语义和任务意图基本保持稳定。

Method: 提出MAGNET框架，包含双级记忆：静态记忆将多样化视觉特征链接到稳定功能语义以实现稳健的动作定位；过程记忆捕捉不同工作流程中的稳定任务意图。采用动态记忆进化机制，通过优先访问频繁使用的知识来持续优化两种记忆。

Result: 在AndroidWorld在线基准测试中相比基线有显著改进，离线基准测试在分布偏移下也显示出一致的性能提升。验证了利用界面变化中的稳定结构可以提高智能体性能和泛化能力。

Conclusion: 通过识别和利用UI变化中的稳定功能语义和任务意图，MAGNET框架能够有效适应动态软件环境，提高移动GUI智能体的鲁棒性和泛化能力。
...
TL;DR: MATA是一个用于视觉推理的多智能体分层可训练自动机系统，通过可训练的超智能体选择顶层状态转移，每个智能体运行基于规则的子自动机，共享内存实现透明执行历史，在多个视觉推理基准上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型具有强大的感知能力，但其隐含推理难以解释，在复杂查询上容易产生幻觉。组合方法提高了可解释性，但大多依赖单一智能体或手工制作的流水线，无法决定何时在互补智能体之间协作或在重叠智能体之间竞争。

Method: 引入MATA（多智能体分层可训练自动机），这是一个以分层有限状态自动机形式呈现的多智能体系统。顶层转移由可训练的超智能体选择，每个智能体对应超自动机中的一个状态，运行小型基于规则的子自动机进行可靠的微控制。所有智能体读写共享内存，产生透明的执行历史。为监督超智能体的转移策略，构建转移轨迹树并转换为内存到下一状态对，形成MATA-SFT-90K数据集用于监督微调。

Result: 在多个视觉推理基准测试中，MATA相比单一模型和组合基线方法取得了最先进的结果。微调后的LLM作为转移策略能够理解查询和智能体能力，并能高效选择最优智能体解决任务。

Conclusion: MATA通过多智能体分层可训练自动机框架，在保持可解释性的同时提升了视觉推理性能，解决了现有方法在协作与竞争决策方面的局限性，为复杂视觉推理任务提供了透明且高效的解决方案。
...
TL;DR: 本文提出了一种名为SpikeScore的新方法，用于检测大语言模型的幻觉，该方法通过量化多轮对话中的不确定性波动来提升跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法在同一领域内表现良好，但在跨领域场景下泛化能力差。本文研究了一个重要但被忽视的问题——可泛化的幻觉检测（GHD），旨在在单一领域数据上训练检测器，同时确保在不同相关领域中的鲁棒性能。

Method: 通过模拟大语言模型初始响应后的多轮对话，观察到幻觉引发的多轮对话在不同领域中普遍表现出比事实性对话更大的不确定性波动。基于这一现象，提出了SpikeScore评分，用于量化多轮对话中的突发波动。通过理论分析和实证验证，证明SpikeScore在幻觉和非幻觉响应之间具有强大的跨领域可分离性。

Result: 在多个大语言模型和基准测试上的实验表明，基于SpikeScore的检测方法在跨领域泛化方面优于代表性基线方法，并且超越了先进的面向泛化的方法，验证了该方法在跨领域幻觉检测中的有效性。

Conclusion: SpikeScore通过量化多轮对话中的不确定性波动，为解决幻觉检测的跨领域泛化问题提供了一种有效方法，能够显著提升检测器在不同相关领域中的性能表现。
...
### [127] [RPO:Reinforcement Fine-Tuning with Partial Reasoning Optimization](https://arxiv.org/abs/2601.19404)
...
TL;DR: RPO是一种新的强化微调算法，通过仅生成推理路径的后缀部分来大幅减少训练时的token生成量，相比传统方法能减少约95%的生成开销，显著加速训练过程。
...
Method: 提出RPO（Partial Reasoning Optimization）算法，这是一种即插即用的强化微调方法。与传统方法生成完整推理路径不同，RPO通过经验缓存仅生成推理路径的后缀部分进行训练。

Result: RPO在训练rollout阶段减少了约95%的token生成，显著降低了理论时间开销。相比完整路径强化微调算法，RPO将1.5B模型的训练时间减少90%，7B模型减少72%。同时能与GRPO、DAPO等典型算法集成，在保持性能的同时实现训练加速。

Conclusion: RPO是一种高效的强化微调算法，通过部分推理优化大幅减少了训练计算开销，实现了显著的训练加速，同时保持与原始算法相当的性能表现。
...
### [128] [Fuzzy expert system for the process of collecting and purifying acidic water: a digital twin approach](https://arxiv.org/abs/2601.19527)
*Temirbolat Maratuly,Pakizar Shamoi,Timur Samigulin*

Main category: cs.AI

TL;DR: 本文提出了一种结合模糊专家系统和数字孪生的智能控制系统，用于酸性水净化处理，通过模拟人类推理来维持关键参数在期望水平，并设计了简单直观的控制策略。


<details>
  <summary>Details</summary>
Motivation: 酸性水净化对于减少排放、降低腐蚀风险、实现处理水回用以及降低运营成本至关重要。自动化净化过程可以减少人员参与，降低工人伤害风险。原油中的酸性成分在处理过程中会释放到酸性水中，若不妥善处理会对环境造成严重威胁并加速设备腐蚀。

Method: 开发了模糊专家系统与定制数字孪生相结合的控制方案。数字孪生使用Honeywell UniSim Design R492开发，阀门动态通过MATLAB系统辨识建模，实时数据交换使用OPC DA实现。模糊控制器采用分程控制策略控制两个阀门，测试了21种不同初始压力条件和5种去模糊化策略，共105个测试场景。
...
### [129] [Benchmarks Saturate When The Model Gets Smarter Than The Judge](https://arxiv.org/abs/2601.19532)
*Marthe Ballon,Andres Algaba,Brecht Verbeken,Vincent Ginis*

Main category: cs.AI

TL;DR: Omni-MATH-2是一个经过人工修订的数学基准数据集，包含4181个精确答案问题和247个非标准问题，通过审核确保可编译性、可解性和可验证性，显著减少了数据集噪声，为模型性能评估提供了更精确的工具。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型基准测试存在数据集和评估方法的不准确性，这些问题削弱了基准测试的有效性。作者希望通过创建高质量的修订数据集来减少数据集噪声，提供更精确的模型性能评估。

Method: 创建Omni-MATH-2数据集，对原始Omni-MATH进行人工修订：1）审核每个问题确保LaTeX可编译性、可解性和可验证性；2）添加缺失的图表或信息；3）标记需要证明、估计或图像的问题；4）移除杂乱内容；5）使用专家标注评估评估者噪声，比较GPT-5 mini与原始Omni-Judge的差异。

Result: 1）修订后的数据集显著减少了数据集噪声；2）评估者噪声分析显示，在评估者分歧中，Omni-Judge在96.4%的情况下是错误的；3）随着问题难度增加，需要更胜任的评估者来防止评估错误掩盖模型间的真实差异；4）对于标记的非标准问题子集，两个评估者都未能识别当前的失败模式。

Conclusion: 数据集质量和评估者可靠性对于开发准确的模型性能基准都至关重要。高质量的修订数据集和胜任的评估者对于准确评估大语言模型的数学能力是必要的，特别是在问题变得更加复杂时。
...
<details>
  <summary>Details</summary>
Motivation: 传统启发式函数需要人工设计，依赖专家知识且耗时。随着LLM和进化框架的发展，自动化启发式设计成为可能。本文旨在扩展EoH框架，利用LLM的上下文学习能力自动生成高质量的A*搜索启发式函数。

Method: 提出A-CEoH（算法上下文EoH）框架，采用领域无关的提示增强策略，将A*算法代码融入提示词中，利用LLM的上下文学习能力。在Unit-Load Pre-Marshalling Problem（UPMP）和滑动拼图问题（SPP）两个领域进行实验验证。

Result: 计算实验表明，A-CEoH能够显著提高生成启发式函数的质量，在UPMP和SPP问题上甚至能够超越专家设计的启发式函数。

Conclusion: A-CEoH框架通过将算法代码融入提示词的创新方法，有效利用LLM的上下文学习能力，实现了高质量启发式函数的自动生成，为自动化启发式设计提供了新思路。
...
### [131] [Agentic Design Patterns: A System-Theoretic Framework](https://arxiv.org/abs/2601.19752)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于基础模型的智能体AI系统存在幻觉、推理能力差等问题，且设计常常是临时性的，导致应用不可靠和脆弱。现有的智能体设计模式描述缺乏严格的系统理论基础，多为高层次或基于便利性的分类，难以实际实施。
...
Conclusion: 这项工作为智能体AI系统的工程化提供了理论基础和实用工具，有助于构建更模块化、可理解和可靠的自主系统，标准化了智能体设计沟通，推动了该领域的系统化发展。
...
TL;DR: 本文提出了一种基于规则的激活安全新范式，将LLM激活建模为可组合的认知元素，通过定义谓词规则实时检测违规行为，提高精度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活的安全监控方法存在精度低、灵活性差和缺乏可解释性的问题，这些方法基于广泛的滥用数据集训练，难以捕捉细粒度的领域特定行为。

Method: 提出规则化激活安全范式：1) 将激活建模为认知元素（CEs），如"发出威胁"和"支付处理"等细粒度可解释因子；2) 构建基于CEs的谓词规则框架，实时检测违规行为；3) 开发GAVEL开源框架和自动化规则创建工具。

Result: 基于规则的组合式激活安全方法提高了检测精度，支持领域定制化，为可扩展、可解释和可审计的AI治理奠定了基础。

Conclusion: 规则化激活安全范式通过将激活建模为可组合的认知元素，实现了更高精度的领域特定行为检测，同时支持透明度和可审计性，为AI安全监控提供了新的实用框架。
...
### [133] [An Interpretable Recommendation Model for Psychometric Data, With an Application to Gerontological Primary Care](https://arxiv.org/abs/2601.19824)
...
TL;DR: 该研究提出了一种用于老年初级医疗保健的推荐系统，通过心理测量数据结构提供可视化解释，以解决医疗环境中推荐系统的挑战。


<details>
  <summary>Details</summary>
Motivation: 医疗环境中推荐系统面临多重挑战：缺乏公开临床数据、用户难以理解推荐原因、遵循推荐存在风险、以及推荐效果不确定性。特别是在老年初级医疗保健领域，需要能够生成可解释推荐以帮助制定个性化护理计划。

Method: 开发了一种推荐模型，利用心理测量数据的结构来提供忠实于模型且能被护理专业人员理解的可视化解释。专注于老年初级医疗保健这一细分领域，通过离线性能评估和用户研究验证模型效果。

Result: 在巴西研究伙伴收集的医疗数据集上进行了比较性离线性能评估，并进行了评估模型生成的可视化解释可解释性的用户研究。结果显示该模型能够推进推荐系统在这一医疗细分领域的应用。

Conclusion: 提出的推荐模型能够帮助护理专业人员制定个性化护理计划，随着人口结构变化带来的需求增长，该模型有望在老年初级医疗保健领域发挥重要作用。
...
### [134] [Routing End User Queries to Enterprise Databases](https://arxiv.org/abs/2601.19825)
*Saikrishna Sudarshan,Tanay Kulkarni,Manasi Patwardhan,Lovekesh Vig,Ashwin Srinivasan,Tanmay Tulsidas Verlekar*

Main category: cs.AI

TL;DR: 该论文研究了在多数据库企业环境中自然语言查询的路由问题，通过扩展现有NL-to-SQL数据集构建了更现实的基准测试，并提出了一种基于推理的模块化重排序策略，显著优于嵌入方法和直接LLM提示方法。


<details>
  <summary>Details</summary>
Motivation: 在多数据库企业环境中，随着数据库规模增大和领域重叠增加，自然语言查询的路由变得越来越具有挑战性，特别是面对模糊查询时，需要更结构化、更鲁棒的基于推理的解决方案。

Method: 提出了一种模块化、推理驱动的重排序策略，通过显式建模模式覆盖、结构连接性和细粒度语义对齐来改进查询路由。

Result: 该方法在所有评估指标上一致地优于仅使用嵌入的方法和直接LLM提示的基线方法。

Conclusion: 在多数据库企业环境中，显式建模模式覆盖、结构连接性和语义对齐的推理驱动方法能够显著提高自然语言查询路由的性能，特别是在面对大规模、领域重叠的数据库和模糊查询时。
...
### [135] [Visual Generation Unlocks Human-Like Reasoning through Multimodal World Models](https://arxiv.org/abs/2601.19834)
...
TL;DR: 本文首次系统研究了视觉生成何时及如何提升推理能力，提出视觉优势假说：在物理世界相关任务中，视觉生成能更自然地作为世界模型，而纯语言模型会遭遇表征限制或先验知识不足的瓶颈。
...
Method: 从世界模型视角出发，理论层面将内部世界建模形式化为思维链推理的核心组件，分析不同形式世界模型的区别；实证层面识别需要交错视觉-语言思维链推理的任务，构建新的评估套件VisWorld-Eval，并在最先进的统一多模态模型上进行控制实验。

Result: 实验表明，在有利于视觉世界建模的任务上，交错思维链显著优于纯语言思维链，但在其他任务上没有明显优势。这验证了视觉优势假说。
