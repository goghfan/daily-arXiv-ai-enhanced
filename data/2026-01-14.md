...
TL;DR: 该论文提出了一种基于边缘AI的实时路边感知节点，用于多类交通违规分析和安全事件传播，在印度等新兴经济体中解决交通执法不对称问题。


<details>
  <summary>Details</summary>
Motivation: 印度等新兴经济体快速机动化导致严重的执法不对称问题：2023年记录了超过1100万次违规，而警力密度仅为每4000辆车一名警官。传统监控和人工罚单无法应对这种规模，需要自主、协同且节能的边缘AI感知基础设施。

Method: 系统集成YOLOv8 Nano进行高精度多目标检测，DeepSORT实现时间一致的车辆跟踪，以及基于规则的OCR后处理引擎，能够识别符合MoRTH AIS 159和ISO 7591视觉对比标准的退化或多语言车牌。部署在NVIDIA Jetson Nano上，通过TensorRT FP16量化优化。

Result: 系统在9.6W功耗下维持28-30帧/秒的推理速度，实现97.7%的违规检测准确率和84.9%的OCR精度，涵盖五种违规类别：闯红灯、斑马线违规、逆行、非法掉头和超速。相比YOLOv4 Tiny、PP YOLOE S和Nano DetPlus，平均精度提升10.7%，每瓦精度提高1.4倍。

Conclusion: 该路边边缘AI分析节点能够通过V2X协议向联网车辆和智能交通系统后端发布标准化的CAM和DENM类型安全事件，证明路边边缘AI分析可以增强协同感知和主动道路安全管理，符合IEEE智能车辆生态系统。
...
TL;DR: 论文提出了ROAD基准测试，用于评估LiDAR点云分类在持续学习和迁移学习中的表现，特别关注同时发生的领域偏移和标签演变问题。


<details>
  <summary>Details</summary>
Motivation: 3D感知系统在实际应用中需要适应不断变化的对象定义和传感器领域，但目前3D点云感知中的持续学习和迁移学习研究相对不足，特别是在同时面临领域偏移和标签演变的情况下。

Method: 提出了ROAD基准测试，专门针对LiDAR点云分类，考虑了领域偏移以及三种标签演变形式：类别分裂、类别扩展和类别插入。使用大规模数据集（Waymo、NuScenes、Argoverse2）评估零样本迁移、线性探测和持续学习方法。

Result: 评估了现有方法在现实偏移下的局限性，包括骨干架构、训练目标和持续学习方法的影响，为未来稳健3D感知研究建立了强基线。

Conclusion: 现有方法在现实偏移下存在局限性，ROAD基准测试为未来研究提供了全面的评估框架，有助于推动稳健3D感知系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 2D拖拽编辑已取得显著进展，但3D场景的直观拖拽编辑仍然面临挑战。现有3D编辑方法在几何相关内容编辑方面存在局限，需要一种更高效、直观的3D编辑解决方案
...
Conclusion: 3DGS-Drag填补了变形基和2D编辑基3D编辑方法之间的空白，为真实3D场景提供了高效、直观的拖拽编辑能力，推动了3D内容创作的进步
...
### [4] [Predicting Region of Interest in Human Visual Search Based on Statistical Texture and Gabor Features](https://arxiv.org/abs/2601.07998)
*Hongwei Lin,Diego Andrade,Mini Das,Howard C. Gifford*

Main category: cs.CV

TL;DR: 该研究探索了结合Gabor特征和GLCM纹理特征来建模早期视觉搜索行为，通过两种特征融合管道预测人类注视区域，并在乳腺断层合成图像上验证了有效性。
...
Method: 提出了两种特征融合管道来整合Gabor和GLCM特征，用于缩小可能的人类注视区域。使用模拟的乳腺断层合成图像进行评估，并与基于阈值的模型观察者进行比较。同时收集了人类观察者的眼动追踪数据。

Result: 结果显示，提出的管道预测的注视候选区域与基于阈值的模型观察者在定性上一致。观察到GLCM均值与Gabor特征响应之间存在强相关性，表明尽管公式不同，这些特征编码了相关的图像信息。眼动追踪数据进一步表明预测的注视区域与早期注视行为具有一致性。

Conclusion: 这些发现强调了结合结构和纹理特征对于建模视觉搜索的价值，支持开发感知信息化的观察者模型。特征融合方法能够有效预测人类早期视觉搜索行为，为视觉搜索建模提供了新的思路。
...
TL;DR: 提出一种解耦驾驶员外观特征的视觉语言模型框架，通过分离身份特征与行为线索来提升分心驾驶检测的零样本性能


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的分心驾驶检测器在真实场景中表现不佳，主要原因是模型将驾驶员特定外观特征（如服装、年龄、性别）与行为线索纠缠在一起，导致决策基于"驾驶员是谁"而非"驾驶员在做什么"

Method: 提出主体解耦框架：1) 提取驾驶员外观嵌入；2) 在零样本分类前从图像嵌入中移除外观影响；3) 通过Stiefel流形上的度量投影正交化文本嵌入，提高可分性同时保持原始语义

Result: 实验表明该方法相比现有基线方法取得了一致的性能提升，证明了其在道路安全应用中的潜力

Conclusion: 通过解耦驾驶员外观特征并正交化文本嵌入，能够有效提升视觉语言模型在分心驾驶检测中的零样本性能，为实际道路安全应用提供了有前景的解决方案
...
TL;DR: 提出AKT模型解决无人机影像中玉米点级定位的三大挑战：极小目标、高计算成本、农业场景复杂性，通过PKAN模块和PAA注意力机制提升性能


<details>
  <summary>Details</summary>
Motivation: 高分辨率无人机摄影测量在精准农业中至关重要，但玉米点级定位面临三大挑战：1) 目标像素比极小（<0.1%）；2) 超高分辨率图像（>3000x4000像素）的二次注意力计算成本过高；3) 农业场景特有的稀疏目标分布和环境变化问题，通用视觉模型处理不佳

Method: 提出加性Kolmogorov-Arnold Transformer (AKT)：1) 用Pade Kolmogorov-Arnold Network (PKAN)模块替代传统多层感知机，增强小目标特征提取的功能表达能力；2) 引入PKAN加性注意力(PAA)建模多尺度空间依赖，降低计算复杂度；3) 构建包含1,928张高分辨率无人机图像、约501,000个点标注的PML数据集

Result: AKT平均F1分数达62.8%，优于最先进方法4.2%，同时FLOPs减少12.6%，推理吞吐量提升20.7%；在下游任务中，株数统计的平均绝对误差为7.1，株间距估计的均方根误差为1.95-1.97厘米
...
TL;DR: 提出了一种新的统计理想观察者模型，通过设置最小可提取图像特征阈值来实现整体视觉搜索处理，减少自由参数并缩小系统规模


<details>
  <summary>Details</summary>
Motivation: 开发一个能够执行整体视觉搜索处理的统计理想观察者模型，通过减少自由参数来简化系统，为医学图像感知、计算机视觉等领域提供优化工具

Method: 建立基于最小可提取图像特征阈值的统计理想观察者模型，通过设置阈值限制来实现整体视觉搜索处理，从而减少模型中的自由参数

Result: 开发了一个新的统计理想观察者框架，能够有效执行整体视觉搜索处理，同时通过参数缩减简化系统结构

Conclusion: 该模型在医学图像感知、计算机视觉、性能基准测试、特征选择评估、目标检测识别以及传感器评估等多个领域具有广泛应用前景
...
TL;DR: CASHEW和CASHEW-RL是两个提升视觉语言模型多步推理稳定性的方法，通过聚合多个推理轨迹并进行视觉验证来减少不一致性，在多个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解和推理任务中表现良好，但其多步推理存在不稳定性问题——对相同输入的重复采样会产生不同的推理轨迹和不一致的最终预测。

Method: 提出了两种互补方法：1) CASHEW推理时框架，通过迭代聚合多个候选推理轨迹形成更高质量的推理路径，并通过显式视觉验证过滤幻觉步骤；2) CASHEW-RL学习变体，使用组序列策略优化(GSPO)训练，通过复合奖励鼓励基于最小充分视觉证据的正确答案，并根据任务难度自适应分配推理努力。

Result: 在13个图像理解、视频理解和视频推理基准测试中显示出显著的性能提升，包括在ScienceQA上提升23.6个百分点，在EgoSchema上提升8.1个百分点。

Conclusion: CASHEW和CASHEW-RL方法有效解决了视觉语言模型多步推理的不稳定性问题，通过推理时聚合和学习内部化聚合行为，显著提升了模型在各种多模态任务中的表现。
...
TL;DR: TP-Blend是一个无需训练的轻量级框架，通过两个独立的文本提示（一个指定混合对象，一个定义目标风格）在单个去噪轨迹中同时注入对象和风格，解决了现有方法难以同时引入新对象和新风格的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本条件的扩散编辑方法在处理单个对象替换时表现良好，但当需要同时引入新对象和新风格时存在困难。现有方法难以在保持高质量的同时实现内容和外观的精确控制。

Method: TP-Blend采用两个互补的注意力处理器：1）交叉注意力对象融合（CAOF）：通过平均头部注意力定位对任一提示响应强烈的空间标记，然后解决熵正则化的最优传输问题，将完整的多头特征向量重新分配到这些位置；2）自注意力风格融合（SASF）：通过细节敏感实例归一化在每个自注意力层注入风格，使用一维高斯滤波器分离低频和高频分量，仅混合高频残差以保留全局几何结构，并交换键值矩阵以实现上下文感知的纹理调制。

Result: TP-Blend能够生成高分辨率、照片般逼真的编辑结果，在内容和外观上都实现了精确控制。在定量保真度、感知质量和推理速度方面均超越了最近的基线方法。

Conclusion: TP-Blend是一个有效的训练免费框架，能够同时处理对象替换和风格转换，在保持高质量输出的同时实现了内容和外观的精确控制，为扩散模型的编辑能力提供了新的解决方案。
...
TL;DR: 提出基于解码器的深度学习框架，从潜在表示生成可制造的3D结构，优化增材制造约束条件


<details>
  <summary>Details</summary>
Motivation: 解决传统3D生成方法忽视制造约束的问题，提高生成结构的可制造性和实用性

Method: 采用基于深度学习的解码器框架，学习从抽象表示到几何有效3D结构的映射，同时考虑悬垂角度、壁厚、结构完整性等制造约束

Result: 在多种物体类别上验证了方法的有效性，生成的结构相比朴素方法显著提高了可制造性，并成功进行了实际3D打印

Conclusion: 神经解码器能够学习复杂映射函数，生成满足制造约束的几何有效3D对象，为优化增材制造的设计生成提供了有效解决方案
...
TL;DR: 提出了一种无需训练、仅使用视觉的零样本异常检测框架DIVAD，通过DDIM反演和重构来检测和定位异常，无需细粒度提示或辅助模态。


<details>
  <summary>Details</summary>
Motivation: 当前零样本异常检测方法要么依赖语言等辅助模态生成细粒度提示，要么仅能进行图像级分类而缺乏空间定位精度。需要一种简单有效的纯视觉方法，避免对提示的依赖。

Method: 利用预训练的DDIM模型，将输入图像反演到潜在空间，从固定中间时间步开始去噪重构。由于扩散模型仅在正常数据上训练，重构结果会呈现正常外观。通过比较输入图像与重构图像的差异来检测和定位异常。

Result: 在VISA数据集上取得了最先进的性能，展示了强大的异常定位能力，无需辅助模态，减少了对提示的依赖。

Conclusion: DIVAD框架为仅使用视觉的零样本异常检测提供了一种简单有效的解决方案，推动了该领域从依赖提示向更自主检测的方向发展。
...
TL;DR: 提出CBD概念多样性度量，结合Margin不确定性度量，构建高效混合输入选择方法，显著提升DNN微调效果并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: DNN微调需要标注新数据，但标注成本高、耗时长。现有多样性选择方法计算密集且难以扩展到大规模数据集，限制了实际应用。

Method: 提出概念多样性(CBD)度量，利用视觉语言模型(VLM)高效评估图像输入多样性；结合Margin不确定性度量构建混合输入选择方法。

Result: CBD与几何多样性(GD)高度相关但计算时间大幅减少；CBD-based选择方法在多种DNN模型、输入集和预算下均优于现有最佳基线方法。

Conclusion: CBD-based方法在保持接近简单不确定性方法选择时间的同时，显著提升输入选择效果，具有高效性和可扩展性，适用于重复大规模输入选择场景。
...
<details>
  <summary>Details</summary>
Motivation: 科学复合图通常包含多个带标签的面板，但在实际流程中，图注经常缺失或仅提供图级摘要，这使得面板级理解变得困难。现有方法难以处理开放描述中的多样化表达方式。

Method: 提出FigEx2视觉条件框架，包含噪声感知门控融合模块，自适应过滤token级特征以稳定检测查询空间。采用分阶段优化策略，结合监督学习和强化学习，利用CLIP对齐和BERTScore语义奖励确保严格的多模态一致性。构建了BioSci-Fig-Cap基准数据集。

Result: FigEx2在检测任务上达到0.726 mAP@0.5:0.95的优异性能，在METEOR和BERTScore上分别显著优于Qwen3-VL-8B模型0.51和0.24分。更重要的是，该模型在未经微调的情况下展现出对分布外科学领域的卓越零样本迁移能力。

Conclusion: FigEx2通过创新的噪声感知融合模块和强化学习优化策略，有效解决了科学复合图面板定位和描述生成问题，在跨学科任务中表现出强大的零样本迁移能力，为科学图像理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学出版物中的科学图像篡改对研究完整性和可重复性构成威胁，但生物医学伪造检测面临领域特异性伪影、复杂纹理和非结构化布局等独特挑战。

Method: 提出视觉语言引导框架，结合扩散合成与视觉语言提示实现语义可控的伪造生成；引入Rescind基准数据集和Integscan检测框架，集成注意力增强视觉编码与提示条件语义对齐；采用视觉语言模型验证循环确保语义保真度。

Result: 在Rescind和现有基准上的广泛实验表明，Integscan在伪造检测和定位方面达到最先进性能，为自动化科学完整性分析奠定了坚实基础。

Conclusion: 该研究建立了首个视觉语言引导的生物医学图像伪造生成与检测框架，通过创新的数据集和模型架构，显著提升了生物医学图像伪造检测的准确性和实用性。
...
TL;DR: 研究表明，在训练数据中策略性地加入少量（10%）噪声数据可以显著提升CNN模型在噪声测试条件下的鲁棒性，同时对干净数据的性能影响很小。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像数据经常受到噪声和失真的影响，而传统训练通常使用高质量数据。研究旨在探索通过控制性引入训练噪声来提升模型在实际噪声环境中的鲁棒性。

Method: 使用CIFAR-10数据集，评估三种常见噪声类型（高斯噪声、椒盐噪声、高斯模糊）在不同强度和训练集污染水平下的影响。采用Resnet-18模型进行实验。

Result: 实验发现，在训练中仅加入10%的噪声数据就足以显著降低测试损失，并在完全噪声测试条件下提高准确率，同时对干净数据的性能影响最小。

Conclusion: 策略性暴露于噪声可以作为一种简单有效的正则化方法，在传统数据清洁度和实际环境鲁棒性之间提供了实用的权衡。
...
TL;DR: DINO-AugSeg：一种利用DINOv3特征进行小样本医学图像分割的新框架，通过小波特征增强和上下文融合模块提升分割性能
...
<details>
  <summary>Details</summary>
Motivation: 解决预训练模型在真实世界部署环境中遇到的分布偏移问题，减少对大量真实世界数据收集的依赖，提高模型在特定领域的适应性和部署效率。

Method: 采用三阶段框架：1）通过控制修复技术在领域特定背景中合成目标对象；2）通过多模态评估验证生成结果，包括目标检测、美学评分和视觉语言对齐；3）使用用户偏好分类器捕捉主观选择标准。

Result: 该管道能够高效构建高质量、可部署的数据集，同时显著减少对真实世界数据收集的依赖，提高模型在特定领域的适应性和性能。

Conclusion: 提出的自动化管道为生成领域特定合成数据集提供了有效解决方案，通过扩散模型和多模态评估机制，实现了高质量数据集的快速构建，有助于解决模型部署中的分布偏移问题。
...
TL;DR: PathoGen：基于扩散模型的病理图像生成方法，通过可控的高保真病灶修复技术解决病理AI训练数据稀缺问题，在多种组织类型上优于现有生成方法，并能提升下游分割性能。


<details>
  <summary>Details</summary>
Motivation: 病理学AI模型开发面临专家标注数据稀缺的严重制约，特别是罕见病理和代表性不足的疾病亚型。现有数据增强方法无法生成足够真实的病灶形态，难以保持组织病理学特征的空间关系和细胞结构。

Method: 提出PathoGen，一种基于扩散模型的生成模型，能够在良性病理图像中进行可控、高保真的病灶修复。利用扩散模型的迭代细化过程合成具有自然组织边界、保留细胞结构和真实染色特征的病灶。

Result: 在肾脏、皮肤、乳腺和前列腺四个不同诊断挑战的数据集上验证，定量评估显示PathoGen在图像保真度和分布相似性上优于条件GAN和Stable Diffusion等最先进生成基线。使用PathoGen合成的病灶增强训练集能提升下游分割性能，特别是在数据稀缺情况下。
...
<details>
  <summary>Details</summary>
Motivation: 音频-视觉语义分割（AVSS）需要在像素级识别发声物体的基础上增加语义理解能力。现有方法将任务分解为两个子任务，但仍有改进空间，特别是在处理移动物体和静止发声物体时面临挑战。

Method: 提出SSP框架：1）使用光流捕捉运动动态，为分割提供时间上下文；2）引入两种文本提示（物体类别和场景描述）处理静止发声物体；3）设计视觉-文本对齐模块实现跨模态整合；4）采用后掩码技术训练模型学习光流模式。

Result: 实验结果表明，SSP框架在音频-视觉分割任务上优于现有AVS方法，能够提供更高效和精确的分割结果。

Conclusion: SSP框架通过整合光流和文本提示，有效解决了音频-视觉语义分割中的关键挑战，为多模态场景理解提供了新的解决方案。
...
TL;DR: SubTTA提出了一种新的测试时适应方法，通过对齐视觉和文本模态的语义子空间来改善零样本预测，从而更可靠地指导自训练过程。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时适应方法严重依赖零样本预测作为伪标签进行自训练，但在分布偏移下不可靠。这主要由于两个根本限制：1）模态间隙：分布偏移导致视觉和文本模态之间的差距，使跨模态关系不准确；2）视觉干扰：视觉嵌入编码了丰富但与任务无关的噪声，在分布偏移下常常淹没任务特定语义。

Method: SubTTA通过对齐两种模态的语义子空间来增强零样本预测。具体方法：1）提取两种模态的主子空间，通过最小化弦距离将视觉流形对齐到文本语义锚点；2）将对齐后的视觉特征投影到任务特定的文本子空间，通过将视觉嵌入约束在有效语义范围内来过滤任务无关噪声；3）在净化后的空间上执行标准TTA以细化决策边界。

Result: 在各种基准测试和VLM架构上的广泛实验表明，SubTTA的有效性，相比最先进的TTA方法平均提升2.24%。

Conclusion: SubTTA通过语义子空间对齐有效解决了现有TTA方法在分布偏移下的局限性，通过减少模态间隙和消除视觉干扰，显著提升了视觉语言模型的测试时适应性能。
...
TL;DR: 本文通过层级的掩码分析揭示了多模态大语言模型中视觉-文本融合的演化模式，发现融合发生在特定层而非均匀分布，某些模型存在后期"回顾"现象，并提出了无需训练的对比注意力框架来提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉-语言理解方面取得了显著进展，但其内部如何整合视觉和文本信息仍不清楚。为了填补这一知识空白，需要对模型内部的视觉-文本融合机制进行系统性分析。

Method: 1. 对多种架构进行系统性的层级掩码分析，揭示视觉-文本融合在模型内部的演化过程；2. 分析层级注意力演化，观察注意力模式的变化；3. 提出无需训练的对比注意力框架，建模早期融合层与最终层之间的注意力转换。

Result: 研究发现：1. 融合出现在几个特定层而非均匀分布；2. 某些模型存在后期"回顾"现象，在输出生成前重新激活视觉信号；3. 观察到对无关区域的持续高注意力噪声，以及对文本对齐区域逐渐增强的注意力；4. 提出的对比注意力框架在各种MLLM和基准测试中验证了分析的有效性，并提升了多模态推理性能。

Conclusion: 本文通过系统性分析揭示了多模态大语言模型中视觉-文本融合的内部机制，发现了融合的层级特性和注意力演化模式，并基于这些洞察提出了有效的训练无关方法，为理解和改进多模态模型提供了重要见解。
...
Method: 引入实例对齐字幕框架，将每个文本声明链接到具有外观和运动属性的特定对象实例，能够捕获谁导致了异常、每个实体在做什么、影响了谁以及解释的空间定位。
...
Conclusion: 提出的实例对齐字幕方法为可验证和可操作推理提供了框架，同时VIEW360+数据集为可信赖和可解释异常检测的未来研究提供了稳健基准。
...
<details>
  <summary>Details</summary>
Motivation: 火箭发射时极端成像条件（浓密颗粒雾霾、超过120dB的亮度变化）严重阻碍了羽流流场、冲击波结构、喷管振荡等关键机械参数的光学测量，传统图像数据质量差，影响后续摄影测量和测速分析

Method: 硬件-算法协同设计框架：1）定制空间变化曝光传感器单次拍摄获取多曝光数据，实现鲁棒雾霾评估；2）物理感知去雾算法动态估计雾霾密度、区域自适应光照优化、多尺度熵约束融合，有效分离雾霾与场景辐射

Result: 在真实发射图像和受控实验中验证，该框架在恢复羽流和发动机区域物理准确视觉信息方面表现出优越性能，为提取粒子速度、流动不稳定频率、结构振动等关键机械参数提供可靠图像基础
...
TL;DR: 提出SISTA框架，通过多级对齐改进医学视觉语言预训练，消除假阴性样本并实现图像块与词语的对齐，提升下游任务性能。
...
Method: 提出SISTA框架，在图像-报告和图像块-词语两个层面进行语义对齐：1) 通过引入报告间相似性消除假阴性样本；2) 有效对齐图像块与相关词语标记。

Result: 实验表明该框架在不同数据集的下游任务（图像分类、分割、目标检测）中提升迁移性能，特别是在有限标注数据的细粒度任务上取得显著改进。

Conclusion: SISTA框架通过多级语义对齐有效改进医学视觉语言预训练，消除假阴性问题并实现更精细的对齐，在多个下游任务中表现出优越性能。
...
TL;DR: 该技术报告介绍了RoboSense2025挑战赛中跨平台3D目标检测任务的获奖解决方案，基于PVRCNN++框架，通过数据增强和自训练策略提升跨平台泛化能力，在比赛中获得第三名。


<details>
  <summary>Details</summary>
Motivation: 解决跨平台3D目标检测中的域差距问题，提升模型在不同平台数据上的泛化性能，以适应实际应用中不同传感器配置的场景。

Method: 基于PVRCNN++框架，结合点云和体素特征，采用定制化数据增强和基于伪标签的自训练策略来缩小域差距。

Result: 在挑战赛第一阶段目标域上，Car类别的3D AP达到62.67%；第二阶段目标域上，Car和Pedestrian类别分别达到58.76%和49.81%，获得比赛第三名。

Conclusion: 通过数据增强和自训练策略有效提升了跨平台3D目标检测的泛化能力，在RoboSense2025挑战赛中取得了优异成绩。
...
TL;DR: CogniMap3D是一个受生物启发的动态3D场景理解与重建框架，模仿人类认知过程，通过持久记忆库存储静态场景，支持多访问场景下的高效空间知识存储与检索。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法在处理动态场景和多访问场景时存在局限性，缺乏类似人类认知的持久记忆机制。需要开发能够模拟人类认知过程、支持长期场景理解和更新的框架。

Method: 框架包含三个核心能力：1）多阶段运动线索框架识别动态对象；2）认知映射系统存储、回忆和更新多访问的静态场景；3）因子图优化策略精炼相机位姿。利用深度和相机位姿先验识别动态区域，将静态元素与记忆库匹配。

Result: 在视频深度估计、相机位姿重建和3D映射任务上达到最先进性能，有效支持跨长序列和多访问的连续场景理解。
...
TL;DR: 提出一个基于文本指令驱动的人脸表情生成框架，能够从一张人脸图像开始，生成从一个指定表情到另一个表情的平滑过渡序列。


<details>
  <summary>Details</summary>
Motivation: 传统的3D虚拟化身通常只有六种基本表情，为了模拟更真实的情感变化，需要能够渲染任意两个表情之间的过渡。文本描述能够提供对人类情感状态的多样化描述，从而大大扩展表情库和表情过渡的可能性。

Method: 1. 引入指令驱动面部表情分解器（IFED）模块，促进多模态数据学习，捕捉文本描述与面部表情特征之间的相关性。2. 提出指令到面部表情过渡（I2FET）方法，利用IFED和顶点重建损失函数来细化潜在向量的语义理解，根据给定指令生成表情序列。3. 提出面部表情过渡模型，生成表情之间的平滑过渡。

Result: 在CK+和CelebV-HQ数据集上的广泛评估表明，所提出的模型优于最先进的方法。结果显示该框架能够根据文本指令生成面部表情轨迹。

Conclusion: 文本提示允许对人类情感状态进行多样化描述，从而可以大大扩展面部表情库及其之间的过渡。该框架有望在各种实际应用中找到用途。
...
<details>
  <summary>Details</summary>
Motivation: Zhang等人使用简单角点模型获取角点特征存在理论缺陷，因为相邻角点的灰度信息会相互影响。为了解决这个问题，需要开发能够准确检测相邻角点的新方法。

Method: 使用二阶高斯方向导数滤波器对两种典型的高分辨率角点模型进行平滑处理，推导出这些模型的SOGDD表示，发现高分辨率角点的许多特征，并提出如何选择高斯滤波尺度来获取图像强度变化信息。基于此提出了一种新的高分辨率角点检测方法。

Result: 实验结果表明，所提出的方法在定位误差、对图像模糊变换的鲁棒性、图像匹配和3D重建方面优于最先进的方法。

Conclusion: 提出的基于SOGDD的高分辨率角点检测方法能够有效解决相邻角点相互影响的问题，在多个性能指标上优于现有方法，具有实际应用价值。
...
TL;DR: 该研究构建了GI-Bench基准，系统评估了12个多模态大语言模型在胃肠内镜工作流中的表现，发现顶级模型在诊断推理上接近初级内镜医师，但在病灶定位和事实准确性方面存在显著瓶颈。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在胃肠病学中显示出潜力，但其在完整临床工作流程中的表现以及与人类基准的比较尚未得到验证，需要系统评估以确定其临床实用性。

Method: 构建GI-Bench基准，涵盖20种细粒度病变类别。评估12个MLLM在五阶段临床工作流中的表现：解剖定位、病灶识别、诊断、发现描述和管理。使用Macro-F1、mIoU和多维度Likert量表将模型性能与6名人类内镜医师（3名初级医师和3名住院医师）进行对比。

Result: Gemini-3-Pro达到最先进性能。在诊断推理方面，顶级模型（Macro-F1 0.641）优于住院医师（0.492），与初级内镜医师相当（0.727；p>0.05）。但存在"空间定位瓶颈"：人类病灶定位（mIoU >0.506）显著优于最佳模型（0.345；p<0.05）。定性分析显示"流畅性-准确性悖论"：模型生成的报告语言可读性优于人类（p<0.05），但事实准确性显著较低（p<0.05），存在"过度解释"和视觉特征幻觉。

Conclusion: MLLM在胃肠内镜诊断推理方面已接近人类初级医师水平，但在空间定位和事实准确性方面仍存在显著瓶颈。GI-Bench建立了动态排行榜，持续跟踪MLLM在临床内镜中的性能演进。
...
<details>
  <summary>Details</summary>
Motivation: 多站点脑MRI数据聚合可以增强深度学习模型训练，但引入了由站点特定变化（如扫描仪厂商、采集参数和成像协议差异）导致的非生物异质性，这会损害模型的泛化能力。现有方法通常依赖有限的配对旅行者数据，或无法有效分离风格与解剖结构，且大多只处理单序列协调，限制了在实际多序列MRI场景中的应用。
...
Result: 在4,163个T1和T2加权MRI上的评估表明，MMH在图像特征聚类、体素级比较、组织分割以及下游年龄和站点分类任务上，均优于现有最先进的方法。
...
TL;DR: MobiDiary框架利用IMU和Wi-Fi信号生成人类可读的日常活动描述，通过统一传感器编码器解决物理信号与语言描述之间的语义鸿沟，在多个基准测试中达到最先进性能。
...
Method: 提出MobiDiary框架：1）使用统一传感器编码器，利用运动和无线信号的共享归纳偏差；2）采用基于补丁的机制捕捉局部时间相关性；3）集成异构位置嵌入统一不同传感器的空间上下文；4）使用基于Transformer的自回归解码器逐词生成连贯的动作描述。

Result: 在多个公开基准测试（XRF V2、UWash、WiFiTAD）上评估，MobiDiary在描述指标（BLEU@4、CIDEr、RMC）上达到最先进性能，在连续动作理解方面优于专门基线方法。

Conclusion: MobiDiary能够从异构物理信号有效生成自然语言活动描述，解决了传统预定义标签方法的限制，为智能家居中的隐私保护活动识别提供了新途径。
...
TL;DR: FUME：首个基于深度学习、利用双气体光学成像检测奶牛瘤胃酸中毒的方法，通过CO2和CH4排放模式分类健康状态，在低计算成本下实现高精度分割和分类。
...
Method: 提出FUME（Fused Unified Multi-gas Emission Network）深度学习框架，利用红外相机捕捉CO2和CH4排放模式。采用轻量级双流架构，包含权重共享编码器、模态特定自注意力机制和通道注意力融合模块，联合优化气体羽流分割和健康状态分类（健康、过渡、酸中毒）。

Result: 创建首个双气体光学成像数据集（8,967标注帧，6个pH水平）。FUME达到80.99% mIoU分割精度和98.82%分类准确率，仅使用1.28M参数和1.97G MACs，在分割质量上优于现有方法且计算成本降低10倍。消融研究表明CO2提供主要判别信号，双任务学习对最优性能至关重要。

Conclusion: 该研究证实了基于气体排放的牲畜健康监测可行性，为实用的体外酸中毒检测系统铺平道路。代码已开源。
...
TL;DR: 该研究比较了基于EVA-ViT图像编码器与LLaMA或ChatGPT LLM的多模态方法，用于减少幻觉问题并检测胸部X光图像中的疾病。研究发现文本RAG能有效减少幻觉，图像RAG能提高预测置信度和校准，GPT LLM表现优于Llama模型。


<details>
  <summary>Details</summary>
Motivation: 解决多模态医学图像分析中的幻觉问题，提高胸部X光疾病检测的准确性和可靠性，通过比较不同RAG方法和LLM模型来优化诊断系统。

Method: 使用NIH胸部X光图像数据集，采用EVA-ViT作为图像编码器，结合LLaMA或ChatGPT LLM构建多模态系统。比较了三种方法：基于图像的RAG、基于文本的RAG和基线方法。文本RAG利用外部知识信息，图像RAG使用KNN方法。

Result: 文本RAG能有效减少幻觉问题；图像RAG提高了预测置信度和校准；GPT LLM在性能、幻觉率和预期校准误差(ECE)方面均优于Llama模型。

Conclusion: 该研究揭示了数据不平衡和复杂多阶段结构的挑战，但提出了大规模实验环境和平衡使用示例的解决方案，为医学图像诊断系统的优化提供了重要参考。
...
TL;DR: HIPPO提出了一种针对视频大语言模型的整体感知并行推测解码框架，通过语义感知的token保留和并行推测解码算法，实现了最高3.51倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的视频大语言模型推测解码方法主要关注剪枝冗余视觉token以减少计算负担，但无法达到与纯文本大语言模型相当的推理加速效果。这主要源于两个限制：1）现有剪枝策略无法充分保留视觉语义token，降低了草稿质量和接受率；2）即使进行激进剪枝（如移除90%视觉token），草稿模型的剩余推理成本仍限制了整体加速效果。

Method: HIPPO提出两个关键技术：1）语义感知token保留方法，融合全局注意力分数与局部视觉语义，在高剪枝率下保留语义信息；2）视频并行推测解码算法，将草稿生成和目标验证阶段解耦并重叠执行。

Result: 在四个视频大语言模型和六个基准测试上的实验表明，HIPPO相比原始自回归解码实现了最高3.51倍的推理加速。

Conclusion: HIPPO通过整体感知的并行推测解码框架，有效解决了视频大语言模型推理加速的瓶颈，显著提升了推理效率。
...
TL;DR: 论文引入KidVis基准测试，基于人类视觉发展理论评估MLLMs的基础视觉能力，发现即使最先进的GPT-5也远低于6-7岁儿童水平，且存在"缩放定律悖论"。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在高级推理任务上表现出色，但尚不清楚它们是否具备与人类直觉相当的基础视觉原语能力。研究者希望通过基于人类视觉发展理论的基准测试来探究这一问题。

Method: 引入KidVis基准测试，将视觉智能分解为6种基础能力（专注、追踪、辨别、记忆、空间、闭合），包含10类低语义依赖的视觉任务。评估20个最先进的MLLMs，并与人类生理基线（6-7岁儿童）进行对比。

Result: 人类儿童平均得分接近完美（95.32），而最先进的GPT-5仅得67.33。发现"缩放定律悖论"：单纯增加模型参数无法线性提升这些基础视觉能力。

Conclusion: 当前MLLMs尽管具备推理能力，但缺乏实现广义视觉智能所需的基本生理感知原语，揭示了模型在基础视觉能力方面的根本性缺陷。
...
TL;DR: M3SR：一种用于光谱重建任务的多尺度、多感知Mamba架构，通过多感知融合块增强特征理解能力，在U-Net结构中有效提取全局、中间和局部特征，实现多尺度精确重建。
...
Method: 提出M3SR架构，设计多感知融合块增强模型对输入特征的全面理解和分析能力。将多感知融合块集成到U-Net结构中，有效提取和融合全局、中间和局部特征，实现多尺度精确重建。

Result: 大量定量和定性实验表明，M3SR在计算成本更低的情况下，性能优于现有最先进方法。

Conclusion: M3SR通过多尺度、多感知的Mamba架构成功解决了光谱重建中的特征理解不足和尺度单一问题，实现了更准确的高光谱图像重建。
...
TL;DR: IQARAG是一个无需训练的框架，通过检索增强生成技术提升大型多模态模型的图像质量评估能力，避免了计算成本高昂的微调过程。
...
Result: 在KADID、KonIQ、LIVE Challenge和SPAQ等多个IQA数据集上的广泛实验表明，IQARAG有效提升了大型多模态模型的图像质量评估性能。
...
<details>
  <summary>Details</summary>
Motivation: 无人机在商业和国防应用中广泛使用，但也越来越多地被用于针对性攻击，构成重大安全挑战。现有基于视觉的无人机检测系统存在精度限制，难以区分无人机和小型鸟类。
...
Conclusion: YOLOBirDrone架构有效提高了无人机与鸟类的检测和分类精度，为解决无人机检测中的误判问题提供了有效解决方案。
...
Method: 提出UM-Text统一多模态模型，包含视觉语言模型(VLM)处理指令和参考图像，精心设计文本内容和布局；提出UM-Encoder结合各种条件信息的嵌入，由VLM根据输入指令自动配置；提出区域一致性损失在潜在空间和RGB空间为字形生成提供更有效的监督；设计定制的三阶段训练策略；贡献UM-DATA-200K大规模视觉文本图像数据集。

Result: 在多个公共基准测试上的广泛定性和定量结果表明，该方法实现了最先进的性能。

Conclusion: UM-Text通过统一的视觉语言模型和精心设计的训练策略，有效解决了视觉文本编辑中的风格一致性问题，在自然语言指令驱动的视觉文本生成方面取得了显著进展。
...
TL;DR: 本文提出了一种名为个体化探索变换器（IET）的单图像超分辨率方法，通过引入个体化探索注意力机制，让每个token自适应地选择注意力候选，在保持计算效率的同时实现更精确的信息聚合。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的单图像超分辨率方法虽然通过建模长距离依赖取得了显著性能，但其特征密集型注意力计算带来高计算成本。大多数现有方法将图像划分为固定组并在组内限制注意力，这种组级注意力忽略了token相似性的固有不对称性，无法实现灵活且token自适应的注意力计算。

Method: 提出个体化探索变换器（IET），引入新颖的个体化探索注意力（IEA）机制，允许每个token自适应地选择其自身的内容感知且独立的注意力候选。这种token自适应和不对称设计能够在保持计算效率的同时实现更精确的信息聚合。

Result: 在标准超分辨率基准测试上的大量实验表明，IET在可比较的计算复杂度下实现了最先进的性能。

Conclusion: 提出的个体化探索变换器通过token自适应的注意力机制有效解决了现有组级注意力方法的局限性，在保持计算效率的同时显著提升了单图像超分辨率的性能。
...
TL;DR: 该研究系统分析了视觉语言模型在感知退化下的语义错配问题，发现即使分割指标下降不大，也会导致严重的下游VLM行为故障，包括幻觉、安全关键实体遗漏和不一致的安全判断。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在多模态基准测试中表现良好，但其对现实感知退化的鲁棒性尚不清楚。在自动驾驶和具身AI等安全关键应用中，可靠的感知对语义推理和决策至关重要，因此需要研究VLM在感知退化下的语义可靠性。

Method: 使用Cityscapes数据集上的语义分割作为代表性感知模块，引入感知现实的退化，这些退化仅导致传统分割指标适度下降。然后分析下游VLM行为，并提出一套语言级错配指标来量化幻觉、关键遗漏和安全误解，分析这些指标与分割质量的关系。

Result: 研究发现，即使像素级分割指标下降不大，也会导致严重的VLM行为故障，包括幻觉对象提及、安全关键实体遗漏和不一致的安全判断。多个对比性和生成性VLM都表现出像素级鲁棒性与多模态语义可靠性之间的明显脱节。

Conclusion: 当前基于VLM的系统存在关键限制，像素级鲁棒性不能保证多模态语义可靠性。这凸显了在安全关键应用中需要开发明确考虑感知不确定性的评估框架的重要性。
...
TL;DR: Geo-NVS-w是一个几何感知的新视角合成框架，通过SDF表示和几何保持损失，在非结构化野外图像集合中实现高质量渲染，同时大幅降低能耗。
...
Method: 使用基于有符号距离函数(SDF)的几何表示来指导渲染过程，并引入新颖的几何保持损失来确保精细结构细节的保留。
...
<details>
  <summary>Details</summary>
Motivation: 地理空间点云语义分割面临跨区域域偏移问题，现有域适应方法需要源域数据，但由于隐私、法规和数据传输限制，源域数据通常不可用，因此需要研究无源无监督域适应方法
...
Result: 该方法能有效缓解长尾分布引起的特征崩溃问题，纠正头部类别的过度主导，防止模型预测偏向多数类别，在无源域数据情况下实现有效的域适应

Conclusion: LoGo框架为地理空间点云的无源无监督域适应提供了有效解决方案，通过局部-全局双重共识机制解决了长尾分布和域偏移的挑战
...
Conclusion: 该系统显示出强大的AI辅助全景评估潜力，具有可解释的AI特征，支持临床信心
...
TL;DR: 本文提出了一种基于BLIP-2的轻量级多模态任务平台，集成YOLO-World和YOLOv8-Seg模型，用于无人机边缘设备的实时视觉理解与交互，解决了大模型计算成本与有限计算资源之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 无人机在复杂场景中需要实时视觉理解和交互能力，但大型视觉语言模型的高计算成本与无人机边缘设备的有限计算资源之间存在矛盾。需要一种轻量化的解决方案来满足无人机应用的多任务需求。

Method: 1. 将BLIP-2与YOLO模型深度集成，利用YOLO的精确感知结果进行目标检测和实例分割；2. 设计基于K-Means聚类的内容感知关键帧采样机制，实现智能帧选择和时序特征拼接；3. 实施统一的多任务适应提示优化方案，将YOLO模型的结构化事件日志作为上下文信息注入BLIP-2输入。

Result: 该平台扩展了BLIP-2在无人机应用中的多任务能力，无需对无人机数据进行任务特定微调。通过YOLO的精确感知增强视觉注意力理解和推理，关键帧采样机制使轻量级BLIP-2能够有效处理视频级交互任务，提示优化方案指导模型生成准确且上下文相关的输出。

Conclusion: 提出的轻量级多模态任务平台成功解决了无人机边缘设备上大型视觉语言模型计算资源受限的问题，通过模型集成、智能帧采样和提示优化，实现了高效的实时视觉理解与交互能力，为无人机应用提供了实用的解决方案。
...
TL;DR: SPARK是一个自标定的实时多相机点云重建框架，能同时处理点云融合和相机外参不确定性，适用于动态场景和大规模相机系统。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多视角融合、相机外参不确定性和大规模相机系统扩展性方面存在困难，需要一种能够实时处理动态场景且具有良好扩展性的多相机3D重建方法。

Method: SPARK包含两个核心模块：(1) 几何感知的在线外参估计模块，利用多视角先验并强制跨视角和时间一致性；(2) 置信度驱动的点云融合策略，在像素和点级别建模深度可靠性和可见性。

Result: 在真实世界多相机系统上的实验表明，SPARK在外参精度、几何一致性、时间稳定性和实时性能方面优于现有方法，并具有线性扩展性。
...
TL;DR: 提出MMLGNet多模态框架，利用CLIP等视觉语言模型将高光谱成像和LiDAR等异构遥感模态与自然语言语义对齐，通过双向对比学习在共享潜在空间中对齐视觉特征和手工文本嵌入。
...
Method: 采用模态特定编码器，通过双向对比学习在共享潜在空间中对齐视觉特征与手工文本嵌入。受CLIP训练范式启发，使用简单的CNN编码器实现异构遥感模态与语言语义的对齐。
...
TL;DR: 提出了一种基于局部块的面部动画重定向方法，可将源视频中的面部动画转移到风格化3D角色上，保持表情语义


<details>
  <summary>Details</summary>
Motivation: 在数字动画时代，为虚拟角色生成逼真面部动画的需求推动了各种重定向方法的发展。然而，当重定向应用于偏离人类面部结构的风格化或夸张3D角色时，会出现挑战。需要同时考虑目标角色的面部结构和可能的运动范围，以保持原始面部运动所假定的语义。

Method: 提出局部块重定向方法，包含三个模块：1) 自动块提取模块从源视频帧提取局部块；2) 重演模块生成相应的重演目标局部块；3) 权重估计模块计算每帧目标角色的动画参数，创建完整面部动画序列。

Result: 大量实验表明，该方法能成功将源面部表情的语义含义转移到面部特征比例变化相当大的风格化角色上。

Conclusion: 该方法有效解决了风格化3D角色面部动画重定向的挑战，能够保持表情语义，适用于面部特征比例差异较大的情况。
...
Method: 提出心脏推理模板(CRT)为复杂心脏疾病提供逐步规范化诊断流程，简化推理路径构建；开发CardiacMind强化学习方案，包含三个新颖奖励：程序数量奖励(PQtR)促进详细推理，程序质量奖励(PQlR)促进跨视图和模态的证据整合，超声心动图语义奖励(ESR)将逐步描述与视觉内容关联。

Result: 方法在15种复杂心脏疾病的多视图超声心动图诊断中提升48%，在CardiacNet-PAH上提升5%；用户研究显示93.33%的临床医生同意其推理逻辑符合心脏病专家思维方式。
...
TL;DR: 本文提出NAR方法，一种噪声自适应正则化方法，用于处理遥感多标签分类中的标签噪声问题，通过区分加性噪声和减性噪声，采用置信度标签处理机制和早期学习正则化来提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 遥感多标签分类中，由于使用主题产品或众包标注来降低成本，常常引入部分错误的标签噪声。现有方法通常将噪声标注作为监督信号，缺乏针对不同噪声类型（加性噪声、减性噪声、混合噪声）的自适应学习机制。

Method: 提出NAR方法：1）在半监督学习框架中明确区分加性和减性噪声；2）采用置信度标签处理机制，动态保留高置信度标签，暂时停用中等置信度标签，通过翻转纠正低置信度标签；3）结合早期学习正则化来稳定训练并减轻对噪声标签的过拟合。

Result: 在加性、减性和混合噪声场景下的实验表明，NAR相比现有方法持续提高了鲁棒性。在减性和混合噪声场景下性能提升最为显著，表明自适应抑制和选择性纠正噪声监督是有效的策略。

Conclusion: NAR方法通过自适应区分噪声类型并采用置信度标签处理机制，结合早期学习正则化，为遥感多标签分类中的噪声鲁棒学习提供了有效策略，特别是在减性和混合噪声场景下表现优异。
...
TL;DR: 该论文提出了一种名为静态-动态协作（SDC）的框架来解决少样本类增量学习（FSCIL）中的稳定性-可塑性困境，通过将学习过程分为静态保留阶段和动态学习阶段来平衡旧知识的保留和新知识的获取。
...
Method: 提出静态-动态协作（SDC）框架，将FSCIL分为两个阶段：1）静态保留阶段（SRS）：在基础会话中使用充足数据训练初始模型，并保留关键部分作为静态记忆以保持基础旧知识；2）动态学习阶段（DLS）：引入额外的动态投影器与之前的静态记忆联合训练，利用增量动态类别信息。

Result: 在三个公共基准数据集和一个真实世界应用数据集上的广泛实验表明，该方法相比其他竞争方法取得了最先进的性能，能够更好地保留旧知识同时持续适应新类别。

Conclusion: 通过将FSCIL分为静态保留和动态学习两个阶段，SDC框架能够有效平衡稳定性与可塑性，在少样本类增量学习中实现了更好的旧知识保留和新知识适应能力。
...
TL;DR: 该研究开发了一个结合稳健特征选择的放射组学框架，用于预测高级别浆液性卵巢癌患者对新辅助化疗的治疗反应，通过整合不同特征选择方法提高了预测性能。
...
Method: 采用自动随机化算法模拟观察者间变异性，平衡特征稳健性和预测准确性。使用四种反应评估指标（CRS、RECIST、体积减少、直径减少），研究不同解剖部位病灶。基于治疗前后CT扫描提取特征并训练模型，使用独立队列进行外部验证。

Result: 体积减少预测在所有病灶组合时表现最佳（AUC 0.83），网膜病灶对CRS预测最佳（AUC 0.77），盆腔病灶对直径减少预测最佳（AUC 0.76）。
...
Method: 提出模态解耦RGB-T检测框架MDQF，采用DETR-like检测器作为RGB和TIR图像的分支，在每层细化阶段通过查询选择与适配进行跨分支查询融合

Result: 实验表明该方法优于现有RGB-T检测器，实现了更好的模态独立性，且可使用非配对RGB或TIR图像单独优化各分支

Conclusion: MDQF框架有效平衡了模态互补与分离，在极端条件下通过排除退化模态并利用高质量查询修正预测，提升了RGB-T检测的鲁棒性
...
Result: 实验揭示了任务的固有复杂性，同时证明了视觉语言模型生成上下文敏感体量方案的潜力

Conclusion: 数据集和分析建立了基础基准，突出了数据驱动建筑设计未来研究的重大机遇
...
Method: 提出了HazardForge管道，利用图像编辑模型通过布局决策算法和验证模块生成包含移动、侵入和远距离物体的危险场景。使用该管道构建了MovSafeBench基准测试，包含7,254张图像和对应的问答对，涵盖13个物体类别。

Result: 实验表明，视觉语言模型在包含异常物体的条件下性能显著下降，在需要细微运动理解的场景中下降幅度最大。

Conclusion: HazardForge能够有效生成具有时空动态的危险场景，MovSafeBench基准测试揭示了视觉语言模型在异常场景理解方面的局限性，特别是在运动理解方面存在明显缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 传统鱼缸水质管理方法效率低下、劳动密集且容易出错，导致水生环境不理想。需要一种能够持续监测多种水质参数并实现自动化控制的解决方案。

Method: 开发了基于物联网的智能鱼缸系统，集成ESP32微控制器、pH、TDS、温度和浊度传感器，以及伺服喂食器和水泵执行器。系统具备边缘处理能力，通过Blynk物联网平台实现云连接，并配备智能警报机制和可配置冷却期。

Result: 在10升鱼缸环境中测试显示，系统传感器平均准确率达96%，异常检测响应时间为1.2秒。自动喂食和水循环模块在长期测试中保持97%的运行可靠性，显著减少了人工干预。
...
TL;DR: EfficientFSL是一个专门为Vision Transformers设计的查询式微调框架，用于少样本分类，通过极少的可训练参数实现高性能，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 大型模型如Vision Transformers在少样本分类中表现出色，但微调需要大量GPU内存和训练时间，不适合资源受限的实际场景。需要一种既能保持高性能又能降低计算开销的解决方案。

Method: 提出EfficientFSL框架：1) 轻量级可训练前向块生成任务特定查询，从预训练模型中间表示中提取特征；2) 组合块融合多层输出增强特征表示；3) 支持-查询注意力块通过调整原型来对齐查询集分布，缓解分布偏移。

Result: 在四个域内少样本数据集和六个跨域数据集上达到最先进性能，仅需极少可训练参数，显著降低计算开销。

Conclusion: EfficientFSL为Vision Transformers提供了一种高效、实用的少样本分类解决方案，在保持高性能的同时大幅减少计算需求，适用于实际低资源场景。
...
### [59] [Closed-Loop LLM Discovery of Non-Standard Channel Priors in Vision Models](https://arxiv.org/abs/2601.08517)
*Tolgay Atinc Uzun,Dmitry Ignatov,Radu Timofte*

Main category: cs.CV

TL;DR: 本文提出使用大型语言模型进行神经架构搜索，特别针对通道配置优化问题，通过AST突变生成大量结构数据训练LLM，在CIFAR-100上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 通道配置优化是一个复杂的组合优化问题，传统启发式方法难以处理张量形状兼容性和计算预算约束。作者认为LLMs能够以传统方法无法实现的方式推理架构代码结构，为NAS提供变革性方法。

Method: 将搜索任务构建为条件代码生成序列，LLM基于性能指标迭代优化架构规范。通过AST突变生成大量形状一致的有效架构作为训练数据，解决数据稀缺问题，让LLM学习通道配置与性能之间的潜在关系。

Result: 在CIFAR-100上的实验验证了该方法的有效性，模型在准确率上取得了统计显著的提升。分析表明LLM成功获取了领域特定的架构先验知识，与随机搜索有明显区别。
...
TL;DR: 本文提出了一种名为约束数据集蒸馏（CD²）的新框架，用于解决少样本类增量学习中的灾难性遗忘问题。该方法通过数据集蒸馏模块和蒸馏约束模块，在有限的增量样本中学习紧凑的类相关线索，同时保留先前学到的知识。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）面临灾难性遗忘的关键问题。现有方法通常使用外部存储器存储先前知识，并以相同方式处理增量类，无法充分保留先前的重要知识。受知识蒸馏工作的启发，需要一种新方法来更好地保留先前学到的知识。

Method: 提出CD²框架，包含两个核心模块：1）数据集蒸馏模块（DDM）：在分类器指导下合成高度压缩的样本，迫使模型从少量增量样本中学习紧凑的类相关线索；2）蒸馏约束模块（DCM）：引入设计的损失函数来约束先前学到的类分布，更充分地保留蒸馏知识。

Result: 在三个公共数据集上的大量实验表明，该方法优于其他最先进的竞争对手，证明了其在少样本类增量学习任务中的优越性。

Conclusion: CD²框架通过数据集蒸馏和约束机制有效解决了FSCIL中的灾难性遗忘问题，能够从有限增量样本中学习紧凑的类相关线索，同时充分保留先前学到的知识，在多个数据集上表现出优越性能。
...
TL;DR: VideoHEDGE是一个检测视频语言模型幻觉的模块化框架，通过生成多个答案变体并进行语义聚类，计算可靠性分数来识别幻觉。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型经常产生高置信度的幻觉，而现有的不确定性度量方法往往无法与正确性对齐，需要更有效的幻觉检测方法。

Method: VideoHEDGE框架：1) 从原始视频和经过光度和时空扰动的变体中生成基线答案和多个高温生成结果；2) 使用自然语言推理或嵌入方法将文本输出聚类为语义假设；3) 基于聚类概率质量计算三种可靠性分数：语义熵、RadFlag和视觉增强语义熵。

Result: 在SoccerChat基准测试中，VASE在三个7B视频语言模型上始终获得最高的ROC-AUC，特别是在较大扰动预算下；嵌入聚类与NLI聚类性能相当但计算成本更低；领域微调减少幻觉频率但校准改进有限。

Conclusion: VideoHEDGE提供了一种有效的视频语言模型幻觉检测框架，VASE是最可靠的指标，嵌入聚类是计算高效的替代方案，领域微调对校准改进有限。
...
TL;DR: MoCha是一个创新的视频角色替换框架，仅需单帧掩码即可实现可控角色替换，无需逐帧分割或显式结构指导，通过条件感知RoPE和RL后训练提升效果，并构建了三个专门数据集解决训练数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频角色替换方法主要依赖重建范式，需要逐帧分割掩码和显式结构指导（如骨架、深度），这在复杂场景（遮挡、角色-物体交互、异常姿态、挑战性光照）中泛化能力有限，容易产生视觉伪影和时间不一致问题。

Method: 提出MoCha框架：1）仅需单帧任意掩码，无需逐帧分割；2）引入条件感知RoPE适应多模态输入条件并增强面部身份；3）采用基于强化学习的后训练阶段；4）构建三个专门数据集：UE5渲染的高保真数据集、当前肖像动画技术合成的表情驱动数据集、现有视频-掩码对增强的数据集。

Result: 大量实验表明，该方法在性能上显著优于现有最先进方法，在复杂场景中表现出更好的泛化能力和时间一致性。
...
TL;DR: 提出两种互补策略（放射组学指纹和健康人格）来提升膝关节MRI自动评估的准确性和可解释性，在保持可解释性的同时达到或超越深度学习模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统放射组学特征在群体层面预定义，虽然可解释性强但过于限制，难以捕捉患者特异性变异；端到端深度学习性能好但缺乏可解释性。需要既准确又可解释的解决方案。

Method: 1. 放射组学指纹：动态构建患者特异性特征集，通过图像条件预测器估计特征使用概率，结合透明逻辑回归进行分类；2. 健康人格：使用扩散模型为每个患者合成无病理的基线MRI，通过对比病理图像与健康人格的特征差异来定位疾病。

Result: 在三个临床任务上的实验结果显示，两种方法单独或组合使用时，性能达到或超越最先进的深度学习模型，同时支持多层次的可解释性。案例研究进一步展示了这些方法如何促进人类可解释的生物标志物发现和病理定位。

Conclusion: 放射组学指纹和健康人格是两种互补的策略，能够在保持临床可解释性的同时实现高性能的膝关节MRI自动评估，为临床采用提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 源自由域自适应（SFDA）面临感知场与计算效率的权衡问题，现有方法难以在域不变特征学习中平衡这两者。虽然Mamba的选择性扫描机制提供了长距离依赖建模的线性复杂度解决方案，但Visual Mamba在捕获通道级频率特征和保持空间鲁棒性方面仍有局限。
...
TL;DR: 本文针对视觉语言模型在测试时提示调优中的校准问题，提出了一种语义正交校准方法，在保持语义相关性的同时改善不确定性估计的校准性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在医疗、自动驾驶等关键决策系统中的广泛应用，其不确定性估计的校准变得至关重要。然而，现有的VLM测试时提示调优研究主要关注提升判别性能，而校准维度被严重忽视。现有方法强制文本提示嵌入完全正交，但作者发现这会过度分离语义相关的类别，导致模型过度自信。

Method: 提出语义正交校准方法，使用基于Huber的规范化器，在强制原型平滑分离的同时保持语义邻近性。该方法避免了完全正交约束的缺陷，在改善校准的同时维持语义相关性。

Result: 通过全面的实证验证，SoC方法在多个数据集上一致地改善了校准性能，同时保持了有竞争力的判别能力。相比先前的正交性方法，SoC在不确定性估计校准方面表现更优。

Conclusion: 语义正交校准方法有效地解决了视觉语言模型测试时提示调优中的校准问题，在保持语义相关性的同时显著改善了不确定性估计的校准性能，为关键决策系统中的VLM应用提供了更可靠的不确定性估计。
...
Method: 提出CtrlFuse框架，包含多模态特征提取器、参考提示编码器(RPE)和提示语义融合模块(PSFM)。RPE通过输入掩码指导微调预训练分割模型来动态编码任务特定语义提示，PSFM将这些语义显式注入融合特征。通过并行分割和融合分支的协同优化实现任务性能与融合质量的相互增强。

Result: 实验表明在融合可控性和分割准确性方面都达到了最先进的结果，适应后的任务分支甚至超越了原始分割模型的性能。

Conclusion: CtrlFuse通过交互式掩码提示实现了可控的图像融合，在提升融合质量的同时增强了下游语义感知任务的性能，为智能无人系统的环境感知提供了有效解决方案。
...
TL;DR: 本文提出一个基于四大支柱的深度伪造检测可靠性评估框架，分析了五种最先进方法，揭示了显著进展和关键局限
...
Method: 提出基于四大支柱的可靠性评估框架：可迁移性、鲁棒性、可解释性和计算效率。对五种最先进的深度伪造检测方法进行了分析

Result: 分析揭示了深度伪造检测方法的显著进展，但也暴露了关键局限性。评估框架能够全面评估检测系统的可靠性

Conclusion: 需要超越简单分类性能的综合评估框架来评估深度伪造检测系统的可靠性。提出的四大支柱框架为系统评估提供了基础，揭示了当前方法的进展和不足
...
TL;DR: Salience-SGG提出了一种新的场景图生成框架，通过迭代显著性解码器强调具有显著空间结构的三元组，解决长尾分布问题并改善空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 场景图生成面临长尾分布问题，少数谓词类别主导而许多其他类别代表性不足，导致模型对罕见关系表现不佳。现有的无偏SGG方法虽然解决了偏差问题，但往往以牺牲空间理解为代价，过度依赖语义先验。

Method: 提出了Salience-SGG框架，包含迭代显著性解码器（ISD），强调具有显著空间结构的三元组。同时提出了语义无关的显著性标签来指导ISD的学习过程。

Result: 在Visual Genome、Open Images V6和GQA-200数据集上的评估显示，Salience-SGG取得了最先进的性能，并通过Pairwise Localization Average Precision指标证明能够改善现有无偏SGG方法的空间理解能力。

Conclusion: Salience-SGG通过强调空间显著性，有效解决了场景图生成中的长尾分布问题，同时保持了良好的空间理解能力，优于现有的无偏SGG方法。
...
<details>
  <summary>Details</summary>
Motivation: 急性缺血性卒中病灶在MRI中的准确分割是卒中诊断和治疗的关键。虽然深度学习已成功应用于自动分割，但现有方法大多基于U-Net框架，主要差异在于损失函数选择、深度监督、残差连接和注意力机制的使用。许多实现不公开，且最佳配置仍不明确。

Method: 提出ISLA模型，系统优化损失函数、卷积架构、深度监督和注意力机制，在三个多中心数据库（超过1500名AIS参与者）上训练。进一步研究无监督域适应以提高对外部临床数据集的泛化能力。

Result: ISLA在外部测试集上优于两种最先进的AIS病灶分割方法。代码和训练模型将公开以促进重用和可重复性。

Conclusion: ISLA是一个稳健的分割框架，通过系统优化实现了对急性缺血性卒中病灶的准确分割，并在外部验证中表现出优越性能，同时提供公开可用的代码和模型。
...
TL;DR: 使用CycleGAN实现多通道荧光显微镜图像到伪H&E染色组织病理学图像的无配对图像转换
...
Method: 采用Cycle-Consistent Adversarial Network (CycleGAN)方法，将C01和C02荧光通道合并为RGB，学习荧光和H&E域之间的双向映射，无需配对训练数据。架构使用基于ResNet的生成器（带残差块）和PatchGAN判别器，通过对抗损失、循环一致性和恒等损失进行训练。
...
<details>
  <summary>Details</summary>
Motivation: 从X光片诊断牙科疾病耗时且具有挑战性，因为诊断证据往往很细微。现有的基于自然图像设计的物体检测模型难以检测视觉支持较少的牙科疾病。

Method: 提出DentalX方法，包含结构上下文提取模块，该模块学习辅助任务：牙科解剖结构的语义分割。模块提取有意义的结构上下文并将其整合到主要的疾病检测任务中，以增强对细微牙科疾病的检测。

Result: 在专用基准测试上的大量实验表明，DentalX在两个任务上都显著优于先前的方法。这种相互受益在模型优化过程中自然产生，因为两个任务之间的相关性被有效捕捉。

Conclusion: DentalX通过利用口腔结构信息来缓解X光片中的视觉模糊性，为牙科疾病检测提供了一种有效的上下文感知方法，代码已开源。
...
TL;DR: 本研究评估了计算机视觉方法在濒危两栖动物胡拉彩蛙个体识别中的应用，比较了局部特征匹配和全局特征嵌入模型，开发了结合两者优势的两阶段工作流，并部署为支持保护监测的Web应用。


<details>
  <summary>Details</summary>
Motivation: 对于濒危两栖动物的监测，准确的非侵入性个体识别至关重要。传统标记方法对极度濒危物种不适用，需要开发基于计算机视觉的摄影重识别技术。
...
Conclusion: 对于该物种，零样本深度局部特征匹配优于全局特征嵌入，可作为照片识别的默认方法。开发的两阶段工作流结合了可扩展性和准确性，已部署为支持保护监测的Web应用。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D视觉定位任务面临两大挑战：1）现有模型推理能力有限，需要大量标注数据进行监督训练；2）现有合成数据方法成本高但性能提升有限。因此需要一种能自动生成高质量3D视觉定位数据并提升模型推理能力的方法。

Method: 提出一个自动化的3D视觉定位数据合成管道，能够生成包含推理过程的3D视觉定位数据。利用这些生成的数据对大型语言模型进行微调，开发出Reason3DVG-8B模型。

Result: Reason3DVG-8B在仅使用3D-GRAND方法1.6%训练数据的情况下，性能超越了之前的LLM-based方法3D-GRAND，证明了所提出数据的有效性和推理在3D视觉定位中的重要性。

Conclusion: 通过自动合成包含推理过程的3D视觉定位数据，能够显著提升3D视觉定位模型的性能，同时大幅降低数据收集成本，为3D视觉理解任务提供了新的有效解决方案。
...
TL;DR: Motive是一个基于梯度的运动归因框架，用于视频生成模型，能够识别影响运动质量的训练数据片段，并通过数据筛选提升视频生成的时间一致性和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型发展迅速，但数据如何影响运动质量的研究仍然不足。现有方法主要关注视觉外观，缺乏专门针对运动动态的数据归因框架。

Method: 提出Motive框架，通过运动加权的损失掩码将时间动态与静态外观分离，实现高效可扩展的运动特定影响计算。该框架可扩展到现代大规模高质量视频数据集和模型。

Result: 在文本到视频模型中，Motive能识别强烈影响运动的训练片段，指导数据筛选。使用Motive选择的高影响力数据，在VBench上提高了运动平滑度和动态程度，相比预训练基础模型获得74.1%的人类偏好胜率。
...
TL;DR: 3AM是一个训练时增强方法，将MUSt3R的3D感知特征集成到SAM2中，通过轻量级特征融合器结合多级几何特征，实现仅需RGB输入即可在宽基线运动场景下保持几何一致性的视频对象分割。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象分割方法（如SAM2）依赖外观特征，在大视角变化下表现不佳；传统3D实例分割方法需要相机位姿、深度图等预处理，成本高昂。需要一种仅需RGB输入就能在宽基线运动中保持几何一致性的解决方案。

Method: 提出3AM训练时增强方法：1）集成MUSt3R的3D感知特征到SAM2；2）轻量级特征融合器融合多级MUSt3R特征（编码隐式几何对应关系）；3）结合SAM2的外观特征，实现基于空间位置和视觉相似性的几何一致性识别；4）提出视场感知采样策略，确保帧观察空间一致的对象区域以学习可靠的3D对应关系。

Result: 在宽基线运动挑战数据集（ScanNet++、Replica）上，3AM显著优于SAM2及其扩展方法，在ScanNet++精选子集上达到90.6% IoU和71.7% Positive IoU，比最先进的VOS方法分别提升15.9和30.4个百分点。

Conclusion: 3AM通过融合3D感知特征和外观特征，实现了仅需RGB输入就能在宽基线运动场景下保持几何一致性的视频对象分割，无需相机位姿或预处理，在挑战性数据集上显著优于现有方法。
...
TL;DR: 该论文提出了一种基于零样本扩散框架的水印去除方法，将水印去除重新定义为视角合成问题，通过语义保持的视角变换有效去除多种水印方案。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像的水印认证机制广泛部署，但评估这些水印方案对抗复杂去除攻击的脆弱性至关重要。现有水印虽然对像素空间和频域攻击具有鲁棒性，但在语义保持的视角变换下可能存在漏洞。
...
Conclusion: 该研究揭示了不可见水印的基本脆弱性：即使对像素空间和频域攻击具有鲁棒性的水印，在语义保持的视角变换下仍然脆弱。这为水印系统的鲁棒性评估和设计提供了重要指导。
...
### [77] [EmbeddingRWKV: State-Centric Retrieval with Reusable States](https://arxiv.org/abs/2601.07861)
*Haowen Hou,Jie Yang*

Main category: cs.CL

TL;DR: 提出State-Centric Retrieval统一检索范式，通过"状态"连接嵌入模型和重排器，显著提升RAG系统效率


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统的两阶段流水线（嵌入模型+重排器）存在效率低下问题，因为阶段间缺乏信息共享，导致大量冗余计算

Method: 1) 通过微调RWKV-based LLM进行状态表示学习，创建EmbeddingRWKV统一模型；2) 设计基于状态的重排器，利用预计算信息；3) 采用均匀层选择策略减少计算量

Result: 重排阶段获得5.4-44.8倍加速；仅使用25%层数即可保持98.62%性能；在高质量检索和重排结果的同时显著提升系统效率
...
### [78] [A Human-Centric Pipeline for Aligning Large Language Models with Chinese Medical Ethics](https://arxiv.org/abs/2601.07954)
*Haoan Jin,Han Ying,Jiacheng Ji,Hanhui Xu,Mengyue Wu*

Main category: cs.CL

TL;DR: 提出了MedES基准和监护人循环框架，用于在中文医疗伦理背景下对齐大语言模型，通过监督微调和领域特定偏好优化显著提升了模型在伦理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医疗任务中应用广泛，但在复杂现实场景下与医疗伦理的细致要求对齐仍然研究不足，特别是在中文医疗伦理背景下缺乏专门的评估框架。

Method: 1) 构建MedES基准：从260个权威中文医疗、伦理和法律来源创建动态、场景中心的基准；2) 监护人循环框架：使用专门的自动化评估器（在领域内准确率超过97%）生成针对性提示并提供结构化伦理反馈；3) 通过监督微调和领域特定偏好优化对齐7B参数的大语言模型。

Result: 在中文医疗伦理背景下进行的实验表明，对齐后的模型在核心伦理任务上显著优于更大的基线模型，在质量和综合评估指标上都有明显提升。

Conclusion: 该工作为中文医疗领域的大语言模型伦理对齐提供了实用且可适应的框架，并表明通过模块化替换底层规范语料库，类似的伦理对齐流程可以在其他法律和文化环境中实例化。
...
### [79] [Knowing But Not Doing: Convergent Morality and Divergent Action in LLMs](https://arxiv.org/abs/2601.07972)
*Jen-tse Huang,Jiantong Qin,Xueli Qiu,Sharon Levy,Michelle R. Kaufman,Mark Dredze*

Main category: cs.CL

TL;DR: 研究通过ValAct-15k数据集评估LLMs在现实决策场景中的价值对齐表现，发现模型间高度一致但与人类差异显著，且存在知识-行动差距


<details>
  <summary>Details</summary>
Motivation: 尽管价值对齐对AI安全和社会兼容性至关重要，但大型语言模型如何在现实决策场景中表征和执行人类价值观仍缺乏深入探索

Method: 创建ValAct-15k数据集（包含3,000个来自Reddit的求助场景），基于Schwartz基本人类价值观理论评估10个前沿LLMs（中美各5个）和55名人类参与者，使用场景问题和传统价值观问卷

Result: 模型间在场景决策中表现出近乎完美的一致性（Pearson r≈1.0），而人类差异显著（r∈[-0.79,0.98]）；人类和LLMs都显示出自我报告价值观与实际行为之间的弱相关性（r=0.4,0.3）；当被要求"持有"特定价值观时，LLMs表现下降达6.6%

Conclusion: 对齐训练虽然产生了规范性的价值观趋同，但未能消除人类特有的价值观认知与行动之间的不一致性，LLMs表现出角色扮演厌恶
...
### [80] [Explaining Generalization of AI-Generated Text Detectors Through Linguistic Analysis](https://arxiv.org/abs/2601.07974)
*Yuxi Xia,Kinga Stańczak,Benjamin Roth*

Main category: cs.CL

TL;DR: 本文系统研究了AI文本检测器在不同生成条件下的泛化问题，通过语言特征分析揭示了泛化性能与特定语言特征（如时态使用和代词频率）之间的关联。


<details>
  <summary>Details</summary>
Motivation: AI文本检测器在领域内基准测试中表现良好，但在面对不同生成条件（如未见过的提示、模型家族或领域）时泛化能力不足。先前研究虽然报告了这些泛化差距，但对根本原因的理解有限。

Method: 构建了一个全面的基准测试，涵盖6种提示策略、7个大语言模型和4个领域数据集，生成了多样化的人类和AI生成文本。在此基础上，在各种生成设置上微调基于分类的检测器，并评估其跨提示、跨模型和跨数据集的泛化能力。通过计算80种语言特征在训练和测试条件之间的特征偏移与泛化准确率的相关性来解释性能差异。

Result: 分析显示，特定检测器和评估条件的泛化性能与时态使用、代词频率等语言特征显著相关。这为理解AI文本检测器的泛化行为提供了新的语言学视角。

Conclusion: 通过系统性的语言特征分析，本研究揭示了AI文本检测器泛化性能与特定语言模式之间的关联，为改进检测器的跨条件泛化能力提供了理论基础和实证依据。
...
TL;DR: 本文提出了一个构建法语和德语多模态事实核查数据集的数据收集与处理流程，通过聚合ClaimReview信息、抓取完整辟谣文章、规范化裁决结果，并利用大语言模型进行证据提取和理由生成，为多语言多模态虚假信息验证研究奠定基础。


<details>
  <summary>Details</summary>
Motivation: 在线平台虚假信息快速传播，迫切需要强大、及时、可解释且多语言的事实核查资源。现有数据集范围有限，通常缺乏多模态证据、结构化标注以及声明、证据和裁决之间的详细关联。

Method: 开发了综合数据收集与处理流程：聚合ClaimReview信息流，抓取完整辟谣文章，规范化异构的声明裁决，并用结构化元数据和对齐的视觉内容进行丰富。使用最先进的大语言模型和多模态大语言模型进行(i)预定义证据类别下的证据提取和(ii)连接证据与裁决的理由生成。

Result: 通过G-Eval和人工评估表明，该流程能够实现不同组织或媒体市场间事实核查实践的细粒度比较，促进开发更可解释和基于证据的事实核查模型，为未来多语言多模态虚假信息验证研究奠定基础。

Conclusion: 该研究提出的多语言多模态事实核查数据集构建流程有效解决了现有数据集的局限性，为开发更强大、可解释的事实核查系统提供了重要资源，推动了多语言多模态虚假信息验证领域的研究进展。
...
TL;DR: VULCA-Bench是一个多文化艺术批评基准，用于评估视觉语言模型在超越表面视觉感知的文化理解能力，包含7,410个图像-评论对，涵盖8种文化传统，采用五层文化理解框架。


<details>
  <summary>Details</summary>
Motivation: 现有VLM基准主要评估L1-L2能力（物体识别、场景描述、事实问答），而缺乏对高阶文化解释能力的评估。需要建立能够系统评估模型文化理解深度的基准。

Method: 创建包含7,410个匹配图像-评论对的多文化数据集，涵盖8种文化传统，支持中英双语。采用五层文化理解框架（L1视觉感知到L5哲学美学），具体化为225个文化特定维度，并由专家撰写双语评论。

Result: 初步结果表明，高层推理（L3-L5）比视觉和技术分析（L1-L2）更具挑战性。数据集、评估脚本和标注工具已在补充材料中以CC BY 4.0许可提供。

Conclusion: VULCA-Bench填补了VLM文化理解评估的空白，为系统评估模型在多元文化背景下的深层理解能力提供了标准化工具，有助于推动更具文化敏感性的AI发展。
...
TL;DR: 该论文提出纵向建模与评估范式，针对行为序列数据更新NLP流程的四个部分，解决传统文档级评估在纵向研究中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统NLP将文档视为独立无序样本，但在纵向研究中，文档嵌套在作者内部并按时间排序形成行为序列，需要专门的建模方法。

Method: 提出纵向建模评估范式：1) 按人群(横断面)和时间(前瞻性)划分评估集；2) 区分个体间差异与个体内动态的准确度指标；3) 默认包含历史信息的序列输入；4) 支持不同粒度潜在状态表示的模型内部结构。

Result: 在包含17k篇日记转录和238名参与者PTSD症状严重程度的数据集上，传统文档级评估与作者提出的生态有效建模评估得出显著不同甚至相反的结论。
...
TL;DR: DyCP是一种轻量级上下文管理方法，通过动态分割和检索相关记忆来提升LLM在长对话中的性能，减少延迟并提高回答质量。


<details>
  <summary>Details</summary>
Motivation: 随着对话长度增加，大型语言模型（LLMs）会出现响应延迟增加和回答质量下降的问题。现有方法要么依赖额外的LLM调用来构建记忆，要么进行离线记忆构建而不考虑当前用户话语，这会导致效率低下或破坏对话连续性。

Method: DyCP是一种轻量级上下文管理方法，在查询时动态分割和检索相关记忆。它保留了对话的顺序结构，无需预定义主题边界，并支持高效、自适应的上下文检索。

Result: 在三个长对话基准测试（LoCoMo、MT-Bench+和SCM4LLMs）以及多个LLMs上，DyCP持续提高了回答质量，同时减少了响应延迟。

Conclusion: 尽管现代LLMs扩展了上下文窗口，但实际的长上下文处理能力仍有限，因此有效的上下文管理仍然至关重要。DyCP通过动态上下文管理解决了这一挑战。
...
TL;DR: CVP方法在情感分析中表现良好，跨领域迁移能力强，但线性假设存在近似性，有改进空间。


<details>
  <summary>Details</summary>
Motivation: 人文学科中的情感分析需要情境化的连续评分，CVP方法虽然提供了解决方案，但其跨领域可移植性和基本假设尚未充分探索。

Method: 评估CVP方法在不同文体、历史时期、语言和情感维度上的表现，检验其跨语料库迁移能力，并深入分析其线性假设。

Result: CVP方法在跨领域迁移时性能损失很小，表现出良好的可移植性；但其线性假设只是近似的，存在进一步发展的潜力。

Conclusion: CVP是一种有效的可移植方法，能捕捉可泛化的模式，但其线性假设的近似性表明该方法仍有改进空间。
...
TL;DR: LLM Review框架通过盲审机制解决多智能体协作中的创意同质化问题，在科幻创作任务上超越基线方法，小模型结合该框架可媲美大模型


<details>
  <summary>Details</summary>
Motivation: 大语言模型在创意生成方面存在困难，而多智能体框架虽然能提升推理能力，却可能因交互导致内容同质化，反而抑制创造力

Method: 提出LLM Review框架，采用盲审机制：智能体交换针对性反馈但独立修改，保持创意轨迹的多样性；同时构建SciFi-100科幻写作数据集，结合LLM评分、人工标注和规则化新颖性指标进行综合评估

Result: 实验表明LLM Review在多智能体基线方法中表现最优，且小模型结合该框架可以超越更大的单智能体模型，表明交互结构可能替代模型规模

Conclusion: 通过盲审机制的多智能体协作框架能有效提升创意生成质量，交互结构设计对创造力提升的重要性可能超过模型规模本身
...
TL;DR: 研究发现大语言模型的推理能力可以通过干预内部表征来激活，无需显式的思维链提示。通过稀疏自编码器识别与推理相关的潜在特征，仅操控单个特征即可显著提升模型推理准确性。


<details>
  <summary>Details</summary>
Motivation: 虽然思维链提示已被证明能提升大语言模型的推理性能，但其作用机制尚不明确。本研究旨在探究思维链提示是否是大语言模型推理的唯一触发机制，以及是否存在其他方式可以激活模型的推理能力。

Method: 使用稀疏自编码器分析大语言模型的内部表征，识别与推理行为因果相关的潜在特征。通过干预这些特征来研究其对模型推理性能的影响，并在多个模型系列和推理基准上进行实验验证。

Result: 研究发现：1）操控单个推理相关的潜在特征可显著提升模型准确性，无需显式思维链提示；2）对于大型模型，潜在特征操控能达到与标准思维链提示相当的性能，同时输出更高效；3）推理导向的内部状态在生成早期就被触发，并能覆盖提示层级的指令。

Conclusion: 大语言模型的多步推理能力由可被外部激活的潜在内部激活支持，思维链提示只是激活这一机制的有效方式之一，而非必要原因。这表明存在多种方式可以触发大语言模型的推理能力。
...
TL;DR: 语言模型通过自回归输出链可以实现通用计算，即使是随机初始化的模型在训练前就具备这种能力，训练主要提升的是可编程性而非计算表达能力。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型的计算能力边界，探讨其是否能够执行通用计算，以及训练对计算表达能力的影响。

Method: 证明语言模型的自回归输出链足以执行通用计算，能够模拟任何算法在任何输入上的执行。通过实验展示随机初始化语言模型在训练前就具备通用计算能力。

Result: 语言模型确实能够实现通用计算，且这种能力在模型随机初始化时就已存在。训练主要改善的是可编程性，使自然语言接口能够更好地访问这些内在能力。

Conclusion: 语言模型的计算表达能力是固有的，训练主要提升的是通过自然语言提示访问这些能力的能力（可编程性），而非创造计算能力本身。
...
TL;DR: 本文提出了一个全面的置信度估计评估框架，针对大语言模型在语言变异性下的三个新维度：提示扰动鲁棒性、语义等价答案稳定性、语义不同答案敏感性，发现现有方法在这些方面存在严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有置信度估计方法主要关注校准和区分度，但忽视了语言变异性带来的挑战。在真实使用场景中，置信度应该在语义等价的提示或答案变化下保持稳定，而在答案含义不同时应该变化。现有评估框架无法捕捉这些重要特性。

Method: 提出了一个全面的置信度估计评估框架，包含三个新维度：1) 提示扰动鲁棒性 - 测量置信度对语义等价提示变化的稳定性；2) 语义等价答案稳定性 - 测量对相同含义不同表述答案的置信度一致性；3) 语义不同答案敏感性 - 测量对含义不同答案的置信度区分能力。

Result: 研究发现常见的大语言模型置信度估计方法在这些新指标上表现不佳：在传统校准或区分度指标上表现良好的方法，往往对提示变化不够鲁棒，或对答案变化不够敏感。这揭示了现有评估框架的局限性。

Conclusion: 该框架揭示了现有置信度估计评估在真实世界应用中的局限性，为选择和设计更可靠的置信度估计方法提供了实用指导。需要开发能够同时满足校准、区分度以及语言变异性要求的新方法。
...
TL;DR: 本文研究了面向工具调用智能体RAG系统的查询建议问题，针对用户提出的不可回答查询，提出了一种能够生成相似且可回答查询建议的方法，通过动态少样本学习从相关工作流中检索示例，提升了用户与RAG系统的交互效果。


<details>
  <summary>Details</summary>
Motivation: 当前基于工具调用的智能体RAG系统虽然强大，但其知识范围有限，超出范围的查询可能导致幻觉问题。虽然有护栏框架可以阻止超出范围的问题，但尚未有研究探讨如何为不可回答的查询提供可回答的替代建议，这对于改善用户与RAG系统的交互体验至关重要。

Method: 提出了一种鲁棒的动态少样本学习方法，从相关工作流中检索示例来生成查询建议。该方法可以自我学习（例如从先前的用户查询中学习），因此在实际应用中易于部署。核心挑战在于确保建议的查询在RAG的多步骤工作流中是可回答的。

Result: 在基于两个真实世界用户查询收集的无标签问题数据集构建的三个基准数据集上进行了评估。实验结果表明，该方法能够产生更相关且可回答的查询建议，优于少样本和仅检索的基线方法，从而实现了更安全、更有效的用户与智能体RAG的交互。

Conclusion: 本文首次研究了面向工具调用智能体RAG的查询建议问题，提出的动态少样本学习方法能够有效处理不可回答的用户查询，生成相似且可回答的替代建议，显著改善了用户与RAG系统的交互体验，为智能体RAG系统的实用化提供了重要支持。
...
TL;DR: ExoFormer通过引入外部锚点投影解决Transformer中早期层注意力投影复用的问题，在性能、数据效率和注意力下沉方面均有提升，但存在表示崩溃现象。


<details>
  <summary>Details</summary>
Motivation: Transformer中复用早期层注意力投影作为残差连接存在根本矛盾：第一层既要作为深层稳定参考，又要作为有效计算块。需要解决这一张力。

Method: 提出ExoFormer，在顺序层栈外学习专用的外部锚点投影，将锚点角色与计算细化解耦。通过统一的归一化混合框架（研究不同系数粒度：元素级、头级、标量级）应用于所有注意力路径。

Result: ExoFormer变体持续优于内部锚点对应模型；动态变体在下游任务准确率上比基线提高2.13个百分点，数据效率更高（用1.84倍更少的token达到相同验证损失）；注意力下沉比标准门控注意力减少2倍；但所有变体都表现出表示崩溃迹象。

Conclusion: 通过卸载假说解释表示崩溃：外部锚点保留必要token身份，允许各层专门进行计算细化。外部锚点设计有效解决了早期层双重角色的矛盾，但需要进一步研究表示崩溃现象。
...
TL;DR: 该研究针对大型推理模型在长格式多步输出中的置信度校准问题，构建了包含34.7万条推理轨迹的基准测试集RMCB，评估了十多种表示学习方法，发现文本编码器在区分能力上最优，而结构感知模型在校准方面最优，两者存在权衡关系。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在关键领域应用时存在校准不足的问题，这影响了其可靠性。当前缺乏准确估计这些模型长格式、多步输出置信度的方法，需要建立全面的基准来评估和改进置信度估计技术。

Method: 1. 构建RMCB基准测试集：包含来自6个流行大型推理模型的347,496条推理轨迹，涵盖临床、金融、法律、数学推理等高风险领域及复杂通用推理任务，所有样本都有正确性标注。
2. 大规模实证评估：测试超过10种不同的基于表示的方法，包括序列模型、基于图的方法和文本编码器架构。
3. 评估指标：使用AUROC衡量区分能力，使用ECE（预期校准误差）衡量校准质量。

Result: 1. 发现区分能力（AUROC）和校准质量（ECE）之间存在持续的权衡关系：文本编码器获得最佳AUROC（0.672），而结构感知模型获得最佳ECE（0.148），没有单一方法能在两方面都占优。
2. 增加架构复杂性并不能可靠地超越简单的序列基线，表明仅依赖块级隐藏状态的方法存在性能上限。
3. 建立了该任务迄今为止最全面的基准，为未来研究提供了严格的基线。

Conclusion: 该研究揭示了当前基于表示的置信度估计方法的局限性，特别是在区分能力和校准质量之间的权衡。RMCB基准为评估和改进大型推理模型的置信度估计提供了重要资源，表明需要开发新的方法来同时优化这两个关键指标。
...
TL;DR: WISE-Flow：一个工作流中心框架，通过从历史服务交互中提取可重用程序经验，使LLM智能体能够自我进化，减少错误并提高执行一致性


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体在用户服务中广泛部署，但存在新任务易出错、重复相同失败模式、运行间差异大的问题。通过环境特定训练或手动修补修复失败成本高且难以扩展。

Method: 提出WISE-Flow框架，将历史服务交互转换为可重用程序经验：1）通过前提条件增强的动作块诱导工作流；2）部署时对齐智能体执行轨迹到检索到的工作流；3）执行前提条件感知的可行性推理以实现状态基础的下一个动作

Result: 在ToolSandbox和τ²-bench上的实验显示，该方法在不同基础模型上均能带来一致的性能改进

Conclusion: WISE-Flow框架能够使智能体在用户服务环境中自我进化，通过工作流重用历史经验，有效减少错误并提高执行稳定性
...
TL;DR: SwiftMem：一种查询感知的智能体记忆系统，通过时间和语义维度索引实现亚线性检索，相比现有方法搜索速度提升47倍
...
Method: 提出SwiftMem系统，包含：1）时间索引实现对数时间范围查询；2）语义DAG-Tag索引通过分层标签结构将查询映射到相关主题；3）嵌入-标签协同整合机制，基于语义簇重组存储以提高缓存局部性。

Result: 在LoCoMo和LongMemEval基准测试中，SwiftMem相比最先进的基线方法实现了47倍的搜索加速，同时保持竞争性的准确性。

Conclusion: SwiftMem通过查询感知的亚线性检索解决了现有记忆系统的延迟瓶颈，使内存增强的LLM智能体能够实际部署。
...
TL;DR: 通过微调函数向量提升大语言模型的关系表示能力，使用约20个词对微调后，在关系型词补全任务上表现优于原始因果中介分析得到的向量，并引入复合函数向量增强类比推理能力。


<details>
  <summary>Details</summary>
Motivation: 概念间的关系表示是智能系统理解世界的核心前提。虽然已有研究通过因果中介分析发现少量注意力头编码了任务表示（函数向量），但这些向量在关系表示方面仍有改进空间，需要更有效的方法来编码和操作关系知识。

Method: 1. 使用少量示例（约20个词对）微调从因果中介分析得到的函数向量；2. 引入复合函数向量，即微调函数向量的加权组合；3. 在推理时将复合向量插入LLM激活中，增强关系知识提取和类比推理能力。

Result: 1. 微调后的函数向量在关系型词补全任务上表现优于原始向量，适用于不同规模的模型；2. 微调向量在关系词解码方面表现更好，与人类语义关系相似性判断更一致；3. 复合函数向量显著提升了在认知科学和SAT基准测试中挑战性类比问题的性能。

Conclusion: 激活修补作为一种可控机制，在编码和操作关系知识方面具有巨大潜力，既能提升大语言模型的可解释性，又能增强其推理能力，为关系表示和类比推理研究提供了新方向。
...
TL;DR: 本文研究了在政治问答中基于提示设计的LLM清晰度自动评估，使用CLARITY数据集比较了不同提示策略对GPT-5.2性能的影响，发现推理提示能显著提升清晰度评估准确率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM自动评估主要关注事实准确性，但在政治问答中清晰度评估同样重要。虽然已有数据集提供清晰度和回避性的人工标注，但提示设计对自动清晰度评估的影响尚未充分探索。

Method: 使用SemEval 2026共享任务的CLARITY数据集，比较GPT-3.5基线模型与GPT-5.2在三种提示策略下的表现：简单提示、思维链提示、以及带少量示例的思维链提示。评估指标包括准确率、类别指标和层次精确匹配。

Result: GPT-5.2在清晰度预测上始终优于GPT-3.5基线，带少量示例的思维链提示将准确率从56%提升至63%。思维链提示在回避性评估上达到最高准确率34%，但细粒度类别改进不稳定。主题识别方面，推理提示将准确率从60%提升至74%。

Conclusion: 提示设计能可靠提升高层次清晰度评估性能，但细粒度回避性评估和主题检测仍然具有挑战性，即使使用结构化推理提示也难以完全解决。
...
### [98] [Evaluating Implicit Regulatory Compliance in LLM Tool Invocation via Logic-Guided Synthesis](https://arxiv.org/abs/2601.08196)
*Da Song,Yuheng Huang,Boqi Chen,Tianshuo Cong,Randy Goebel,Lei Ma,Foutse Khomh*

Main category: cs.CL

TL;DR: LogiSafetyGen框架将非结构化法规转换为线性时序逻辑预言机，通过逻辑引导的模糊测试生成安全关键轨迹，并构建LogiSafetyBench基准测试，评估LLM在满足功能目标的同时遵守隐式法规的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试往往忽视隐式法规合规性，无法评估LLM是否能自主执行强制性安全约束，这在高风险领域尤为重要，因为系统必须严格遵守超越简单功能正确性的监管标准。

Method: 提出LogiSafetyGen框架：1) 将非结构化法规转换为线性时序逻辑(LTL)预言机；2) 采用逻辑引导的模糊测试合成有效的安全关键轨迹；3) 基于此框架构建LogiSafetyBench基准，包含240个人工验证的任务，要求LLM生成同时满足功能目标和潜在合规规则的Python程序。

Result: 对13个最先进的LLM进行评估发现：更大的模型虽然在功能正确性上表现更好，但经常优先考虑任务完成而非安全性，导致不合规行为。

Conclusion: 需要开发能够同时确保功能正确性和法规合规性的LLM系统，特别是在高风险领域，当前模型倾向于优先完成任务而忽视安全约束。
...
<details>
  <summary>Details</summary>
Motivation: SPIN方法在迭代过程中当前奖励优势会逐渐消失导致优化不稳定，同时参考策略的使用造成了训练奖励公式与生成指标之间的不一致问题。

Method: 提出了基于三元组的自对弈微调方法，包含两个关键设计：1) 引入历史优势，比较迭代生成响应与初始策略生成的原型合成响应；2) 在自对弈框架中引入熵约束，支持无参考微调。

Result: 在各种任务上，T-SPIN不仅性能优于SPIN，而且在迭代过程中表现出稳定的演化。相比监督微调，仅需25%的样本就能达到相当甚至更好的性能。

Conclusion: T-SPIN通过历史优势和熵约束有效解决了SPIN的局限性，在标注数据稀缺的情况下表现出色，为语言模型微调提供了更稳定高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在生物医学、材料和金融等高风险领域部署大语言模型时，需要注入私有、领域特定的知识，这些知识具有专有性、快速演化和公共预训练中代表性不足的特点。现有两种主要私有知识注入范式各有明显缺点：微调迭代成本高且存在灾难性遗忘风险；检索增强生成(RAG)在专业私有语料库中脆弱，存在证据碎片化、检索漂移和长上下文压力等问题。

Method: 受多模态LLM将异构模态对齐到共享语义空间的启发，提出Generation-Augmented Generation (GAG)方法，将私有专业知识视为额外的专家模态，通过紧凑的表示级接口将其对齐到冻结的基础模型，避免提示时证据序列化，同时支持即插即用专业化和可扩展的多领域组合。
...
Conclusion: GAG通过表示级接口将私有专业知识作为专家模态注入，有效解决了私有知识注入中的关键挑战，在保持基础模型通用能力的同时显著提升领域专业性能，支持可扩展的多领域部署。
...
TL;DR: MoE模型性能主要由总参数量和专家稀疏度决定，而非仅考虑总参数和激活参数。研究发现专家数量增加会轻微损害性能，因此提出MoE设计原则：在约束条件下最大化总参数、最小化稀疏度和专家数量。


<details>
  <summary>Details</summary>
Motivation: 当前MoE语言模型设计仅考虑总参数（内存占用）和激活参数（推理成本），但研究发现这两个因素不足以描述最优架构。需要更全面的框架来指导MoE设计，解决架构模糊性问题。

Method: 通过系统研究分析MoE性能的决定因素，发现性能主要由总参数(N_total)和专家稀疏度(s:=n_exp/n_topk)决定。研究专家数量和top-k选择之间的关系，分析在内存约束下专家数量增加对核心模型维度的影响。

Result: MoE性能主要取决于总参数和专家稀疏度，专家数量增加会轻微损害性能，因为它迫使减少核心模型的深度和宽度以满足内存约束。专家数量和top-k选择不会在稀疏度中"抵消"。

Conclusion: 提出MoE设计原则：在给定约束条件下最大化总参数、最小化稀疏度（最大化n_topk）和专家数量。这为MoE架构设计提供了稳健的框架，解决了架构模糊性问题。
...
TL;DR: Med-CoReasoner框架通过并行英语和本地语言推理、概念抽象和知识整合，显著提升多语言医学推理能力，在低资源语言上效果尤为明显。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在英语医学任务上表现良好，但在多语言环境下存在显著差距，特别是在本地语言的推理能力较弱，这限制了全球医疗公平部署。需要解决多语言医学推理的挑战。

Method: 提出Med-CoReasoner框架：1) 并行激发英语和本地语言推理；2) 将推理抽象为结构化概念；3) 通过概念级对齐和检索，将本地临床知识整合到英语逻辑框架中。结合英语推理的结构稳健性和本地语言的实践专业知识。

Result: 在三个基准测试中，Med-CoReasoner平均提升多语言推理性能5%，在低资源语言上提升尤为显著。模型蒸馏和专家评估证实该方法能产生临床合理且文化背景相关的推理轨迹。

Conclusion: Med-CoReasoner通过语言协同推理框架有效缩小了多语言医学推理差距，将英语的结构优势与本地语言的实践知识相结合，为全球医疗公平部署提供了可行方案。
...
TL;DR: DART是一个强化学习框架，通过动态构建rollout树在长链推理中自发发现工具使用机会，无需人工标注，显著提升LLMs在复杂任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前工具集成推理范式虽然能增强LLMs的计算能力，但在长链思维推理中集成工具使用仍未被充分探索，主要由于训练数据稀缺以及在不损害模型内在长链推理能力的前提下集成工具的挑战。

Method: DART框架在训练期间构建动态rollout树来发现有效的工具使用机会，在promising位置分支以探索多样化的工具集成轨迹。随后，基于树的优势估计过程识别并奖励那些工具调用对解决方案有积极贡献的特定子轨迹，从而强化这些有益行为。

Result: 在AIME和GPQA-Diamond等具有挑战性的基准测试上的广泛实验表明，DART显著优于现有方法，成功地将工具执行与长链CoT推理相协调。

Conclusion: DART通过强化学习框架实现了在长链推理中自发工具使用，无需人工标注，有效解决了工具集成与长链推理协调的挑战，为增强LLMs的计算能力提供了新途径。
...
### [104] [D$^2$Plan: Dual-Agent Dynamic Global Planning for Complex Retrieval-Augmented Reasoning](https://arxiv.org/abs/2601.08282)
*Kangcheng Luo,Tinglang Wu,Yansong Feng*

Main category: cs.CL

TL;DR: D²Plan：一种用于复杂检索增强推理的双智能体动态全局规划范式，通过推理器和净化器的协作解决搜索链构建无效和推理劫持问题


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的检索增强LLMs在多跳推理任务中存在两个关键失败模式：1）无效的搜索链构建，导致错误查询或遗漏关键信息检索；2）推理被外围证据劫持，使模型将干扰项误认为有效证据
...
Conclusion: D²Plan通过双智能体协作和动态全局规划有效解决了检索增强推理中的关键挑战，提升了多跳推理任务的性能和鲁棒性
...
### [105] [Enhancing Sentiment Classification and Irony Detection in Large Language Models through Advanced Prompt Engineering Techniques](https://arxiv.org/abs/2601.08302)
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过高级提示工程技术（如少样本学习、思维链提示等）来提升大语言模型在情感分析任务中的性能，特别是针对GPT-4o-mini和gemini-1.5-flash模型

Method: 使用高级提示工程技术（少样本学习、思维链提示、自一致性）与基线方法对比，在情感分类、基于方面的情感分析和反讽检测等任务上评估GPT-4o-mini和gemini-1.5-flash模型，使用准确率、召回率、精确率和F1分数作为评估指标

Result: 高级提示技术显著提升情感分析性能：少样本方法在GPT-4o-mini上表现最佳，思维链提示使gemini-1.5-flash在反讽检测任务上性能提升高达46%

Conclusion: 提示策略需要根据具体模型和任务进行定制，强调提示设计必须与LLM架构和任务语义复杂度对齐的重要性
...
TL;DR: CLaS-Bench是一个用于评估多语言大模型语言控制能力的轻量级基准测试，涵盖32种语言，通过语言控制和语义相关性两个维度评估各种干预技术。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门评估大语言模型多语言控制技术的基准测试和评估协议，无法量化各种干预技术的有效性，阻碍了多语言NLP领域的发展。

Method: 提出CLaS-Bench基准测试，评估包括残差流DiffMean干预、探针方向、语言特定神经元、PCA/LDA向量、稀疏自编码器等多种多语言控制技术，通过语言控制和语义相关性两个维度计算调和平均得分。

Result: 研究发现简单的基于残差的DiffMean方法在所有语言中始终优于其他方法；层分析显示语言特定结构主要出现在后期层；控制方向根据语系聚类。

Conclusion: CLaS-Bench是首个标准化的多语言控制基准测试，既能支持语言表示的科学分析，又能作为低成本适应替代方案的实际评估工具。
...
TL;DR: 首个针对语音对话中精神操控检测的研究，通过合成多说话者语音数据集评估模态对检测准确性的影响，发现模型在语音上的召回率显著低于文本，人类评估者也表现出类似的不确定性。


<details>
  <summary>Details</summary>
Motivation: 先前关于精神操控检测的研究仅关注文本对话，忽略了操控策略在语音中的表现。本研究旨在填补这一空白，探索多模态对话系统中精神操控检测的挑战。

Method: 创建了首个语音精神操控检测基准SPEECHMENTALMANIP，通过高质量、声音一致的文本转语音技术将文本数据集增强为多说话者音频。使用少样本大型音频-语言模型和人工标注，评估模态对检测准确性和感知的影响。

Result: 模型在语音检测上表现出高特异性但召回率显著低于文本，表明对训练中缺失的声学或韵律线索敏感。人类评估者在音频设置下也表现出类似的不确定性，凸显了操控性语音固有的模糊性。

Conclusion: 研究强调了在多模态对话系统中进行模态感知评估和安全对齐的必要性，语音中的精神操控检测比文本更具挑战性，需要更全面的方法。
...
TL;DR: 该研究提出了一种基于学生个性特征调整LLM教学策略的个性化教育框架，通过构建教学法与个性特征的分类体系，让LLM导师能够根据模拟学生个性调整教学策略。


<details>
  <summary>Details</summary>
Motivation: 当前LLM教学系统未能考虑学生个性特征，而不同的教学策略对不同个性的学生效果不同，不匹配的教学策略可能对学生学习成果产生负面影响。

Method: 首先基于教育学文献构建连接教学法与个性特征的分类体系，然后模拟师生对话，让LLM导师根据模拟学生个性调整教学策略，最后通过人类教师进行评估。
...
TL;DR: Latent-GRPO通过潜在空间几何特性生成内在奖励，替代昂贵的外部验证器，实现2倍以上训练加速，同时保持模型性能。
...
Method: 提出Latent-GRPO框架，基于潜在空间几何特性：正确推理轨迹的终端token表示形成密集聚类，错误轨迹则分散为离群点。引入迭代鲁棒质心估计算法，通过球面投影缓解幅度波动，通过迭代聚合估计鲁棒的"真理质心"。

Result: 在多个数据集上，该方法保持模型性能的同时，相比基线实现2倍以上训练加速。广泛结果展示了强大的泛化能力和鲁棒性。
...
TL;DR: JudgeRLVR提出两阶段法官-生成范式，通过先训练模型判断解决方案的正确性，再基于此进行生成，在数学推理任务上实现了更好的准确率-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR仅优化最终答案正确性，导致模型陷入漫无目的的冗长探索，依赖试错而非结构化规划。启发式约束如长度惩罚会截断必要推理步骤，造成效率与验证之间的困难权衡。

Method: 提出JudgeRLVR两阶段范式：第一阶段训练模型判断具有可验证答案的解决方案响应；第二阶段使用从法官模型初始化的普通生成RLVR对同一模型进行微调。
...
Conclusion: 判别能力是高效生成的前提，通过学习区分有效解决方案，模型可以内化指导信号来剪枝搜索空间。JudgeRLVR的两阶段法官-生成范式在保持推理质量的同时显著提高了效率。
...
TL;DR: 提出一个结合NLP和形式化验证的框架，用于开发具有共情能力的治疗对话机器人，通过Transformer提取对话特征，用随机混合自动机建模治疗会话，并通过统计模型检查验证共情属性。


<details>
  <summary>Details</summary>
Motivation: 对话机器人在心理健康治疗路径中作为支持工具的应用日益增多，但当前开发实践缺乏系统化的方法来指定和验证共情这一关键非功能性需求。

Method: 提出一个集成自然语言处理和形式化验证的框架：1) 使用基于Transformer的模型提取对话特征；2) 将特征转换为治疗会话的随机混合自动机模型；3) 通过统计模型检查验证共情相关属性；4) 通过策略合成指导智能体行为塑造。

Result: 初步结果显示：形式化模型能够较好地捕捉治疗会话的动态特性，并且定制的策略能够提高满足共情需求的概率。

Conclusion: 该框架为开发具有共情能力的治疗对话机器人提供了一种系统化的方法，结合了NLP的特征提取能力和形式化验证的严格性，有望改善心理健康支持工具的质量。
...
TL;DR: 论文研究了大型语言模型中的"溢出"现象——普通文本提示会引发过度输出，导致成本增加、延迟和服务性能下降，并提出了BenchOverflow基准来量化评估和缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在普通交互设置下产生过度输出的"溢出"现象，这种现象不同于越狱或提示注入，但会导致服务成本上升、延迟增加和跨用户性能下降，在经济和环境方面都有重要影响。

Method: 提出BenchOverflow基准，包含9种纯文本提示策略，使用标准化协议（5000个新token预算）评估9个开源和闭源模型，通过CSR指标和ECDF函数量化尾部风险，并测试了简单的缓解措施（简洁性提醒）。

Result: 观察到长度分布明显右移和重尾现象，CSR指标显示尾部风险显著，溢出现象在不同模型家族和攻击向量间存在异质性，轻量级缓解措施能有效减少尾部风险。

Conclusion: 长度控制应被视为可测量的可靠性、成本和可持续性问题，BenchOverflow为选择最小化资源浪费的部署方案和评估防御措施提供了实用基础。
...
TL;DR: MHEL-LLaMo是一个用于历史文本实体链接的无监督集成方法，结合小型语言模型和大型语言模型，在六个欧洲语言的历史文本基准测试中优于现有方法，无需微调即可提供可扩展解决方案。
...
Method: 提出MHEL-LLaMo，一种无监督集成方法，结合小型语言模型和大型语言模型。使用多语言双编码器进行候选实体检索，通过指令调优的LLM进行NIL预测和候选选择。利用SLM的置信度分数区分简单和困难样本，仅对困难案例应用LLM。

Result: 在六个欧洲语言（英语、芬兰语、法语、德语、意大利语和瑞典语）的四个历史文本基准测试中，MHEL-LLaMo优于最先进模型，无需微调，为低资源历史实体链接提供了可扩展解决方案。

Conclusion: MHEL-LLaMo通过结合SLM和LLM的优势，在降低计算成本的同时防止简单案例的幻觉，为历史文本实体链接提供了有效且可扩展的无监督解决方案。
...
TL;DR: STAGE是一个统一的电影剧本叙事理解基准测试，包含知识图谱构建、场景级事件摘要、长上下文剧本问答和角色扮演四个任务，基于150部中英文电影的清洗剧本和标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要针对问答或对话生成等单一子任务，缺乏评估模型能否构建连贯故事世界并在多种推理和生成任务中保持一致性的能力。电影剧本作为丰富的长篇叙事形式，包含复杂角色关系、时序事件和对话驱动交互，需要更全面的评估框架。

Method: STAGE定义了四个基于共享叙事世界表示的任务：1)知识图谱构建，2)场景级事件摘要，3)长上下文剧本问答，4)剧本内角色扮演。基准测试提供150部中英文电影的清洗剧本、精心策划的知识图谱、事件和角色中心标注。

Result: STAGE基准测试提供了全面的数据集和评估框架，能够对模型在构建世界表示、抽象和验证叙事事件、长叙事推理以及生成角色一致响应等方面的能力进行整体评估。

Conclusion: STAGE填补了现有基准测试的空白，为评估模型在长篇叙事理解方面的综合能力提供了统一框架，特别强调构建连贯故事世界并在多种任务中保持一致性，推动了叙事智能研究的发展。
...
TL;DR: STAR框架通过分析输出概率偏移检测推理时间后门攻击，利用恶意输入诱导路径的高后验概率与低先验概率之间的统计差异，实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs集成Chain-of-Thought等推理机制，显式推理暴露了推理时间后门攻击的新攻击面。这些攻击注入恶意推理路径而不改变模型参数，由于生成语言连贯的路径，能有效逃避传统检测方法。
...
Conclusion: STAR框架为检测推理时间后门攻击提供了有效解决方案，通过分析概率偏移统计差异，实现了高效、鲁棒的检测性能，对保护LLMs推理安全具有重要意义。
...
TL;DR: 该论文研究了无限维空间中条件收敛、无条件收敛和绝对收敛的区别及其对计算算法的影响，建立了无条件收敛的七个等价条件，并将其应用于随机梯度下降和信号处理等计算实践。


<details>
  <summary>Details</summary>
Motivation: 在有限维空间中，条件收敛、无条件收敛和绝对收敛是等价的，但在无限维Banach空间中，根据Dvoretzky-Rogers定理，这些概念严格分离。这种分离对计算算法有重要影响，需要建立统一的理论框架来理解无条件收敛的性质，并将其应用于现代计算实践中。

Method: 作者提出了一个综合性的特征定理，统一了无条件收敛的七个等价条件：置换不变性、网收敛、子级数检验、符号稳定性、有界乘子性质、弱一致收敛等。这些理论结果直接应用于算法稳定性分析。

Result: 建立了无条件收敛的七个等价条件的完整理论框架，证明了它们在无限维空间中的等价性。这些结果直接应用于计算实践：1）解释了随机梯度下降中梯度累积的置换不变性；2）为基于框架的信号处理中的系数阈值化提供了理论依据。
...
TL;DR: Deep Research Bench II：一个包含132个研究任务、9430个细粒度二元评分标准的基准测试，用于评估深度研究系统生成报告的质量，发现现有最强模型仅满足不到50%的标准。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究系统评估存在两个主要问题：1）未能充分测试系统分析证据和撰写连贯报告的能力；2）评估标准过于粗糙或完全由LLM定义，导致评分存在偏见且难以验证解释。需要更严谨的评估方法。

Method: 构建Deep Research Bench II基准测试，包含132个基于事实的研究任务，覆盖22个领域。通过四阶段LLM+人工流程创建9430个细粒度二元评分标准，涵盖信息回忆、分析和呈现三个维度，确保标准原子化、可验证且与专家判断一致。

Result: 评估多个最先进的深度研究系统发现，即使最强的模型也只能满足不到50%的评分标准，揭示了当前深度研究系统与人类专家之间存在显著差距。

Conclusion: Deep Research Bench II提供了一个更严谨、可验证的评估框架，能够有效衡量深度研究系统的能力，并揭示了当前系统在生成高质量研究报告方面仍有很大改进空间。
...
TL;DR: Ministral 3系列是参数高效的密集语言模型，包含3B、8B、14B三种尺寸，每个尺寸都有基础版、指令微调版和推理版三种变体，采用Cascade Distillation技术训练，具备图像理解能力，使用Apache 2.0许可证。


<details>
  <summary>Details</summary>
Motivation: 针对计算和内存受限的应用场景，开发参数高效的密集语言模型，满足不同规模需求的同时保持高性能。

Method: 采用Cascade Distillation技术，这是一种迭代剪枝和持续训练结合蒸馏的方法，通过蒸馏技术优化模型参数。

Result: 发布了Ministral 3系列模型，包含3B、8B、14B三种参数规模，每个规模都有基础模型、指令微调模型和推理模型三种变体，所有模型都具备图像理解能力。

Conclusion: Ministral 3系列为计算和内存受限的应用提供了高效的密集语言模型解决方案，通过Cascade Distillation技术实现了参数效率优化，并在Apache 2.0许可证下开源。
...
TL;DR: ExpSeek提出了一种在Web智能体中主动寻求经验干预的方法，通过熵阈值确定干预时机，设计步骤级定制经验内容，显著提升了智能体性能。


<details>
  <summary>Details</summary>
Motivation: 现有经验干预方法主要在任务执行前被动注入全局上下文，难以适应智能体与环境交互过程中动态变化的上下文观察，需要更灵活、主动的经验干预机制。

Method: ExpSeek将经验干预转向步骤级主动寻求：1）利用模型内在信号估计步骤级熵阈值来确定干预时机；2）设计步骤级定制化的经验内容。

Result: 在Qwen3-8B和32B模型上的四个具有挑战性的Web智能体基准测试中，ExpSeek分别实现了9.3%和7.5%的绝对性能提升。实验验证了熵作为自触发信号的可行性，并发现即使是4B小规模经验模型也能显著提升更大智能体模型的性能。

Conclusion: ExpSeek通过步骤级主动经验寻求机制，有效解决了传统被动经验注入方法的局限性，为Web智能体的经验干预提供了新的技术范式。
...
TL;DR: OrderProbe：一个用于评估大语言模型结构重建能力的确定性基准，使用中、日、韩语的四字固定表达式，通过精确匹配评分来测试模型从乱序输入中恢复原始结构的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在语义理解方面表现出色，但从乱序输入中重建内部结构的能力尚未得到充分探索。句子级别的恢复任务难以进行自动化评估，因为存在多种有效的词序排列。

Method: 引入OrderProbe基准，使用中文、日文和韩文的固定四字表达式，这些语言具有唯一的规范词序，支持精确匹配评分。提出一个诊断框架，评估模型在恢复准确性之外的多维度能力：语义保真度、逻辑有效性、一致性、鲁棒敏感性和信息密度。

Result: 在12个广泛使用的大语言模型上的实验表明，结构重建任务对前沿系统仍然具有挑战性：零样本恢复准确率经常低于35%。观察到语义回忆和结构规划之间存在一致性的分离，表明结构鲁棒性并非语义能力的自动副产品。

Conclusion: 结构重建能力是大语言模型的一个独立且具有挑战性的维度，需要专门的评估方法。OrderProbe基准为评估模型的结构处理能力提供了可靠的工具，揭示了当前模型在结构规划方面的局限性。
...
TL;DR: LALITA框架通过基于词汇和语言特征筛选源句，为低资源语言机器翻译训练优化平行语料库构建，显著减少数据需求并提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 低资源语言机器翻译面临数据获取成本高的问题，人工翻译生成足够数据过于昂贵，需要开发能够筛选源句形成高效平行文本的框架，以优化低资源环境下的MT系统性能。

Method: 提出LALITA框架，基于词汇和语言特征进行源句选择，通过评估英-印地双语文本确定有效的句子选择策略，主要训练复杂句子（包括现有和合成数据集），在50K到800K英语句子的不同数据规模下进行测试。

Result: LALITA显著提高了翻译质量，在多个语言（印地语、奥里亚语、尼泊尔语、挪威尼诺斯克语和德语）上减少数据需求超过一半，同时展示了在数据增强方面的实用性。

Conclusion: LALITA框架为低资源语言机器翻译提供了一种高效的数据筛选方法，不仅降低了训练成本，还通过优化数据选择策略提升了系统性能，具有广泛的应用潜力。
...
TL;DR: 该研究探讨了大型语言模型中道德价值观与政治立场之间的因果关系，通过道德条件控制来观察模型政治倾向的变化，发现道德条件能显著影响模型的政治坐标。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要通过直接探测或人口统计角色工程来评估LLMs的政治倾向，但社会心理学认为政治意识形态是基本道德直觉的下游结果。本研究旨在探索道德价值观与政治定位之间的因果关系。

Method: 将道德取向作为可控条件，让模型支持或拒绝特定道德价值观，然后使用政治罗盘测试评估由此产生的政治倾向变化。通过角色框架和模型规模调节，使用替代评估工具验证结果的稳健性。

Result: 道德条件能引起模型政治坐标的显著、特定价值观的偏移。这些效应受到角色框架和模型规模的系统性调节，并且在实例化相同道德价值观的替代评估工具中具有稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 潜在习语表达（PIEs）与特定语言社区的日常经验密切相关，是评估NLP系统语言和文化能力的挑战性任务。需要创建高质量的多语言多模态数据集来支持相关研究

Method: 创建了XMPIE数据集，包含34种语言的平行数据，每个PIE配有5张图像（从习语到字面意义的连续谱），包括语义相关和随机干扰项。数据由语言专家根据多语言指南创建

Result: 构建了一个高质量的多语言多模态习语理解基准数据集，支持跨语言和跨模态的习语理解研究，可用于评估模型在不同语言和模态间的迁移能力

Conclusion: XMPIE数据集为评估多语言多模态习语理解提供了有价值的基准，支持跨语言和跨模态的习语理解研究，有助于深入了解共享文化方面
...
<details>
  <summary>Details</summary>
Motivation: 随着极限学习领域的发展，语言识别已被证明不可能，但语言生成是可处理的。然而，在现实世界应用中，需要考虑语言生成的安全性影响，因此需要建立安全语言生成的理论基础。

Method: 基于极限学习的计算范式，形式化了安全语言识别和安全语言生成的任务。通过理论证明分析这些任务的可行性边界。

Result: 1. 安全语言识别是不可能的；2. 安全语言生成至少与普通语言识别一样困难（同样不可能）；3. 讨论了若干不可处理和可处理的具体情况。

Conclusion: 该研究为安全语言生成提供了首个理论框架，揭示了其基本限制，并指出了在特定条件下可能实现安全生成的方向，对实际应用具有重要指导意义。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM作为评分者的范式面临三个主要挑战：评分标准因提示词敏感性而不稳定、推理过程缺乏可审计证据、评分尺度与人类标准不对齐。这些问题限制了基于规则的评估的可扩展性和可靠性。

Method: 提出RULERS框架，包含三个核心组件：1) 将自然语言评分标准编译为版本化不可变包；2) 强制执行结构化解码和确定性证据验证；3) 应用基于Wasserstein距离的轻量级后校准，整个过程不更新模型参数。

Result: 在论文和摘要评估基准上的实验表明，RULERS在人类一致性方面显著优于代表性基线方法，对对抗性评分标准扰动保持强稳定性，并使较小模型能够与大型专有评分者相媲美。

Conclusion: 可靠的LLM评分需要可执行的评分标准、可验证的证据和校准的尺度，而不仅仅是提示词措辞。RULERS框架通过编译器-执行器架构实现了这一目标，为可扩展的基于规则的评估提供了有效解决方案。
...
TL;DR: LLMs在仇恨言论净化任务中经常出现虚假拒绝行为，本研究系统分析了触发这种拒绝的上下文和语言偏见，并提出跨语言翻译策略来缓解此问题。
...
Method: 评估了9个LLM在英语和多语言数据集上的表现，分析语义毒性和目标群体对拒绝率的影响，并提出跨翻译策略（英语→中文净化→英语）。

Result: LLMs对语义毒性较高的输入和针对特定群体（国籍、宗教、政治意识形态）的内容拒绝率更高；多语言数据集总体拒绝率低于英语，但仍存在语言依赖的系统性偏见；跨翻译策略显著减少了虚假拒绝同时保持原内容。

Conclusion: 跨语言翻译策略提供了一种有效且轻量级的缓解方法，能够减少LLM在仇恨言论净化任务中的虚假拒绝行为。
...
### [127] [QuantEval: A Benchmark for Financial Quantitative Tasks in Large Language Models](https://arxiv.org/abs/2601.08689)
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在金融量化任务中的评估较为零散且主要局限于知识问答，缺乏对模型在实际量化金融工作中综合能力的系统性评估，特别是策略编码和回测能力。

Method: 开发了QuantEval基准测试，包含三个评估维度：知识问答、量化数学推理和量化策略编码。特别引入了CTA式回测框架，能够执行模型生成的交易策略并使用金融性能指标进行评估。还进行了大规模监督微调和强化学习实验。

Result: 评估了多个开源和专有的大语言模型，发现它们与人类专家存在显著差距，特别是在推理和策略编码方面。通过领域对齐数据的微调和强化学习实验，模型性能得到了持续改进。

Conclusion: QuantEval基准测试能够促进大语言模型在量化金融能力方面的研究，加速其在真实交易工作流程中的实际应用。研究团队还发布了完整的确定性回测配置以确保严格的可复现性。
...
### [128] [RAGShaper: Eliciting Sophisticated Agentic RAG Skills via Automated Data Synthesis](https://arxiv.org/abs/2601.08699)
*Zhengwei Tao,Bo Li,Jialong Wu,Guochen Yan,Huanyao Zhang,Jiahao Xu,Haitao Mi,Wentao Zhang*

Main category: cs.CL

TL;DR: RAGShaper是一个自动化合成RAG任务和智能体轨迹的数据生成框架，通过构建包含对抗性干扰的信息树和约束导航策略，提升智能体在噪声环境中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前智能体检索增强生成系统面临高质量训练数据稀缺的问题，传统人工标注方法无法规模化且难以捕捉处理检索失败所需的动态推理策略，这阻碍了鲁棒智能体的开发。
...
Conclusion: RAGShaper通过自动化合成包含真实世界噪声和复杂性的训练数据，有效解决了RAG智能体开发中的数据稀缺问题，为构建更鲁棒的检索增强智能体提供了可行方案。
...
TL;DR: PrivGemo是一个保护隐私的知识图谱增强推理框架，通过双塔设计和记忆引导的暴露控制，在保持原始知识图谱本地化的同时，支持远程推理，解决了现有隐私处理方法的结构泄露、不可控远程交互等四大限制。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱隐私处理方法主要关注实体名称掩码，但仍面临结构泄露、不可控远程交互、脆弱的多跳多实体推理以及经验复用有限等问题。特别是许多实际知识图谱是私有的，将检索到的三元组或探索轨迹发送到闭源LLM API会带来泄露风险。

Method: PrivGemo采用双塔设计：保持原始知识图谱本地化，同时在匿名化视图上进行远程推理，超越名称掩码限制语义和结构暴露。支持多跳多实体推理，通过检索连接所有主题实体的匿名长跳路径，同时在本地知识图谱上进行验证。采用分层控制器和隐私感知经验记忆减少不必要的探索和远程交互。

Result: 在六个基准测试上的综合实验表明，PrivGemo实现了整体最先进的结果，比最强基线高出17.1%。此外，PrivGemo使较小模型（如Qwen3-4B）能够达到与GPT-4-Turbo相当的推理性能。

Conclusion: PrivGemo通过隐私保护的检索增强框架，有效解决了知识图谱增强推理中的隐私泄露问题，在保持推理性能的同时显著降低了隐私风险，为私有知识图谱与大型语言模型的结合提供了安全有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理包含数千行数字、多个链接工作表以及嵌入图表和收据等视觉内容的企业级电子表格时存在困难。现有的电子表格推理方法通常依赖单表压缩或全上下文编码，限制了可扩展性，且无法反映真实用户与复杂多模态工作簿的交互方式。

Method: 提出了FRTR（From Rows to Reasoning）框架：1）将Excel工作簿分解为细粒度的行、列和块嵌入；2）采用带有互惠排名融合（RRF）的混合词汇-密集检索；3）集成多模态嵌入以同时处理数值和视觉信息。

Result: 在FRTR-Bench基准测试中，使用Claude Sonnet 4.5达到74%的答案准确率，相比之前最先进方法的24%有显著提升。在SpreadsheetLLM基准测试中，使用GPT-5达到87%的准确率，同时与上下文压缩方法相比减少了约50%的令牌使用量。

Conclusion: FRTR框架通过细粒度分解、混合检索和多模态集成，有效解决了LLMs在大规模企业电子表格推理中的挑战，显著提升了性能和效率，为复杂工作簿的智能分析提供了实用解决方案。
...
TL;DR: 论文提出了Attributional NLI框架，将社会心理学原理融入自然语言推理，通过Undercover-V文本游戏评估LLM在多智能体环境中的意图推理能力，发现神经符号智能体表现最佳。


<details>
  <summary>Details</summary>
Motivation: 传统自然语言推理无法捕捉多智能体环境中复杂的意图驱动推理，而意图归因推理是LLM在交互系统中的关键但未被充分探索的能力。

Method: 提出Attributional NLI框架，结合溯因意图推理和演绎验证。通过Undercover-V文本游戏实验，比较三种LLM智能体：仅使用演绎推理的标准NLI智能体、使用溯因-演绎推理的Att-NLI智能体、以及结合外部定理证明器的神经符号Att-NLI智能体。

Result: 实验显示归因推理能力存在明显层次：神经符号智能体表现最佳，平均胜率达到17.08%，显著优于其他智能体。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的Text-to-SQL方法在提示中包含完整数据库模式，导致上下文过长和预填充延迟增加。虽然用户查询通常聚焦于重复的表集合，但当前推理引擎在处理不同表顺序的查询时会生成冗余的前缀缓存副本

Method: 1. 离线预计算表表示作为KV缓存；2. 计算表缓存时保持表间主外键关系；3. 构建Table Trie结构支持高效KV缓存查找；4. 引入缓存管理系统，包括查询重排序策略提高缓存命中率和并行化模型推理与缓存加载的计算加载流水线

Result: TableCache在实验中实现了高达3.62倍的Time to First Token (TTFT)加速，性能下降可忽略不计

Conclusion: 通过预计算表KV缓存和高效缓存管理，TableCache显著减少了Text-to-SQL任务中LLM推理的延迟，同时保持了模型性能
...
TL;DR: 该研究提出了一种基于注意力的方法，将航空影像和地理定位文本结合在空间邻域内，用于预测环境变量，在多个变量类别上优于单模态基线。


<details>
  <summary>Details</summary>
Motivation: 文本作为生态学的新兴数据源，具有独特信息价值，可与地理空间数据互补，提供传统数据源无法捕捉的局部尺度环境信息。但文本数据在生态学中的贡献定义不清，可用性稀疏不规则，与地理空间数据整合困难。

Method: 提出基于注意力的方法，结合视觉（航空影像）和文本表示与地理位置编码，通过注意力模块动态选择对预测任务有用的空间邻居。在EcoWikiRS数据集上应用，该数据集包含瑞士高分辨率航空影像和描述当地环境条件的维基百科句子。

Result: 模型在预测SWECO25数据立方体中103个环境变量的任务上，持续优于单位置或单模态（仅图像或仅文本）基线。按主题组分析变量时，在气候、土壤、人口和土地利用/土地覆盖变量上表现出显著性能提升。

Conclusion: 结合文本和图像数据时包含空间上下文具有明显优势，基于注意力的空间邻域整合方法能有效利用文本和视觉信息的互补性，提升环境变量预测性能。
...
TL;DR: 提出Multiplex Thinking方法，通过采样K个候选token并聚合其嵌入为单个连续多路复用token，实现随机软推理机制，在保持词汇嵌入先验的同时压缩序列长度，并通过强化学习优化。


<details>
  <summary>Details</summary>
Motivation: 大语言模型使用思维链（CoT）解决复杂推理任务时会产生长序列、低带宽的token序列，而人类通常通过维护可能下一步的概率分布进行软推理。受此启发，希望开发更高效的推理机制。

Method: 提出Multiplex Thinking方法：在每个思考步骤中采样K个候选token，将它们的嵌入聚合为单个连续多路复用token。该方法保持词汇嵌入先验和标准离散生成的采样动态，同时诱导可处理的多路复用轨迹概率分布，可直接通过策略强化学习优化。

Result: 在具有挑战性的数学推理基准测试中，Multiplex Thinking从Pass@1到Pass@1024都持续优于强离散CoT和RL基线方法，同时产生更短的序列。

Conclusion: Multiplex Thinking是一种自适应的随机软推理机制，在模型置信度高时接近离散CoT行为，在不确定时紧凑表示多个可能下一步，不增加序列长度，实现了更高效的推理。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型智能体在学术审稿系统中的行为动态，特别是在引入Elo排名系统后审稿人之间的互动模式，以及这种系统如何影响审稿质量和决策准确性。

Method: 使用真实会议论文提交，构建多轮审稿互动模拟系统，其中多个具有不同人格的LLM智能体审稿人参与审稿，由领域主席协调。比较了基线设置与包含Elo评分和审稿人记忆的条件设置。

Result: 模拟结果显示：1) 引入Elo评分提高了领域主席决策的准确性；2) 审稿人发展出适应性审稿策略，能够利用Elo系统但并未提高审稿努力程度；3) 发现了多个有趣的审稿动态现象。

Conclusion: Elo排名系统能够改善学术审稿的决策质量，但审稿人会发展出适应性策略来利用系统，这可能带来新的挑战。研究为理解LLM智能体在复杂评审系统中的行为提供了重要见解。
...
### [136] [Bridging the Trust Gap: Clinician-Validated Hybrid Explainable AI for Maternal Health Risk Assessment in Bangladesh](https://arxiv.org/abs/2601.07866)
*Farjana Yesmin,Nusrat Shirmin,Suraiya Shabnam Bristy*

Main category: cs.AI

TL;DR: 本研究提出了一种结合模糊逻辑和SHAP解释的混合可解释AI框架，用于孕产妇健康风险预测，在资源受限环境中提升临床信任度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在孕产妇健康风险预测方面显示出潜力，但在资源受限的临床环境中，由于缺乏可解释性和信任度，其临床采用面临关键障碍。

Method: 开发了模糊-XGBoost混合模型，结合先验模糊逻辑和后验SHAP解释，使用1,014份孕产妇健康记录进行训练，并通过孟加拉国14名医疗专业人员的系统性反馈进行验证。

Result: 模型达到88.67%准确率（ROC-AUC：0.9703）；验证研究显示71.4%的医疗专业人员偏好混合解释，54.8%表示对临床使用具有信任；SHAP分析确定医疗可及性为主要预测因子。

Conclusion: 结合可解释模糊规则和特征重要性解释能增强实用性和信任度，为孕产妇医疗保健中的XAI部署提供实用见解，同时识别了产科史、孕龄和连接障碍等关键差距。
...
### [137] [When Models Know When They Do Not Know: Calibration, Cascading, and Cleaning](https://arxiv.org/abs/2601.07965)
*Chenjie Hao,Weyl Lu,Yuko Ishiwaka,Zengyi Li,Weier Wan,Yubei Chen*

Main category: cs.AI

TL;DR: 提出一种无需训练的通用校准方法，利用模型置信度识别"不知道"的情况，应用于模型级联和数据清洗，提高AI效率与可靠性。


<details>
  <summary>Details</summary>
Motivation: 当模型能够识别自身不知道的情况时，会开启许多可能性。关键问题是如何让模型意识到自己不知道，一个可行方法是利用模型内部信号计算置信度来反映其无知程度。先前特定领域研究表明校准可以提供可靠的置信度估计。

Method: 提出简单、有效、无需训练的通用方法，适用于视觉和语言模型，通过模型校准、级联和数据清洗来更好地利用模型识别"不知道"的能力。基于两个关键观察：单个模型内置信度越高对应准确率越高，验证集校准的模型在测试集上保持校准。在此基础上引入两个应用：1）基于校准优势路由的模型级联；2）基于模型集成和校准置信度的数据清洗方法。

Result: 通过校准置信度的可比性路由信号，级联大小模型可在几乎不损失准确率的情况下提高效率，级联两个规模相当的模型可获得超越任一单独模型的性能。利用多个专家及其校准置信度，设计的数据清洗方法在ImageNet和MMLU数据集中有效识别错误标注样本，平衡了精确度和检测率。

Conclusion: 让模型能够识别自身不知道的情况是实现更高效、可靠、可信AI的实用步骤。校准置信度提供了可靠且可比较的度量，支持模型级联和数据清洗等应用。
...
### [138] [Reasoning over Precedents Alongside Statutes: Case-Augmented Deliberative Alignment for LLM Safety](https://arxiv.org/abs/2601.08000)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在遵循安全原则时面临两难：要么过度拒绝良性请求，要么安全性不足。OpenAI的审慎对齐方法依赖详细的"代码式"安全规则，但这种方法在开源LLM中效果不佳，因为开源模型通常缺乏高级推理能力。

Method: 提出CADA（案例增强的审慎对齐）方法，通过强化学习在自生成的安全推理链上进行训练，使用案例增强的简单代码而非详细的规则枚举，使模型能够进行更灵活、适应性更强的安全推理。

Result: 实验发现：1）引用详细安全规则对无害性提升不一致且会系统性降低有用性；2）基于案例增强的简单代码训练能产生更鲁棒和泛化的安全行为；3）CADA方法有效提升无害性、增强对抗攻击的鲁棒性、减少过度拒绝，同时在不同基准测试中保持实用性。

Conclusion: 案例增强的审慎对齐方法为提升LLM安全性提供了实用替代方案，避免了规则导向方法的僵化性，在保持有用性的同时实现了更好的安全性和适应性。
...
### [139] [Internal Deployment Gaps in AI Regulation](https://arxiv.org/abs/2601.08005)
...
<details>
  <summary>Details</summary>
Motivation: 前沿AI法规主要关注部署给外部用户的系统，但高风险的内部部署（如研发自动化、关键业务流程加速、敏感数据处理）可能逃避监管。论文旨在揭示内部AI系统监管的漏洞，促使政策制定更加审慎。

Method: 通过分析美国和欧盟2025年前沿AI法规，识别内部部署系统的监管缺口，包括范围模糊性、时点合规评估、信息不对称等问题，并探讨这些缺口存在的原因及可能的解决方案。

Result: 识别出三个关键监管缺口：1) 范围模糊性允许内部系统逃避监管义务；2) 时点合规评估无法捕捉内部系统的持续演化；3) 信息不对称削弱监管意识和监督能力。这些缺口源于可测量性、激励机制和信息获取等方面的矛盾。

Conclusion: 内部部署的AI系统存在显著的监管漏洞，可能导致高风险应用逃避监管。政策制定者需要审慎考虑这些缺口，通过权衡不同方法的利弊，有意识地而非偶然地制定针对内部AI系统的监管政策。
...
### [140] [Integrating Attendance Tracking and Emotion Detection for Enhanced Student Engagement in Smart Classrooms](https://arxiv.org/abs/2601.08049)
*Keith Ainebyona,Ann Move Oguti,Joseph Walusimbi,Ritah Kobusingye*

Main category: cs.AI

TL;DR: SCASED系统结合考勤自动化与面部情绪识别，通过物联网技术实时监测课堂参与度，为教师提供学生情绪状态数据以改进教学策略。


<details>
  <summary>Details</summary>
Motivation: 当前智慧教室技术主要关注考勤自动化，对学生课堂中的情绪和认知参与度关注不足，限制了教师识别学生分心情况并实时调整教学策略的能力。

Method: 基于物联网的系统，使用树莓派摄像头和OpenCV进行人脸检测，采用微调的MobileNetV2模型分类四种学习相关情绪状态（投入、无聊、困惑、沮丧），通过会话机制管理考勤和情绪监测，数据通过云端仪表板可视化。

Result: 使用DAiSEE数据集评估，情绪分类准确率达到89.5%。结果表明，考勤数据与情绪分析的结合能为教师提供更多课堂动态洞察，支持更及时的教学调整。

Conclusion: SCASED系统成功整合了考勤自动化与情绪识别功能，为教师提供了更全面的课堂参与度监测工具，有助于实现更响应式的教学实践。
...
### [141] [Forecast Aware Deep Reinforcement Learning for Efficient Electricity Load Scheduling in Dairy Farms](https://arxiv.org/abs/2601.08052)
*Nawazish Alia,Rachael Shawb,Karl Mason*

Main category: cs.AI

TL;DR: 本文提出了一种用于奶牛场能源调度的深度强化学习框架，通过改进PPO算法结合需求预测和自适应KL散度控制，在电池储能和水加热调度中实现了比传统方法更低的用电成本和更高的电网独立性。


<details>
  <summary>Details</summary>
Motivation: 奶牛养殖是高能耗行业，严重依赖电网供电。随着可再生能源的集成，实时平衡供需变得困难。现有强化学习方法通常假设完全了解未来价格或发电情况，这在动态环境中不现实，且标准PPO变体使用固定阈值导致训练不稳定。

Method: 提出深度强化学习框架用于奶牛场负荷调度，重点关注电池储能和水加热。开发了两种PPO变体：1) Forecast Aware PPO：结合短期需求预测和可再生能源预测，使用小时和月份残差校准；2) PID KL PPO：采用比例积分微分控制器自适应调节KL散度，实现稳定的策略更新。

Result: 使用真实奶牛场数据训练，方法比PPO降低1%用电成本，比DQN降低4.8%，比SAC降低1.5%。在电池调度方面，PPO减少电网输入13.1%，展示了在现代奶牛养殖可持续能源管理中的可扩展性和有效性。

Conclusion: 该研究提出的深度强化学习框架通过结合预测能力和自适应训练机制，有效解决了奶牛场能源调度中的挑战，为实现可持续能源管理和支持联合国可持续发展目标7提供了实用解决方案。
...
### [142] [A New Strategy for Verifying Reach-Avoid Specifications in Neural Feedback Systems](https://arxiv.org/abs/2601.08065)
...
Conclusion: 提出的统一框架克服了传统后向方法的可扩展性限制，为神经反馈系统的可达避免属性验证提供了更有效的解决方案
...
TL;DR: MemoBrain是一个用于工具增强型智能体的执行记忆模型，通过构建依赖感知的记忆来管理长时推理过程中的中间状态和逻辑关系，解决上下文累积问题，提升长时推理的连贯性和目标对齐。


<details>
  <summary>Details</summary>
Motivation: 在工具增强型智能体框架中，复杂推理通常是长时程的，导致推理轨迹和临时工具产物不断积累，超出大语言模型有限的上下文容量。缺乏显式记忆机制会破坏逻辑连续性并削弱任务对齐能力，因此记忆不是辅助效率问题，而是维持长时程连贯、目标导向推理的核心组件。
...
Conclusion: MemoBrain通过显式的认知控制机制管理推理轨迹，而非被动积累上下文，为工具增强型智能体提供了维持长时程连贯推理的关键记忆能力。
...
TL;DR: 研究分析了事故后车道变换行为特征，构建了相关数据集，并开发了基于图注意力机制的轨迹预测框架，显著提升了预测精度和碰撞风险分析可靠性。


<details>
  <summary>Details</summary>
Motivation: 交通事故发生后，后续车辆需要变换车道绕过障碍物，这种事故后车道变换行为中，目标车道车辆可能拒绝让行，增加了复杂性和碰撞风险。然而，目前对事故后车道变换的行为特征和运动模式了解不足。

Method: 1. 从无人机视频中提取车辆轨迹，构建事故后车道变换数据集；2. 进行实证分析，比较事故后车道变换与强制车道变换、自主车道变换的差异；3. 开发基于图注意力机制的轨迹预测框架，将让行行为建模为辅助交互感知任务，指导条件变分自编码器和基于Transformer的解码器预测车道变换者轨迹。

Result: 1. 事故后车道变换持续时间更长、插入速度更低、碰撞风险更高；2. 79.4%的事故后车道变换涉及新跟随者至少一次不让行行为（DLCs为21.7%，MLCs为28.6%）；3. 提出的模型在不同预测时间范围内，平均位移误差和最终位移误差比现有基准方法提升超过10%；4. 模型通过降低误撞率和提高冲突预测准确性，提供了更可靠的碰撞风险分析；5. 模型在不同地点收集的额外数据集上验证了可迁移性。

Conclusion: 研究揭示了事故后车道变换的独特行为特征，开发了有效的轨迹预测框架，显著提升了预测性能，为智能交通系统中的安全评估和决策提供了重要工具。
...
Method: 提出交替活跃/非活跃阶段的内存范式：用户活跃时进行低延迟实时对话（轻量级检索现有记忆和上下文）；用户非活跃时进行更计算密集的记忆提取、整合和维护。还设计了AI伴侣基准来全面评估对话质量和记忆能力

Result: 实验表明，该系统（使用非常弱的模型：Qwen2.5-7B-Instruct量化int4）在大多数指标上优于没有记忆的等效原始LLM，并且与具有16k上下文窗口的GPT-3.5表现相当

Conclusion: 提出的内存范式在嵌入式硬件严格约束下最小化延迟的同时保持了长期个性化，为资源受限环境中的AI伴侣系统提供了可行解决方案
...
TL;DR: Project Synapse是一个用于自主解决最后一公里配送中断的新型智能体框架，采用分层多智能体架构，使用LangGraph编排复杂工作流，并通过LLM作为评判者协议评估性能。


<details>
  <summary>Details</summary>
Motivation: 解决最后一公里配送中的复杂中断问题，传统方法难以处理这些动态、多变的场景，需要更智能的自主决策系统。

Method: 采用分层多智能体架构，中央解决监督智能体进行战略任务分解，委托给专门的工人智能体执行战术操作，使用LangGraph管理复杂循环工作流。

Result: 通过从6000多个真实用户评论中提取的30个复杂中断场景数据集进行验证，使用带有明确偏差缓解的LLM作为评判者协议评估系统性能。

Conclusion: Project Synapse框架能够有效处理最后一公里配送中断，通过智能体协作和系统化工作流管理提供自主解决方案。
...
TL;DR: 提出基于模型的分层多智能体强化学习框架，用于多核平台的温度和能量感知调度，通过LLM语义特征提取实现零样本部署，比Linux ondemand调度器能效提升7.09倍，决策延迟比基于表格的方法快8300倍。


<details>
  <summary>Details</summary>
Motivation: 现有DVFS和任务分配方法要么依赖忽略停顿时间的利用率启发式方法，要么需要大量离线分析生成表格，无法实现运行时自适应。需要一种能够动态适应、考虑热管理和能量性能平衡的调度方案。

Method: 提出模型驱动的分层多智能体强化学习框架：1) 两个协作智能体分解指数级动作空间；2) 准确的环境模型使用回归技术预测热动态和性能状态；3) LLM语义特征提取从OpenMP程序中提取13个代码级特征；4) Dyna-Q启发框架结合直接强化学习和基于模型的规划。

Result: 在BOTS和PolybenchC基准测试中，在NVIDIA Jetson TX2、Jetson Orin NX、RubikPi和Intel Core i7平台上，相比Linux ondemand调度器：能效提升7.09倍，完工时间改善4.0倍。首次决策延迟比基于表格的分析快8300倍，后续决策延迟仅358ms。

Conclusion: 该框架通过LLM语义特征提取和环境模型实现了零样本部署，无需特定工作负载的分析样本，在动态嵌入式系统中具有实际部署价值，收敛速度比无模型方法快20倍。
...
Method: 提出HS2C框架，基于图同质性进行结构和语义压缩。结构上，通过结构熵最小化原则进行全局层次划分，识别自然凝聚的同质性社区，丢弃随机连接噪声。语义上，将检测到的结构同质性传递给LLM，使其能够基于预定义的社区类型进行差异化的语义聚合，将冗余背景上下文压缩为简洁的社区级共识。

Result: 在10个节点级基准测试中，HS2C通过向LLMs提供结构和语义压缩的输入，同时提升了压缩率和下游推理准确率。在7个不同的图级基准测试中进一步验证了HS2C的任务泛化能力。

Conclusion: HS2C框架通过利用图同质性进行结构和语义压缩，有效提升了LLMs在图理解任务中的性能，验证了其优越性和可扩展性。
...
Method: 使用世界冠军AI进行自博弈竞赛，通过统计分析揭示游戏平衡问题，包括先手优势和子目标计分设置的问题。

Result: 研究发现存在明显的先手优势，子目标计分设置存在问题。基于这些发现，提出了引入先手优势补偿分机制、针对不同牌型优化子目标计分的规则调整方案。

Conclusion: 相比传统通过多轮轮换位置来平衡先手优势的方法，提出的每局补偿分机制更适合在线玩家。研究实现了修订后的在线麻将游戏，这是利用AI系统数据评估国际麻将游戏平衡性并开发更适合在线玩家的修订版本的初步尝试。
...
TL;DR: SANC(E3)是一个理论框架，提出表征单元不是预先给定的，而是在有限激活容量下通过竞争选择、重建和压缩的稳定结果自发形成的，统一了感知、想象、预测、规划和行动。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统预设固定的基本单元（如token、像素等），回避了表征单元如何自发形成和稳定的核心问题。本文旨在提出一个理论框架，解释智能系统如何从经验中自组织出表征单元。
...
Conclusion: SANC(E3)为理解智能如何从有限资源约束下的经验自组织中涌现提供了理论基础，将认知功能统一为单一的能量最小化过程，超越了预设表征单元的现有方法。
...
TL;DR: MPCI-Bench是首个多模态成对情境完整性基准，用于评估智能体隐私行为，包含配对的正负实例，覆盖三个层级：规范性种子判断、情境丰富的故事推理和可执行的智能体行动轨迹。
...
Method: 提出MPCI-Bench基准，包含从同一视觉源衍生的配对正负实例，分为三个层级：规范性种子判断、情境丰富的故事推理和可执行的智能体行动轨迹。通过三原则迭代精炼流程确保数据质量。

Result: 对最先进多模态模型的评估显示：1）系统性地无法平衡隐私与效用；2）存在明显的模态泄漏差距，敏感视觉信息比文本信息泄漏更频繁。

Conclusion: MPCI-Bench填补了多模态情境完整性评估的空白，揭示了当前模型在隐私保护方面的系统性缺陷，特别是视觉信息泄漏问题，为未来智能体情境完整性研究提供了重要基准。
...
TL;DR: 论文提出从传统的手工设计数值奖励函数转向基于语言的目标规范，利用大语言模型从自然语言描述合成奖励函数，并探讨了在多智能体强化学习中通过语义表示而非显式工程化数值信号实现协调的研究方向。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，奖励工程（手动设计奖励函数以引导期望行为）面临根本性挑战，包括信用分配模糊性、环境非平稳性以及交互复杂性的组合增长。传统的手工数值奖励设计方法难以应对这些挑战。

Method: 提出利用大语言模型实现语言介导的奖励规范，包括：1）从自然语言描述直接合成奖励函数（如EUREKA方法）；2）在线自适应奖励公式（如CARD方法）；3）基于可验证奖励的强化学习范式，将语言监督作为传统奖励工程的替代方案。

Result: 研究表明语言介导的监督可以作为传统奖励工程的可行替代方案，实现语义奖励规范、动态奖励适应以及与人类意图的更好对齐。但存在计算开销、对幻觉的鲁棒性以及扩展到大型多智能体系统的挑战。

Conclusion: 论文展望了一个研究方向：协调不是来自显式工程化的数值信号，而是来自共享的语义表示。这代表了从手工数值奖励到基于语言的目标规范的范式转变，为多智能体强化学习中的奖励工程提供了新的解决方案。
...
TL;DR: T3是一个诊断基准，用于评估LLM在Pearl因果阶梯上的因果判断能力，包含454个专家策划的情景，识别出两种病理：L1的"怀疑陷阱"和L3的非单调缩放悖论。


<details>
  <summary>Details</summary>
Motivation: 需要严格评估大型语言模型在因果判断方面的能力，特别是在Pearl因果阶梯的不同层次上，以诊断模型在因果推理中的系统性缺陷。

Method: 开发T3基准，包含454个专家策划的情景，将性能分解为效用（敏感性）、安全性（特异性）和明智拒绝三个维度，应用于前沿模型进行诊断分析。

Result: 发现两种病理：1）L1层的"怀疑陷阱"：安全调整的模型（如Claude Haiku）拒绝60%的有效链接；2）L3层的非单调缩放悖论：更大的GPT-5.2在模糊反事实上比GPT-4-Turbo差55分，主要由于过度犹豫而非幻觉。

Conclusion: T3基准能有效诊断LLM因果判断的病理，验证了过程验证协议（RCA）能恢复结构化验证下的决定性因果判断，为模型评估和改进提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现代智能体系统（如工具增强语言模型）面临巨大的动作空间（数千个API或检索操作），但经验表明只有少数动作对性能有实际影响。这促使研究者探索如何高效识别相关动作集。

Method: 采用上下文线性奖励模型，假设动作相关性遵循结构化稀疏性：只有少量动作在潜在状态下具有非零效应。将动作发现建模为块稀疏恢复问题，分析基于正交匹配追踪的贪婪算法。

Result: 在不相干性、信号强度和动作覆盖的标准假设下，证明贪婪算法能以高概率精确恢复相关动作集，所需样本数随稀疏度和潜在维度多项式增长，但仅随总动作数对数增长。同时提供参数估计误差保证。
...
TL;DR: OpenMic：基于AutoGen的多智能体系统，将用户提供的生活话题转化为3-5分钟的中文脱口秀表演并生成叙事喜剧视频


<details>
  <summary>Details</summary>
Motivation: 中文脱口秀生成需要文化背景的幽默、精确的节奏、舞台表演提示和隐式的多步推理。现有中文幽默数据集更适合幽默理解和评估，而非长篇脱口秀生成，导致直接监督与目标任务不匹配。

Method: 1. 构建基于AutoGen的端到端多智能体系统OpenMic；2. 采用多轮迭代循环规划协调多个专业智能体共同优化幽默、节奏和可表演性；3. 使用检索增强生成（RAG）进行素材基础和想法扩展；4. 微调专门的JokeWriter以更好地内化脱口秀特有的铺垫-笑点结构和长距离callback。

Result: OpenMic系统能够将用户提供的生活话题转化为完整的3-5分钟中文脱口秀表演，并进一步生成叙事喜剧视频。

Conclusion: OpenMic通过多智能体架构、RAG增强和专门的JokeWriter微调，解决了中文脱口秀生成中的文化背景、节奏控制和数据集不匹配等挑战，实现了从话题到完整表演的端到端生成。
...
### [156] [AtomMem : Learnable Dynamic Agentic Memory with Atomic Memory Operation](https://arxiv.org/abs/2601.08323)
*Yupeng Huo,Yaxi Lu,Zhong Zhang,Haotian Chen,Yankai Lin*

Main category: cs.AI

TL;DR: AtomMem将记忆管理重构为动态决策问题，通过原子CRUD操作将记忆流程转化为可学习的决策过程，在长上下文基准测试中优于静态工作流方法。
...
Method: 将高层记忆过程解构为基本的原子CRUD操作，将记忆工作流转化为可学习的决策过程，结合监督微调和强化学习训练自主的、任务对齐的记忆管理策略。

Result: 在3个长上下文基准测试中，训练后的AtomMem-8B模型持续优于先前的静态工作流记忆方法，分析显示学习型框架能让智能体发现结构化、任务对齐的记忆管理策略。

Conclusion: AtomMem通过将记忆管理重构为动态决策问题，实现了优于静态工作流方法的性能，展示了学习型记忆框架在发现任务对齐策略方面的优势。
...
### [157] [Semantic Laundering in AI Agent Architectures: Why Tool Boundaries Do Not Confer Epistemic Warrant](https://arxiv.org/abs/2601.08333)
*Oleg Romanchuk,Roman Bondar*

Main category: cs.AI

TL;DR: 论文指出LLM智能体架构将信息传输机制与认知验证机制混为一谈，导致"语义洗钱"问题，即缺乏充分依据的命题通过可信接口被系统接受，这是盖梯尔问题的架构实现。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体架构存在系统性缺陷，将信息传输机制与认知验证机制混淆，导致命题在没有充分依据的情况下被系统接受，形成"语义洗钱"问题。

Method: 通过形式化分析将这种架构缺陷定义为"语义洗钱"，并证明这是盖梯尔问题的架构实现。提出"不可避免的自许可定理"和"依据侵蚀原理"作为理论基础。

Result: 证明了在标准架构假设下，循环认知验证无法被消除。扩展、模型改进和LLM作为评判方案在结构上都无法解决这个类型层面的问题。

Conclusion: LLM智能体架构存在根本性的认知验证缺陷，语义洗钱问题在现有架构下不可避免，需要重新思考智能体架构设计来解决这一类型层面的问题。
...
### [158] [Thematic Working Group 5 -- Artificial Intelligence (AI) literacy for teaching and learning: design and implementation](https://arxiv.org/abs/2601.08380)
...
Conclusion: 通过系统化的策略开发，能够有效提升教师的AI素养和教学能力，促进AI在教育领域的有效整合。
...
### [159] [A Qualitative Model to Reason about Object Rotations (QOR) applied to solve the Cube Comparison Test (CCT)](https://arxiv.org/abs/2601.08382)
*Zoe Falomir*

Main category: cs.AI

TL;DR: 本文提出了一个定性推理模型QOR，用于解决立方体比较测试问题，通过构建概念邻域图CNGRLO来关联旋转运动与立方体表面特征的位置和方向变化。


<details>
  <summary>Details</summary>
Motivation: Ekstrom等人（1976）提出的立方体比较测试（CCT）需要复杂的空间推理能力，传统方法难以有效处理。本文旨在开发一个定性推理模型来系统化地解决这类涉及物体旋转的空间推理问题。

Method: 构建了概念邻域图CNGRLO，将旋转运动与立方体表面特征的位置变化和方向变化关联起来。通过该图生成组合表，用于计算旋转推理中的推断。

Result: 成功开发了QOR模型，能够有效解决立方体比较测试问题。概念邻域图CNGRLO和组合表为物体旋转的定性推理提供了系统化的计算框架。

Conclusion: QOR模型为物体旋转的定性推理提供了一种有效方法，概念邻域图CNGRLO是该模型的核心创新，能够系统化处理涉及旋转的空间推理问题，为类似的空间认知任务提供了理论框架。
...
### [160] [Deconstructing Pre-training: Knowledge Attribution Analysis in MoE and Dense Models](https://arxiv.org/abs/2601.08383)
*Bo Wang,Junzhuo Li,Hong Chen,Yuanlin Chu,Yuxuan Fan,Xuming Hu*

Main category: cs.AI

TL;DR: MoE架构通过神经元级归因指标Gated-LPI揭示了稀疏模型与密集模型在预训练知识获取上的差异：MoE形成低熵骨干核心、早期稳定化、功能鲁棒性强的分布式知识存储。


<details>
  <summary>Details</summary>
Motivation: 研究MoE架构在预训练过程中如何影响知识获取，以及这种过程与密集架构有何不同，目前尚不清楚。MoE虽然解耦了模型容量和每token计算量，但其知识获取机制仍不明确。

Method: 引入Gated-LPI（对数概率增加）神经元级归因指标，分解跨神经元的对数概率增加。对MoE和密集架构进行时间分辨的知识获取动态比较，跟踪1.2M训练步骤（约5.0T tokens）的MoE模型和600K训练步骤（约2.5T tokens）的密集模型。

Result: 发现三个关键模式：1）低熵骨干：MoE中约1%的神经元捕获超过45%的正向更新，形成高效用核心；2）早期稳定：MoE在<100K步骤内锁定稳定重要性分布，而密集模型在整个训练中保持波动；3）功能鲁棒性：屏蔽最重要的MoE注意力头仅使关系HIT@10下降<10%，而密集模型下降>50%，表明稀疏性促进分布式而非脆弱的存储。

Conclusion: 稀疏性从训练早期就培养出内在稳定和分布式的计算骨干，有助于弥合稀疏架构与训练时可解释性之间的差距。MoE展现出与密集模型不同的知识获取动态，具有更强的稳定性和鲁棒性。
...
### [161] [Creativity in AI as Emergence from Domain-Limited Generative Models](https://arxiv.org/abs/2601.08388)
*Corina Chutaux*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI创造力研究主要关注评估生成输出的新颖性、多样性或实用性，将创造力视为待评估的属性而非待建模的现象。随着大规模多模态生成系统展现复杂模式重组能力，需要从生成角度理解机器创造力的本质和限制。

Method: 提出生成视角的AI创造力框架，将其分解为四个相互作用组件：基于模式的生成、诱导的世界模型、上下文基础、任意性。分析这些组件在多模态生成系统中的表现，关注生成动态与领域特定表示之间的相互作用。

Result: 建立了一个技术框架，将创造力视为AI系统中的涌现现象，而非事后评估标签。通过分解创造力组件，为研究生成模型在受限环境中的创造性行为提供了结构性和情境性条件分析。

Conclusion: 创造力应被理解为生成动态与领域特定表示相互作用的涌现属性。该框架为将创造力作为AI系统内在现象进行研究提供了理论基础，超越了传统的评估导向方法。
...
### [162] [Owen-Shapley Policy Optimization (OSPO): A Principled RL Algorithm for Generative Search LLMs](https://arxiv.org/abs/2601.08403)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过强化学习进行个性化推荐时，标准方法依赖稀疏的序列级奖励，导致信用分配差距，难以确定哪些token驱动成功，特别是在需要从非指定语言推断潜在用户意图时

Method: 提出Owen-Shapley策略优化框架，基于token对结果的边际贡献重新分配序列级优势，使用基于势的奖励塑造和Shapley-Owen归因来分配片段级信用，同时保持最优策略，无需参数化价值模型

Result: 在Amazon ESCI和H&M Fashion数据集上的实验显示，相比基线方法获得了一致的性能提升，并且在测试时对训练期间未见过的分布外检索器表现出显著的鲁棒性

Conclusion: OSPO框架有效解决了个性化推荐中稀疏奖励的信用分配问题，通过语义连贯单元的联盟识别驱动性能的响应部分，无需额外计算开销
...
### [163] [Hybrid Distillation with CoT Guidance for Edge-Drone Control Code Generation](https://arxiv.org/abs/2601.08412)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成方面潜力巨大，但高资源消耗与无人机平台的实时轻量需求存在矛盾。需要解决如何在资源受限的无人机上实现智能控制的问题。

Method: 1) 构建包含多种主流无人机SDK的高质量数据集，包含指令-代码-推理链和反事实负样本；2) 使用DeepSeek-Coder-V2-Lite作为教师模型，采用混合黑盒白盒蒸馏策略生成思维链软标签；3) 结合硬标签的加权交叉熵损失将复杂推理能力迁移到学生模型；4) 针对无人机控制场景进行提示调优工程。

Result: 蒸馏后的轻量模型在保持高代码生成准确率的同时，在部署和推理效率方面显著提升，有效证明了该方法在实现无人机精准轻量智能控制方面的可行性和优越性。

Conclusion: 该方法成功解决了大型模型资源消耗与无人机轻量需求之间的矛盾，通过知识蒸馏等技术实现了复杂推理能力向小型模型的高效迁移，为资源受限平台的智能控制提供了有效解决方案。
...
### [164] [RubricHub: A Comprehensive and Highly Discriminative Rubric Dataset via Automated Coarse-to-Fine Generation](https://arxiv.org/abs/2601.08430)
*Sunzhu Li,Jiale Zhao,Miteto Wei,Huimin Ren,Yang Zhou,Jingwen Yang,Shunyu Liu,Kaike Zhang,Wei Chen*

Main category: cs.AI

TL;DR: 提出RubricHub框架，通过从粗到细的评分标准生成解决开放生成任务中缺乏真实标签的问题，在多个领域实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 在开放生成任务中，由于缺乏真实标签，基于可验证奖励的强化学习面临监督天花板效应。现有基于评分标准的方法存在可扩展性瓶颈和标准粗糙的问题，无法捕捉细微差异。

Method: 提出自动化从粗到细评分标准生成框架，结合原则引导合成、多模型聚合和难度演化，生成全面且具有高度区分度的评分标准。基于此构建RubricHub大规模多领域数据集，采用两阶段后训练流程：基于评分标准的拒绝采样微调和强化学习。

Result: RubricHub包含约11万个样本，覆盖多个领域。后训练的Qwen3-14B模型在HealthBench上达到69.3分，超越GPT-5等前沿专有模型，实现SOTA性能。

Conclusion: RubricHub框架通过自动化生成细粒度评分标准，有效解决了开放生成任务中的监督瓶颈，为语言模型在复杂推理任务中的性能提升提供了新途径。
...
### [165] [YaPO: Learnable Sparse Activation Steering Vectors for Domain Adaptation](https://arxiv.org/abs/2601.08441)
*Abdelaziz Bounhar,Rania Hossam Elmohamady Elbadry,Hadi Abdine,Preslav Nakov,Michalis Vazirgiannis,Guokan Shang*

Main category: cs.AI

TL;DR: YaPO是一种无需参考模型的方法，通过稀疏自编码器学习稀疏导向向量，实现更解耦、可解释且高效的LLM控制，在文化对齐等细粒度任务上优于密集导向方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活干预的密集导向向量方法存在神经元多语义性导致的潜在因素纠缠问题，限制了在文化对齐等细粒度场景下的效果和稳定性，需要区分紧密相关的价值观和行为。

Method: 提出YaPO（Yet another Policy Optimization），一种无需参考模型的方法，在稀疏自编码器的潜在空间中学习稀疏导向向量，通过优化稀疏编码实现解耦的导向方向。
...
### [166] [Beyond Linearization: Attributed Table Graphs for Table Reasoning](https://arxiv.org/abs/2601.08444)
*Yuxiang Wang,Junhao Gan,Shengxiang Gao,Shenghao Ye,Zhengyi Yang,Jianzhong Qi*

Main category: cs.AI

TL;DR: TABGR提出了一种基于图结构的表格推理方法，通过构建属性表格图(ATG)来保留表格结构，并采用问题引导的个性化PageRank机制缓解"中间丢失"问题，在基准测试中准确率提升高达9.7%。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的表格推理方法存在三个关键问题：1）线性化处理丢失了表格结构信息；2）缺乏显式推理路径导致结果可解释性差；3）存在"中间丢失"问题（模型难以处理长序列中间部分的信息）。这些限制影响了表格推理的准确性和可解释性。

Method: 提出TABGR模型，包含两个核心组件：1）属性表格图(ATG)：将表格表示为图结构，节点包括行、列、单元格，边表示它们之间的关系，显式保留表格结构；2）问题引导的个性化PageRank(QG-PPR)：基于问题语义对表格数据进行重排序，缓解"中间丢失"问题，使模型更关注相关信息。

Result: 在两个常用基准测试上的实验表明，TABGR在准确率上持续优于现有最先进模型，最高提升达9.7%。该方法不仅提高了推理准确性，还通过图结构提供了可解释的推理路径。

Conclusion: TABGR通过图结构表示和问题引导的重排序机制，有效解决了传统表格线性化方法的局限性，在保持表格结构的同时提供了可解释的推理路径，显著提升了表格推理的性能。
...
### [167] [An Under-Explored Application for Explainable Multimodal Misogyny Detection in code-mixed Hindi-English](https://arxiv.org/abs/2601.08457)
...
Method: 开发多模态可解释Web应用：1) 文本检测使用XLM-RoBERTa和mBERT模型，训练于4,193条混合语言评论；2) 表情包检测使用mBERT+EfficientNet和mBERT+ResNet，训练于4,218个表情包；3) 采用SHAP和LIME技术提供特征重要性解释；4) 通过CUQ和UEQ问卷进行用户体验评估。

Result: 构建了一个完整的Web应用系统，支持文本和表情包中的厌女内容检测，并提供可解释性分析。通过人类评估者使用CUQ和UEQ问卷评估了系统可用性。

Conclusion: 该系统为研究人员和内容审核员提供了一个实用工具，有助于推动该领域研究、打击基于性别的数字暴力，并确保安全的数字空间。
...
### [168] [SUMMPILOT: Bridging Efficiency and Customization for Interactive Summarization System](https://arxiv.org/abs/2601.08475)
*JungMin Yun,Juhwan Choi,Kyohoon Jin,Soojin Jang,Jinhee Jang,YoungBin Kim*

Main category: cs.AI

TL;DR: SummPilot是一个基于交互的可定制摘要系统，结合自动摘要效率与个性化需求，通过大语言模型实现自动和交互式摘要，支持语义图、实体聚类等交互组件。


<details>
  <summary>Details</summary>
Motivation: 解决自动摘要效率与个性化需求之间的矛盾，为用户提供能够根据个人兴趣和需求定制的摘要生成系统。

Method: 开发SummPilot系统，利用大语言模型实现自动和交互式摘要功能，包含语义图、实体聚类和可解释性评估等交互组件，让用户通过交互理解文档内容并个性化摘要。

Result: 演示和用户研究表明SummPilot在可定制摘要方面具有适应性和实用性，能够有效满足用户个性化需求。

Conclusion: SummPilot成功结合了自动摘要效率与个性化定制，通过交互式组件为用户提供了灵活、可定制的摘要生成解决方案。
...
### [169] [What If TSF: A Benchmark for Reframing Forecasting as Scenario-Guided Multimodal Forecasting](https://arxiv.org/abs/2601.08509)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测方法大多是单模态的，主要依赖历史模式外推。虽然大语言模型在多模态预测方面显示出潜力，但现有基准主要提供回顾性或未对齐的原始上下文，无法确定模型是否真正利用了文本输入。实践中，人类专家会结合历史证据和假设场景，在不同场景下对相同观测数据产生不同的预测。

Method: 提出了What If TSF (WIT)基准，通过提供专家构建的合理或反事实场景，为场景引导的多模态预测提供严格的测试平台。该基准旨在评估模型能否基于上下文文本（特别是未来场景）进行条件预测。

Result: 开发了WIT基准测试平台，可用于评估多模态时间序列预测模型在场景引导下的性能。该基准已在GitHub上开源。

Conclusion: WIT基准填补了现有多模态预测评估的空白，通过提供专家构建的场景，能够更准确地评估模型是否真正利用了文本上下文进行预测，特别是在不同假设场景下的预测能力。
...
### [170] [Sketch-Based Facade Renovation With Generative AI: A Streamlined Framework for Bypassing As-Built Modelling in Industrial Adaptive Reuse](https://arxiv.org/abs/2601.08531)
*Warissara Booranamaitree,Xusheng Du,Yushu Cai,Zhengyang Wang,Ye Zhang,Haoran Xie*

Main category: cs.AI

TL;DR: 提出结合生成式AI和视觉语言模型的三阶段框架，直接从粗略结构草图和文本描述生成建筑立面改造方案，无需详细现有建模。


<details>
  <summary>Details</summary>
Motivation: 立面改造比完全拆除更可持续，但现有工作流程需要详细的现有建模，耗时耗力且需要反复修改。需要一种能直接处理粗略草图和文本描述的方法来快速生成改造方案。

Method: 三阶段框架：1) 微调VLM模型预测需要修改的区域和添加组件；2) 稳定扩散模型生成新元素详细草图，通过生成修复管道与原轮廓合并；3) 使用ControlNet将结果细化为逼真图像。

Result: 在数据集和实际工业建筑上的实验表明，该框架能生成保留原始结构同时提升立面细节质量的改造方案，有效绕过详细现有建模需求。

Conclusion: 该方法使建筑师能快速探索设计替代方案，迭代早期概念，更清晰地传达改造意图，为立面改造提供了高效的工作流程。
...
### [171] [Learner-Tailored Program Repair: A Solution Generator with Iterative Edit-Driven Retrieval Enhancement](https://arxiv.org/abs/2601.08545)
*Zhenlong Dai,Zhuoluo Zhao,Hengning Wang,Xiu Tang,Sai Wu,Chang Yao,Zhipeng Gao,Jingyuan Chen*

Main category: cs.AI

TL;DR: 该论文提出了一种面向学习者的程序修复框架LSG，通过两阶段方法：首先检索修复方案指导LLMs识别和修复bug，然后基于检索方案提供解释，并通过迭代检索优化提升编程辅导效果。


<details>
  <summary>Details</summary>
Motivation: 当前智能编程辅导系统大多只修复学习者的错误代码，而不提供bug的根本原因分析。为了解决这一不足，研究者提出了LPR（学习者定制程序修复）任务，旨在同时修复代码并提供bug描述。

Method: 提出LSG框架：第一阶段使用修复方案检索框架构建解决方案数据库，采用编辑驱动的代码检索方法获取有价值的解决方案，指导LLMs识别和修复bug；第二阶段提出解决方案引导的程序修复方法，在检索方案指导下修复代码并提供解释；此外提出迭代检索增强方法，利用生成代码的评估结果迭代优化检索方向。

Result: 实验结果表明，该方法在一系列基线模型上取得了显著优势，验证了LSG框架在新提出的LPR任务中的有效性。

Conclusion: LSG框架能够有效提升程序修复性能，同时为编程学习者提供bug描述，在智能编程辅导场景中具有实际应用价值。
...
### [172] [WaterCopilot: An AI-Driven Virtual Assistant for Water Management](https://arxiv.org/abs/2601.08559)
*Keerththanan Vickneswaran,Mariangel Garcia Andarcia,Hugo Retief,Chris Dickens,Paulo Silva*

Main category: cs.AI

TL;DR: WaterCopilot是一个AI驱动的虚拟助手，用于跨界河流流域的水资源管理，通过整合静态政策文件和实时水文数据，提供多语言交互、透明源引用和可视化功能。


<details>
  <summary>Details</summary>
Motivation: 跨界河流流域的水资源管理面临数据碎片化、实时访问有限以及整合不同信息源复杂性的挑战，需要统一的交互平台来弥合这些差距。

Method: 基于检索增强生成(RAG)和工具调用架构开发WaterCopilot，包含两个自定义插件：iwmi-doc-plugin用于文档语义搜索，iwmi-api-plugin用于查询实时数据库，提供环境流量警报、降雨趋势、水库水位等动态洞察。

Result: 使用RAGAS框架评估，WaterCopilot总体得分0.8043，答案相关性0.8571，上下文精确度0.8009。系统支持多语言交互、自动阈值警报，并与Limpopo河流域数字孪生集成。
...
### [173] [ViDoRe V3: A Comprehensive Evaluation of Retrieval Augmented Generation in Complex Real-World Scenarios](https://arxiv.org/abs/2601.08620)
...
TL;DR: ViDoRe v3是一个全面的多模态RAG基准测试，包含视觉丰富文档的多种查询类型，涵盖10个专业领域数据集，提供高质量的人工标注，用于评估RAG系统在视觉元素理解和跨文档信息合成方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG基准测试主要关注文本数据、单文档理解，或将检索和生成分开评估，无法捕捉真实场景中RAG系统面临的复杂性，如视觉元素解释、跨文档信息合成和准确来源追溯等挑战。

Method: 构建了一个包含10个数据集的多模态RAG基准测试，涵盖约26,000个文档页面和3,099个人工验证的查询，每个查询支持6种语言。通过12,000小时的人工标注工作，提供了检索相关性、边界框定位和验证参考答案的高质量标注。

Result: 评估显示：视觉检索器优于文本检索器；后期交互模型和文本重排序显著提升性能；混合或纯视觉上下文能提高答案生成质量。但现有模型在非文本元素处理、开放式查询和细粒度视觉定位方面仍有困难。

Conclusion: ViDoRe v3基准测试揭示了当前RAG系统在视觉丰富文档处理方面的能力与局限，为多模态RAG研究提供了重要评估工具，鼓励社区解决视觉元素理解和跨文档信息合成等挑战。
...
### [174] [Resisting Manipulative Bots in Memecoin Copy Trading: A Multi-Agent Approach with Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.08641)
...
<details>
  <summary>Details</summary>
Motivation: 模因币投资热潮中跟单交易流行，但存在操纵机器人、被跟随钱包未来表现不确定、交易执行延迟等问题。单一LLM难以处理复杂的资产分配任务，且在加密货币领域缺乏足够的领域知识。

Method: 受资产管理团队结构启发，将复杂任务分解为子任务，协调专业智能体协作解决。采用少样本思维链提示，使每个智能体获取专业模因币交易知识、解释多模态数据并生成可解释决策。

Result: 在1000个模因币项目交易数据集上的实证评估显示，该系统在识别高质量模因币项目和KOL钱包方面的精确度分别达到73%和70%，选出的KOL在这些项目中总共创造了50万美元利润。

Conclusion: 提出的可解释多智能体系统有效解决了模因币跟单交易的挑战，通过任务分解和专业智能体协作，在性能和可解释性方面优于现有方法。
...
### [175] [Prism: Towards Lowering User Cognitive Load in LLMs via Complex Intent Understanding](https://arxiv.org/abs/2601.08653)
*Zenghua Liao,Jinzhi Liao,Xiang Zhao*

Main category: cs.AI

TL;DR: Prism框架通过逻辑依赖建模实现复杂意图理解，显著提升LLM与用户协作效率，在逻辑一致性、用户满意度和任务完成时间方面取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过顺序或并行提问澄清用户意图，但未能解决核心挑战：建模澄清问题间的逻辑依赖关系。社交平台用户目标模糊且动态变化，需要更有效的复杂意图理解框架。

Method: 提出Prism框架，包含四个模块：复杂意图分解模块（分解意图并识别逻辑依赖）、逻辑澄清生成模块（基于依赖组织澄清问题）、意图感知奖励模块（通过奖励函数评估澄清轨迹并生成训练数据）、自进化意图调优模块（通过数据驱动反馈迭代优化）。

Result: Prism在澄清交互、意图执行和认知负载基准测试中持续优于现有方法，达到最先进的逻辑一致性，将逻辑冲突降至11.5%，用户满意度提升14.4%，任务完成时间减少34.8%。

Conclusion: Prism通过建模逻辑依赖关系实现了逻辑一致且高效的意图澄清，显著改进了LLM与用户在社交平台上的协作效果，为复杂意图理解提供了有效解决方案。
...
TL;DR: 为本科生设计的强化学习入门教程，通过实例驱动的方式降低学习门槛，帮助理解理论与代码实现之间的差距
...
Method: 采用清晰、实例驱动的解释方法，通过动手实践示例和易于理解的讲解，帮助学生建立从概念到实现的能力

Result: 该教程旨在为学生提供应用强化学习技术所需的基础技能，使他们在实际场景中能够自信地应用这些技术

Conclusion: 通过降低强化学习的学习门槛，该教程有助于培养更多学生掌握这一重要的人工智能技术，促进理论知识与实践应用的结合
...
TL;DR: 提出Pced框架，通过并行解码和检索感知对比解码规则，解决RAG中多文档推理与速度的权衡问题


<details>
  <summary>Details</summary>
Motivation: 检索增强生成面临权衡：长提示拼接支持多文档推理但造成预填充瓶颈，而单独编码文档KV缓存速度快但破坏了跨文档交互

Method: 提出并行专家上下文解码(Pced)，将检索文档视为独立"专家"，通过检索感知对比解码规则同步它们的预测，将证据聚合从注意力机制转移到解码过程

Result: 该方法无需构建跨文档共享注意力，就能恢复跨文档推理能力

Conclusion: Pced框架提供了训练自由的解决方案，有效平衡了RAG中的多文档推理需求与计算效率
...
### [178] [PersonaDual: Balancing Personalization and Objectivity via Adaptive Reasoning](https://arxiv.org/abs/2601.08679)
...
Method: 提出PersonaDual框架，首先通过SFT训练学习两种推理模式，然后通过强化学习（提出的DualGRPO）进一步优化模式选择，实现基于上下文的自适应切换
...
### [179] [All Required, In Order: Phase-Level Evaluation for AI-Human Dialogue in Healthcare and Beyond](https://arxiv.org/abs/2601.08690)
*Shubham Kulkarni,Alexander Lyzhov,Shiva Chaitanya,Preetam Joshi*

Main category: cs.AI

TL;DR: 提出OIP-SCE评估方法，通过检查临床对话中所有必需义务是否按正确顺序满足并提供清晰证据，解决现有评估方法忽视对话全程合规性的问题


<details>
  <summary>Details</summary>
Motivation: 现有对话AI评估方法大多忽视了合规性依赖于完整对话过程这一关键点，需要一种能够检查临床义务是否按正确顺序满足并提供可审计证据的评估框架

Method: 提出Obligatory-Information Phase Structured Compliance Evaluation (OIP-SCE)方法，该方法检查每个必需的临床义务是否在正确阶段被满足，并提供清晰的证据供临床医生审查

Result: 通过两个案例研究（呼吸病史采集和福利验证）展示了OIP-SCE的有效性，证明阶段级证据能够将政策转化为可共享、可操作的步骤

Conclusion: OIP-SCE为临床医生提供了可控的检查机制，为工程师提供了清晰的实现规范，创建了单一可审计的评估界面，使AI能力与临床工作流程对齐，支持常规安全使用
...
### [180] [Evaluating the Ability of Explanations to Disambiguate Models in a Rashomon Set](https://arxiv.org/abs/2601.08703)
...
<details>
  <summary>Details</summary>
Motivation: 当前可解释人工智能(XAI)面临两个主要问题：1) 对于性能相似的Rashomon模型集合，解释需要能够区分不同模型的行为以帮助选择部署模型；2) 解释本身因解释器不同而变化，需要评估方法。现有评估方法依赖与理想真实解释的比较，这掩盖了Rashomon集合内的行为差异。

Method: 提出了三个解释评估原则，并开发了新的评估方法AXE。AXE不需要真实解释作为基准，而是基于特定原则评估解释质量。该方法能够检测对抗性公平清洗，即从Rashomon集合中选择替代模型，这些模型保持相同预测但误导解释器生成虚假解释。

Result: AXE能够以100%的成功率检测对抗性公平清洗行为。与基于模型敏感性或真实解释比较的先前评估策略不同，AXE能够确定何时使用受保护属性进行预测。评估指标如果依赖与理想真实解释的比较，会掩盖Rashomon集合内的行为差异。

Conclusion: 提出的解释评估原则和AXE方法能够突出Rashomon集合中模型的行为差异，帮助从集合中选择模型。该方法能够有效检测对抗性公平清洗，确保解释评估的可靠性，特别是在涉及受保护属性的情况下。
...
### [181] [Learning from Demonstrations via Capability-Aware Goal Sampling](https://arxiv.org/abs/2601.08731)
*Yuanlin Duan,Yuning Wang,Wenjie Qiu,He Zhu*

Main category: cs.AI

TL;DR: Cago是一种基于能力感知的目标采样方法，通过动态跟踪智能体在专家轨迹上的能力，选择刚好超出当前能力范围的中介目标来引导学习，解决模仿学习在长时程环境中的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在长时程环境中存在严重问题：完美复制演示不现实，小错误会累积导致灾难性失败。现有方法要么只使用演示进行策略初始化，要么用于奖励塑造，无法有效处理智能体能力与专家轨迹之间的差距。

Method: Cago方法动态跟踪智能体在专家轨迹上的能力水平，基于此信号选择"刚好超出智能体当前能力范围"的中介目标（目标采样）。这形成了一个自适应课程，逐步引导智能体向完整任务解决方案前进，而不是直接模仿专家轨迹。

Result: 实验结果表明，Cago在多种稀疏奖励、目标条件任务中显著提高了样本效率和最终性能，一致优于现有的从演示学习方法基线。

Conclusion: Cago通过能力感知的目标采样方法，有效缓解了模仿学习对专家轨迹的脆弱依赖，实现了更稳健的学习过程，特别适用于长时程、稀疏奖励的环境。
...
### [182] [AI as Entertainment](https://arxiv.org/abs/2601.08768)
*Cody Kommers,Ari Holtzman*

Main category: cs.AI

TL;DR: 该论文指出AI系统主要被设计和评估为提升生产力的智能工具，但娱乐正成为AI的重要应用场景，而AI领域缺乏评估娱乐内容社会影响的框架，提出了"厚重娱乐"作为评估AI生成文化内容的新框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统主要被定位为提升生产力的智能工具，但数据显示AI已广泛用于娱乐目的，尤其是年轻人群体。AI公司为收回巨额基础设施投资，娱乐可能成为其主要商业模式，而现有AI评估框架主要关注文化危害，缺乏评估文化内容积极价值的体系。

Method: 通过分析当前AI评估实践，识别出评估框架的不对称性：主要关注智能的益处和危害，但文化内容评估几乎只关注危害。借鉴人文学科见解，提出"厚重娱乐"框架，该框架考虑娱乐在意义建构、身份形成和社会连接中的作用，而非仅仅最小化危害。

Result: 研究发现AI已广泛用于娱乐目的，特别是年轻人群体，这代表了巨大的潜在收入来源。娱乐可能成为主要AI公司的主要商业模式，这将深刻影响未来技术发展方向。当前评估实践存在关键不对称性，缺乏评估文化内容积极价值的框架。

Conclusion: AI领域需要新的评估框架来应对娱乐AI的社会影响。提出的"厚重娱乐"框架能够更全面地评估AI生成文化内容的价值，考虑其在意义建构、身份形成和社会连接中的作用。长期来看，AI可能像社交媒体一样，其核心价值不在于"智能"本身，而在于其社会文化功能。
...
### [183] [Uncovering Political Bias in Large Language Models using Parliamentary Voting Records](https://arxiv.org/abs/2601.08785)
*Jieying Chen,Karen de Jong,Andreas Poole,Jan Burakowski,Elena Elderson Nosti,Joep Windt,Chendi Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于议会投票记录构建政治偏见基准的通用方法，并在荷兰、挪威、西班牙三个国家案例中应用，发现最先进的LLMs普遍表现出左倾或中间倾向，并对右翼保守政党存在明显负面偏见。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在数字平台和决策系统中的深度应用，对其政治偏见的担忧日益增长。尽管已有大量研究关注性别和种族等社会偏见，但对政治偏见的系统性研究仍然有限，尽管其具有直接的社会影响。

Method: 提出了一种通用方法，通过将模型生成的投票预测与经过验证的议会投票记录对齐来构建政治偏见基准。在三个国家案例中实例化该方法：PoliBiasNL（荷兰）、PoliBiasNO（挪威）和PoliBiasES（西班牙）。还提出了一种方法，通过将基于投票的位置与CHES维度关联，在共享的二维CHES空间中可视化LLMs和政治政党的意识形态。
...
### [184] [Application of Ideal Observer for Thresholded Data in Search Task](https://arxiv.org/abs/2601.07976)
*Hongwei Lin,Howard C. Gifford*

Main category: eess.IV

TL;DR: 提出了一种基于阈值视觉搜索的拟人化模型观察者，通过选择性处理高显著性特征来提高图像质量评估性能，在噪声环境中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一种更符合人类视觉系统的任务型图像质量评估方法，通过选择性处理高显著性特征来过滤无关变异，提高诊断准确性和计算效率。

Method: 采用两阶段框架：候选区域选择和决策制定。在候选选择阶段使用阈值化数据细化感兴趣区域，阶段特异性特征处理优化性能。通过模拟评估阈值化对特征图、候选定位和多特征场景的影响。

Result: 阈值化通过排除低显著性特征显著提高了观察者性能，特别是在噪声环境中。中间阈值通常优于无阈值处理，表明仅保留相关特征比保留所有特征更有效。模型能用更少图像进行有效训练，同时保持与人类性能的一致性。

Conclusion: 该新颖框架能预测临床现实任务中的人类视觉搜索性能，并为资源有限的模型观察者训练提供解决方案。该方法在计算机视觉、机器学习、国防安全图像分析等需要模拟人类视觉搜索和检测任务的领域具有应用潜力。
...
### [185] [A Single-Parameter Factor-Graph Image Prior](https://arxiv.org/abs/2601.08749)
...
<details>
  <summary>Details</summary>
Motivation: 传统图像模型通常使用固定参数，无法适应不同图像区域的特征变化。本文旨在开发一种能够自动适应图像局部特性的分段平滑模型，以提高图像处理任务的效果。

Method: 采用因子图框架，引入NUP（参数未知的正态分布）先验，构建分段平滑图像模型。算法实现结合共轭梯度迭代和高斯消息传递，使局部参数能够自适应调整。

Result: 提出的模型和算法在图像去噪和对比度增强任务中表现出良好性能，能够有效保留图像细节同时去除噪声，并增强图像对比度。

Conclusion: 基于因子图和NUP先验的分段平滑图像模型具有自适应局部参数的能力，通过共轭梯度和高斯消息传递的迭代计算，在图像处理应用中取得了令人满意的结果。
...
### [186] [M3CoTBench: Benchmark Chain-of-Thought of MLLMs in Medical Image Understanding](https://arxiv.org/abs/2601.08758)
...
<details>
  <summary>Details</summary>
Motivation: 当前医疗图像理解基准通常只关注最终答案而忽略推理路径，这使得判断过程不透明，难以辅助医生诊断。医疗领域诊断决策依赖于细微的视觉线索和顺序推理，思维链推理与临床思维过程自然契合，但缺乏专门的评估基准。

Method: 1) 构建多样化、多难度级别的数据集，涵盖24种检查类型；2) 设计13个不同难度的任务；3) 开发专门针对临床推理的思维链评估指标（正确性、效率、影响、一致性）；4) 对多个多模态大语言模型进行性能分析。

Result: M3CoTBench系统性地评估了多种医疗图像任务中的思维链推理，揭示了当前多模态大语言模型在生成可靠且临床可解释的推理方面存在的局限性。

Conclusion: 该基准旨在促进开发透明、可信且诊断准确的医疗AI系统，填补了医疗图像理解中思维链推理评估的空白，为未来研究提供了系统化的评估框架。
