...
TL;DR: 本文评估了500M和2.2B参数的SmolVLM2变体在盲人和低视力用户可访问性描述质量方面的表现，引入了两个新的评估框架，并系统评估了不同提示策略和移动设备部署性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在生成视频描述方面表现出色，但其高内存、计算和部署需求限制了实际应用，特别是对于依赖详细、上下文感知描述的盲人和低视力用户。

Method: 使用500M和2.2B参数的SmolVLM2变体在AVCaps（户外）和Charades（室内）数据集上进行评估；引入多上下文BLV框架和导航辅助框架；系统评估四种不同提示设计策略；在智能手机上部署模型，评估FP32和INT8精度变体。

Result: 评估了模型大小对可访问性描述质量的影响，开发了专门针对BLV可访问性评估的新框架，测试了不同提示策略的有效性，并评估了移动设备上的实际性能限制。

Conclusion: 通过系统评估不同规模的视觉语言模型，为盲人和低视力用户开发了专门的可访问性评估框架，并验证了在资源受限移动设备上的部署可行性。
...
Method: LUA在最终VAE解码步骤之前对潜在代码执行超分辨率，作为即插即用组件集成，无需修改基础模型或添加额外扩散阶段，通过潜在空间中的单次前向传递实现高分辨率合成。

Result: LUA在感知质量上与像素空间超分辨率相当，但解码和上采样时间降低了近3倍（1024像素生成仅增加0.42秒，而相同架构的像素空间SR需要1.87秒），且在不同VAE的潜在空间中表现出强泛化能力。
...
TL;DR: 提出自一致性采样（SCS）方法，通过视觉扰动和轨迹截断重采样来解决多模态大语言模型中结果奖励强化学习存在的不可靠轨迹问题，在多个基准测试上显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的结果奖励强化学习在多选题设置中存在一个被忽视的问题：通过错误推理链猜测出正确答案的不可靠轨迹与真实推理获得相同奖励，这影响了学习效果。

Method: 提出自一致性采样（SCS）：对每个问题引入小视觉扰动，并对初始轨迹进行重复截断和重采样，通过结果轨迹的一致性得分来在策略更新时降低不可靠轨迹的权重。
...
TL;DR: 本文介绍了编码器增强的因果解码器模型架构，该架构在训练效率上优于因果变换器，并能实现更高的压缩率。研究表明，通过估计每个token的熵并训练模型接近但不超越该熵值，可以提升模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于因果（下一个token预测）的大语言模型虽然是最有效的语言压缩算法，但用它们来准确估计语言熵在计算上不可行。本文旨在开发更高效的模型架构来估计语言熵，并探索熵对模型泛化能力的影响。

Method: 引入编码器增强的因果解码器模型架构，该架构在训练效率上优于因果变换器。通过估计每个token的熵，训练模型使其接近但不超越估计的熵值。

Result: 实验证明，编码器增强的因果解码器模型在压缩率上优于因果变换器，即使使用有限的硬件资源也能实现。此外，训练模型接近但不超越估计的熵值能够显著提升模型的泛化能力。

Conclusion: 语言预测受到语言固有信息熵的限制，存在语言模型准确性的上限和语言压缩的下界。通过考虑熵约束训练模型，可以提升泛化能力，这为语言模型的优化提供了新的方向。
...
TL;DR: 提出了Socratic Self-Refine (SSR)框架，通过将模型响应分解为可验证的(子问题，子答案)对，进行细粒度评估和精确优化，提升LLM在复杂任务上的推理能力。
...
Method: SSR框架将模型响应分解为可验证的(子问题，子答案)对，通过受控重解和自一致性检查进行步骤级置信度估计，精确定位不可靠步骤并迭代优化。

Result: 在五个推理基准测试和三个LLM上的实证结果显示，SSR持续优于最先进的迭代自优化基线方法。

Conclusion: SSR不仅带来性能提升，还为评估和理解LLM内部推理过程提供了一种原则性的黑盒方法。
...
Method: 使用AMD Instinct MI300X GPU进行大规模预训练、通用指令微调以及与人类偏好的对齐。虽然预训练token数量较少，但通过专门训练方法开发了Instella-Long（支持128K上下文）和Instella-Math（数学推理增强）两个变体。

Result: Instella在完全开源模型中达到最先进水平，与同规模领先的开源权重模型具有竞争力。两个专业变体分别实现了长上下文处理和数学推理能力的提升。
...
TL;DR: 本文提出了生成对抗蒸馏（GAD）方法，用于黑盒知识蒸馏，通过将学生LLM作为生成器，训练判别器区分学生和教师模型的输出，形成极小极大博弈。


<details>
  <summary>Details</summary>
Motivation: 黑盒蒸馏仅能从专有教师模型的文本输出中学习，无法访问其内部logits或参数，需要更有效的蒸馏方法。

Method: GAD将学生LLM作为生成器，训练判别器区分学生和教师模型的响应，判别器作为策略奖励模型与学生共同进化，提供稳定、自适应的反馈。

Result: 实验结果表明GAD持续优于常用的序列级知识蒸馏方法，Qwen2.5-14B-Instruct学生模型在LMSYS-Chat自动评估中与教师模型GPT-5-Chat表现相当。
...
TL;DR: ParoQuant是一种仅对权重进行后训练量化的方法，通过成对旋转量化和通道级缩放来平衡通道间幅度，减小量化组内动态范围，从而有效处理LLM中的异常值问题，在推理任务上比AWQ平均提升2.4%准确率且开销小于10%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)的权重和激活中存在异常值，导致量化误差大和精度严重下降，特别是在推理型LLM中误差会在长思维链中累积。现有PTQ方法要么无法充分抑制异常值，要么在推理时引入显著开销。

Method: 提出成对旋转量化(ParoQuant)，结合硬件高效的独立Givens旋转和通道级缩放，平衡通道间幅度并缩小每个量化组内的动态范围。同时协同设计推理内核以充分利用GPU并行性，保持旋转和缩放操作在运行时轻量。

Result: 在推理任务上，ParoQuant比AWQ平均提升2.4%准确率，同时保持小于10%的开销。

Conclusion: 该方法为推理型LLM的更高效和准确部署铺平了道路。
