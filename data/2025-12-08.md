...
TL;DR: AREA3D是一种主动3D重建方法，通过解耦视图不确定性建模与基础重建模型，结合视觉语言指导，实现高效、准确的场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有主动重建方法依赖手工设计的几何启发式规则，可能导致冗余观测且重建质量提升有限。需要更智能的视图选择策略来提高重建效率和质量。

Method: 提出AREA3D框架：1) 解耦视图不确定性建模与前馈3D重建模型，实现精确的不确定性估计而无需昂贵的在线优化；2) 集成视觉语言模型提供高层语义指导，鼓励超越纯几何线索的信息丰富且多样化的视点选择。
...
TL;DR: 论文提出"尺度锚定"问题：零样本超分辨率时空预测中，由于低分辨率数据的奈奎斯特频率限制，模型在高分辨率推理时无法处理未见的高频信号，导致误差被锚定在低分辨率水平。作者提出频率表示学习框架来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为在不同分辨率下保持相似误差即表示成功的多分辨率泛化，但作为数值求解器替代的深度学习模型应该在分辨率增加时减少误差。低分辨率数据受奈奎斯特频率限制，无法表示高频物理定律，导致模型在高分辨率推理时无法处理未见频率分量，误差被锚定在低分辨率水平。
...
Conclusion: 识别了"尺度锚定"这一新问题，并提出频率表示学习作为解决方案。该方法通过改善频率表示能力，使模型能够更好地泛化到更高分辨率，误差随分辨率提升而降低，实现了真正的多分辨率泛化。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量内容，但由于需要迭代采样，推理速度较慢。作者观察到在确定性采样中存在特征不变性，希望通过利用这种不变性来加速推理过程。

Method: 提出InvarDiff方法：1）通过少量确定性运行计算每个时间步、每个层、每个模块的二进制缓存计划矩阵；2）使用重采样校正避免连续缓存时的漂移；3）基于分位数的变化度量决定哪些模块在哪些步骤可以重用而非重新计算；4）在步骤尺度应用相同的不变性准则，实现跨时间步缓存；5）推理时按照矩阵指导进行步骤优先和分层缓存。
...
TL;DR: 开发了一种可解释的AI-ECG模型，用于预测冠状动脉CT血管造影中四支主要冠状动脉的严重或完全狭窄，在内外验证集上均显示出良好的预测性能。
...
Method: 开发了一种可解释的AI-ECG模型，用于预测四支主要冠状动脉（RCA、LM、LAD、LCX）在冠脉CTA上的严重或完全狭窄。模型在内部和外部验证集上进行评估，并在临床正常心电图亚组中测试稳健性，进行人口统计学和采集时间亚组分析，基于血管特异性发生率阈值进行风险分层，并进行可解释性分析。

Result: 内部验证集上，模型对RCA、LM、LAD、LCX的AUC分别为0.794、0.818、0.744、0.755；外部验证集上AUC分别为0.749、0.971、0.667、0.727。在临床正常心电图亚组中性能保持稳定，亚组分析进一步证实模型稳定性。风险分层在校准和累积事件曲线上显示一致分离。可解释性分析揭示了高低风险组间不同的波形差异。

Conclusion: 该可解释的AI-ECG模型能够有效预测冠状动脉严重狭窄，在内外验证集上均表现出良好性能，且对无明显心电图异常的患者也保持稳健。模型提供了冠状动脉狭窄的心电图相关新见解，为大规模CAD筛查提供了有前景的无创工具。
...
TL;DR: 本文提出了ChromouVQA基准测试，基于石原式色觉检查图构建大规模多任务视觉问答数据集，用于评估视觉语言模型在复杂背景下的目标识别能力。
...
Method: 扩展经典点阵图，采用多种填充几何形状，变化色度分离、密度、大小、遮挡和旋转，记录完整元数据以确保可重复性。基准包含九个视觉问答任务，包括识别、计数、比较和空间推理。

Result: 人类和视觉语言模型的评估显示存在较大差距，特别是在细微色度对比或破坏性几何填充条件下。同时提出了模型无关的对比学习方法，通过对齐轮廓与其伪装渲染来改善全局形状恢复。
...
TL;DR: 提出基于迁移学习的生成式降尺度框架，通过轻量级U-Net编码器预训练学习时空表征，再结合扩散模型重建高分辨率卫星图像，在亚洲区域验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了解决从粗分辨率卫星图像重建高分辨率图像的问题，特别是在有限训练数据的情况下，需要开发一种能够保持物理一致性的降尺度方法，以改进环境暴露评估和长期环境监测。

Method: 结合轻量级U-Net迁移编码器和基于扩散的生成模型。首先在粗分辨率数据上预训练U-Net学习时空表征，然后冻结编码器并将其作为物理有意义的潜在特征转移到更大的降尺度模型中。使用MERRA-2再分析数据（50km）作为低分辨率源域，GEOS-5 Nature Run（7km）作为高分辨率目标域。

Result: 模型在季节性区域划分中表现出色（R² = 0.65到0.94），优于确定性U-Net、变分自编码器和先前的迁移学习基线。通过半变异函数、ACF/PACF和基于滞后的RMSE/R²评估显示，预测的降尺度图像保持了物理一致的空间变异性和时间自相关性，能够在G5NR记录之外实现稳定的自回归重建。
...
<details>
  <summary>Details</summary>
Motivation: 地球观测数据具有固有的异质性，来自不同传感器、地理区域、采集时间和大气条件。训练域和部署域之间的分布偏移严重限制了预训练遥感模型的泛化能力，使得无监督域自适应在实际应用中变得至关重要。

Method: 提出FlowEO框架，利用流匹配学习从源域到目标域图像分布的语义保持映射。该方法处理遥感图像分类和语义分割中的挑战性域自适应配置，包括SAR到光学图像的转换以及自然灾害引起的时空和语义偏移。

Result: 在四个数据集上的广泛实验表明，FlowEO在域自适应方面优于现有的图像转换方法，同时达到相当或更好的感知图像质量。实验涵盖了SAR到光学转换、自然灾害引起的时空和语义偏移等多种适应场景。

Conclusion: FlowEO展示了基于流匹配的无监督域自适应在遥感领域的潜力，能够有效解决地球观测数据中的分布偏移问题，为大规模环境监测和分析提供了新的解决方案。
...
TL;DR: 提出无需人工标注、仅用自合成数据训练VLM评判器的框架，在多个评测基准上超越更大模型
...
Method: 三阶段迭代框架：1)生成多样化多模态指令-响应对；2)生成推理轨迹和评判，过滤不符合质量要求的样本；3)基于正确评判答案和推理轨迹进行训练

Result: Llama-3.2-11B评判器在VL-RewardBench上从0.38提升到0.51，在通用性、幻觉和推理维度表现突出，超越Llama-3.2-90B、GPT-4o和Claude 3.5 Sonnet等更大模型
...
TL;DR: 本文提出了一种用于细粒度图像生成的分层嵌入器概念，结合超类和子类语义信息解决语义纠缠问题，并引入超分辨率技术增强图像细节，同时提出高效的ProAttention机制。
...
TL;DR: 提出像素等效潜在合成(PELC)原则和DecFormer模型，实现扩散模型中与像素空间合成等效的潜在空间合成，解决传统线性插值在掩码边界产生的伪影和颜色偏移问题


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型中的潜在修复主要依赖在降采样掩码下的VAE潜在线性插值，这种方法无法实现像素等效的合成，导致掩码边界出现大伪影、全局退化以及颜色偏移

Method: 提出像素等效潜在合成(PELC)原则，要求潜在合成应与像素空间合成等效。开发DecFormer，一个770万参数的Transformer模型，预测每通道的混合权重和离流形残差校正，实现掩码一致的潜在融合

Result: DecFormer在FLUX.1系列模型上恢复了全局颜色一致性、软掩码支持、锐利边界和高保真掩码，边缘误差指标相比标准掩码插值降低达53%。作为修复先验，轻量级LoRA配合DecFormer达到与完全微调的FLUX.1-Fill模型相当的保真度

Conclusion: PELC原则为像素等效的潜在编辑提供了通用方案，DecFormer实现了与像素空间合成等效的潜在合成，显著提升了扩散模型中掩码合成的质量，同时保持了高效性
...
TL;DR: DEAR数据集是首个基于人类主观偏好的图像渲染美学评估基准，通过大规模众包收集了13,648名评估者对图像对的偏好评分，用于超越传统失真评估的美学风格研究。
...
Method: 基于MIT-Adobe FiveK数据集构建DEAR数据集，通过大规模众包收集人类对图像渲染风格的成对偏好评分，每个图像对由25名不同评估者评分，共13,648名评估者参与。

Result: 创建了包含细致、上下文敏感美学偏好的标注数据集，支持风格偏好预测、美学基准测试和个性化美学建模等应用，并在HuggingFace发布了100张图像的标记子集。

Conclusion: DEAR是首个系统性地基于人类主观偏好解决图像渲染美学评估的数据集，为超越传统失真评估的美学研究提供了新的基准和任务方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机固定速率捕获能耗高，事件相机虽功耗低但产生异步事件流而非RGB视频。需要一种既能降低捕获功耗又能为下游应用提供标准视频输出的解决方案。
...
Conclusion: 混合捕获范式（稀疏RGB关键帧+连续事件流）结合扩散模型方法能有效降低功耗同时重建高质量RGB视频，为监控、机器人和可穿戴系统提供实用的低功耗视频解决方案。
...
TL;DR: CARD是一种无需训练的扩散模型扩展，专门处理空间相关噪声的图像恢复问题，通过噪声白化技术提升真实传感器噪声下的恢复性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型大多假设噪声是独立同分布的高斯噪声，但真实世界传感器（如滚动快门传感器）会产生空间相关噪声，这限制了现有方法在实际应用中的有效性。
...
Conclusion: CARD通过显式处理相关高斯噪声，显著提升了扩散模型在真实世界图像恢复任务中的性能，同时提出的CIN-D数据集填补了相关噪声评估的关键空白。
...
Result: COM4D能够从单目视频中重建完整且持久的4D场景，包含多个交互物体，并在现有的4D物体和组合3D重建问题上达到最先进水平。
...
TL;DR: 本文提出了自动驾驶领域的首个时间理解基准测试TAD，包含近6000个QA对和7个任务，评估了9个模型并发现现有SOTA模型表现不佳，提出了两种无需训练的方法提升性能达17.72%。
...
Method: 提出了TAD基准测试，包含近6000个QA对和7个任务；评估了9个开源和闭源通用模型以及SOTA自动驾驶专业模型；提出了两种无需训练的方法：Scene-CoT（利用思维链）和TCogMap（结合第一人称时间认知地图）。

Result: 现有SOTA模型在TAD上表现不佳，主要因为细粒度运动理解不完善；提出的Scene-CoT和TCogMap方法与现有VLM集成后，在TAD上的平均准确率最高提升了17.72%。

Conclusion: TAD基准测试填补了自动驾驶时间理解评估的空白，提出的方法有效提升了模型性能，旨在推动自动驾驶时间理解领域的未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法主要依赖文本或图像提示，但语言存在歧义，图像编辑繁琐，难以提供直观精确的几何控制。需要一种能接受多种几何输入并实现精确空间控制的方法。

Method: SpaceControl是一种训练免费的测试时方法，可接受从粗略图元到详细网格的多种几何输入，与预训练生成模型无缝集成，无需额外训练。通过可控参数在几何保真度和输出真实感之间进行权衡。

Result: 广泛的定量评估和用户研究表明，SpaceControl在几何忠实度方面优于基于训练和优化的基线方法，同时保持高视觉质量。提供了交互式用户界面，支持在线编辑超二次曲面并直接转换为带纹理的3D资产。

Conclusion: SpaceControl提供了一种直观精确的3D生成空间控制方法，支持多种几何输入，在几何保真度和视觉质量之间取得良好平衡，便于创意工作流程的实际部署。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然革新了3D资产创建，但缺乏交互式编辑和精细化控制的方法。现有基于扩散或优化的方法速度慢、破坏原始资产特性或缺乏精细控制能力

Method: 提出一种状态感知的前馈模型，直接从用户提供的2D视图预测紧凑、特征丰富的高斯表示属性更新，利用测试时训练创建状态感知的迭代工作流程

Result: 该方法能够以交互速度执行高保真局部细节细化、局部绘制和一致的全局重着色等多种任务，单个架构支持多样化编辑操作
...
Method: 提出组正交低秩适配(GOLA)框架：1)采用奇异值分解量化秩重要性；2)冻结关键秩保留预训练先验；3)将冗余秩聚类分组；4)设计组间正交约束策略，强制不同组学习互补特征
...
<details>
  <summary>Details</summary>
Motivation: 传统Structure-from-Motion处理耗时且计算成本高，且大多数公开数据因相机姿态变化不足、场景元素遮挡和数据噪声等问题而不适合处理。

Method: 提出PoolNet深度学习框架，支持帧级和场景级的野外数据验证，能够区分适合SfM处理的场景与不适合的场景。

Result: 模型成功区分了适合SfM处理的场景与不适合的场景，同时显著减少了获取结构运动数据所需的时间，优于现有最先进算法。

Conclusion: PoolNet为解决SfM数据处理中的时间消耗和数据适用性问题提供了一个有效的深度学习解决方案，能够快速筛选适合处理的图像数据。
...
TL;DR: ShaRP提出了一种改进的注意力剪枝框架，通过段感知因果掩码、位置去偏和令牌去重技术，在浅层解码器实现高效视觉令牌剪枝，加速视频大语言模型推理。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在预填充阶段因处理大量视觉令牌而面临高计算负载。现有注意力剪枝方法在浅层解码器会导致显著性能下降，特别是在高压缩率下，这限制了其加速效果。

Method: 提出ShaRP框架，集成三个关键技术：1) 段感知因果掩码，增强令牌选择能力；2) 位置去偏，减少位置编码偏差影响；3) 令牌去重，避免冗余令牌处理。

Result: 实验表明ShaRP在多个视频理解基准测试中取得有竞争力的性能，能在高压缩率下保持稳定性能，无需重新训练即可实现有效剪枝。

Conclusion: ShaRP为加速VLLM推理建立了新范式，通过在浅层解码器实现有效剪枝，解决了现有注意力剪枝方法在早期层性能下降的问题。
...
TL;DR: LoC-Path是一个高效的多模态大语言模型框架，通过减少WSI图像中的冗余特征来降低计算成本，同时保持与现有SOTA方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSI）理解面临巨大挑战，因为其具有千兆像素级规模和诊断相关区域的极端稀疏性。现有病理学中的切片级多模态大语言模型依赖处理数千个补丁特征的笨重编码器，导致计算成本过高，而人类专家主要依赖关键区域进行诊断。

Method: 提出LoC-Path框架：1）设计稀疏令牌合并器（STM）和MAE预训练的重采样器，去除局部冗余并将全局冗余的图块令牌压缩为紧凑的切片级表示集；2）提出交叉注意力路由适配器（CARA）和令牌重要性评分器（TIS），以计算高效的方式将压缩的视觉表示与语言模型集成。

Result: 广泛实验表明，该方法在性能上与现有最先进的全切片MLLMs相当，同时需要显著更低的计算和内存资源。

Conclusion: 通过减少WSI图像中的冗余特征，可以构建高效的多模态大语言模型框架，在保持诊断性能的同时大幅降低计算成本，为病理学图像分析提供了更实用的解决方案。
...
Method: 首先对视频VAE潜在空间进行统计分析，识别出两个对扩散训练至关重要的光谱特性：偏向低频的时空频率谱和由少数模式主导的通道特征谱。为诱导这些特性，提出了两种轻量级、与主干无关的正则化方法：局部相关性正则化和潜在掩码重建。

Result: 实验表明，SSVAE在文本到视频生成中实现了3倍的收敛速度提升和10%的视频奖励增益，优于现有的开源VAE。

Conclusion: 通过优化VAE潜在空间的光谱结构，可以显著改善扩散模型的训练效率和生成质量，为视频生成系统提供了有效的改进方案。
...
Method: 结合视觉语言模型(VLMs)的强大推理能力和SAM2的精细空间分割能力，提出Dynamic Prior方法，无需任务特定训练即可鲁棒识别动态物体。

Result: 在合成和真实世界视频上的实验表明，该方法在运动分割上达到SOTA性能，并显著提升结构3D理解的准确性和鲁棒性。

Conclusion: Dynamic Prior方法能够有效解决动态物体对3D重建的干扰问题，无需专门训练即可集成到现有先进管道中，提升整体性能。
...
TL;DR: 提出基于遗传算法的SGBM+WLS立体匹配参数优化框架，用于无人机林业应用，自动搜索最优参数配置，提升测距精度同时保持处理效率。
...
Method: 提出遗传算法（GA）参数优化框架，自动搜索SGBM和WLS滤波器的最优参数配置。采用多图像质量指标评估方法，为资源受限的无人机系统提供实用解决方案。

Result: 实验结果表明，GA优化方法相比基线配置：均方误差降低42.86%，峰值信噪比提升8.47%，结构相似性提升28.52%。在不同成像条件下表现出优异的泛化性能。

Conclusion: 提出的GA参数优化框架有效解决了传统立体匹配算法的手动调参问题，显著提升了无人机林业测距的精度和鲁棒性，为资源受限系统提供了实用解决方案。
...
TL;DR: 提出基于计算机视觉的无人机自主修剪系统，结合YOLO目标检测与SGBM立体视觉，无需昂贵LiDAR即可实现精确树枝检测与深度估计
...
TL;DR: 该研究评估了无人机林业操作中的树木分支分割方法，在三种分辨率下测试了22种深度学习配置，建立了精度与效率的权衡基准。
...
Result: 在22种配置中：256x256分辨率下U-Net+MiT-B4表现最佳；512x512下MiT-B4在IoU、Dice、TS-IoU和Boundary-F1领先；1024x1024下U-Net+MiT-B3在IoU/Dice和精度最优，U-Net++在边界质量最佳；PSPNet效率最高但精度降低。

Conclusion: 研究为嵌入式林业系统建立了多分辨率基准，提供了精度与效率的权衡参考，有助于实际应用中的模型选择。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型在结合视觉语言模型和扩散模型时，难以充分平衡足够的交互与灵活的实现，主要因为两者表示差异巨大。视觉语言模型的各层包含从低层细节到高层语义的丰富层次信息，但现有方法未能充分利用这些信息。

Method: 提出ParaUni方法：1）并行提取视觉语言模型各层特征，通过层集成模块（LIM）高效整合细粒度细节和语义抽象，为扩散模型提供融合表示条件；2）设计层动态调整机制（LDAM），利用强化学习中不同层对不同奖励的响应差异，通过RL对齐各层层次特性以促进多奖励改进。

Result: 大量实验表明，ParaUni通过利用互补的多层特征显著提高了生成质量，并在强化学习阶段显示出多奖励提升的强大潜力。

Conclusion: ParaUni通过并行特征提取和层动态调整机制，有效解决了统一多模态模型中交互与实现的平衡问题，充分利用了视觉语言模型的层次信息，为多模态生成提供了更优的解决方案。
...
TL;DR: TED-4DGS：一种基于时间激活和嵌入的变形方案，用于动态3D高斯泼溅（4DGS）的率失真优化压缩，在多个真实数据集上达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有动态3DGS方法存在局限性：时空4DGS使用过度指定、短寿命的高斯基元，而规范3DGS变形缺乏显式时间控制。需要设计更紧凑高效的变形方案和率失真优化压缩策略。

Method: 基于稀疏锚点的3DGS表示，每个规范锚点分配可学习的时间激活参数控制其出现和消失，轻量级锚点时间嵌入查询共享变形库产生锚点特定变形。采用基于INR的超先验建模锚点属性分布，结合通道自回归模型捕获锚点内相关性。

Result: 在多个真实数据集上实现了最先进的率失真性能，这是首次为动态3DGS表示追求率失真优化压缩框架的尝试之一。

Conclusion: TED-4DGS统一了两种方法的优势，通过时间激活和嵌入的变形方案实现了高效的4DGS压缩，为动态3D场景表示提供了紧凑且性能优异的解决方案。
...
TL;DR: 该研究开发了针对朱拉隆功大学的建筑识别数据集CUBR，并在无线自组织联邦学习场景中测试了Vision Transformer模型，结果显示联邦学习比自训练获得更高准确率。


<details>
  <summary>Details</summary>
Motivation: 随着边缘设备性能提升，在边缘设备上进行训练成为趋势。无线自组织联邦学习（WAFL）通过设备间通信实现协作学习，但作为任务导向的传感器系统，需要为每个特定任务构建专门的数据集。本研究针对泰国朱拉隆功大学开发专门的建筑识别数据集。

Method: 开发了朱拉隆功大学建筑识别数据集（CUBR），并在无线自组织联邦学习场景中使用Vision Transformer模型（WAFL-ViT）进行测试，与自训练场景进行对比。

Result: WAFL场景下的训练比自训练场景获得更好的准确率。数据集已在GitHub上公开提供。

Conclusion: 针对特定任务构建专门数据集对于任务导向的传感器系统至关重要，无线自组织联邦学习在边缘设备协作学习中表现出优于自训练的性能。
...
TL;DR: 该论文提出了情感图像风格化任务，开发了EmoStyle框架，通过构建数据集和情感-内容推理器，实现了在保持内容一致性的同时增强情感表达的图像风格化。


<details>
  <summary>Details</summary>
Motivation: 现有图像风格化方法主要关注视觉外观转换，但忽略了风格所承载的情感影响。为了填补这一空白，需要开发能够唤起特定情感同时保持内容一致性的图像风格化方法。

Method: 1. 构建EmoStyleSet数据集：从ArtEmis中提取内容-情感-风格化图像三元组；2. 提出情感-内容推理器：自适应整合情感线索与内容，学习连贯的风格查询；3. 开发风格量化器：将连续风格特征转换为情感相关的码本条目。
...
<details>
  <summary>Details</summary>
Motivation: 现有多对比度MRI重建方法存在两个主要问题：1) 需要为每种k空间欠采样模式单独训练模型，缺乏泛化能力；2) 主要关注空间信息而忽视频率特征，或仅提取浅层频率特征，未能充分利用跨模态频率信息。

Method: UniFS包含三个核心模块：跨模态频率融合模块、基于自适应掩码的提示学习模块、双分支互补细化模块。这些模块协同工作，从不同k空间欠采样模式中提取领域不变特征，同时自适应地处理各自的变化。

Result: 在BraTS和HCP数据集上，针对多种k空间欠采样模式和加速因子（包括未见过的模式）进行评估，实验结果表明UniFS在多种场景下均达到最先进的性能。

Conclusion: UniFS通过统一的频率-空间融合框架，成功解决了多对比度MRI重建中的泛化问题，能够处理多种k空间欠采样模式而无需重新训练，显著提升了模型的泛化性能。
...
TL;DR: 提出一种利用2D视觉语言模型挖掘自动驾驶场景中稀有物体的跨模态框架，通过智能数据挖掘提升3D物体检测性能，显著减少标注负担并提高检测精度。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中，仅依赖点云数据的稀有物体检测仍然具有挑战性。虽然视觉语言模型在图像理解方面表现出强大能力，但其通过智能数据挖掘来增强3D物体检测的潜力尚未得到充分探索。

Method: 提出新颖的跨模态框架，利用2D视觉语言模型从驾驶场景中识别和挖掘稀有物体。方法综合了物体检测、语义特征提取、降维和多方面异常检测技术，形成可解释的流程。结合隔离森林和t-SNE异常检测方法与基于概念的过滤，有效识别语义上有意义的稀有物体。

Result: 在nuScenes数据集上的实验表明，这种概念引导的数据挖掘策略提升了3D物体检测模型的性能，同时仅使用少量训练数据。与相同数量的随机数据相比，在具有挑战性的物体类别（如拖车和自行车）上表现出特别显著的改进。

Conclusion: 该方法能够提取和标注目标稀有物体概念（如工程车辆、摩托车和障碍物），大幅减少标注负担并专注于最有价值的训练样本。这一发现对安全关键自动驾驶系统中数据集的效率管理具有重要意义。
...
TL;DR: WaterWave：一种基于小波时域一致性场的隐式表示方法，用于提升水下视频增强的时序一致性，无需配对数据，通过时域频率先验和光流校正模块实现自然流畅的视频增强。


<details>
  <summary>Details</summary>
Motivation: 现有水下视频增强方法通常直接逐帧应用单图像增强模型，缺乏时序一致性。由于水下成像复杂，获取配对视频数据困难，需要一种无需配对数据且能保持时序一致性的视频增强方法。

Method: 提出WaterWave方法：1）从局部时域频率角度观察动态场景中的时序一致性先验；2）在小波时域一致性场中进行隐式表示；3）在时域一致性先验约束下，渐进过滤和衰减不一致成分，同时保留运动细节；4）设计水下光流校正模块，考虑水下传输特性更准确表示时域频带。

Result: 实验表明WaterWave显著提升了使用单图像水下增强方法生成的视频质量。在下游水下跟踪任务（如UOSTrack和MAT）中表现出高潜力，精确度分别比原始视频提升19.7%和9.7%。

Conclusion: WaterWave通过利用时域一致性先验和光流校正，在无需配对数据的情况下实现了时序一致的水下视频增强，为水下视频处理和跟踪任务提供了有效解决方案。
...
Method: 1. 创建Know-Show基准：从Charades、Action Genome和Ego4D数据集构建，包含2.5K人工编写的问题，涵盖空间（人物、物体、人物-物体、手-物体）和时间维度五个互补场景。2. 提出GRAM插件：通过基于注意力的视频token选择和显式时间戳编码，无需训练即可增强视频语言模型的细粒度基础能力。

Result: 实验显示现有视频语言模型（Qwen、VideoLLaVA、GPT-4o、Gemini等）在"展示所知"和"基于所见推理"方面存在显著困难，特别是在细粒度的手-物体交互场景中。Know-Show为评估基础推理能力建立了统一标准。

Conclusion: Know-Show基准揭示了当前视频语言模型在时空基础推理方面的局限性，提出的GRAM插件能够有效增强模型的基础能力，为开发可解释和可靠的多模态推理系统提供了重要见解。
...
Method: 提出DashFusion框架：1) 双流对齐模块通过时间对齐（跨模态注意力）和语义对齐（对比学习）同步多模态特征；2) 监督对比学习利用标签信息细化模态特征；3) 分层瓶颈融合通过压缩瓶颈token逐步整合多模态信息。

Result: 在CMU-MOSI、CMU-MOSEI和CH-SIMS三个数据集上评估，DashFusion在各种指标上达到最先进性能。消融研究证实了所提对齐和融合技术的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有DETR风格的单阶段ST-SGG模型存在两个主要问题：1）可学习查询在语义上无信息且实例无关初始化；2）仅依赖单模态视觉特征进行谓词分类。需要整合VLM的常识推理能力来解决这些问题。

Method: 提出VOST-SGG框架：1）双源查询初始化策略，将"关注什么"与"在哪里关注"解耦，实现语义基础的what-where推理；2）多模态特征库，融合来自VLM的视觉、文本和空间线索，改进谓词分类。

Result: 在Action Genome数据集上的大量实验表明，该方法达到了最先进的性能，验证了整合VLM辅助的语义先验和多模态特征对ST-SGG的有效性。

Conclusion: VOST-SGG通过整合VLM的常识推理能力和多模态特征，有效解决了现有单阶段ST-SGG模型的局限性，在时空场景图生成任务上取得了显著改进。
...
TL;DR: 该论文基于"枯叶模型"开发了一种贝叶斯理想观察者方法，用于图像分割任务，为人类和视觉算法提供了性能评估的理论上限。


<details>
  <summary>Details</summary>
Motivation: 人类视觉环境由空间中分布的不同表面组成，物体间的遮挡关系决定了场景的可见部分。枯叶模型通过分层叠加物体来模拟这种遮挡关系，但需要一种理论方法来评估图像分割的性能上限。

Method: 基于枯叶模型开发贝叶斯理想观察者方法，推导像素分区的后验概率计算，详细解释计算步骤，并分析实际应用的可行性因素。

Result: 建立了基于独立枯叶模型分布的像素分区贝叶斯理想观察者理论框架，提供了计算后验概率的系统方法。

Conclusion: 枯叶图像模型及相关理想观察者可用于研究有限像素下的分割决策，为人类和视觉算法提供了性能比较的理论上限。
...
TL;DR: CG-VLM是一个无需训练、推理时使用的框架，通过博弈论可解释性方法解决大视觉语言模型中的文本惯性问题，在多个基准测试上达到最先进水平。
...
Method: CG-VLM框架包含两个核心模块：基于Harsanyi交互的认知需求传感器估计即时视觉-文本协同作用，识别需要视觉基础的时刻；聚焦共识归纳模块根据此信号，在注意力崩溃到文本先验之前，有选择地将中间层注意力重新导向视觉标记。

Result: CG-VLM在POPE和CHAIR基准测试上，针对InstructBLIP、LLaVA、Qwen-VL和mPLUG等模型取得了最先进的结果，同时保持了模型的通用能力。

Conclusion: 研究表明，基于标记级别的感知能够实现精确、上下文感知的干预，而不会损害基础知识，为视觉语言模型中的文本惯性问题提供了有效的解决方案。
...
TL;DR: 该研究提出了2K-Characters-10K-Stories数据集，通过解耦控制方案分离身份与瞬态属性，解决了视觉叙事中序列身份一致性的挑战。
...
Method: 1) 引入2K-Characters-10K-Stories多模态风格化叙事数据集；2) 采用人机协同管道，利用专家验证的角色模板和LLM引导的叙事规划；3) 设计解耦控制方案分离持久身份与瞬态属性；4) 实施质量门控循环，整合MMLM评估、自动提示调优和局部图像编辑。

Result: 在该数据集上微调的模型在生成视觉叙事方面达到了与闭源模型相当的性能，实现了像素级一致性。

Conclusion: 该研究通过大规模解耦控制数据集和人机协同管道，有效解决了视觉叙事中的序列身份一致性问题，为可控视觉叙事提供了可靠的数据基础。
...
TL;DR: MedDIFT：基于预训练扩散模型特征的免训练3D医学图像配准框架，在肺CT数据集上达到与先进学习方法相当的性能


<details>
  <summary>Details</summary>
Motivation: 传统医学图像配准方法依赖局部强度相似性度量，无法捕捉全局语义结构，在低对比度或解剖结构变异区域容易产生误匹配。需要一种能利用丰富几何和语义信息的配准方法。

Method: MedDIFT利用预训练潜在医学扩散模型的多尺度特征作为体素描述符，融合扩散激活形成丰富的体素级描述符，通过余弦相似度进行匹配，可选局部搜索先验。无需任务特定模型训练。

Result: 在公开肺CT数据集上，MedDIFT的配准精度与最先进的学习型UniGradICON模型相当，优于传统B样条配准方法。消融实验证实多级特征融合和适度扩散噪声能提升性能。

Conclusion: 扩散模型的中间表示编码了丰富的几何和语义信息，可用于构建有效的体素描述符。MedDIFT展示了利用预训练扩散模型进行免训练医学图像配准的潜力，为医学图像分析提供了新思路。
...
TL;DR: 提出一种无需蒙皮的三维服装变形生成方法，通过独立估计顶点位置和法线来解耦低频形状与高频褶皱，利用图像传输技术实现高质量动画。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖线性混合蒙皮获取低频服装形状并回归高频褶皱，但由于缺乏明确的蒙皮监督，在服装变形时会产生形状错位，从而破坏高频信号并无法恢复高保真褶皱。

Method: 提出蒙皮无关方法：1）独立估计顶点位置（低频形状）和顶点法线（高频褶皱细节）；2）将顶点属性编码为渲染纹理图像，通过2D图像传输实现3D服装变形；3）利用预训练图像模型恢复精细褶皱细节；4）提出多模态融合结合两种频率模态约束，从传输图像中稳健恢复变形3D服装。

Result: 大量实验表明，该方法在各种服装类型上显著提高了动画质量，比现有最先进方法恢复了更精细的褶皱。

Conclusion: 该方法通过解耦低频形状和高频褶皱、利用图像传输技术，实现了高质量的三维服装变形生成，无需依赖手动UV分区，具有良好的可扩展性。
...
TL;DR: Fast SceneScript：一种用于3D场景布局估计的新型结构化语言模型，通过多令牌预测和置信度引导解码，在保持精度的同时显著加速推理速度


<details>
  <summary>Details</summary>
Motivation: 当前基于语言模型的感知通用方法虽然取得了SOTA结果，但依赖自回归的下一个令牌预测，推理速度较慢。需要一种既能保持精度又能显著加速3D场景布局估计的方法。

Method: 1. 采用多令牌预测减少自回归迭代次数；2. 为结构化语言模型适配自推测解码；3. 引入置信度引导解码改进令牌可靠性评分机制；4. 设计参数高效机制减少多令牌预测的参数开销

Result: 在ASE和Structured3D基准测试中，Fast SceneScript能在每个解码器推理步骤生成多达9个令牌而不损失精度，仅增加约7.5%的额外参数

Conclusion: Fast SceneScript通过多令牌预测和置信度引导解码，实现了3D场景布局估计的高精度和高效推理，为结构化语言模型的实际应用提供了有效解决方案
...
TL;DR: InverseCrafter是一种高效的4D视频生成方法，将4D生成任务重新定义为潜在空间中的修复问题，避免了传统方法需要微调视频扩散模型的计算开销。
...
Result: InverseCrafter在相机控制任务中实现了可比较的新视角生成和更优的测量一致性，且计算开销几乎为零。同时，在通用视频修复和编辑任务中也表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统学习型ISP管道严重依赖大规模配对raw-to-sRGB数据集，数据获取成本高昂成为主要瓶颈，需要解决这一限制。

Method: 引入基于最优传输的无监督训练框架，首次将非平衡最优传输应用于跨域转换任务；采用"专家判别器委员会"作为混合对抗正则器，针对特定ISP失败模式提供专门梯度指导。

Result: 配对训练模式下在所有指标上超越原始配对方法，非配对模式在定量和定性表现上能与原始配对训练方法相媲美甚至超越。

Conclusion: 提出的UOT框架有效解决了ISP训练对配对数据的依赖问题，为学习型ISP提供了更灵活、鲁棒的训练方案，代码和预训练模型已开源。
...
TL;DR: 提出一种基于EXIF元数据的自监督方法检测AI生成图像，通过分类和排序EXIF标签学习摄影图像特征，实现跨模型检测和强泛化能力
...
Method: 使用自监督方法，通过分类相机模型、场景类型等分类EXIF标签，以及排序焦距、光圈值等连续EXIF标签来训练特征提取器；然后使用高斯混合模型进行单类检测，或结合空间打乱补丁的高频残差进行二元分类

Result: 在各种生成模型上的广泛实验表明，该方法显著提升了检测性能，对野外样本具有强泛化能力，并对常见的良性图像扰动具有鲁棒性
...
TL;DR: LeAD-M3D：一种实时单目3D目标检测器，通过三个关键技术组件实现SOTA精度和实时推理，无需额外模态或几何先验


<details>
  <summary>Details</summary>
Motivation: 解决单目3D检测中的深度模糊、视角变化和3D推理计算成本高的问题。现有方法要么依赖LiDAR或几何先验来补偿缺失深度，要么牺牲效率来获得竞争性精度。

Method: 1. 非对称增强去噪蒸馏(A2D2)：通过质量和重要性加权的深度特征损失，将几何知识从干净图像教师模型转移到混合噪声学生模型；2. 3D感知一致性匹配(CM3D)：将3D MGIoU整合到匹配分数中，改进预测到真实值的分配；3. 置信度门控3D推理(CGI3D)：通过将昂贵的3D回归限制在置信度最高的区域来加速检测。

Result: 在KITTI和Waymo上达到SOTA精度，在Rope3D上获得最佳报告的汽车AP，同时运行速度比之前的高精度方法快3.6倍。

Conclusion: LeAD-M3D证明了在单目3D检测中同时实现高保真度和实时效率是可行的，无需LiDAR、立体视觉或几何假设。
...
TL;DR: 本文系统分析了基于世界模型的测试时验证器在空间推理任务中的表现，揭示了现有方法（如MindJourney）的局限性，并提出了一种基于空间断言验证的新框架ViSA，该框架在部分基准上表现更好，但在复杂任务中仍面临世界模型信息瓶颈的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在需要多视角理解和具身视角转换的空间推理任务中存在局限。虽然MindJourney等近期方法通过测试时扩展（让世界模型想象动作条件轨迹）和启发式验证器选择有用视角来弥补这一差距，但这些验证器的实际效果和可靠性尚未得到系统评估。

Method: 1. 系统分析现有测试时验证器（如MindJourney）在基准测试中的行为，使用不确定性分析揭示其校准问题和系统偏差；2. 提出基于空间断言的验证框架ViSA，将测试时奖励建立在可验证的、帧锚定的微观主张上；3. 在SAT-Real和MMSI-Bench等基准上进行实验评估。

Result: 1. MindJourney的验证器提供很少有意义的校准，随机评分通常能同样有效地降低答案熵，暴露了系统性的动作偏差和不可靠的奖励信号；2. ViSA框架在SAT-Real基准上持续改善空间推理，并通过更平衡的探索行为纠正轨迹选择偏差；3. 在具有挑战性的MMSI-Bench上，包括ViSA在内的所有验证器都无法实现一致的扩展，表明当前世界模型形成了信息瓶颈，想象的视角无法丰富细粒度推理。

Conclusion: 研究揭示了基于世界模型的测试时验证在空间推理中的"坏、好、丑"三个方面：坏的是现有验证器缺乏校准且存在系统偏差；好的是ViSA等原则性验证器能在某些基准上改善性能；丑的是当前世界模型本身成为信息瓶颈，限制了在复杂任务中的推理能力。这为未来改进世界模型和验证机制指明了方向。
...
TL;DR: 该研究提出了一种将Phase-OTDR一维数据转换为灰度图像的新方法，通过图像化处理和迁移学习模型，实现了对光纤中六种事件的高精度检测，准确率超过98%。


<details>
  <summary>Details</summary>
Motivation: 传统Phase-OTDR数据分析方法在处理复杂光纤传感数据时存在局限性，需要更鲁棒和高效的分析技术来提升光纤监测系统的准确性和可靠性。

Method: 使用Gramian Angular Difference Field、Gramian Angular Summation Field和Recurrence Plot三种技术将一维Phase-OTDR数据转换为灰度图像，然后组合成多通道RGB表示，最后利用EfficientNetB0和DenseNet121等迁移学习模型进行分类。

Result: EfficientNetB0模型达到98.84%的分类准确率，DenseNet121模型达到98.24%的准确率。通过5折交叉验证，测试准确率分别为99.07%和98.68%，证明了方法的可靠性。

Conclusion: 图像化分析方法在解释复杂光纤传感数据方面具有变革性潜力，能够显著提高光纤监测系统的准确性和可靠性，同时减少数据集大小并提升分析效率。
...
Method: 提出C3不确定性量化方法，包含三个核心创新：1）通过严格适当评分规则训练视频模型的正确性和校准性；2）在潜在空间估计不确定性，避免像素空间方法的不稳定性和高昂训练成本；3）将密集潜在空间不确定性映射到可解释的像素级不确定性，提供高分辨率不确定性热图。
...
TL;DR: ZRNet：基于物理知识的框架，通过Zernike图模块和频率感知对齐损失，联合预测Zernike系数并进行光学图像恢复，在复杂大振幅像差下实现最先进的显微图像恢复性能。


<details>
  <summary>Details</summary>
Motivation: 光学像差（特别是深层成像时）显著降低显微图像质量。现有方法通常只处理轻度像差，将问题视为黑盒映射，没有利用波前畸变的光学物理原理。
...
Conclusion: ZRNet通过结合物理知识的方法，能够有效处理复杂大振幅像差，为显微图像恢复提供了更准确且物理一致的解决方案。
...
TL;DR: 该论文通过生成六个合成数据集来量化背景相关性对交通标志识别分类器的影响，使用可解释AI方法分析背景特征的重要性。
...
Method: 系统生成六个合成数据集用于交通标志识别任务，这些数据集仅在相机变化程度和背景相关性上有所不同。通过控制变量法量化背景相关性、不同相机变化水平和交通标志形状对分类性能和背景特征重要性的独立影响。

Result: 结果包括量化了背景特征在何时以及多大程度上获得重要性以支持分类任务，这取决于训练域的变化。具体量化了背景相关性对分类器性能的影响程度。

Conclusion: 通过合成数据集的系统实验，能够更可靠地量化背景特征在分类任务中的重要性，为可解释AI方法的验证提供了更可靠的基准，有助于区分合理的相关性学习和过拟合。
...
<details>
  <summary>Details</summary>
Motivation: 现有点云补全方法主要依赖欧氏距离，忽略了点云内在的非线性几何结构，导致几何一致性不足和语义模糊问题。需要一种能够显式捕捉流形拓扑结构的方法来改善补全质量。
...
Conclusion: 通过显式地整合非线性几何信息，特别是测地线距离和流形拓扑结构，可以显著提升点云补全的几何一致性和语义连贯性，为点云处理提供了新的研究方向。
...
Method: 通过仅使用文本上下文和二进制分割掩码（不包含敏感图像）提示教师LLM生成专家监督数据集，然后对本地可部署VLM进行监督微调和直接偏好优化。

Result: 使用生成的数据集微调VLM显著提高了手术领域知识，相比基础VLM有大幅提升。

Conclusion: 验证了一种数据高效且符合隐私保护要求的方法，用于训练手术领域优化的本地可部署VLM进行手术场景理解。
...
### [55] [AQUA-Net: Adaptive Frequency Fusion and Illumination Aware Network for Underwater Image Enhancement](https://arxiv.org/abs/2512.05960)
*Munsif Ali,Najmul Hassan,Lucia Ventura,Davide Di Bari,Simonepietro Canese*

Main category: cs.CV

TL;DR: AQUA-Net是一个轻量级水下图像增强模型，通过频率融合和光照感知分支在空间、频率和光照域联合处理，有效恢复水下图像色彩平衡和对比度，同时保持较低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 水下图像常因波长依赖的光吸收和散射而出现严重色彩失真、低对比度和雾状外观。现有深度学习模型计算复杂度高，限制了其在实时水下应用中的实际部署。

Method: 提出AQUA-Net模型，集成残差编码器-解码器与双辅助分支（频率域和光照域）。频率融合编码器从傅里叶域提取频率线索丰富空间表示；光照感知解码器基于Retinex理论，通过学习的照明图进行自适应曝光校正，分离反射和光照效应。

Result: 在多个基准数据集上的实验表明，AQUA-Net在定性和定量评估中与SOTA方法表现相当，同时使用更少的参数。消融研究证实频率和光照分支提供了互补贡献，改善了可见性和色彩表示。模型展示了强大的泛化能力和鲁棒性。

Conclusion: AQUA-Net通过联合空间、频率和光照设计，能够恢复不同水下条件下的色彩平衡、视觉对比度和感知真实性，为实际水下成像应用提供了有效解决方案。同时提出了一个高分辨率的地中海真实水下视频数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有高保真视频扩散模型面临两个主要冗余问题：全局时空注意力的二次复杂度和长迭代降噪轨迹的计算开销。现有加速方法通常只针对单一维度，存在收益递减问题

Method: USV是一个端到端可训练框架，学习动态的、数据和时间步依赖的稀疏化策略，联合协调模型内部计算和采样过程的稀疏化，包括剪枝冗余注意力连接、自适应合并语义相似token、减少降噪步数

Result: 在大规模视频生成基准测试中，USV实现了降噪过程83.3%的加速和端到端22.7%的加速，同时保持高视觉保真度

Conclusion: 统一的动态稀疏化为实现高效高质量视频生成提供了实用路径，多维度协同设计使先前独立的加速策略产生强相互增强效应
...
TL;DR: FNOpt是一个自监督布料模拟框架，使用傅里叶神经算子参数化的神经优化器，将时间积分转化为优化问题，无需地面真值数据，能跨分辨率泛化并捕捉细尺度褶皱。


<details>
  <summary>Details</summary>
Motivation: 现有神经模拟器通常依赖大量地面真值数据或牺牲细尺度细节，且在跨分辨率和运动模式时泛化能力差。需要一种无需大量标注数据、能保持细节并跨分辨率泛化的布料模拟方法。

Method: 将时间积分公式化为优化问题，训练一个由傅里叶神经算子参数化的分辨率无关神经优化器。仅使用粗网格上的物理损失进行自监督训练，无需地面真值数据。

Result: FNOpt在布料模拟基准数据集上优于先前基于学习的方法，在分布外设置中表现出更高的准确性和鲁棒性。能够泛化到更细分辨率，捕捉细尺度褶皱并保持模拟稳定性。

Conclusion: 基于FNO的元优化是先前布料神经模拟器的有吸引力的替代方案，减少了对精心策划数据的需求，提高了跨分辨率的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 深度传感器仅提供5%所需测量数据时，稀疏重建会引入几何误差，这对自动驾驶和机器人应用不可接受。现有几何深度学习隐含假设多个几何约束组合能提升性能，但本文挑战这一假设。

Method: 采用离散拉普拉斯算子进行曲率正则化，提供稳定梯度和噪声抑制，仅增加15%训练开销且零推理成本，相比复杂的多正则项组合更有效。

Result: 在稀疏测量条件下（5%数据），重建精度比标准变分自编码器提升18.1%，证明单一精心设计的正则项优于复杂的多正则项组合。

Conclusion: 精心设计的单一正则化项（离散拉普拉斯曲率正则化）不仅匹配而且超越复杂多正则项组合的效果，挑战了几何深度学习中多约束组合必然提升性能的隐含假设。
...
TL;DR: 提出CCVD模型解决定制化文本到视频生成中的持续学习问题，通过概念特定属性保留和任务感知概念聚合来缓解灾难性遗忘，使用可控条件合成解决概念忽略问题。


<details>
  <summary>Details</summary>
Motivation: 现有定制化文本到视频生成方法假设个性化概念是静态不变的，无法处理随时间递增的新概念学习，并且在持续学习新概念时会出现遗忘和概念忽略问题。

Method: 开发了CCVD模型，包含：1）概念特定属性保留模块和任务感知概念聚合策略来捕获旧概念特征并基于相关性组合所有主题和运动适配器；2）可控条件合成通过层特定区域注意力引导的噪声估计来增强区域特征并对齐视频上下文与用户条件。

Result: 广泛的实验比较表明，CCVD模型在定制化文本到视频生成任务中优于现有方法。

Conclusion: CCVD模型能够持续学习新概念，有效解决定制化文本到视频生成中的遗忘和概念忽略问题，在多种生成任务中表现优异。
...
TL;DR: 提出UG-FedDA框架，结合不确定性量化与联邦域适应，解决多中心阿尔茨海默病MRI分类中的站点异质性和隐私问题，在三个公开数据集上取得良好性能。
...
Method: 提出不确定性引导的联邦域适应(UG-FedDA)框架，使用自注意力transformer提取多模板ROI特征，整合不确定性量化指导特征对齐，通过降低不确定样本权重缓解源-目标分布偏移。

Result: 在ADNI、AIBL和OASIS三个数据集上，UG-FedDA在三个分类任务中均取得一致跨域改进：AD vs NC准确率90.54%、89.04%、77.78%；MCI vs AD准确率80.20%、71.91%、79.73%；NC vs MCI准确率76.87%、73.91%、83.73%。

Conclusion: UG-FedDA框架不仅能高效适应多中心数据，还能在严格隐私保护下处理跨站点MRI异质性，为阿尔茨海默病早期诊断提供了鲁棒且临床适用的解决方案。
...
TL;DR: 本文提出VRSA视觉推理序列攻击方法，通过将有害文本分解为多个相关子图像序列，诱导多模态大语言模型逐步外化并聚合完整有害意图，实现对MLLMs的安全风险评估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在跨模态理解和生成方面能力强大，但更多模态也带来更多安全漏洞。现有研究主要关注文本模态的推理安全风险，而视觉模态的类似威胁被严重忽视。需要全面评估视觉推理任务中的潜在安全风险。
...
Conclusion: VRSA方法有效揭示了多模态大语言模型在视觉推理任务中的安全漏洞，为全面评估MLLMs的安全风险提供了新视角，表明视觉模态的推理安全威胁不容忽视。
...
<details>
  <summary>Details</summary>
Motivation: 用户经常对相机图像进行后期编辑以达到理想的照片处理风格。虽然RAW域编辑提供更高的准确性和灵活性，但大多数编辑都是在相机显示参考输出（如8位sRGB JPEG）上进行的，因为RAW图像很少被存储。现有RAW重建方法可以从sRGB图像恢复RAW数据，但这些方法通常针对像素级RAW重建保真度进行优化，在不同渲染风格和编辑操作下性能会下降。

Method: 引入一个可插拔的编辑感知损失函数，包含模块化的可微分图像信号处理器（ISP），模拟具有可调参数的真实照片处理流程。训练期间，每个ISP模块的参数从精心设计的分布中随机采样，模拟实际相机处理中的变化。损失在sRGB空间中计算，通过这个可微分ISP渲染地面真实和重建的RAW数据。

Result: 集成该损失函数可将各种编辑条件下的sRGB重建质量提高1.5-2 dB PSNR。当应用于元数据辅助的RAW重建方法时，该方法能够针对目标编辑进行微调，获得进一步增益。

Conclusion: 由于照片编辑是消费级成像中RAW重建的主要动机，这种简单而有效的损失函数提供了一种通用机制，可增强现有方法的编辑保真度和渲染灵活性。
...
Result: 实验表明SCAIL实现了最先进的性能，并通过精心策划的数据管道和全面基准测试，将角色动画推向工作室级的可靠性和真实感。
...
TL;DR: 提出神经隐式颅面模型（NICE），使用隐式神经表示进行解剖重建和手术结果预测，在正颌手术中提高面部外观预测准确性。


<details>
  <summary>Details</summary>
Motivation: 正颌手术对于矫正牙颌面骨骼畸形至关重要，但术后面部外观预测困难，因为骨骼移动与面部软组织之间存在复杂的非线性相互作用。现有方法要么计算效率低，要么无法完全捕捉这些复杂相互作用。

Method: 提出神经隐式颅面模型（NICE），包含形状模块和手术模块。形状模块使用区域特定的隐式符号距离函数（SDF）解码器重建面部表面、上颌骨和下颌骨；手术模块使用区域特定的变形解码器，由共享的手术潜在代码驱动，结合解剖先验知识，输出点位移场来精确建模手术结果。

Result: 大量实验表明，NICE优于当前最先进方法，显著提高了关键面部区域（如嘴唇和下巴）的预测准确性，同时稳健地保持了解剖完整性。
...
TL;DR: 提出了一种免聚类、单阶段的可学习原型框架，通过多样性正则化增强组织病理学图像中类内异质性的覆盖，在弱监督语义分割任务上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 组织病理学中的弱监督语义分割面临三个主要挑战：类间同质性、类内异质性以及CAM引起的区域收缩问题。现有两阶段方法成本高、对超参数敏感，且原型发现与分割学习分离，限制了其效果和效率。

Method: 提出了一种免聚类、单阶段的可学习原型框架，通过多样性正则化来增强形态学类内异质性的覆盖，避免了传统聚类方法的问题。

Result: 在BCSS-WSSS数据集上实现了最先进的性能，在mIoU和mDice指标上优于先前方法。定性分割图显示更清晰的边界和更少的错误标签，激活热图显示可学习原型覆盖了每个类别内更多样化和互补的区域。

Conclusion: 提出的免聚类单阶段可学习原型框架有效解决了组织病理学弱监督语义分割中的挑战，通过多样性正则化增强了类内异质性的覆盖，实现了更好的分割性能。
...
TL;DR: 该研究比较了不同合成面部数据生成技术在面部识别任务中的效果，评估了8个主要数据集上的准确率、Rank-1、Rank-5等指标，发现合成数据能捕捉真实变化但性能仍落后于真实数据。


<details>
  <summary>Details</summary>
Motivation: 面部识别在可解释性、人口统计偏差、隐私、老化、姿态变化、光照变化、遮挡和表情等方面仍面临挑战。隐私法规导致多个数据集质量下降，引发法律、伦理和隐私问题。合成面部数据生成被提出作为有前景的解决方案，能缓解隐私问题、控制面部属性、减轻人口统计偏差，并为改进基于真实数据训练的模型提供补充数据。

Method: 研究比较了使用不同技术生成的合成面部数据集在面部识别任务中的有效性。在八个领先数据集上评估了准确率、Rank-1、Rank-5以及假阳性率为0.01%时的真阳性率，提供了文献中未广泛探索的比较分析。

Result: 结果表明合成数据能够捕捉现实变化，但强调需要进一步研究来缩小与真实数据的性能差距。扩散模型、GANs和3D模型等技术显示出实质性进展，但挑战仍然存在。
...
TL;DR: 提出一个结合数据驱动和解析建模的交通标志识别合成数据集生成方法，通过GAN纹理生成和解析场景调制创建真实感交通标志图像，并发布包含105500张图像的德国交通标志数据集


<details>
  <summary>Details</summary>
Motivation: 解决交通标志识别中真实数据获取困难、新标志数据稀缺的问题，同时为可解释AI和鲁棒性测试提供可控参数化的合成数据

Method: 结合GAN纹理生成（数据驱动）和解析场景调制（物理正确光照），生成包含污垢、磨损等真实感效果的交通标志图像，提供详细参数化控制

Result: 创建了Synset Signset Germany数据集，包含105500张图像、211个德国交通标志类别，包括2020年新发布的罕见标志，提供掩码、分割图像和详细元数据

Conclusion: 该方法成功生成了高质量合成交通标志数据集，支持可解释AI和鲁棒性测试，在真实数据集GTSRB上验证了真实感，优于现有合成数据集CATERED
...
TL;DR: 提出一种名为EditThinker的深思熟虑式图像编辑框架，通过"思考-编辑"循环来提升指令跟随能力，显著改善现有图像编辑模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑方法虽然能生成高质量图像，但指令跟随能力有限，主要因为生成过程的随机性和缺乏深思熟虑。单次编辑成功率受限于模型缺乏像人类一样的批判性思考和迭代改进能力。

Method: 提出深思熟虑式编辑框架，模拟人类认知循环，通过迭代执行"思考-编辑"循环：批判结果、精炼指令、重复生成直到满意。训练单一MLLM模型EditThinker作为推理引擎，联合生成批判分数、推理过程和精炼指令。使用强化学习对齐EditThinker的思考与编辑过程。

Result: 在四个基准测试上的广泛实验表明，该方法显著提升了任何图像编辑模型的指令跟随能力，改进幅度很大。

Conclusion: 提出的EditThinker框架通过引入深思熟虑机制，有效解决了现有图像编辑模型指令跟随能力不足的问题，为社区提供了数据构建框架、数据集和模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有科学问答系统多为通用聊天机器人，缺乏针对特定教育领域的微调模型。虽然存在生物医学和科学文献的领域特定BERT变体，但尚未有针对大学课程材料的基础模型。本研究旨在填补这一空白，探索将基础模型适配到教育领域的可行性。

Method: 构建了包含1203个问答对的SQuAD格式自定义数据集，数据来源于大学模块手册，包含手动和合成生成的条目。使用PyTorch对BERT模型进行微调，并通过精确匹配和F1分数评估性能。

Result: 结果显示，即使是适度的微调也能显著改善假设构建和知识提取能力。微调后的BERT模型在课程信息问答任务上表现出色，证明了将基础模型适配到教育领域的可行性。

Conclusion: 研究表明，使用学术问答对微调BERT模型可以获得有效结果，为开发首个针对大学领域的特定问答模型奠定了基础，展示了构建自主教育知识系统的潜力。
...
TL;DR: 本文探讨了诗歌提示模式作为提示工程新工具的价值，通过诗歌创作提示来评估语言模型对著名诗人作品的描述、评价和改写倾向


<details>
  <summary>Details</summary>
Motivation: 当前提示工程主要关注算法倾向和偏见研究，而创意工作者和学者利用LLMs进行创作和探索写作边界。本研究旨在探索诗歌提示模式作为提示工程新工具的潜力，并评估模型对原创创意作品的适应和改写倾向

Method: 提出诗歌提示模式作为提示工程工具，使用诗歌创作提示来评估三个著名诗人模型，测试模型对原创作品的描述、评价以及为特定受众改写作品的意愿

Result: 研究发现诗歌提示模式是提示工程师工具箱的有用补充，通过诗歌提示可以揭示模型对创意作品的描述和评价方式，以及模型为适应不同受众而改写原创作品的倾向

Conclusion: 创意文本提示，特别是诗歌提示模式，为提示工程提供了新的方法论工具，有助于更深入地理解语言模型处理创意内容的方式和倾向，对创意写作和AI伦理研究具有重要意义
...
TL;DR: 该研究探索使用思维链提示工程结合语义搜索和知识图谱来改进LLM生成临床笔记的质量，相比标准单次提示有更好表现。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录数据激增，但医生手动撰写临床笔记耗时过长，增加了患者等待时间并可能延误诊断。大型语言模型具有生成类似人类文章的能力，但需要改进其在临床笔记生成中的应用。

Method: 采用思维链提示工程，结合传统思维链与语义搜索结果，并融入基于临床本体构建的知识图谱，以丰富生成临床笔记的领域特定知识。使用ICD代码和基本患者信息作为输入，在CodiEsp测试数据集的六个临床案例上使用GPT-4进行测试。

Result: 研究结果显示，该方法在生成临床笔记方面优于标准单次提示技术。
...
TL;DR: 本文提出CoT-Recipe方法，通过调节元训练中CoT和非CoT示例的比例，显著提升大语言模型在新颖任务上的推理能力，即使在没有上下文CoT示例的情况下也能获得高达300%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 虽然链式思维提示结合少样本上下文学习在大型语言模型中解锁了显著的推理能力，但当预训练知识不足时，这种方法在新颖任务上效果不佳。作者旨在解决在缺乏足够预训练知识的情况下，如何让模型通过上下文学习掌握新颖抽象推理任务的问题。

Method: 使用CoT-ICL Lab框架在受控环境中研究该问题，提出元训练技术来学习新颖的上下文推理任务。发现CoT示例过多会降低性能，因此提出CoT-Recipe方法，正式调节元训练序列中CoT和非CoT示例的混合比例。

Result: 通过CoT-Recipe的精心调节，即使在没有上下文CoT示例的情况下，也能将transformer在新颖任务上的准确率提升高达300%。在预训练LLMs（Qwen2.5系列）的符号推理任务中应用这些技术，观察到准确率提升高达130%。

Conclusion: CoT-Recipe方法通过调节元训练中CoT和非CoT示例的比例，有效解决了CoT提示在新颖任务上效果不佳的问题，显著提升了模型在缺乏预训练知识情况下的推理能力，为上下文学习提供了新的优化方向。
...
TL;DR: 本文研究了粉红粘液新闻（Pink Slime Journalism）的检测问题，发现大型语言模型（LLMs）能显著降低现有检测系统性能，并提出了一种能抵抗LLM对抗攻击的鲁棒学习框架。


<details>
  <summary>Details</summary>
Motivation: 粉红粘液新闻作为一种低质量、自动生成的虚假地方新闻报道，威胁着美国2800万人依赖的本地新闻生态。需要对其语言、风格和词汇特征进行细粒度分析以检测这类欺骗性内容。

Method: 首先对粉红粘液内容进行了全面研究以揭示其区分性模式，并基于这些洞察提出检测策略。特别关注了LLMs作为新的对抗性攻击向量，并设计了一个专门抵抗LLM对抗攻击的鲁棒学习框架。

Result: 研究发现即使是消费者可访问的LLMs也能显著削弱现有检测系统，使其F1分数性能下降高达40%。提出的鲁棒学习框架能抵抗LLM对抗攻击，性能提升高达27%。

Conclusion: 粉红粘液新闻对本地新闻生态系统构成严重威胁，LLMs的出现加剧了这一挑战。需要开发专门针对LLM对抗攻击的鲁棒检测系统，以应对自动化粉红粘液新闻的不断演变。
...
<details>
  <summary>Details</summary>
Motivation: 挑战语言变化是简化的朴素假设，为形态丰富、资源匮乏的语言演化提供新见解，解决数据稀缺问题

Method: 使用弱监督方法，通过100+高精度正则表达式生成伪标签微调多语言BERT，通过置信度加权集成融合符号和神经输出

Result: 系统在147万词历时语料库中达到52.4%特征检测率，梵语形态复杂性未减少而是动态重新分布，早期动词特征呈周期性下降，复杂性转向复合词扩展和新哲学术语

Conclusion: 混合神经符号方法为计算文献学提供可靠工具，系统产生良好校准的不确定性估计，置信度与准确性高度相关，支持梵语复杂性动态重新分布的发现
...
TL;DR: 该论文研究了语言模型评估中的自我偏好偏见问题，发现即使通过简单的文本扰动（如同义词替换）可以降低自我偏好，但要完全消除这种偏见仍然面临根本性挑战。


<details>
  <summary>Details</summary>
Motivation: 语言模型评估器在评估LM输出质量时存在自我偏好偏见，即倾向于选择自己生成的答案而非其他模型或人类的答案。这种偏见难以消除，因为前沿的LM评估器即使在没有标注来源的情况下也能识别自己的输出。

Method: 采用黑盒扰动方法对成对比较中的评估候选文本进行修改，通过模糊作者身份来降低自我识别能力。具体使用简单的扰动策略如同义词替换少量词语。

Result: 简单的扰动（如少量词语的同义词替换）可以预测性地降低自我偏好。但当将扰动扩展到更彻底地消除评估候选之间的风格差异时，自我偏好会重新出现。

Conclusion: 自我识别和自我偏好可以在多个语义层面上发生，尽管初步结果有希望，但要完全缓解这种偏见仍然具有挑战性。这表明需要更深入的方法来解决LM评估中的根本偏见问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长文本生成任务（如摘要）中经常产生幻觉（输出内容与输入上下文不符）。现有方法通过迭代批判和精炼来减少幻觉，但要么需要额外的测试时计算，要么需要更强大的教师模型，导致成本高且不实用。

Method: 提出SCRPO框架：1）利用LLM自身的批判和精炼能力构建偏好数据集；2）应用偏好学习来改进同一LLM，使其生成更忠实的摘要。

Result: 在三个摘要基准测试（XSUM、CNNDM和SAMSum）上，该方法在忠实度指标上优于最先进的自监督学习方法，同时保持或改进了衡量摘要整体质量的其他指标。与测试时精炼相比，该方法不仅提高了效率，还生成了更忠实的摘要。

Conclusion: SCRPO提供了一种有效且实用的自监督训练方法，能够显著减少LLM在摘要任务中的幻觉问题，同时保持或提升摘要质量，且不需要额外的测试时计算或更强大的教师模型。
...
TL;DR: 该论文提出了一种稀疏量化格式(SQ-format)，这是一种统一的数据格式，旨在平衡大语言模型后训练量化中的准确性和效率，通过结合稀疏化和量化技术实现性能与吞吐量的帕累托改进。
...
Method: 提出稀疏量化格式(SQ-format)，这是一种统一的数据格式，利用稀疏矩阵可以在高精度下加速，低精度矩阵乘法也可以相应加速的事实。该格式特别适用于具有异常值不平等状态的激活值，使其静态压缩成为可能。

Result: 展示了使用SQ-format实现的最先进的后训练量化性能，提出了支持该格式所需的硬件要求，并为下一代AI加速器提供了设计探索和见解。

Conclusion: SQ-format是一种有前景的统一数据格式，能够在后训练量化中实现性能与吞吐量的帕累托改进，为未来AI加速器设计提供了新的方向。
...
TL;DR: 首个关于预训练语言模型在拼写纠正中有效性的实证研究，特别关注低资源语言，发现大型语言模型在微调数据量大时表现最佳，并发布了LMSpell工具包。


<details>
  <summary>Details</summary>
Motivation: 拼写纠正对于低资源语言仍然是一个挑战性难题。虽然预训练语言模型已被用于拼写纠正，但其应用仍局限于少数语言，且缺乏跨模型的系统比较。

Method: 对多种预训练语言模型（包括编码器、编码器-解码器和大型语言模型）在拼写纠正任务上的效果进行实证研究，特别关注低资源语言。开发了LMSpell工具包，包含评估函数以补偿LLM的幻觉问题，并以僧伽罗语为例进行案例研究。

Result: 研究发现，当微调数据集较大时，大型语言模型（LLMs）的表现优于其他模型（编码器基和编码器-解码器模型）。这一观察结果即使在LLM未预训练的语言中也成立。

Conclusion: 该研究为低资源语言的拼写纠正提供了首个跨预训练语言模型的实证比较，表明大型语言模型在数据充足时具有优势，并提供了实用的工具包LMSpell来支持相关研究。
...
TL;DR: 该研究针对大语言模型在音乐领域知识不足的问题，构建了MusWikiDB向量数据库（包含320万条音乐相关维基百科段落）和ArtistMus基准测试（1000个关于500位艺术家的问答），通过检索增强生成显著提升了音乐问答的准确性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在开放领域问答方面取得进展，但在音乐相关推理方面效果有限，因为预训练数据中音乐知识稀疏。虽然音乐信息检索和计算音乐学已经探索了结构化和多模态理解，但很少有资源支持基于艺术家元数据或历史背景的事实性和上下文音乐问答。

Method: 1. 构建MusWikiDB：包含320万条来自14.4万音乐相关维基百科页面的段落向量数据库；2. 创建ArtistMus基准：包含1000个关于500位多样化艺术家的问题，涵盖流派、出道年份、主题等元数据；3. 使用检索增强生成（RAG）方法进行系统评估；4. 进行RAG风格微调以提升事实回忆和上下文推理能力。

Result: 1. RAG显著提高事实准确性：开源模型提升高达+56.8个百分点（如Qwen3 8B从35.0提升到91.8），接近专有模型性能；2. RAG风格微调进一步提升事实回忆和上下文推理，在领域内和领域外基准测试中都有改善；3. MusWikiDB相比通用维基百科语料库准确率提高约6个百分点，检索速度提高40%。

Conclusion: MusWikiDB和ArtistMus资源推动了音乐信息检索和领域特定问答研究，为音乐等文化丰富领域的检索增强推理建立了基础。RAG方法能有效弥补大语言模型在音乐知识方面的不足，显著提升音乐问答性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前的安全模型评估主要集中于英语，缺乏对语言和文化多样性的考虑。东南亚语言在安全基准中代表性不足，机器翻译的英语数据无法捕捉低资源语言的细微差别，而该地区存在独特的文化敏感性和安全关切。

Method: 开发SEA-SafeguardBench基准，包含8种东南亚语言、21,640个人工验证样本，分为三个子集：通用场景、真实场景和内容生成场景，以本地化方式反映文化规范和危害情境。

Result: 实验结果显示，即使是先进的LLM和防护系统在处理东南亚文化场景和危害情境时也面临挑战，与英语文本相比表现不佳。

Conclusion: 需要开发本地化的人工验证安全基准来更好地评估多语言安全模型，特别是在东南亚等代表性不足的地区，以应对文化特定的安全关切。
...
TL;DR: 该研究评估了大型语言模型在放射学报告中病灶级别偶然瘤检测的性能，发现结合解剖学上下文提示的生成式LLM显著优于传统监督模型，性能接近人类专家水平。


<details>
  <summary>Details</summary>
Motivation: 当前基于文档级别的分类系统在检测需要随访的偶然瘤方面存在局限性，研究旨在评估LLM在细粒度病灶级别检测中的表现，以改进放射学工作流程中的自动化偶然发现监测。

Method: 使用包含400份标注放射学报告和1,623个验证病灶的数据集，比较三种监督式transformer编码器与四种生成式LLM配置。引入新颖的推理策略，使用病灶标记输入和解剖学感知提示来增强模型推理能力，通过类别特异性F1分数评估性能。

Result: 解剖学感知的GPT-OSS-20b模型表现最佳，偶然瘤阳性宏F1达到0.79，超过所有监督基线（最高宏F1：0.70），接近0.76的标注者间一致性。解剖学基础显著提升GPT模型性能，多数投票集成进一步将宏F1提升至0.90。

Conclusion: 结合结构化病灶标记和解剖学上下文的生成式LLM显著优于传统监督编码器，性能接近人类专家水平，为放射学工作流程中的自动化偶然发现监测提供了可靠且可解释的途径。
...
TL;DR: 该研究系统评估了Tree-of-Thought推理在孟加拉语数学应用题上的效果，相比Chain-of-Thought能进一步提升性能，特别是在中大型语言模型中表现更佳。


<details>
  <summary>Details</summary>
Motivation: 数学应用题是NLP中最具挑战性的任务之一，需要语言理解和多步数值推理。虽然Chain-of-Thought提示显示出潜力，但其线性结构容易传播错误，限制了整体效果。研究旨在探索Tree-of-Thought推理在孟加拉语这种低资源语言数学问题上的应用。

Method: 使用SOMADHAN数据集，在计算和token成本限制下，选取100个代表性数学问题，在多个大型语言模型（包括GPT-OSS和LLaMA变体）上评估标准提示、Chain-of-Thought和Tree-of-Thought策略。

Result: CoT将基线准确率从78%（标准提示）提升到平均83%，而ToT进一步将性能提升高达5个百分点，在GPT-OSS-120B上达到88%准确率。ToT在中大型模型中特别有效，但对较小模型优势较小。

Conclusion: Tree-of-Thought是解决孟加拉语等低资源语言数学问题的稳健框架。结构化推理方法如ToT比CoT能提供更可靠和全局一致的结果，为多语言NLP中更好的推理策略铺平道路。
...
TL;DR: 该研究提出了一种基于检索增强生成的方法，为医学问答任务生成多语言推理轨迹，覆盖英语、意大利语和西班牙语，并在多个医学QA基准测试中取得了8B参数LLM的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 现有医学问答方法主要依赖英语，且大多通过蒸馏通用大语言模型获得，这引发了对其医学知识可靠性的担忧。研究旨在开发基于事实医学知识的多语言推理方法，支持更安全、透明的多语言临床决策支持工具。

Method: 采用检索增强生成方法，基于维基百科医学信息生成多语言推理轨迹。将MedQA和MedMCQA数据集扩展到意大利语和西班牙语，生成50万条推理轨迹。通过上下文学习（few-shot）和监督微调两种方式利用这些轨迹。

Result: 在医学QA基准测试的域内和域外设置中，生成的推理轨迹显著提升了模型性能。使用这些轨迹的8B参数LLM取得了最先进的结果，证明了该方法在多语言医学推理中的有效性。
...
TL;DR: ILVR框架通过交错潜在视觉表示与文本生成，解决了多模态大语言模型中视觉重编码计算成本高的问题，同时避免了过度压缩特征导致的感知精度损失。


<details>
  <summary>Details</summary>
Motivation: 当前交错推理范式虽然能增强多模态大语言模型，但反复重编码像素密集图像的计算成本过高。潜在视觉推理虽能绕过这一瓶颈，却面临两难选择：要么过度压缩特征牺牲精确感知建模，要么因静态非交错结构而无法处理动态问题。

Method: 提出交错潜在视觉推理(ILVR)框架，将文本生成与作为后续推理具体演化线索的潜在视觉表示交错结合。采用自监督策略，通过动量教师模型从辅助图像中有选择地蒸馏相关特征为稀疏监督目标，自适应选择机制引导模型自主生成上下文感知的视觉信号。
...
TL;DR: 开发了ClinEdu多智能体教学模拟器和ClinTeach数据集，训练出首个用于临床医学教育的多模态苏格拉底式导师MedTutor-R1，在模拟评估中表现优异


<details>
  <summary>Details</summary>
Motivation: 临床培训需求增长与专家指导稀缺之间存在显著差距，现有研究主要关注一对一知识传授，忽视了团队协作推理这一关键技能

Method: 开发ClinEdu多智能体教学模拟器，创建ClinTeach苏格拉底教学对话数据集，通过指令微调和强化学习训练MedTutor-R1，使用三维评估标准优化策略

Result: MedTutor-R1在平均教学评分上比基础模型提升超过20%，与o3相当，在处理不同数量学生时表现出高适应性

Conclusion: MedTutor-R1展示了在临床医学教育中进行一对多苏格拉底式教学的可行性，验证了教学模拟器ClinEdu的有效性
...
TL;DR: 比较两种模型在捷克宪法法院判决检索中的表现：通用OpenAI嵌入模型显著优于领域特定BERT模型，尽管绝对nDCG值较低，但差异具有统计显著性。


<details>
  <summary>Details</summary>
Motivation: 案例法检索是一项耗时的任务，通常通过查询数据库完成。本研究旨在比较不同模型在捷克宪法法院判决检索中的表现，特别是在处理来自遗留司法数据库的异构标签时，需要能够应对噪声标签的评估框架。

Method: 比较了两种模型在三种不同设置下的表现：(1)通用大型嵌入模型(OpenAI)，(2)领域特定BERT模型(在约30,000个判决上使用滑动窗口和注意力池化从头训练)。提出了噪声感知评估方法，包括IDF加权的关键词重叠作为分级相关性、通过两个阈值(0.20平衡、0.28严格)进行二值化、配对自助法显著性检验，以及支持定性分析的nDCG诊断。

Result: 尽管绝对nDCG值较低(在噪声标签下预期如此)，但通用OpenAI嵌入模型在两种阈值设置下，在@10/@20/@100三个召回点上都明显优于领域预训练的BERT模型；差异具有统计显著性。诊断分析表明低绝对值是由于标签漂移和强理想标准，而非缺乏实用性。

Conclusion: 通用嵌入模型在捷克宪法法院判决检索任务中优于领域特定模型。提出的评估框架足够稳健，可用于噪声黄金数据集下的评估，这在处理来自遗留司法数据库的异构标签数据时很典型。低nDCG绝对值反映了标签质量问题而非模型效用不足。
...
TL;DR: 提出一种基于多指标融合的方法，通过树模型结合人类判断来改进大语言模型输出的忠实度评估，创建了跨问答和对话领域的数据集用于复现和测试。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型输出的忠实度评估存在局限性，需要更准确的方法来评估模型输出的可靠性，以便在更多样化的场景中安全部署这些模型。

Method: 将多个基础忠实度指标融合成一个综合指标，使用树模型确定各指标的重要性权重，权重基于人类对LLM响应忠实度的判断来驱动，并创建了跨领域数据集用于评估。

Result: 融合后的指标在所有测试领域中都显示出与人类判断更强的相关性，提高了评估LLM输出忠实度的能力。

Conclusion: 提出的指标融合方法能更准确地评估大语言模型的忠实度，增强了模型的可信度，为在更广泛场景中部署LLM提供了支持，同时创建的数据集促进了跨领域忠实度评估的研究。
...
<details>
  <summary>Details</summary>
Motivation: LLM在文本分类中依赖精心设计的提示且计算成本高昂，需要更高效、可扩展的方法来结合传统分类器的鲁棒性和LLM的适应性

Method: 提出CICLe框架，整合轻量级基础分类器和Conformal Prediction，通过自适应减少候选类别来指导LLM提示，减少所需的shot数量和提示长度

Result: CICLe在多样NLP分类基准上表现优异：超越基础分类器，在样本充足时优于few-shot提示基线，低数据场景表现相当；减少shot数量34.45%，提示长度25.16%；在类别不平衡任务中表现突出
...
### [90] [Capturing Classic Authorial Style in Long-Form Story Generation with GRPO Fine-Tuning](https://arxiv.org/abs/2512.05747)
*Jinlong Liu,Mohammed Bahja,Venelin Kovatchev,Mark Lee*

Main category: cs.CL

TL;DR: 提出了一种基于GRPO和多奖励训练框架的细粒度风格控制故事生成方法，使用作者验证信号训练句子转换器作为风格奖励，结合内容和完整性奖励，在马克·吐温小说风格生成上超越GPT-4o等基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在开放式故事生成方面表现优异，但细粒度的风格控制能力有限。现有方法通常依赖浅层线索（如姓名或主题）来模拟作者风格，缺乏稳健的评估方法。

Method: 采用Group Relative Policy Optimization（GRPO）训练框架和自定义多奖励设置。风格奖励来自使用作者验证信号微调的句子转换器，结合内容和完整性评分来稳定长篇叙事生成。以马克·吐温的《哈克贝利·费恩历险记》作为参考风格范例进行实验。

Result: 8B参数模型在作者验证风格指标上超越了GPT-4o和Claude Sonnet 4等更大基线模型，获得0.628的风格分数和具有竞争力的内容质量。结果表明中等规模模型通过任务特定训练可以实现有效的风格控制生成。

Conclusion: 该方法证明了中等规模模型通过特定训练可以实现细粒度风格控制的故事生成。虽然输出明显与目标风格对齐，但叙事完整性仍是挑战，未来需要改进全局连贯性和故事结局建模。
...
TL;DR: 最高法院口头辩论中的打断行为对女性律师的语义内容影响不大，但会显著增加负面情绪表达
...
Result: 打断前后的语义相似度保持较高水平，表明打断不会显著改变论证内容；但对女性律师的打断包含显著更高的负面情绪
...
TL;DR: 研究显示，为AI模型分配专家或低知识水平角色提示，在解决研究生级别客观选择题时，对准确率提升基本无效，甚至有时会降低性能。


<details>
  <summary>Details</summary>
Motivation: 探究在商业、教育和政策领域，通过为AI模型分配特定角色（专家或低知识水平角色）是否能提升其在困难客观选择题上的表现，为实际应用提供技术指导。

Method: 在GPQA Diamond和MMLU-Pro两个研究生级别数据集上测试六个模型，采用三种角色分配方法：领域匹配专家角色、领域不匹配专家角色、低知识水平角色，并与无角色基线进行比较。

Result: 角色提示总体上未能提升准确率：领域匹配专家角色基本无显著影响（除Gemini 2.0 Flash外）；领域不匹配专家角色有时会降低性能；低知识水平角色通常损害准确率。

Conclusion: 为AI模型分配角色提示在提升事实性准确率方面效果有限，角色提示可能更适合改变输出风格而非改善事实性能。
...
<details>
  <summary>Details</summary>
Motivation: 直接将大语言模型应用于临床领域面临保持事实准确性和避免幻觉的挑战，需要一种能够结合医学领域专业知识的方法来提高医学问答系统的可靠性。

Method: 使用检索增强生成（RAG）框架，结合医学文献检索和开源大语言模型（LLaMA~2和Falcon），采用低秩适应（LoRA）进行高效领域微调，通过检索相关医学文献来支撑模型回答。
...
Conclusion: 检索增强生成与开源大语言模型的结合为可靠的生物医学问答提供了潜力，指向实用的临床信息学应用，通过基于检索证据的回答提高了事实正确性和减少了幻觉。
...
TL;DR: M4-RAG是一个大规模多语言多模态检索增强生成基准，覆盖42种语言和56种地区方言，包含8万多个文化多样性的图像-问题对，用于评估跨语言和模态的检索增强视觉问答性能。
...
Method: 构建M4-RAG基准，包含42种语言、56种方言和8万多个文化多样性图像-问题对。创建受控检索环境，包含数百万个精心策划的多语言文档，平衡真实性和可重复性。

Result: 系统评估显示，RAG对小规模视觉语言模型有持续益处，但对大规模模型无法扩展，甚至常常降低其性能，暴露了模型规模与当前检索效果之间的关键不匹配。

Conclusion: M4-RAG为推进下一代RAG系统提供了基础，这些系统能够跨语言、模态和文化背景进行无缝推理，揭示了当前RAG方法在大规模模型上的局限性。
...
TL;DR: 提出两种无监督的LLM忠实度评估指标：基于信息论的语义忠实度(SF)和基于热力学的语义熵产生(SEP)，通过将LLM建模为二分信息引擎来量化模型对给定任务的忠实程度。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型对给定任务的忠实度是一个复杂挑战，需要开发有效的无监督评估指标来量化模型输出与输入上下文的一致性。

Method: 将LLM建模为二分信息引擎，隐藏层作为麦克斯韦妖控制从上下文C到答案A的转换。将QCA三元组建模为共享主题上的概率分布，使用转移矩阵Q和A分别编码查询目标和实际结果。通过凸优化同时推断这两个矩阵，用KL散度计算语义忠实度(SF)，并将其映射到[0,1]区间。同时提出基于热力学的语义熵产生(SEP)指标。

Result: SF指标能够量化任何给定QCA三元组的忠实度，SEP指标显示高忠实度通常对应低熵产生。这两个指标可以单独或联合用于LLM评估和幻觉控制，并在公司SEC 10-K文件摘要任务上进行了验证。

Conclusion: 提出的SF和SEP指标为LLM忠实度评估提供了有效的无监督方法，基于信息论和热力学原理，能够量化模型输出与输入上下文的一致性，有助于LLM评估和幻觉控制。
...
Method: 采用两部分课程设计：第一部分教授基础机器学习概念，第二部分专注于当代大语言模型应用。课程包括详细的架构设计、实施策略、评估方法，并在两个七周的暑期学期中实施。

Result: 这种整合方法显著增强了学生对AI领域的理解，并更好地为他们应对行业需求做好了准备。

Conclusion: 该教学方法成功地将传统机器学习与现代LLM技术相结合，为学生提供了全面的人工智能教育，有效提升了他们在快速发展的AI领域的竞争力。
...
TL;DR: 该论文通过形式化证明指出，任何算法（包括AI模型）都无法产生超出其初始算法本身已有功能的新功能能力，因此无法实现真正的创造性突破。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能的快速发展，人们开始思考人类距离实现人工通用智能（AGI）还有多远。本文旨在探讨算法（包括AI）的能力上限，特别是它们是否能够实现真正的创造性突破。

Method: 采用先前研究中关于AGI的定义，即"在某个研究领域中能够创造和创新，从而解锁该领域中新的、先前未知的功能能力"。基于此定义，作者进行了形式化证明，分析算法的能力边界。

Result: 形式化证明表明：任何算法都无法展示超出其初始算法本身已有功能的新功能能力。AI模型只能展示现有功能能力及其组合和排列，无法实现真正的创造性突破。

Conclusion: 该证明对AI发展的未来和人类智能的起源都有重要启示。它表明算法本质上无法实现真正的创造性，这对AGI的实现提出了根本性挑战，并促使我们重新思考人类智能的本质。
...
TL;DR: 该论文主张可能性理论是解决Dempster-Shafer理论悖论的根本方案，通过可能性与必要性测度的二元框架建立逻辑一致的不确定性处理基础，避免DST的逻辑陷阱。


<details>
  <summary>Details</summary>
Motivation: 解决Dempster-Shafer理论中的悖论和危机，为不确定性处理提供逻辑一致且数学严谨的基础框架。

Method: 采用Bychkov文章中发展的公理化方法，基于可能性与必要性测度的二元装置，从零开始构建不确定性处理框架。通过比较概率、证据和可能性三种范式，以经典医疗诊断困境为例进行分析。

Result: 可能性理论能够正确处理矛盾数据，避免DST的逻辑陷阱，使形式推理更接近自然智能的逻辑。

Conclusion: 可能性理论不仅是替代方案，而是解决DST悖论的根本方案，为不确定性推理提供了更接近人类自然智能的数学基础。
...
TL;DR: 论文主张将AI研究目标从"自我改进"转向"协同改进"，强调人类研究者与AI系统协作进行AI研究，以实现共同超级智能


<details>
  <summary>Details</summary>
Motivation: 当前AI领域的自我改进目标充满危险且难以完全实现，需要更可行、更安全的目标来推动AI发展

Method: 提出"协同改进"概念，专注于提升AI系统与人类研究者协作进行AI研究的能力，包括从构思到实验的全过程

Result: 协同改进既能加速AI研究进程，又能通过人机共生赋予双方更安全的超级智能能力

Conclusion: 将人类研究改进纳入循环将使我们更快、更安全地实现超级智能，协同改进是比自我改进更可行、更优越的目标
...
TL;DR: ChipMind：一种基于知识图谱增强的推理框架，专门用于处理冗长的集成电路规格文档，通过将电路规格转换为领域知识图谱并结合自适应检索机制，显著提升LLM在硬件设计中的推理能力。
...
Result: 在工业级规格推理基准测试中，ChipMind显著优于现有最先进基线方法，平均提升34.59%（最高达72.73%），有效解决了LLM在硬件设计中的实际部署难题。

Conclusion: ChipMind框架填补了LLM辅助硬件设计在学术研究和实际工业部署之间的关键空白，为处理复杂电路规格文档提供了有效的解决方案，推动了LLM在集成电路开发中的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型从研究原型转向生产系统，从业者需要可靠方法来验证模型输出是否满足所需约束。基于采样的估计方法虽然能提供模型行为的直观认识，但无法提供可靠的保证。

Method: BEAVER框架使用新颖的token trie和frontier数据结构，系统性地探索生成空间，对任何前缀封闭的语义约束都能在每次迭代中维护可证明可靠的概率边界。

Result: 在正确性验证、隐私验证和安全代码生成任务中，BEAVER在相同计算预算下相比基线方法实现了6-8倍更紧的概率边界，识别出3-4倍更多高风险实例。
...
TL;DR: 该论文提出将"意志薄弱"（akrasia）作为分析AI智能体不一致性的核心概念，开发了Akrasia基准测试来衡量模型在诱惑下违背自身先前承诺的情况。


<details>
  <summary>Details</summary>
Motivation: 大语言模型表现出一种特殊的不一致性：它们"知道"正确答案但未能据此行动。这种全局判断与局部冲动之间的张力在人类哲学中被称为"意志薄弱"（akrasia）。作者认为这一概念对于分析AI智能体系统中的不一致性和目标漂移具有基础性意义。

Method: 提出了Akrasia基准测试的初步版本，包含四种结构化提示条件：基线（B）、同义词（S）、时间（T）和诱惑（X），用于测量模型局部响应与其先前承诺相矛盾的情况。该基准支持跨模型家族、解码策略和诱惑类型的"自我控制"量化比较。

Result: 基准测试能够量化比较不同模型在面临诱惑时的"自我控制"能力，并展示了微观层面的意志薄弱如何在多智能体系统中累积为宏观层面的不稳定性，可能被解释为"阴谋"或故意不对齐行为。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在复杂场景中存在多推理语义建模有限、逻辑鲁棒性不足、易受误导解释等问题，需要提升其认知推理能力。

Method: 提出MIND推理框架，包含：1）RAD范式自动生成多样化推理依据扩展数据集；2）P2CL策略分两阶段增强多推理正学习和主动逻辑判别纠正；3）MCA优化策略解决多推理语义空间表示纠缠问题。
...
TL;DR: 本文介绍了OntoAxiom基准测试，用于评估大型语言模型在识别本体公理方面的能力，测试了12个LLM在不同提示策略下的表现，发现逐公理提示效果更好，但性能因公理类型和本体领域而异。
...
Method: 引入OntoAxiom基准测试，包含9个中等规模本体（共17,118个三元组，2,771个公理），专注于子类、不相交、子属性、定义域和值域公理。评估12个LLM在三种few-shot设置和两种提示策略下的表现：直接查询所有公理的方法与逐公理查询方法。

Result: 逐公理提示策略比直接方法获得更高的F1分数，但性能在不同公理类型间差异显著。领域影响明显：FOAF本体在子类公理上得分0.642，而音乐本体仅0.218。大型LLM优于小型模型，但小型模型在资源受限环境中仍有价值。总体性能不足以完全自动化公理识别。

Conclusion: 虽然LLM性能尚未达到完全自动化公理识别的水平，但可以为本体工程师提供有价值的候选公理，支持本体的开发和精炼。逐公理提示策略更有效，性能受公理类型和本体领域影响，大型模型表现更好但小型模型在特定场景下仍可用。
...
TL;DR: 提出DeepDist SLS求解器，通过区分PMS和WPMS的权重更新策略、新初始化方法和优先满足单元/硬子句的decimation方法，显著提升了(W)PMS求解性能。


<details>
  <summary>Details</summary>
Motivation: 现有随机局部搜索算法主要关注子句权重方案设计，但未能充分区分部分最大可满足性(PMS)和加权部分最大可满足性(WPMS)，通常采用统一的权重更新策略，忽视了两种问题类型的关键结构差异。

Method: 1) 首次提出针对PMS和WPMS实例采用不同条件更新子句权重的新权重方案；2) 引入适应两种实例类型特点的新初始化方法；3) 提出优先满足单元子句和硬子句的decimation方法；4) 基于这些方法开发了DeepDist SLS求解器。

Result: 在近期MaxSAT评估的基准测试中，DeepDist优于最先进的SLS求解器。与TT-Open-WBO-Inc结合的混合求解器超越了MaxSAT评估2024冠军SPB-MaxSAT-c-Band和SPB-MaxSAT-c-FPS的性能。

Conclusion: 提出的区分PMS和WPMS的权重更新策略、新初始化方法和decimation方法有效提升了(W)PMS求解性能，DeepDist及其混合版本在基准测试中表现出色，代码已开源。
...
TL;DR: 本文提出进化推理优化框架，通过进化算法让大语言模型获得类似人类的系统2推理能力，发现GPT-5等最新模型推理能力有限，但通过简单进化循环可显著增强较弱模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在特定任务上表现出色，但在通用智能和系统2推理能力方面仍有不足。本文旨在探索机器智能能否像人类一样获得真正的推理能力，而不仅仅是特定技能。

Method: 提出进化推理优化框架，将多个LLM作为种群初始化，采用进化策略进行"适者生存"选择，通过进化循环最大化最佳个体的量化推理分数。

Result: 实验发现：1）GPT-5等最新LLM的系统2推理能力仍然有限；2）通过ERO的简单进化循环，相对较弱的模型（如Qwen-7B）能够涌现出强大的推理能力。

Conclusion: 进化推理优化框架能够有效增强LLM的推理能力，为机器获得类似人类的系统2推理能力提供了可行路径，表明通过进化方法可以弥补当前LLM在通用智能方面的不足。
...
TL;DR: 论文认为LLM不是AGI的死胡同，而是AGI的必要基础。真正的瓶颈在于缺乏一个能够协调、约束和绑定LLM模式库的System-2协调层，而不是LLM本身的结构限制。


<details>
  <summary>Details</summary>
Motivation: 针对"LLM只是模式匹配器，结构上无法进行推理或规划，因此是AGI的死胡同"这一批评观点，作者认为这种结论错误地识别了瓶颈所在。真正的挑战不是LLM本身，而是缺乏一个能够有效协调LLM模式库的高级认知层。

Method: 提出了UCCT理论，将推理建模为受有效支持度(rho_d)、表征不匹配(d_r)和自适应锚定预算(gamma log k)控制的相变过程。基于此理论开发了MACI架构，实现了诱饵机制（行为调制的辩论）、过滤机制（苏格拉底式判断）和持久性机制（事务性记忆）。

Result: 通过将常见批评重新框架为可测试的协调失败，论证了通往AGI的道路应该通过LLM而不是绕过LLM。无基础的生成只是对底层最大似然先验的无诱饵检索，而"推理"则是在锚点将后验概率向目标导向约束转移时出现的。

Conclusion: LLM是AGI的必要System-1基础，而缺失的System-2协调层才是真正的瓶颈。通过UCCT理论和MACI架构，展示了如何构建这样的协调层，从而证明LLM是通往AGI的可行路径而非障碍。
...
TL;DR: 本文提出了一种多模态肿瘤智能体（MOA），用于低级别胶质瘤IDH1突变预测，结合了基于TITAN基础模型的病理学工具和临床基因组数据推理，在TCGA-LGG队列中验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 低级别胶质瘤中IDH1突变具有重要的临床意义，能够定义不同的亚组并影响预后和治疗决策。目前需要更准确的方法来预测这种突变，以指导个性化治疗。

Method: 开发了多模态肿瘤智能体（MOA），整合了基于TITAN基础模型的病理学工具用于IDH1突变预测，同时通过PubMed、Google Search和OncoKB对结构化临床和基因组输入进行推理。在TCGA-LGG队列的488名患者中进行了评估。

Result: MOA在没有病理学工具的情况下（F1=0.826）优于临床基线（F1=0.798）。当与病理学特征融合后，MOA达到最高性能（F1=0.912），超过了病理学基线（0.894）和融合的病理学-临床基线（0.897）。

Conclusion: MOA能够通过外部生物医学资源捕获互补的突变相关信息，实现了准确的IDH1突变预测，为低级别胶质瘤的精准医疗提供了有力工具。
...
TL;DR: 使用GPT-5开发的论文正确性检查器发现，顶级AI会议和期刊发表的论文中存在显著数量的客观错误，且错误数量随时间增加，人类专家验证的准确率达83.2%
...
Method: 开发基于GPT-5的论文正确性检查器，系统识别顶级AI会议和期刊已发表论文中的客观错误（如公式、推导、计算、图表错误），排除主观考量。人类专家验证AI识别的潜在错误。

Result: 发表论文包含显著数量的客观错误，且每篇论文平均错误数随时间增加：NeurIPS从2021年的3.8个增至2025年的5.9个（增长55.3%）；ICLR从2018年的4.1个增至2025年的5.2个；TMLR从2022/23年的5.0个增至2025年的5.5个。人类专家验证316个潜在错误，确认263个为真实错误，准确率83.2%。AI检查器能为75.8%的识别错误提出正确修正。

Conclusion: 前沿大语言模型在检测和修正已发表论文中的客观错误方面具有潜力，有助于建立更坚实的知识基础。虽然大多数问题相对较小，但修正它们可以减少文献混淆并增强可复现性。
...
TL;DR: TRACE框架通过透明推理和一致性评估诊断大视觉语言模型的推理轨迹，而非仅评估最终答案，利用辅助推理集分解复杂问题并暴露标准评估忽略的失败


<details>
  <summary>Details</summary>
Motivation: 可靠数学和科学推理仍是大型视觉语言模型的开放挑战，标准最终答案评估常掩盖推理错误，导致无声失败持续存在

Method: 引入TRACE框架，利用辅助推理集（紧凑的子问题-答案对）分解复杂问题，通过基于一致性的指标评估中间步骤，暴露标准评估忽略的失败

Result: 实验显示ARS间的一致性与最终答案正确性相关，帮助定位推理步骤中的失败，提供模型改进的可操作信号；TRACE定义置信区域区分可靠与不可靠推理路径

Conclusion: TRACE框架为大型视觉语言模型的数学和科学推理提供了透明评估方法，支持有效的过滤、调试和模型改进
...
Method: 提出变分量子Rainbow DQN（VQR-DQN），将环拓扑变分量子电路集成到Rainbow DQN中，利用量子叠加和纠缠特性，将人力资源分配问题建模为基于军官能力、事件调度和转移时间的马尔可夫决策过程

Result: 在四个人力资源分配基准测试中，VQR-DQN相比随机基线实现了26.8%的归一化完工时间减少，相比Double DQN和经典Rainbow DQN分别提升了4.9-13.4%
...
<details>
  <summary>Details</summary>
Motivation: 创建动态、可扩展的物理问题基准测试，以评估语言模型在科学推理方面的能力，超越传统静态数据集，支持更全面的模型评估。

Method: 构建参数化物理问题数据集，每个问题都配有结构化推理步骤和生成真实解的Python代码。包含三种问题格式：MC-Symbolic、MC-Numerical和自由形式。引入三个新颖评估指标：一致性分数、失败率和混淆率。

Result: 使用最先进的指令调优语言模型进行实验，揭示了模型在科学推理方面的优势和局限性，表明SymPyBench能够有效评估模型的推理能力和不确定性。

Conclusion: SymPyBench为开发更鲁棒和可解释的推理系统奠定了基础，通过其动态、代码驱动的特性提供了更全面的模型评估框架。
...
### [113] [Two-Stage Camera Calibration Method for Multi-Camera Systems Using Scene Geometry](https://arxiv.org/abs/2512.05171)
*Aleksandr Abramov*

Main category: eess.IV

TL;DR: 提出一种新颖的两阶段多相机标定方法，无需精确平面图、物理访问或同步视频流，仅需静态图像即可完成标定


<details>
  <summary>Details</summary>
Motivation: 传统多相机标定方法在现实环境中面临挑战：缺乏精确平面图、无法物理放置标定图案、视频流不同步，需要一种更灵活实用的解决方案

Method: 两阶段标定方法：第一阶段通过标注自然几何基元（平行、垂直、等长线段）进行单相机部分标定，估计关键参数并将有效视场投影到水平面；第二阶段通过交互操作投影多边形与平面图对齐，确定剩余外参

Result: 方法实现为实用web服务，比较分析和演示视频证实了方法的适用性、准确性和灵活性，可在传统方法不可行的场景中部署精确多相机跟踪系统

Conclusion: 该方法克服了传统多相机标定的限制，仅需静态图像即可实现精确标定，为先前认为不可行的场景提供了可行的多相机跟踪系统部署方案
...
### [114] [CATNUS: Coordinate-Aware Thalamic Nuclei Segmentation Using T1-Weighted MRI](https://arxiv.org/abs/2512.05329)
...
TL;DR: CATNUS是一种基于3D U-Net架构的丘脑核团分割方法，通过坐标卷积层增强定位精度，支持多种MRI序列，在分割准确性和泛化性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 丘脑核团在脑功能和神经精神疾病中具有重要作用，但由于核团体积小、对比度有限、分辨率不足以及个体解剖变异等因素，准确分割丘脑核团仍然具有挑战性。

Method: 提出CATNUS方法，采用3D U-Net架构并增强坐标卷积层，提供更精确的大核团和小核团定位。提供预训练模型变体，支持定量T1图、MPRAGE和FGATIR序列。

Result: 与FreeSurfer、THOMAS和HIPS-THOMAS相比，CATNUS在多个核团上显示出改进的分割准确性和稳健的重测可靠性。在不同扫描仪、场强和厂商的旅行被试数据上表现出强大的分布外泛化能力。

Conclusion: CATNUS提供了一种准确且可泛化的丘脑核团分割解决方案，具有促进大规模神经影像研究和支持真实世界临床评估的强大潜力。
...
TL;DR: CLIDE是一种基于条件似然近似的零样本检测方法，用于检测AI生成的图像，特别是在特定图像领域（如艺术图像）中表现出色。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型（特别是基于扩散的方法）的快速发展，合成图像的真实感显著提高。虽然已有全监督和少样本方法，但维护更新的数据集耗时且困难。现有零样本方法难以适应特定图像领域（如艺术图像），限制了其实际应用。

Method: CLIDE是一种基于条件似然近似的零样本检测方法。该方法计算以真实图像为条件的似然，使其能够适应不同的图像领域。

Result: 在大规模通用数据集上表现出最先进的性能，在领域特定案例中显著优于现有方法。结果证明了方法的鲁棒性，并强调了AI生成图像检测任务需要广泛、领域感知的泛化能力。

Conclusion: CLIDE通过条件似然近似实现了有效的零样本检测，特别是在适应特定图像领域方面表现出色，为AI生成图像检测提供了更实用的解决方案。
