...
TL;DR: 本文展示了持久同调（PH）这一拓扑数据分析方法如何为艺术风格提供客观且可解释的见解，能够以统计确定性区分不同艺术家、同一艺术流派内的艺术家，以及区分艺术家作品与AI生成的模仿作品。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观性。随着数学工具的发展，需要更结构化的方法来分析艺术风格。

Method: 使用持久同调（PH）这一拓扑数据分析方法，通过数学结构分析艺术作品的特征。

Result: PH方法能够以统计确定性区分不同艺术家、同一艺术流派内的艺术家，并能有效区分艺术家原作与AI生成的模仿作品。
...
TL;DR: 提出了一种使用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化解决了训练数据有限的问题，能够生成多样化的面部表情并用于图像编辑。


<details>
  <summary>Details</summary>
Motivation: 由于可用于训练的动物面部图像在数量和变化上都很有限，特别是在不同个体的面部表情方面，因此生成动物面部表情具有挑战性。本研究聚焦于在系统神经科学和进化研究中广泛研究的猕猴。

Method: 使用StyleGAN2模型，实施：1）通过运动转移合成新面部表情图像的数据增强；2）基于初始训练StyleGAN2模型的潜在表示进行样本选择；3）损失函数优化以确保准确再现细微动作。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专为生成高质量双人肖像而设计的大规模基准数据集，包含超过10万张图像，涵盖多种场景、服装和双人互动，并配有丰富的元数据。同时提出了DHumanDiff基线方法，用于双人肖像生成，具有增强的面部一致性和个性化生成与语义驱动场景创建的平衡。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制在情感记忆保存和婚礼摄影规划等方面具有巨大应用潜力，但缺乏基准数据集阻碍了高质量双人肖像生成的发展。

Method: 构建了包含10万+图像的PairHuman数据集，提供详细的图像描述、人物定位、人体关键点和属性标签等元数据；提出了DHumanDiff基线方法，专门用于双人肖像生成，增强面部一致性并平衡个性化人物生成与语义驱动场景创建。

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化的肖像，具有优越的视觉质量，并符合人类偏好。

Conclusion: PairHuman数据集和DHumanDiff方法为双人肖像生成提供了有效的解决方案，能够产生高质量、个性化的肖像作品，数据集已公开可用。
...
Result: 该方法成功去除了中子成像数据的噪声，与基于前向模型生成的数据进行基准测试时，显示出比非机器学习方法（如BM3D）更低的重建误差和更优的边缘保持指标。
...
TL;DR: SaFeR-CLIP通过将不安全概念重定向到语义上最接近的安全替代品，最小化表示变化，在保持强大安全性的同时，比先前方法恢复高达8.0%的零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 改善视觉语言模型（如CLIP）安全性的微调通常以泛化性能显著下降为代价，这种权衡源于将不安全概念强制对齐到单一预定义安全目标的刚性对齐策略，破坏了模型学习的语义结构。

Method: 提出一种邻近感知方法：将不安全概念重定向到语义上最接近的安全替代品以最小化表示变化。引入SaFeR-CLIP微调框架，应用最小干预原则。

Result: SaFeR-CLIP成功调和了安全性和性能，在保持强大安全性的同时，比先前方法恢复高达8.0%的零样本准确率。还贡献了NSFW-Caps基准，包含1000个高度对齐的配对用于分布偏移下的安全性测试。
...
Method: 三阶段框架：1) 将栅格化输入提升为3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，建立跨视图的部件级对应关系；3) 在栅格到矢量转换中执行路径整合和结构优化。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来解决自回归3D网格生成模型的质量-速度权衡问题，实现并行化推理和增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统3D网格制作耗时且难以扩展，现有自回归模型存在质量与速度的权衡问题，且严格的序列依赖限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云配准技术来检索、生成和集成网格组件，解耦生成过程的序列依赖。

Result: Mesh RAG在各种基础自回归网格生成模型中显著提升网格质量，加速生成速度，并支持增量编辑，无需重新训练模型。

Conclusion: Mesh RAG框架有效克服了自回归网格生成模型的局限性，为3D内容创作提供了高效、可扩展的解决方案。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合LLM场景布局推理、程序生成和扩散式3D生成技术，无需手动建模即可生成可遍历的纹理化环境。
...
Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解，将自然语言描述转换为可遍历的完整纹理环境，支持在标准游戏引擎中立即探索或编辑。

Result: 系统完全模块化，支持对布局、规模和风格的细粒度控制，生成几何一致、视觉丰富且实时渲染高效的世界，可直接在游戏引擎中使用。

Conclusion: 这项工作代表了向可访问、大规模生成世界构建迈出的一步，推进了3D生成AI在游戏、模拟和沉浸式社交环境中的前沿应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有地球观测数据集主要关注语义理解任务，缺乏将多模态感知与可测量生物物理变量对齐的基准。为了填补这一空白，作者创建了REO-Instruct数据集。

Method: 通过混合人机协作流程生成和验证结构化文本注释，整合了共配准的Sentinel-2和ALOS-2影像数据，建立了森林生态场景中的认知可解释逻辑链。

Result: 对通用视觉语言模型的综合评估显示，当前模型在数值推理方面存在困难，这突显了科学视觉语言模型面临的关键挑战。
...
TL;DR: BOP-ASK是一个用于物体交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖6个任务，旨在解决现有VLMs在细粒度空间理解方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现良好，但忽略了真实应用所需的细粒度空间理解能力，包括精确3D定位、物体物理兼容性、物体可供性和多步骤空间规划。

Method: 利用BOP数据集的6D物体姿态生成细粒度标注，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 在BOP-ASK上训练的模型优于基线模型，并展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行细粒度物体中心空间推理的新兴能力。

Conclusion: BOP-ASK为训练和评估VLMs提供了丰富资源，证明了其在提升模型细粒度空间理解能力方面的有效性，并将公开发布数据集和生成流程。
...
TL;DR: 提出Parts-Mamba模型，结合GCN和Mamba架构，通过部件特定扫描特征和部件-身体融合模块，在骨架动作识别中有效处理遮挡导致的不完整骨架问题，在NTU数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实场景中由于遮挡或通信质量问题，捕获的骨架往往不完整，现有GCN模型在缺失局部上下文时表现不佳，需要增强捕捉和保持远距离关节上下文信息的能力。

Method: 提出混合GCN-Mamba模型Parts-Mamba，通过部件特定扫描特征捕捉部件特定信息，利用部件-身体融合模块保持非邻近关节上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效处理不完整骨架问题，通过增强上下文信息捕捉能力显著提升骨架动作识别性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein（JGW）目标，将传统的GW距离框架扩展到支持多对一和多对多对象匹配，提供了部分同构分布的非负差异度量，并在点云对象表示中实现了高效求解。


<details>
  <summary>Details</summary>
Motivation: 传统的Gromov-Wasserstein距离仅限于单对象间的成对匹配，无法满足需要多对一或多对多对象匹配的应用场景需求。

Method: 通过引入JGW目标扩展GW框架，支持集合间的同时匹配，并采用最优传输中的传统算法（包括熵正则化）来求解点云对象表示。

Result: 与其他GW变体相比，该方法在准确性和计算效率上表现更优，在合成和真实数据集上的实验验证了其在多形状匹配（包括几何形状和生物分子复合物）中的有效性。
...
TL;DR: 本文提出REPA方法，通过在逆问题求解过程中对齐扩散模型与预训练自监督编码器（如DINOv2）的内部表示，显著提升了重建质量和感知真实性，同时减少了所需的离散化步骤。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，在生成模型中强制对齐扩散模型与预训练自监督编码器的内部表示能提供强大的归纳偏置，改善收敛性和样本质量。本文旨在将此思想扩展到逆问题领域，利用预训练生成模型作为先验。

Method: 提出REPA方法，在推理时对齐扩散模型或基于流的模型与预训练自监督视觉编码器（如DINOv2）的表示。尽管逆问题中缺乏真实信号，但通过将模型表示与近似目标特征对齐来指导重建过程。

Result: 在超分辨率、框内修复、高斯去模糊和运动去模糊等任务上的广泛实验表明，该方法能一致提升重建质量，同时通过减少所需离散化步骤提供显著的效率增益，且不影响底层求解器的性能。

Conclusion: REPA方法通过表示对齐有效提升了逆问题求解的感知保真度，理论分析揭示了REPA正则化与DINOv2嵌入空间中散度度量的关系，以及REPA更新如何引导模型内部表示朝向干净图像表示。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视频时空推理任务。R-AVST包含5K个未修剪视频和27K个对象，覆盖100种音频-视频事件类型；AVST-Zero采用强化学习方法直接优化模型行为。
...
Method: 1) 构建R-AVST数据集：使用基于LLM的关键对象提取、自动空间标注和人工质量检查的流水线；2) 提出AVST-Zero模型：基于强化学习，避免中间监督，通过精心设计的多维度奖励直接优化行为。

Result: 实验验证了R-AVST数据集在推进音频-视频时空推理方面的有效性，AVST-Zero模型相比现有模型表现出有竞争力的性能。生成了超过8K个高质量、均匀分布的问题-答案对用于基准测试。
...
TL;DR: 本文提出了Warm Diffusion，一种统一的模糊-噪声混合扩散模型，通过联合控制模糊和噪声来整合热扩散和冷扩散的优点，解决热扩散在早期生成步骤中的随机行为和冷扩散的数据流形外问题。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，未能利用图像高频细节与低频结构之间的强相关性，导致早期生成步骤中的随机行为；而冷扩散仅使用模糊，忽略了噪声在塑造数据流形中的作用，导致性能下降。需要整合两者的优势。

Method: 提出Warm Diffusion（BNMD模型），采用分治策略，利用图像中的频谱依赖性，通过解耦去噪和去模糊过程来简化分数模型估计，并分析模糊-噪声比来研究模型学习动态与数据流形变化之间的权衡。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成中的有效性。

Conclusion: Warm Diffusion通过联合控制模糊和噪声，成功整合了热扩散和冷扩散的优点，为图像生成提供了一种有效的统一框架。
...
TL;DR: DeltaDeno是一种无需训练、零样本的异常生成方法，通过对比两个扩散分支的差异来定位和编辑缺陷，使用最小提示对在共享调度下操作，生成逼真的局部缺陷。
...
Method: 提出Delta-Denoising方法，通过对比两个扩散分支的差异来生成图像特定的定位图，指导潜在修复过程。包括token级提示优化来对齐共享内容并强化异常token，以及在预测区域应用空间注意力偏置。

Result: 在公开数据集上的实验表明，DeltaDeno实现了出色的生成效果、真实性和下游检测性能的一致提升。

Conclusion: DeltaDeno是一种有效的零样本异常生成方法，能够在无需训练的情况下生成逼真的异常样本，并提升下游检测任务的性能。
...
TL;DR: 本文提出了一种基于扩散模型的视频超分辨率方法DGAF-VSR，通过光流引导的对齐模块和特征域时序条件模块，解决了现有方法中的误差累积、空间伪影以及感知质量与保真度之间的权衡问题。
...
Method: 提出了DGAF-VSR方法，包含光流引导的对齐模块（OGWM）来保持对齐特征中的高频细节，以及特征域时序条件模块（FTCM）在特征域提供密集指导。

Result: 在合成和真实世界数据集上的实验表明，DGAF-VSR在感知质量（DISTS降低35.82%）、保真度（PSNR增益0.20 dB）和时间一致性（tLPIPS降低30.37%）方面均优于现有最先进方法。

Conclusion: DGAF-VSR通过改进帧对齐和特征补偿机制，在视频超分辨率任务中实现了更好的感知质量、保真度和时间一致性。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中牙齿的准确分割对数字化牙科至关重要，但邻牙粘连会导致严重的解剖形状失真，现有方法难以处理。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在时空视频定位任务中与现实需求存在显著差距，主要由于基准测试范围有限，导致模型存在类别偏见、推理过度简化以及语言鲁棒性差的问题。

Method: 提出Forward-Backward-Refinement标注流程，结合多方向跟踪和智能错误校正；提出DeepSTG系统评估框架量化数据集质量；提出PG-TAF训练免费两阶段框架，将STVG分解为高级时间定位和细粒度时空传播。

Result: 在复杂真实场景中性能平均下降10.4%，特别是在小/遮挡物体和复杂空间关系上表现不佳。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试揭示了当前STVG模型的局限性，PG-TAF框架通过分解任务有效提升了时空视频定位性能，为复杂真实场景下的物体定位提供了有效解决方案。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理风险的基准，提出了隐私感知与推理框架，构建了包含合成个体档案的双语多模态数据集，评估了50多个基础模型和商业模型，揭示了现有模型存在重大未测量的推理型隐私风险。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型展现出复杂推理能力，带来了超越简单属性感知的个体级链接隐私风险。现有隐私基准在结构上不足以应对这种新威胁，主要评估隐私感知而未能解决更关键的隐私推理风险。

Method: 提出了隐私感知与推理框架，构建了包含合成个体档案的双语多模态数据集，其中标识符与敏感属性精心链接。设计了9个挑战性任务来评估完整的PPR谱系，从属性检测到跨图像重识别和链式推理。

Result: 大规模评估50多个模型发现：(1)许多模型存在重大未测量的推理型隐私风险；(2)感知级指标无法有效预测推理风险；(3)现有安全对齐对此类推理型攻击不一致且无效。
...
<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预先估计的光学流，但在欠采样情况下不准确且会降低重建质量。

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光学流，通过光学流方程耦合作为物理正则化，并结合数据一致性损失。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确的运动估计和改进的时间保真度。

Conclusion: 隐式联合建模与流正则化约束在推进动态MRI重建方面具有潜力。
...
TL;DR: 本文提出了FingerCap任务，旨在生成捕捉手部动作详细手指级语义的文本描述，并构建了包含4万对手部动作视频和字幕的大规模数据集FingerCap-40K。为了解决时间稀疏性问题，提出了FiGOP方法，通过结合RGB关键帧和后续手部关键点来恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部动作对于视觉感知、具身智能和多模态通信至关重要。当前视频多模态大语言模型在捕捉细微、高频的手指动态方面存在时间稀疏性瓶颈。

Method: 提出了FiGOP方法，将每个RGB关键帧与后续手部关键点配对，直到下一个关键帧。使用轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征集成。FiGOP将经典GOP概念适应于手指运动，在不增加RGB密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，强大的开源和闭源视频多模态大语言模型在手指级推理方面仍然困难，而FiGOP增强模型在HandJudge和人类研究中均获得一致增益。

Conclusion: FiGOP方法有效解决了视频多模态大语言模型在手指级手部动作理解中的时间稀疏性问题，显著提升了细粒度手部动作描述的生成质量。
...
TL;DR: 本文提出了一种基于点监督的面部表情定位方法，使用单时间戳标注训练，通过高斯实例自适应强度建模和双分支框架实现表情实例识别和分类。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖完整的时间边界标注，成本高且耗时。本文研究点监督面部表情定位，仅需每个实例的单时间戳标注，降低标注成本。

Method: 提出双分支框架：1）类无关表情强度分支，使用高斯实例自适应强度建模模块生成软伪标签；2）类感知顶点分类分支，基于伪顶点帧区分宏表情和微表情。还引入强度感知对比损失增强特征学习。

Result: 在SAMM-LV、CAS(ME)$^2$和CAS(ME)$^3$数据集上的大量实验证明了所提框架的有效性。

Conclusion: 提出的点监督面部表情定位框架通过软伪标签和双分支设计，在减少标注成本的同时实现了准确的表情定位和分类。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR贴图的5帧序列，使用视频扩散架构统一处理多种材料任务，在质量和多样性上显著超越现有方法。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量：RGB外观和四个PBR贴图，将其表述为5帧序列并使用视频扩散架构来捕捉相关性。

Result: 在MatHybrid-410K混合数据集上训练，实现了原生1024×1024分辨率合成，在质量和多样性上大幅超越现有方法。

Conclusion: MatPedia提供了一个统一的框架，能够处理文本到材料生成、图像到材料生成和本征分解等多种材料任务，解决了现有方法的碎片化问题。
...
TL;DR: 提出了一种面向边缘设备的烹饪食物图像合成方法，通过配方和烹饪状态引导生成器，在保持时间一致性和烹饪合理性的同时，显著提升了生成图像的真实性。


<details>
  <summary>Details</summary>
Motivation: 解决在边缘设备上从原始食物图像合成逼真烹饪图像的问题，现有方法要么生成结果不真实，要么计算资源需求过高，无法在边缘设备上部署。

Method: 引入了首个基于烤箱的烹饪进程数据集，包含厨师标注的熟度级别；提出了边缘高效的配方和烹饪状态引导生成器；设计了领域特定的烹饪图像相似度（CIS）度量作为训练损失和进度监控信号。
...
TL;DR: 本文提出SeeCLIP框架解决开放集领域泛化问题，通过语义增强的提示学习和双工对比学习来平衡已知类结构风险和未知类开放空间风险，特别针对与已知类视觉相似的'硬未知'类别。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理领域偏移和未知类别时面临结构风险与开放空间风险的权衡困境，特别是在区分与已知类具有细粒度视觉相似性的'硬未知'类别时容易产生过度自信问题。

Method: 提出语义增强的CLIP框架：1）语义感知提示增强模块将图像分解为判别性语义标记；2）双工对比学习通过排斥和凝聚目标定位未知提示；3）语义引导扩散模块通过扰动语义标记合成伪未知样本。

Result: 在五个基准测试上实现了一致性改进，相比最先进方法准确率提升3%，H-score提升5%。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集领域泛化中的风险平衡问题，特别是在处理'硬未知'类别方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯原语来拟合场景，导致显著的存储和计算开销。现有的修剪方法依赖手动设计的标准或引入额外的可学习参数，效果欠佳。

Method: 提出自然选择启发的修剪框架，将生存压力建模为不透明度的正则化梯度场；引入具有有限不透明度先验的不透明度衰减技术，加速选择过程。

Result: 与3DGS相比，在15%预算下实现超过0.6 dB PSNR增益，在紧凑3DGS中达到最先进性能。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中进行身份替换来实现人脸去识别，使用化学置换机制启发的方法在h空间进行语义位移，并通过优化发现身份编辑方向。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间难以取得良好平衡，需要一种更有效的方法来保护隐私同时保持图像质量。

Method: 使用预训练无条件扩散模型的潜在h空间，通过优化引导发现身份编辑方向，提出线性和测地线编辑方案来导航潜在流形，使用新型试剂损失函数监督属性保留和身份抑制。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留之间实现了优越的权衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过潜在空间的身份置换有效解决了人脸去识别问题，为隐私保护提供了新的解决方案。
...
TL;DR: DepthFocus是一个可引导的视觉Transformer，将立体深度估计重新定义为意图驱动的控制，能够根据深度偏好动态调整计算，在复杂场景中实现选择性感知。
...
Method: 引入DepthFocus，一个可引导的视觉Transformer，通过标量深度偏好条件化，动态调整计算以聚焦于目标深度。训练主要使用新构建的50万多层合成数据集，捕捉多样透射效果。

Result: 在BOOSTER等传统单深度基准上达到最先进性能，在新提出的真实和合成多深度数据集上定量展示意图对齐估计，并在未见透射场景上表现出强泛化能力。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在混合文化线索下稳健性的对抗性测试套件，发现现有模型在简单图像堆叠扰动下准确率大幅下降，注意力会系统性地转向干扰线索。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的文化维度评估通常只关注单一文化概念，忽略了多文化线索共存场景，而模型在这种情况下的稳定性对支持多元文化社会至关重要。

Method: 开发了ConfusedTourist文化对抗稳健性测试套件，通过图像堆叠和基于图像生成的扰动来测试VLMs对地理线索变化的稳定性。

Result: 实验显示VLMs在简单图像堆叠扰动下准确率大幅下降，基于图像生成的变体效果更差；可解释性分析表明失败源于注意力系统性地转向干扰线索。

Conclusion: 视觉文化概念混合会严重损害最先进的VLMs性能，迫切需要开发更具文化稳健性的多模态理解模型。
...
TL;DR: RacketVision是一个用于体育分析的新型数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态标注和传统球位置标注，支持复杂人机交互研究。


<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，解决现有数据集缺乏细粒度球拍姿态标注的问题，支持复杂的人-物交互分析。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态细粒度标注和球位置标注，设计三个相互关联的任务：细粒度球跟踪、关节式球拍姿态估计和预测性球轨迹预测。
...
<details>
  <summary>Details</summary>
Motivation: 虽然LVLMs在图像级任务中表现出色，但在实例级任务（如视觉定位和对象检测）中仍存在性能差距。同时，行人跟踪与自然语言结合的新任务（如Referring MOT等）需要模型在高级语义层面理解跟踪对象，这正是LVLMs的优势所在。

Method: 提出OmniPT框架，采用RL-Mid Training-SFT-RL训练阶段：基于预训练的LVLM权重，首先进行RL阶段使模型输出固定且可监督的边界框格式；然后使用大量行人相关数据集进行中期训练；最后在几个行人跟踪数据集上进行监督微调，再进行另一个RL阶段以提高跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提出的方法能够比之前的方法表现更好。

Conclusion: OmniPT框架成功地将行人跟踪任务建模为基础模型可执行的任务，并通过多阶段训练策略实现了优于现有方法的性能，特别是在语义理解和指令跟随方面表现出色。
...
TL;DR: ReBrain是一个基于检索增强扩散的框架，用于从稀疏CT扫描重建脑部MRI。它结合了布朗桥扩散模型和检索机制，通过从先验数据库中检索相似CT切片作为参考，确保结构连续性，并在检索失败时使用球面线性插值提供补充指导。


<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI检查。现有的从CT合成MRI的方法在低剂量协议下效果不佳，因为稀疏的CT体积导致重建完整脑部MRI体积具有挑战性。

Method: 提出ReBrain框架：1) 使用布朗桥扩散模型在2D维度合成MRI切片；2) 通过微调检索模型从先验数据库检索结构和病理相似的CT切片作为参考；3) 使用ControlNet分支整合检索切片指导中间MRI切片生成；4) 在检索失败时应用球面线性插值提供补充指导。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中实现了最先进的性能。

Conclusion: ReBrain通过结合扩散模型和检索机制，有效解决了从稀疏CT扫描重建脑部MRI的挑战，在稀疏条件下实现了优异的跨模态重建性能。
...
TL;DR: STAR建模通过利用图像先验知识（如中心偏差和局部性）来保持采样性能，同时提供足够灵活的序列顺序以支持推理时的图像编辑。
...
Method: 使用在图像补丁位置定义的格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保图像连接部分观察作为序列前缀。

Result: STAR在保持采样性能的同时，提供了后补完成能力，且无需对广泛采用的语言AR模型架构进行重大更改。

Conclusion: STAR通过定制但结构化的随机化策略，相比随机排列更好地平衡了采样性能和序列顺序灵活性。
...
TL;DR: REArtGS++提出了一种基于平面高斯分布和时间几何约束的通用铰接物体重建方法，改进了REArtGS在螺旋关节和多部件物体上的不足。
...
Method: 使用解耦螺旋运动建模关节，通过部件运动混合联合优化部件感知的高斯分布和关节参数，引入平面高斯和时间一致性正则化来施加时间连续几何约束。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在可泛化的部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时间几何约束和平面高斯分布，实现了更通用和准确的铰接物体重建与关节参数估计。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要使用单次规划或迭代精炼，前者限于简单运动，后者计算成本高。需要一种既能生成高质量运动规划又高效的方法。

Method: 提出基于草图验证的规划框架：预测多个候选运动计划，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代精炼直到找到满意的运动计划。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。消融研究显示增加轨迹候选数量能持续提升性能。

Conclusion: SketchVerify框架通过测试时采样和验证循环，在保持高效率的同时显著提升了运动规划质量，为视频生成提供了更好的运动控制信号。
...
TL;DR: DiverseVAR是一种无需额外训练即可恢复VAR模型生成多样性的方法，通过抑制输入中的关键组件并放大输出中的关键组件来提升多样性，同时保持高保真度合成。
...
Method: 通过分析发现特征图中的关键组件是早期尺度多样性形成的关键因素，DiverseVAR通过抑制输入中的关键组件并放大输出中的关键组件来恢复生成多样性。

Result: 实验结果表明该方法显著增强了生成多样性，且对性能影响可忽略不计。

Conclusion: DiverseVAR有效释放了VAR模型的固有生成潜力，在保持高保真度合成的同时解决了多样性崩溃问题。
...
TL;DR: 本文提出了一种基于平面高斯抛光的类别无关铰接物体重建框架，仅需单状态下的稀疏视角RGB图像即可实现高保真度的部件级表面重建。
...
Conclusion: 该方法仅使用稀疏视角RGB图像即可实现高质量的铰接物体重建，代码将公开提供。
...
Method: 提出ReCoVAD框架，包含两个核心通路：(i)反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；(ii)意识通路使用中等规模视觉语言模型生成文本事件描述和精炼异常分数，持续更新记忆和原型提示，并集成大语言模型定期审查积累的描述。

Result: 在UCF-Crime和XD-Violence数据集上，ReCoVAD实现了最先进的免训练性能，同时仅处理之前方法所用帧数的28.55%和16.04%。
...
TL;DR: 本文提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本三种路径的相互作用，并首次发现模型依赖不同路径取决于问答对齐格式。基于此提出了识别和干预关键幻觉头的方法。
...
Method: 提出了与transformer因果架构对齐的综合干预框架，分析不同干预路径对幻觉的影响。发现幻觉源于三种路径的相互作用，并基于此提出了针对判别式和生成式格式的简单有效方法，识别和干预关键幻觉头。

Result: 在多个基准测试上的实验表明，该方法能够持续减少各种对齐类型下的幻觉。

Conclusion: LVLMs的幻觉问题并非源于单一因果路径，而是多种路径相互作用的结果。通过针对不同问答对齐格式设计相应的干预策略，可以有效减少模型幻觉。
...
TL;DR: 提出一种基于语义掩码引导编辑真实LiDAR扫描的新方法，通过扩散模型生成合成LiDAR点云，用于增强自动驾驶系统的训练数据多样性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样的点云数据，特别是边缘案例场景。从真实世界获取这些数据具有挑战性，而现有模拟方法耗时且难以捕捉真实场景复杂性。

Method: 将点云转换为2D距离视图图像，使用基于凸包的语义掩码作为条件引导，实现基于扩散的生成方法，确保几何一致性和真实感。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态推理模型在生成长推理链时存在冗余自反思问题，而基于静态视觉参考的CoT压缩方法对多模态推理增益有限。

Method: ChainV首先基于前一步推理进行粗略视觉补丁选择，然后根据平均注意力强度识别最具代表性的原子视觉提示，并通过一致性评估机制评估提示可靠性，指导模型自适应调整自反思水平。

Result: 在MathVista基准测试中，ChainV在MIMO-VL-RL上实现了2.3%的准确率提升，同时推理延迟降低51.4%，输出token长度缩短24.5%。

Conclusion: ChainV通过动态整合视觉提示，显著提高了多模态推理的准确性和效率，特别是在需要视觉提示进行多步符号推理的数学密集型任务中表现突出。
...
### [44] [PEGS: Physics-Event Enhanced Large Spatiotemporal Motion Reconstruction via 3D Gaussian Splatting](https://arxiv.org/abs/2511.17116)
*Yijun Xu,Jingrui Zhang,Hongyi Liu,Yuhan Chen,Yuanyang Wang,Qingyao Guo,Dingwen Wang,Lei Yu,Chu He*

Main category: cs.CV

TL;DR: PEGS是一个结合物理先验和事件流增强的3D高斯泼溅框架，用于解决大时空尺度刚性运动重建中的运动模糊和物理一致性不足问题。


<details>
  <summary>Details</summary>
Motivation: 大时空尺度刚性运动重建面临建模范式限制、严重运动模糊和物理一致性不足的挑战。

Method: 提出三重监督方案：加速度约束确保物理合理性，事件流提供高时间分辨率指导，卡尔曼正则器融合多源观测；设计运动感知模拟退火策略自适应调度训练过程；构建首个RGB-事件配对数据集。

Result: 实验表明PEGS在大时空尺度运动重建方面优于主流动态方法。

Conclusion: PEGS框架通过整合物理先验和事件流增强，成功实现了大时空尺度下的去模糊目标聚焦建模和运动恢复。
...
### [45] [Off the Planckian Locus: Using 2D Chromaticity to Improve In-Camera Color](https://arxiv.org/abs/2511.17133)
*SaiKiran Tedla,Joshua E. Little,Hakki Can Karaimer,Michael S. Brown*

Main category: cs.CV

TL;DR: 本文提出从1D相关色温(CCT)转向2D色度空间，并使用轻量级多层感知器(MLP)替代传统CCT插值，以改善非普朗克光源(如LED)下的比色映射精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的比色映射方法主要针对普朗克光源设计，而现代LED光源可能显著偏离普朗克轨迹，暴露了依赖传统一维CCT进行光照表征的局限性。

Method: 采用基于2D色度特征的轻量级MLP替代传统CCT插值，通过包含代表性LED光源的光箱校准程序训练MLP模型。

Result: 在多种LED照明条件下验证，该方法平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署，计算成本可忽略不计。
...
TL;DR: 本文提出MuM模型，通过将掩码自编码扩展到任意多视图，使用统一掩码和轻量级解码器，在3D视觉任务上超越了DINOv3和CroCo v2。
...
Method: 将掩码自编码扩展到任意多视图，对所有视图进行统一掩码，使用带有帧间注意力的轻量级解码器。

Result: 在包括前馈重建、密集图像匹配和相对姿态估计等下游任务中，MuM模型的表现优于最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过扩展MAE到多视图并简化架构，可以更有效地学习3D视觉特征，在多个3D视觉任务上取得state-of-the-art性能。
...
TL;DR: 提出了一个多阶段优化框架，将高性能的浮点图像压缩模型转化为高效的硬件友好型整数实现，包括动态范围感知量化、混合精度搜索和通道剪枝技术。
...
Method: 引入噪声混合视觉流设计：新视觉流接收带可调噪声的低质量图像，原始视觉流接收带一致噪声的低质量图像；采用保真度感知对抗训练实现单步推理。
...
TL;DR: 提出了CC-DiceCE损失函数来解决医学图像分割中传统Dice损失对小病灶欠分割的问题，通过基于CC-Metrics框架的实例级评估，在nnU-Net框架下与blob loss进行对比。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）由于小病灶相对体积较小而对整体损失贡献可忽略，导致对小病灶的欠分割问题。

Method: 基于CC-Metrics框架开发CC-DiceCE损失函数，在nnU-Net标准化框架下与blob loss和DiceCE基线进行对比评估。

Result: CC-DiceCE损失提高了病灶检测率（召回率），分割性能仅有最小程度下降，但会产生稍多的假阳性。在多数据集研究中，CC-DiceCE通常优于blob loss。

Conclusion: CC-DiceCE损失函数能有效提升小病灶的检测性能，在保持分割质量的同时提高召回率，是处理医学图像中小病灶分割问题的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，存在改进空间。作者希望结合判别式和生成式方法的优势来提升轨迹预测性能。
...
Conclusion: DiffRefiner通过结合判别式轨迹提议和生成式精炼，显著提升了扩散基规划的轨迹预测性能，证明了该混合方法的有效性。
...
TL;DR: UI-Styler是一种新型超声图像风格迁移框架，通过模式匹配和类别感知提示策略，在保留源图像结构内容的同时实现跨设备的语义对齐，显著提升下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 解决超声图像在不同采集设备间存在的域偏移问题，特别是在固定黑盒下游推理模型重用场景下，现有无配对图像翻译方法缺乏类别特定的语义对齐，影响诊断准确性。

Method: 提出UI-Styler框架，包含模式匹配机制将目标图像纹理模式迁移到源图像，以及基于目标域伪标签的类别感知提示策略，确保诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）中持续优于现有UIT方法，达到最先进性能。

Conclusion: UI-Styler通过有效的风格迁移和语义对齐机制，成功解决了超声图像跨设备域适应问题，为医学图像分析提供了实用的解决方案。
...
TL;DR: FireScope是一个基于视觉语言模型(VLM)的推理生成框架，用于预测野火风险地图，通过结合Sentinel-2影像和气候数据，在跨大陆评估中展现出优异的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有野火风险预测方法缺乏因果推理和多模态理解能力，难以实现可靠的泛化。需要开发能够整合视觉、气候和地理因素进行连续风险地图推理的方法。

Method: 提出FireScope-Bench大规模数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格。开发基于VLM的推理生成框架，通过强化学习和视觉监督学习预测风险栅格并生成互补的推理轨迹。
...
Method: 系统评估自监督特征在不同模态（音频、视频、多模态）和领域（唇部运动、通用视觉内容）的表现，分析三个关键维度：检测效果、编码信息的可解释性和跨模态互补性。

Result: 发现大多数自监督特征能捕获深度伪造相关信息且具有互补性，模型主要关注语义上有意义的区域而非虚假伪影，但无法可靠跨数据集泛化。
...
TL;DR: PostCam是一个用于新颖视角视频生成的框架，支持动态场景中相机轨迹的后捕捉编辑，通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，实现精确的相机运动控制和高保真视频生成。
...
Method: 提出查询共享交叉注意力模块，融合6-DoF相机位姿和2D渲染视频帧控制信号；采用两阶段训练策略：先学习粗略相机控制，再结合视觉信息细化运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性方面比现有最优方法提升超过20%，同时达到最高的视频生成质量。

Conclusion: PostCam通过统一的特征表示和两阶段训练策略，成功实现了精确的相机运动控制和高质量的视频生成，显著优于现有方法。
...
Method: 设计4D感知视觉表示（将1D时间嵌入3D位置形成4D嵌入）和时空动作表示（扩展传统空间动作表示包含时间信息），通过交叉注意力机制融合多模态表示到LLM中进行时空动作预测。

Result: 在机器人操作任务中验证了方法的优越性。

Conclusion: VLA-4D模型通过统一的视觉和动作表示框架，实现了机器人操作的空间平滑性和时间一致性。
...
TL;DR: 本文提出CA-SAM方法，通过轻量级的对齐层模块和持续学习策略，在医疗图像分割任务中平衡SAM模型的计算效率和性能，解决异构隐私政策下的联合训练问题。


<details>
  <summary>Details</summary>
Motivation: 医疗图像分割中，机构间的异构隐私政策使得联合训练不可行，需要在不产生灾难性遗忘的情况下从数据流中持续学习。SAM模型虽然提供强大的零样本先验，但参数量大、计算开销高，限制了实际部署。

Method: 提出对齐层（Alignment Layer）轻量级模块，对齐编码器-解码器特征分布以高效适配SAM到特定医疗图像；基于此提出CA-SAM持续学习策略，自动适配对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验保持对未见医疗数据集的强性能。

Result: 在九个医疗分割数据集的持续学习场景下进行实验，CA-SAM实现了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM方法通过轻量级对齐层和持续学习策略，有效解决了医疗图像分割中的实际部署挑战。
...
Method: 首先通过轻量级跟踪和重建模块构建局部一致的子图，然后逐步对齐并融合到全局高斯地图中，强制跨视角几何一致性，全局地图反馈纠正局部漂移并增强跟踪鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 体积CT面临极端token缩放、几何各向异性和弱监督等独特挑战，使得标准Transformer和对比学习方法难以直接应用，需要开发专门的基础模型。
...
Conclusion: SPECTRE是一个可扩展、开放、完全基于Transformer的3D医学影像基础模型，能够学习几何一致且临床有意义的特征表示。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接学习连续3D语义占据，在保持恒定内存的同时实现长距离监督和推理，在自监督Occ3D-nuScenes基准上比之前方法提升26%语义RayIoU。
...
Method: 引入基于查询的自监督框架，通过独立4D时空查询在相邻帧间采样，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，采用压缩场景表示在保持近场细节的同时平滑压缩远距离区域。

Result: 在自监督Occ3D-nuScenes基准上，语义RayIoU比之前相机方法提升26%，运行速度为11.6 FPS，证明直接4D查询监督能够实现强大的自监督占据学习。

Conclusion: 直接4D查询监督能够实现强大的自监督占据学习，QueryOcc框架在3D场景几何和语义学习方面表现出色，为自动驾驶等应用提供了有效的解决方案。
...
TL;DR: 提出了一种基于四元数保真项的颜色图像盲去卷积方法，通过四元数卷积核建模颜色通道间的相互依赖关系，有效提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转为灰度图或单独处理每个颜色通道，忽略了颜色通道之间的关系，这限制了彩色图像盲去卷积的性能。

Method: 设计了一个新颖的四元数保真项，利用四元数卷积核的特性，包含一个捕获整体模糊的非负卷积核和三个无约束的对应RGB通道的卷积核来建模颜色通道间的未知相互依赖关系，并在盲去卷积过程中使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法能有效去除伪影，显著提升去模糊效果。

Conclusion: 该方法作为彩色图像去卷积的强大工具具有很大潜力，通过四元数建模颜色通道间关系能有效改善盲去卷积性能。
...
TL;DR: 本文提出四种相关性反馈策略来改进视觉语言模型的检索性能，包括伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈，实验表明这些方法能提升3-5%的检索性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本的搜索中的相关性反馈机制可以改进视觉语言模型的检索性能，避免需要微调和扩大模型规模，提供模型无关的改进方案。

Method: 提出四种反馈策略：1）伪相关性反馈（PRF）基于排名靠前结果优化查询嵌入；2）生成式相关性反馈（GRF）使用合成标题进行查询优化；3）注意力反馈总结器（AFS）整合多模态细粒度特征；4）使用真实标题作为显式反馈的上界基准。

Result: 在Flickr30k和COCO数据集上的实验显示，GRF、AFS和显式反馈相比无反馈检索，在较小VLM上提升MRR@5指标3-5%，在较大VLM上提升1-3%。AFS与显式反馈类似，能缓解查询漂移并在多轮检索中比GRF更稳健。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，通过预训练、多粒度微调和强化学习优化，在立体化学信息识别方面取得突破性进展。


<details>
  <summary>Details</summary>
Motivation: 现有OCSR系统在准确识别立体化学信息方面存在挑战，特别是楔形键、环构象等细微视觉线索的区分。需要开发能够精确识别立体异构体的新方法。

Method: 采用三阶段训练范式：1)大规模噪声数据集预训练建立基础感知能力；2)多粒度微调探索化学键分类和原子定位辅助任务；3)强化学习后训练优化，引入新的立体化学结构数据集。

Result: MolSight在多样化数据集上的实验表明，即使参数规模相对紧凑，通过GRPO算法也能进一步提升立体分子识别性能，在(立体)化学光学结构识别方面达到最先进水平。

Conclusion: MolSight框架通过创新的三阶段训练策略，成功解决了OCSR中立体化学信息识别的关键挑战，为化学信息学和大规模数据挖掘提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要局限于估计俯仰角和偏航角，且在大角度输入时精度下降。需要一种能够可靠估计滚转角并提升其他姿态参数预测性能的方法。

Method: 提出双模态输入方法，包括来自触摸屏的电容图像和屏下指纹传感器获取的指纹补丁。通过融合这两种模态信息实现全面的指尖姿态估计。
...
TL;DR: 本文提出SpatialGeo方法，通过融合几何和语义特征来解决多模态大语言模型在空间推理能力上的不足，显著提升了空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在三维空间的空间推理能力有限，主要原因是视觉编码器（如CLIP）的嵌入损失了空间信息，仅关注实例级语义特征。

Method: 提出基于几何和语义特征分层融合的新型视觉编码器，通过分层适配器将自监督学习的几何特征补充到CLIP中，使用随机特征丢弃避免仅依赖CLIP编码器的平凡解。

Result: SpatialGeo在空间推理任务中显著提升准确率，在SpatialRGPT-Bench上将最先进模型的准确率提高了至少8.0%，推理时内存成本减少约50%。

Conclusion: 通过融合几何和语义特征的方法有效增强了MLLMs的空间感知能力，在保持高效性的同时显著提升了空间推理性能。
...
Method: 采用相对姿态初始化与显式特征匹配，进行局部联合优化提升姿态估计，然后引入包含几何一致性约束的全局优化阶段，通过束调整和特征轨迹进一步优化姿态。
...
<details>
  <summary>Details</summary>
Motivation: 生成图像模型在处理透明物体时表现不佳，特别是折射效果，因为折射光线会与图像其他部分表面相交形成颜色约束。现有模型未能充分学习光学定律来准确渲染折射物体。

Method: 在生成过程的每一步中，使用斯涅尔折射定律对物体边界内外像素进行扭曲和合并同步。对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图进行同步来恢复其外观。

Result: 该方法生成的图像在光学上更加合理，能够更好地遵守物理约束条件。

Conclusion: 通过同步像素和应用光学物理定律，可以有效改进生成图像中透明物体的折射渲染质量，生成更符合真实光学效果的图像。
...
TL;DR: 提出了一个统一的多媒体绘画过程生成框架，通过语义驱动的风格控制机制，将多种媒介嵌入扩散模型的条件空间，并使用跨媒介风格增强，实现一致的纹理演化和跨风格的过程转移。
...
Result: 构建了大规模真实绘画过程数据集，在跨媒介一致性、时间连贯性和最终图像保真度方面取得了优异结果，在LPIPS、DINO和CLIP指标上表现强劲。

Conclusion: 提出的感知距离剖面(PDP)曲线能够定量建模创作序列（构图、色块划分和细节精炼），准确反映人类艺术创作进程，为绘画过程生成提供了有效解决方案。
...
TL;DR: DSeq-JEPA提出了一种判别性顺序联合嵌入预测架构，通过识别主要判别区域并按重要性顺序预测后续区域，结合了JEPA的潜在预测和GPT风格的顺序推理，在多个视觉任务中表现出优于I-JEPA的性能。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知启发，传统I-JEPA对所有区域进行统一独立处理，缺乏明确的预测顺序概念。人类视觉会从最具信息量的区域开始选择性、顺序性地部署注意力。
...
Conclusion: DSeq-JEPA成功桥接了预测性和自回归自监督学习，通过引入判别性顺序预测机制显著提升了视觉表示学习的效果。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占据估计方法，相比高斯表示方法减少了84%的基元数量，实现了75%的内存占用降低、124%的推理速度提升和5.9%的mIoU改进，是首个支持实时推理的占据模型。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中广泛使用，但大量高斯基元显著增加内存需求，不适合实时推理。超二次曲面允许减少基元数量和降低内存需求，但缺乏超二次曲面光栅器来支持模型监督。

Method: 提出SuperQuadricOcc方法，采用基于超二次曲面的场景表示。通过利用多层二十面体细分的高斯近似超二次曲面，在训练期间实现高斯光栅化监督。

Result: 在Occ3D数据集上，相比之前的高斯方法，实现了75%的内存占用减少、124%的推理速度提升和5.9%的mIoU改进，无需使用时间标签。基元数量减少了84%。

Conclusion: 这是首个支持实时推理的占据模型，同时保持有竞争力的性能。通过超二次曲面的使用显著减少了场景建模所需的基元数量。
...
TL;DR: 提出ATAC方法，通过在CLIP嵌入空间计算增强诱导漂移向量来推断语义恢复方向，显著提升对抗攻击下的鲁棒性，计算开销小。


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配方面表现出色，但对图像对抗扰动高度脆弱。对抗性微调成本过高，现有测试时防御方法鲁棒性有限。

Method: 在CLIP嵌入空间中操作，计算增强诱导漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法高出近50%，计算开销极小。在非常规和极端设置下保持最先进鲁棒性，甚至对自适应攻击也实现非平凡鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏体素与邻居空间解耦且边界锐利，在优化过程中容易陷入局部最小值，而SDF提供的平滑连续几何场在独立参数化的稀疏体素间难以保持一致性。

Method: 采用视觉几何模型进行鲁棒几何初始化，并通过空间平滑损失在父子体素组和兄弟体素组之间强制执行相干关系。

Result: 在多个基准测试中，该方法实现了强大的重建精度，同时保持了一致的快速收敛速度。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型对小输入扰动仍然脆弱，现有概率鲁棒性(PR)方法假设固定已知的扰动分布，这在实践中不现实。

Method: 基于高斯混合模型(GMM)与多层感知机(MLP)头部和双三次上采样开发NPPR估计器，覆盖输入依赖和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上对ResNet18/50、WideResNet50和VGG16进行广泛实验验证，NPPR相比现有方法提供高达40%更保守(更低)的PR估计。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像配准中高维变形空间的挑战和体素级监督稀缺的问题，现有强化学习方法将空间投影到粗粒度低维表示，限制了捕捉空间变异变形的能力。

Method: 引入随机高斯策略头建模潜在特征分布，结合无监督预热和弱监督微调，通过组相对策略优化实现多轨迹采样稳定训练。
...
### [76] [MCMoE: Completing Missing Modalities with Mixture of Experts for Incomplete Multimodal Action Quality Assessment](https://arxiv.org/abs/2511.17397)
*Huangbiao Xu,Huanqi Wu,Xiao Ke,Junyi Wu,Rui Xu,Jinglin Xu*

Main category: cs.CV

TL;DR: 本文提出了一种名为MCMoE的新型多模态动作质量评估框架，通过专家混合机制解决推理阶段模态缺失问题，在完整和不完整多模态学习中都取得了最先进的结果。
...
### [77] [MMT-ARD: Multimodal Multi-Teacher Adversarial Distillation for Robust Vision-Language Models](https://arxiv.org/abs/2511.17448)
*Yuqi Li,Junhao Dong,Chuanguang Yang,Shiping Wen,Piotr Koniusz,Tingwen Huang,Yingli Tian,Yew-Soon Ong*

Main category: cs.CV

TL;DR: MMT-ARD是一个多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构和动态权重分配策略，显著提升视觉语言模型的对抗鲁棒性和训练效率。


<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗知识蒸馏方法存在知识多样性有限、收敛速度慢以及鲁棒性与准确性难以平衡的问题，需要更有效的多教师协作框架来提升视觉语言模型的对抗鲁棒性。

Method: 提出双教师知识融合架构，分别优化干净特征保留和鲁棒特征增强；引入基于教师置信度的动态权重分配策略；设计自适应sigmoid加权函数来平衡跨模态知识传递强度。

Result: 在ImageNet和零样本基准测试中，ViT-B-32模型的鲁棒准确率提升+4.32%，零样本准确率提升+3.5%，训练效率比传统单教师方法提高2.3倍。

Conclusion: MMT-ARD框架在增强多模态大模型的对抗鲁棒性方面表现出有效性和可扩展性，为安全关键应用中的视觉语言模型部署提供了更可靠的解决方案。
...
### [78] [Counterfactual World Models via Digital Twin-conditioned Video Diffusion](https://arxiv.org/abs/2511.17481)
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，无法回答反事实查询（如"如果移除这个物体会发生什么？"），这在评估AI物理行为等应用中越来越重要。
...
### [79] [Radar2Shape: 3D Shape Reconstruction from High-Frequency Radar using Multiresolution Signed Distance Functions](https://arxiv.org/abs/2511.17484)
...
<details>
  <summary>Details</summary>
Motivation: 解决从高频雷达信号进行3D形状重建的复杂性问题，现有深度学习方法无法表示任意形状或难以处理有限视角采集的真实雷达信号，而光学3D重建方法直接将雷达信号视为相机视图效果不佳。

Method: 采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以雷达信号频率为条件，以从粗到细的方式扩散到该潜在空间。
...
### [80] [An Artificial Intelligence Framework for Measuring Human Spine Aging Using MRI](https://arxiv.org/abs/2511.17485)
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并研究脊柱年龄差距（SAG）与脊柱退行性疾病及生活方式因素的关联。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，易受年龄相关退化的影响。通过MRI识别这些退化，开发能够评估脊柱健康的生物标志物具有重要意义。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退化聚类，建立深度学习模型，通过消融研究确定最佳模型配置，计算脊柱年龄差距（SAG）并与临床条件关联分析。

Result: 研究发现SAG与椎间盘突出、骨赘、椎管狭窄、骨折等脊柱退行性疾病相关，也与吸烟、体力劳动等生活方式因素有关。

Conclusion: 脊柱年龄差距（SAG）可作为衡量整体脊柱健康的有用生物标志物，对临床评估脊柱健康状况具有重要价值。
...
### [81] [Downscaling Intelligence: Exploring Perception and Reasoning Bottlenecks in Small Multimodal Models](https://arxiv.org/abs/2511.17487)
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的规模化发展，实际应用需要更小、更高效的系统。本文旨在系统分析缩小大型语言模型规模如何影响多模态能力，特别是视觉能力。

Method: 采用Extract+Think方法：首先进行视觉提取调优，训练模型一致地提取与指令相关的视觉细节；然后应用逐步推理生成答案。

Result: 研究发现LLM缩小规模对视觉能力的影响比对推理能力的影响更大，视觉提取调优方法显著提升了模型效率和性能。

Conclusion: Extract+Think方法为多模态模型的高效性和性能设定了新标准，通过分离视觉提取和推理过程有效解决了LLM缩小规模带来的视觉能力瓶颈。
...
### [82] [Video-R4: Reinforcing Text-Rich Video Reasoning with Visual Rumination](https://arxiv.org/abs/2511.17490)
*Yolo Yunlong Tang,Daiki Shimada,Hang Hua,Chao Huang,Jing Bi,Rogerio Feris,Chenliang Xu*

Main category: cs.CV

TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大信息区域、重新编码像素来增强文本丰富视频的理解能力，在多个视频QA任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，无法处理需要反复检查的细小瞬态文本线索，导致幻觉和细粒度证据识别失败。受人类暂停、放大和重读关键区域的行为启发。
...
### [83] [Native 3D Editing with Full Attention](https://arxiv.org/abs/2511.17501)
...
Method: 创建大规模多模态数据集用于指令引导的3D编辑，涵盖添加、删除和修改任务。探索两种条件策略：传统交叉注意力机制和新型3D令牌连接方法。

Result: 令牌连接方法更加参数高效且性能更优。广泛评估表明该方法在生成质量、3D一致性和指令保真度方面优于现有的2D提升方法。

Conclusion: 提出的原生3D编辑框架在效率、一致性和质量方面设定了新的基准，为3D内容创作提供了更优的解决方案。
...
### [84] [Towards Hyper-Efficient RAG Systems in VecDBs: Distributed Parallel Multi-Resolution Vector Search](https://arxiv.org/abs/2511.16681)
*Dong Liu,Yanxuan Yu*

Main category: cs.CL

TL;DR: 本文提出了语义金字塔索引（SPI），一种用于向量数据库的多分辨率向量索引框架，通过查询自适应分辨率控制来解决现有RAG系统中检索速度和语义相关性之间的权衡问题。
...
Method: SPI在文档嵌入上构建语义金字塔，通过轻量级分类器为每个查询动态选择最优分辨率级别，实现从粗到细表示的渐进式检索。

Result: 在多个RAG任务上的评估显示，SPI实现了高达5.7倍的检索加速和1.8倍的内存效率提升，同时端到端QA F1分数提高了2.5分。

Conclusion: SPI框架与现有向量数据库基础设施兼容，使其可在生产RAG系统中直接部署，为检索增强生成系统提供了高效的解决方案。
...
### [85] [Bench360: Benchmarking Local LLM Inference from 360°](https://arxiv.org/abs/2511.16682)
*Linus Stuhlmann,Mauricio Fadel Argerich,Jonathan Fürst*

Main category: cs.CL

TL;DR: Bench360是一个用于本地LLM推理的综合性基准测试框架，支持用户自定义任务、数据集和指标，自动测试不同模型、推理引擎和量化级别，并整合系统指标与任务特定指标。
...
Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动测试选定的LLM、推理引擎和量化级别，涵盖单流、批处理和服务器等使用场景，跟踪计算性能、资源使用、部署和任务特定指标。

Result: 在四个常见LLM任务、三个硬件平台和四个推理引擎上的测试揭示了任务性能与系统效率之间的权衡，不同推理引擎和模型存在显著差异，没有单一最佳设置。
...
### [86] [How Well Do LLMs Understand Tunisian Arabic?](https://arxiv.org/abs/2511.16683)
*Mohamed Mahdi*

Main category: cs.CL

TL;DR: 本研究评估了大型语言模型对突尼斯阿拉伯语的理解能力，发现现有模型在低资源语言处理上存在显著差异，强调了在AI系统中纳入低资源语言的重要性。
...
Method: 引入包含突尼斯阿拉伯语、标准突尼斯阿拉伯语和英语平行翻译及情感标签的新数据集，在三个任务上对多个流行LLM进行基准测试：音译、翻译和情感分析。

Result: 结果显示模型之间存在显著差异，突显了它们在理解和处理突尼斯方言方面的优势和局限性。

Conclusion: 通过量化这些差距，本研究强调了在下一代AI系统中纳入低资源语言的重要性，确保技术保持可访问性、包容性和文化根基。
...
### [87] [Ellipsoid-Based Decision Boundaries for Open Intent Classification](https://arxiv.org/abs/2511.16685)
...
<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类服从各向同性分布，将边界限制为球形，忽略了不同特征方向上的分布方差，限制了开放意图检测能力。

Method: 1) 使用监督对比学习获得已知样本的判别性特征空间；2) 应用可学习矩阵参数化椭圆作为每个已知类的边界；3) 通过设计的双重损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准测试和问题分类数据集上达到最先进性能，椭圆边界的灵活性展现出优越的开放意图检测能力和泛化潜力。

Conclusion: EliDecide方法通过椭圆决策边界有效解决了传统球形边界的局限性，在复杂开放世界场景下具有强大的文本分类任务泛化能力。
...
### [88] [Prompt-Based Value Steering of Large Language Models](https://arxiv.org/abs/2511.16688)
...
Method: 开发了一种实用、可复现且模型无关的程序，使用Schwartz基本人类价值观理论，通过对话数据集进行结构化评估，比较基线提示词和明确基于价值观的提示词。

Result: 实验表明，即使不改变模型或动态优化提示词，价值观引导也是可能的。

Conclusion: 该方法为评估和引导语言模型输出与特定人类价值观对齐提供了一种有效途径，无需模型修改或复杂的动态优化。
...
### [89] [Concept-Based Interpretability for Toxicity Detection](https://arxiv.org/abs/2511.16689)
*Samarth Garg,Deeksha Varshney,Divya Singh*

Main category: cs.CL

TL;DR: 本文提出了一种基于概念梯度的可解释性技术，通过分析毒性检测中的子类型属性（如淫秽、威胁、侮辱等）作为概念，识别导致分类错误的过度归因问题，并引入目标词典集和词-概念对齐分数来量化错误，最后通过无词典增强策略验证模型对更广泛毒性语言模式的归因。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中有害内容的传播日益严重，尽管毒性检测在文本数据方面取得了显著进展，但基于概念的解释在毒性检测中的探索仍然有限。现有方法往往存在概念对目标类的不均衡归因，导致分类错误。

Method: 1. 利用毒性检测数据集中的子类型属性作为概念指标；2. 提出基于概念梯度(CG)的可解释性技术，测量概念变化对模型输出的直接影响；3. 构建目标词典集捕获导致误分类的毒性词；4. 计算词-概念对齐(WCA)分数量化过度归因程度；5. 引入无词典增强策略，生成排除预定义毒性词典的毒性样本。

Result: 该方法能够更因果地解释模型决策，识别出导致分类错误的过度归因问题，并通过WCA分数量化毒性词对误分类的贡献程度。无词典增强策略验证了即使移除显性词汇重叠，模型仍可能对更广泛的毒性语言模式产生过度归因。

Conclusion: 概念梯度方法为毒性检测提供了更深入的可解释性洞察，揭示了模型对毒性概念的过度归因问题。目标词典集和WCA分数有效量化了误分类机制，无词典增强策略表明模型可能学习到超越显性词汇的毒性模式，为改进毒性检测模型提供了重要方向。
...
TL;DR: 该论文研究了AI检测模型对经过AI轻微润色的人类撰写文章的误判问题，构建了两个阿拉伯语数据集来评估14个大型语言模型和商业AI检测器的性能。
...
Result: 所有AI检测器都错误地将大量文章归因于AI。最佳LLM Claude-4 Sonnet准确率为83.51%，但对LLaMA-3轻微润色的文章准确率降至57.63%；最佳商业模型originality.AI准确率为92%，但对Mistral或Gemma-3轻微润色的文章准确率降至12%。

Conclusion: AI检测器对经过AI轻微润色的人类撰写文章存在严重误判问题，性能显著下降，需要改进检测算法以应对这种挑战。
...
TL;DR: 本文复现了基于最近邻的测试时训练方法，使用RoBERTa嵌入和Faiss索引检索20个最近邻，在多个语言模型上验证了该方法能显著降低困惑度和bits-per-byte指标，特别是在结构化数据集上效果最明显。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在语言模型上的有效性，特别是通过最近邻检索进行推理时微调的方法，探索该方法在不同规模和架构模型上的通用性。

Method: 使用预训练的RoBERTa嵌入和Faiss索引检索每个测试输入的20个最近邻序列，对每个邻居应用一次梯度更新，在GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型上进行实验。

Result: 测试时训练显著降低了困惑度和bits-per-byte指标，在GitHub和EuroParl等结构化数据集上改进最大。未在The Pile上预训练的模型受益更多，小模型能接近大模型性能。
...
TL;DR: 本文对多语言大语言模型内部语言信息的表示结构进行了系统分析，发现语言信息在第一个transformer块中急剧分离，并在整个模型深度中保持线性可分性。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型比英语中心模型在中文对齐方面高出4.21倍。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息结构以及跨层涌现方式的系统分析仍然有限。本文旨在通过全面探测研究来理解多语言LLM中语言编码的层间动态和几何结构。
...
TL;DR: 提出了一种基于语言模型的ontology嵌入方法，用于解决SNOMED CT中OOV查询的分层概念检索问题，在评估中优于基线方法。
...
Method: 基于语言模型的ontology嵌入方法，构建了OOV查询标注数据集，测试最直接子类和不太相关祖先的检索。

Result: 该方法在SNOMED CT上的评估中优于SBERT和两种词汇匹配基线方法。

Conclusion: 该方法具有通用性，可以扩展到其他本体，并发布了代码、工具和评估数据集。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中是否编码了共情行为，以及这种特征是否可以被检测和操控，特别是比较安全训练模型与未审查模型的差异。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上测试检测和操控能力，分析激活空间中的线性方向。

Result: 检测准确率极高（AUROC 0.996-1.00），但跨模型探针一致性有限。操控成功率因模型而异：Qwen双向操控成功率达65.3%，Phi-3为61.7%，Dolphin仅支持亲共情操控（94.4%成功率），反共情操控会导致崩溃。

Conclusion: 共情编码独立于安全训练而存在，但安全训练可能影响操控的鲁棒性而非阻止操控。不同模型在检测和操控能力上存在显著差异，需要更多模型验证。
...
TL;DR: 本文提出了一个基于扩展心智理论和激进具身认知的翻译心智ABC框架，将翻译重新定义为整合情感、行为和认知的动态具身活动，而非基于表征的心理模型。
...
Result: 提出了翻译作为社会文化实践中的技能性参与，意义在具身互动中实时共同创造的新理解。

Conclusion: 翻译不是静态语言对应关系的操纵，而是通过大脑-身体-环境互动循环涌现的具身活动，重构了翻译的本质和过程。
...
TL;DR: 本研究重新探讨了不及物动词的句法分类与语义特征的关系，使用可解释维度验证了非作格/非宾格与施事性/终结性之间的关联。


<details>
  <summary>Details</summary>
Motivation: 虽然传统观点认为描述施事行为的动词更倾向于非作格句法，描述终结事件的动词更倾向于非宾格句法，但Kim等人(2024)发现人类对施事性和终结性的评分并不能很好地预测不及物动词的句法行为。

Method: 使用基于施事性和终结性两极种子词计算的可解释维度来重新研究这个问题。

Result: 研究结果支持了非作格/非宾格与施事性/终结性之间的联系，并证明使用可解释维度结合人类判断可以为难以在评分任务中评估的语义属性提供有价值的证据。

Conclusion: 可解释维度与人类判断相结合的方法能够有效揭示不及物动词句法分类与语义特征之间的关系，为语义属性评估提供了新的研究视角。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型容易受到后门攻击，攻击者可通过特定触发器引导模型生成有害内容，需要有效的防御机制。

Method: 提出PEPPER方法，通过重写输入提示为语义不同但视觉相似的描述，并添加不显眼的元素来破坏攻击触发器。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率同时保持生成质量，并能与其他防御方法结合使用。

Conclusion: PEPPER提供了强大且可推广的防御能力，能有效对抗文本到图像扩散模型的后门攻击。
...
TL;DR: 本文提出了一种无需参考标准的度量方法，用于评估大语言模型生成响应的简洁性，通过计算压缩比和词移除压缩来量化非必要内容。
...
Method: 该方法通过三种计算的平均值来量化非必要内容：1）原始响应与LLM抽象摘要之间的压缩比；2）原始响应与LLM提取摘要之间的压缩比；3）词移除压缩，即LLM在保持意义的前提下尽可能移除非必要词语。

Result: 实验结果表明，所提出的度量方法能够识别LLM输出中的冗余，为对话AI系统中响应简洁性的自动评估提供了实用工具，无需人工标注的基准真值。

Conclusion: 本文提出的无参考度量方法能够有效评估LLM响应的简洁性，为自动化评估提供了可行方案，避免了依赖人工标注的需求。
...
TL;DR: 本文提出Soft Concept Mixing (SCM)方法，通过在训练中引入软概念表示来弥合LLMs在软概念推理与离散token训练之间的差距，提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常通过生成离散token进行推理，这限制了其表达能力。虽然软概念推理是很有前景的方向，但LLMs是在离散token上训练的，存在训练与推理方式不匹配的问题。

Method: 提出SCM训练方案：构建软概念向量（嵌入的概率加权平均），将其混合到模型隐藏状态中，并使用强化学习优化整个潜在推理过程。

Result: 在五个推理基准测试上的实验表明，SCM提高了LLMs的推理性能，同时保持了稳定的训练动态。

Conclusion: SCM通过直接暴露软表示给模型，有效弥合了软概念推理与离散token训练之间的差距，提升了LLMs的推理能力。
...
TL;DR: 本研究提出了一种新的训练方案，通过将TRMs的潜在推理视为分类器自由引导和隐式策略改进算法，显著提升了训练效率，在保持质量的同时减少了18倍前向传播次数并消除了停止机制。


<details>
  <summary>Details</summary>
Motivation: 探索如何以最小改动进一步提高小型循环架构（如TRMs）在复杂推理任务上的效率，特别是在抽象推理语料库（ARC）上的表现。

Method: 将TRMs的潜在推理框架化为分类器自由引导和隐式策略改进算法，提出为每个训练循环提供目标的新训练方案。

Result: 方法显著提升训练效率，总前向传播次数减少18倍，消除了停止机制，在仅0.8M参数下在ARC-1上达到24%准确率，优于大多数LLMs。

Conclusion: 提出的训练方案有效提升了TRMs的训练效率，在保持性能的同时大幅减少了计算资源需求，为小型模型在复杂推理任务上的应用提供了新思路。
...
TL;DR: 本研究探讨了训练数据的统计特性与诱导头形成之间的关系，发现批大小和上下文大小的组合可以预测诱导头形成点，表面二元重复频率和可靠性强烈影响诱导头形成，并确定了这两个值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 诱导头被认为是现代语言模型具有出色上下文学习能力的基础，但其形成的精确机制尚不清楚。本研究旨在揭示训练数据统计特性与诱导头形成之间的具体关系。

Method: 通过分析自然和合成数据的统计特性，研究批大小、上下文大小、表面二元重复频率和可靠性等因素对诱导头形成的影响，并建立预测模型。

Result: 发现批大小和上下文大小的简单组合可以预测诱导头形成点；表面二元重复频率和可靠性强烈影响诱导头形成，存在精确的帕累托边界；当频率和可靠性较低时，类别性和边际分布形状变得重要。

Conclusion: 局部依赖关系中的高二元重复频率和可靠性足以促进诱导头形成，但当这些值较低时，需要考虑数据的类别性和分布形状等其他因素。
...
TL;DR: 本文提出了一种新颖的论点感知定量摘要任务，旨在揭示对话中论点的主张-理由结构，并通过ARQUSUMM框架实现该任务，在论点结构识别和定量摘要方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有对话摘要研究虽然考虑了句子间的论证关系，但未能深入揭示句子内部的深层论点结构，无法明确展示论点的claim-reason结构和量化论证强度。

Method: 提出ARQUSUMM框架：1）利用LLM少样本学习识别句子中的命题及其claim-reason关系；2）采用论点结构感知的聚类算法聚合论点并量化其支持度。

Result: 实验表明ARQUSUMM在对话和定量摘要任务上优于现有模型，生成的摘要能更好地表示论点结构，具有更高的文本质量和量化准确性。

Conclusion: ARQUSUMM框架成功实现了论点感知的定量摘要，能够有效揭示对话中的深层论点结构，为用户提供更有帮助的摘要信息。
...
TL;DR: 本研究提出了一种监督微调方法来增强领域特定信息抽取，针对湖南历史名人领域设计了细粒度的模式引导指令模板，并对四个大型语言模型进行了参数高效的指令微调，显著提升了在低资源环境下对湖南历史名人信息抽取的性能。


<details>
  <summary>Details</summary>
Motivation: 系统性的湖南历史名人数据资源有限，通用模型在领域知识抽取和结构化输出生成方面表现不佳，需要解决低资源环境下的信息抽取问题。

Method: 设计针对湖南历史名人领域的细粒度模式引导指令模板，构建指令微调数据集，对Qwen2.5-7B、Qwen3-8B、DeepSeek-R1-Distill-Qwen-7B和Llama-3.1-8B-Instruct四个模型进行参数高效的指令微调。

Result: 所有模型在微调后都表现出显著的性能提升，其中Qwen3-8B表现最佳，在使用100个样本和50次训练迭代的情况下达到了89.3866的分数。

Conclusion: 本研究为垂直领域大语言模型的微调提供了新思路，展示了其在文化遗产知识抽取和知识图谱构建中具有成本效益的应用潜力。
...
TL;DR: 本文构建了视觉说服力判断数据集，引入视觉说服因素分类法，探索了认知引导和知识注入策略，发现VLMs存在召回偏向，高层次的语义对齐是最强预测因子，简单的干预策略效果有限，而基于对象的理性解释能显著提升性能。
...
Result: VLMs存在召回偏向——过度预测高说服力，对低/中层次特征的判别能力较弱；高层次语义对齐是最强预测因子；简单指令或无引导推理支架效果有限，而基于对象的理性解释能显著提升精度和F1分数。
...
<details>
  <summary>Details</summary>
Motivation: 随着对透明度和可解释性需求的增加，自动评分系统需要开发广泛接受的可解释解决方案，以用于大规模真实世界评估。

Method: 开发AnalyticScore框架：1)提取可识别响应元素，2)使用LLM将响应转化为人类可解释的特征值，3)应用直观的有序逻辑回归模型进行评分。

Result: AnalyticScore在评分准确性上优于许多不可解释方法，在ASAP-SAS数据集的10个项目上平均仅比不可解释SOTA方法低0.06 QWK，且特征化行为与人类注释者高度一致。
...
TL;DR: MUCH是首个用于公平评估声明级不确定性量化方法的基准，包含4,873个样本，涵盖四种欧洲语言和四种开源LLM，提供完整生成对数以支持白盒方法开发，并提出高效的确定性分割算法。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型可靠性不足的问题，通过声明级不确定性量化来提升模型可信度，并创建公平、可复现的评估基准。

Method: 构建包含多语言和多模型的数据集，提供24个生成对数，开发确定性分割算法（仅需0.2%生成时间），支持实时监控。

Result: 创建了MUCH基准数据集，包含4,873个样本，涵盖英法西德四种语言和四种开源LLM，分割算法效率显著提升。

Conclusion: 当前不确定性量化方法在性能和效率方面仍有很大改进空间，MUCH基准为未来方法开发提供了公平评估平台。
...
TL;DR: 本文探索使用上下文压缩作为无监督预训练任务来改进LLM的文本表示能力，提出的LLM2Comp模型在多种任务上优于基于token级预测的方法，且数据效率更高。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要是因果模型，专注于下一token预测，不适合生成整体文本表示。虽然已有研究引入预训练任务来适应LLM，但大多依赖token级预测目标，存在局限性。

Method: 采用上下文压缩作为预训练任务，让模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测，并结合对比学习进一步优化表示质量。

Result: 实验表明精心设计的压缩目标能显著提升LLM文本表示，优于基于token级预训练任务的模型。LLM2Comp在广泛任务上超越当代基于LLM的文本编码器，且数据效率更高，需要更少训练数据。

Conclusion: 上下文压缩是一种有效的预训练任务，能够显著提升LLM的文本表示能力，在多个下游任务上表现优异且具有更好的样本效率。
...
TL;DR: 本文介绍了LangMark，一个用于自动后编辑（APE）的多语言数据集，包含20多万个英语到7种语言的三元组（源文本、机器翻译输出、人工后编辑翻译），并证明大语言模型通过少量示例提示可以有效提升机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 尽管神经机器翻译技术不断发展，但缺乏针对NMT输出的大规模多语言数据集阻碍了有效APE系统的开发。

Method: 构建并发布了LangMark数据集，包含206,983个由专业语言学家标注的三元组，涵盖英语到巴西葡萄牙语、法语、德语、意大利语、日语、俄语和西班牙语的翻译。利用该数据集，实证研究了大语言模型通过少量示例提示执行APE的效果。

Result: 研究表明，大语言模型通过少量示例提示可以有效执行自动后编辑，优于领先的商业甚至专有机器翻译系统。
...
TL;DR: 本文介绍了PLLuM项目中用于微调基于Transformer的大语言模型的指令数据集，提出了有机、转换和合成指令的功能类型学，并分享了关于使用人工编写与合成指令数据集在基础LLM语言适应中的观察。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目开发的大语言模型提供指令数据集，并研究不同类型指令在语言模型适应中的影响，为其他LLM的类似数据集开发提供指导。

Method: 使用有机、转换和合成三种类型的指令来构建数据集，并分析人工编写与合成指令在语言适应中的差异。

Result: 开发了PLLuM指令语料库（PLLuMIC）的第一个代表性子集，并分享了关于指令类型对语言模型适应影响的观察。

Conclusion: PLLuMIC数据集可用于指导其他LLM的类似数据集开发，不同类型指令在语言模型适应中具有不同的影响和价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖后生成信号，无法提前防止不可靠响应。LLM经常产生流畅但事实错误的幻觉回答，需要更有效的早期弃权机制。
...
Conclusion: ABCA通过基于方面的因果分析实现了有效的早期弃权，解决了现有方法无法提前防止不可靠响应的问题。
...
TL;DR: 本文提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合在统一框架中，通过注意力机制动态确定各特征类型的相对重要性，在新闻文本分类任务中优于传统统计模型和纯语义深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖统计特征但忽略上下文语义，现代深度学习方法利用语义特征但可能忽视简单高效统计指标，需要结合两种特征的优势来提升新闻文本分类性能。

Method: 提出AGFF模型，使用基于注意力的机制动态融合统计特征和语义特征，通过注意力权重平衡两种特征类型的贡献。
...
TL;DR: 本文提出了一种名为E³的层剪枝框架，通过可微分掩码优化和熵感知知识蒸馏，解决了大语言模型层剪枝中的性能下降、训练成本高和加速有限等挑战。
...
Method: 提出了E³框架，包含两个关键创新：(1) 使用Gumbel-TopK采样器的可微分掩码优化方法，实现高效精确的剪枝掩码搜索；(2) 熵感知自适应知识蒸馏策略，提升任务性能。

Result: 在多种模型架构和基准测试上的广泛实验表明，该方法优于现有最优方法。在Qwen3-32B模型上剪枝25%层时，在MATH-500上达到96%准确率，仅比原始模型(96.8%)下降0.8%，优于现有SOTA(95%)，推理速度提升1.33倍，仅消耗0.5B tokens(占后训练数据量的0.5%)。

Conclusion: E³框架能够有效平衡任务性能、训练经济性和推理效率，为大语言模型的层剪枝提供了一种实用的解决方案。
...
TL;DR: PARROT框架用于评估大语言模型在权威和说服压力下的准确性退化现象（谄媚行为），通过双盲实验、置信度追踪和八状态行为分类系统，发现先进模型表现稳健而老旧/小型模型出现严重认知崩溃。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在社交压力下的准确性退化问题，特别是谄媚行为（过度顺从），旨在为模型安全部署提供鲁棒性评估标准。

Method: 使用PARROT框架：(i) 双盲评估比较中性问题和权威性错误问题；(ii) 基于对数似然的置信度校准追踪；(iii) 八状态行为分类系统识别失败模式。评估22个模型在13个领域的1,302个MMLU式选择题。

Result: 先进模型（如GPT-5、GPT-4.1）跟随率低（≤11%）且准确性损失小，而老旧/小型模型出现严重认知崩溃（GPT-4: 80%, Qwen 2.5-1.5B: 94%）。国际法和全球知识领域脆弱，初等数学相对稳健。

Conclusion: 抵抗过度拟合压力应作为模型安全部署的主要目标之一，与准确性、危害避免和隐私同等重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有表格问答数据集主要是英文且表格格式完美，与现实场景存在显著差距，需要创建能评估模型在视觉噪声和多语言环境下性能的基准。

Method: 创建包含视觉不完美表格的多语言数据集，模拟扫描文档中的真实噪声，评估主流视觉语言模型在这些挑战下的表现。

Result: 评估显示领先的视觉语言模型存在两个主要失败点：面对视觉噪声时性能严重下降（最佳模型下降超过35%），以及存在一致的英语优先偏见，推理能力无法有效迁移到其他语言。
...
<details>
  <summary>Details</summary>
Motivation: 为了可靠地评估大型语言模型在爱沙尼亚语中的语言能力和推理技能，需要创建高质量的本地化基准测试集，而直接机器翻译可能无法充分捕捉语言的文化和语境特征。

Method: 由翻译专家进行人工翻译和本地化适配，同时设计针对爱沙尼亚语特点和WinoGrande数据集翻译挑战的详细提示，用于机器翻译实验。

Result: 模型在人工翻译的爱沙尼亚语数据集上表现略低于原始英语测试集，在机器翻译数据上表现明显更差；提示工程对翻译质量或模型准确性的改进有限。

Conclusion: 在数据集翻译和适配过程中需要语言专家的参与，以确保对大型语言模型语言能力和推理技能的评估可靠且可解释。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究普通话单音节词在自发对话中的音高轮廓实现方式，特别关注词义对音高轮廓的影响。

Method: 使用广义加性模型将观察到的音高轮廓分解为与不同控制变量和语义预测因子相关的组成音高轮廓，分析词义对音高实现的影响。

Result: 即使控制了词长、性别、说话者身份、声调环境、元音高度和话语位置等变量，词义仍是音高实现的强预测因子。词义比词本身是更好的预测因子，异形同音词具有不同的音高轮廓。

Conclusion: 研究结果表明语义对普通话声调实现有重要影响，挑战了传统的普通话声调理论，但符合判别性词汇模型的理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示将语言在视觉上下文中进行基础化，以构建更鲁棒的自然语言理解系统，避免文本偏见和表面启发式的影响。

Method: 使用文本到图像模型生成前提的视觉表示，通过余弦相似度和视觉问答两种技术进行推理比较。

Result: 该方法在无需任务特定微调的情况下实现了高准确率，并通过对立数据集的验证证明了其鲁棒性。
...
TL;DR: 本文提出了选择性RoPE（Selective RoPE），一种输入依赖的旋转位置编码机制，通过任意角度旋转来改进语言建模和序列任务性能。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控机制的启发，作者希望将输入依赖的位置编码引入到softmax变换器中，以改进语言相关任务的性能。

Method: 提出选择性RoPE机制，通过输入依赖的旋转角度来编码位置信息，适用于线性和softmax变换器，并揭示了softmax注意力中隐藏的旋转结构。

Result: 在语言建模和困难序列任务（如复制、状态跟踪和检索）上，选择性RoPE的输入依赖旋转机制显著提升了性能。

Conclusion: 选择性RoPE通过输入依赖的任意角度旋转，成功改进了位置编码机制，在多种序列任务中表现出优越性能。
...
TL;DR: ReVeL框架将多项选择题重写为开放式问题，以解决MCQA中选项泄露信号导致评估不准确的问题，提升模型真实能力和训练效率。


<details>
  <summary>Details</summary>
Motivation: 多项选择题(MCQA)中的选项可能泄露可利用信号，导致准确率指标不能真实反映模型能力，并鼓励在强化微调(RFT)中出现猜测行为。

Method: 提出ReVeL框架，将多项选择题按答案类型分类，分别应用不同的重写和验证方案，转换为开放式问题，同时保持答案可验证性。

Result: 在20k MCQA示例上应用ReVeL，使用GRPO微调Qwen2.5-VL模型。训练后的模型在多项选择基准上匹配MCQA准确率，在开放式问题上提升约6个百分点，显示出更好的数据效率和更稳健的奖励信号。

Conclusion: ReVeL框架能有效减少MCQA基准中的分数膨胀(相对OpenQA高达20个百分点)，提高判断准确性，同时降低成本和延迟，为模型评估和训练提供了更可靠的方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统RLVR在定理证明等任务中扩展性有限，因为中间推理过程至关重要但最终答案难以直接可靠验证，而token级监督微调容易退化为机械记忆

Method: 两阶段训练：首先在数学计算和证明数据上进行自监督训练（掩码填充和步骤重排序），然后在仅结果可验证的数学计算数据集上进行RLVR微调

Result: 在Qwen2.5-3B和DeepSeek-R1-Distill-Qwen-1.5B上评估，在固定采样和解码预算下，相比原始RLVR平均提升：Pass@1 +9.86%、Pass@5 +5.27%、Pass@8 +4.00%
...
### [122] [MRI Super-Resolution with Deep Learning: A Comprehensive Survey](https://arxiv.org/abs/2511.16854)
...
TL;DR: 这篇论文是关于磁共振成像超分辨率技术的综述，重点介绍了深度学习在MRI超分辨率中的应用，涵盖了理论基础、架构设计、学习策略、数据集和性能指标，并提出了系统分类法来组织这些方法。


<details>
  <summary>Details</summary>
Motivation: 高分辨率MRI在临床和研究中至关重要，但成本高昂且受技术限制。超分辨率技术通过从低成本低分辨率扫描生成高分辨率图像，有望在不增加硬件的情况下提高诊断准确性和效率。

Method: 论文系统回顾了基于深度学习的MRI超分辨率技术，从计算机视觉、计算成像、逆问题和MR物理学的角度分析，提出了系统分类法来组织这些方法。

Result: 论文提供了对现有和新兴MRI超分辨率技术的深入研究，考虑了临床和研究环境中的独特挑战，并强调了社区需要解决的关键问题和未来方向。

Conclusion: 论文总结了MRI超分辨率技术的最新进展，提供了开源资源、工具和教程，旨在促进该领域的发展和应用。
...
### [123] [Avoiding Quality Saturation in UGC Compression Using Denoised References](https://arxiv.org/abs/2511.16876)
*Xin Xiong,Samuel Fernández-Menduiña,Eduardo Pavez,Antonio Ortega,Neil Birkbeck,Balu Adsumilli*

Main category: eess.IV

TL;DR: 本文提出了一种检测用户生成内容(UGC)压缩中质量饱和(QS)问题的高效方法，通过计算相对于去噪UGC的均方误差(D-MSE)来识别失真饱和(DS)，避免了传统需要多次编码解码的低效NRM评估过程。


<details>
  <summary>Details</summary>
Motivation: 传统编解码器在压缩噪声UGC内容时会导致质量饱和问题，即增加码率无法改善视觉质量反而保留输入伪影。现有通过多次评估非参考指标的方法效率低下。

Method: 提出两种失真饱和检测方法：DSD(基于输入相关阈值)和RDSD(使用低复杂度压缩估计饱和点拉格朗日值)，通过计算相对于去噪UGC的D-MSE来识别失真饱和。

Result: 实验表明，在AVC编码中避免在饱和区域编码，相比忽略QS的基线方法，在多个不同NRM上实现了8%-20%的BD-rate节省。

Conclusion: 所提方法可作为预处理步骤帮助标准兼容编解码器避免UGC压缩中的质量饱和问题，显著提高编码效率。
...
### [124] [OmniLens++: Blind Lens Aberration Correction via Large LensLib Pre-Training and Latent PSF Representation](https://arxiv.org/abs/2511.17126)
*Qi Jiang,Xiaolong Qian,Yao Gao,Lei Sun,Kailun Yang,Zhonghua Yi,Wenyong Li,Ming-Hsuan Yang,Luc Van Gool,Kaiwei Wang*

Main category: eess.IV

TL;DR: OmniLens++是一个用于盲镜头像差校正的框架，通过扩展镜头库设计规范增加退化多样性，并引入潜在PSF表示作为光学退化先验指导，解决了现有方法在数据扩展性和先验指导方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有镜头库预训练方法在泛化能力上面临两个挑战：数据扩展困难和缺乏光学退化先验指导。OmniLens++旨在解决这些问题，提升盲像差校正的泛化性能。

Method: 1. 扩展镜头设计规范以增加退化多样性；2. 量化光学退化的空间变化模式和严重程度进行均匀采样；3. 引入潜在PSF表示，使用VQVAE框架学习PSF的潜在特征；4. 通过建模光学退化过程约束退化先验学习。

Result: 在真实世界镜头和合成镜头库的多种像差实验中，OmniLens++在盲像差校正中展现出最先进的泛化能力。AODLibpro被验证为可扩展的基础，LPR能进一步挖掘大规模镜头库的潜力。

Conclusion: OmniLens++通过改进数据可扩展性和引入光学退化先验指导，显著提升了盲镜头像差校正的泛化能力，为大规模镜头库的有效训练提供了新方法。
...
TL;DR: Cognitive BASIC是一种基于BASIC风格的最小化提示语言和模型内解释器，它将大型语言模型的推理结构化为显式的逐步执行轨迹。


<details>
  <summary>Details</summary>
Motivation: 受复古BASIC简洁性的启发，重新利用编号行和简单命令作为可解释的认知控制层，使现代LLM能够可靠地模拟这类短程序，实现模型内部的透明多步推理。

Method: 使用自然语言解释器文件指定命令语义、内存更新和日志行为，通过心理模型解释器提取声明性和程序性知识，检测矛盾并在必要时生成解决方案。

Result: 在三个LLM上对知识提取、冲突检测和推理任务的基准测试表明，所有模型都能执行Cognitive BASIC程序，总体表现强劲但不均匀。

Conclusion: Cognitive BASIC提供了一种结构化LLM推理的有效方法，使模型能够进行透明可控的多步推理过程。
...
TL;DR: 提出了一个系统性的基准测试修订框架，通过分析响应模式统计特征来识别潜在无效问题，结合LLM初步评审减少人工工作量，实现高效可扩展的基准测试质量改进。
...
Method: 基于AI评估中常用的核心假设——平均分足以概括模型性能，构建单维潜在结构测量实验。通过统计响应模式分析，当项目的经验估计统计值超出预期范围时，标记为潜在问题供专家评审。同时引入LLM-judge进行初步评审。

Result: 在9个广泛使用的基准测试中，该方法指导专家评审识别问题问题的精度高达84%。LLM初步评审进一步减少了人工工作量。

Conclusion: 该框架为系统性的基准测试修订提供了高效且可扩展的解决方案，能够显著提升基准测试的可靠性。
...
Conclusion: HDR机制有效解决了多车辆协同驾驶中的奖励信号问题，为高频连续控制任务提供了有效的奖励设计方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI科学方法将科学发现视为孤立的优化过程，忽视了知识生产的社会性和历史性本质，无法有效表示个体认知轨迹和集体学科记忆的结构化背景。

Method: 提出三层分层架构：个体层构建研究者的认知模型，领域层映射集体知识为学科概念图，跨学科层作为正交编排引擎，分离记忆存储与智能体执行。

Result: 在四个综合任务中评估显示，MirrorMind能够超越简单事实检索，实现结构化、个性化和洞察生成的科学推理，促进跨学科合作。

Conclusion: 通过整合个体认知深度和集体学科广度，MirrorMind推动了AI科学家从简单检索向结构化和个性化科学推理的转变。
...
### [130] [Budget-Aware Tool-Use Enables Effective Agent Scaling](https://arxiv.org/abs/2511.17006)
...
TL;DR: 本文研究了在明确工具调用预算下如何有效扩展工具增强智能体，提出了预算跟踪器和BATS框架，使智能体具备预算意识并动态调整策略，提升了成本-性能的帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体在扩展工具调用预算时缺乏预算意识，导致性能很快达到上限，无法充分利用增加的预算资源。
...
Conclusion: 预算意识是有效扩展工具增强智能体的关键，提出的方法为理解智能体在预算约束下的扩展行为提供了实证见解。
...
TL;DR: 本文重新解释了扩散模型在逆问题求解中的作用，提出DAPS++方法，将扩散阶段与数据驱动优化解耦，提高了计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下的基于分数的扩散方法在实践中存在局限性：先验提供有限指导，重建主要由测量一致性项驱动，导致推理过程与扩散动力学脱节。

Method: 将扩散重新解释为期望最大化（EM）框架中的初始化阶段，引入DAPS++方法，使似然项更直接地指导推理，同时保持数值稳定性。

Result: DAPS++需要更少的函数评估次数（NFEs）和测量优化步骤，在多种图像恢复任务中实现了高计算效率和稳健的重建性能。

Conclusion: 通过解耦扩散阶段和数据驱动优化，DAPS++方法为理解统一扩散轨迹在实践中保持有效提供了新见解，并显著提升了逆问题求解的效率。
...
### [132] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出了一种名为MIR的简单而有效的多智能体强化学习增强策略，专门针对稀疏奖励（如情节奖励）场景，通过激励智能体探索影响队友的行为来改善团队探索和算法性能。


<details>
  <summary>Details</summary>
Motivation: 情节奖励在强化学习中是一个重大挑战。虽然内在奖励方法在单智能体强化学习中表现有效，但在多智能体强化学习（MARL）中的应用仍存在问题。主要困难源于两个因素：（1）随着探索空间的扩展，导致奖励的联合行动轨迹呈指数级稀疏；（2）现有方法往往未能考虑可以影响团队状态的联合行动。

Method: 本文引入了相互内在奖励（MIR），这是一种针对具有极稀疏奖励（如情节奖励）的MARL的简单而有效的增强策略。MIR激励个体智能体探索影响其队友的行为，当与原始策略结合时，能有效刺激团队探索并提高算法性能。

Result: 为了全面实验验证，作者将代表性的单智能体MiniGrid环境扩展为MiniGrid-MA，这是一系列具有稀疏奖励的MARL环境。评估结果表明，在MiniGrid-MA设置中，所提出的方法相较于最先进的方法表现出更优越的性能。

Conclusion: MIR策略能够有效解决多智能体强化学习中稀疏奖励的挑战，通过激励智能体间的相互影响行为，显著改善了团队探索效率和算法整体性能。
...
### [133] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了基于分层任务抽象机制(HTAM)的EarthAgent多智能体系统，专门用于复杂地理空间分析，显著优于现有的单智能体和多智能体系统。
...
Method: 引入HTAM框架，将多智能体系统构建为反映领域内在任务依赖图的逻辑层次结构，将复杂问题分解为顺序层，每层子智能体在前一层输出上操作。
...
### [134] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
...
<details>
  <summary>Details</summary>
Motivation: 为实现智能AI系统的持续自主性、推理和交互能力，需要补充明确的认知、合作和治理模型，以完善其关于智能体的假设。

Method: 利用AAMAS社区开发的概念工具，包括BDI架构、通信协议、机制设计和制度建模，将自适应数据驱动方法与结构化推理协调模型进行对齐。

Result: 提出了一个连接形式理论和实践自主性的智能体视角，为实现既具备能力和灵活性，又具有透明性、合作性和问责性的智能系统指明了路径。

Conclusion: AAMAS社区的概念工具为智能AI系统提供了必要的基础，通过整合自适应方法与结构化模型，能够构建更完善的智能系统，弥合形式理论与实际自主性之间的差距。
...
### [135] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了在LLM监控中使用合成数据和离策略数据对探针泛化性能的影响，发现响应生成策略显著影响探针性能，且训练数据域的变化会导致更大的性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多行为的自然示例稀少，研究人员不得不依赖合成或离策略的LLM响应来训练探针，但这种方法对探针泛化能力的影响尚未得到系统评估。

Method: 在八个不同的LLM行为上测试线性和注意力探针，比较不同响应生成策略对探针性能的影响，并评估从离策略数据到策略内数据的泛化能力。

Result: 响应生成策略显著影响探针性能，但影响程度因行为而异；从离策略数据到策略内测试的成功泛化可预测策略内泛化；欺骗和伪装行为探针可能无法从离策略数据泛化到策略内数据。

Conclusion: 在缺乏策略内数据时，使用同域离策略数据比使用不同域的策略内数据产生更可靠的探针，强调需要能够更好处理LLM监控中分布偏移的方法。
...
### [136] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
*Jiaxi Liu,Chengyuan Ma,Hang Zhou,Weizhe Tang,Shixiao Liang,Haoyang Ding,Xiaopeng Li,Bin Ran*

Main category: cs.AI

TL;DR: 本文提出了一种自发的风险感知选择性协同感知框架，通过轻量级感知覆盖摘要广播和仅在检测到风险相关盲区时启动针对性合作，显著降低了通信带宽需求，同时保持了安全关键对象的感知精度。


<details>
  <summary>Details</summary>
Motivation: 现有协同感知方法需要传输大量与驾驶安全无关的感知数据，超出了可用通信带宽，且大多数框架依赖预定义的通信伙伴，不适合动态交通环境。

Method: 提出SRA-CP框架，包含去中心化协议、感知风险识别模块和选择性信息交换融合模块。车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动合作，并根据共享感知覆盖选择合适的通信伙伴。

Result: 在公共数据集上的评估显示，SRA-CP相比通用协同感知方法仅使用20%的通信带宽，安全关键对象的平均精度损失小于1%，相比不包含风险感知的选择性协同感知方法性能提升15%。

Conclusion: SRA-CP框架有效解决了协同感知中的通信带宽和动态环境适应性问题，在保持感知精度的同时显著降低了通信开销。
...
Method: 使用先进自然语言处理技术，包括大语言模型、检索增强生成和结构化思维链提示，构建包含初步评估、多维评估、内容提取、基于量规的评分和详细报告的框架。
